...
tab$add(tab)
err = tab$dump("After adding this to itself")
tab$addCst(12)
err = tab$dump("After adding a constant to this")

# - manipulation of VI
err = itab$dump("Initial Vector of Id")
itab$add(itab)
err = itab$dump("After adding this to itself")
itab$addCst(12)
err = itab$dump("After adding a constant to this")
