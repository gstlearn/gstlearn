...
vec1 = VectorHelper_simulateUniform(db$getNSample())
vec2 = VectorHelper_simulateUniform(db$getNSample())
err = db$addColumns(vec1,"Simu1",ELoc_Z(),0)
err = db$addColumns(vec2,"Simu1",ELoc_Z(),1)
varioparam = VarioParam_createMultipleFromGrid(db,Vario_getNLag()=4)
