...
regular Grid (named **grid**). The variable _nech_ will contain the
number of samples within _grid_. The number of meshes is voluntarily
limited. The mesh is square with dimension 1. The origin (lower left
corner) is set to (10,20) in order to be able to distinguish coordinates
along first and second axes.
...
indices of the locators in use (_x__ and __z_) and the indices of the
...
a 2-D Grid while there is only *one* coordinate field (locator _x_).
In order to avoid any missunderstanding, let us recall this important
fact.
