...
int main(int argc, char* argv[])
{
  ...
  if (flag_grid)
  {
    if (npgs == 1)
    {
      ...
      if (simpgs(
            dbin, dbout, ruleprop, model[0][0], model[0][1], neighU, nbsimu,
            seed, 0, 0, 0, 0, nbtuba, nboot, niter, 1))
        goto label_end;
    }
    else
    {
      ...
      if (simbipgs(
            dbin, dbout, ruleprop, model[0][0], model[0][1], model[1][0],
            model[1][1], neighU, nbsimu, seed, 0, 0, 0, 0, nbtuba, nboot, niter,
            1))
        goto label_end;
    }
    dbfmt.setFlags(true, false, true, true, true);
