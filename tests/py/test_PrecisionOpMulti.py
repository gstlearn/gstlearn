...
def create(nvar=1, multistruct=True, nostatType="Fake", nx1=[4, 4], nx2=[4, 4]):
    ...
    modelMulti = gl.Model.createFromParam(
        gl.ECov.MATERN, param=1, sills=sills1, range=20
    )

    if multistruct:
        modelMulti2 = gl.Model.createFromParam(
            gl.ECov.MATERN, param=2, sills=sills2, range=10
        )
    ...
    if nostat:
        ...
        if nvar == 1:
            if multistruct:
                grid2 = grid.clone()
            rho1 = eps + (grid["x1"] - np.min(grid["x1"]) + eps) / (
                eps + np.max(grid["x1"]) - np.min(grid["x1"])
            )
            rho2 = eps + (grid["x2"] - np.min(grid["x2"]) + eps) / (
                eps + np.max(grid["x2"]) - np.min(grid["x2"])
            )
        ...
        if nvar == 2:
            rho = (eps + grid["x1"] - np.min(grid["x1"])) / (
                eps + np.max(grid["x1"]) - np.min(grid["x1"])
            )
            rho = logit(rho, 20, 10)
...
class PrecisionOpMultiLocal:
    def __init__(self, model, meshes, matrix=False):
        ...
        for i in range(self.ncovar):
            ...
            modelMono = gl.Model.createFromParam(
                cova.getType(), param=cova.getParam(), range=cova.getRangeIso(), sill=1
            )
    ...
    def evalDirect(self, inv):
        ...
        for ii in range(self.ncovar):
            for i in range(self.nvar):
                self.Qop[ii].evalDirect(
                    inv[(s + self.nvertex[ii] * i) : (s + self.nvertex[ii] * (i + 1))],
                    self.temp[ii],
                )
                for j in range(self.nvar):
                    outv[
                        (s + self.nvertex[ii] * j) : (s + self.nvertex[ii] * (j + 1))
                    ] += self.invsill[ii][i, j] * np.array(self.temp[ii].getVector())
        ...
        for ii in range(self.ncovar):
            ...
            for i in range(self.nvar):
                self.Qop[ii].evalSimulate(
                    gl.VectorDouble(gauss[iadx : (iadx + nv)]), self.temp[ii]
                )
                iady = s
                for j in range(0, self.nvar):
                    outv[iady : (iady + nv)] += self.cholsill[ii][j, i] * np.array(
                        self.temp[ii].getVector()
                    )
                    iady += nv
                iadx += nv
            s = iadx
