...
model.addCovFromParam(gl.ECov.NUGGET, sill=2.0)
# Add a Isotropic Spherical (with sill=1.2 and range=5)
model.addCovFromParam(gl.ECov.SPHERICAL, range=5.0, sill=1.2)
# Add an Anisotropic Exponential (with sill=2.3 and ranges=c(4.,7))
model.addCovFromParam(gl.ECov.EXPONENTIAL, sill=2.3, ranges=[4.0, 7.0])
# Add an Anisotropic rotated Cubic (with sill=1.1, ranges=c(3.1,2.1) and angles=(10,0)
model.addCovFromParam(gl.ECov.CUBIC, sill=1.1, ranges=[3.1, 2.1], angles=[10.0, 0.0])
...
model = gl.Model.createFromParam(
    gl.ECov.GAUSSIAN, ranges=[3.0, 1.0], sills=np.array([[2.0, 1.0], [1.0, 4.0]])
)

model.display()

# Creating an isotropic Gaussian Model in 2-D using shortcut
model = gl.Model.createFromParam(gl.ECov.GAUSSIAN, range=5.0)
