...
def check_nrows(db, nrows):
    ...
    if nrows == db.getNSampleActive():
        ...
        if db.getNSampleActive() != db.getNSample():
            raise ValueError(
                f"Error of dimension. Your number of lines ({nrows}) has to be equal to "
...
def findColumnNames(self, columns):
    ...
    if isinstance(columns, str) or is_list_type(
        columns, (str, np.str_)
    ):  # get variable(s) by name
        ...
        if not (len(columns)) == 1:
            raise ValueError(
                "The input for columns should not be a list of several Locators"
            )
...
def setitem(self, name, tab):
    ...
    if isinstance(tab, (float, np.floating, int, np.integer, bool, np.bool_)):
        ...
        nvars = len(
            ColNames
        )  # this means we will only modify existing columns, not create ones
    ...
    if len(ColNames) == nvars:  # modify existing variables only
        ...
        isinstance(columns, (str, np.str_)) and nvars > 1 and len(ColNames) == 0
    ):  # create new variables from a unique name
        new_names = gl.generateMultipleNames(columns, nvars)

    else:
        raise ValueError(
            f"Wrong type or length of input ({columns}): the input should correspond"
    ...
    for i, name in enumerate(new_names):
        ...
        if len(ExistingNames) > 1:
            raise ValueError(
                f"There is more than one variable name corresponding to '{name}' "
...
a["var1"] = x
u = np.sum(
    np.abs(a["var1"][:, 0] - np.array([-1.0856306, 0.99734545, 0.2829785, -1.50629471]))
)
if u > 1e-7:
...
u = np.sum(
    np.abs(
        a["var1"][:, 0] - 2 * np.array([-1.0856306, 0.99734545, 0.2829785, -1.50629471])
    )
)
if u > 1e-7:
...
u1 = np.sum(
    np.abs(
        a["var*"][:, 0] - 2 * np.array([-1.0856306, 0.99734545, 0.2829785, -1.50629471])
    )
)
u2 = np.sum(
    np.abs(
        a["var*"][:, 1] - 3 * np.array([-1.0856306, 0.99734545, 0.2829785, -1.50629471])
    )
)
...
u = np.sum(
    np.abs(a["var*"] - np.array([[0.0, 0.0], [1.0, 1.0], [1.0, 1.0], [0.0, 0.0]]))
)
if not u == 0:
...
try:
    ...
    raise Exception(
        "This should raise an error (number of existing variables"
...
try:
    ...
    raise Exception(
        "This should raise an error (setting an array to a non-existing locator)"
    )
