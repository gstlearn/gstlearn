...
def startingIndex(dat):
    ...
    nvar = dat.getNLoc(gl.ELoc.Z)
    count = np.array(
        [0 for i in range(nvar)]
    )  # std::vector<int> de zéros de taille nvar
    for iech in np.arange(dat.getNSample(True)):  # useSel = True
        count += heterobool(dat, iech)
    starind = 0 * count
    starind[1:] = np.cumsum(count)[
        :-1
    ]  # premier indice à 0 puis deuxième au nombre d'échantillons
...
def GeneralCase(dat, model):
    ...
    if measurementErrorAllEquals(dat):
        update(
            dat, sillMat, 0
        )  ##### ATTENTION, je n'ai pas traité le cas où le premier échantillon
    ...
    for iech in np.arange(dat.getNSample()):
        isdefined = heterobool(
            dat, iech
        )  # privilegier un proto heterobool(const &db, int, &vector<int>)
        ...
        if ~model.getCovAniso(0).isNoStat() and measurementErrorAllEquals(dat):
            ...
            if (
                matrices[hetero] is None
            ):  # verifier si la matrice est de dimension 0 x 0
                matrices[hetero] = np.linalg.inv(
                    sillMat[indexdefined, :][:, indexdefined]
                )
...
def computeInvNoise(dat, model, verbose=True, debug=False):
    ...
    if (
        not model.getCovAniso(0).isNoStat()
        and dat.isAllIsotopic()
        and measurementErrorAllEquals(dat)
    ):
        if verbose:
            print("---------------------------------------------------------")
...
def createDb(
    ...
    if nostat:
        ...
        dat["v12"] = np.abs(np.random.uniform(size=ndat)) * np.sqrt(
            dat["v11"] * dat["v22"]
        )
