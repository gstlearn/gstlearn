...
if nvar == 2:
    ...
    epsNugget = 0.001
    sillsNugg = np.array([[0.001, 0.0], [0.0, 0.001]])

model = gl.Model.createFromParam(gl.ECov.MATERN, param=1, range=20, sills=sills)
# %% Data creation (2 variables)
dat = gl.Db.createFillRandom(
    ndat=100, ndim=2, nvar=0, coormin=[20, 20], coormax=[80, 80], seed=234
)
