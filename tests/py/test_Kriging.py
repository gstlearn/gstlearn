...
def modelReduce(model, var):
    ...
    for i in range(covlist.getNCov()):
        ...
        covar = gl.CovAniso.createAnisotropicMulti(
            ctxt,
            cova.getType(),
            cova.getRanges(),
            sills,
...
def test_kriging_internal(
    ...
    model = gl.Model.createFromParam(
        gl.ECov.EXPONENTIAL, range=2.0, flagRange=False, sills=sills
    )
    np.random.seed(seed)
    ...
    ##################################################
    db, indF = createDbIn(
        ndat, nvar, percent, 2, selDbin, measurement_error, ndrift, flag_isotopic, seed
    )
    ...
    krig = (db["z*"].T.reshape(1, -1).T[indF].T @ weights).reshape(
        -1,
    )

    varest = np.sum(c0t * weights, axis=0).T.reshape(
        -1,
    )

    nt = target.getNSample(useSel=True)
    c0v = np.atleast_2d(np.ones(nt)).T @ np.atleast_2d(
        np.array([modeln.getTotalSill(i, i) for i in range(nvar)])
    )
    c0v = c0v.T.reshape(
        -1,
    )
    var = (
        ...
        + varest
        - 2
        * np.sum(weights * c0, axis=0).T.reshape(
            -1,
        )
    )
    std = np.sqrt(var)
    ...
    if test:
        if compute_vars:
            stdref = target["*stdev"][indOut].T.reshape(
                -1,
            )
            status = gt.checkEqualityVectors(
                stdref, std, tolerance=tol, message=casetxt
            )
            if not status:
            ...
            varestref = target["*varz"][indOut].T.reshape(
                -1,
            )
            status = gt.checkEqualityVectors(
                varestref, varest, tolerance=tol, message=casetxt
            )
            if not status:
            ...
            krigref = target["*estim"][indOut].T.reshape(
                -1,
            )
            status = gt.checkEqualityVectors(
                krigref, krig, tolerance=tol, message=casetxt
            )
            if not status:
                print("Estimation")
                            ...
                            for nvar in [1, 2, 3]:
                                ...
                                for iso in isolist:
                                    for cv in [False, True]:
                                        _, _, _, _, _, good = test_kriging_internal(
                                            ndat,
                                            nx,
                                            nvar,
                                            percent,
