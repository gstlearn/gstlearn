...
# The number of meshes is voluntarily limited.
# The mesh is square with dimension 1.
...
# called *rank*, *x1* and *x2*. Note that the last two fields are considered
...
# a value is returned which corresponds to the number of the newly created
# *attribute*.
# **Important remark: all numerical variables used to identify a field within
# a Db are considered as indices, i.e. they are numbered starting from 0**
...
# contents of the *grid* Db. We check that the field *first* is the fourth
# (i.e. attribute #3).
...
# Note that the newly created fields are automatically named using the provided
# string (*second*) as a radix: the variables names are "second-1", "second-2"
# and "second-3".
...
# We recall that the index numbering starts from 0.
# Therefore field *first-1*  corresponds to the index 5.
...
# We can see that the 7 existing fields currently correspond to the 7 first
# columns of the Data Base *grid*. The second display gives the indices of the
# locators in use (*x* and *z*) and the indices of the attributes corresponding
# to the ranks of the items for each locator type.

# Things become more interesting if a field is deleted. To avoid any ambiguity,
# the field is designated by its name (say *x1*)
...
# (say *x* for coordinates), the locator type is unique and sorted continuously
# starting from 1.
# Therefore, when we suppressed the variable *x1* (which corresponded to the
# locator type *x* and locator rank *1*), the variable *x2* is modified:
# its name and locator type are not changed but the locator rank is update
# from *2* to *1*.
...
# number of positions is still equal to 7. Instead, the rank of the attribute
...
# Obviously, trying to read the field which corresponds to the field *x1*
...
# Note that at any time, the coordinate vectors can be regenerated.
# To avoid confusion, the newly generated coordinate fields are named using
...
# Similarly, we can generate a field containing the sample rank (similar as
# the information contained in the Field #1). Here again, we generate a new
# field containing this rank information: in order to avoid confusion,
# the new variable is called *RANK* (uppercase). Note that this field does
...
#   precisely when using the variable (it must be updated in case of addition
#   or deletion of other variables)
...
# We first run the function which uses the defaulted *NamingConvention* where
