...
int main(int argc, char* argv[])
{
  ...
  Id nfex = 2;
  Id seedin = 13227;
  Db* dbin = Db::createFillRandom(
    ndat, ndim, nvar, nfex, 0, 0., 0., VectorDouble(), VectorDouble(),
    VectorDouble(), seedin);
  ...
  Id nout = 20;
  Id seedout = 134484;
  Db* dbout = Db::createFillRandom(
    nout, ndim, 0, nfex, 0, 0., 0., VectorDouble(), VectorDouble(),
    VectorDouble(), seedout);
  ...
  {
    if (model->evalCovMatRHSInPlaceFromIdx(
          Sigma0, dbin, dbout, sampleRanks, iout, krigopt, false))
      break;
    if (verbose && iout == 0)
    ...
    printVector(
      result, "Sample " + std::to_string(iout + 1), true,
