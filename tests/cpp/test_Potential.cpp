...
static ModelGeneric* st_duplicate_for_potential(const Model* model)
{
  ...
  DriftList* drifts =
    DriftFactory::createDriftListForGradients(model->getDriftList());
...
int main(int argc, char* argv[])
{
  ...
  if (mode == 0 || mode == 1)
  {
    ...
    // Iso-Potential file
    VectorDouble tabiso = {30., 1., 80., 1., 40., 2., 50., 2.};
    Db* dbiso = Db::createFromSamples(
      4, ELoadBy::SAMPLE, tabiso, {"x", "iso"}, {"x1", "layer"});

    // Gradient file
    VectorDouble tabgrd = {0., 1.};
    Db* dbgrd = Db::createFromSamples(
      1, ELoadBy::SAMPLE, tabgrd, {"x", "gx"}, {"x1", "g1"});
    ...
    (void)krigingPotential(
      dbiso, dbgrd, nullptr, grid, new_model, 0., 0., true, true, false, true,
      0, verbose);
    ...
    // Cross-validation
    (void)
      xvalidPotential(dbiso, dbgrd, nullptr, new_model, 0., 0., true, verbose);
  ...
  if (mode == 0 || mode == 2)
  {
    ...
    // Iso-Potential file
    VectorDouble tabiso = {7., 6., 1., 5., 6.,  1., 6., 5., 1., 3.,
                           6., 2., 7., 7., 2.,  8., 3., 2., 8., 1.,
                           3., 7., 9., 3., 10., 5., 3., 3., 1., 3.};
    Db* dbiso = Db::createFromSamples(
      10, ELoadBy::SAMPLE, tabiso, {"x", "y", "iso"}, {"x1", "x2", "layer"});

    // Gradient file
    VectorDouble tabgrd = {1., 6., 1., 0., 9., 2., -1., 1., 7., 8., 0., -1};
    Db* dbgrd = Db::createFromSamples(
      3, ELoadBy::SAMPLE, tabgrd, {"x", "y", "gx", "gy"},
      {"x1", "x2", "g1", "g2"});
    ...
    ModelGeneric* new_model = st_duplicate_for_potential(model);
    (void)krigingPotential(
      dbiso, dbgrd, dbtgt, grid, new_model, 0., 0., true, false, false, false,
      0, true);
