...
int main(int argc, char* argv[])
{
  ...
  message(
    "This test is meant to test Kriging Efficiency using Moving "
    "Neighborhood\n");
  message("- the Output Grid contains %d nodes\n", grid->getNSample(true));
  message(
    "- the Neighborhood is fixed at nmaxi=%d nsect=%d radius=%lf\n", nmaxi,
    nsect, radius);
  ...
  for (Id icas = 0; icas < static_cast<Id>(nechs.size()); icas++)
  {
    ...
    for (Id iloop = 0; iloop < nloop; iloop++)
    {
      ...
      neighM = NeighMoving::create(
        false, nmaxi, radius, nmini, nsect, nsmax, VectorDouble(),
        VectorDouble(), ileaf > 0, ileaf);
  ...
  neighM = NeighMoving::create(
    false, nmaxi, radius, nmini, nsect, nsmax, VectorDouble(), VectorDouble(),
    true, leaf_size);
  kriging(data, grid, model, neighM, true, false);
