...
int main(int argc, char *argv[])
{
  ...
  Db* db = Db::createFillRandom(200, 2, 0);
  Model* model = Model::createFromParam(ECov::EXPONENTIAL, TEST, 1, 1, {0.1, 0.3}, MatrixSymmetric(), {30., 0});
  Model* modelfit = Model::createFromParam(ECov::EXPONENTIAL);

  message("Test fit likelihood\n");
  simtub(nullptr, db, model, nullptr, 3000);
  modelfit->fitLikelihood(db);
  modelfit->display();
