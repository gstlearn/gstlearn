...
int main(int argc, char* argv[])
{
  ...
  if (mode == 0 || mode == 1)
  {
    ...
    Model* model = Model::createFromParam(
      ECov::CUBIC, 0., 1., 1., {10., 45.}, MatrixSymmetric(), {30., 0.});
    ...
    printVector(sel3, "sel1 && sel2", true, true);
  ...
  if (mode == 0 || mode == 2)
  {
    ...
    printVector(
      ...
      "Vector of Relative ranks of active samples", true, true);

    printVector(
      db->getRankRelativeToAbsoluteVec(),
      "Vector of Absolute ranks of active samples", true, true);
    delete db;
  }
