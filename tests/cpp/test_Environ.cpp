...
int main(int argc, char* argv[])
{
  ...
  OptDbg::display();
  message(
    "Current value of Neighborhood Debug = %d\n", OptDbg::query(EDbg::NBGH));
  ...
  message("\n");
  printVector(vect, "Vector Display (initial)", true, true);
  ...
  OptCst::display();
  message("\n");
  printVector(vect, "Vector Display (modified)", true, true);
  ...
  message(
    "Questioning known keyword = %lf\n", OptCustom::query("My first Trial"));
  message(
    "Questioning unknown keyword (with default) = %lf\n",
    OptCustom::query("bidon", 123.));
  message(
    "Questioning unknown keyword (without default) = %lf\n",
