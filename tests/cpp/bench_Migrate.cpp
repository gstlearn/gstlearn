...
int main(int argc, char* argv[])
{
  ...
  if (!dmax.empty()) printVector(dmax, "- using Dmax criterion", true, true);
  message("\n");

  // Migrate Grid -> Point (no interpolation)
  timer.reset();
  (void)migrate(
    grid, data, "Simu", 1, dmax, false, false, false,
    NamingConvention("G2Pn", false, false));
  timer.displayIntervalMilliseconds(
    "Migrate Grid to Point (no interpolation)", 5);

  // Migrate Grid -> Point (with interpolation)
  timer.reset();
  (void)migrate(
    grid, data, "Simu", 1, dmax, false, true, false,
    NamingConvention("G2Py", false, false));
  timer.displayIntervalMilliseconds(
    "Migrate Grid to Point (with interpolation)", 5);

  // Migrate Grid -> Grid
  timer.reset();
  (void)migrate(
    grid, grid, "Simu", 1, dmax, false, false, false,
  ...
  timer.reset();
  (void)migrate(
    grid, grid, "Simu", 1, dmax, true, false, false,
  ...
  timer.reset();
  (void)migrate(
    data, grid, "Simu", 1, dmax, false, false, false,
  ...
  timer.reset();
  (void)migrate(
    data, grid, "Simu", 1, dmax, true, false, false,
  ...
  timer.reset();
  (void)migrate(
    data, grid, "Simu", 1, dmax, true, false, true,
    NamingConvention("P2Gyb", false, false));
  timer.displayIntervalMilliseconds(
    "Expand Point to Grid (with balltree)", 1500);
  ...
  timer.reset();
  (void)migrate(
    data, data, "Simu", 1, dmax, true, false, false,
  ...
  timer.reset();
  (void)migrate(
    data, data, "Simu", 1, dmax, true, false, true,
    NamingConvention("P2Pyb", false, false));
  timer.displayIntervalMilliseconds(
    "Expand Point to Point (with balltree)", 19000);
