...
int main(int argc, char* argv[])
{
  ...
  NeighMoving* neigh2 = NeighMoving::create(false,
                                            nmaxi,
                                            radius,
                                            1,
                                            1,
  ...
  message("Computing time includes Data to Target distance evaluation for "
          "comparison\n");
  ...
  if (mode <= 0 || mode == 1)
  {
    ...
    for (Id i = 0; i < ntarget; i++)
    {
      ...
      if (verbose) printVector(indices1, "", true, true);
    }
  ...
  if (mode <= 0 || mode == 2)
  {
    ...
    for (Id i = 0; i < ntarget; i++)
    {
      ...
      if (verbose) printVector(indices2, String(), true, true);
    }
  ...
  {
    for (Id i = 0; i < ntarget; i++)
      if (!checkDistances1[i].isEqual(checkDistances2[i], 0.01))
      {
        messerr("Vector of indices are different at rank %d", i);
        printVector(checkDistances1[i], "- Standard search", true, true);
        printVector(checkDistances2[i], "- BallTree search", true, true);
      }
  }
