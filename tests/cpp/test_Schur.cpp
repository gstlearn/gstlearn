                       ...
                       MatrixSymmetric& PriorCov)
{
  ...
  print_vector("Prior Mean", PriorMean, true, true);
  message("Prior Variance-Covariance Matrix\n");
  PriorCov.display();
  VectorDouble beta = Kcalc.getPostMean();
  if (!beta.empty()) print_vector("Posterior Mean", beta, true, true);
  message("Posterior Variance-Covariance Matrix\n");
  Kcalc.getPostCov()->display();

  print_vector("Kriging Value(s)", Kcalc.getEstimation(), true, true);
  print_vector("Standard Deviation of Estimation Error", Kcalc.getStdv(), true, true);
  print_vector("Variance of Estimator", Kcalc.getVarianceZstar(), true, true);
...
static void _secondTest(Db* data, Db* target, ModelGeneric* model, const VectorDouble& means)
{
  ...
  print_vector("- Collocated Variable ranks:", varColCok, false, false);
  ...
  print_vector("Kriging Value(s)", KcalcP.getEstimation(), true, true);
  print_vector("Standard Deviation of Estimation Error", KcalcP.getStdv(), true, true);
  print_vector("Variance of Estimator", KcalcP.getVarianceZstar(), true, true);
...
static void _fourthTest(Db* data, Db* target, ModelGeneric* model, const VectorDouble& means)
{
  ...
  print_vector("Kriging Value(s)", Kcalc1.getEstimation(), true, true);
  print_vector("Standard Deviation of Estimation Error", Kcalc1.getStdv(), true, true);
  print_vector("Variance of Estimator", Kcalc1.getVarianceZstar(), true, true);
  ...
  print_vector("Kriging Value(s)", Kcalc2.getEstimation(), true, true);
}
