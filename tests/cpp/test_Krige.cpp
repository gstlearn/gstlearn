...
int main(int argc, char* argv[])
{
  ...
  if (mode == 0 || mode == 4)
  {
    ...
    data_res = data->clone();
    declustering(
      data_res, model, 3, neighM, grid, VectorDouble(), ndiscs, false, true);
  }
  ...
  if (mode == 0 || mode == 5)
  {
    ...
    message(
      "- Number of Kriging System equations (covariance) = %d\n", ktest.CSize);
  ...
  if (mode == 0 || mode == 10)
  {
    ...
    data_res = data->clone();
    declustering(
      data_res, model, 2, neighU, nullptr, VectorDouble(), VectorInt(), false,
      true);
  }
