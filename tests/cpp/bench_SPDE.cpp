...
int main(int argc, char* argv[])
{
  ...
  for (Id ncov = 0; ncov < ncov_tot; ncov++)
  {
    ...
    model = Model::createFromParam(
      ECov::MATERN, TEST, 1, matern_param, {0.1, 0.3}, MatrixSymmetric(),
      {30., 0.});
    if (ncov >= 1)
      model->addCovFromParam(
        ECov::MATERN, TEST, 1, matern_param, {0.3, 0.2}, MatrixSymmetric(),
        {-10., 0.});
    ...
    for (Id ifois = 0; ifois < nfois; ifois++)
    {
      ...
      if (mode < 0 || mode == 1)
      {
        ...
        params.setNMC(10);
        (void)krigingSPDE(
          dat, grid, model, true, true, useCholesky, nullptr, nullptr, nullptr,
          nullptr, nullptr, nullptr, nullptr, params, verbose,
      ...
      if (mode < 0 || mode == 2)
      {
        ...
        law_set_random_seed(seed);
        (void)simulateSPDE(
          nullptr, grid, model, nsim, useCholesky, nullptr, nullptr, nullptr,
      ...
      if (mode < 0 || mode == 3)
      {
        ...
        law_set_random_seed(seed);
        (void)simulateSPDE(
          dat, grid, model, nsim, useCholesky, nullptr, nullptr, nullptr,
          nullptr, nullptr, nullptr, SPDEParam(), verbose,
          NamingConvention(namconv));
