...
#define DREAL double  // #define DREAL float
...
public:
  ...
  typedef DREAL (* GetVertexParamOnEdge)(void*, int, int);
  typedef void (* GetSteinerOnEdge)(void*, int, DREAL, DREAL*);
  typedef void (* GetVertexParamOnFace)(void*, int, int, DREAL*);
  typedef void (* GetEdgeSteinerParamOnFace)(void*, int, DREAL, int, DREAL*);
  typedef void (* GetSteinerOnFace)(void*, int, DREAL*, DREAL*);

  // A callback function for mesh refinement.
  typedef bool (* TetSizeFunc)(DREAL*, DREAL*, DREAL*, DREAL*, DREAL*, DREAL);
  ...
  //   remaining points.  Each point occupies three DREALs. 
  // 'pointattributelist':  An array of point attributes.  Each point's
  //   attributes occupy 'numberofpointattributes' DREALs.
  // 'pointmtrlist': An array of metric tensors at points. Each point's
  //   tensor occupies 'numberofpointmtr' DREALs.
  ...
  //   tetrahedron's attributes occupy 'numberoftetrahedronattributes' DREALs.
  // 'tetrahedronvolumelist':  An array of constraints, i.e. tetrahedron's
  //   volume; one DREAL per element.  Input only.
  ...
  int  *tetrahedronlist;
  DREAL *tetrahedronattributelist;
  DREAL *tetrahedronvolumelist;
  int  *neighborlist;
  int  *tet2facelist;
  int  *tet2edgelist;
  ...
  //   remaining seeds.  Three DREALs per hole. 
  DREAL *holelist;
  int numberofholes;
  ...
  //   straints. Two DREALs per facet constraint.  Note: the facet marker is
  //   actually an integer.
  DREAL *facetconstraintlist;
  ...
  //   by the remaining segment constraints.  Three DREALs per constraint. 
  //   Note the segment endpoints are actually integers.
  DREAL *segmentconstraintlist;
...
public:
  ...
  DREAL brio_ratio;                                             // '-b/' 0.125.
  DREAL facet_separate_ang_tol;                                 // '-p', 179.9.
  DREAL facet_overlap_ang_tol;                                  // '-p/',  0.1.
  DREAL facet_small_ang_tol;                                   // '-p//', 15.0.
  DREAL maxvolume;                                               // '-a', -1.0.
  DREAL minratio;                                                 // '-q', 0.0.
  DREAL mindihedral;                                              // '-q', 5.0.
  DREAL optmaxdihedral;                                               // 165.0.
  DREAL optminsmtdihed;                                               // 179.0.
  DREAL optminslidihed;                                               // 179.0.  
  DREAL epsilon;                                               // '-T', 1.0e-8.
  DREAL coarsen_percent;                                         // -R1/#, 1.0.
...
void  exactinit(int, int, int, DREAL, DREAL, DREAL);
...
DREAL orient4d(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd, DREAL *pe,
               DREAL ah, DREAL bh, DREAL ch, DREAL dh, DREAL eh);
...
///////////////////////////////////////////////////////////////////////////////
  ...
  // The structure of a point is an array of DREALs.  Its acutal size is 
...
///////////////////////////////////////////////////////////////////////////////
  ...
  public:
    triface tt; 
    face ss;
    DREAL key, cent[6];  // circumcenter or cos(dihedral angles) at 6 edges.
...
///////////////////////////////////////////////////////////////////////////////
  ...
  public:
    ...
    DREAL smlen; // for useinsertradius.
    point parentpt;
...
///////////////////////////////////////////////////////////////////////////////
  ...
  public:
    ...
    DREAL bak_tetprism_vol; // The value to be minimized.
    DREAL tetprism_vol_sum;
    int remove_large_angle; // Remove a large dihedral angle at edge.
    DREAL cosdihed_in; // The input cosine of the dihedral angle (> 0).
    DREAL cosdihed_out; // The improved cosine of the dihedral angle.
...
///////////////////////////////////////////////////////////////////////////////
  ...
  int sizeoftensor;                     // Number of DREALs per metric tensor.
  ...
  DREAL cosmaxdihed, cosmindihed;    // The cosine values of max/min dihedral.
  DREAL cossmtdihed;     // The cosine value of a bad dihedral to be smoothed.
  DREAL cosslidihed;      // The cosine value of the max dihedral of a sliver.
  DREAL minfaceang, minfacetdihed;     // The minimum input (dihedral) angles.
  DREAL tetprism_vol_sum;   // The total volume of tetrahedral-prisms (in 4D).
  DREAL longest;                          // The longest possible edge length.
  DREAL minedgelength;                               // = longest * b->epsion.
  DREAL xmax, xmin, ymax, ymin, zmax, zmin;         // Bounding box of points.
...
///////////////////////////////////////////////////////////////////////////////
  ...
  inline DREAL elemattribute(tetrahedron* ptr, int attnum);
  inline void setelemattribute(tetrahedron* ptr, int attnum, DREAL value);
  inline DREAL volumebound(tetrahedron* ptr);
  inline void setvolumebound(tetrahedron* ptr, DREAL value);
  ...
  inline DREAL areabound(face& s);
  inline void setareabound(face& s, DREAL value);
  ...
  inline DREAL pointgeomuv(point pt, int i);
  inline void setpointgeomuv(point pt, int i, DREAL value);
  ...
  inline void setpointinsradius(point pt, DREAL value);
  inline DREAL getpointinsradius(point pt);
...
///////////////////////////////////////////////////////////////////////////////
  ...
  DREAL insphere_s(DREAL*, DREAL*, DREAL*, DREAL*, DREAL*);
  DREAL orient4d_s(DREAL*, DREAL*, DREAL*, DREAL*, DREAL*, 
  ...
  int tri_edge_tail(point, point, point, point, point, point, DREAL, DREAL, int,
                    int*, int*);
  int tri_edge_test(point, point, point, point, point, point, int, int*, int*);

  // Triangle-triangle intersection test (robust)
  int tri_edge_inter_tail(point, point, point, point, point, DREAL, DREAL);
  int tri_tri_inter(point, point, point, point, point, point);

  // Linear algebra functions
  inline DREAL dot(DREAL* v1, DREAL* v2);
  inline void cross(DREAL* v1, DREAL* v2, DREAL* n);
  bool lu_decmp(DREAL lu[4][4], int n, int* ps, DREAL* d, int N);
  void lu_solve(DREAL lu[4][4], int n, int* ps, DREAL* b, int N);

  // An embedded 2-dimensional geometric predicate (non-robust)
  DREAL incircle3d(point pa, point pb, point pc, point pd);

  // Geometric calculations (non-robust)
  DREAL orient3dfast(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd);
  inline DREAL norm2(DREAL x, DREAL y, DREAL z);
  inline DREAL distance(DREAL* p1, DREAL* p2);
  void facenormal(point pa, point pb, point pc, DREAL *n, int pivot, DREAL *lav);
  DREAL shortdistance(DREAL* p, DREAL* e1, DREAL* e2);
  DREAL triarea(DREAL* pa, DREAL* pb, DREAL* pc);
  DREAL interiorangle(DREAL* o, DREAL* p1, DREAL* p2, DREAL* n);
  void projpt2edge(DREAL* p, DREAL* e1, DREAL* e2, DREAL* prj);
  void projpt2face(DREAL* p, DREAL* f1, DREAL* f2, DREAL* f3, DREAL* prj);
  bool tetalldihedral(point, point, point, point, DREAL*, DREAL*, DREAL*);
  void tetallnormal(point, point, point, point, DREAL N[4][3], DREAL* volume);
  DREAL tetaspectratio(point, point, point, point);
  bool circumsphere(DREAL*, DREAL*, DREAL*, DREAL*, DREAL* cent, DREAL* radius);
  bool orthosphere(DREAL*,DREAL*,DREAL*,DREAL*,DREAL,DREAL,DREAL,DREAL,DREAL*,DREAL*);
  void planelineint(DREAL*, DREAL*, DREAL*, DREAL*, DREAL*, DREAL*, DREAL*);
  int linelineint(DREAL*, DREAL*, DREAL*, DREAL*, DREAL*, DREAL*, DREAL*, DREAL*);
  DREAL tetprismvol(DREAL* pa, DREAL* pb, DREAL* pc, DREAL* pd);
  bool calculateabovepoint(arraypool*, point*, point*, point*);
  void calculateabovepoint4(point, point, point, point);

  // PLC error reports.
  void report_overlapping_facets(face*, face*, DREAL dihedang = 0.0);
...
///////////////////////////////////////////////////////////////////////////////
  ...
  int  hilbert_split(point* vertexarray, int arraysize, int gc0, int gc1,
                     DREAL, DREAL, DREAL, DREAL, DREAL, DREAL);
  void hilbert_sort3(point* vertexarray, int arraysize, int e, int d,
                     DREAL, DREAL, DREAL, DREAL, DREAL, DREAL, int depth);
  void brio_multiscale_sort(point*,int,int threshold,DREAL ratio,int* depth);
...
///////////////////////////////////////////////////////////////////////////////
  ...
  void scarveholes(int, DREAL*);
  int triangulate(int, arraypool*, arraypool*, int, DREAL*);
  ...
  void interecursive(shellface** subfacearray, int arraysize, int axis,
                     DREAL, DREAL, DREAL, DREAL, DREAL, DREAL, int* internum);
...
///////////////////////////////////////////////////////////////////////////////
  ...
  DREAL getpointmeshsize(point, triface*, int iloc);
...
///////////////////////////////////////////////////////////////////////////////
  ...
  void save_segmentpoint_insradius(point segpt, point parentpt, DREAL r);
  void save_facetpoint_insradius(point facpt, point parentpt, DREAL r);
  ...
  int splitsegment(face *splitseg, point encpt, DREAL, point, point, int, int);
  void repairencsegs(int chkencflag);

  int checkfac4encroach(point, point, point, point checkpt, DREAL*, DREAL*);
  int checkfac4split(face *chkfac, point& encpt, int& qflag, DREAL *ccent);
  int splitsubface(face *splitfac, point, point, int qflag, DREAL *ccent, int);
  void repairencfacs(int chkencflag);

  int checktet4split(triface *chktet, int& qflag, DREAL *ccent);
  int splittetrahedron(triface* splittet,int qflag,DREAL *ccent, int);
...
///////////////////////////////////////////////////////////////////////////////
  ...
  int  splitsliver(triface *, DREAL, int);
...
inline void terminatetetgen(tetgenmesh *m, int x)
{
  ...
  case 2: // Encounter an internal error.
    ...
    message("  the message above, your input data set, and the exact\n");
    message("  command line you used to run this program, thank you.\n");
    break;
  case 3:
    message("A self-intersection was detected. Program stopped.\n");
    message("Hint: use -d option to detect all self-intersections.\n"); 
    break;
  case 4:
    message("A very small input feature size was detected. Program stopped.\n");
    if (m) {
      message("Hint: use -T option to set a smaller tolerance. Current is %g\n",
             m->b->epsilon);
    }
    break;
  case 5:
    message("Two very close input facets were detected. Program stopped.\n");
    message("Hint: use -Y option to avoid adding Steiner points in boundary.\n");
    break;
  case 10: 
...
inline DREAL tetgenmesh::elemattribute(tetrahedron* ptr, int attnum) {
  return ((DREAL *) (ptr))[elemattribindex + attnum];
}

inline void tetgenmesh::setelemattribute(tetrahedron* ptr, int attnum, 
  DREAL value) {
  ((DREAL *) (ptr))[elemattribindex + attnum] = value;
}

// Check or set a tetrahedron's maximum volume bound.

inline DREAL tetgenmesh::volumebound(tetrahedron* ptr) {
  return ((DREAL *) (ptr))[volumeboundindex];
}

inline void tetgenmesh::setvolumebound(tetrahedron* ptr, DREAL value) {
  ((DREAL *) (ptr))[volumeboundindex] = value;
}
...
inline DREAL tetgenmesh::areabound(face& s) 
{
  return ((DREAL *) (s.sh))[areaboundindex];
}

inline void tetgenmesh::setareabound(face& s, DREAL value) 
{
  ((DREAL *) (s.sh))[areaboundindex] = value;
}
...
inline DREAL tetgenmesh::pointgeomuv(point pt, int i) {
  return pt[pointparamindex + i];
}

inline void tetgenmesh::setpointgeomuv(point pt, int i, DREAL value) {
...
inline void tetgenmesh::setpointinsradius(point pt, DREAL value)
{
  pt[pointinsradiusindex] = value;
}

inline DREAL tetgenmesh::getpointinsradius(point pt)
{
...
inline DREAL tetgenmesh::dot(DREAL* v1, DREAL* v2) 
{
  return v1[0] * v2[0] + v1[1] * v2[1] + v1[2] * v2[2];
}

// cross() computes the cross product: n = v1 cross v2.
inline void tetgenmesh::cross(DREAL* v1, DREAL* v2, DREAL* n) 
{
...
inline DREAL tetgenmesh::distance(DREAL* p1, DREAL* p2)
{
...
inline DREAL tetgenmesh::norm2(DREAL x, DREAL y, DREAL z)
{
  return (x) * (x) + (y) * (y) + (z) * (z);
}
