...
namespace gstlrn
{
  ...
  public:
    ...
    void getCoordinatesPerMeshInPlace(Id imesh, Id rank, VectorDouble& coords)
      const override;
    ...
    static MeshETurbo* createFromGrid(
      const DbGrid* dbgrid,
      bool flag_polarized = false,
      bool verbose = false,
      Id mode = 1);
    static MeshETurbo* createFromGridInfo(
      const Grid* grid,
      bool flag_polarized = false,
      bool verbose = false,
      Id mode = 1);
    static MeshETurbo* createFromCova(
      const CovAniso& cova,
      const Db* field,
      double ratio,
      Id nbExt = 0,
    ...
    VectorInt
      getMeshApicesFromCoordinates(const VectorDouble& coords) const override;

    /*! Returns the Mesh from coordinates */
    Id getMeshFromCoordinates(const VectorDouble& coords) const override;
    Id getMeshAndInPlaceWeightsFromCoordinates(
      const VectorDouble& coords,
      VectorDouble& weights) const override;

    Id getMeshFromCoordinates(
  ...
  private:
    ...
    bool _addElementToTriplet(
      NF_Triplet& NF_T,
      Id iech,
      const VectorDouble& coor,
      const VectorInt& indg0,
      bool verbose) const;
    Id _initFromGridInternal(
