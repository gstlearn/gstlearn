...
namespace gstlrn
{
  ...
  public:
    ...
    double
      getCoordinate(Id iech, Id idim, bool flag_rotate = true) const override;
    void getCoordinatesInPlace(
      VectorDouble& coor,
      Id iech,
    ...
    static DbGrid* createCoveringDb(
      const Db* dbin,
      const VectorInt& nx = VectorInt(),
      const VectorDouble& dx = VectorDouble(),
      const VectorDouble& x0 = VectorDouble(),
      const VectorDouble& margin = VectorDouble());
    static DbGrid* createFromPolygon(
      Polygons* polygon,
      const VectorInt& nx = VectorInt(),
      const VectorDouble& dx = VectorDouble(),
      bool flagAddSelection = true,
      bool flagAddSampleRank = true);
    static DbGrid* createCoarse(
      DbGrid* dbin,
      const VectorInt& nmult,
      bool flagCell = true,
      bool flagAddSampleRank = true);
    static DbGrid* createRefine(
      DbGrid* dbin,
      const VectorInt& nmult,
      bool flagCell = true,
      bool flagAddSampleRank = true,
      bool flagInter = true);
    static DbGrid* createFromGridExtend(
      const DbGrid& gridIn,
      ...
      double eps = EPSILON3);
    static DbGrid*
      createFromGridShrink(const DbGrid& gridIn, const VectorInt& deletedRanks);
    static DbGrid* createGrid2D(
      const ELoadBy& order,
      Id nx,
      Id ny,
      double x0 = 0.,
      ...
      bool flagAddSampleRank = true,
      const VectorDouble& tab = VectorDouble());
    static DbGrid* createSqueezeAndStretchForward(
      ...
      Id nzout,
      double thickmin = 0.);
    static DbGrid* createSqueezeAndStretchBackward(
      const DbGrid* grid3Din,
      const DbGrid* surf2D,
      const String& nameTop,
      const String& nameBot,
      ...
      double z0out,
      double dzout);
    static DbGrid* createSubGrid(
      const DbGrid* gridin,
      VectorVectorInt limits,
      bool flagAddCoordinates = false);
    static DbGrid* createMultiple(
      DbGrid* dbin,
      const VectorInt& nmult,
      bool flagAddSampleRank);
    static DbGrid* createDivider(
      DbGrid* dbin,
      const VectorInt& nmult,
      bool flagAddSampleRank);
    static DbGrid* createFillRandom(
      const VectorInt& nx,
      Id nvar = 1,
      Id nfex = 0,
      Id ncode = 0,
    ...
    void getCoordinateFromCornerInPlace(
      VectorDouble& coor,
    ...
    VectorDouble getCoordinatesByIndice(
    ...
    {
      return _grid.getCoordinatesByIndice(
        indice, flag_rotate, shift, dxsPerCell);
    }

    void getCoordinatesByIndiceInPlace(
      VectorDouble& coor,
      const VectorInt& indice,
      bool flag_rotate = true,
      const VectorInt& shift = VectorInt(),
      const VectorDouble& dxsPerCell = VectorDouble()) const
    {
      _grid.getCoordinatesByIndiceInPlace(
        coor, indice, flag_rotate, shift, dxsPerCell);
    }
    ...
    void getCoordinatesPerSampleInPlace(
      VectorDouble& coor,
      Id iech,
    ...
    VectorInt coordinateToIndices(
      const VectorDouble& coor,
      bool centered = false,
      double eps = EPSILON6) const;
    Id coordinateToIndicesInPlace(
      const VectorDouble& coor,
      VectorInt& indices,
      bool centered = false,
      double eps = EPSILON6) const;
    ...
    VectorDouble rankToCoordinates(
      Id rank,
      const VectorDouble& percent = VectorDouble()) const
    {
    ...
    void indicesToCoordinateInPlace(
      const VectorInt& indice,
      VectorDouble& coor,
      const VectorDouble& percent = VectorDouble()) const
    {
    ...
    VectorDouble indicesToCoordinate(
      const VectorInt& indice,
      const VectorDouble& percent = VectorDouble()) const
    {
    ...
    Id centerCoordinateInPlace(
      VectorDouble& coor,
      bool centered = false,
      bool stopIfOut = false,
      double eps = EPSILON6) const;

    VectorInt locateDataInGrid(
      const Db* data,
    ...
    VectorVectorInt getLimitsFromVariableExtend(
      const String& nameTop,
      const String& nameBot,
      const VectorInt& dimExclude = VectorInt()) const;
    Id setSelectionFromVariableExtend(
      const String& nameTop,
      const String& nameBot);
    void clean3DFromSurfaces(
      const VectorString& names,
      const DbGrid* surf2D,
    ...
    VectorVectorDouble getDiscretizedBlock(
    ...
    void getGridPileInPlace(
      Id iuid,
      const VectorInt& indg,
      Id idim0,
      VectorDouble& vec) const;
    void setGridPileInPlace(
      Id iuid,
      const VectorInt& indg,
      Id idim0,
      const VectorDouble& vec);
    VectorDouble getDistanceToOrigin(
