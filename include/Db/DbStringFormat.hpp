...
namespace gstlrn
{
  ...
  public:
    DbStringFormat(
      unsigned char params = FLAG_RESUME | FLAG_VARS,
    ...
    static DbStringFormat* create(
      unsigned char params = FLAG_RESUME | FLAG_VARS,
      const VectorString& names = VectorString(),
      const VectorInt& cols = VectorInt(),
      bool useSel = true);

    static DbStringFormat* createFromKeys(
      unsigned char params,
      const VectorString& names = VectorString(),
      const VectorInt& cols = VectorInt(),
      bool useSel = true);

    static DbStringFormat* createFromFlags(
      bool flag_resume = true,
      bool flag_vars = true,
