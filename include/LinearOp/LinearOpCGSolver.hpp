...
LinearOpCGSolver<TLinOP, Precond>::LinearOpCGSolver(const TLinOP* linop) : ALinearOpCGSolver()
{
  if (linop == nullptr)
...
template<typename TLinOP, typename Precond>
void LinearOpCGSolver<TLinOP, Precond>::solve(const VectorDouble& rhs, VectorDouble& out)
{
  ...
  ::Eigen::Map<Eigen::VectorXd>& out)
{
  ...
  if (_verbose)
  {
    ...
    messageFlush("Number of iterations: " + std::to_string(_cg.iterations()) + "/" + std::to_string(_cg.maxIterations()) + "\n");
    messageFlush("Error: " + std::to_string(_cg.error()) + "\n");
  }
}

template<typename TLinOP, typename Precond>
void LinearOpCGSolver<TLinOP, Precond>::solveWithGuess(const Eigen::Map<const Eigen::VectorXd>& rhs,
                                              const Eigen::Map<const Eigen::VectorXd>& guess,
                                              Eigen::Map<Eigen::VectorXd>& out)
{
...
template<typename TLinOP, typename Precond>
void LinearOpCGSolver<TLinOP, Precond>::solve(const constvect in, const vect out)
{
