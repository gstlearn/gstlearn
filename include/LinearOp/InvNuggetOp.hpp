...
namespace gstlrn
{
   ...
   */
  class GSTLEARN_EXPORT InvNuggetOp: public virtual ASimulable,
                                     public virtual MatrixSparse
  {
  public:
    ...
    {
      return dynamic_cast<const MatrixSparse*>(this);
    }
  ...
  private:
    ...
    void _buildInvNugget(const Db* dbin = nullptr,
                         Model* model = nullptr,
                         const SPDEParam& params = SPDEParam());

  private:
    std::shared_ptr<MatrixSparse>
      _cholNuggetMatrix; // The Cholesky decomposition of the nugget matrix
    double _logDeterminant; // The log determinant of the inverse nugget matrix
    std::pair<double, double>
      _rangeEigenVal; // The range of eigenvalues for the inverse nugget matrix
    bool _flagEigVals; // Flag to indicate if eigenvalues should be computed
  };

#ifndef SWIG
  // Function to build the inverse nugget matrix
  GSTLEARN_EXPORT std::shared_ptr<MatrixSparse>
    buildInvNugget(Db* dbin,
                   Model* model,
                   const SPDEParam& params = SPDEParam());
#endif
} // namespace gstlrn
