...
#define DECLARE_EIGEN_TRAITS(TLinOP)                                           \
  ...
  {                                                                            \
    ...
    {                                                                          \
      template<>                                                               \
      struct traits<gstlrn::TLinOP>                                            \
        : public Eigen::internal::traits<                                      \
            Eigen::SparseMatrix<double, 0, gstlrn::Id>>                        \
      {                                                                        \
      };                                                                       \
    }                                                                          \
  }
...
#define DECLARE_EIGEN_PRODUCT(TLinOP)                                          \
  ...
  struct Eigen::internal::generic_product_impl<                                \
    gstlrn::TLinOP, Rhs, Eigen::SparseShape, Eigen::DenseShape,                \
    Eigen::GemvProduct>                                                        \
    : Eigen::internal::generic_product_impl_base<                              \
        gstlrn::TLinOP, Rhs,                                                   \
        Eigen::internal::generic_product_impl<gstlrn::TLinOP, Rhs>>            \
  {                                                                            \
    typedef typename Product<gstlrn::TLinOP, Rhs>::Scalar Scalar;              \
    template<typename Dest>                                                    \
    static void scaleAndAddTo(                                                 \
      Dest& dst,                                                               \
      const gstlrn::TLinOP& lhs,                                               \
      const Rhs& rhs,                                                          \
      const Scalar& alpha)                                                     \
    {                                                                          \
...
namespace gstlrn
{
  ...
  class ALinearOpEigenCG
    : public Eigen::EigenBase<TLinOP>, // No Export because it's a template
  ...
  public:
    ...
    template<typename Rhs>
    Eigen::Product<TLinOP, Rhs, Eigen::AliasFreeProduct>
      operator*(const Eigen::MatrixBase<Rhs>& x) const
    {
      return Eigen::Product<TLinOP, Rhs, Eigen::AliasFreeProduct>(
        *(dynamic_cast<const TLinOP*>(this)), x.derived());
    }
#endif
  };
} // namespace gstlrn
