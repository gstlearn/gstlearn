...
namespace gstlrn
{
  ...
  public:
    ShiftOpMatrix();
    ShiftOpMatrix(const AMesh* amesh,
                  const CovAniso* cova,
                  const Db* dbout = nullptr,
                  bool verbose = false);
    ShiftOpMatrix(const MatrixSparse* S,
    ...
    static ShiftOpMatrix* create(const AMesh* amesh,
                                 const CovAniso* cova,
                                 const Db* dbout = nullptr,
                                 bool verbose = false);
    static ShiftOpMatrix* createFromSparse(const MatrixSparse* S,
                                           ...
                                           const CovAniso* cova,
                                           bool verbose = false);
    Id initFromMesh(const AMesh* amesh,
                    const CovAniso* cova,
                    const Db* dbout = nullptr,
                    bool flagAdvection = false,
                    bool verbose = false);
    Id initGradFromMesh(const AMesh* amesh,
                        const CovAniso* cova,
                        bool verbose = false,
                        double tol = EPSILON10);
    Id initFromCS(const MatrixSparse* S,
                  const VectorDouble& TildeC,
                  const VectorDouble& Lambda,
                  const CovAniso* cova,
                  bool verbose = false);
    ...
    Id getNCovAnisoGradParam() const { return _nCovAnisoGradParam; }
    ...
    const VectorDouble& getLambdaGrads(Id idim) const
    {
  ...
  private:
    ...
    static void
      _loadAux(VectorDouble& tab, const EConsElem& type, Id imesh = 0);
    ...
    void _loadHHGrad(const AMesh* amesh,
                     ...
                     Id igparam,
                     Id ipref);
    double _computeGradLogDetHH(const AMesh* amesh,
                                Id igparam,
                                Id ipref,
    ...
    static void _projectMesh(const AMesh* amesh,
                             ...
                             Id imesh,
                             double coeff[3][2]);
    Id _preparMatrices(const AMesh* amesh,
                       Id imesh,
                       MatrixSquare& matu,
    ...
    MatrixSparse*
      _BuildSGradfromMap(std::map<std::pair<Id, Id>, double>& tab) const;
