...
public:
  ...
  MultiGridSolver(MultiGridSolver&& other)   noexcept  ;
  MultiGridSolver& operator=(const MultiGridSolver& other) = delete;
  MultiGridSolver& operator=(MultiGridSolver&& other) noexcept;
  ...
  const ALinearOp& getOperator(size_t lvl) const { return *_operators[lvl].get(); }
  const std::vector<std::unique_ptr<const ALinearOp>>& getOperators() const { return _operators; }
  ...
  VectorDouble vCycle(const VectorDouble& rhs, const VectorDouble& u, size_t lvl) const;
private:
