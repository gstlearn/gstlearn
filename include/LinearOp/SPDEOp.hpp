...
namespace gstlrn
{
  ...
  public:
    ASPDEOp(
      const IPrecisionOp* const popKriging = nullptr,
    ...
    VectorDouble
      kriging(const VectorDouble& dat, const IProj* proj = nullptr) const;
    VectorDouble krigingWithGuess(
      const VectorDouble& dat,
      const VectorDouble& guess) const;

    VectorDouble computeDriftCoeffs(
      const VectorDouble& Z,
      const MatrixDense& driftMat,
      bool verbose = false) const;
    VectorDouble simCond(
      const VectorDouble& dat,
      const IProj* projK = nullptr,
      const IProj* projS = nullptr) const;
  ...
  public:
    Id krigingWithGuess(const constvect inv, const constvect guess, vect out)
      const;
    ...
    static Id centerDataByDriftMat(
      VectorDouble& Z,
      const MatrixDense& driftMat,
  ...
  private:
    ...
    Id _solveWithGuess(const constvect in, const constvect guess, vect out)
      const;
  ...
  public:
          ...
          precond)
    {
      ...
      if (a != nullptr)
      {
        ...
        auto* b =
          dynamic_cast<LinearOpCGSolver<SPDEOp, MultiGridSolver>*>(_solver);
