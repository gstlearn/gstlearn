...
namespace gstlrn
{
  ...
  public:
    ProjConvolution(
      const VectorDouble& convolution = VectorDouble(),
  ...
  private:
    Id _getConvSize() const { return static_cast<Id>(_convolution.size()); }
  ...
  private:
    ...
    bool _isVecDimCorrect(
      const constvect valonseismic,
      const constvect valonvertex) const;

  protected:
    Id _addPoint2mesh(const constvect valonseismic, vect valonvertex)
      const override;
    Id _addMesh2point(const constvect valonvertex, vect valonseismic)
      const override;
#endif
