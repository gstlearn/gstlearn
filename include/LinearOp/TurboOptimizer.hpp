...
namespace gstlrn
{
  ...
  public:
    ...
    VectorInt
      interpolate_rows(const VectorDouble& x, const VectorDouble& y) const
    {
      return interpolate(x, y).rows;
    }

    VectorInt
      interpolate_cols(const VectorDouble& x, const VectorDouble& y) const
    {
      return interpolate(x, y).cols;
    }

    VectorDouble
      interpolate_values(const VectorDouble& x, const VectorDouble& y) const
    {
      return interpolate(x, y).values;
    }

    VectorInt getQ_rows() const { return getQ().rows; }

    VectorInt getQ_cols() const { return getQ().cols; }

    VectorDouble getQ_values() const { return getQ().values; }
    ...
    Id getHalf() const { return _half; }

    Id getCenter() const { return _center; }

    Id getNxred() const { return _nxred; }

    Id getPoncif() const { return _poncif; }

    const VectorDouble& getTildeCT() const { return _TildeC_T; }

    const VectorDouble& getLambdaT() const { return _Lambda_T; }

    const VectorDouble& getQT() const { return _Q_T; }

    const VectorDouble& getST() const { return _S_T; }

  private:
    Id _getNMeshes() const { return ((_nx - 1) * (_ny - 1) * TO_npercell); }

    Id _getNVertices() const { return _nx * _ny; }

    Id _getNVertices_red() const { return _nxred * _nxred; }

    double _getMeshSize() const { return (_dx * _dy / TO_npercell); }
    ...
    static void _printVector(
      const std::string& title,
      VectorDouble& uu,
      I32 width = 10,
      I32 ndec = 3);
    static void _printMatrix(
      const std::string& title,
      Id nrow,
      Id ncol,
      VectorDouble& uu,
      ...
      I32 ndec = 6);
    static void
      _invert_3x3(VectorDouble& uu, VectorDouble& vv, double tol = 1.e-6);
    static void _prodMatrix(
      Id size,
      const VectorDouble& aa,
      const VectorDouble& bb,
      VectorDouble& cc);
    static void _prodMatrixVector(
      Id size,
      const VectorDouble& aa,
      const VectorDouble& bb,
      VectorDouble& cc);
    ...
    TripletND
      _getMatrixFromTemplate(const VectorDouble& matin, Id nperline) const;
    Id _addWeights(
      Id icas,
      double x,
      double y,
    ...
    VectorDouble _expandTripletToMatrix(
      Id row_begin,
      Id row_end,
      Id col_begin,
      Id col_end,
