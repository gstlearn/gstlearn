...
namespace gstlrn
{
  ...
  public:
    ...
    virtual void
      prodLambda(const VectorDouble& x, VectorDouble& y, const EPowerPT& power)
        const;
    virtual ~AShiftOp();
    ...
    static std::shared_ptr<CovAniso>
      cloneAndCast(const std::shared_ptr<CovAniso>& cova);

    Id getSize() const override { return _napices; }

#ifndef SWIG
    virtual void
      addProdLambda(const constvect x, vect y, const EPowerPT& power) const;
    ...
    void prodLambda(const constvect x, VectorDouble& y, const EPowerPT& power)
      const;
#endif
#ifndef SWIG
