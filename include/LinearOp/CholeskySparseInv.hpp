...
namespace gstlrn
{
  ...
  public:
    MatchPattern(const SpMat& A, const SpMat& pattern)
    {
      ...
      std::copy(
        pattern.outerIndexPtr(), pattern.outerIndexPtr() + m_cols + 1,
        m_outer.begin());
      m_inner.resize(m_nnz);
      std::copy(
        pattern.innerIndexPtr(), pattern.innerIndexPtr() + m_nnz,
        m_inner.begin());
      m_val.resize(m_nnz);
    ...
    SpMat operator()()
    {
      return Eigen::Map<SpMat>(
        m_cols, m_cols, m_nnz, m_outer.data(), m_inner.data(), m_val.data());
    }
  };

  template<typename SpChol, typename SpMat>
  typename SpChol::MatrixType
    partial_inverse(const SpChol& llt, [[maybe_unused]] const SpMat& pattern)
  {
