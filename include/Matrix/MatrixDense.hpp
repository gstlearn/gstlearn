...
namespace gstlrn
{
  ...
  public:
    ...
    static void
      _addGeneral(MatrixDense& res, const MatrixDense& other, double cst);
    static void _addGeneral(
      MatrixDense& res,
      const MatrixDense& other1,
      const MatrixDense& other2);
    static void
      _prodGeneral(MatrixDense& res, const MatrixDense& other, double cst);
    ...
    void prodMatMatNoCheck(const MatrixDense& x, const MatrixDense& y)
    {
      ...
      {
        eigenMat().noalias() =
          x.eigenMat().transpose() * y.eigenMat().transpose();
      }
    ...
    static double traceProd(
      const MatrixDense& a,
      MatrixDense& b); // Warning: b is modified to reduce memory allocation
    static MatrixDense* createFromVVD(const VectorVectorDouble& X);
    static MatrixDense* createFromVD(
...
#ifndef SWIG
    static void
      sum(const MatrixDense* mat1, const MatrixDense* mat2, MatrixDense* mat3);
#endif
    double sum() const;
  ...
  public:
    ...
    {
      return Eigen::Map<const Eigen::MatrixXd, Eigen::Unaligned>(
        _eigenMatrix.data(), getNRows(), getNCols());
    }

    Eigen::Map<Eigen::MatrixXd, Eigen::Unaligned> eigenMat()
    {
      return Eigen::Map<Eigen::MatrixXd, Eigen::Unaligned>(
        _eigenMatrix.data(), getNRows(), getNCols());
    }
#endif
