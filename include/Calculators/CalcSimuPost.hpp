...
namespace gstlrn
{
  ...
  protected:
    ...
    virtual void
      _transformFunction(const VectorDouble& tabin, VectorDouble& tabout) const
    {
      DECLARE_UNUSED(tabin, tabout);
    }
  ...
  private:
    ...
    VectorInt
      _samplesInCellIdenticalSpaceDimension(const VectorInt& indblock) const;
    VectorInt _samplesInCellDifferentSpaceDimension() const;
    void _upscaleFunction(const VectorVectorDouble& Y_p_k_s,
                          VectorDouble& tabout) const;
    void _readIn(Id iech, const VectorInt& indices, VectorDouble& tabin) const;
    void _statisticsFunction(const VectorVectorDouble& Y_p,
                             VectorDouble& tabout) const;
