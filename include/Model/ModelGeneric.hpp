...
namespace gstlrn
{
  ...
  public:
    ...
    void addDrift(
      const ADrift*
        drift); // TODO: check that the same driftM has not been already defined
  ...
  protected: // TODO : pass into private to finish clean
    ...
    std::shared_ptr<ATransform>
      _transform; /* Transformation associated to the Model */
  };

  GSTLEARN_EXPORT Id computeCovMatSVCLHSInPlace(
    MatrixSymmetric& cov,
    const MatrixSymmetric& Sigma,
    const MatrixDense& F1,
    Id type = 1,
    Id idx = 0);
  GSTLEARN_EXPORT Id computeCovMatSVCRHSInPlace(
    MatrixDense& cov,
    const MatrixSymmetric& Sigma,
    const MatrixDense& F1,
    ...
    Id type2 = 1,
    Id idx2 = 0);
  GSTLEARN_EXPORT Id computeDriftMatSVCRHSInPlace(
    MatrixDense& mat,
    const MatrixDense& F,
    Id type = 1,
    Id idx = 0,
