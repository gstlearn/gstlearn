...
namespace gstlrn
{
  ...
  public:
    ...
    {
      return static_cast<const CovList*>(getCov());
    }

    CovList* getCovListModify()
    {
    ...
    const CovBase* getCovBase(Id icov) const
    {
      return getCovList()->getCov(icov);
    }

    CovBase* getCovBase(Id icov)
    {
      return getCovListModify()->getCovModify(icov);
    }
