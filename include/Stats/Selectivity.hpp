...
namespace gstlrn
{
  ...
  public:
    ...
    static Selectivity* createByCodes(
      ...
      double proba = TEST,
      bool verbose = false);
    static Selectivity* createByKeys(
    ...
    void
      storeInDb(Db* db, Id iech0, Id iptr, double zestim, double zstdev) const;
    void interpolateSelectivity(const Selectivity* selecin);

    void setFlagTonnageCorrect(bool flagTonnageCorrect)
    {
      _flagTonnageCorrect = flagTonnageCorrect;
    }
  ...
  private:
    ...
    static void _interpolateInterval(
      double zval,
      double zi0,
      double zi1,
      double ti0,
      ...
      double* qval,
      double tol = EPSILON3);
    void _concatenate(VectorString& names, const ESelectivity& code, Id mode)
      const;
    static bool _isMultiplied(const ESelectivity& code);
    void
      _defineAutomaticCutoffs(const VectorDouble& tab, double eps = EPSILON3);
  ...
  GSTLEARN_EXPORT Id dbSelectivity(
    Db* db,
    const String& name,
    const VectorDouble& zcuts,
