...
namespace gstlrn
{
   ...
   */
  class GSTLEARN_EXPORT ACov: public ICloneable,
                              public AStringable,
  ...
  public:
    ...
    virtual double
      eval0(Id ivar = 0, Id jvar = 0, const CovCalcMode* mode = nullptr) const;
    ...
    virtual double
      evalSpectrumOnRN(const VectorDouble& freq, Id ivar = 0, Id jvar = 0)
        const;
    ...
    virtual void
      updateCovByPoints(Id icas1, Id iech1, Id icas2, Id iech2) const;
    ...
    void
      optimizationPreProcess(Id mode, const std::vector<SpacePoint>& ps) const;
    ...
    MatrixSymmetric
      evalCovMat0(const Db* db, Id iech, const KrigOpt& krigopt = KrigOpt())
        const;
    MatrixDense evalCovMat(
      const Db* db1,
    ...
    MatrixSymmetric evalCovMatSym(
      const Db* db1,
      const VectorInt& nbgh1 = VectorInt(),
      Id ivar0 = -1,
      const CovCalcMode* mode = nullptr,
      bool cleanOptim = true) const;
    MatrixSparse* evalCovMatSparse(
      const Db* db1_arg,
    ...
    Id evalCovMat0InPlace(
      MatrixSymmetric& mat,
      const Db* db,
      Id iech,
      const KrigOpt& krigopt = KrigOpt()) const;
    Id evalCovMatInPlace(
      MatrixDense& mat,
      const Db* db1,
      const Db* db2 = nullptr,
      Id ivar0 = -1,
    ...
    Id evalCovMatInPlaceFromIdx(
      ...
      const CovCalcMode* mode = nullptr,
      bool cleanOptim = true) const;
    Id evalCovMatSymInPlaceFromIdx(
      MatrixSymmetric& mat,
      const Db* db1,
      const VectorVectorInt& index1,
      const CovCalcMode* mode = nullptr,
      bool cleanOptim = true) const;
    Id evalCovMatRHSInPlaceFromIdx(
      MatrixDense& mat,
      const Db* db1,
      const Db* db2,
      const VectorVectorInt& index1,
    ...
    virtual Id addEvalCovVecRHSInPlace(
      vect vect,
...
#endif
    /////////////////////////////////////////////////////////////////////////////////
    void eval0CovMatBiPointInPlace(
      MatrixSymmetric& mat,
    ...
    double evalIvarIpas(
      ...
      Id jvar = 0,
      const CovCalcMode* mode = nullptr) const;
    double evalIvarIpasIncr(
      const VectorDouble& dincr,
      Id ivar = 0,
      Id jvar = 0,
      const CovCalcMode* mode = nullptr) const;
    VectorDouble evalIvarNlag(
      const VectorDouble& vec_step,
      const VectorDouble& dir = VectorDouble(),
      Id ivar = 0,
      Id jvar = 0,
      const CovCalcMode* mode = nullptr) const;
    MatrixSquare evalNvarIpas(
      double step,
      const VectorDouble& dir = VectorDouble(),
      const CovCalcMode* mode = nullptr) const;
    MatrixSquare evalNvarIpasIncr(
      const VectorDouble& dincr,
      const CovCalcMode* mode = nullptr) const;
    double evalIsoIvarIpas(
      double step,
      Id ivar = 0,
      Id jvar = 0,
      const CovCalcMode* mode = nullptr) const;
    VectorDouble evalIsoIvarNlag(
      const VectorDouble& vec_step,
      Id ivar = 0,
      Id jvar = 0,
      const CovCalcMode* mode = nullptr) const;
    MatrixSquare
      evalIsoNvarIpas(double step, const CovCalcMode* mode = nullptr) const;
    ...
    double evalAverageDbToDb(
      ...
      Id seed = 434132,
      const CovCalcMode* mode = nullptr) const;
    double evalAverageIncrToIncr(
      const VectorVectorDouble& d1,
      const VectorVectorDouble& d2,
      Id ivar = 0,
      Id jvar = 0,
      const CovCalcMode* mode = nullptr) const;
    double evalAveragePointToDb(
      const SpacePoint& p1,
      const Db* db2,
      Id ivar = 0,
      Id jvar = 0,
    ...
    double extensionVariance(
      ...
      Id ivar = 0,
      Id jvar = 0) const;
    double samplingDensityVariance(
      const Db* db,
      const VectorDouble& ext,
      const VectorInt& ndisc,
      const VectorDouble& angles = VectorDouble(),
    ...
    void evaluateMatInPlace(
      ...
      double weight = 1.,
      const CovCalcMode* mode = nullptr) const;
    VectorDouble evaluateFromDb(
      Db* db,
      Id ivar = 0,
      Id jvar = 0,
      const CovCalcMode* mode = nullptr) const;
    double evaluateOneIncr(
      double hh,
    ...
    {
      return static_cast<Id>(_ctxt.getNDim(ispace));
    }
  ...
  private:
    ...
    void _optimizationPreProcessForTarget(
      const Db* db,
    ...
    Id _evalCovMatRHSInPlaceBlock(
      MatrixDense& mat,
      const Db* db2,
      const VectorVectorInt& index1,
      const VectorVectorInt& index2,
      const KrigOpt& krigopt = KrigOpt()) const;
    Id _evalCovMatRHSInPlacePoint(
      MatrixDense& mat,
      const VectorVectorInt& index1,
      const VectorVectorInt& index2,
      const KrigOpt& krigopt = KrigOpt()) const;
  ...
  protected:
    ...
    void setNVar(Id nvar) { _ctxt.setNVar(nvar); }

    virtual void _optimizationSetTarget(SpacePoint& pt) const;
    virtual void
      _optimizationPreProcess(Id mode, const std::vector<SpacePoint>& ps) const;

    VectorInt _getActiveVariables(Id ivar0) const;
    static void _updateCovMatrixSymmetricForVerr(
      const Db* db1,
      AMatrix* mat,
      const VectorVectorInt& index1);

    virtual SpacePoint&
      _optimizationLoadInPlace(Id iech, Id mode, Id rank) const;
  ...
  private:
    ...
    DbGrid* _discretizeBlock(
      const VectorDouble& ext,
      const VectorInt& ndisc,
      const VectorDouble& angles = VectorDouble(),
      const VectorDouble& x0 = VectorDouble()) const;
    Db* _discretizeBlockRandom(const DbGrid* dbgrid, Id seed = 34131) const;
  ...
  protected:
    ...
    mutable bool
      _optimPreProcessedData; // True if Data have been pre-processed for optimization
