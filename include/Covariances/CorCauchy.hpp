...
/**
 * \brief
 * This class describes the multivariate Cauchy correlation function.
 *
 */
class GSTLEARN_EXPORT CorCauchy: public ACov
{
public:
  CorCauchy(const VectorDouble& ranges      = VectorDouble(),
            const VectorDouble& angles      = VectorDouble(),
            const VectorDouble& coeffScales = VectorDouble(),
            const VectorDouble& params      = VectorDouble(),
            bool flagRange                  = true);
  CorCauchy(const CorCauchy& r);
  CorCauchy& operator=(const CorCauchy& r);
  virtual ~CorCauchy();
  static CorCauchy* create(const ECov& type,
...
private:
  ...
  mutable CorAniso _corCauchy; // an auxiliary Cauchy covariance function
  VectorDouble _coeffScales;   // scale factor for each variable
  VectorDouble _params;        // Cauchy parameter for each variable
