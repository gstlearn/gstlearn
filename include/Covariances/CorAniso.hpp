...
namespace gstlrn
{
  ...
  public:
    ...
    double evalCovOnSphere(
      double alpha,
      Id degree = 50,
      bool scaleDistanceByRadius = true,
      const CovCalcMode* mode = nullptr) const override;
    VectorDouble evalSpectrumOnSphere(
      Id n,
    ...
    static bool isOptimizationInitialized(
      const std::vector<SpacePoint>& p1As,
      const Db* db = nullptr);

    void _optimizationPreProcess(Id mode, const std::vector<SpacePoint>& ps)
      const override;
    ...
    double evalSpectrumOnRN(const VectorDouble& freq, Id ivar = 0, Id jvar = 0)
      const override;
    SpectrumOnRN* simulateOnRN(Id ns = 1000) const override;

    MatrixDense simulateSpectralOmega(Id nb) const override;
    double evalSpectrumRatio(
      const VectorDouble& freq,
      Id ivar,
      Id jvar,
      const ACov* cov0) const override;

    static CorAniso* create(
      const CovContext& ctxt,
      const ECov& type,
    ...
    static CorAniso* createIsotropic(
      const CovContext& ctxt,
      const ECov& type,
      double range,
      double param = 1.,
      bool flagRange = true);
    static CorAniso* createAnisotropic(
      const CovContext& ctxt,
      const ECov& type,
      const VectorDouble& ranges,
      double param = 1.,
      const VectorDouble& angles = VectorDouble(),
      bool flagRange = true);
    static CorAniso* createIsotropicMulti(
      const CovContext& ctxt,
      const ECov& type,
      double range,
      double param = 1.,
      bool flagRange = true);
    static CorAniso* createAnisotropicMulti(
      const CovContext& ctxt,
      const ECov& type,
      const VectorDouble& ranges,
      double param = 1.,
    ...
    {
      return _aniso.getMatrixDirect();
    }

    const MatrixSquare& getAnisoInvMat() const
    {
      return _aniso.getMatrixInverse();
    }
    ...
    void setFlagAnisoOnMesh(bool flagAnisoOnMesh)
    {
      _flagAnisoOnMesh = flagAnisoOnMesh;
    }

    inline bool getFlagAnisoOnMesh() const { return _flagAnisoOnMesh; }

    Id makeElemNoStatOnMesh(
    ...
    void makeRangeNoStatDb(
      const String& namecol,
      Id idim = 0,
      const Db* db = nullptr);
    void makeScaleNoStatDb(
      const String& namecol,
      Id idim = 0,
      const Db* db = nullptr);
    void makeAngleNoStatDb(
      const String& namecol,
      Id idim = 0,
      const Db* db = nullptr);
    void makeTensorNoStatDb(
      const String& namecol,
      Id idim = 0,
      Id jdim = 0,
      const Db* db = nullptr);
    ...
    void makeTensorNoStatFunctional(
    ...
    {
      return static_cast<TabNoStatCovAniso&>(*_tabNoStat);
    }
    ...
    {
      return getTabNoStatCovAniso().isDefinedForTensor();
    }

    bool isNoStatForAnisotropy() const
    {
      return getTabNoStatCovAniso().isDefinedForAnisotropy();
    }

    bool isNoStatForRotation() const
    {
      return getTabNoStatCovAniso().isDefinedForRotation();
    }

    VectorDouble evalCovOnSphereVec(
      const VectorDouble& alpha,
      Id degree = 50,
      bool scaleDistanceByRadius = true,
      const CovCalcMode* mode = nullptr) const;
    Array
      evalCovFFT(const VectorDouble& hmax, Id N = 128, Id ivar = 0, Id jvar = 0)
        const;
    VectorDouble getMarkovCoeffs() const;
    void setMarkovCoeffs(const VectorDouble& coeffs);

    void setMarkovCoeffsBySquaredPolynomials(
    ...
    void
      updateCovByPoints(Id icas1, Id iech1, Id icas2, Id iech2) const override;
    ...
    void
      optimizationTransformSP(const SpacePoint& ptin, SpacePoint& ptout) const;
    void optimizationTransformSPNew(const SpacePoint& ptin, SpacePoint& ptout)
      const;
    String toStringParams(const AStringFormat* strfmt = nullptr) const;
    String
      toStringNoStat(const AStringFormat* strfmt = nullptr, Id i = 0) const;
    void appendParams(
      ListParams& listParams,
      std::vector<covmaptype>* gradFuncs = nullptr) override;
    double evalDerivativeBasis(
      const SpacePoint& p1,
      const SpacePoint& p2,
      Id ivar,
      Id jvar,
