...
namespace gstlrn
{
  ...
  public:
    ...
    static CovAniso* createFromParam(
      const ECov& type,
      double range,
      double sill = 1.,
      double param = 1,
      ...
      bool flagRange = true);
    static CovAniso* createIsotropic(
      const CovContext& ctxt,
      const ECov& type,
      double range,
      double sill = 1.,
      double param = 1.,
      bool flagRange = true);
    static CovAniso* createAnisotropic(
      const CovContext& ctxt,
      const ECov& type,
      ...
      const VectorDouble& angles = VectorDouble(),
      bool flagRange = true);
    static CovAniso* createIsotropicMulti(
      const CovContext& ctxt,
      const ECov& type,
      double range,
      ...
      double param = 1.,
      bool flagRange = true);
    static CovAniso* createAnisotropicMulti(
      const CovContext& ctxt,
      const ECov& type,
      const VectorDouble& ranges,
      const MatrixSymmetric& sills,
    ...
    {
      return getCorAniso()->getAniso().getRotation();
    }
    ...
    {
      return getCorAniso()->getAniso().getAngles();
    }

    const MatrixSquare& getAnisoRotMat() const
    {
      return getCorAniso()->getAniso().getMatrixDirect();
    }

    const MatrixSquare& getAnisoInvMat() const
    {
      return getCorAniso()->getAniso().getMatrixInverse();
    }
    ...
    {
      return getCorAniso()->getAniso().getMatrixDirect().getValue(idim, jdim);
    }
    ...
    {
      return getCorAnisoModify()->getParamInfoScales();
    }

    std::vector<ParamInfo>& getAnglesParam()
    {
      return getCorAnisoModify()->getParamInfoAngles();
    }
  ...
  GSTLEARN_EXPORT double
    scale2range(const ECov& type, double scale, double param = 1.);
  GSTLEARN_EXPORT double
    range2scale(const ECov& type, double range, double param = 1.);
} // namespace gstlrn
