...
namespace gstlrn
{
  ...
  public:
    ...
    static CorGneiting* create(
      ...
      const VectorDouble&
        ranges, // spatial scales or ranges according to flagRange value
      const VectorDouble& angles = VectorDouble(),
      double separability =
        1.0, // in [0,1] with a time-space separable model for 0, and "pure" Gneiting for 1
    ...
    {
      return _corS->getKappa(ivar, jvar);
    }
    ...
    SpectrumOnRN* simulateSpace(Id ns) const
    {
      return _corS->simulateOnRN(ns);
    };
  ...
  private:
    std::shared_ptr<const CorGaussianMixture>
      _corS; // the space covariance function (ndim, nvar >=1)
    std::shared_ptr<const CorAniso>
      _corT; // the time covariance function (ndim = 1, nvar = 1)
