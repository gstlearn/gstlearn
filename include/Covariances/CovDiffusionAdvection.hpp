...
namespace gstlrn
{
  ...
  public:
    ...
    static CovDiffusionAdvection*
      create(CovAniso* markovL = nullptr,
             CovAniso* markovR = nullptr,
             double scaleTime = 1.,
             const VectorDouble& vel = VectorDouble(2),
             double sigma2 = 1.);
    ...
    std::complex<double>
      evalSpatialSpectrum(VectorDouble freq, double time) const;
    Array
      evalCovFFT(const VectorDouble& hmax, double time = 0, Id N = 128) const;
