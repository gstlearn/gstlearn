...
namespace gstlrn
{
  ...
  private:
    ...
    static void _extract(const Id* nxmap,
                         ...
                         VectorDouble& tabin,
                         VectorDouble& tabout);
    Id _vmap_general(Db* db, Id radius, const NamingConvention& namconv);
    Id _vmap_grid(DbGrid* dbgrid, const NamingConvention& namconv);
    ...
    static void
      _vmap_rescale(double scale, VectorDouble& tab1, VectorDouble& tab2);
    static void
      _vmap_shift(VectorDouble& tab, VectorDouble& tabm1, VectorDouble& tabm2);
    void _vmap_store(VectorDouble& tab, Id iptr);
    void _vmap_normalize(Id nv2);
    Id _findNeighCell(const VectorInt& indg0,
  ...
  private:
    DbGrid*
      _dbmap; // Pointer to the already existing output DbGrid (not to be deleted)
    Id _IPTV;
    Id _IPTW;
  };
