...
namespace gstlrn
{
  ...
  public:
    ...
    static Vario* createRegularizeFromModel(
      const Model& model,
    ...
    static Vario* createReduce(
      const Vario& varioIn,
      const VectorInt& varcols,
      const VectorInt& dircols,
      bool asSymmetric = false);
    static Vario* computeFromDb(
      const VarioParam& varioparam,
      Db* db,
    ...
    VectorDouble
      getHhVec(Id idir = 0, Id ivar = 0, Id jvar = 0, bool compress = true)
        const;
    VectorDouble
      getSwVec(Id idir = 0, Id ivar = 0, Id jvar = 0, bool compress = true)
        const;
    VectorDouble
      getUtilizeVec(Id idir = 0, Id ivar = 0, Id jvar = 0, bool compress = true)
        const;
    ...
    VectorDouble
      getGRange(Id ivar = -1, Id jvar = -1, Id idir = -1, bool flagSill = false)
        const;
    ...
    Id getAddressForGg(Id idir, Id ivar, Id jvar, Id ilag, Id orient = ITEST)
      const;
    ...
    {
      return _varioparam.getDirParam(idir);
    }
    ...
    {
      return _varioparam.getDate(idate, icas);
    }
    ...
    {
      return static_cast<Id>(getDirParam(idir).getNDim());
    }
    ...
    {
      return getDirParam(idir).getMaximumDistance();
    }
    ...
    VectorInt getGrincrs(Id idir) const
    {
      return getDirParam(idir).getGrincrs();
    }

    double getGrincr(Id idir, Id idim) const
    {
      return getDirParam(idir).getGrincr(idim);
    }
    ...
    {
      return _bipts[_getBiPtsRank(idir, rank)];
    }

    bool
      keepPair(Id idir, SpaceTarget& T1, SpaceTarget& T2, double* dist) const;
  ...
  private:
    ...
    Id _compute(
      ...
      Id niter_UK,
      bool verbose);
    Id _calculateGeneral(Db* db, bool flag_sample = false, Id verr_mode = 0);
    ...
    static double _linear_interpolate(
      Id n,
      const VectorDouble& x,
      const VectorDouble& y,
      double x0);
    MatrixSquare _evalAverageDbIncr(
      Model* model,
      const Db& db,
  ...
  private:
    ...
    mutable Model*
      _model; // Model pointer (not to be deleted) for drift removal
