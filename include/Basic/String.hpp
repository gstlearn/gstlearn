...
namespace gstlrn
{
  ...
  GSTLEARN_EXPORT bool matchKeyword(
    const String& string1,
    const String& string2,
    bool caseSensitive = true);
  GSTLEARN_EXPORT bool matchRegexp(
    const String& string1,
    const String& string2,
    bool caseSensitive = true);
  GSTLEARN_EXPORT Id getRankInList(
    const VectorString& list,
    const String& match,
    bool caseSensitive = true);
  GSTLEARN_EXPORT Id decodeInString(
    const String& symbol,
    const String& node,
    Id* facies,
    bool caseSensitive = true);
  GSTLEARN_EXPORT Id decodeInList(
  ...
  GSTLEARN_EXPORT String concatenateString(
    ...
    double value,
    const String& delim = "-");
  GSTLEARN_EXPORT String concatenateStrings(
    const String& delimt = ".",
    const String& string1 = "",
    const String& string2 = "",
  ...
  GSTLEARN_EXPORT VectorString generateMultipleNames(
    ...
    Id number,
    const String& delim = "-");
  GSTLEARN_EXPORT VectorString expandList(
    const VectorString& list,
    const String& match,
    bool onlyOne = false);
  GSTLEARN_EXPORT VectorString
    expandList(const VectorString& list, const VectorString& matches);
  ...
  GSTLEARN_EXPORT VectorInt decodeGridSorting(
    const String& string,
    const VectorInt& nx,
    bool verbose = false);
  ...
  GSTLEARN_EXPORT String toStrVectorVec(
    const String& title,
    constvect tab,
    ...
    bool flagSkipZero = false)
  {
    ...
    Id ncutil = ncols;
    Id nrutil = nrows;
    if (_getMaxNCols() > 0 && ncutil > _getMaxNCols() && !flagOverride)
      ncutil = _getMaxNCols();
    if (_getMaxNRows() > 0 && nrutil > _getMaxNRows() && !flagOverride)
      nrutil = _getMaxNRows();
    Id npass = static_cast<Id>(
      ceil(static_cast<double>(ncutil) / static_cast<double>(_getNBatch())));
    ...
    if (colnames.empty())
      ...
      colSize = MAX(
        MIN(_getColumnName(), getMaxStringSize(colnames) + 1),
        _getColumnSize());
    Id rowSize = 0;
    if (rownames.empty())
  ...
  GSTLEARN_EXPORT String toStrMatrix(
    const String& title,
    const AMatrix& mat,
    bool flagOverride = false,
    bool flagSkipZero = false);
  GSTLEARN_EXPORT String toStrTrimMat(
    ...
    bool newLineAfterTitle = false)
  {
    ...
    Id ncutil = ncols;
    if (_getMaxNCols() > 0 && ncutil > _getMaxNCols() && !flagIgnoreMaxNCols)
      ncutil = _getMaxNCols();
    ...
    bool newLineAfterTitle = true)
  {
    ...
    Id nrutil = nrows;
    if (_getMaxNRows() > 0 && nrutil > _getMaxNRows() && !flagIgnoreMaxNRows)
      nrutil = _getMaxNRows();
