...
namespace gstlrn
{
  ...
  GSTLEARN_EXPORT bool isEqualExtended(
    double v1,
    double v2,
  ...
  GSTLEARN_EXPORT StatResults ut_statistics(
    Id nech,
    const double* tab,
    const double* sel = nullptr,
    const double* wgt = nullptr);
  GSTLEARN_EXPORT void
    ut_stats_mima_print(const char* title, Id nech, double* tab, double* sel);
  GSTLEARN_EXPORT void ut_facies_statistics(
    Id nech,
    double* tab,
    double* sel,
    Id* nval,
    Id* mini,
    Id* maxi);
  GSTLEARN_EXPORT void ut_classify(
    Id nech,
    const double* tab,
    double* sel,
    Id nclass,
  ...
  GSTLEARN_EXPORT std::map<Id, Id>
    getMapAbsoluteToRelative(const VectorDouble& sel, bool verbose = false);
  GSTLEARN_EXPORT Id
    getRankMapAbsoluteToRelative(const std::map<Id, Id>& map, Id iabs);
  GSTLEARN_EXPORT Id
    getRankMapRelativeToAbsolute(const std::map<Id, Id>& map, Id irel);
  ...
  GSTLEARN_EXPORT double
    modifyOperator(const EOperator& oper, double oldval, double value);
  ...
  GSTLEARN_EXPORT void print_range(
    const char* title,
    Id ntab,
    const double* tab,
    const double* sel);
  ...
  GSTLEARN_EXPORT void
    convertIndptrToIndices(Id ncumul, const I32* cumul, I32* tab);
} // namespace gstlrn
