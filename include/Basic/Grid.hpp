...
namespace gstlrn
{
  ...
  public:
    ...
    void
      getCoordinatesByRankInPlace(VectorDouble& coor, Id rank, bool flag_rotate)
        const;
    ...
    void getCoordinatesByCornerInPlace(
      VectorDouble& coor,
      const VectorInt& icorner) const;
    VectorDouble getCellCoordinatesByCorner(
      Id node,
      const VectorInt& shift = VectorInt(),
      const VectorDouble& dxsPerCell = VectorDouble()) const;
    void getCellCoordinatesByCornerInPlace(
      VectorDouble& coor,
      Id node,
      const VectorInt& shift = VectorInt(),
      const VectorDouble& dxsPerCell = VectorDouble()) const;
#ifndef SWIG
    double indiceToCoordinate(
      Id idim0,
    ...
    void coordinateToIndicesInPlace(
      VectorInt& indices,
      const VectorDouble& coor,
      bool centered,
      double eps) const;
    ...
    {
      return _rotation.getMatrixDirect().getValues();
    }
    ...
    {
      return _rotation.isSame(grid.getRotation());
    }
  ...
  private:
    const MatrixSquare& _getRotMat() const
    {
      return _rotation.getMatrixDirect();
    }

    const MatrixSquare& _getRotInv() const
    {
