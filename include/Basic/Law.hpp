...
namespace gstlrn
{
  ...
  GSTLEARN_EXPORT double law_df_gamma(
    double value,
    double alpha,
    double rate = 1.,
    bool isLog = false);
  GSTLEARN_EXPORT double law_IGamma(double alpha, double rate = 1.);
  GSTLEARN_EXPORT double law_df_IGamma(
    double value,
    double alpha,
    double rate = 1.,
    bool isLog = false);
  ...
  GSTLEARN_EXPORT VectorDouble
    law_df_poisson_vec(VectorInt is, double parameter);
  ...
  GSTLEARN_EXPORT double
    law_stable_a(double alpha, double beta, double gamma, double delta);
  GSTLEARN_EXPORT double law_stable_a1(double beta, double gamma, double delta);
  GSTLEARN_EXPORT double
    law_stable(double alpha, double beta, double gamma, double delta);
  ...
  GSTLEARN_EXPORT double law_df_bigaussian(
    VectorDouble& vect,
    VectorDouble& mean,
    MatrixSymmetric& correl);
  GSTLEARN_EXPORT double
    law_df_quadgaussian(VectorDouble& vect, MatrixSymmetric& correl);
  GSTLEARN_EXPORT double
    law_df_multigaussian(VectorDouble& vect, MatrixSymmetric& correl);
  GSTLEARN_EXPORT VectorDouble
    law_multigaussian(const VectorDouble& mu, MatrixSymmetric& sigma);
  GSTLEARN_EXPORT VectorInt law_random_path(Id nech);
  GSTLEARN_EXPORT VectorDouble law_exp_sample(
    const double* tabin,
    Id mode,
    Id nvar,
    Id nechin,
