...
public:
  ...
  const MatrixSGeneral& getMatrixDirect() const { return _rotMat; }
  const MatrixSGeneral& getMatrixInverse() const { return _rotInv; }
  ...
  int setMatrixDirect(const MatrixSGeneral& rotmat);
...
private:
  ...
  MatrixSGeneral _rotMat;
  MatrixSGeneral _rotInv;
};
