...
namespace gstlrn
{
  ...
  public:
    ...
    inline VectorT(const Vector& vec)
      : _v(vec)
    {
    }
    ...
    inline VectorT(InputIt first, InputIt last)
      : _v()
    {
...
#ifndef SWIG
    inline VectorT(std::initializer_list<T> init)
      : _v(init)
    {
    }
...
#ifndef SWIG
    inline VectorT& operator=(const Vector& vec)
    {
      _v = vec;
      return (*this);
    }
    ...
    inline VectorT& operator=(VectorT&& other) noexcept
    {
      _v.swap(other._v);
      return (*this);
    }

    inline VectorT& operator=(std::initializer_list<T> init)
    {
      _v = init;
      return (*this);
    }
    ...
    inline bool operator==(const VectorT& other) const
    {
      return _v == other._v;
    }
    ...
    inline void insert(size_type i, const T& value)
    {
      _v.insert(begin() + i, value);
    }

    inline void insert(size_type i, size_type count, const T& value)
    {
      _v.insert(begin() + i, count, value);
    }
    ...
    inline void remove(size_type i, size_type count)
    {
      _v.erase(begin() + i, begin() + i + count);
    }
  ...
  {
    if (pos >= size()) my_throw("VectorT<T>::at: index out of range");
    return operator[](pos);
  }
  ...
  VectorT<T>& VectorT<T>::operator<<(const VectorT<T>& v)
  {
    reserve(size() + v.size());
    std::for_each(
      v.cbegin(), v.cend(), [&](const T& value) { push_back(value); });
    return (*this);
  }
