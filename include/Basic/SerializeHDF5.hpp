...
namespace gstlrn
{
  ...
  namespace SerializeHDF5
  {
    ...
    inline H5::H5File fileOpenRead(const String& fname)
    {
      ...
      if (version != "1.0.0")
      {
        messerr("File %s has format version %s, expected 1.0.0",
                filepath.c_str(),
                version.c_str());
      }
      return file;
      ...
      getGroup(const H5::Group& parent, const String& name, bool verbose = true)
    {
          ...
          || parent.childObjType(name) != H5O_TYPE_GROUP)
      {
        ...
        if (verbose)
          messerr("Cannot find group %s in parent group %s",
                  name.c_str(),
                  parent_name.c_str());
        return std::nullopt;
      }
  ...
  template<typename T>
  bool SerializeHDF5::readVec(const H5::Group& grp,
                              const String& title,
                              VectorT<T>& vec)
  {
    ...
    {
      messerr("Cannot read HDF5 Variable of name %s in group %s",
              title.c_str(),
              grp_name.c_str());
      return false;
    }
    ...
    {
      messerr("HDF5 Variable of name %s in group %s has %d dims, but we expect "
              "only 1",
              title.c_str(),
              grp_name.c_str(),
              ds.getSimpleExtentNdims());
      return false;
    }
                              ...
                              VectorString& vec)
  {
    ...
    if (ds.getSimpleExtentNdims() != 1)
    {
      messerr("HDF5 String Variable of name %s in group %s has %d dims, but we "
              "expect only 1",
              title.c_str(),
              grp_name.c_str(),
              ds.getSimpleExtentNdims());
    ...
    data.read(static_cast<void*>(data_ptr.data()),
              H5::StrType{0, H5T_VARIABLE});
                               ...
                               const VectorString& vec)
  {
    ...
    var.write(static_cast<void*>(data_ptr.data()),
              H5::StrType{0, H5T_VARIABLE});
    return true;
  }

  template<typename T>
  bool SerializeHDF5::readValue(const H5::H5Object& grp,
                                const String& name,
                                T& value)
  {
    const auto grp_name = grp.getObjName();
    ...
    {
      messerr("Could not read value '%s' in group '%s': mismatch in datatypes",
              name.c_str(),
              grp_name.data());
      return false;
    }
  ...
  template<typename T>
  bool SerializeHDF5::writeValue(H5::H5Object& grp,
                                 const String& name,
                                 const T& value)
  {
    std::string grp_name;
