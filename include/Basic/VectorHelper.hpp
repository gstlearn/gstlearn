...
namespace gstlrn
{
  ...
  public:
    ...
    static VectorVectorDouble
      initVVDouble(Id nval1, Id nval2, double value = 0.);
...
#endif
    static void
      dumpStats(const String& title, const VectorDouble& vectin, Id nmax = -1);
    static void
      dumpRange(const String& title, const VectorDouble& vectin, Id nmax = -1);
    static void dumpRange(const String& title, const VectorInt& vect);
    static void
      dumpNNZ(const String& title, const VectorDouble& vect, Id nclass = 10);

    static void
      capInPlace(VectorDouble& vec, double vmin = TEST, double vmax = TEST);
    static void capInPlaceVVD(
      VectorVectorDouble& vec,
      double vmin = TEST,
      double vmax = TEST);
    static double
      extensionDiagonal(const VectorDouble& mini, const VectorDouble& maxi);
    ...
    static VectorDouble
      quantiles(const VectorDouble& vec, const VectorDouble& probas);

    static bool isEqual(
      const VectorDouble& v1,
      const VectorDouble& v2,
      double eps = EPSILON10);
    static bool isEqualExtended(
      const VectorDouble& v1,
      const VectorDouble& v2,
      double eps = EPSILON10,
    ...
    static VectorDouble sequenceVD(
      double valFrom,
      double valTo,
      double valStep = 1.,
      double ratio = 1.);
...
#ifndef SWIG
    static void addMultiplyConstantInPlace(
      double val1,
      const constvect in,
      vect out,
      Id iad);
    static double innerProductCV(const constvect veca, const constvect vecb);
    static void addMultiplyVectVectInPlace(
      const constvect in1,
      const constvect in2,
      vect out,
      Id iad);
...
#endif
    static void
      addInPlace(const double* veca, const double* vecb, double* res, Id size);
    ...
    static void
      subtractInPlace(const constvect in1, const constvect in2, vect outv);
#endif
    static void multiplyComplexInPlace(
    ...
    static void
      copy(const VectorDouble& vecin, VectorDouble& vecout, Id size = -1);
    ...
    static void mean1AndMean2ToStdev(
    ...
    static void normalizeFromGaussianDistribution(
      VectorDouble& vec,
      double mini = 0.,
      double maxi = 1.);
    static VectorDouble normalScore(
    ...
    static VectorDouble
      concatenate(const VectorDouble& veca, const VectorDouble& vecb);
    static void
      concatenateInPlace(VectorDouble& veca, const VectorDouble& vecb);
    ...
    static double
      innerProduct(const double* veca, const double* vecb, Id size = -1);

    static VectorDouble
      crossProduct3D(const VectorDouble& veca, const VectorDouble& vecb);
    static void
      crossProduct3DInPlace(const double* a, const double* b, double* v);

    static VectorDouble
      cumsum(const VectorDouble& vecin, bool flagAddZero, bool revert = false);
    ...
    static void cumulate(
      ...
      double coeff = 1.,
      double addval = 0.);
    static void getMostSignificant(
    ...
    static VectorDouble simulateBernoulli(
      ...
      double velse = 0.);
    static VectorDouble
      simulateGaussian(Id n = 1, double mean = 0., double sigma = 1.);
    static void simulateGaussianInPlace(
      VectorDouble& vec,
      double mean = 0.,
      double sigma = 1.);
    static VectorInt sampleRanks(
      Id ntotal,
      double proportion = 0.,
      Id number = 0,
      Id seed = 242141,
    ...
    static VectorInt
      sort(const VectorInt& vecin, bool ascending = true, Id size = -1);
    static VectorDouble
      sort(const VectorDouble& vecin, bool ascending = true, Id size = -1);
    static void
      sortInPlace(VectorInt& vecin, bool ascending = true, Id size = -1);
    static void
      sortInPlace(VectorDouble& vecin, bool ascending = true, Id size = -1);
    ...
    static VectorInt
      orderRanks(const VectorInt& vecin, bool ascending = true, Id size = -1);
    static VectorInt orderRanks(
    ...
    static VectorDouble
      sample(const VectorDouble& vecin, const VectorInt& indKeep);

    static void arrangeInPlace(
      Id safe,
      VectorInt& ranks,
      VectorDouble& values,
      bool ascending = true,
    ...
    static void
      unflattenInPlace(const VectorDouble& vd, VectorVectorDouble& vvd);
    ...
    static VectorVectorDouble
      unflatten(const VectorDouble& vd, const VectorInt& sizes);
    static void linearCombinationInPlace(
      double val1,
      const VectorDouble& vd1,
      double val2,
      const VectorDouble& vd2,
      VectorDouble& outv);
    static void linearCombinationVVDInPlace(
      double val1,
    ...
    static void
      extractInPlace(const VectorDouble& vecin, VectorDouble& vecout, Id start);
    static void
      mergeInPlace(const VectorDouble& vecin, VectorDouble& vecout, Id start);

    static void transformVD(VectorDouble& tab, Id oper_choice = 1);

    static void squeezeAndStretchInPlaceForward(
      const VectorDouble& vecin,
      VectorDouble& vecout,
      double origin,
      double mesh,
      double top,
      double bot);
    static void squeezeAndStretchInPlaceBackward(
      const VectorDouble& vecin,
      VectorDouble& vecout,
      double origin,
      double mesh,
    ...
    static VectorDouble
      reduce(const VectorDouble& vecin, const VectorInt& vindex);
    static VectorDouble
      compress(const VectorDouble& vecin, const VectorInt& vindex);
    ...
    template<typename T>
    static VectorNumT<T>
      subtract(const VectorNumT<T>& v1, const VectorNumT<T>& v2)
    {
      VectorNumT<T> vecout;
    ...
    template<typename T>
    static VectorNumT<T>
      subtractCst(const VectorNumT<T>& v1, T v2, bool flagOpposite = false)
    {
      VectorNumT<T> vecout;
    ...
    template<typename T>
    static VectorNumT<T>
      multiply(const VectorNumT<T>& v1, const VectorNumT<T>& v2)
    {
      VectorNumT<T> vecout;
    ...
    template<typename T>
    static VectorNumT<T>
      divide(const VectorNumT<T>& v1, const VectorNumT<T>& v2)
    {
      VectorNumT<T> vecout;
      divide(vecout, v1, v2);
      return vecout;
    ...
    template<typename T>
    static VectorNumT<T>
      divideCst(const VectorNumT<T>& v1, T v2, bool flagOpposite = false)
    {
      VectorNumT<T> vecout;
    ...
    static void add(
      VectorNumT<T>& vecout,
      const VectorNumT<T>& v1,
      const VectorNumT<T>& v2)
    {
      static_assert(
        (std::is_same_v<T, Id> && std::is_integral_v<T>)
      ...
      {
        messerr(
          "Impossible to add vectors of different dimensions: V1(%d) and "
          "V2(%d)\n",
          n1, n2);
        return;
      }
      if (&vecout != &v1 && &vecout != &v2) vecout.resize(n1);
      for (Id i = 0; i < n1; i++)
      ...
      const VectorNumT<T>& v2)
    {
      ...
      {
        messerr(
          "Impossible to subtract vectors of different dimensions: V1(%d) and "
          "V2(%d)\n",
          n1, n2);
        return;
      }
      ...
      const VectorNumT<T>& v2)
    {
      ...
      {
        messerr(
          "Impossible to multiply vectors of different dimensions: V1(%d) and "
          "V2(%d)\n",
          n1, n2);
        return;
      }
    ...
    static void multiply(VectorNumT<T>& vecout, const VectorNumT<T>& v1, U v2)
    {
      static_assert(
        std::is_same_v<T, U> || (std::is_same_v<T, Id> && std::is_integral_v<U>)
          || (std::is_same_v<T, double> && std::is_arithmetic_v<U>),
        "Invalid type for multiplication operator between VectorNumT and "
        "scalar");
      auto n1 = static_cast<Id>(v1.size());
      if (&vecout != &v1) vecout.resize(n1);
      if (isNA(v2))
    ...
    template<typename T>
    static void
      multiplyAssign(VectorNumT<T>& vecout, const VectorNumT<T>& v1, T v2)
    {
      ...
      const VectorNumT<T>& v2)
    {
      ...
      {
        messerr(
          "Impossible to divide vectors of different dimensions: V1(%d) and "
          "V2(%d)\n",
          n1, n2);
        return;
      }
      if (&vecout != &v1 && &vecout != &v2) vecout.resize(n1);
      for (Id i = 0; i < n1; i++)
        vecout[i] = (isNA(v1[i]) || isNA(v2[i]) || v2[i] == 0.) ? getNA<T>()
                                                                : v1[i] / v2[i];
    }
      ...
      bool flagOpposite = false)
    {
      ...
      auto n1 = static_cast<Id>(v1.size());
      if (&vecout != &v1) vecout.resize(n1);
      if (isNA(v2)) vecout.fill(getNA<T>());
      {
        if (flagOpposite)
        {
          for (Id i = 0; i < n1; i++)
            vecout[i] = (isNA(v1[i]) || v1[i] == 0.)
                        ? getNA<T>()
                        : static_cast<T>(v2) / v1[i];
        }
        else
        {
          for (Id i = 0; i < n1; i++)
            vecout[i] = (isNA(v1[i]) || static_cast<T>(v2) == 0.)
                        ? getNA<T>()
                        : v1[i] / static_cast<T>(v2);
        }
      }
    }
    ...
    template<typename T>
    static void
      divideAssign(VectorNumT<T>& vecout, const VectorNumT<T>& v1, T v2)
    {
  ...
  template<typename T>
  inline VectorNumT<T>
    operator+(const VectorNumT<T>& v1, const VectorNumT<T>& v2)
  {
    VectorNumT<T> result;
