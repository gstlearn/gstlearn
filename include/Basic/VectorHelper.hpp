...
namespace gstlrn
{
  ...
  public:
    ...
    static void capInPlaceVVD(VectorVectorDouble& vec,
                              double vmin = TEST,
                              double vmax = TEST);
    static double
    ...
    static void cumulate(VectorDouble& veca,
                         ...
                         double coeff = 1.,
                         double addval = 0.);
    static void getMostSignificant(const VectorDouble& vec,
                                   double tol = EPSILON6,
                                   Id nmax = -1);
...
#ifndef SWIG
    template<typename T>
    static void add(VectorNumT<T>& vecout,
                    ...
                    const VectorNumT<T>& v2)
    {
      ...
      if (n1 != n2)
      {
        messerr("Impossible to add vectors of different dimensions: V1(%d) and "
                "V2(%d)\n",
                n1,
                n2);
        return;
    ...
    template<typename T>
    static void subtract(VectorNumT<T>& vecout,
                         const VectorNumT<T>& v1,
                         const VectorNumT<T>& v2)
    {
      static_assert((std::is_same_v<T, Id> && std::is_integral_v<T>)
      ...
      if (n1 != n2)
      {
        messerr("Impossible to subtract vectors of different dimensions: "
                "V1(%d) and V2(%d)\n",
                n1,
                n2);
        return;
    ...
    template<typename T>
    static void multiply(VectorNumT<T>& vecout,
                         const VectorNumT<T>& v1,
                         const VectorNumT<T>& v2)
    {
      static_assert((std::is_same_v<T, Id> && std::is_integral_v<T>)
      ...
      if (n1 != n2)
      {
        messerr("Impossible to multiply vectors of different dimensions: "
                "V1(%d) and V2(%d)\n",
                n1,
                n2);
        return;
    ...
    static void multiply(VectorNumT<T>& vecout, const VectorNumT<T>& v1, U v2)
    {
      static_assert(std::is_same_v<T, U>
                      ...
                      || (std::is_same_v<T, double> && std::is_arithmetic_v<U>),
                    "Invalid type for multiplication operator between "
                    "VectorNumT and scalar");
      auto n1 = static_cast<Id>(v1.size());
      if (&vecout != &v1) vecout.resize(n1);
      if (isNA(v2))
        vecout.fill(getNA<T>());
    ...
    template<typename T>
    static void divide(VectorNumT<T>& vecout,
                       const VectorNumT<T>& v1,
                       const VectorNumT<T>& v2)
    {
      static_assert((std::is_same_v<T, Id> && std::is_integral_v<T>)
      ...
      if (n1 != n2)
      {
        messerr("Impossible to divide vectors of different dimensions: V1(%d) "
                "and V2(%d)\n",
                n1,
                n2);
        return;
