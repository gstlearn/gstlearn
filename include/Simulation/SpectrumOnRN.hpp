...
public:
...
#endif
  virtual void _computeValues(const VectorDouble& coor, VectorDouble& values) = 0;
  ...
  void compute(Db* dbout, const VectorBool& activeArray, VectorVectorDouble& tab);
  ...
  bool _isValidNSpectrum(Id i) const { return (i >= 0) && (i < getNSpectrum()); };
...
protected:
  ...
  // working variables
  VectorDouble _u; // working vector of length _ns used by the method _computeValues
  VectorDouble _v; // working vector of length _ns used by the method _computeValues
  double _SQRT2;
};
...
#endif
  void _computeValues(const VectorDouble& coor, VectorDouble& values) override;
...
protected:
#ifndef SWIG
  std::vector<std::unique_ptr<SpectrumOnRN>> _sps; // vector of elementary spectrums
#endif
