...
namespace gstlrn
{
  ...
  public:
    ...
    KrigingSystemSimpleCase&
      operator=(const KrigingSystemSimpleCase& m) = delete;
    ...
    Id getNDim() const
    {
      return (_model != nullptr) ? static_cast<Id>(_model->getNDim()) : 0;
    }
  ...
  private:
    ...
    void
      _estimateCalcul(Id status, Id iechout, KrigingAlgebraSimpleCase& algebra)
        const;
    void _simulateCalcul(Id status);
    void _neighCalcul(Id status, const VectorDouble& tab, Id iechout);
    void _estimateVarZ(Id status, Id iechout, KrigingAlgebraSimpleCase& algebra)
      const;
    void _estimateStdv(Id status, Id iechout, KrigingAlgebraSimpleCase& algebra)
      const;
    void
      _estimateEstim(Id status, KrigingAlgebraSimpleCase& algebra, Id iechout)
        const;
    void _dumpKrigingResults(
      Id status,
      Id iechout,
