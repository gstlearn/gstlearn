...
namespace gstlrn
{
  ...
  private:
    ...
    Id ISC(Id ic, Id i) const
    {
      return _startIso + _ptrPerLayer[ic] + (i) - (ic)-1;
    }
    ...
    double IAD_ISO(Id ic, Id i) const
    {
      return _rankIso[_ptrPerLayer[ic] + (i)];
    }

    double TGT_COO(Id it, Id i) const
    {
      return _dbtgt->getCoordinate(IAD_TGT(it), i);
    }

    double TGT_VAL(Id it, Id idim) const
    {
      return (idim >= _ndim)
             ? 0.
    ...
    {
      return (idim >= _ndim) ? 0. : _dbgrd->getCoordinate(IAD_GRD(ig), idim);
    }

    double GRD_VAL(Id ig, Id idim) const
    {
      return (idim >= _ndim)
             ? 0.
             : _dbgrd->getLocVariable(ELoc::G, IAD_GRD(ig), idim);
    }

    double ISO_COO(Id ic, Id j, Id idim) const
    {
      return (idim >= _ndim) ? 0. : _dbiso->getCoordinate(IAD_ISO(ic, j), idim);
    }

    Id EXT(Id iext) const { return _startExt + (iext); }

    bool _isEnvironmentValid(DbGrid* dbout, Id nring = 1);
    void _environmentManage(
      bool flag_pot,
      bool flag_grad,
      bool flag_trans,
      Id option_part = 0);
    ...
    void _calculatePoint(
      ...
      Id iech0,
      VectorDouble& result);
    void _convertPotentialToLayer(
      Id isimu,
      const double* potval,
      VectorDouble& result) const;
    void _estimateResult(
      bool flag_grad,
      DbGrid* dbout,
      double refpot,
      VectorDouble& zdual,
    ...
    void _convertDistance(
      ...
      double* dist_euc,
      double* dist_geo);
    void _xvalidCalculate(
      MatrixSymmetric& lhs,
      bool flag_dist_conv,
      VectorDouble& zval,
      MatrixSymmetric& lhs_orig,
      MatrixDense& rhs);
    static void _tempere(
      DbGrid* dbout,
      Id iech,
      double dist_tempere,
      double reskrige,
    ...
    double
      _evaluateRefPot(DbGrid* dbgrid, VectorDouble& zdual, MatrixDense& rhs);
    void _evaluatePotential(
      DbGrid* dbgrid,
      double refpot,
      Id isimu,
      Id nbsimu,
  ...
  GSTLEARN_EXPORT Id krigingPotential(
    Db* dbiso,
    Db* dbgrd,
    Db* dbtgt,
    DbGrid* dbout,
    ...
    Id option_part = 0,
    bool verbose = false);
  GSTLEARN_EXPORT Id simulatePotential(
    Db* dbiso,
    Db* dbgrd,
    Db* dbtgt,
    DbGrid* dbout,
    ...
    Id nbtuba = 100,
    bool verbose = false);
  GSTLEARN_EXPORT Id xvalidPotential(
    Db* dbiso,
    Db* dbgrd,
    Db* dbtgt,
    ModelGeneric* model,
