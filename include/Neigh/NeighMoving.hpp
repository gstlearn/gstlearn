...
namespace gstlrn
{
  ...
  public:
    ...
    {
      return _biPtDist->getAnisoRotMats();
    }

    const VectorDouble& getAnisoCoeffs() const
    {
      return _biPtDist->getAnisoCoeffs();
    }
    ...
    VectorVectorDouble
      getEllipsoid(const VectorDouble& target, Id count = 360) const;
    VectorVectorDouble getSectors(const VectorDouble& target) const;
    VectorVectorDouble
      getZoomLimits(const VectorDouble& target, double percent = 20) const;
