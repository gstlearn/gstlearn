...
The classes and functions documentation is provided directly in the package as "DocStrings". Users can directly access to the documentation of a class (i.e. 'Vario') by typing the python command `help(Vario)`. The full python package documentation is also available in html files generated by Sphinx [See here](https://gstlearn.org/) for more details.
...
For building python package documentation, additional requirements are needed:

* Sphinx (with sphinx_rtd_theme, sphinx-cmake) and  myst_parser packages
...
python3 -m pip install sphinx sphinx_rtd_theme sphinx-cmake myst_parser
````
...
python -m pip install sphinx sphinx_rtd_theme sphinx-cmake myst_parser
````
