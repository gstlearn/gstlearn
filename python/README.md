...
1. For building the *gstlearn* Python package, the requirements for compiling *gstlearn C++ library* must be installed: See [instructions here](https://github.com/gstlearn/gstlearn#required-tools-installation)
  
2. The following tools must be also available (See [required tools installation](#required-development-tools-installation) instructions below):
