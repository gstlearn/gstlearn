...
if out_type == "asciidoc":
    ...
    notebook_node = re.sub(
        r".*data:image/png;base64,.*", "data:image/png;base64,XXX", notebook_node
    )
    ...
    notebook_node = re.sub(
        r".*CholmodTypeConversionWarning",
        "XXX: CholmodTypeConversionWarning",
        notebook_node,
    )
    ...
    notebook_node = re.sub(
        r".*site-packages is not writeable", "#NO_DIFF#XXX", notebook_node
    )
    notebook_node = re.sub(
        r"Requirement already satisfied.*", "#NO_DIFF#XXX", notebook_node
    )
    # Remove this: +*Out[3]:*+
