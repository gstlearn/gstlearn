...
#' @param names a list of strings with the radix of the simulation in db
...
qc_simulations <- function(db, names, nbsimu, vario_param, vario_model, title, stat_keys) {
  # statistics
  ...
  i = 1
  p = plot.init() +
    plot.hist(db = db, name = names, useSel = TRUE, bins = 25,
              col = "orange", bg = "gray") +
    plot.decoration(xlab = names, title = title)
  plot.end(p)

  # base map
  p = plot.init(asp=1)
  if (db$isGrid()) {
    p = p + plot.raster(db, name = names, flagLegend = TRUE,
  ...
  for (i in 1:nbsimu) {
    ...
    db$extractVersion(names, "Local", version = i-1)
    err   = db$setLocator(name = names, locatorType = ELoc_Z(),
                          cleanSameLocator = TRUE)
    err   = vario$compute(db)
    vario_list[[1 + length(vario_list)]] <- vario
    db$deleteColumn("Local")
  }
...
# variograms
qc_simulations(db = data, names = prefix, nbsimu = nbsimu,
               vario_param = varioParam, vario_model = model,
               title = paste0("Gibbs sampler: np = ", np), stat_keys = stat_keys)
# statistics
tab = dbStatisticsMono(data, names = prefix, opers = opers)$toTL()
...
for (s in 1:nbsimu){
  vn = paste(prefix, s, sep = ".S")
  datavn = data$getColumn(prefix, version=s-1)
  qc_inBounds[s] <- sum(
    (data["upperBound"] >= datavn)&(data["lowerBound"] <= datavn),
    na.rm = TRUE)
}
