...
```{r}
filepath = paste(Sys.getenv('GSTLEARN_DATA'),"Pollution","Pollution.dat",sep="/")

mydb = Db_createFromCSV(filepath,CSVformat())
err = mydb$setLocators(c("X","Y"),ELoc_X())
...
#err = mymodel$fit(myvario,c(ECov_NUGGET(), ECov_EXPONENTIAL()))
```
...
We initiate a Neighborhood (Moving with a small number of samples for Demonstration)
...
```{r}
mydb$setLocator("Pb",ELoc_Z())
err = RawToGaussianByLocator(mydb, myanamPb)
if (verbose)
    mydb$display()
```
...
```{r}
mydb$setLocator("Zn",ELoc_Z())
err = RawToGaussianByLocator(mydb, myanamZn)
if (verbose)
    mydb$display()
```
...
if (verbose)
  err = mygrid$statisticsByLocator(ELoc_Z(),
                                   EStatOption_fromValues(c(4,5,1,3)), 
                                   flagIso=TRUE, flagVariableWise=TRUE, flagPrint=TRUE)
```
...
if (verbose)
    dbfmt = DbStringFormat()
    dbfmt$setFlags(flag_resume=FALSE, flag_vars=FALSE, flag_extend=FALSE, flag_stats=TRUE)
