...
ECov_fromValue(2)$getDescr()
ECov_fromValues(c(1,3,4))
ECov_printAll()
```
...
opt = Option_AutoFit()
err = myModelConstrained$fit(myVarioOmni,covlist,FALSE,opt,constr)
myModelConstrained
```
...
```{r}
statOptions = VectorString()
statOptions$push_back("mini")
statOptions$push_back("maxi")
statOptions$push_back("mean")
statOptions$push_back("stdev")

if (verbose)
{
  names = VectorString()
  names$push_back("Simu.Y.*")
  err = mygrid$statistics(names, statOptions)
#  err = mygrid$statistics(names, statOptions, TRUE, TRUE,TRUE)
}
```
...
statOptions = VectorString()
statOptions$push_back("Mean")
err = mygrid$statistics(names,statOptions,TRUE,FALSE,FALSE)
if (verbose)
    mygrid$display()
```
