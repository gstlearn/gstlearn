...
```{r}
dataDir = "/home/drenard/project_gstlearn/gstlearn/doc/data/Pollution"
...
mydirs = DirParam_createMultiple(2, 4, 10, 1.)
...
```{r}
if (graphics)
    axs = plot.varmod(myvario,title="Multi-Directional Variogram of Pb")
```
...
**
Impossible to call *fit* as we need to pass a std::vector<ECov>
**

```{r}
mymodel = Model_createFromDb(mydb)
#err = mymodel$fit(myvario,c(ECov_EXPONENTIAL(),ECov_SPHERICAL()))
```

An artificial model is created on the fly
```{r}
mymodel$addCovFromParam(ECov_SPHERICAL(),range=10,sill=2)
```
...
opt = Option_AutoFit()
err = myModelConstrained$fit(myVarioOmni,c(ECov_SPHERICAL()),
                             FALSE,opt,constr)
myModelConstrained
```
...
```{r}
mygrid = DbGrid()
mygrid$resetCoveringDb(mydb,c(80,72),c(0.5,0.5),c(107.,481.),c(2,2))
if (verbose)
    mygrid$display()
```
