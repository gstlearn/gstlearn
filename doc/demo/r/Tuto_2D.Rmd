...
err = myVarioParamOmni$addDir(mydir)
```
...
```{r}
myvmap = vmapFromDb(db=mydb, nxx=c(20,20))
if (verbose)
    err = myvmap$display()
```
...
if (graphics)
  plot.init() +
  plot.varmod(myvario,mymodelG) +
  plot.decoration(title="Model for Gaussian Pb")
```
...
```{r}
if (verbose)
  err = dbStatisticsMono(mygrid, mygrid$getNamesByLocator(ELoc_Z()),
...
{
    p = plot.init()
    p = p + plot(mygrid,name="Simu.Y.Pb")
...
err = mygrid$deleteColumn("Simu.Y.*")
if (verbose)
    er = mygrid$display()
```
...
{
  p = plot.init()
  p = p + plot(mygrid,name="Z.Simu.Y.Pb")
...
```{r}
err = mygrid$statisticsBySample(mygrid$getNamesByLocator(ELoc_Z()),
...
```{r}
err = mydb$setLocator("Y.Pb",ELoc_Z(),0)
err = mydb$setLocator("Y.Zn",ELoc_Z(),1)
myvario = Vario(myvarioParam)
err = myvario$compute(mydb,ECalcVario_VARIOGRAM())
...
```{r}
err = mygrid$deleteColumn("Z.Simu*")
