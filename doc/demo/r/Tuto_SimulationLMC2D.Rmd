...
err = dat$setColumn(tab = rnorm(ndat), name = "Z")
err = dat$setLocator(name = "Z", locatorType = ELoc_Z());
...
# QC of simulations
# ------------------------------------------------------------
qc_simulations <- function(grid, names, nsim, var_model, title, stat_keys) {
  ...
  i = 1
  hist(grid[names[i]], xlab = names, main = title, nc = 25, col = "orange", bg = "gray")
  # p = plot.init() +
  ...
  # base map
  p = plot.init(asp=1) +
    plot.raster(grid, name = names, flagLegend = TRUE, legendName = "Y",
                palette = "Spectral") +
    # plot.symbol(dat, color = "red") +
    plot.decoration(xlab = "Easting", ylab = "Northing",
                    title = title)
  ...
  for (i in 1:nsim) {
    ...
    err = grid$extractVersion(names, "Local", version = i-1)
    err   = grid$setLocator(name = "Local", locatorType = ELoc_Z(), cleanSameLocator = TRUE)
    err   = vario$compute(grid)
    vario_list[[1 + length(vario_list)]] <- vario
    grid$deleteColumn("Local")
  }
...
for (i in seq_along(list_title)) {
  ...
  err = qc_simulations(grid = grid, ssname, nsim,
...
err = grid$statisticsBySample("SPDE.Z",opers=EStatOption_fromKeys(c("MEAN", "STDV")),
                        namconv = NamingConvention("SPDE.ZMC"))

# conditional simulations
p1 = plot.init(asp=1) + plot.raster(grid, name = "SPDE.Z", version=0, palette = "Spectral")
p2 = plot.init(asp=1) + plot.raster(grid, name = "SPDE.Z", version=1, palette = "Spectral")
p3 = plot.init(asp=1) + plot.raster(grid, name = "SPDE.Z", version=2, palette = "Spectral")
p4 = plot.init(asp=1) + plot.raster(grid, name = "SPDE.Z", version=3, palette = "Spectral")
ggarrange(p1, p2, p3, p4, nrow = 2, ncol = 2)

# estimate by MC
p = plot.init() +
  plot.correlation(grid, namex = "SPDE.Z.estim", namey = "SPDE.ZMC.MEAN",
                   flagDiag = TRUE, diagColor = "red", flagSameAxes = TRUE, bins=100) +
...
# stdev
p = plot.init() +
  plot.correlation(grid, namex = "kriging.Z.stdev", namey = "SPDE.ZMC.STDV",
                   flagDiag = TRUE, diagColor = "red", flagSameAxes = TRUE, bins=100) +
...
p1 = plot.init(asp=1) +
  ...
  plot.decoration(xlab = "Easting", ylab = "Northing")
p2 = plot.init(asp=1) +
  plot.raster(grid, name = "SPDE.ZMC.STDV",
              palette = "Spectral", flagLegend = TRUE, legendName = "SPDE") +
  plot.decoration(xlab = "Easting", ylab = "Northing")
ggarrange(p1, p2, nrow = 1, ncol = 2)
```
...
p1 = plot.init(asp=1) +
  ...
  plot.decoration(xlab = "Easting", ylab = "Northing")
p2 = plot.init(asp=1) +
  plot.raster(grid, name = "SPDE.ZMC.MEAN",
              palette = "Spectral", flagLegend = TRUE, legendName = "SPDE") +
  plot.decoration(xlab = "Easting", ylab = "Northing")
ggarrange(p1, p2, nrow = 1, ncol = 2)
