...
plot.grid(grid, name = "Z", asp = 1)
plot.hist(grid, name = "Y", xlab = "Y", title = "Simulated samples (Y variable)", nbins = 100, 
          col = "gray", fill = "skyblue")
plot.hist(grid, name = "Z", xlab = "Z", title = "Simulated samples (Z variable)", nbins = 100, 
          col = "gray", fill = "orange")

stats = dbStatisticsMonoT(grid, c("Y","Z"),
                       opers=EStatOption_fromKeys(c("NUM","MINI","MAXI","MEAN","STDV")))$toTL()
knitr::kable(x = round(
  dbStatisticsMonoT(grid, c("Y","Z"),
    opers=EStatOption_fromKeys(c("NUM","MINI","MAXI","MEAN","STDV")))$toTL(), 2),
...
plot.point(data, color_name = "Z", size_name = "Z", show.legend.color = TRUE)
plot.hist(data, name = "Y", xlab = "Y", title = "Sampled data (Y variable)", nbins = 100, 
          col = "gray", fill = "skyblue")
plot.hist(data, name = "Z", xlab = "Z", title = "Sampled data (Z variable)", nbins = 100, 
          col = "gray", fill = "orange")

stats = dbStatisticsMonoT(data, c("Y","Z"),
                       opers=EStatOption_fromKeys(c("NUM","MINI","MAXI","MEAN","STDV")))$toTL()
knitr::kable(x = round(stats, 2),
...
knitr::kable(x = round(selectivity$eval(data)$toTL(), 3),
...
knitr::kable(x = round(selectivity$evalFromAnamorphosis(anam)$toTL(), 3),
...
plot.hist(data, name = "Gaussian.Z", nbins = 25, col = "gray", fill = "yellow", 
          xlab = "Normal score")
data$display()
```
...
plot.varmod(vario_raw, model_raw, title = "Raw variable")
```
...
model$display()
plot.varmod(vario, model, title = "Gaussian variable")
...
plot.varmod(vario_raw, model, title = "Raw variable (model of the Gaussian)")
...
plot.grid(blocs, name = "G_Pts.Gaussian.Z.estim", title = "Punctual Simple Kriging of Y",
          legend.name = "Estimation")
plot.grid(blocs, name = "G_Pts.Gaussian.Z.stdev", title = "Punctual Simple Kriging of Y",
          legend.name = "Std. Dev.")
```
...
plot.grid(blocs, asp = 1, show.legend = TRUE, name = "Pts_Recovery.Q-estim-0",
          title = "Point conditional expectation",
          legend.name = "Q(0.0)")
# Tonnage (the anamorphosis is not used)
plot.grid(blocs, asp = 1, show.legend = TRUE, name = "Pts_Recovery.T-estim-0",
          title = "Point conditional expectation",
          legend.name = "T(0.0)")
plot.grid(blocs, asp = 1, show.legend = TRUE, name = "Pts_Recovery.T-estim-1",
          title = "Point conditional expectation",
          legend.name = "T(1.0)")
```
...
blocs["T_1"][indsel] = NA
plot.correlation(blocs, name1="Pts_Recovery.T-estim-1", name2="T_1", 
                 xlab = "Computed value", ylab = "Theoretical value",
...
plot.correlation(blocs, name1="Pts_Recovery.T-estim-0.75", name2="T_0.75", 
                 xlab = "Computed value", ylab = "Theoretical value",
...
plot.varmod(vario, model, title = "Variogram of the Gaussian variable")
...
model$setActiveFactor(-1)
plot.varmod(vario_raw, model, title = "Variogram of the raw variable")
model$setActiveFactor(0)
```
...
for (i in 1:nf) 
{
  ...
  ax <- plot.model(model, hmax=hmax, asCov = asCov, end.plot = FALSE, padd=ax,
                 title = "Covariance of the different factors")
}
ax = ax + geom_hline(yintercept = 1.0, colour = "red")
plot(ax)

# model of the raw variable
model$setActiveFactor(-1)
ax = plot.model(model, hmax=hmax, asCov = asCov, end.plot = FALSE,
                 title = "Covariance of the raw variable")
ax = ax + geom_hline(yintercept = varZ, colour = "red")
plot(ax)
...
```{r}
# H_1 = -Y
plot.correlation(blocs, 
                 name1 = "G_Pts.Gaussian.Z.estim",
                 name2 = "DK_Pts.F.1.estim", 
                 xlab = "Gaussian variable",
                 ylab = "H_1 = -Y",
                 title = "Values of the Simple Kriging",
                 flagDiag = FALSE, end.plot = FALSE) + geom_abline(intercept = 0, slope = -1, colour = "red")
plot.correlation(blocs, 
...
```{r, echo=TRUE, eval=TRUE}
plot.correlation(blocs, name1 = "DK_Pts*1.estim", name2 = "DK_Blk*1.estim",
                 xlab = "Point estimation", ylab = "Block estimation", 
                 flagDiag = TRUE,
                 title = "First factor (Gaussian model)")
plot.correlation(blocs, name1 = "DK_Pts*2.estim", name2 = "DK_Blk*2.estim",
                 xlab = "Point estimation", ylab = "Block estimation", 
                 flagDiag = TRUE,
                 title = "Second factor (Gaussian model)")
plot.correlation(blocs, name1 = "DK_Pts*3.estim", name2 = "DK_Blk*3.estim",
                 xlab = "Point estimation", ylab = "Block estimation", 
                 flagDiag = TRUE,
...
plot.grid(panels,
          ...
          name = "DK_Blk.*.estim", show.legend = TRUE )

plot.grid(panels,
          xlab = "Easting", ylab = "Northing", asp = 1,
          title = "Std. of the first factor estimate (panels)",
          name = "DK_Blk.*.stdev", show.legend = TRUE )
```
...
grid$deleteColumns("SK.*")
grid$deleteColumns("CE.*")
grid$deleteColumns("DK.*")
grid$deleteColumns("sel-.*")
# Selectivity curve : Z
...
knitr::kable(x = round(Z_est$eval(grid)$toTL(), 3),
...
grid$setName("DK.Z-estim", "DK.Z.estim")
grid$setName("DK.Z-stdev", "DK.Z.stdev")
...
plot(Y, H %*% psi.LN, type = "l", col = "red", xlab = "Gaussian value", ylab = "Raw value", main = paste0("Lognormal m=", m, " and sigma = ", sigma))
lines(Y, m * exp(sigma*Y - 1/2 * sigma^2), col = "blue", lwd = 2, lty = 2)
legend("topleft", legend = c("Hermite", "Theoretical"), col = c("red", "blue"), lty = c(1,2))
...
plot(Y, H %*% psi.ind, type = "l", col = "red", xlab = "Gaussian value", ylab = "Raw value", main = paste0("Indicator of ", yc))
...
plot(Y, H %*% psi.metal, type = "l", col = "red", xlab = "Gaussian value", ylab = "Raw value", main = paste0("Metal for cutoff = ", yc))
lines (Y, m * exp(sigma * Y - 1/2 * sigma^2) * as.numeric(Y >= yc), col = "blue", lty = 2)
legend("topleft", legend = c("Hermite", "Theoretical"), col = c("red", "blue"), lty = c(1,2))
...
# Raw variable
plot.correlation(grid, name1 = nm_estim, name2 = nm_qc_estim, 
                 xlab = nm_estim, ylab = nm_qc_estim,
                 title = "Disjunctive Kriging - Estimation value",
                 flagDiag = TRUE)
plot.correlation(grid, name1 = nm_stdev, name2 = nm_qc_stdev, 
                 xlab = nm_stdev, ylab = nm_qc_stdev,
...
for (est in c("SK", "DK", "CE")) {
  plot.correlation(grid, name1=paste0(est, ".Z.estim"), name2="Z", flagDiag = TRUE,
...
plot.correlation(grid, name1="CE.Z.estim", name2="SK.Z.estim", flagDiag = TRUE,
                 xlab = "Conditional expectation",
                 ylab = "Simple kriging",
                 title = "Point raw variable")

plot.correlation(grid, name1="DK.Z.estim", name2="SK.Z.estim", flagDiag = TRUE,
                 xlab = "Disjunctive Kriging",
                 ylab = "Simple kriging",
                 title = "Point raw variable")
...
for (est in c("SK", "DK", "CE")){
  ...
  plot.grid(grid, name = paste0(est, ".Z.", type), 
            xlab = "Easting", ylab = "Northing",
            title = paste("Estimator = ", est), legend.name = type) 
  
  if (type == "estim") col_fill = "orange"
  if (type == "stdev") col_fill = "skyblue"
  
  plot.hist(grid, name = paste0(est, ".Z.", type), nbins = 50, col = "gray", fill = col_fill,
            xlab = paste0(est, ".Z.", type),
...
knitr::kable(x = round(
  dbStatisticsMonoT(grid, nm,
    opers=EStatOption_fromKeys(c("NUM","MINI","MAXI","MEAN","STDV")))$toTL(), 3),
