...
```{r dgm_center_fn, echo=FALSE, eval=TRUE}
# Centering the data on the target grid
dgm_center <- function(data, target) {
  err = 0.0
  if (target$isGrid()) {
    dx = target$getDXs()
    x0 = target$getX0s()
...
UV_to_XY <- function(target, UV) {
  ...
  r   = t(matrix(rot$getMatrixDirectByVector(), nrow = target$getNDim(), ncol = target$getNDim()))
...
XY_to_UV <- function(target, XY) {
  ...
  rot = Rotation(); rot$setAngles(target$getAngles())
  r   = (matrix(rot$getMatrixDirectByVector(), 
...
bounding_xy <- function(target, delta = 0) {
  ...
  bb_uv = bb_uv %*% t(matrix(rot$getMatrixDirectByVector(), nrow = 2, ncol = 2))
...
```{r test_centering_scattered, echo=TRUE, eval=TRUE}
data_man1 = data_ini1$clone()
err = dgm_center(data_man1, target)
```
...
err = db_center_point_to_grid(data_auto1, target)
```
...
err = dgm_center(data_man2, target)
```
...
err = db_center_point_to_grid(data_auto2, target)
```
