...
p = ggDefaultGeographic() + 
    ...
    plot.decoration(xlab = "Easting", ylab = "Northing", title = "Kriging (Manual)")
ggPrint(p)
```
...
p = ggDefault() + 
    plot.correlation(db1 = grid, namex = "manual.kriging", namey = "manual.simu", asPoint = FALSE, 
                     flagDiag = TRUE, diagColor = "red", flagSameAxes = FALSE, bins=100) +
...
p = ggDefault() + 
    plot.correlation(db1 = grid, namex = "manual.kriging", namey = "manual.cond.simu", 
                     asPoint = FALSE, 
                     flagDiag = TRUE, diagColor = "red", flagSameAxes = FALSE, bins=100) +
...
p = ggDefault() + 
    plot.correlation(db1 = grid, namex = "manual.cond.expectation", namey = "manual.kriging", 
                     asPoint = FALSE, 
                     flagDiag = TRUE, diagColor = "red", flagSameAxes = FALSE, bins=100) +
...
# plot
p = ggDefaultGeographic() + 
    plot.grid(grid, nameRaster = "spde", useSel = TRUE, 
            flagLegendRaster = TRUE, palette = "Spectral") +
  plot.decoration(xlab = "Easting", ylab = "Northing", 
...
err = grid$setName(old_name = "spde.manual.simu.estim", name = "spde.kriging")

# plot of kriging
p = ggDefaultGeographic() + 
    plot.grid(grid, nameRaster = "spde.kriging", useSel = TRUE, palette = "Spectral",
            flagLegendRaster = TRUE) +
    plot.point(dat, col = "red") +
    plot.decoration(xlab = "Easting", ylab = "Northing", 
                  title = "Kriging (SPDE)")
...
# correlation
p = ggDefault() + 
    plot.correlation(grid, namex = "spde.kriging", namey = "manual.simu",asPoint = FALSE,
                     flagDiag = TRUE, diagColor = "red", flagRegr = TRUE, 
                     regrColor = "black", regrLinetype = 2,
                     flagSameAxes = TRUE, bins=100
            ) +
...
# cross plot
p = ggDefault() + 
    plot.correlation(db1 = grid, namex = "manual.kriging", namey = "spde.kriging", asPoint = FALSE, 
                     flagDiag = TRUE, diagColor = "red", flagSameAxes = FALSE, bins=100) +
...
print(paste0("Quadratic form: relative difference = ", 
             round(100*(quad_api-quad_chol)/quad_chol,2),"%"))
...
print(paste0("relative difference = ", round(100*(b-a)/a, 2),"%"))
...
print(paste0("relative difference = ", round(100*(logdet_api/logdet-1),4),"%"))
...
print(paste0(100*round((b/a-1),2),"%"))
...
monoSensitivity <- function(name, from, to, ncalc, actual, p0, seed = 0, nsim = 10, 
                            flag.verbose = FALSE, 
...
if(flag.test) {
  ...
  print(paste0("Using SPDE api: LL = ", computeLL(par = param, flag.chol = FALSE, nsim = 10, 
                                                  seed = 1212)))
}
```
