...
spde = SPDE()
err  = spde$init(model, field = gridExt, calc = ESPDECalcMode_SIMUNONCOND(), mesh = mesh)
...
spdeRes = SPDE()
err     = spdeRes$init(model, field = gridExt, data = dat, calc = ESPDECalcMode_KRIGING(), mesh = mesh)
err     = spdeRes$compute()
...
a = detQ(cholQ_TT)
b = pcm$computeLogDetOp(nsimus = nsim, seed = 0)
...
a = detQ(cholQ)
b = pcm$computeLogDetQ(nsimus = nsim, seed = 0)
...
# computing the logdet using API (TODO: SPDE$computeLogDet(nbsimus, seed) returns -logdet)
logdet_api = spdeRes$computeLogDet(nbsimus = nsim, seed = 0)
...
a = - 1/2 * (-logdet + quad)
b = spdeRes$computeLogLike(nbsimus = nsim, seed = 0)
...
computeLL <- function(par, flag.chol = TRUE, nsim  = 0, seed = 0) {
  ...
  spde = SPDE()
  err  = spde$init(field = gridExt, mesh = mesh, model = m, data = dd, calc = cc)
  err  = spde$compute()
  ...
  if (flag.chol) {
    ...
    ll  = spde$computeLogLike(nbsimus = nsim, seed = seed)
  }
  ll
}
