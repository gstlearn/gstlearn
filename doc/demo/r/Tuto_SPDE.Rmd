...
spde = SPDE(model, field = gridExt, calc = ESPDECalcMode_SIMUNONCOND(), mesh = mesh)
...
spdeRes = SPDE(model, field = gridExt, data = dat, calc = ESPDECalcMode_KRIGING(), mesh = mesh)
err     = spdeRes$compute()
...
computeLL <- function(par, flag.chol = TRUE, nsim  = 0, seed = 0) {
  ...
  spde = SPDE(field = gridExt, mesh = mesh, model = m, data = dd, calc = cc)
  err  = spde$compute()
