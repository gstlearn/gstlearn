...
for (i in 1:N) {
  val = matrix(nrow = surfaces$getNSample(useSel = FALSE), ncol = nbsim[i])
  for (k in 1:nbsim[i]) {
    val[,k] = surfaces$getColumn(name = paste0("Y_", i), version=k-1, useSel=FALSE)
  }
...
```{r}
err = surfaces$clearSelection()
err = simuPostPropByLayer(dbin = surfaces, dbout = grid,
                          names = paste0("Z_", 1:length(nbsim), "*"),
                          flag_match=FALSE,
