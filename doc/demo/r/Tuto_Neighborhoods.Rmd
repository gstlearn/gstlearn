...
for (node in 1:nech){
  edges = data.frame(grid$getCellEdges(node))
...
ggDefaultGeographic()+
  ...
  plot.point(dataSel,
             color='blue')+
  plot.neigh(neigh, grid, node, flagCell=TRUE)+
  plot.decoration(title="Standard Neighborhood")
...
plotting the result 
Chunk 17 isn't working. More precisely, it appears that plot.neigh presents an issue when trying to represent anything but a standard moving neighborhood with a grid as a "grid" parameter. PLease wait until the next gstlearn version release to have that problem fixed. 
An alternative would be to use ggplot meanwhile. 
```{r}
...
```
Again, the plot.neigh doesn't work. The concerned lines were disabled. The corresponding line was disabled until the code is corrected. Apparently this is due to the absence of a condition in the code of plot.neigh (more precisely, getCellEdges doesn't apply to a db which is not a grid)
...
```
Similarily to chunk 22, it doesn't work. The concerned lines were disabled
...
```
Again, the problematic lines were disabled 
