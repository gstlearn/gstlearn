...
It will also demonstrate the quality of the polynomial expandsion for a gien known distribution of the data (i.e. lognormal).
...
mod_y$addCova(ECov_SPHERICAL(),range=1,sill=1)
h     <- seq(from = 0, to =1, by = 0.01)
r = mod_y$getCovAnisoList()$evalIvarNpas(0,0,h)
C_theo<- GM_LN_Cov_theo(lambda = lambda)
...
model.eval.GM <- function(model, anam, h){
  r = model$getCovAnisoList()$evalIvarNpas(0,0,h)
  psi <- anam$getPsiHn()
  nh <- length(psi) - 1
