...
p1 = plot.init(asp=1) + plot.raster(grid, name="SimuSPDE.S1")
p2 = plot.init(asp=1) + plot.raster(grid, name="SimuSPDE.S2")
p3 = plot.init(asp=1) + plot.raster(grid, name="SimuSPDE.S3")
p4 = plot.init(asp=1) + plot.raster(grid, name="SimuSPDE.S4")
...
```{r Data_extraction}
data = Db_createSamplingDb(grid, number=100, names=c("x1", "x2", "SimuSPDE.S1"))
err = data$setName("*SimuSPDE.S1", "data")
data
```
...
p1 = plot.init(asp=1) + plot.raster(grid, name="SimuSPDE.data.S1")
p2 = plot.init(asp=1) + plot.raster(grid, name="SimuSPDE.data.S2")
p3 = plot.init(asp=1) + plot.raster(grid, name="SimuSPDE.data.S3")
p4 = plot.init(asp=1) + plot.raster(grid, name="SimuSPDE.data.S4")
ggarrange(p1, p2, p3, p4, ncol = 2, nrow = 2)
```
