...
plot.decoration(p,title="Display of Grid Cells")
```
...
```{r}
p = plot.grid(mygrid, name_raster="Simu.1")
...
```{r}
p = plot(mygrid)
p = plot(mypoly, fill=NA, colour='yellow', linewidth=2, padd=p)
...
```{r}
p = plot(vario, ivar=1,jvar=0,idir=-1)
p = plot.decoration(p, title="Cross-variogram - Two directions")
p
```

```{r}
p = plot(vario, ivar=-1,jvar=-1,idir=-1)
...
```{r}
p = plot(model,ivar=1,jvar=1, idir=0)
...
Testing figure Overlay. This does not function correctly (yet).

```{r}
p = plot(vario, idir=0,ivar=-1,jvar=-1)
p = plot.decoration(p, title="Overlay test")
p = plot(model,ivar=1,jvar=1,codir=vario$getCodirs(0),
                color='blue', padd=p$layers[[1]])
```
