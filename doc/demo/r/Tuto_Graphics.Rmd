...
Using another collor scale. Among the different choices, one makes it possible to create a color scale on the fly, specifying the starting (yellow), middle (red) and ending (blue) colors.

```{r}
ggDefault(2) + plot.grid(mygrid, palette=c("yellow","red","blue"), na.value="yellow")
```
...
```{r}
p1 = ggDefault(2) + plot(mygrid)
p2 = ggDefault(2) + plot(mypoint)
