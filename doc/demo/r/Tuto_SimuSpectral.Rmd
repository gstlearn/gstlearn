...
grid$setName("Simu", "SimuRef")
stopifnot (all(abs(val - grid["SimuRef"]) < 1e-6))
...
# statistics
knitr::kable(dbStatisticsMono(db = grid, names = paste0(prefix, "*"), opers = opers)$toTL(),
             digits = 3, caption = title)

var_name = NC_getNameEncoded(prefix, NULL, 1, 1, 1, ns)
err = grid$extractVersion("Simu", var_name, version=0)
...
grid$deleteColumn(var_name)

# variograms on all simulations
vl = list()
for (s in 1:ns) {
  var_name = NC_getNameEncoded(prefix, NULL, 1, 1, s, ns)
  err = grid$extractVersion("Simu", var_name, version=s-1)
  err = grid$setLocator(name = var_name,
