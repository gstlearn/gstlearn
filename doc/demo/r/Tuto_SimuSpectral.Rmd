...
mod  = Model_createFromParam(type = ECov_fromKey(type_cov), range=1, 
                             ranges = ranges[1:ndim], param = nu, flagRange=FALSE)
# using the *SimuSpectral* interface
sim = SimuSpectralRN(1, nb, 100, 123, NULL, TRUE)
err = sim$setModelGeneric(mod)
err = sim$simulate(mod$getCovAniso(0))
...
err = sim$compute(dbout = grid)
# 
stopifnot (all(abs(val - grid["Simu"]) < 1e-6))

# using the simuSpectral interface
err  = simuSpectral(NULL, dbout = grid, model = mod, nbsimu = ns, ns = nb, seed=seed, cov0 = NULL, 
