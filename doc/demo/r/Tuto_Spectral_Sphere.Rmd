...
ns = 100     # number of spectral components
```


```{r model_definition, echo=TRUE, eval = TRUE}
err = defineDefaultSpace(ESpaceType_SN(), param=6371)
...
nd = 100
spevec = cumsum(cova$evalSpectrumOnSphere(nd))
plot.init() +
...
modelSph2 = ModelGeneric()
modelSph2$setCov(cova)
err = simuSpectral(dbin=NULL, dbout=grd, model=modelSph2, ns=ns, nd=nd, seed=seed, verbose=TRUE,
                   namconv = NamingConvention(covname))
```
...
```{r Visu2D}
# simu_name = paste0(covname, ".V1.S1")
simu_name = NC_getNameEncoded(covname, NULL)
p = plot.init(asp=1) +
...
mode = CovCalcMode()
mode$setAsVario(TRUE)
mod_vario = cova$evalCovOnSphereVec(alpha, 50, FALSE, mode = mode)
...
err = simuSpectral(dbin = NULL, dbout = grd, model = modelSph2, seed = 2584, ns=ns, nbsimu=nsim,
                   namconv = NamingConvention(covname))

# simulation and experimental variograms
varios <- matrix(NaN, nrow = nsim, ncol = ndisc)
for (s in 1:nsim) {
  nm0 = NC_getNameEncoded(covname, NULL, 1, 1, s, nsim)
  grd$extractVersion(covname, nm0, version=s-1)
  grd$setLocator(nm0, ELoc_Z())
  vs = vario_on_S2(grd = grd, nm_var = nm0, lag = c(1,0), nlag = 90, ndisc = ndisc, dmax = pi)
  varios[s, 1:length(vs$hh)] <- vs$gg
  grd$deleteColumn(nm0)
}
dist = vs$hh
