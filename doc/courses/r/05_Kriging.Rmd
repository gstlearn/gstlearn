...
#--- MP --- Bloc déplacé depuis plus bas
dat$getExtremas()
```
...
```{r Creating_Grid}
#--- MP --- Bloc séparé en deux
grid = DbGrid_create(x0=c(65,530),dx=c(4.94, 4.96),nx=c(82,138)) #--- Changement dans les arguments pour inclure tous les points
# grid = DbGrid_create(x0=c(65,535),dx=c(4.94, 4.96),nx=c(81,137)) #--- Anciens params
```
...
#--- MP ---
dbfmt = DbStringFormat_createFromFlags(flag_extend=TRUE) ## Simplification de la commande
grid$display(dbfmt)
```
...
We then the fit a model **fitmod** (add the Universality Condition)

```{r Fitting_Model}
#--- MP --- Retrait de la condition d'universalité: trop tôt...
fitmod = Model()
...
p = p + plot.grid(grid,
                  show.legend.raster = TRUE, palette="Spectral",legend.name.raster="°C") ## Added flag to show legend, remove zlim > not working
p = p + plot.point(dat,flagCst = T,pch=18,cex=1.5)
p = p + plot.decoration(title="Simple Kriging over whole Grid")
ggPrint(p)
```
By default, the `plot.grid` function plots the variable with locator `z1`, which in our case corresponds to the kriging predictor (as the `kriging` function automatically assigns the locator `z1` to it). To plot another variable, we can simply specify their name in the `name_raster` argument of the `plot.grid` function. 

For instance, we can plot the kriging standard deviation using the following code.

```{r Simple_Kriging_St_Dev}
p = ggDefaultGeographic()
p = p + plot.grid(grid,name_raster="SK.January_temp.stdev",
...
Finally, we plot the new kriging prediction over the grid using the `plot.grid` function and the data points. 
...
p = ggDefaultGeographic()
p = p + plot.grid(grid,name_raster="OK.January_temp.stdev",
                  show.legend.raster = TRUE, palette="Spectral",legend.name.raster="°C")
p = p + plot.point(dat,flagCst = T,pch=18,cex=1.5)
...
Let us compare the results from the simple and ordinary kriging predictors. To do so, we create a correlation plot between the two predictors using the `plot.correlation` function.
```{r Comparing_Ordinary_and_Simple_Kriging_Estimations,results="hide"}
#--- MP ----
p = ggplot()
p = p + plot.correlation(grid,name1="OK.January_temp.estim",name2="SK.January_temp.estim", #--- Explict variable names
...
#--- MP ---
p = ggplot()
p = p + plot.correlation(grid,name1="OK.January_temp.stdev",name2="SK.January_temp.stdev", #--- Explict variable names
                     flagRegr=TRUE, flagSameAxes=TRUE, bins=100)
...
## Plot
p = ggDefaultGeographic()
p = p + plot.grid(grid,name_raster="OK*estim",
                  show.legend.raster = TRUE, palette="Spectral",legend.name.raster="°C")
p = p + plot.point(dat,flagCst = T,pch=18,cex=1.5)
...
#--- MP --- Removed example without flagAbsSize
p = ggDefaultGeographic()
...
```{r}
#---- MP ----- Removed radius
small.neigh = NeighMoving_create(nmini=1, nmaxi=1)
```
...
## Plot
p = ggDefaultGeographic()
p = p + plot.grid(grid,name_raster="Small*estim",
                  show.legend.raster = TRUE, palette="Spectral",legend.name.raster="°C")
p = p + plot.point(dat,flagCst = T,pch=18,cex=1.5)
...
## Plot
p = ggDefaultGeographic()
p = p + plot.grid(grid,name_raster="Reduced*estim",
                  show.legend.raster = TRUE, palette="Spectral",legend.name.raster="°C")
p = p + plot.point(dat,flagCst = T,pch=18,cex=1.5)
...
## Plot
p = ggDefaultGeographic()
p = p + plot.grid(grid,name_raster="Moving*estim",
                  show.legend.raster = TRUE, palette="Spectral",legend.name.raster="°C")
p = p + plot.point(dat,flagCst = T,pch=18,cex=1.5)
...
p = ggDefaultGeographic()
p = p + plot.grid(grid,name_raster="Moving*stdev",
                  show.legend.raster = TRUE, palette="Spectral",legend.name.raster="°C")
# 
p = p + plot.point(dat,flagCst = T,pch=18,cex=1.5)
