...
p = p + plot.point(dat, nameColor="January_temp", show.legend.color = TRUE,
                   legend.name.color="Temperature")
p = p + plot.decoration(title="January Temperature", xlab="Easting", ylab="Northing")
ggPrint(p)
```

A more elaborated graphic representation displays the samples with a symbol proportional to the Elevation (**name_size**) and a color representing the Temperature (**nameColor**).

```{r}
p = ggDefaultGeographic()
p = p + plot.point(dat, name_size="Elevation", nameColor="January_temp", show.legend.color = TRUE,
