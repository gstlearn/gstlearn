...
p1 = plot.init(dims=c(6,6)) +
  plot(grid, name=NC_getNameEncoded("SimuPGS", NULL, 1, 1, 1, 0), version=0, palette=cmap) +
  plot(data, nameColor="facies", nameSize="connect", palette=cmap)
p2 = plot.init(dims=c(6,6)) +
  plot(grid, name=NC_getNameEncoded("SimuPGS", NULL, 1, 1, 2, 0), version=1, palette=cmap) +
  plot(data, nameColor="facies", nameSize="connect", palette=cmap)
p3 = plot.init(dims=c(6,6)) +
  plot(grid, name=NC_getNameEncoded("SimuPGS", NULL, 1, 1, 3, 0), version=2, palette=cmap,) +
  plot(data, nameColor="facies", nameSize="connect", palette=cmap)
p4 = plot.init(dims=c(6,6)) +
  plot(grid, name=NC_getNameEncoded("SimuPGS", NULL, 1, 1, 4, 0), version=3, palette=cmap) +
  plot(data, nameColor="facies", nameSize="connect", palette=cmap)
...
accept <- function(data, grid, name, isimu, verbose=FALSE, transBinary=TRUE, faccc=2)
{
  ...
  # Perform the labeling into connected components
  nameLocal = "Local"
  err = grid$extractVersion(name, nameLocal, version=isimu-1)
  err = grid$setLocator(nameLocal, ELoc_Z(), cleanSameLocator=TRUE)
  ...
  if (retval && transBinary)
    grid$setColumn(grid["Morpho*"] == cc_list[1], name, version=isimu-1)
    grid[name] = (grid["Morpho*"] == cc_list[1])

  grid$deleteColumn("Morpho*")
  grid$deleteColumn(nameLocal)
  retval
}
```
...
nb.valid = 0
for (i in rev(seq_len(nbsimu)))
{
  name = NC_getNameEncoded("SimuPGS", NULL, 1, 1, i, nbsimu)
  cat("Simulation",name)
  isValid = accept(data, grid, "SimuPGS", i)
  if (isValid)
  {
    cat(" is valid\n")
    nb.valid = nb.valid + 1
  ...
  {
    cat(" is invalid\n")
    grid$deleteVersion("SimuPGS", i-1)
  }
}
...
```{r Probability}
err = grid$statisticsBySample(c("SimuPGS*"),EStatOption_fromKeys(c("MEAN")))
```
