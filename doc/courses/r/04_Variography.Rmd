...
#--- MP ---
varioParamOmni = VarioParam_createOmniDirection(npas=40, dpas=10,toldis = 0.1) ## Added toldis
dat$setLocator("January_temp",ELoc_Z()) ## Added variable selection
...
We now plot the experimental variogram using **ggplot** and the dedicated function `plot.varmod`. In the resulting figure, the experimental variogram is plotted in blue, and the dashed blacked line corresponds to the value of the variance of the data.

```{r Plot_Experimental_Variogram}
ggplot() + plot.varmod(varioexp)
```

The `draw_psize` argument  of function `plot.varmod` allows to adapt the size of experimental variogram points in the plot so that it is proportional to the number of pairs of points used to compute the value. 
...
#--- MP ---
types = ECov_fromKeys(c("NUGGET","CUBIC","SPHERICAL")) ## Added types
...
err = fitmod$fit(varioexp, types=types, constraints=constraints, optvar=Option_VarioFit(TRUE)) ## Changed
ggplot() + plot.varmod(varioexp, fitmod)
```
...
err = fitmod$fit(varioexp, types=types, constraints=constraints, optvar=Option_VarioFit(TRUE)) ## Changed for coherence with before
ggplot() + plot.varmod(varioexp, fitmod)
```
...
```{r Variogram_Map}
#--- MP ---
grid.vmap = db_vmap_compute(dat) ## removed argument
...
modelVM
```
It is then possible to plot the variogram map resulting from the fitted model. To do so, we start by evaluating the fitted variogram model on the the experimental variogram map grid. This is done using the function `dbgrid_model` which we supply with both the experimental variogram map and the fitted model. This function adds a additional variable to the `Db` containing the experimental variogram map corresponding to the evaluations of the variogram model. Then, the function `plot.grid` allows to directly plot this new variable.
