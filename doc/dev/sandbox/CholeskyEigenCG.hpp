...
namespace gstlrn
{
  ...
  class GSTLEARN_EXPORT CholeskyEigenCG
    : public ALinearOp,
      public Eigen::EigenBase<CholeskyEigenCG>
  {
  public:
    ...
    Eigen::Product<CholeskyEigenCG, Rhs, Eigen::AliasFreeProduct>
      operator*(const Eigen::MatrixBase<Rhs>& x) const
    {
      return Eigen::Product<CholeskyEigenCG, Rhs, Eigen::AliasFreeProduct>(
        *this,
        x.derived());
    }
    ...
    void evalInverse(const VectorDouble& vecin,
                     VectorDouble& vecout) const override;
  ...
  protected:
    void
      _evalDirect(const VectorDouble& inv, VectorDouble& outv) const override;
  ...
  private:
    ...
    Eigen::SimplicialLDLT<Eigen::SparseMatrix<double>>
      _cholSolver; // (for Eigen library storage)
#endif
    const MatrixSparse*
      _matCS; // Stored by compliance with ALinearOp. Not to be deleted
  };
  ...
  namespace Eigen
  {
    namespace internal
    {
      ...
      template<typename Rhs>
      struct generic_product_impl<CholeskyEigenCG,
                                  Rhs,
                                  SparseShape,
                                  DenseShape,
                                  GemvProduct> // GEMV stands for matrix-vector
        : generic_product_impl_base<CholeskyEigenCG,
                                    Rhs,
                                    generic_product_impl<CholeskyEigenCG, Rhs>>
      {
        typedef typename Product<CholeskyEigenCG, Rhs>::Scalar Scalar;

        template<typename Dest>
        static void scaleAndAddTo(Dest& dst,
                                  const CholeskyEigenCG& lhs,
                                  const Rhs& rhs,
                                  const Scalar& alpha)
        {
          ...
          for (Index i = 0; i < lhs.cols(); ++i) dst[i] = outv[i];
        }
      };
