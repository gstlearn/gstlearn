...
namespace gstlrn
{
  ...
  bool ModelOptimVMap::_checkConsistency()
  {
    ...
    {
      messerr(
        "Number of variables in Dbmap (%d) must match the one in Model (%d)",
        nvar, _model->getNVar());
      return false;
    }
    ...
    {
      messerr(
        "'_dbmap'(%d) and '_model'(%d) should have same Space Dimensions", ndim,
        _model->getNDim());
      return false;
    }
  ...
  ModelOptimVMap* ModelOptimVMap::createForOptim(
    ModelGeneric* model,
    const DbGrid* dbmap,
    const Constraints* constraints,
    const ModelOptimParam& mop)
  {
    ...
    if (mop.getFlagGoulard())
    {
      ...
      {
        mcv->setFitSills(
          ModelFitSillsVMap::createForOptim(dbmap, model, constraints, mop));
