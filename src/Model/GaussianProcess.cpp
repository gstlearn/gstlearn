...
namespace gstlrn
{
  ...
  void GaussianProcess::init(const ModelGeneric* model, const Db* data)
  {
    ...
    if (data != nullptr) _data = std::shared_ptr<Db>(data->clone());
  }
    ...
    bool trace)
  {
    ...
    {
      _model->fitNew(
        _data.get(), nullptr, nullptr, nullptr, ModelOptimParam(),
        nb_neighVecchia, verbose, trace, reml);
    }
  }
  ...
  void GaussianProcess::simulate(Db* out, Id nbsimus)
  {
    ...
    {
      simtub(
        getData().get(), out, dynamic_cast<Model*>(getModel().get()), &neigh,
        nbsimus, seed);
    }
  }
} // namespace gstlrn
