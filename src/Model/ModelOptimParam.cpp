...
namespace gstlrn
{
  ...
  ModelOptimParam* ModelOptimParam::create(
    bool auth_aniso,
    bool auth_rotation,
    bool lock_samerot,
    bool lock_rot2d,
  ...
  String ModelOptimParam::toString(const AStringFormat* /*strfmt*/) const
  {
    ...
    sstr << "- Anisotropy                   " << NOK[getAuthAniso()]
         << std::endl;
    sstr << "- Anisotropy Rotation          " << NOK[getAuthRotation()]
         << std::endl;
    sstr << "- Global Rotation              " << NOK[getLockSamerot()]
         << std::endl;
    sstr << "- Rotation around Z only       " << NOK[getLockRot2d()]
         << std::endl;
    sstr << "- Lock third dimension         " << NOK[getLockNo3d()]
         << std::endl;
    sstr << "- Lock 2-D Isotropy            " << NOK[getLockIso2d()]
         << std::endl;
    sstr << "- Use the Goulard option       " << NOK[getFlagGoulard()]
         << std::endl;
    if (getFlagIntrinsic())
