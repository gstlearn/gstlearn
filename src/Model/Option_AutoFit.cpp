...
namespace gstlrn
{
  ...
  String Option_AutoFit::toString(const AStringFormat* /*strfmt*/) const
  {
    ...
    sstr << toStrTitle(1, "Optimization parameters");
    ...
    sstr << "- Minimum increment value           " << getEpsdelta()
         << std::endl;
    sstr << "- Variance percentage for stripping " << getTolsigma()
         << std::endl;
    sstr << "- Initial increment value           " << getInitdelta()
         << std::endl;
