...
namespace gstlrn
{
  ...
  ModelGeneric::ModelGeneric(const ModelGeneric& r)
    : ASerializable(r)
  {
    _cova = (r._cova != nullptr)
            ? std::dynamic_pointer_cast<ACov>(r._cova->cloneShared())
            : nullptr;
    _driftList = (r._driftList != nullptr) ? r._driftList->clone() : nullptr;
    _ctxt = r._ctxt;
    _transform =
      (r._transform != nullptr)
        ? std::dynamic_pointer_cast<ATransform>(r._transform->cloneShared())
        : nullptr;
  }
  ...
  static MatrixDense _transformF(const MatrixDense& F1, Id type, Id idx)
  {
    ...
    switch (type)
    {
      ...
      case 4:
        F1loc = F1;
        F1loc.fill(0.);
        for (Id i = 0; i < F1.getNRows(); i++) F1loc.setValue(i, idx, 1.);
        break;
    }
    return (F1loc);
  }

  Id computeCovMatSVCLHSInPlace(
    MatrixSymmetric& cov,
        ...
        for (Id lvar = 0; lvar < nvar; lvar++)
        {
          for (Id pvar = 0; pvar < nvar; pvar++)
          {
            ...
            value += Sigma.getValue(shifti + iech, shiftj + jech)
                   * F1loc.getValue(iech, lvar) * F1loc.getValue(jech, pvar);
          }
        }
      ...
      for (Id jech = 0; jech < nech2; jech++)
      {
        ...
        for (Id lvar = 0; lvar < nvar; lvar++)
        {
          for (Id pvar = 0; pvar < nvar; pvar++)
          {
            ...
            value += Sigma.getValue(shifti + iech, shiftj + jech)
                   * F1loc.getValue(iech, lvar) * F2loc.getValue(jech, pvar);
          }
        }
        cov.setValue(iech, jech, value);
      }
  ...
  Id computeDriftMatSVCRHSInPlace(
    MatrixDense& mat,
    const MatrixDense& F,
    Id type,
    Id idx,
    bool flagCenteredFactors)
  {
    if (flagCenteredFactors)
    {
      ...
      {
        case 1:
        case 3: mat.setRowToConstant(0, 1.); break;
        case 2: mat.setRowToConstant(0, 0.); break;
        case 4: mat.setValue(0, 0, (idx == 0) ? 1. : 0.); break;
      }
    }
    else
    {
      ...
      {
        case 1:
        case 3: mat.setRow(0, F.getRow(0)); break;
  ...
  void ModelGeneric::fitNew(
    ...
    bool reml)
  {
    AModelOptim* amopt = AModelOptimFactory::create(
      this, db, vario, dbmap, constraints, mop, nb_neighVecchia, reml);
    ...
    if (mcv != nullptr) mcv->deleteFitSills();
  }
  ...
  void ModelGeneric::setTransform(const ATransform* transform)
  {
    ...
    _transform =
      std::dynamic_pointer_cast<ATransform>(transform->cloneShared());
  }
  ...
  bool ModelGeneric::deserializeH5(H5::Group& grp)
  {
    ...
    Id varioNumber = 0;
    ret =
      ret && SerializeHDF5::readValue(*modelG, "Version Number", varioNumber);
    if (varioNumber != 2)
    {
      messerr(
        "ModelGeneric::deserializeH5: Unsupported version %d", varioNumber);
      return false;
    }
  ...
  bool ModelGeneric::serializeH5(H5::Group& grp) const
  {
    ...
    ret = ret
       && SerializeHDF5::writeValue(
            modelG, "Version Number", static_cast<Id>(varioNumber));

    // Serialize the CovContext characteristics
    ret = ret
       && SerializeHDF5::writeValue(modelG, "NDim", static_cast<Id>(getNDim()));
