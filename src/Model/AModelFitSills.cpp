...
namespace gstlrn
{
          ...
          Birk_v.setValue(k,
                          ...
                          Birk_v.getValue(k, ivar)
                            + alpha[icov].getValue(ivar0, ivar)
                                * (_ge[icov].getValue(0, k)
                                   - value * _ge[icov0].getValue(0, k))
                                / Nir_v[ivar]);
        }
      }
  ...
  void
    AModelFitSills::_updateCurrentSillDiag(Id icov0,
                                           Id ivar0,
                                           std::vector<MatrixSymmetric>& alpha,
                                           VectorDouble& xr)
  {
  ...
  Id AModelFitSills::_sillFittingIntrinsic()
  {
    ...
    for (Id iter = 0; iter < _mop.getMaxiter(); iter++)
    {
      ...
      if (_goulardWithoutConstraint(1,
                                    _nvar,
                                    1,
                                    _npadir,
                                    _wt,
  ...
  bool AModelFitSills::_convergenceReached(Id iter,
                                           double crit,
                                           double crit_mem) const
  {
    if (_trace)
    {
      ...
      for (Id icova = 0; icova < _ncova; icova++)
      {
        ...
        printVector(_sill[icova].getValues(),
                    " Current parameters ",
                    true,
                    false);
      }
  ...
  {
    message("  Summary of Sill Fitting - Count of Iterations = %4d - Final "
            "Cost = %lf\n",
            niter,
            _score);
    for (Id icova = 0; icova < _ncova; icova++)
