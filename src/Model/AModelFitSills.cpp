/******************************************************************************/
/*                                                                            */
/*                            gstlearn C++ Library                            */
/*                                                                            */
/* Copyright (c) (2023) MINES Paris / ARMINES                                 */
/* Authors: gstlearn Team                                                     */
/* Website: https://gstlearn.org                                              */
/* License: BSD 3-clause                                                      */
/*                                                                            */
/******************************************************************************/
#include "Model/AModelFitSills.hpp"
#include "Basic/MathFunc.hpp"
#include "Covariances/CovAniso.hpp"
#include "Matrix/EigenVectors.hpp"
#include "Model/Constraints.hpp"
#include "Model/ModelCovList.hpp"

#define IJDIR(ijvar, ipadir) ((ijvar) * _npadir + (ipadir))
#define WT(ijvar, ipadir) wt[IJDIR(ijvar, ipadir)]
#define GG(ijvar, ipadir) gg[IJDIR(ijvar, ipadir)]
#define _WT(ijvar, ipadir) _wt[IJDIR(ijvar, ipadir)]
#define _GG(ijvar, ipadir) _gg[IJDIR(ijvar, ipadir)]
#define _WT2(ijvar, ipadir) _wt2[IJDIR(ijvar, ipadir)]
#define _GG2(ijvar, ipadir) _gg2[IJDIR(ijvar, ipadir)]
#define TAB(ijvar, ipadir) tabin[IJDIR(ijvar, ipadir)]
#define DD(idim, ijvar, ipadir) _dd[idim][IJDIR(ijvar, ipadir)]

namespace gstlrn
{
  AModelFitSills::AModelFitSills(
    ModelCovList* model,
    const Constraints* constraints,
    const ModelOptimParam& mop)
    : _ndim(0)
    , _nvar(0)
    , _nvs2(0)
    , _ncova(0)
    , _nbexp(0)
    , _npadir(0)
    , _wt()
    , _gg()
    , _ggc()
    , _wtc()
    , _wt2()
    , _gg2()
    , _dd()
    , _ge()
    , _ge1()
    , _ge2()
    , _alphau()
    , _sill1()
    , _sill()
    , _verbose(false)
    , _trace(false)
    , _iterg(0)
    , _score(0)
    , _model(model)
    , _constraints(constraints)
    , _mop(mop)
    , _calcmode()
  {
  }

  AModelFitSills::AModelFitSills(const AModelFitSills& m)
    : _ndim(m._ndim)
    , _nvar(m._nvar)
    , _nvs2(m._nvs2)
    , _ncova(m._ncova)
    , _nbexp(m._nbexp)
    , _npadir(m._npadir)
    , _wt(m._wt)
    , _gg(m._gg)
    , _ggc(m._ggc)
    , _wtc(m._wtc)
    , _wt2(m._wt2)
    , _gg2(m._gg2)
    , _dd(m._dd)
    , _ge(m._ge)
    , _ge1(m._ge1)
    , _ge2(m._ge2)
    , _alphau(m._alphau)
    , _sill1(m._sill1)
    , _sill(m._sill)
    , _verbose(m._verbose)
    , _trace(m._trace)
    , _iterg(m._iterg)
    , _score(m._score)
    , _model(m._model)
    , _constraints(m._constraints)
    , _mop(m._mop)
    , _calcmode(m._calcmode)
  {
  }

  AModelFitSills& AModelFitSills::operator=(const AModelFitSills& m)
  {
    if (this != &m)
    {
      _ndim = m._ndim;
      _nvar = m._nvar;
      _nvs2 = m._nvs2;
      _ncova = m._ncova;
      _nbexp = m._nbexp;
      _npadir = m._npadir;
      _wt = m._wt;
      _gg = m._gg;
      _ggc = m._ggc;
      _wtc = m._wtc;
      _wt2 = m._wt2;
      _gg2 = m._gg2;
      _dd = m._dd;
      _ge = m._ge;
      _ge1 = m._ge1;
      _ge2 = m._ge2;
      _alphau = m._alphau;
      _sill1 = m._sill1;
      _sill = m._sill;
      _verbose = m._verbose;
      _trace = m._trace;
      _iterg = m._iterg;
      _score = m._score;
      _model = m._model;
      _constraints = m._constraints;
      _mop = m._mop;
      _calcmode = m._calcmode;
    }
    return (*this);
  }

  AModelFitSills::~AModelFitSills() {}

  void AModelFitSills::_storeSillsInModel() const
  {
    for (Id icov = 0; icov < _ncova; icov++)
    {
      Id ijvar = 0;
      for (Id ivar = ijvar = 0; ivar < _nvar; ivar++)
        for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
          _model->setSill(icov, ivar, jvar, _sill[icov].getValue(ivar, jvar));
    }
  }

  void
    AModelFitSills::_resetInitialSill(std::vector<MatrixSymmetric>& sill) const
  {
    Id ncova = static_cast<Id>(sill.size());
    for (Id icova = 0; icova < ncova; icova++)
    {
      for (Id ivar = 0; ivar < _nvar; ivar++)
        for (Id jvar = 0; jvar < _nvar; jvar++)
          sill[icova].setValue(ivar, jvar, ivar == jvar);
    }
  }

  /****************************************************************************/
  /*!
   **  Manage memory for variogram fitting
   **
   ** \return  Error returned code
   **
   ** \param[in]  flag_exp  1 for experimental variogram
   **
   *****************************************************************************/
  void AModelFitSills::_allocateInternalArrays(bool flag_exp)
  {
    _wt.fill(TEST, _npadir * _nvs2);
    _gg.fill(TEST, _npadir * _nvs2);
    _ge.clear();
    for (Id icova = 0; icova < _ncova; icova++)
      _ge.push_back(MatrixDense(_nvs2, _npadir));
    _sill.clear();
    for (Id icova = 0; icova < _ncova; icova++)
      _sill.push_back(MatrixSymmetric(_nvar));

    if (flag_exp)
    {
      _wtc.fill(TEST, _nbexp);
      _ggc.fill(TEST, _nbexp);
      _dd.clear();
      for (Id idim = 0; idim < _ndim; idim++)
        _dd.push_back(VectorDouble(_npadir * _nvs2, TEST));
    }

    if (_mop.getFlagIntrinsic())
    {
      _alphau.clear();
      for (Id icova = 0; icova < _ncova; icova++)
        _alphau.push_back(MatrixSymmetric(1));
      _ge1.clear();
      _ge1.push_back(MatrixDense(_nvs2, _npadir));
      _ge2.clear();
      for (Id icova = 0; icova < _ncova; icova++)
        _ge2.push_back(MatrixDense(_nvs2, _npadir));
      _wt2.fill(TEST, _nvs2 * _npadir);
      _gg2.fill(TEST, _nvs2 * _npadir);
    }
  }

  Id AModelFitSills::_goulardWithConstraints()
  {
    _score = 0.;
    const auto& consSill = _constraints->getConstantSills();

    /* Core allocation */
    std::vector<MatrixSymmetric> matcor;
    matcor.reserve(_ncova);
    for (Id icova = 0; icova < _ncova; icova++)
      matcor.push_back(MatrixSymmetric(_nvar));

    /* Initialize the Goulard system */

    _initializeGoulard();

    /* Update according to the eigen values */

    bool flag_positive = true;
    for (Id icov = 0; icov < _ncova; icov++)
      if (!_makeDefinitePositive(icov)) flag_positive = false;

    if (!flag_positive)
    {
      for (Id icov = 0; icov < _ncova; icov++)
        for (Id ivar = 0; ivar < _nvar; ivar++)
        {
          _sill[icov].fill(0.);
          if (!FFFF(consSill[ivar]))
            _sill[icov].setValue(ivar, ivar, consSill[ivar] / _ncova);
          else
            _sill[icov].setValue(ivar, ivar, 1.);
        }

      /* Perform the optimization under constraints */

      _optimizeUnderConstraints();
    }

    return (0);
  }

  /****************************************************************************/
  /*!
   **  Initialize the system for Goulard algorithm
   **
   *****************************************************************************/
  void AModelFitSills::_initializeGoulard()
  {
    MatrixSymmetric aa(_ncova);
    VectorDouble bb(_ncova);
    MatrixDense Ae(_ncova, 1);
    VectorDouble be(1);
    MatrixDense Ai(_ncova, _ncova);
    VectorDouble bi(_ncova);
    VectorDouble res(_ncova);
    const auto& consSill = _constraints->getConstantSills();

    /* Initialize the constraints matrices */

    Ae.fill(1.);
    bi.fill(0.);
    for (Id icov = 0; icov < _ncova; icov++)
      for (Id jcov = 0; jcov < _ncova; jcov++)
        Ai.setValue(icov, jcov, icov == jcov);

    /* Loop on the variables */

    Id ijvar = 0;
    for (Id ivar = 0; ivar < _nvar; ivar++)
      for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
      {
        /* Reset the arrays */

        bb.fill(0.);
        aa.fill(0.);

        for (Id ipadir = 0; ipadir < _npadir; ipadir++)
        {
          double wtloc = _WT(ijvar, ipadir);
          double ggloc = _GG(ijvar, ipadir);
          if (FFFF(wtloc) || FFFF(ggloc)) continue;
          for (Id icov = 0; icov < _ncova; icov++)
          {
            double geloc1 = _ge[icov].getValue(ijvar, ipadir);
            if (FFFF(geloc1)) continue;
            bb[icov] += wtloc * ggloc * geloc1;
            for (Id jcov = 0; jcov <= icov; jcov++)
            {
              double geloc2 = _ge[jcov].getValue(ijvar, ipadir);
              if (FFFF(geloc2)) continue;
              aa.updValue(icov, jcov, EOperator::ADD, wtloc * geloc1 * geloc2);
            }
          }
        }

        Id retcode = 0;
        if (ivar == jvar && !consSill.empty() && !FFFF(consSill[ivar]))
        {
          be[0] = consSill[ivar];
          retcode = aa.minimizeWithConstraintsInPlace(bb, Ae, be, Ai, bi, res);
        }
        else
        {
          retcode = aa.minimizeWithConstraintsInPlace(
            bb, MatrixDense(), VectorDouble(), MatrixDense(), VectorDouble(),
            res);
        }

        /* Update (taking into account possible constraints) */

        if (retcode)
        {
          for (Id icov = 0; icov < _ncova; icov++) res[icov] = (ivar == jvar);
          if (ivar == jvar && !consSill.empty() && !FFFF(consSill[ivar]))
          {
            double temp = consSill[ivar] / _ncova;
            for (Id icov = 0; icov < _ncova; icov++) res[icov] = temp;
          }
        }

        /* Store in the output matrix */

        for (Id icov = 0; icov < _ncova; icov++)
          _sill[icov].setValue(ivar, jvar, res[icov]);
      }
  }

  /*****************************************************************************/
  /*!
   ** Make sure the current matrix of sills is definite positive
   ** (diagonal unchanged)
   **
   ** \param[in]      icov0    Index of the target basic structure
   ** \param[in]      eps      Tolerance
   **
   *****************************************************************************/
  Id AModelFitSills::_makeDefinitePositive(Id icov0, double eps)
  {
    VectorDouble muold(_nvar);
    VectorDouble norme1(_nvar);
    const auto& consSill = _constraints->getConstantSills();

    for (Id ivar = 0; ivar < _nvar; ivar++)
      muold[ivar] = _sill[icov0].getValue(ivar, ivar);

    Id flag_positive = _truncateNegativeEigen(icov0);

    if (flag_positive) return flag_positive;

    for (Id ivar = 0; ivar < _nvar; ivar++)
    {
      if (FFFF(consSill[ivar]))
        norme1[ivar] = 1.;
      else
      {
        if (ABS(_sill[icov0].getValue(ivar, ivar)) > eps)
          norme1[ivar] = sqrt(muold[ivar] / _sill[icov0].getValue(ivar, ivar));
        else
          norme1[ivar] = (ABS(muold[ivar]) < eps) ? 1. : 0.;
      }
    }

    for (Id ivar = 0; ivar < _nvar; ivar++)
      for (Id jvar = 0; jvar <= ivar; jvar++)
        _sill[icov0].setValue(
          ivar, jvar,
          _sill[icov0].getValue(ivar, jvar) * norme1[ivar] * norme1[jvar]);

    return flag_positive;
  }

  void AModelFitSills::_optimizeUnderConstraints()
  {
    double score_old, xrmax;

    /* Core allocation */

    VectorDouble xr(_nvar);
    std::vector<MatrixSymmetric> alpha;
    const auto& consSill = _constraints->getConstantSills();
    alpha.reserve(_ncova);
    for (Id icova = 0; icova < _ncova; icova++)
      alpha.push_back(MatrixSymmetric(_nvar));

    /* Calculate the initial score */

    double score_new = _calculateScore();
    for (Id icov = 0; icov < _ncova; icov++) alpha[icov] = _sill[icov];

    /***********************/
    /* Iterative procedure */
    /***********************/

    /* Optional printout */

    for (Id ivar = 0; ivar < _nvar; ivar++)
    {
      if (FFFF(consSill[ivar]))
        xr[ivar] = 1.;
      else
        xr[ivar] = sqrt(consSill[ivar] / _sumSills(ivar, alpha));
    }

    for (Id iter = 0; iter < _mop.getMaxiter(); iter++)
    {
      _iterg++;
      for (Id icov0 = 0; icov0 < _ncova; icov0++)
      {
        for (Id ivar0 = 0; ivar0 < _nvar; ivar0++)
        {
          double denom =
            _sumSills(ivar0, alpha) - alpha[icov0].getValue(ivar0, ivar0);
          if (!FFFF(consSill[ivar0]) && denom < 1e-30) continue;
          if (!FFFF(consSill[ivar0]))
          {
            xrmax = sqrt(consSill[ivar0] / denom);
            xr[ivar0] = _minimizeP4(icov0, ivar0, xrmax, xr, alpha);

            if (isZero(xr[ivar0]))
            {
              xr[ivar0] = 1.;
              for (Id jcov = 0; jcov < _ncova; jcov++)
              {
                if (jcov == icov0) continue;
                for (Id jvar = 0; jvar < _nvar; jvar++)
                  alpha[jcov].setValue(ivar0, jvar, 0.);
              }
            }
          }

          /* Update 'alpha' (diagonal only) */

          if (!FFFF(consSill[ivar0])) _updateAlphaDiag(icov0, ivar0, xr, alpha);

          /* Update 'sills' for the structures other than the current one */

          _updateOtherSills(icov0, ivar0, alpha, xr);

          /* Update the sill matrix for the current structure */
          /* (except diagonal in the constrained case)        */

          _updateCurrentSillGoulard(icov0, ivar0);

          /* Update sill matrix for the current structure (for diagonal) */

          if (!FFFF(consSill[ivar0]))
            _updateCurrentSillDiag(icov0, ivar0, alpha, xr);

          /* Make sure the current matrix of sills if definite positive */
          /* (diagonal unchanged)                                       */

          (void)_makeDefinitePositive(icov0);

          /* Update 'alpha' for the current structure */

          for (Id ivar = 0; ivar < _nvar; ivar++)
            _updateAlphaNoDiag(icov0, ivar, xr, alpha);
        }
      }

      /* Update the score */

      score_old = score_new;
      score_new = _calculateScore();
      if (_convergenceReached(iter, score_new, score_old)) break;
    }

    /* Optional printout */

    _score = score_new;
  }

  Id AModelFitSills::_truncateNegativeEigen(Id icov0)
  {
    MatrixSymmetric cc(_nvar);
    for (Id ivar = 0; ivar < _nvar; ivar++)
      for (Id jvar = 0; jvar <= ivar; jvar++)
        cc.setValue(ivar, jvar, _sill[icov0].getValue(ivar, jvar));

    auto eigenvectors = EigenVectors(cc);
    const auto& valpro = eigenvectors.getEigenValues();
    const MatrixSquare* vecpro = &eigenvectors.getEigenVectors();

    /* Check positiveness */

    Id flag_positive = 1;
    for (Id ivar = 0; ivar < _nvar; ivar++)
      if (valpro[ivar] <= 0) flag_positive = 0;
    if (flag_positive) return flag_positive;

    /* Calculate the new coregionalization matrix */

    for (Id ivar = 0; ivar < _nvar; ivar++)
      for (Id jvar = 0; jvar <= ivar; jvar++)
      {
        double sum = 0.;
        for (Id kvar = 0; kvar < _nvar; kvar++)
          sum += MAX(valpro[kvar], 0.) * vecpro->getValue(ivar, kvar)
               * vecpro->getValue(jvar, kvar);
        _sill[icov0].setValue(ivar, jvar, sum);
      }

    return flag_positive;
  }

  double AModelFitSills::_calculateScore() const
  {
    double coeff;
    double ggloc;
    double geloc;
    double wtloc;
    double temp;

    message("Score\n");
    double scoreLocal = 0.;
    for (Id ivar = 0, ijvar = 0; ivar < _nvar; ivar++)
      for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
      {
        coeff = (ivar == jvar) ? 1 : 2;
        for (Id ipadir = 0; ipadir < _npadir; ipadir++)
        {
          ggloc = _GG(ijvar, ipadir);
          if (FFFF(ggloc)) continue;
          wtloc = _WT(ijvar, ipadir);
          if (wtloc <= 0.) continue;

          geloc = 0.;
          for (Id icov = 0; icov < _ncova; icov++)
            geloc += _sill[icov].getValue(ivar, jvar)
                   * _ge[icov].getValue(ijvar, ipadir);
          temp = ggloc - geloc;
          scoreLocal += coeff * wtloc * temp * temp;

          message(
            "ipadir=%d wt=%lf gg=%lf ge=%lf\n", ipadir, wtloc, ggloc, geloc);
        }
      }

    message("---> Resulting score = %lf\n", scoreLocal);
    return scoreLocal;
  }

  double AModelFitSills::_calculateScoreMP(
    Id nvar,
    Id npadir,
    VectorDouble& wt,
    VectorDouble& gg,
    const MatrixDense& mp) const
  {
    double temp;
    double coeff;
    double ggloc;
    double wtloc;

    double scoreLocal = 0.;
    for (Id ivar = 0, ijvar = 0; ivar < nvar; ivar++)
      for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
      {
        coeff = (ivar != jvar) ? 2. : 1.;
        for (Id ipadir = 0; ipadir < npadir; ipadir++)
        {
          ggloc = GG(ijvar, ipadir);
          if (FFFF(ggloc)) continue;
          wtloc = WT(ijvar, ipadir);
          if (wtloc <= 0.) continue;
          temp = ggloc - mp.getValue(ijvar, ipadir);
          scoreLocal += coeff * wtloc * temp * temp;
        }
      }
    return scoreLocal;
  }

  double AModelFitSills::_sumSills(
    Id ivar0,
    std::vector<MatrixSymmetric>& alpha) const
  {
    double Sr = 0;
    for (Id icov = 0; icov < _ncova; icov++)
      Sr += alpha[icov].getValue(ivar0, ivar0);
    return Sr;
  }

  /****************************************************************************/
  /*!
   ** Minimization of the order-4 polynomial
   **
   ** \return Value for which the fourth order polynomial is minimum.
   **
   ** \param[in] icov0    Index of the target basic structure
   ** \param[in] ivar0    Index of the target variable
   ** \param[in] xrmax    Maximum value for the solution
   ** \param[in] xr       Current vector of sqrt(constraint/(sum of the sills))
   ** \param[in] alpha    Current auxiliary matrices alpha
   **
   *****************************************************************************/
  double AModelFitSills::_minimizeP4(
    Id icov0,
    Id ivar0,
    double xrmax,
    VectorDouble& xr,
    std::vector<MatrixSymmetric>& alpha)
  {
    double retval, a, c, d, s;

    /* Core allocation */

    VectorDouble Nir_v(_nvar);
    VectorDouble Mrr_v(_npadir);
    VectorDouble Crr_v(_npadir);
    MatrixDense Airk_v(_npadir, _nvar);
    MatrixDense Birk_v(_npadir, _nvar);
    VectorDouble xx(2);
    VectorDouble xt(2);
    VectorDouble xest(2);
    VectorDouble x(3);
    const auto& consSill = _constraints->getConstantSills();

    Id irr = _combineVariables(ivar0, ivar0);

    for (Id ivar = 0; ivar < _nvar; ivar++)
    {
      Id irl = _combineVariables(ivar0, ivar);
      Nir_v[ivar] = 0.;
      for (Id k = 0; k < _npadir; k++)
        Nir_v[ivar] +=
          _WT(irl, k) * _ge[icov0].getValue(0, k) * _ge[icov0].getValue(0, k);
    }

    for (Id k = 0; k < _npadir; k++)
    {
      Mrr_v[k] = 0.;
      for (Id jcov = 0; jcov < _ncova; jcov++)
      {
        if (jcov == icov0) continue;
        Mrr_v[k] += alpha[jcov].getValue(ivar0, ivar0)
                  * (_ge[jcov].getValue(0, k) - _ge[icov0].getValue(0, k));
      }
    }

    for (Id k = 0; k < _npadir; k++)
      for (Id ivar = 0; ivar < _nvar; ivar++)
      {
        Id irl = _combineVariables(ivar0, ivar);
        double value = 0.;
        for (Id l = 0; l < _npadir; l++)
          value += _WT(irl, l) * _GG(irl, l) * _ge[icov0].getValue(0, l);
        value *= _ge[icov0].getValue(0, k) / Nir_v[ivar];
        Airk_v.setValue(k, ivar, _GG(irl, k) - value);
      }

    for (Id k = 0; k < _npadir; k++)
      for (Id ivar = 0; ivar < _nvar; ivar++)
      {
        Id irl = _combineVariables(ivar0, ivar);
        Birk_v.setValue(k, ivar, 0.);
        for (Id icov = 0; icov < _ncova; icov++)
        {
          if (icov == icov0) continue;
          double value = 0.;
          for (Id l = 0; l < _npadir; l++)
            value +=
              _WT(irl, l) * _ge[icov].getValue(0, l) * _ge[icov].getValue(0, l);
          Birk_v.setValue(
            k, ivar,
            Birk_v.getValue(k, ivar)
              + alpha[icov].getValue(ivar0, ivar)
                  * (_ge[icov].getValue(0, k) - value * _ge[icov0].getValue(0, k))
                  / Nir_v[ivar]);
        }
      }

    for (Id k = 0; k < _npadir; k++)
      Crr_v[k] = _GG(irr, k) - consSill[ivar0] * _ge[icov0].getValue(0, k);

    a = 0.;
    for (Id k = 0; k < _npadir; k++) a += _WT(irr, k) * Mrr_v[k] * Mrr_v[k];

    c = 0.;
    for (Id k = 0; k < _npadir; k++)
      for (Id ivar = 0; ivar < _nvar; ivar++)
      {
        if (ivar != ivar0)
        {
          Id irl = _combineVariables(ivar0, ivar);
          s = xr[ivar] * Birk_v.getValue(k, ivar);
          c += _WT(irl, k) * s * s;
        }
        else
        {
          c -= _WT(irr, k) * Mrr_v[k] * Crr_v[k];
        }
      }
    c *= 2.;

    d = 0.;
    for (Id k = 0; k < _npadir; k++)
      for (Id ivar = 0; ivar < _nvar; ivar++)
      {
        if (ivar == ivar0) continue;
        Id irl = _combineVariables(ivar0, ivar);
        d -= _WT(irl, k) * Airk_v.getValue(k, ivar) * xr[ivar]
           * Birk_v.getValue(k, ivar);
      }

    d *= 4.;

    Id number = solve_P3(4. * a, 0., 2. * c, d, x);

    switch (number)
    {
      case 1: retval = MAX(0., MIN(xrmax, x[0])); break;

      case 3:
      {
        Id nin = 0;
        xx[0] = x[0];
        xx[1] = x[2];
        for (Id k = 0; k < 2; k++)
        {
          xt[k] = MAX(0., MIN(xrmax, xx[k]));
          xest[k] =
            (a * xt[k] * xt[k] * xt[k] * xt[k] + c * xt[k] * xt[k] + d * xt[k])
            / 2.;
          if (isEqual(xt[k], xx[k])) nin++;
        }
        if (nin == 1)
        {
          retval = (isEqual(xt[0], xx[0])) ? xx[0] : xx[1];
        }
        else
        {
          retval = (xest[0] < xest[1]) ? xt[0] : xt[1];
        }
      }
      break;

      default:
        retval = xr[ivar0];
        // mes_abort("st_minimize_P4: Number of extrema is impossible");
        break;
    }

    if (retval < 1.e-9) retval = 0.;

    return retval;
  }

  void AModelFitSills::_updateAlphaDiag(
    Id icov0,
    Id ivar0,
    VectorDouble& xr,
    std::vector<MatrixSymmetric>& alpha)
  {
    const auto& consSill = _constraints->getConstantSills();
    double srm = _sumSills(ivar0, alpha) - alpha[icov0].getValue(ivar0, ivar0);
    double value = consSill[ivar0] / (xr[ivar0] * xr[ivar0]) - srm;
    alpha[icov0].setValue(ivar0, ivar0, MAX(0., value));
  }

  void AModelFitSills::_updateOtherSills(
    Id icov0,
    Id ivar0,
    std::vector<MatrixSymmetric>& alpha,
    VectorDouble& xr)
  {
    for (Id jcov = 0; jcov < _ncova; jcov++)
    {
      if (jcov == icov0) continue;
      for (Id ivar = 0; ivar < _nvar; ivar++)
      {
        double value = alpha[jcov].getValue(ivar0, ivar) * xr[ivar0] * xr[ivar];
        _sill[jcov].setValue(ivar0, ivar, value);
        _sill[jcov].setValue(ivar, ivar0, value);
      }
    }
  }

  void AModelFitSills::_updateCurrentSillGoulard(Id icov0, Id ivar0)
  {
    VectorDouble mv(_npadir);
    const auto& consSill = _constraints->getConstantSills();

    // Loop on the variables

    for (Id ivar = 0; ivar < _nvar; ivar++)
    {
      if (ivar == ivar0 && !FFFF(consSill[ivar0])) continue;

      for (Id ilagdir = 0; ilagdir < _npadir; ilagdir++)
      {
        mv[ilagdir] = 0.;
        for (Id icov = 0; icov < _ncova; icov++)
        {
          if (icov == icov0) continue;
          mv[ilagdir] +=
            _sill[icov].getValue(ivar0, ivar) * _ge[icov].getValue(0, ilagdir);
        }
      }

      double tot1 = 0.;
      double tot2 = 0.;
      Id ivs2 = _combineVariables(ivar0, ivar);

      for (Id ilagdir = 0; ilagdir < _npadir; ilagdir++)
      {
        double wtloc = _WT(ivs2, ilagdir);
        double ggloc = _GG(ivs2, ilagdir);
        double geloc = _ge[icov0].getValue(0, ilagdir);

        if (!FFFF(ggloc))
        {
          tot1 += wtloc * geloc * (ggloc - mv[ilagdir]);
          tot2 += wtloc * geloc * geloc;
        }
      }
      double value = tot1 / tot2;
      _sill[icov0].setValue(ivar0, ivar, value);
      _sill[icov0].setValue(ivar, ivar0, value);
    }
  }

  void AModelFitSills::_updateCurrentSillDiag(
    Id icov0,
    Id ivar0,
    std::vector<MatrixSymmetric>& alpha,
    VectorDouble& xr)
  {
    double value = xr[ivar0] * xr[ivar0] * alpha[icov0].getValue(ivar0, ivar0);
    if (value < 0.) value = 0.;
    _sill[icov0].setValue(ivar0, ivar0, value);
  }

  void AModelFitSills::_updateAlphaNoDiag(
    Id icov0,
    Id ivar0,
    VectorDouble& xr,
    std::vector<MatrixSymmetric>& alpha)
  {
    const auto& consSill = _constraints->getConstantSills();
    for (Id ivar = 0; ivar < _nvar; ivar++)
    {
      if (ivar == ivar0 && !FFFF(consSill[ivar0])) continue;
      double value =
        _sill[icov0].getValue(ivar0, ivar) / (xr[ivar0] * xr[ivar]);
      alpha[icov0].setValue(ivar0, ivar, value);
    }
  }

  Id AModelFitSills::_combineVariables(Id ivar0, Id jvar0)
  {
    if (ivar0 > jvar0) return (ivar0 + jvar0 * (jvar0 + 1) / 2);
    return (jvar0 + ivar0 * (ivar0 + 1) / 2);
  }

  Id AModelFitSills::_sillFittingIntrinsic()
  {
    _score = 0.;
    double crit_mem = MAXIMUM_BIG;
    for (Id icov = 0; icov < _ncova; icov++)
      _alphau[icov] = 1. / static_cast<double>(_ncova);

    /* Iterative procedure */

    for (Id iter = 0; iter < _mop.getMaxiter(); iter++)
    {
      _iterg++;

      /* Initialize the arrays for the first pass */

      for (Id ipadir = 0; ipadir < _npadir; ipadir++)
      {
        Id ijvar = 0;
        for (Id ivar = 0; ivar < _nvar; ivar++)
        {
          for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
          {
            double sum = 0.;
            for (Id icov = 0; icov < _ncova; icov++)
              sum += _alphau[icov].getValue(0, 0)
                   * _ge[icov].getValue(ijvar, ipadir);
            _ge1[0].setValue(ijvar, ipadir, sum);
          }
        }
      }

      /* Call Goulard with 1 structure (no constraint) */

      _resetInitialSill(_sill1);
      if (_goulardWithoutConstraint(
            1, _nvar, 1, _npadir, _wt, _gg, _ge1, _sill1))
        return 1;

      /* Initialize the arrays for the second pass */

      Id ijvar = 0;
      for (Id ivar = 0; ivar < _nvar; ivar++)
        for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
        {
          double pivot = _sill1[0].getValue(ivar, jvar);
          for (Id ipadir = 0; ipadir < _npadir; ipadir++)
          {
            _GG2(ijvar, ipadir) =
              (isZero(pivot)) ? 0. : _GG(ijvar, ipadir) / pivot;
            _WT2(ijvar, ipadir) = _WT(ijvar, ipadir) * pivot * pivot;
            for (Id icov = 0; icov < _ncova; icov++)
              _ge2[icov].setValue(
                ijvar, ipadir, _ge[icov].getValue(ijvar, ipadir));
          }
        }

      /* Call Goulard with 1 variable (no constraint) */

      if (_goulardWithoutConstraint(
            1, 1, _ncova, _npadir * _nvs2, _wt2, _gg2, _ge2, _alphau))
        return 1;

      /* Stopping criterion */

      if (_convergenceReached(iter, _score, crit_mem)) break;
      crit_mem = _score;
    }

    /* Patch the final model */

    for (Id icov = 0; icov < _ncova; icov++)
    {
      Id ijvar = 0;
      for (Id ivar = 0; ivar < _nvar; ivar++)
        for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
        {
          double newval =
            _alphau[icov].getValue(0, 0) * _sill1[0].getValue(ivar, jvar);
          _model->setSill(icov, ivar, jvar, newval);
          _model->setSill(icov, jvar, ivar, newval);
        }
    }
    return 0;
  }

  Id AModelFitSills::_goulardWithoutConstraint(
    Id niter,
    Id nvar,
    Id ncova,
    Id npadir,
    VectorDouble& wt,
    VectorDouble& gg,
    std::vector<MatrixDense>& ge,
    std::vector<MatrixSymmetric>& sill) const
  {
    Id allpos;
    double temp, crit, crit_mem, coeff;
    const MatrixSquare* vecpro;

    /*******************/
    /* Initializations */
    /*******************/

    double sum = 0.;
    double sum1 = 0.;
    double sum2 = 0.;
    Id nvs2Local = nvar * (nvar + 1) / 2;

    /* Core allocation */

    MatrixDense mp(nvs2Local, npadir);
    std::vector<MatrixDense> fk;
    fk.reserve(ncova);
    for (Id icova = 0; icova < ncova; icova++)
      fk.push_back(MatrixDense(nvs2Local, npadir));
    MatrixSymmetric cc(nvar);

    // The array 'aicLocal' is similar to '_aic' stored within CovBase class
    // However it is not stored there as 'nvar' is defined locally.
    std::vector<MatrixSymmetric> aicLocal;
    aicLocal.reserve(ncova);
    for (Id icova = 0; icova < ncova; icova++)
      aicLocal.push_back(MatrixSymmetric(nvar));
    std::vector<MatrixSymmetric> alphak;
    alphak.reserve(ncova);
    for (Id icova = 0; icova < ncova; icova++)
      alphak.push_back(MatrixSymmetric(nvar));

    /********************/
    /* Pre-calculations */
    /********************/

    for (Id ivar = 0, ijvar = 0; ivar < nvar; ivar++)
      for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
        for (Id ipadir = 0; ipadir < npadir; ipadir++)
        {
          mp.setValue(ijvar, ipadir, 0.);
          for (Id icov = 0; icov < ncova; icov++)
            mp.setValue(
              ijvar, ipadir,
              mp.getValue(ijvar, ipadir)
                + (sill[icov].getValue(ivar, jvar) * ge[icov].getValue(ijvar, ipadir)));
        }

    for (Id icov = 0; icov < ncova; icov++)
      for (Id ivar = 0, ijvar = 0; ivar < nvar; ivar++)
        for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
        {
          sum1 = sum2 = 0;
          aicLocal[icov].setValue(ivar, jvar, 0.);
          for (Id ipadir = 0; ipadir < npadir; ipadir++)
          {
            if (FFFF(WT(ijvar, ipadir))) continue;
            temp = WT(ijvar, ipadir) * ge[icov].getValue(ijvar, ipadir);
            fk[icov].setValue(ijvar, ipadir, temp);
            sum1 += temp * GG(ijvar, ipadir);
            sum2 += temp * ge[icov].getValue(ijvar, ipadir);
          }
          alphak[icov].setValue(ivar, jvar, 1. / sum2);
          aicLocal[icov].setValue(ivar, jvar, sum1 / sum2);
        }
    crit = _calculateScoreMP(nvar, npadir, wt, gg, mp);

    /***********************/
    /* Iterative procedure */
    /***********************/

    for (Id iter = 0; iter < niter; iter++)
    {
      _iterg++;

      /* Loop on the elementary structures */

      for (Id icov = 0; icov < ncova; icov++)
      {

        /* Establish the coregionalization matrix */

        for (Id ivar = 0, ijvar = 0; ivar < nvar; ivar++)
          for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
          {
            sum = 0;
            for (Id ipadir = 0; ipadir < npadir; ipadir++)
            {
              mp.setValue(
                ijvar, ipadir,
                mp.getValue(ijvar, ipadir)
                  - (sill[icov].getValue(ivar, jvar) * ge[icov].getValue(ijvar, ipadir)));
              sum +=
                fk[icov].getValue(ijvar, ipadir) * mp.getValue(ijvar, ipadir);
            }
            coeff = aicLocal[icov].getValue(ivar, jvar)
                  - alphak[icov].getValue(ivar, jvar) * sum;
            cc.setValue(ivar, jvar, coeff);
            cc.setValue(jvar, ivar, coeff);
          }

        /* Computing and sorting the eigen values and eigen vectors */

        auto eigenvectors = EigenVectors(cc);
        const auto& valpro = eigenvectors.getEigenValues();
        vecpro = &eigenvectors.getEigenVectors();

        Id kvar = 0;
        allpos = 1;
        while ((kvar < nvar) && allpos)
        {
          if (valpro[kvar++] < 0) allpos = 0;
        }

        /* Calculate the new coregionalization matrix */

        for (Id ivar = 0, ijvar = 0; ivar < nvar; ivar++)
          for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
          {
            if (allpos)
            {
              sill[icov].setValue(ivar, jvar, cc.getValue(ivar, jvar));
            }
            else
            {
              sum = 0.;
              for (kvar = 0; kvar < nvar; kvar++)
                sum +=
                  (MAX(valpro[kvar], 0.) * vecpro->getValue(ivar, kvar)
                   * vecpro->getValue(jvar, kvar));
              sill[icov].setValue(ivar, jvar, sum);
            }
            for (Id ipadir = 0; ipadir < npadir; ipadir++)
              mp.setValue(
                ijvar, ipadir,
                mp.getValue(ijvar, ipadir)
                  + (sill[icov].getValue(ivar, jvar) * ge[icov].getValue(ijvar, ipadir)));
          }
      }

      /* Update the global criterion */

      crit_mem = crit;
      crit = _calculateScoreMP(nvar, npadir, wt, gg, mp);

      /* Stopping criterion */

      if (_convergenceReached(iter, crit, crit_mem)) break;
    }

    _score = crit;
    return (0);
  }

  bool
    AModelFitSills::_convergenceReached(Id iter, double crit, double crit_mem)
      const
  {
    if (_trace)
    {
      message("  Sill Fitting Iteration %3d - Cost = %lf\n", iter + 1, crit);
      for (Id icova = 0; icova < _ncova; icova++)
      {
        message("  - Covariance %d :", icova + 1);
        printVector(
          _sill[icova].getValues(), " Current parameters ", true, false);
      }
    }
    double eps = _mop.getTolred();
    return (ABS(crit) < eps || ABS(crit - crit_mem) / ABS(crit) < eps);
  }

  void AModelFitSills::printFitSillSummary(Id niter) const
  {
    message(
      "  Summary of Sill Fitting - Count of Iterations = %4d - Final Cost = "
      "%lf\n",
      niter, _score);
    for (Id icova = 0; icova < _ncova; icova++)
    {
      message("  - Covariance %d: Final parameters = ", icova + 1);
      for (Id ivar = 0; ivar < _nvar; ivar++)
        for (Id jvar = 0; jvar < _nvar; jvar++)
          message("%lf ", _model->getSill(icova, ivar, jvar));
      message("\n");
    }
  }

  Id AModelFitSills::_fitSillMatrices()
  {
    Id status = 0;
    _score = 0.;
    _iterg = 0;
    if (!_mop.getFlagIntrinsic())
    {

      /* No intrinsic hypothesis */

      if (_constraints == nullptr || FFFF(_constraints->getConstantSillValue()))
      {
        /* Without constraint on the sill */

        status = _goulardWithoutConstraint(
          _mop.getMaxiter(), _nvar, _ncova, _npadir, _wt, _gg, _ge, _sill);
      }
      else
      {
        /* With constraint on the sill */

        status = _goulardWithConstraints();
      }

      // Store the sills into the Model
      _storeSillsInModel();
    }
    else
    {
      status = _sillFittingIntrinsic();
    }

    if (_verbose) printFitSillSummary(_iterg);

    return (status);
  }
} // namespace gstlrn
