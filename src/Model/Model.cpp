...
namespace gstlrn
{
  ...
  Model& Model::operator=(const Model& m)
  {
    if (this != &m)
    {
      ...
      setCovAnisoList(
        dynamic_cast<CovAnisoList*>(m.getCovAnisoList()->clone()));
      if (m._driftList != nullptr) _driftList = m._driftList->clone();
      _ctxt = m._ctxt;
    }
    return (*this);
  }
    ...
    bool flagRange)
  {
    ...
    Id ndimRanges =
      1; // By default, the argument 'range' gives the space dimension.
    ...
    model->addCovFromParam(
      type, range, sill, param, ranges, sills, angles, flagRange);
    return model;
  }
    ...
    bool verbose)
  {
    ...
    {
      messerr(
        "Problem when creating Model from fitting an Experimental variogram");
      delete model;
      return nullptr;
    }
    return model;
  ...
  String Model::toString(const AStringFormat* /*strfmt*/) const
  {
    ...
    sstr << "Number of drift equation(s)  = " << getNDriftEquation()
         << std::endl;
    ...
    if (ndrift > 0) sstr << toStrTitle(1, "Drift Part");

    sstr << _driftList->toString();

    if (isFlagLinked()) sstr << "Drifts are linked" << std::endl;
    ...
    bool flagRange)
  {
    ...
    if (!ranges.empty())
    {
      ...
      {
        messerr(
          "Mismatch between the dimension of 'ranges' (%d)",
    ...
    if (!angles.empty())
    {
      ...
      {
        messerr(
          "Mismatch between the dimension of 'angles' (%d)",
    ...
    if (!sills.empty())
    {
      ...
      {
        messerr(
          "Mismatch between the number of rows 'sills' (%d)", sills.getNRows());
    ...
    const ECalcMember& member) const
  {
    ...
    if (member != ECalcMember::LHS && covalist->isFiltered(icov)) return (0.);
  ...
  Id Model::setAnam(const gstlrn::AAnam* anam, const VectorInt& strcnt)
  {
    ...
    if (hasAnam())
    {
      ...
      {
        messerr(
          "Impossible to reach the internal CovLMCAnamorphosis structure");
        return 1;
      }
      cov->setAnam(anam);
    }
  ...
  Id Model::unsetAnam()
  {
    ...
    {
      messerr(
        "Impossible to unset 'anam' from the covariance part of the Model");
  ...
  bool Model::_deserializeAscii(std::istream& is)
  {
    ...
    for (Id icova = 0; ret && icova < ncova; icova++)
    {
      ...
      if (flag_aniso != 0)
      {
        ...
        for (Id idim = 0; idim < ndim; idim++)
          ret = ret
             && _recordRead<double>(
                  is, "Anisotropy coefficient", aniso_ranges[idim]);
        if (!ret) return ret;
        for (Id idim = 0; idim < ndim; idim++) aniso_ranges[idim] *= range;

        ret =
          ret
          && _recordRead<Id>(is, "Flag for Anisotropy Rotation", flag_rotation);
        if (!ret) return ret;
        if (flag_rotation != 0)
        {
          ...
          for (Id idim = 0; ret && idim < ndim; idim++)
            for (Id jdim = 0; ret && jdim < ndim; jdim++)
              ret = ret
                 && _recordRead<double>(
                      is, "Anisotropy Rotation Matrix", aniso_rotmat[lec++]);
        }
      }
      if (!ret) return ret;
    ...
    for (Id ivar = 0; ret && ivar < nvar; ivar++)
      ...
      {
        ret = ret
           && _recordRead<double>(is, "Variance-covariance at Origin", value);
  ...
  bool Model::_serializeAscii(std::ostream& os) const
  {
    ...
    ret = ret
       && _recordWrite<Id>(os, "Number of basic covariance terms", getNCov());
    ...
    for (Id icova = 0; ret && icova < getNCov(); icova++)
    {
      ...
      ret = ret
         && _recordWrite<double>(
              os, "Covariance characteristics", cova->getParam());

      // Writing the Anisotropy information

      ret = ret
         && _recordWrite<Id>(
              os, "Anisotropy Flag", static_cast<Id>(cova->getFlagAniso()));
      ...
      ret = ret
         && _recordWrite<Id>(
              os, "Anisotropy Rotation Flag",
              static_cast<Id>(cova->getFlagRotation()));
      ...
      for (Id idim = 0; ret && idim < ndim; idim++)
        for (Id jdim = 0; ret && jdim < ndim; jdim++)
          ret = ret
             && _recordWrite<double>(
                  os, "", cova->getAnisoRotMatElement(jdim, idim));
    ...
    for (Id ibfl = 0; ret && ibfl < getNDrift(); ibfl++)
    {
      ...
      ret = ret
         && _recordWrite<String>(os, "Drift Identifier", drift->getDriftName());
    }
    ...
    if (getNDrift() <= 0)
      ...
      {
        ret =
          ret && _recordWrite<double>(os, "Mean of Variables", getMean(ivar));
      }
    ...
    for (Id ivar = 0; ret && ivar < getNVar(); ivar++)
      for (Id jvar = 0; ret && jvar < getNVar(); jvar++)
        ret = ret
           && _recordWrite<double>(os, "", getContext()->getCovar0(ivar, jvar));
  ...
  const CovAnisoList* Model::castInCovAnisoListConst(Id icov) const
  {
    ...
    {
      messerr(
        "The member '_cova' in this model cannot be converted into a pointer "
        "to CovAnisoList");
      return nullptr;
    }
    ...
    {
      messerr(
        "The rank 'icov' (%d) is not valid. The CovAnisoList contains %d "
        "covariances",
  ...
  Id Model::stabilize(double percent, bool verbose)
  {
    ...
    for (Id icov = 0; icov < ncov; icov++) setSill(icov, 0, 0, 1. - total);
  ...
  void
    Model::gofDisplay(double gof, bool byValue, const VectorDouble& thresholds)
  {
  ...
  bool Model::deserializeH5(H5::Group& grp)
  {
    ...
    for (Id icov = 0; ret && icov < ncov; icov++)
    {
      ...
      if (flag_aniso)
      {
        ...
        // Rotation
        ret =
          ret && SerializeHDF5::readValue(*covG, "FlagRotation", flag_rotation);
        if (flag_rotation)
