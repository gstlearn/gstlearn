...
namespace gstlrn
{
                              ...
                              bool flagRange)
  {
    ...
    if (!sills.empty())
    {
      ...
      {
        messerr("Mismatch between the number of rows 'sills' (%d)",
                sills.getNRows());
  ...
  bool Model::_serializeAscii(std::ostream& os) const
  {
    ...
    for (Id icova = 0; ret && icova < getNCov(); icova++)
    {
      ...
      ret = ret
         && _recordWrite<Id>(os,
                             "Anisotropy Flag",
                             static_cast<Id>(cova->getFlagAniso()));
      ...
      for (Id idim = 0; ret && idim < ndim; idim++)
        for (Id jdim = 0; ret && jdim < ndim; jdim++)
          ret = ret
             && _recordWrite<double>(os,
                                     "",
                                     cova->getAnisoRotMatElement(jdim, idim));
  ...
  const CovAnisoList* Model::castInCovAnisoListConst(Id icov) const
  {
    ...
    if (covalist == nullptr)
    {
      messerr("The member '_cova' in this model cannot be converted into a "
              "pointer to CovAnisoList");
      return nullptr;
    }
    ...
    {
      messerr("The rank 'icov' (%d) is not valid. The CovAnisoList contains %d "
              "covariances",
              icov,
              covalist->getNCov());
      return nullptr;
