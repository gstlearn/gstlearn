...
namespace gstlrn
{
  ...
  void simu_func_categorical_transf(Db* db, Id verbose, Id isimu, Id nbsimu)
  {
    ...
    rule->gaus2facResult(
      ModCat.propdef, db, ModCat.flag_used, ModCat.ipgs, isimu, nbsimu);
  ...
  static Id st_keep(Id flag_gaus, Id flag_prop, Id file, Id type)
  {
    ...
    else
    {
      ...
      switch (type)
      {
        case 0: /* Gaussian */ keep = (flag_gaus && !flag_prop); break;

        case 1: /* Facies */ keep = (!flag_gaus && !flag_prop); break;

        case 2: /* Proportion */ keep = (flag_prop); break;
   ...
   **
   *****************************************************************************/
  static void st_check_facies_data2grid(
    Db* dbin,
    Db* dbout,
    Id flag_check,
    Id flag_show,
    ...
    const NamingConvention& namconv)
  {
    ...
    for (Id igrf = 0; igrf < 2; igrf++)
    {
      ...
      {
        messerr(
          "The number of variables in the model #%d (%d) should be 1", igrf + 1,
          model1->getNVar());
        goto label_end;
      }
    ...
    if (flag_cond)
    {
      ...
      if (db_locator_attribute_add(
            dbin, ELoc::GAUSFAC, ngrf * nbsimu, 0, 0., &iptr))
        goto label_end;

      /* Non-conditional simulations at data points */
      if (db_locator_attribute_add(
            dbin, ELoc::SIMU, ngrf * nbsimu, 0, 0., &iptr_DN))
        goto label_end;
    }

    /* (Non-) Conditional simulations at target points */
    if (db_locator_attribute_add(
          dbout, ELoc::SIMU, ngrf * nbsimu, 0, 0., &iptr_RN))
      goto label_end;
    ...
    propdef = proportion_manage(
      1, 1, flag_stat, ngrf, 0, nfacies, 0, dbin, dbprop, propcst, propdef);
    ...
    if (flag_cond)
    {
      ...
      AGibbs* gibbs =
        GibbsFactory::createGibbs(dbin, modvec, rule->getRho(), false);
      ...
      for (Id isimu = 0; isimu < nbsimu; isimu++)
      {
        for (Id igrf = 0; igrf < ngrf; igrf++)
          if (rule->evaluateBounds(
                propdef, dbin, dbout, isimu, igrf, ipgs, nbsimu))
            goto label_end;
    ...
    if (dbout != nullptr)
    {
      if (!st_keep(flag_gaus, flag_prop, RESULT, TYPE_PROP) && iptr_RP >= 0)
        ...
        namconv.setNamesAndLocators(
          NULL, VectorString(), ELoc::Z, -1, dbout, iptr_RP, "Props", nbsimu,
          false);
      ...
      if (!st_keep(flag_gaus, flag_prop, RESULT, TYPE_GAUS))
        ...
        namconv.setNamesAndLocators(
          NULL, VectorString(), ELoc::Z, -1, dbout, iptr_RN, "Gaus",
          ngrf * nbsimu, false);
      ...
      if (!st_keep(flag_gaus, flag_prop, RESULT, TYPE_FACIES) && iptr_RF >= 0)
        ...
        namconv.setNamesAndLocators(
          NULL, VectorString(), ELoc::Z, -1, dbout, iptr_RF, String(), nbsimu);
    }
    ...
    if (dbin != nullptr)
    {
      if (!st_keep(flag_gaus, flag_prop, DATA, TYPE_GAUS))
        ...
        namconv.setNamesAndLocators(
          NULL, VectorString(), ELoc::Z, -1, dbin, iptr_DN, "Gaus",
          ngrf * nbsimu, false);
      ...
      if (!st_keep(flag_gaus, flag_prop, DATA, TYPE_FACIES) && iptr_DF >= 0)
        ...
        namconv.setNamesAndLocators(
          NULL, VectorString(), ELoc::Z, -1, dbin, iptr_DF, String(), nbsimu,
          false);
    ...
    const NamingConvention& namconv)
  {
    ...
    propdef = proportion_manage(
      1, 1, flag_stat, ngrf[0], ngrf[1], nfac[0], nfac[1], dbin, dbprop,
      propcst, propdef);
    if (propdef == nullptr) goto label_end;
    simu_define_func_update(simu_func_categorical_update);
    simu_define_func_scale(simu_func_categorical_scale);
    ...
    if (db_locator_attribute_add(
          dbout, ELoc::FACIES, npgs * nbsimu, 0, 0., &iptr_RF))
      goto label_end;
    ...
    {
      if (db_locator_attribute_add(
            dbin, ELoc::FACIES, npgs * nbsimu, 0, 0., &iptr_DF))
        goto label_end;
    }
    ...
    {
      if (db_locator_attribute_add(
            dbin, ELoc::GAUSFAC, ngrftot * nbsimu, 0, 0., &iptr))
        goto label_end;
    }
    ...
    {
      if (db_locator_attribute_add(
            dbin, ELoc::SIMU, ngrftot * nbsimu, 0, 0., &iptr_DN))
        goto label_end;
    }

    /* Non-conditional gaussian simulations at target points */
    if (db_locator_attribute_add(
          dbout, ELoc::SIMU, ngrftot * nbsimu, 0, 0., &iptr_RN))
      goto label_end;
    ...
    for (Id ipgs = 0; ipgs < npgs; ipgs++)
    {
      ...
      if (flag_cond)
      {
        ...
        AGibbs* gibbs = GibbsFactory::createGibbs(
          dbin, modvec[ipgs], rules[ipgs]->getRho(), false);
        ...
        for (Id isimu = 0; isimu < nbsimu; isimu++)
        {
          if (rules[ipgs]->gaus2facData(
                propdef, dbin, dbout, flag_used[ipgs], ipgs, isimu, nbsimu))
            goto label_end;
        }
      ...
      for (Id igrf = 0; igrf < 2; igrf++)
      {
        ...
        local_seed = 0;
        if (situba.simulate(
              dbin, dbout, models[ipgs][igrf], neigh, icase, false, true))
          goto label_end;
      }
    ...
    if (dbout != nullptr)
    {
      if (!st_keep(flag_gaus, flag_prop, RESULT, TYPE_PROP) && iptr_RP >= 0)
        ...
        namconv.setNamesAndLocators(
          NULL, VectorString(), ELoc::Z, -1, dbout, iptr_RP, "Props", nfactot,
          false);
      ...
      if (!st_keep(flag_gaus, flag_prop, RESULT, TYPE_FACIES) && iptr_RF >= 0)
        ...
        namconv.setNamesAndLocators(
          NULL, VectorString(), ELoc::Z, -1, dbout, iptr_RF, String(),
          npgs * nbsimu);
    }
    ...
    if (dbin != nullptr)
    {
      ...
      if (!st_keep(flag_gaus, flag_prop, DATA, TYPE_FACIES) && iptr_DF >= 0)
        ...
        namconv.setNamesAndLocators(
          NULL, VectorString(), ELoc::Z, -1, dbin, iptr_DF, String(),
          npgs * nbsimu, false);
    ...
    const NamingConvention& namconv)
  {
    ...
    propdef = proportion_manage(
      1, 0, 1, 1, 0, nvar, 0, dbin, NULL, VectorDouble(), propdef);
    if (propdef == nullptr) goto label_end;
