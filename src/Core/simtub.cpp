...
namespace gstlrn
{
  ...
  void simu_func_categorical_transf(Db* db, Id verbose, Id isimu, Id nbsimu)
  {
    ...
    rule->gaus2facResult(ModCat.propdef,
                         db,
                         ModCat.flag_used,
                         ModCat.ipgs,
                         isimu,
                                 ...
                                 const ELoc& locatorType)
  {
    if (get_LOCATOR_NITEM(db, locatorType) <= 0)
      messageAbort("%s : Attributes %d are mandatory",
                   method,
                   locatorType.getValue());
  }
                                        ...
                                        ANeigh* neigh)
  {
    ...
    if (flag_cond && neigh != nullptr)
    {
      ...
      {
        messerr("The Space Dimension of the Neighborhood (%d)",
      ...
      {
        messerr("The Cross-Validation can only be processed with Moving "
                "neighborhood");
        return 1;
      }
    }
                                        ...
                                        Id nbsimu)
  {
    ...
    check_mandatory_attribute("st_check_facies_data2grid",
                              dbgrid,
                              ELoc::FACIES);
    number = 0;
    if (flag_check)
      mestitle(1,
               "Checking facies of data against closest grid node (PGS=%d)",
               ipgs + 1);
      ...
      for (isimu = 0; isimu < nbsimu; isimu++)
      {
        ...
        if (flag_show)
        {
          if (facdat == facres)
            ...
            dbgrid
              ->setSimvar(ELoc::FACIES, jech, isimu, 0, ipgs, nbsimu, 1, 0.);
        }
            ...
            const NamingConvention& namconv)
  {
    ...
    {
      messerr("Calculating the facies proportions is incompatible with storing "
              "the Gaussian values");
      goto label_end;
    }
    ...
    if (flag_cond)
    {
      ...
      if (db_locator_attribute_add(dbin,
                                   ELoc::GAUSFAC,
                                   ngrf * nbsimu,
                                   0,
                                   0.,
    ...
    if (db_locator_attribute_add(dbout,
                                 ELoc::SIMU,
                                 ngrf * nbsimu,
                                 0,
                                 0.,
    ...
    if (flag_cond)
    {
      ...
      for (Id isimu = 0; isimu < nbsimu; isimu++)
      {
        for (Id igrf = 0; igrf < ngrf; igrf++)
          if (rule->evaluateBounds(propdef,
                                   dbin,
                                   dbout,
                                   isimu,
                                   igrf,
    ...
    if (flag_cond && !flag_gaus && (flag_check || flag_show))
      st_check_facies_data2grid(dbin,
                                dbout,
                                flag_check,
                                flag_show,
                                0,
    ...
    if (dbout != nullptr)
    {
      if (!st_keep(flag_gaus, flag_prop, RESULT, TYPE_PROP) && iptr_RP >= 0)
        ...
        namconv.setNamesAndLocators(NULL,
                                    VectorString(),
                                    ELoc::Z,
                                    -1,
                                    dbout,
              ...
              const NamingConvention& namconv)
  {
    ...
    for (Id ipgs = 0; ipgs < npgs; ipgs++)
    {
      ...
      for (Id igrf = 0; igrf < 2; igrf++)
      {
        ...
        {
          messerr("Variable #%d needs the underlying GRF #%d",
                  ipgs + 1,
                  igrf + 1);
          messerr("No corresponding Model is provided");
          goto label_end;
        ...
        {
          messerr("The number of variables in Model #%d (%d) for Variable %d "
                  "should be 1",
                  igrf + 1,
                  ipgs + 1,
                  models[ipgs][igrf]->getNVar());
          goto label_end;
        }
    ...
    for (Id ipgs = 0; ipgs < npgs; ipgs++)
    {
      ...
      if (flag_cond)
      {
        ...
        AGibbs* gibbs = GibbsFactory::createGibbs(dbin,
                                                  modvec[ipgs],
                                                  rules[ipgs]->getRho(),
                                                  false);
        ...
        for (Id isimu = 0; isimu < nbsimu; isimu++)
        {
          ...
          for (Id igrf = 0; igrf < ngrf[ipgs]; igrf++)
            if (rules[ipgs]->evaluateBounds(propdef,
                                            dbin,
                                            dbout,
                                            isimu,
                                            igrf,
        ...
        for (Id isimu = 0; isimu < nbsimu; isimu++)
        {
          if (rules[ipgs]->gaus2facData(propdef,
                                        dbin,
                                        dbout,
                                        flag_used[ipgs],
                                        ipgs,
      ...
      if (nbounds > 0)
      {
        ...
        if (nvarDb > 0)
        {
          if (nvarDb != nvar)
          {
            ...
            messerr("Their count (%d) must match the number of variables "
                    "defined in the Model (%d)",
                    nvarDb,
                    nvar);
            return 1;
          }
             ...
             Id verbose)
  {
    ...
    if (manageExternalInformation(1, ELoc::F, dbin, dbout, &flag_ext_created))
      goto label_end;
    if (manageExternalInformation(1,
                                  ELoc::NOSTAT,
                                  dbin,
                                  dbout,
                                  &flag_nostat_created))
  ...
  label_end:
    ...
    (void)manageExternalInformation(-1,
                                    ELoc::NOSTAT,
                                    dbin,
                                    dbout,
                                    &flag_nostat_created);
