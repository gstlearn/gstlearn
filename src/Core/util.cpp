...
namespace gstlrn
{
   ...
   **
   *****************************************************************************/
  static void st_keypair_attributes(
    Keypair* keypair,
    Id mode,
    Id origin,
    Id /*nrow*/,
  ...
  static void st_keypair_copy(Keypair* keypair, Id type, Id start, void* values)
  {
    ...
    if (type == 1)
    {
      ...
      for (Id i = 0; i < size; i++) keypair->values[i + start] = icopy[i];
    }
    else
    {
      rcopy = static_cast<double*>(values);
      for (Id i = 0; i < size; i++) keypair->values[i + start] = rcopy[i];
    }
  }
    ...
    const double* values)
  {
    ...
    st_keypair_copy(
      keypair, 2, 0, static_cast<void*>(const_cast<double*>(values)));
  }
  ...
  void del_keypair(const char* keyword, Id flag_exact)
  {
    ...
    {
      for (Id i = KEYPAIR_NTAB - 1; i >= 0; i--) del_keypone(i);
    }
  ...
  Id get_keypair(const char* keyword, Id* nrow, Id* ncol, VectorDouble& values)
  {
    ...
    *ncol = keypair->ncol;
    ...
    values.resize(size);
    for (Id i = 0; i < size; i++) values[i] = keypair->values[i];
      ...
      for (i = 0; i < KEYPAIR_NTAB; i++)
      {
        ...
        if (flag_short)
        {
          if (keypair->origin == 1)
            ...
            message("R ");
          message(
            "- %s (%d x %d)\n", keypair->keyword, keypair->nrow, keypair->ncol);
        }
        else
          printMatrix(
            keypair->values, keypair->ncol, keypair->nrow, keypair->keyword);
      }
  }
  ...
  double ut_distance(Id ndim, const double* tab1, const double* tab2)
  {
    ...
    if (flag_sphere)
    {
      ...
      distance =
        GH::geodeticAngularDistance(tab1[0], tab1[1], tab2[0], tab2[1], R);
    }
    else
    {
