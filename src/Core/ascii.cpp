...
namespace gstlrn
{
  ...
  static void st_record_write(const char* format, ...)
  {
    ...
    else
    {
      ...
      long2 =
        (!ASCII_BUFFER.empty()) ? static_cast<Id>(ASCII_BUFFER.size()) : 0;
   ...
   *****************************************************************************/
  static void
    st_filename_patch(const char* ref_name, Id rank, Id mode, String& filename)
  {
    if (rank == 0)
    {
      switch (mode)
      {
        case 0:
          (void)
            gslSPrintf(filename, "%s/%s.%s", STUDY.data(), ref_name, EXT_DAT);
          break;

        case 1: (void)gslSPrintf(filename, "%s.%s", ref_name, EXT_OUT); break;

        case -1: (void)gslSPrintf(filename, "%s", ref_name); break;
      }
    }
    else
    {
      switch (mode)
      {
        case 0:
          (void)gslSPrintf(
            filename, "%s/%s%1d.%s", STUDY.data(), ref_name, rank, EXT_DAT);
          break;

        case 1:
          (void)gslSPrintf(filename, "%s%1d.%s", ref_name, rank, EXT_OUT);
          break;

        case -1: (void)gslSPrintf(filename, "%s%1d", ref_name, rank); break;
      }
    }
  }
    ...
    bool verbose)
  {
    ...
    if (mode == OLD)
    {
      ...
      {
        messerr(
          "Error: in the File (%s), its Type (%s) does not match the requested "
          "one (%s)",
    ...
    bool flagAddSampleRank)
  {
    ...
    db = Db::createFromSamples(
      nrow, ELoadBy::SAMPLE, tab, VectorString(), VectorString(),
      flagAddSampleRank);
