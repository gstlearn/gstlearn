...
#define F(i, j) (st_index(i, j))
    ...
    for (Id oper = 1; oper <= noper; oper++)
    {
      ...
      for (Id is = 0; is < ndiv; is++, number++)
      {
        ...
        for (Id i = 0; i < 2; i++)
        {
          ...
          st_relem_define(split->relems[i],
                          ncur,
                          relem0->facies,
                          1 - i,
                          &DIVS(is, 0));
  ...
  static void st_set_rho(double rho, Local_Pgs* local_pgs)
  {
    ...
    rho2 = rho * rho;
    st_variogram_define_vars(local_pgs->vario,
                             local_pgs->rule,
                             local_pgs->ngrf);
...

  {
    ...
    {
      if (rule_thresh_define(local_pgs->propdef,
                             local_pgs->db,
                             local_pgs->rule,
                             ifac + 1,
                             0,
    ...
    if (ABS(total - 1.) > EPSILON3)
      messerr("In st_calculate_thresh_stat, the sum of Probabilities (%lf) is "
              "not close to 1.",
              total);
    return (0);
  }
  ...
  static void st_varcalc_uncorrelated_grf(Local_Pgs* local_pgs, Id idir)
  {
    ...
    for (ilag = 0; ilag < vario->getNLag(idir); ilag++)
    {
      ...
      vario_order_get_bounds(local_pgs->vorder,
                             idir,
                             ilag,
                             &local_pgs->ifirst,
                             &local_pgs->ilast);
  ...
  static double st_rule_calcul(Local_Pgs* local_pgs, Id* string)
  {
    ...
    else
    {
      ...
      {
        local_pgs->idircur = idir;
        st_variogram_patch_C00(local_pgs,
                               local_pgs->vario,
                               idir,
...

  {
    ...
    if (split->nrule <= 0)
    {
      ...
      if (nprod > 0)
      {
        st_rule_product(split,
                        nprod,
                        num[0],
                        nby[0],
                        *ptr[0],
                        split->relems[0]->Rfipos,
                        num[1],
                        nby[1],
                        *ptr[1],
                        split->relems[1]->Rfipos);
        if (verbose)
          st_rules_print("Split",
                         split->nrule,
                         split->nbyrule,
                         split->Srules,
                         split->Sfipos);
                                 ...
                                 MatrixSymmetric& JJ)
  {
    ...
    for (Id ipair = local_pgs->ifirst; ipair < local_pgs->ilast; ipair++)
    {
      ...
      (void)rule_thresh_define(local_pgs->propdef,
                               local_pgs->db,
                               local_pgs->rule,
                               ifac1,
                               i1,
    ...
    st_optim_onelag_pgs(Local_Pgs* local_pgs, double tolsort, Id new_val)
  {
    ...
    while (!flag_sortie)
    {
      if (barrier)
      {
        ...
        AMatrix::linearCombinationInPlace(Hess,
                                          0.,
                                          npar,
                                          Hess,
                                          penalize / (eigval[3] * eigval[3]),
                                          d2);
        AMatrix::linearCombinationInPlace(JJ,
                                          0.,
                                          npar,
                                          JJ,
                                          penalize / (eigval[3] * eigval[3]),
      ...
      else
      {
        ...
        {
          VH::linearCombinationInPlace(delta / normgrad,
                                       hsd,
                                       0.,
                                       VectorDouble(),
                                       step);
                                                ...
                                                Id idir)
  {
    ...
    for (Id ilag = 0; ilag < vario->getNLag(idir); ilag++)
      for (Id igrf = 0; igrf < ngrf; igrf++)
        ...
        {
          iad = vario->getAddressForGg(idir, igrf, jgrf, ilag, 1);
          vario->setGgByIndex(idir,
                              iad,
                              st_param_expand(local_pgs, igrf, jgrf, 1));
          if (vario->getSwByIndex(idir, iad) > 0.)
            vario->setHhByIndex(idir,
                                iad,
                                vario->getHhByIndex(idir, iad)
                                  / vario->getSwByIndex(idir, iad));
          iad = vario->getAddressForGg(idir, igrf, jgrf, ilag, -1);
          vario->setGgByIndex(idir,
                              iad,
                              st_param_expand(local_pgs, igrf, jgrf, -1));
        ...
        for (Id ivar = 0; ivar < nvar; ivar++)
          for (Id jvar = 0; jvar <= ivar; jvar++)
          {
            if (vario->getFlagAsym())
            {
              ...
              vario->setHhByIndex(idir,
                                  iad,
                                  vario->getHhByIndex(idir, iad) - dist);
              vario->setSwByIndex(idir,
                                  iad,
                                  vario->getSwByIndex(idir, iad) + 1);
              iad = vario->getAddressForGg(idir, ivar, jvar, ilag, -1);
              vario->setGgByIndex(idir, iad, 0.);
              vario->setHhByIndex(idir,
                                  iad,
    ...
    switch (mode)
    {
      ...
      case 1:
        ...
        if (flag_stat)
        {
          ...
          local_pgs->stat_thresh.resize(nfacies * 2 * 2,
                                        0.); // Do not use ngrf, use 2 instead
        }
                                   ...
                                   Id opt_correl)
  {
    ...
    st_set_rho(golden_search(st_rho_search,
                             static_cast<void*>(local_pgs),
                             GS_TOLSTOP_RHO,
                             -1.,
                             1.,
                                    ...
                                    Id opt_correl)
  {
    ...
    {
      st_set_rho(rule->getRho(), &local_pgs);
      if (st_variopgs_calcul_norho(vario,
                                   rule,
                                   &local_pgs,
                                   ngrf,
                                   opt_correl,
...

  {
    ...
    for (Id idir = 0; idir < vario->getNDir(); idir++)
    {
      ...
      for (Id ilag = 0; ilag < vario->getNLag(idir); ilag++)
      {
        ...
        for (Id ipair = local_pgs->ifirst; ipair < local_pgs->ilast; ipair++)
        {
          vario_order_get_indices(local_pgs->vorder,
                                  ipair,
                                  &iech,
                                  &jech,
                                  &dist);
                         ...
                         bool flagGg)
  {
    ...
    for (Id idir = 0; idir < vario1->getNDir(); idir++)
    {
      ...
      {
        messerr("Both variograms should share the same number of flags for "
                "Direction #%d",
                idir + 1);
        return 1;
      }
                       ...
                       Id opt_correl)
  {
    ...
    if (flag_stat)
    {
      if (!propcst.empty())
      {
        ...
        {
          messerr("Number of proportions in 'propcst' (%d) should match Number "
                  "of Facies in 'rule' (%d)",
                  static_cast<Id>(propcst.size()),
                  rule->getNFacies());
          return nullptr;
        }
        props = propcst;
      ...
      else
      {
        ...
        {
          messerr("Number of Facies in 'db' (%d) should match Number of facies "
                  "in 'rule' (%d)",
                  static_cast<Id>(props.size()),
                  rule->getNFacies());
          return nullptr;
        }
      }
    ...
    if (flag_stat)
      ...
      error = st_variogram_pgs_nostat(db,
                                      dbprop,
                                      vario,
                                      rule,
                                      props,
                   ...
                   Id verbose)
  {
    ...
    scores = st_relem_evaluate(Pile_Relem,
                               verbose,
                               fgrf,
                               fcmp,
                               &local_pgs,
