...
#define STAT_THRESH(ifac, igrf, rank)                                          \
  (local_pgs->stat_thresh[2 * (nfacies * (igrf) + (ifac)) + (rank)])
...
#define F(i, j) (st_index(i, j))
...

  {
    ...
    for (Id i = 0; i < 2; i++) split->relems[i] = nullptr;
    return (split);
  }
    ...
    for (Id oper = 1; oper <= noper; oper++)
    {
      ...
      for (Id is = 0; is < ndiv; is++, number++)
      {
        ...
        for (Id i = 0; i < 2; i++)
        {
          ...
          st_relem_define(
            split->relems[i], ncur, relem0->facies, 1 - i, &DIVS(is, 0));
  ...
  static void st_set_rho(double rho, Local_Pgs* local_pgs)
  {
    ...
    st_variogram_define_vars(
      local_pgs->vario, local_pgs->rule, local_pgs->ngrf);
   ...
   *****************************************************************************/
  static double
    st_get_proba_ind(double correl, double* low, double* up, Id iconf)
  {
    Id ier, infin[2];
    ...
    if (!TEST_DISCRET)
    {
      ...
      else
      {
        ...
        mvndst(
          2, low, up, infin, &correl, maxpts, abseps, releps, &err, &proba,
          &ier);
      }
    }
    else
...

  {
    ...
    if (ABS(total - 1.) > EPSILON3)
      messerr(
        "In st_calculate_thresh_stat, the sum of Probabilities (%lf) is not "
        "close to 1.",
        total);
    return (0);
  }
   ...
   **
   *****************************************************************************/
  static Id st_vario_pgs_variable(
    Id mode,
    Id ngrf,
    Id nfacies,
    Id flag_one,
    ...
    const Rule* rule)
  {
    ...
    switch (mode)
    {
      case 1:
        ...
        {
          iptr =
            db->addColumnsByConstant(number, 0., "Lower Rank", ELoc::RKLOW);
          if (iptr < 0) return (1);
      ...
      case 0:
        ...
        for (Id iech = 0; iech < db->getNSample(); iech++)
        {
          ...
          for (Id i = 0; i < nloop; i++)
          {
            ...
            st_set_bounds(
              db, flag_one, ngrf, nfacies, ifac, iech, t1min, t1max, t2min,
              t2max);
          }
        }
        break;
   ...
   **
   *****************************************************************************/
  static void st_variogram_patch_C00(
    Local_Pgs* local_pgs,
    Vario* vario,
    Id idir,
    double rho)
  ...
  static void trace_add_row(Local_Pgs* local_pgs)
  {
    ...
    for (Id icol = 0; icol < ncol; icol++) tracepgs->trace[iad + icol] = TEST;
    tracepgs->nrow = nrow;
  }
  ...
  static double st_func_search_stat(double correl, void* user_data)
  {
    ...
    local_pgs = static_cast<Local_Pgs*>(user_data);
    Id iconf0 = 0;
  ...
  static double st_func_search_nostat(double correl, void* user_data)
  {
    ...
    if (local_pgs->flag_stat)
      for (i = 0; i < static_cast<Id>(local_pgs->stat_proba.size()); i++)
    ...
    for (ipair = local_pgs->ifirst; ipair < local_pgs->ilast; ipair++)
    {
      ...
      if (local_pgs->flag_stat)
      {
        ...
        ifac1 = static_cast<Id>(local_pgs->db->getZVariable(i1, 0)) - 1;
        if (ifac1 < 0 || ifac1 >= local_pgs->nfacies) continue;
        ifac2 = static_cast<Id>(local_pgs->db->getZVariable(i2, 0)) - 1;
      ...
      if (FFFF(proba))
      {
        ...
        {
          low[0] =
            local_pgs->db->getLocVariable(ELoc::L, i1, local_pgs->igrfcur);
          up[0] =
            local_pgs->db->getLocVariable(ELoc::U, i1, local_pgs->igrfcur);
          low[1] =
            local_pgs->db->getLocVariable(ELoc::L, i2, local_pgs->igrfcur);
          up[1] =
            local_pgs->db->getLocVariable(ELoc::U, i2, local_pgs->igrfcur);
        }
        else
        {
          low[0] =
            local_pgs->db->getLocVariable(ELoc::RKLOW, i1, local_pgs->igrfcur);
          up[0] =
            local_pgs->db->getLocVariable(ELoc::RKUP, i1, local_pgs->igrfcur);
          low[1] =
            local_pgs->db->getLocVariable(ELoc::RKLOW, i2, local_pgs->igrfcur);
          up[1] =
            local_pgs->db->getLocVariable(ELoc::RKUP, i2, local_pgs->igrfcur);
        }
    ...
    const double* values)
  {
    ...
    if (2 + origin + number > ncol)
      messageAbort(
        "Error in Trace dimension (ncol=%d origin=%d number=%d)", ncol, origin,
        number);
   ...
   *****************************************************************************/
  static Id
    st_varcalc_from_vario_stat(Vario* vario, Local_Pgs* local_pgs, Id ngrf)
  {
    Id iad;
      ...
      for (Id ilag = 0; ilag < vario->getNLag(idir); ilag++)
      {
        ...
        for (Id igrf = 0; igrf < ngrf; igrf++)
        {
          ...
          result = golden_search(
            st_func_search_stat, static_cast<void*>(local_pgs), GS_TOLSTOP, -1.,
            1., &testval, &niter);
  ...
  static void st_varcalc_uncorrelated_grf(Local_Pgs* local_pgs, Id idir)
  {
    ...
    for (ilag = 0; ilag < vario->getNLag(idir); ilag++)
    {
      ...
      vario_order_get_bounds(
        local_pgs->vorder, idir, ilag, &local_pgs->ifirst, &local_pgs->ilast);
      ...
      for (igrf = 0; igrf < ngrf; igrf++)
      {
        local_pgs->igrfcur = igrf;
        result = golden_search(
          st_func_search_nostat, static_cast<void*>(local_pgs), GS_TOLSTOP, -1.,
          1., &testval, &niter);
        trace_define(local_pgs, idir + 1, ilag + 1, 2 * igrf, 1, &testval);
        trace_define(local_pgs, idir + 1, ilag + 1, 2 * igrf + 1, 1, &niter);
    ...
    else
    {
      ...
      for (Id idir = 0; idir < local_pgs->vario->getNDir(); idir++)
      {
        ...
        st_variogram_patch_C00(
          local_pgs, local_pgs->vario, idir, local_pgs->rule->getRho());
  ...
  static Id st_update_orientation(Id fac0, Id igrf_cas, VectorInt& fgrf)
  {
    ...
    for (Id igrf = 0; igrf < NGRF; igrf++)
    {
      ...
      if (loc0 - 2 * loc1 > 0)
      {
        ...
        for (Id i = 0; i < nmax; i++) fgrf[i] = st_permut(fgrf[i], igrf);
      }
      loc0 = loc1;
    }
   ...
   **
   *****************************************************************************/
  static VectorDouble st_relem_evaluate(
    Relem* relem,
    Id verbose,
    VectorInt& fgrf,
    VectorInt& fcmp,
    ...
    Id* r_opt)
  {
    ...
    flag_check = static_cast<Id>(get_keypone("Multi_Score_Check", 0.));
    nmax = static_cast<Id>(pow(2., static_cast<double>(NGRF)));
    nrule = relem->nrule;
    ...
    *r_opt = number = 0;
    for (Id ir = 0; ir < nrule; ir++)
    {
      ...
      if (verbose)
      {
        if (indice < 0)
          st_rule_print(
            ir, NRULE, rules, fipos, true, indice, igrf_opt, scores[ir]);
      }
    ...
    set_keypair_int("rule_auto_allrules", 1, nrule, NRULE, rules.data());
    ...
    const VectorInt& fipos1)
  {
    ...
    {
      for (Id ic = 0; ic < nbyrule1; ic++) RULES(ir, ic) = RULES1(i1, ic);
      for (Id ic = 0; ic < NCOLOR; ic++)
    ...
    const VectorInt& fipos2)
  {
    ...
    split->Srules.resize(NRULE * nprod, 0);
    VectorInt& rules = split->Srules;
    ...
    for (Id i1 = 0; i1 < nrule1; i1++)
      for (Id i2 = 0; i2 < nrule2; i2++, ir++)
      {
        ...
        for (Id i = 0; i < nbyrule1; i++) RULES(ir, ic++) = RULES1(i1, i);
        for (Id i = 0; i < nbyrule2; i++) RULES(ir, ic++) = RULES2(i2, i);
        for (Id i = 0; i < NCOLOR; i++)
        {
          if (fipos1[FIPOSAD(i1, i)] > 0)
            fipos[FIPOSAD(ir, i)] =
              fipos1[FIPOSAD(i1, i)] * BASE + st_define_fipos(oper, 1);
          if (fipos2[FIPOSAD(i2, i)] > 0)
            fipos[FIPOSAD(ir, i)] =
              fipos2[FIPOSAD(i2, i)] * BASE + st_define_fipos(oper, 0);
        }
        ...
        if (flag_debug)
        {
          ...
          st_rule_print(
            ir, nbyrule1 + nbyrule2 + 1, rules, fipos, false, -1, -1, TEST);
        }
      }
    split->nrule = nprod;
...

  {
    ...
    for (Id i = 0; i < 2; i++) st_relem_explore(split->relems[i], verbose);
    ...
    if (split->nrule <= 0)
    {
      ...
      {
        st_rule_product(
          split, nprod, num[0], nby[0], *ptr[0], split->relems[0]->Rfipos,
          num[1], nby[1], *ptr[1], split->relems[1]->Rfipos);
        if (verbose)
          st_rules_print(
            "Split", split->nrule, split->nbyrule, split->Srules,
            split->Sfipos);
      }
    }
  }
  ...
  static void st_relem_explore(Relem* relem, Id verbose)
  {
    ...
    for (Id is = 0; is < relem->nsplit; is++)
    {
      ...
      st_rule_glue(
        relem, split->nrule, split->nbyrule, split->Srules, split->Sfipos);
      if (verbose)
        st_rules_print(
          "Relem", relem->nrule, relem->nbyrule, relem->Rrules, relem->Rfipos);
    }
  }
    ...
    double dist)
  {
    ...
    if (vorder->npair >= vorder->nalloc)
    {
      ...
      if (vorder->flag_dist) vorder->tab_dist.resize(vorder->nalloc);
    }
  ...
  Vario_Order* vario_order_final(Vario_Order* vorder, Id* npair)
  {
    ...
    *npair = 0;
    ...
    {
      vorder =
        vario_order_manage(-1, vorder->flag_dist, vorder->size_aux, vorder);
      *npair = 0;
    }
    else if (vorder->npair > 0)
    {
      ...
      VH::arrangeInPlace(
        1, vorder->tab_sort, vorder->tab_ipas, true, vorder->npair);
      *npair = vorder->npair;
    }
    return (vorder);
  }
   ...
   **
   *****************************************************************************/
  void vario_order_get_auxiliary(
    Vario_Order* vorder,
    Id ipair,
    char* aux_iech,
    char* aux_jech)
   ...
   *****************************************************************************/
  static VectorDouble
    st_compute_params(Local_CorPgs* corpgs, VectorDouble& params_in)
  {
    ...
    MatrixSymmetric& correl)
  {
    ...
    correl.fill(0.);
    for (Id i = 0; i < 4; i++) correl.setValue(i, i, 1.);
   ...
   **
   *****************************************************************************/
  static void st_update_constraints(
    Local_CorPgs* corpgs,
    VectorDouble& Grad,
    MatrixSymmetric& Hess)
  {
    ...
    {
      double value = 0.;
      for (Id j = 0; j < 4; j++) value += v[j] * corpgs->modif.getValue(i, j);
      Grad[i] = value;
    }
      ...
      for (Id j = 0; j < npar; j++)
        for (Id k = 0; k < 4; k++)
          for (Id l = 0; l < 4; l++)
            Hess.updValue(
              i, j, EOperator::ADD,
              corpgs->modif.getValue(i, k) * m.getValue(k, l)
                * corpgs->modif.getValue(j, l));
  }
   ...
   **
   *****************************************************************************/
  static void st_update_constraints_with_JJ(
    Local_CorPgs* corpgs,
    VectorDouble& Grad,
    MatrixSymmetric& Hess,
    MatrixSymmetric& JJ)
      ...
      for (Id j = 0; j < npar; j++)
        for (Id k = 0; k < 4; k++)
          for (Id l = 0; l < 4; l++)
            JJ.updValue(
              i, j, EOperator::ADD,
              corpgs->modif.getValue(i, k) * m.getValue(l, k)
                * corpgs->modif.getValue(j, l));
  }
    ...
    MatrixSymmetric& d2)
  {
    ...
    for (Id i = 0; i < 4; i++)
      ...
      {
        double value = 0;
        value = ev->getValue(3, F(i, 0)) * ev->getValue(3, F(j, 0))
              * invGn.getValue(F(i, 1), F(j, 1));
        value += ev->getValue(3, F(i, 1)) * ev->getValue(3, F(j, 0))
               * invGn.getValue(F(i, 0), F(j, 1));
        value += ev->getValue(3, F(i, 0)) * ev->getValue(3, F(j, 1))
               * invGn.getValue(F(i, 1), F(j, 0));
        value += ev->getValue(3, F(i, 1)) * ev->getValue(3, F(j, 1))
               * invGn.getValue(F(i, 0), F(j, 0));
  ...
  static double st_param_expand(Local_Pgs* local_pgs, Id igrf, Id jgrf, Id idir)
  {
    ...
    switch (corpgs->opt_correl)
    {
      case 0:
        ...
        {
          if (idir > 0) return (corpgs->params[1]);
          return (corpgs->params[2]);
        }
        break;

      case 1:
        if (igrf == 0 && jgrf == 0) return (corpgs->params[0]);
        if (igrf == 1 && jgrf == 1) return (corpgs->params[2]);
  ...
  static void st_set_modif(Local_CorPgs* corpgs)
  {
    ...
    switch (corpgs->opt_correl)
    {
      case 0: /* Full parameters */
        corpgs->npar = 4;
        for (Id i = 0; i < 4; i++) corpgs->modif.setValue(i, i, 1.);
        break;
   ...
   *****************************************************************************/
  static void
    st_define_corpgs(Id option, Id flag_rho, double rho, Local_Pgs* local_pgs)
  {
    Local_CorPgs* corpgs;
    ...
    MatrixSymmetric& JJ)
  {
    ...
    for (Id ifac1 = 0; ifac1 < local_pgs->nfacies; ifac1++)
    {
      for (Id ifac2 = 0; ifac2 < local_pgs->nfacies; ifac2++)
      {
        ...
        {
          mvndst4(
            lower.data(), upper.data(), correl.getValues().data(), maxpts4,
            abseps, releps, &erval, &s, &inform);
   ...
   **
   *****************************************************************************/
  static double st_calcul_nostat(
    Local_Pgs* local_pgs,
    Id flag_deriv,
    Id flag_reset,
    MatrixSymmetric& correl,
    ...
    MatrixSymmetric& JJ)
  {
    ...
    {
      vario_order_get_indices(local_pgs->vorder, ipair, &i1, &i2, &dist);
      Id ifac1 = static_cast<Id>(local_pgs->db->getZVariable(i1, 0));
      Id ifac2 = static_cast<Id>(local_pgs->db->getZVariable(i2, 0));
      double w1 = local_pgs->db->getWeight(i1);
      double w2 = local_pgs->db->getWeight(i2);
    ...
    MatrixSymmetric& JJ)
  {
    ...
    if (local_pgs->flag_stat)
      S = st_calcul_stat(
        local_pgs, flag_deriv, flag_reset, correl, Grad, Hess, JJ);
    else
      S = st_calcul_nostat(
        local_pgs, flag_deriv, flag_reset, correl, Grad, Hess, JJ);
  ...
  static void st_initialize_params(Local_CorPgs* corpgs)
  {
    ...
    switch (corpgs->opt_correl)
    {
      ...
      case 2: corpgs->params[0] = corpgs->params[1] = fabs(r); break;
    }
  }
   ...
   *****************************************************************************/
  static double
    st_optim_onelag_pgs(Local_Pgs* local_pgs, double tolsort, Id new_val)
  {
    MatrixSymmetric invGn(4);
    MatrixSymmetric correl(4);
    MatrixSymmetric Hess(4);
    ...
    while (!flag_sortie)
    {
      ...
      if (flag_moved)
      {
        ...
        Srpen = Spen;
        VH::linearCombinationInPlace(
          1, param_temp, 0, VectorDouble(), corpgs->params);
    ...
    if (OptDbg::query(EDbg::CONVERGE))
    {
      ...
      for (Id i = 0; i < corpgs->npar; i++) message(" %lf", corpgs->params[i]);
      message("\n");
    }
  ...
  static Id st_discard_point(Local_Pgs* local_pgs, Id iech)
  {
    ...
    Id ifac = static_cast<Id>(local_pgs->db->getZVariable(iech, 0));
    ...
    else
    {
      if (get_LOCATOR_NITEM(local_pgs->db, ELoc::RKLOW) <= 0
          && get_LOCATOR_NITEM(local_pgs->db, ELoc::RKUP) <= 0)
        return (0);
      low =
        local_pgs->db->getLocVariable(ELoc::RKLOW, iech, local_pgs->igrfcur);
   ...
   **
   *****************************************************************************/
  static void st_variogram_geometry_pgs_correct(
    Local_Pgs* local_pgs,
    Vario* vario,
    Id idir)
  {
    ...
    for (Id ilag = 0; ilag < vario->getNLag(idir); ilag++)
      for (Id igrf = 0; igrf < ngrf; igrf++)
        for (Id jgrf = 0; jgrf <= igrf; jgrf++)
        {
          iad = vario->getAddressForGg(idir, igrf, jgrf, ilag, 1);
          vario->setGgByIndex(
            idir, iad, st_param_expand(local_pgs, igrf, jgrf, 1));
   ...
   **
   *****************************************************************************/
  static Id st_variogram_geometry_pgs_calcul(
    Local_Pgs* local_pgs,
    Vario* vario,
    Id idir)
  {
    ...
    for (Id iiech = 0; iiech < nech - 1; iiech++)
    {
      ...
      for (Id jjech = iiech + 1; jjech < nech; jjech++)
      {
        ...
        if (ilag < 0) ilag = -ilag;
        if (vario_order_add(
              local_pgs->vorder, iech, jech, NULL, NULL, ilag, idir, dist))
          return 1;
        dist = ABS(dist);
        ...
        for (Id ivar = 0; ivar < nvar; ivar++)
          for (Id jvar = 0; jvar <= ivar; jvar++)
          {
            if (vario->getFlagAsym())
            {
              ...
              vario->setHhByIndex(
                idir, iad, vario->getHhByIndex(idir, iad) - dist);
              vario->setSwByIndex(
                idir, iad, vario->getSwByIndex(idir, iad) + 1);
              iad = vario->getAddressForGg(idir, ivar, jvar, ilag, -1);
              vario->setGgByIndex(idir, iad, 0.);
              vario->setHhByIndex(
                idir, iad, vario->getHhByIndex(idir, iad) + dist);
  ...
  static double st_varcalc_correlated_grf(Local_Pgs* local_pgs, Id idir)
  {
    ...
    for (Id ilag = 0; ilag < vario->getNLag(idir); ilag++)
    {
      ...
      value +=
        (vario->getUtilizeByIndex(idir, vario->getNLag(idir) + ilag)
   ...
   **
   *****************************************************************************/
  static Id st_variopgs_calcul_norho(
    Vario* vario,
    const Rule* rule,
    Local_Pgs* local_pgs,
    Id ngrf,
  ...
  {
    double sum = 0;
    auto* local_pgs = static_cast<Local_Pgs*>(user_data);
   ...
   **
   *****************************************************************************/
  static Id st_variopgs_calcul_rho(
    Vario* vario,
    const Rule* rule,
    Local_Pgs* local_pgs,
    Id ngrf,
    Id opt_correl)
  {
    ...
    st_set_rho(
      golden_search(
        st_rho_search, static_cast<void*>(local_pgs), GS_TOLSTOP_RHO, -1., 1.,
        &testval, &niter),
      local_pgs);
   ...
   **
   *****************************************************************************/
  static Id st_variogram_pgs_nostat(
    Db* db,
    const Db* dbprop,
    Vario* vario,
    const Rule* rule,
    ...
    Id opt_correl)
  {
    ...
    {
      st_set_rho(rule->getRho(), &local_pgs);
      if (st_variopgs_calcul_norho(
            vario, rule, &local_pgs, ngrf, opt_correl, 1))
        goto label_end;
    }
    else
    {
   ...
   **
   *****************************************************************************/
  static void st_calcul_covmatrix(
    Local_Pgs* local_pgs,
    Id* flag_ind,
    Id* iconf,
    double* cov)
    ...
    double* cov)
  {
    ...
    {
      proba =
        st_get_proba_ind(cov[0], low, up, iconf[0]); // TODO: to be checked
    }
    else
    {
      if (flag_ind)
      ...
      else
      {
        ...
        if (!TEST_DISCRET)
        {
          ...
          mvndst(
            4, low, up, infin, cov, maxpts, abseps, releps, &err, &proba, &ier);
        }
        else
        {
          my_throw(
    ...
    double* ploc)
  {
    ...
    else
    {
      ...
      if (!TEST_DISCRET)
      {
        ...
        {
          low[2] =
            local_pgs->db->getLocVariable(ELoc::L, iech1, nfacies + ifac1);
          up[2] =
            local_pgs->db->getLocVariable(ELoc::U, iech1, nfacies + ifac1);
          low[3] =
            local_pgs->db->getLocVariable(ELoc::L, iech2, nfacies + ifac2);
          up[3] =
            local_pgs->db->getLocVariable(ELoc::U, iech2, nfacies + ifac2);
        }
      }
      else
      {
        ...
        {
          low[2] =
            local_pgs->db->getLocVariable(ELoc::RKLOW, iech1, nfacies + ifac1);
          up[2] =
            local_pgs->db->getLocVariable(ELoc::RKUP, iech1, nfacies + ifac1);
          low[3] =
            local_pgs->db->getLocVariable(ELoc::RKLOW, iech2, nfacies + ifac2);
          up[3] =
            local_pgs->db->getLocVariable(ELoc::RKUP, iech2, nfacies + ifac2);
        }
      }
    }
  }
      ...
      for (Id ilag = 0; ilag < vario->getNLag(idir); ilag++)
      {
        ...
        for (Id ipair = local_pgs->ifirst; ipair < local_pgs->ilast; ipair++)
        {
          vario_order_get_indices(
            local_pgs->vorder, ipair, &iech, &jech, &dist);

          /* Calculate the distance vector */

          dist =
            distance_intra(local_pgs->db, iech, jech, local_pgs->d1.data());
          ...
          for (Id ifac = 0; ifac < nfacies; ifac++)
            for (Id jfac = 0; jfac <= ifac; jfac++)
            {
              if (local_pgs->vario->getFlagAsym())
              {
                ...
                vario->setGgByIndex(
                  idir, i,
                  vario->getGgByIndex(idir, i)
                    + st_get_value(
                      local_pgs, flag_ind, iech, jech, ifac, jfac, iconf, cov));
                i = vario->getAddressForGg(idir, ifac, jfac, ilag, -1);
                vario->setGgByIndex(
                  idir, i,
                  vario->getGgByIndex(idir, i)
                    + st_get_value(
                      local_pgs, flag_ind, jech, iech, ifac, jfac, iconf, cov));
              }
              else
              {
    ...
    bool flagGg)
  {
    ...
    for (Id idir = 0; idir < vario1->getNDir(); idir++)
    {
      ...
      {
        messerr(
          "Both variograms should share the same number of flags for Direction "
          "#%d",
          idir + 1);
        return 1;
      }
    ...
    for (Id idir = 0; idir < vario->getNDir(); idir++)
    {
      ...
      for (Id ilag = 0; ilag < vario->getNLag(idir); ilag++)
      {
        ...
        for (Id i = 0; i < vario->getNDim(); i++)
        {
          ...
          local_pgs->d1[i] =
            vario->getHhByIndex(idir, jpas) * vario->getCodir(idir, i);
        }
        st_calcul_covmatrix(local_pgs, &flag_ind, iconf, cov);
          ...
          for (Id jfac = 0; jfac <= ifac; jfac++)
          {
            if (local_pgs->vario->getFlagAsym())
            {
              ...
              vario->setGgByIndex(
                idir, iad,
                st_get_value(
                  local_pgs, flag_ind, 0, 0, ifac, jfac, iconf, cov));
              iad = vario->getAddressForGg(idir, ifac, jfac, ilag, -1);
              vario->setGgByIndex(
                idir, iad,
                st_get_value(
                  local_pgs, flag_ind, 0, 0, jfac, ifac, iconf, cov));
            }
            else
            {
    ...
    const Model* model2)
  {
    ...
    if (!flag_stat)
    {
      ...
      if (dbprop != nullptr && dbprop->getNDim() != varioparam->getNDim())
      {
        messerr("Space Dimension inconsistency between Dbprop and Vario");
        return nullptr;
      }
      if (static_cast<Id>(new_model->getNDim()) != db->getNDim())
      {
   ...
   **
   *****************************************************************************/
  static Id st_variogram_pgs_stat(
    Db* db,
    Vario* vario,
    Vario* varioind,
    const Rule* rule,
    ...
    Id opt_correl)
  {
    ...
    if (flag_stat)
    {
      if (!propcst.empty())
      {
        if (static_cast<Id>(propcst.size()) != nclass)
        {
          messerr(
            "Number of proportions in 'propcst' (%d) should match Number of "
            "Facies in 'rule' (%d)",
            static_cast<Id>(propcst.size()), rule->getNFacies());
          return nullptr;
        }
        props = propcst;
      }
      else
      {
        ...
        if (static_cast<Id>(props.size()) != nclass)
        {
          messerr(
            "Number of Facies in 'db' (%d) should match Number of facies in "
            "'rule' (%d)",
            static_cast<Id>(props.size()), rule->getNFacies());
          return nullptr;
        }
      }
    ...
    if (flag_stat)
      ...
      error = st_variogram_pgs_nostat(
        db, dbprop, vario, rule, props, flag_rho, opt_correl);
    ...
    Id verbose)
  {
    ...
    for (Id i = 0; i < NCOLOR; i++) facies[i] = i + 1;
    ...
    scores = st_relem_evaluate(
      Pile_Relem, verbose, fgrf, fcmp, &local_pgs, &nscore, &r_opt);
    ...
    if (verbose)
    {
      ...
      st_rule_print(
        r_opt, NRULE, Pile_Relem->Rrules, Pile_Relem->Rfipos, false, -1, -1,
        TEST);
    }
    rule = st_rule_encode(&Pile_Relem->Rrules[r_opt * NRULE]);
