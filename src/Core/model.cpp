...
namespace gstlrn
{
   ...
   **
   *****************************************************************************/
  void model_cova_characteristics(
    const ECov& type,
    ...
    double* parmax)
  {
    ...
    (void)gslStrcpy(
      static_cast<char*>(cov_name), STRING_LENGTH, cov->getCovName().c_str());
  ...
  Model* model_combine(const Model* model1, const Model* model2, double r)
  {
    ...
    for (Id i = 0; i < model1->getNCov(); i++)
    {
      ...
      model->addCovFromParam(
        cova->getType(), cova->getRangeIso(), 0., cova->getParam(),
    ...
    const CovCalcMode* mode)
  {
    ...
    while ((residual > tol) && (npivot < npivot_max))
    {
      ...
      {
        for (i = npivot + 1; i < nech; i++)
          for (j = 0; j < npivot; j++) G(npivot, i) -= G(j, i) * G(j, npivot);
      }
      for (i = npivot + 1; i < nech; i++) G(npivot, i) /= G(npivot, npivot);
      ...
      for (i = npivot + 1; i < nech; i++)
      {
        ...
        for (j = 0; j <= npivot; j++) b -= G(j, i) * G(j, i);
        diag[i] = b;
      }

      // Save the new residual element
      residual = 0.;
      for (i = npivot + 1; i < nech; i++) residual += diag[i];
    ...
    *npivot_arg = npivot;
    ...
    for (i = 0; i < npivot; i++) crit[i] /= static_cast<double>(nech);
