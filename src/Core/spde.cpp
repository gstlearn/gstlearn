...
namespace gstlrn
{
  ...
  static Id qchol_cholesky(Id verbose, QChol* QC)
  {
    ...
    if (qchol_getNRows(QC) != qchol_getNCols(QC))
    {
      ...
      messerr("which is not square: %d x %d",
              qchol_getNRows(QC),
              qchol_getNCols(QC));
   ...
   **
   *****************************************************************************/
  static void cs_chol_invert(QChol* qctt,
                             double* xcr,
                             const double* rhs,
                             const double* work)
  {
  ...
  static Id st_check_model(const Db* dbin, const Db* dbout, Model* model)
  {
    ...
    if (dbin != nullptr)
    {
      ...
      else
      {
        ...
        {
          messerr("Model (%d) and Input Db (%d) must refer to the same number "
                  "of variables",
                  nvar,
                  dbin->getNLoc(ELoc::Z));
          return (1);
...

  {
    ...
    if (st_is_model_nugget())
    {
      ...
      for (Id ivar = 0; ivar < nvar; ivar++)
      {
        ...
        QCov[ivar]->Q =
          MatrixSparse::addMatMat(Matelem.QC->Q,
                                  Bi,
                                  st_get_isill(icov0, ivar, ivar));
                          ...
                          bool flag_modif)
  {
    ...
    S_DECIDE.flag_Qchol = (S_DECIDE.flag_case == CASE_SIMULATE
                           && S_DECIDE.flag_Q && S_DECIDE.simu_chol);
    ...
    {
      messerr("'flag_gibbs' requires simulation ('flag_est' and 'flag_std' "
              "must be FALSE)");
      return (1);
    }
    if (S_DECIDE.flag_case != CASE_SIMULATE && flag_modif)
    {
      messerr("'flag_modif' is limited to simulations ('flag_est' and "
              "'flag_std' must be FALSE)");
      return (1);
    }
  ...
  static Id st_m2d_check_pinchout(Db* dbgrid, Id icol_pinch)
  {
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      if (tab[iech] < 0 || tab[iech] > 1)
      {
        ...
        messerr("At grid node %d/%d, the value is %lf",
                iech + 1,
                nech,
                tab[iech]);
        return 1;
                                 ...
                                 double S)
  {
    ...
    if (flag_verbose)
    {
      messerr("Error at Sample #%d/%d for Layer #%d",
              iech + 1,
              db->getNSample(),
              ilayer + 1);
      if (FFFF(M))
    ...
    st_m2d_stats_init(M2D_Environ* m2denv, Db* dbin, Id nlayer, Id verbose)
  {
    ...
    {
      mestitle(2,
               "Global Statistics on Raw Elevations (extended by %4.2lf)",
               percent);
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      if (flag_jter)
      {
        ...
        for (Id ilayer = 0; ilayer < nlayer; ilayer++)
        {
          zmin = dbc->getLocVariable(ELoc::L, iech, ilayer);
          zmax = dbc->getLocVariable(ELoc::U, iech, ilayer);
          st_print_constraints_per_point(ilayer,
                                         iech,
                                         work[ilayer],
                                         TEST,
                                         TEST,
                                       ...
                                       Id* number_hard)
  {
    ...
    *number_hard = number;
    ...
    db = Db::createFromSamples(number,
                               ELoadBy::SAMPLE,
                               tab,
                               VectorString(),
                               VectorString(),
                            ...
                            VectorDouble& work)
  {
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      for (Id ilayer = 0; ilayer < nlayer; ilayer++)
      {
        ...
        work[ilayer] = st_m2d_draw_gaussian(m2denv,
                                            dbc,
                                            verbose,
                                            iter,
                                            ilayer,
                    ...
                    Id verbose)
  {
    ...
    if (st_m2d_drift_manage(m2denv, dbin, dbout, nlayer, verbose, &iatt_f))
      goto label_end;
    st_print_db_constraints("List of Initial Constraining Data",
                            dbin,
                            VectorDouble(),
                            nlayer,
                            verbose);
    ...
    dbc = st_m2d_create_constraints(m2denv,
                                    dbin,
                                    dbout,
                                    ndim,
                                    nlayer,
    ...
    if (st_spde_prepar(dbc, dbout, VectorDouble(), s_option)) goto label_end;
    {
      ...
      st_print_db_constraints("List of Constraining Data at Wells",
                              dbc,
                              ydat,
                              nlayer,
                              verbose);
      ...
      for (Id isimu = 0; isimu < nbsimu; isimu++)
      {
        ...
        for (Id iter = 0; iter < niter; iter++)
        {
          ...
          for (Id ilayer = 0; ilayer < nlayer; ilayer++)
          {
            ...
            AMatrix::productInPlace(ymean_loc,
                                    yvert_loc,
                                    *Matelem.Aproj,
                                    false);
          }
        ...
        if (st_check_gibbs_data("Checking Constraints at Wells",
                                m2denv,
                                dbc,
                                nlayer,
                                verbose,
      ...
      for (Id isimu = 0; isimu < nbsimu; isimu++)
      {
        for (Id ilayer = 0; ilayer < nlayer; ilayer++)
        {
          (void)gslSPrintfCat(string_encode,
                              "Layer-%d_Simu-%d",
                              ilayer + 1,
                              isimu + 1);
