...
namespace gstlrn
{
  typedef struct
  {
    ...
    std::vector<MatrixSparse*>
      Bnugget; /* Sparse matrices for nugget effect (nvs2) */
    std::vector<MatrixSparse*>
      BheteroD; /* Sparse matrices for heterotopy (nvar)*/
    std::vector<MatrixSparse*>
      BheteroT; /* Sparse matrices for heterotopy (nvar)*/
  } SPDE_SS_Environ;
  ...
  static Id qchol_cholesky(Id verbose, QChol* QC)
  {
    ...
    if (qchol_getNRows(QC) != qchol_getNCols(QC))
    {
      ...
      messerr(
        "which is not square: %d x %d", qchol_getNRows(QC), qchol_getNCols(QC));
  ...
  static void st_title(Id flag_igrf, Id flag_icov, Id rank, const char* title)
  {
    ...
    if (flag_decor)
    {
      ...
      if (flag_igrf)
        (void)gslSPrintf(
          string_encode, "%s GRF:%d", string_encode.data(),
          SPDE_CURRENT_IGRF + 1);
      if (flag_icov)
        (void)gslSPrintf(
          string_encode, "%s - COV:%d", string_encode.data(),
          SPDE_CURRENT_ICOV + 1);
      (void)gslSPrintf(string_encode, "%s ) %s", string_encode.data(), title);
    }
    else
    {
   ...
   *****************************************************************************/
  static MatrixSparse*
    st_extract_Q_from_Q(MatrixSparse* Q_in, Id row_auth, Id col_auth)
  {
   ...
   **
   *****************************************************************************/
  static QChol* st_extract_QC_from_Q(
    const char* title,
    QChol* QC_in,
    Id row_auth,
    Id col_auth)
  ...
  static QSimu* st_qsimu_manage(Id mode, QSimu* qsimu)
  {
    ...
    switch (mode)
    {
      case 1: /* Allocation */
        ...
        {
          qsimu->QCtt = st_extract_QC_from_Q(
            "f_f", spde_get_current_matelem(-1).QC, VT_FREE, VT_FREE);
          if (qsimu->QCtt == nullptr) goto label_end;
          if (S_DECIDE.flag_mesh_dbin)
          {
            qsimu->QCtd = st_extract_QC_from_Q(
              "f_gd", spde_get_current_matelem(-1).QC, VT_FREE,
              VT_GIBBS | VT_HARD);
  ...
  static void st_print_all(const char* title)
  {
    ...
    printMatrix(
      Calcul.blin, 1, nblin, "Coefficients of the Linear Combination", 0, 1);
  }
  ...
  static void st_compute_blin(void)
  {
    ...
    {
      for (Id i = 0; i <= p; i++) Calcul.blin[i] = ut_cnp(p, i) * correc;
    }
  ...
  static Id st_check_model(const Db* dbin, const Db* dbout, Model* model)
  {
    ...
    if (dbin != nullptr)
    {
      ...
      {
        messerr(
          "Model (%d) and Input Db (%d) must have the same space dimension",
          ndim, dbin->getNDim());
        return (1);
      }
      ...
      {
        if (dbin->getNLoc(ELoc::Z) != nvar
            && S_DECIDE.flag_case != CASE_MATRICES && !S_DECIDE.flag_gibbs)
        {
          messerr(
            "Model (%d) and Input Db (%d) must refer to the same number of "
            "variables",
    ...
    if (dbout != nullptr)
    {
      ...
      {
        messerr(
          "Model(%d) and output Db(%d) must have same space dimension", ndim,
          dbout->getNDim());
        return (1);
      }
    }
    ...
    if (S_DECIDE.flag_mesh_dbin && !flag_nugget)
    {
      ...
      for (Id ivar = 0; ivar < nvar; ivar++) sill.setValue(ivar, ivar, nugval);
   ...
   **
   *****************************************************************************/
  static Id st_identify_nostat_param(
   ...
   *****************************************************************************/
  static Id
    st_kriging_cholesky(QChol* QC, double* rhs, VectorDouble& work, double* z)
  {
    Id ntarget;
    ...
    for (Id icur = 0; icur < ntarget; icur++) work[icur] = 0.;
...

  {
    ...
    if (isDefaultSpaceSphere())
    {
      ...
      {
        units[imesh] = GH::geodeticTriangleSurface(
          amesh->getCoor(imesh, 0, 0), amesh->getCoor(imesh, 0, 1),
          amesh->getCoor(imesh, 1, 0), amesh->getCoor(imesh, 1, 1),
          amesh->getCoor(imesh, 2, 0), amesh->getCoor(imesh, 2, 1));
      }
    }
    else
    {
      ...
      for (Id imesh = 0; imesh < nmesh; imesh++)
      {
        for (Id icorn = 1; icorn < ncorner; icorn++)
          for (Id idim = 0; idim < ndim; idim++)
            mat.setValue(
              idim, icorn - 1,
              amesh->getCoor(imesh, icorn, idim)
  ...
  static VectorInt st_get_vertex_ranks(AMesh* amesh, Db* dbin, Db* dbout)
  {
    ...
    if (nvertex < (n_in + n_out))
      messageAbort(
        "Nvertex(%d) must be larger than n_in(%d) + n_out(%d)", nvertex, n_in,
        n_out);
...

  {
    ...
    for (Id ivar = 0; ivar < nvar; ivar++) BheteroD[ivar] = nullptr;
    BheteroT.resize(nvar);
    for (Id ivar = 0; ivar < nvar; ivar++) BheteroT[ivar] = nullptr;
   ...
   **
   *****************************************************************************/
  static void st_triangle_center(
    AMesh* amesh,
    Id ncorner,
    Id imesh,
    double center[3],
    double xyz[3][3])
  {
    ...
    ratio = 1. / sqrt(ratio);
    for (Id i = 0; i < 3; i++) center[i] *= ratio;
  }
    ...
    double coeff[2])
  {
    ...
    {
      v[i] = 0.;
      for (Id j = 0; j < 2; j++) v[i] += coeff[j] * (axes[j][i] - center[i]);
    }
   ...
   **
   *****************************************************************************/
  static void st_tangent_calculate(
    double center[3],
    const double srot[2],
    double axes[2][3])
  {
   ...
   *****************************************************************************/
  static MatrixSparse*
    st_spde_fill_S(AMesh* amesh, Model* model, const double* units)
  {
    ...
    for (Id imesh = 0; imesh < amesh->getNMeshes(); imesh++)
    {
      ...
      if (flag_sphere)
      {
        ...
        {
          messerr(
            "Error in st_triangle_center: wrong number or corners: %d",
            ncorner);
          goto label_end;
        }
      ...
      else
      {
        ...
        if (flag_nostat) AMatrix::productInPlace(matv, matw, Calcul.vv, true);
   ...
   *****************************************************************************/
  static VectorDouble
    st_spde_fill_Lambda(Model* model, AMesh* amesh, const VectorDouble& TildeC)
  {
    DECLARE_UNUSED(model);
    VectorDouble Lambda;
    Id nvertex = amesh->getNApices();
   ...
   *****************************************************************************/
  static MatrixSparse*
    st_extract_Q1_nugget(Id row_var, Id col_var, Id* nrows, Id* ncols)
  {
    SPDE_SS_Environ* SS;
    MatrixSparse* B0;
   ...
   **
   *****************************************************************************/
  static MatrixSparse* st_extract_Q1_hetero(
    Id row_var,
    Id col_var,
    Id row_oper,
    Id col_oper,
    ...
    Id* ncols)
  {
    ...
    Q =
      MatrixSparse::addMatMat(&Qn, &Qn, st_get_isill(0, row_var, col_var), 0.);
...

  {
    ...
    if (st_is_model_nugget())
    {
      ...
      for (Id ivar = 0; ivar < nvar; ivar++)
      {
        ...
        QCov[ivar]->Q = MatrixSparse::addMatMat(
          Matelem.QC->Q, Bi, st_get_isill(icov0, ivar, ivar));
    ...
    else
    {
      ...
      for (Id ivar = 0; ivar < nvar; ivar++)
      {
        if (icov0 == 0)
        {
          // Q1_tt_ii
          QCov[ivar]->Q =
            st_extract_Q1_hetero(ivar, ivar, 2, 2, &nrows, &ncols);
          if (QCov[ivar]->Q == nullptr) goto label_end;
        }
        else
        {
   ...
   **
   *****************************************************************************/
  static MatrixSparse* st_spde_build_Q(
    MatrixSparse* S,
...

  {
    ...
    Id nblin = static_cast<Id>(Calcul.blin.size());
    Matelem.QC->Q =
      st_spde_build_Q(Matelem.S, Matelem.Lambda, nblin, Calcul.blin.data());
  ...
  static Id st_spde_build_matrices(Model* model, Id verbose)
  {
    ...
    if (VERBOSE)
      message("Filling TildeC Sparse Matrix performed successfully\n");

    /* Construct the matrix for the sill correction array */

    Matelem.Lambda = st_spde_fill_Lambda(model, amesh, tildec);
    if (VERBOSE)
      message("Filling Lambda Sparse Matrix performed successfully\n");
   ...
   *****************************************************************************/
  static double
    st_chebychev_function(double x, double power, const VectorDouble& blin)
  {
    double value, total;
   ...
   **
   *****************************************************************************/
  static Id st_chebychev_calculate_coeffs(
    Cheb_Elem* cheb_elem,
    Id verbose,
...

  {
    ...
    if (ut_chebychev_coeffs(st_chebychev_function, cheb_elem, blin))
      goto label_end;
    ...
    for (Id idisc = 1; idisc < ndisc; idisc++)
    {
      ...
      numloc =
        ut_chebychev_count(st_chebychev_function, cheb_elem, value, blin);
   ...
   *****************************************************************************/
  static Id
    st_simulate_cholesky(QChol* QC, VectorDouble& work, VectorDouble& zsnc)
  {
    Id nvertex;
    ...
    for (Id ip = 0; ip < nvertex; ip++) work[ip] = law_gaussian();
   ...
   **
   *****************************************************************************/
  static Cheb_Elem* st_spde_cheb_manage(
    Id mode,
    Id verbose,
    double power,
    const VectorDouble& blin,
    ...
    Cheb_Elem* cheb_old)
  {
    ...
    if (mode > 0)
    {
      ...
      ncmax =
        static_cast<Id>(get_keypone("Number_Polynomials_Chebychev", 10001.));
      ndisc =
        static_cast<Id>(get_keypone("Number_Discretization_Chebychev", 100.));
  ...
  label_end:
    if (error)
      cheb_elem =
        st_spde_cheb_manage(-1, 0, 0, VectorDouble(), NULL, cheb_elem);
    return (cheb_elem);
  }
#endif
  /****************************************************************************/
    ...
    switch (mode)
    {
      ...
      case -1: // Deallocation
        for (Id icov = 0; icov < ncova; icov++)
        {
          ...
          Matelem.s_cheb =
            st_spde_cheb_manage(-1, 0, 0, VectorDouble(), NULL, Matelem.s_cheb);
    ...
    SPDE_Option& s_option)
  {
    ...
    if (!flag_force)
    {
      if ((!S_DECIDE.flag_dbin || !S_DECIDE.flag_mesh_dbin) && dbout != nullptr
          && dbout->isGrid())
        dbloc = dbout;
      if ((!S_DECIDE.flag_dbout || !S_DECIDE.flag_mesh_dbout) && dbin != nullptr
          && dbin->isGrid())
        dbloc = dbin;
    }
  ...
  {
    return (S_EXTERNAL_Q[icov0] != nullptr && S_EXTERNAL_A[icov0] != nullptr);
  }
    ...
    SPDE_Option& s_option)
  {
    ...
    for (Id igrf = 0; igrf < st_get_number_grf(); igrf++)
    {
      ...
      for (Id icov = 0; icov < st_get_ncova(); icov++)
      {
        ...
        if (S_DECIDE.flag_dbin)
        {
          if ((S_DECIDE.flag_several && !flag_AQ_defined)
              || !S_DECIDE.flag_mesh_dbin)
          {
            Matelem.Aproj = dynamic_cast<MatrixSparse*>(
              Matelem.amesh->createProjMatrix(dbin, -1, false));
        ...
        {
          Matelem.s_cheb =
            st_spde_cheb_manage(1, VERBOSE, -0.5, Calcul.blin, Matelem.S, NULL);
  ...
  static void st_environ_print(const Db* dbout, const VectorDouble& gext)
  {
    if (S_DECIDE.flag_case == CASE_KRIGING)
    {
      ...
      if (st_get_filnug())
        message(
          "Filtering Nugget effect (Sill=%lg)\n", st_get_nugget_sill(0, 0));
    }
    else
    {
      if (S_DECIDE.flag_dbin)
    ...
    bool flag_modif)
  {
    ...
    S_DECIDE.flag_Qchol =
      (S_DECIDE.flag_case == CASE_SIMULATE && S_DECIDE.flag_Q
       && S_DECIDE.simu_chol);
    ...
    S_DECIDE.flag_onechol =
      static_cast<Id>(get_keypone("Flag_OneChol", S_DECIDE.flag_onechol));
    ...
    {
      messerr(
        "'flag_gibbs' requires simulation ('flag_est' and 'flag_std' must be "
        "FALSE)");
      return (1);
    }
    ...
    {
      messerr(
        "'flag_modif' is limited to simulations ('flag_est' and 'flag_std' "
        "must be FALSE)");
      return (1);
    }
    ...
    if (verbose)
    {
      ...
      message(
        "Perform estimation       = %d\n", S_DECIDE.flag_case == CASE_KRIGING);
      message(
        "Perform simulations      = %d\n", S_DECIDE.flag_case == CASE_SIMULATE);
  ...
  static Id st_m2d_check_pinchout(Db* dbgrid, Id icol_pinch)
  {
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      if (tab[iech] < 0 || tab[iech] > 1)
      {
        ...
        messerr(
          "At grid node %d/%d, the value is %lf", iech + 1, nech, tab[iech]);
        return 1;
      }
    }
    return 0;
   ...
   **
   *****************************************************************************/
  static double st_m2d_draw_elevation(
    M2D_Environ* m2denv,
    Id /*nlayer*/,
    Id /*ilayer*/,
    double lower,
   ...
   **
   *****************************************************************************/
  static void st_print_concatenate_interval(
    const char* title,
    double lower,
    double upper,
    Id tail)
   ...
   **
   *****************************************************************************/
  static void st_print_constraints_per_point(
    Id ilayer,
    Id iech,
    double value,
    double drift,
   ...
   **
   *****************************************************************************/
  static Id st_check_validity_MS(
    Db* db,
    Id ilayer,
    Id iech,
    Id flag_positive,
    ...
    double S)
  {
    ...
    {
      messerr(
        "Error at Sample #%d/%d for Layer #%d", iech + 1, db->getNSample(),
        ilayer + 1);
      if (FFFF(M))
   ...
   *****************************************************************************/
  static double
    st_m2d_get_M(M2D_Environ* m2denv, Db* db, Id type, Id ilayer, Id iech)
  {
    double value;
    Id iatt;
   ...
   **
   *****************************************************************************/
  static double st_m2d_external_drift_increment(
    M2D_Environ* m2denv,
    Db* db,
    Id ilayer0,
    Id iech0)
   ...
   *****************************************************************************/
  static double
    st_m2d_get_drift(M2D_Environ* m2denv, Db* db, Id ilayer0, Id iech0)
  {
   ...
   *****************************************************************************/
  static void
    st_m2d_set_M(M2D_Environ* m2denv, Id nlayer, Id icol_pinch, Db* db, Id iatt)
  {
    double drift;
   ...
   **
   *****************************************************************************/
  static Id st_m2d_drift_inc_manage(
    M2D_Environ* m2denv,
    Id mode,
    Id nlayer,
    Id icol_pinch,
   ...
   *****************************************************************************/
  static void
    st_m2d_stats_init(M2D_Environ* m2denv, Db* dbin, Id nlayer, Id verbose)
  {
    Id nech;
    ...
    {
      mestitle(
        2, "Global Statistics on Raw Elevations (extended by %4.2lf)", percent);
   ...
   *****************************************************************************/
  static void
    st_m2d_stats_updt(M2D_Environ* m2denv, Db* dbc, Id nlayer, Id verbose)
  {
    Id nech;
   ...
   **
   *****************************************************************************/
  static Id st_m2d_initial_elevations(
    M2D_Environ* m2denv,
    Db* dbc,
    Id nlayer,
    VectorDouble& work)
  {
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      for (Id ilayer = 0; ilayer < nlayer; ilayer++)
      {
        ...
        work[ilayer] =
          st_m2d_draw_elevation(m2denv, nlayer, ilayer, zmin, zmax);
      }
      ...
      {
        messerr(
          "At constraining sample #%d/%d, correct interval ordering", iech + 1,
          nech);
        messerr(
          "has not been reached after %d iterations. Run is aborted",
          njter_max);
        for (Id ilayer = 0; ilayer < nlayer; ilayer++)
        {
          zmin = dbc->getLocVariable(ELoc::L, iech, ilayer);
          ...
          st_print_constraints_per_point(
            ilayer, iech, work[ilayer], TEST, TEST, zmin, zmax);
        }
        messerr("\n");
    ...
    for (Id ilayer = 0; ilayer < nlayer; ilayer++)
    {
      ...
      if (m2denv->flag_ed)
      {
        ...
        for (Id iech = 0; iech < dbout->getNSample(); iech++)
        {
          ...
          nb++;
          if (FFFF(m2denv->dmini) || value < m2denv->dmini)
            m2denv->dmini = value;
          if (FFFF(m2denv->dmaxi) || value > m2denv->dmaxi)
            m2denv->dmaxi = value;
        }
      }
   ...
   **
   *****************************************************************************/
  static Id st_m2d_drift_fitting(
    M2D_Environ* m2denv,
    Db* dbc,
    Id nlayer,
    Id number_hard,
    Id verbose)
  {
    ...
    for (Id ilayer = 0; ilayer < nlayer; ilayer++)
    {
      ...
      for (Id i = 0; i < nbfl; i++) b[i] = 0.;
      for (Id i = 0; i < nbfl * nbfl; i++) a[i] = 0.;
   ...
   *****************************************************************************/
  static void
    st_m2d_drift_save(M2D_Environ* m2denv, Db* dbout, Id nlayer, double* gwork)
  {
    double drift, value;
    Id ngrid;
   ...
   *****************************************************************************/
  static void
    st_define_locators(M2D_Environ* m2denv, Db* db, Id ndim, Id nvar, Id nlayer)
  {
    Id ivar;
   ...
   **
   *****************************************************************************/
  static Db* st_m2d_create_constraints(
    M2D_Environ* m2denv,
    Db* dbin,
    Db* dbout,
    Id ndim,
    ...
    Id* number_hard)
  {
    ...
    for (Id iech = 0; iech < dbin->getNSample(); iech++)
    {
      if (!dbin->isActive(iech)) continue;
      if (st_record_sample(
            m2denv, dbin, iech, ndim, natt, nlayer, 0, &number, tab.data()))
        goto label_end;
    }
    *number_hard = number;
    ...
    for (Id iech = 0; iech < dbout->getNSample(); iech++)
    {
      if (!dbout->isActive(iech)) continue;
      if (st_record_sample(
            m2denv, dbout, iech, ndim, natt, nlayer, 0, &number, tab.data()))
        goto label_end;
    }
    ...
    if (number <= 0)
    {
      for (Id iech = 0; iech < dbin->getNSample(); iech++)
      {
        if (!dbin->isActive(iech)) continue;
        if (st_record_sample(
              m2denv, dbin, iech, ndim, natt, nlayer, 1, &number, tab.data()))
          goto label_end;
        if (number > 0) break;
      }
    }
    ...
    db = Db::createFromSamples(
      number, ELoadBy::SAMPLE, tab, VectorString(), VectorString(), 0);
  ...
  static QChol* st_derive_Qc(double s2, QChol* Qc, SPDE_Matelem& Matelem)
  {
    ...
    message(
      "Building Q (Size:%d) with additional nugget effect (%lf) ... ",
      Q->getNCols(), s2);
    Bt = B->transpose();
    if (Bt == nullptr) goto label_end;
   ...
   **
   *****************************************************************************/
  static double st_m2d_draw_gaussian(
    M2D_Environ* m2denv,
    Db* dbc,
    Id verbose,
    Id iter,
    ...
    VectorDouble& work)
  {
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      for (Id ilayer = 0; ilayer < nlayer; ilayer++)
      {
        ...
        work[ilayer] = st_m2d_draw_gaussian(
          m2denv, dbc, verbose, iter, ilayer, iech, work[ilayer], zcum, zmin,
          zmax, YMEAN(ilayer, iech), sigma);
      }
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      for (Id ilayer = 0; ilayer < nlayer; ilayer++)
      {
        ...
        if (!FFFF(zmin))
        {
          ...
          {
            messerr(
              "%s: Sample(%d/%d) of Layer(%d/%d): Depth(%lf) < Lower(%lf)",
        ...
        if (!FFFF(zmax))
        {
          ...
          {
            messerr(
              "%s: Sample(%d/%d) of Layer(%d/%d): Depth(%lf) > Upper(%lf)",
   ...
   *****************************************************************************/
  static M2D_Environ*
    st_m2denv_manage(Id mode, Id flag_ed, double ystdv, M2D_Environ* m2denv_old)
  {
    M2D_Environ* m2denv;
   ...
   **
   *****************************************************************************/
  static void st_m2d_vector_extract(
    M2D_Environ* m2denv,
    Db* dbc,
    Id nlayer,
    VectorDouble& ydat,
   ...
   **
   *****************************************************************************/
  static void st_print_db_constraints(
    const char* title,
    Db* db,
    ...
    Id verbose)
  {
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      for (Id ilayer = 0; ilayer < nlayer; ilayer++)
      {
        ...
        st_print_constraints_per_point(
          ilayer, iech, value, drift, vgaus, lower, upper);
      }
    }
  }
   ...
   *****************************************************************************/
  static void
    st_m2d_stats_gaus(const char* title, Id nlayer, Id nech, double* ydat)
  {
    if (!DEBUG) return;
    for (Id ilayer = 0; ilayer < nlayer; ilayer++)
    {
      (void)gslSPrintf(string_encode, "%s (Layer #%d)", title, ilayer + 1);
      ut_stats_mima_print(string_encode.data(), nech, &YDAT(ilayer, 0), NULL);
    }
  }
    ...
    Id verbose)
  {
    ...
    if (dbin->getNInterval() < nlayer)
    {
      ...
      messerr(
        "to be defined in the Input Db for each layer (nint=%d)",
        dbin->getNInterval());
      goto label_end;
    }
    if (!dbout->isGrid())
    ...
    if (flag_ed && nlayer > dbout->getNLoc(ELoc::F))
    {
      ...
      messerr(
        "- Count of F-variables (%d) must match Count of layers (%d)",
    ...
    st_print_db_constraints(
      "List of Initial Constraining Data", dbin, VectorDouble(), nlayer,
      verbose);
    ...
    dbc = st_m2d_create_constraints(
      m2denv, dbin, dbout, ndim, nlayer, &number_hard);
    ...
    if (st_spde_check(
          dbc, dbout, model, NULL, 0, VectorDouble(), false, true, true, false,
          false, false, false))
      goto label_end;
    ...
    if (st_spde_prepar(dbc, dbout, VectorDouble(), s_option)) goto label_end;
    {
      ...
      st_print_db_constraints(
        "List of Initial Constraining Data", dbc, VectorDouble(), nlayer,
        verbose);
      st_m2d_vector_extract(m2denv, dbc, nlayer, ydat, lwork);
      st_print_db_constraints(
        "List of Constraining Data at Wells", dbc, ydat, nlayer, verbose);
      ...
      for (Id isimu = 0; isimu < nbsimu; isimu++)
      {
        ...
        for (Id iter = 0; iter < niter; iter++)
        {
          ...
          for (Id ilayer = 0; ilayer < nlayer; ilayer++)
          {
            ...
            for (Id i = 0; i < nvertex; i++) yvert_loc[i] *= ysigma;

            // Conditional simulation

            for (Id i = 0; i < nvertex; i++) zkrig[i] = vwork[i] = 0.;
            AMatrix::productInPlace(rhs, *Matelem.Aproj, ydat_loc, true);
            st_kriging_cholesky(Qc, rhs.data(), vwork, zkrig.data());
            for (Id i = 0; i < nvertex; i++) yvert_loc[i] += zkrig[i];

            // Project the Simulation from the vertices onto the Data

            AMatrix::productInPlace(
              ymean_loc, yvert_loc, *Matelem.Aproj, false);
          }
          ...
          if (st_global_gibbs(
                m2denv, dbc, 0, iter, nlayer, ysigma, ymean, ydat, lwork))
            goto label_end;
        ...
        if (st_check_gibbs_data(
              "Checking Constraints at Wells", m2denv, dbc, nlayer, verbose,
              ydat, lwork))
          goto label_end;
        ...
        Bproj = dynamic_cast<MatrixSparse*>(
          Matelem.amesh->createProjMatrix(dbout, -1, false));
      ...
      if (flag_ce || flag_cstd)
      {
        ...
        if (db_simulations_to_ce(
              dbout, ELoc::GAUSFAC, nbsimu, nlayer, &iptr_ce, &iptr_cstd))
          goto label_end;
