...
namespace gstlrn
{
                        ...
                        double* yout)
  {
    ...
    for (ixout = 0; ixout < nxout; ixout++)
    {
      ...
      ktable = static_cast<Id>((frac >= 0.0) ? frac * fntablem1 + 0.5
  ...
  *****************************************************************************/
                      ...
                      VectorDouble& wavelet)
  {
    ...
    if (type >= 0)
    {
      wavelet = st_seismic_wavelet(!flag_operate,
                                   type,
                                   ntw,
                                   tindex,
                                   dz,
      ...
      for (iatt = 0; iatt < natt; iatt++)
      {
        ...
        for (iz = 0; iz < nz; iz++)
          TR_OUT(db,
                 ...
                 itrace,
                 iz,
                 (FFFF(TR_IN(db, iatt_in, iatt, itrace, iz)))
                   ? TEST
                   : tab2[iz + shift]);
      }
    }
                         ...
                         Id flag_stat)
  {
    ...
    for (jx0 = 0; jx0 < NX; jx0++)
    {
      ...
      for (iz0 = 0; iz0 < NZ; iz0++)
      {
        ...
        if (st_estimate_neigh_create(db,
                                     0,
                                     iatt_z1,
                                     iatt_z2,
                                     ix0,
