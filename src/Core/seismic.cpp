                                  ...
                                  double distort)
{
  ...
  if (verbose)
    print_vector("Wavelet",0,ntw2,wavelet);
...
**
*****************************************************************************/
GSTLEARN_EXPORT int seismic_z2t_convert(Db *db_z,
                                    int iatt_v,
                                    Db *db_t)
{
  Db     *db_v;
  ...
  st_seismic_z2t_convert(db_z,iatt_z,nz,z0,z1,dz,
                         db_t,iatt_t,nt,t0,t1,dt,
                         db_v,iatt_v,natt,
                         tz,zt,at,az);
...
**
*****************************************************************************/
GSTLEARN_EXPORT int seismic_t2z_convert(Db *db_t,
                                    int iatt_v,
                                    Db *db_z)
{
  Db     *db_v;
  ...
  st_seismic_t2z_convert(db_t,iatt_t,nt,t0,t1,dt,
                         db_z,iatt_z,nz,z0,z1,dz,
                         db_v,iatt_v,natt,
                         tz,zt,at,az);
...
**
*****************************************************************************/
GSTLEARN_EXPORT int seismic_operate(Db  *db,
                                int  oper)
{
                                 ...
                                 double *wavelet)
{
  ...
  for (itrace=0; itrace<NTRACE; itrace++) {
    for (iatt=0; iatt<natt; iatt++) {
      ...
      for (iz=0; iz<nz; iz++) tab0[iz] = TR_IN(db,iatt_in,iatt,itrace,iz);
      ...
      st_seismic_affect(db,nz,shift,val_before,val_middle,val_after,tab0,tab1);

      /* Perform the convolution */

      st_seismic_convolve(2*ntw+1,-ntw,wavelet,size,0,tab1,size,0,tab2);
...
label_end:
  tab0    = (double *) mem_free((char *) tab0);
...
**
*****************************************************************************/
static int st_absolute_index(Db *db,
                             int ix,
                             int iz)
{
  int indg[3];
                                       ...
                                       ST_Seismic_Neigh *ngh_cur)
{
  ...
  for (i=0; i<ngh_old->nactive; i++)
  {
    if (ngh_cur->ix_ngh[i] != ngh_old->ix_ngh[i] ||
        ngh_cur->iz_ngh[i] != ngh_old->iz_ngh[i]) goto label_end;
  }
                            ...
                            double *lhs)
{
  ...
  for (i=0; i<neqmax * neqmax; i++) lhs[i] = 0.;
  ...
  if (debug_query("kriging"))
    krige_lhs_print(nech,neqmax,nred,flag,lhs);
                            ...
                            double *rhs)
{
  ...
  for (i=0; i<neqmax * NVAR; i++) rhs[i] = 0.;
                           ...
                           double *wgt)
{
  ...
  if (debug_query("kriging"))
    st_wgt_print(ngh,NVAR,nech,nred,flag,wgt);
...
**
*****************************************************************************/
static int st_estimate_sort(int *presence,
                            int *rank)
{
                                    ...
                                    int    flag_stat)
{
  ...
  for (jx0=0; jx0<NX; jx0++)
  {
    ...
    for (iz0=0; iz0<NZ; iz0++)
    {
      ...
      if (st_estimate_neigh_create(db,0,iatt_z1,iatt_z2,ix0,iz0,nbench,nv2max,
                                   npres,presence,ngh_cur)) continue;
      ...
      st_estimate_result(db,ngh_cur,flag_std,nfeq,nred,flag,wgt,rhs,var0,
                         iatt_est,iatt_std);
