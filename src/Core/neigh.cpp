 ...
 **
 *****************************************************************************/
static void st_neigh_print(Db *dbin, Neigh *neigh, int *rank)
{
  ...
  if (flag_ext) for (idim = 0; idim < ndim; idim++)
  {
  ...
  for (iech = sel = 0; iech < dbin->getSampleNumber(); iech++)
  {
    ...
    if (dbin->hasCode())
      tab_printi(NULL, 1, EJustify::RIGHT,
                 static_cast<int>(dbin->getCode(iech)));
    for (idim = 0; idim < ndim; idim++)
    ...
    if (flag_ext)
    {
      for (idim = 0; idim < ndim; idim++)
        tab_printg(NULL, 1, EJustify::RIGHT,
                   dbin->getBlockExtension(iech, idim));
    }
    if (neigh->getType() == ENeigh::MOVING)
 ...
 **
 *****************************************************************************/
static int st_discard_undefined(Db *dbin, int iech)
{
  int ivar;
 ...
 **
 *****************************************************************************/
static void st_bench(Db *dbin, Db *dbout, int iech_out, Neigh *neigh, int *rank)
{
  double z0;
  int iech, idim_bench;
                             ...
                             Neigh *neigh)
{
  ...
  for (idim = 0; idim < ndim; idim++)
    NBGH_x1[idim] = dbout->getCoordinate(iech_out, idim)
        - dbin->getCoordinate(iech_in, idim);
  ...
  if (neigh->getFlagAniso())
  {
    ...
    for (idim = 0; idim < ndim; idim++)
      NBGH_x1[idim] /= neigh->getAnisoCoeff(idim);
  }
 ...
 **
 *****************************************************************************/
static int st_moving_sector_define(Neigh *neigh, double dx, double dy)
{
  double angle;
  int isect;
 ...
 **
 *****************************************************************************/
static void st_moving_sector_nsmax(Neigh *neigh, int nsel, int *rank)
{
 ...
 **
 *****************************************************************************/
static void st_moving_select(Neigh *neigh, int nsel, int *rank)
{
 ...
 **
 *****************************************************************************/
static int st_moving(Db *dbin, Db *dbout, int iech_out, Neigh *neigh, int *rank)
{
                 ...
                 int *rank)
{
  ...
  for (iech = (*nsel); iech < dbin->getSampleNumber(); iech++)
    rank[iech] = -1;
