...
**
*****************************************************************************/
static double st_get_weight(int ipos,
                            int dir)
{
  double value;
...
**
*****************************************************************************/
static int st_skin_grid_shift(Skin *skin,
                              int   indg0[3],
                              int   dir,
                              int  *iad)
{
...
label_end:
  if (error) skin = skin_undefine(skin);
  return(skin);
}
...
**
*****************************************************************************/
static int st_skin_cell_del(Skin *skin,
                            int   rank)
{
...
**
*****************************************************************************/
static int st_skin_cell_already(Skin  *skin,
                                int    ipos)
{
  int i;
...
**
*****************************************************************************/
static void st_skin_cell_mod(Skin  *skin,
                             int    rank,
                             double energy)
{
...
**
*****************************************************************************/
static int st_skin_cell_add(Skin  *skin,
                            int    ipos,
                            double energy)
{
  int rank;
...

{
  ...
  if (debug_query("morpho"))
    message("Skin iteration:%5d - Length:%4d - Energy:%lf\n",
            skin->date,skin->nval,skin->total);
    ...
    if (total >= tirage) 
    {
      ...
      *ipos = skin->address[i];
      if (! FUNC_TO_BE_DONE(*ipos))
        messageAbort("Elligible cell (%d ipos=%d) of the skin is already filled",
                  i,*ipos);
      return;
    }
  }
