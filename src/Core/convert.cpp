 ...
 **
 *****************************************************************************/
static void st_print_verbose(int ndim, int nx[3], double dx[3], double x0[3])
{
                        ...
                        double *dx)
{
  ...
  if (st_grid_read_zycor_header(file, verbose, nx, x0, dx, &test))
    goto label_end;
  ...
  label_end: if (file != nullptr) fclose(file);
  return (error);
}
                        ...
                        double *tab)
{
  ...
  if (nx[0] != nx_r[0] || nx[1] != nx_r[1] || x0[0] != x0_r[0]
      || x0[1] != x0_r[1] || dx[0] != dx_r[0] || dx[1] != dx_r[1])
  {
  ...
  {
    messerr(
        "The number of decoded values (%d) is not equal to the expected number of grid nodes(%d)",
        lec, nech);
    goto label_end;
  }
 ...
 **
 *****************************************************************************/
static int st_compose(FILE *file, int nb)
                      ...
                      double *dx)
{
  ...
  if (st_grid_read_bmp_header(file, verbose, nx, x0, dx, ir, ig, ib, &nbits))
    goto label_end;
                      ...
                      double *tab)
{
  ...
  if (st_grid_read_bmp_header(file, 0, nx, x0, dx, ir, ig, ib, &nbits))
    goto label_end;
                                    ...
                                    double *test)
{
  ...
  if (st_ifpen_read(file, 0, "##########################", &dumint, &dumrel))
    goto label_end;
  if (st_ifpen_read(file, 0, "FILE_DESCRIPTION         #", &dumint, &dumrel))
    goto label_end;
  if (st_ifpen_read(file, 0, "APPLICATION              #", &dumint, &dumrel))
    goto label_end;
  if (st_ifpen_read(file, 0, "SURVEY_NAME              #", &dumint, &dumrel))
    goto label_end;
  if (st_ifpen_read(file, 0, "MATRIX_NAME              #", &dumint, &dumrel))
    goto label_end;
  if (st_ifpen_read(file, 0, "METHOD                   #", &dumint, &dumrel))
    goto label_end;
  if (st_ifpen_read(file, 2, "FLOAT_NULL_VALUE         #", &dumint, test))
    goto label_end;
  if (st_ifpen_read(file, 0, "ROW_COLUMN_ORIENTATION   #", &dumint, &dumrel))
    goto label_end;
  if (st_ifpen_read(file, 0, "REPRESENTATION_CODE      #", &dumint, &dumrel))
    goto label_end;
  if (st_ifpen_read(file, 0, "##########################", &dumint, &dumrel))
    goto label_end;
  if (st_ifpen_read(file, 2, "ANGLE                    #", &dumint, &anglez))
    goto label_end;
  if (st_ifpen_read(file, 1, "ROW_COUNT                #", &nx[1], &dumrel))
    goto label_end;
  if (st_ifpen_read(file, 1, "COLUMN_COUNT             #", &nx[0], &dumrel))
    goto label_end;
  if (st_ifpen_read(file, 2, "ROW_DISTANCE             #", &dumint, &dx[1]))
    goto label_end;
  if (st_ifpen_read(file, 2, "COLUMN_DISTANCE          #", &dumint, &dx[0]))
    goto label_end;
  if (st_ifpen_read(file, 1, "LAYER_COUNT              #", &nx[2], &dumrel))
    goto label_end;
  if (st_ifpen_read(file, 2, "X_ORIGIN                 #", &dumint, &x0[0]))
    goto label_end;
  if (st_ifpen_read(file, 2, "Y_ORIGIN                 #", &dumint, &x0[1]))
    goto label_end;
  if (st_ifpen_read(file, 1, "FACIES_COUNT             #", ncol, &dumrel))
    goto label_end;
  if (st_ifpen_read(file, 0, "DATA_PROP                #", &dumint, &dumrel))
    goto label_end;
  if (st_ifpen_read(file, 0, "##########################", &dumint, &dumrel))
    goto label_end;
                       ...
                       double *dx)
{
  ...
  if (st_grid_read_prop_header(file, verbose, ncol, nx, x0, dx, &test))
    goto label_end;
                       ...
                       double *tab)
{
  ...
  if (st_grid_read_prop_header(file, 0, &ncol, nx, x0, dx, &test))
    goto label_end;

  /* Check the compatibility of the input arguments with the current grid */

  if (ncol != ncol_r || nx[0] != nx_r[0] || nx[1] != nx_r[1] || x0[0] != x0_r[0]
      || x0[1] != x0_r[1] || dx[0] != dx_r[0] || dx[1] != dx_r[1])
  {
    messerr("The grid parameters are incompatible");
    goto label_end;
  ...
  {
    messerr(
        "The number of decoded values (%d) is not equal to the expected number of grid nodes (%d) x number of attributes (%d)",
        lec, nech, ncol);
    goto label_end;
  }
                 ...
                 const VectorString &names)
{
  ...
  if (flag_grid)
    write_rectilinear_mesh(filename, useBinary, dims, xcoor, ycoor, zcoor, ncol,
                           vardim, center, vc.data(), tab);
  else
    write_point_mesh(filename, useBinary, nactive, points, ncol, vardim,
                     vc.data(), tab);
  ...
  label_end: vardim = (int*) mem_free((char* ) vardim);
                     ...
                     double **tab)
{
  ...
  *nvarout = *nechout = 0;
  ...
  nvar = 0;
  varloc = (char**) mem_realloc((char* ) varloc, sizeof(char*) * nvar_special,
                                1);
  ...
  while (1)
  {
    ...
    varloc = (char**) mem_realloc((char* ) varloc, sizeof(char*) * (nvar + 1),
                                  1);
  ...
  while (1)
  {
    ...
    {
      nquant++;
      tabloc = (double*) mem_realloc((char* ) tabloc,
                                     sizeof(double) * nvar * nquant * quantum,
                                     1);
    }
  ...
  tabloc = (double*) mem_realloc((char* ) tabloc, sizeof(double) * nvar * nech,
                                 1);
                     ...
                     double **tab_arg)
{
  ...
  label_key: messerr("Error when looking for keyword %s", refchar);
  goto label_end;

  label_contents: messerr("Error when looking at contents of keyword %s",
                          refchar);
  goto label_end;
