...
namespace gstlrn
{
  ...
  Id db_write_vtk(const char* filename, DbGrid* db, const VectorInt& cols)
  {
    ...
    bool verbose)
  {
    ...
    if (mode > 0)
    {
      ...
      if (verbose)
      {
        ...
        message(
          "String for missing information = %s\n",
    ...
    else
    {
      ...
      if (verbose)
      {
        ...
        message(
          "Number of lines successfully written = %d\n", CSV_ENCODE.nlines);
      }
    }
    return 0;
  }
    ...
    VectorDouble& tab)
  {
    ...
    {
      messerr(
        "Error when opening the CSV file %s for reading", filename.c_str());
      return 1;
    }
    ...
    {
      message(
        "Data table read (%s) successfully\n",
