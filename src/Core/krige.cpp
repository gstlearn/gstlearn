...
namespace gstlrn
{
                       ...
                       double* cvtrans)
  {
    ...
    if (ndim < 1 || ndim > 2)
    {
      messerr("The transitive global estimation is implemented for 1 and 2 "
              "space only");
      return 1;
    }
    if (model->getNVar() != 1)
                         ...
                         Id neq)
  {
    ...
    for (i = -nbefore; i <= nafter; i++)
      ...
      {
        lhs_global.setValue(j + nbefore,
                            i + nbefore,
                            st_cov_exp(i - j, covdd, cov_radius, flag_sym));
        lhs_global.setValue(i + nbefore,
                            j + nbefore,
                ...
                Id dbg_iy)
  {
    ...
    for (ix = 0; ix < db->getNX(0); ix++)
      for (iy = 0; iy < db->getNX(1); iy++)
      {
        ...
        {
          mes_process("3-D Factorial Kriging Analysis",
                      DBOUT->getNSample(),
                      ecr);
          indg[0] = ix;
          indg[1] = iy;
             ...
             const NamingConvention& namconv)
  {
    ...
    {
      messerr("This procedure requires one Variable with Locator SUM in the "
              "Output Db");
    ...
    namconv.setNamesAndLocators(dbin,
                                VectorString(),
                                ELoc::Z,
                                nvar,
                                dbout,
                    ...
                    Id verbose)
  {
    ...
    if (st_sampling_krige_data(dbin,
                               model,
                               beta,
                               ranks1,
                               ranks2,
    ...
    for (Id ip = 0; ip < np; ip++)
      for (Id jp = ip; jp < np; jp++)
        for (Id is = 0; is < ns; is++)
        {
          covpp.setValue(ip,
                         jp,
                         covpp.getValue(ip, jp)
                           + DISTPS(ip, is) * PRODPS(jp, is));
                           ...
                           Model* model_src)
  {
    ...
    covgp = st_inhomogeneous_covgp(dbdat,
                                   dbsrc,
                                   dbout,
                                   flag_source,
                                   model_dat,
    ...
    covgg = st_inhomogeneous_covgg(dbsrc,
                                   dbout,
                                   flag_source,
                                   model_dat,
                                   distgs,
    ...
    for (IECH_OUT = 0; IECH_OUT < DBOUT->getNSample(); IECH_OUT++)
    {
      ...
      model_dat->evalDriftBySampleInPlace(dbout,
                                          IECH_OUT,
                                          ECalcMember::LHS,
                                          driftg);
