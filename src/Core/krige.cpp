...
#define IAD(ix, iy, iz, nn, ss)                                                \
  (((iz) + nn[2]) + ss[2] * (((iy) + nn[1]) + ss[1] * ((ix) + nn[0])))
...
namespace gstlrn
{
  ...
  static Id st_check_environment(Id flag_in, Id flag_out, Model* model)
  {
    ...
    if (model != nullptr)
    {
      ...
      if (flag_in) DBIN->getExtensionInPlace(db_mini, db_maxi, true);

      /* Output Db structure */

      if (flag_out) DBOUT->getExtensionInPlace(db_mini, db_maxi, true);
  ...
  static Id st_block_discretize_alloc(Id ndim, const VectorInt& ndiscs)
  {
    ...
    ntot = 1;
    for (Id idim = 0; idim < ndim; idim++) ntot *= ndiscs[idim];
    if (ntot <= 0) return (1);
    KOPTION.ntot = ntot;
    ...
    if (DBIN->getNLoc(ELoc::BLEX) > 0)
    {
      if (!get_keypair("Data_Discretization", &nrow, &ncol, KOPTION.dsize))
      {
        if (nrow * ncol != ndim)
        {
          ...
          messerr(
            "But its dimension should be %d (instead of %d x %d)", ndim, nrow,
            ncol);
        }
        else
        {
  ...
  static void st_block_discretize(Id mode, Id flag_rand, Id iech)
  {
    ...
    for (i = 0; i < ntot; i++)
    {
      ...
      {
        taille = (mode == 0) ? dbgrid->getDX(idim)
                             : DBOUT->getLocVariable(ELoc::BLEX, iech, idim);
        ...
        if (flag_rand)
          DISC2(i, idim) +=
            taille * law_uniform(-0.5, 0.5) / static_cast<double>(nd);
      }
    }
    ...
    if (mode == 1)
    {
      ...
      switch (KOPTION.calcul.toEnum())
      {
        ...
        case EKrigOpt::E_BLOCK:
          ...
          {
            messerr(
              "Discretization is not allowed if the Target is not a Grid");
            return error;
          }
          if (ndiscs.empty())
          {
   ...
   *****************************************************************************/
  void
    krige_lhs_print(Id nech, Id neq, Id nred, const Id* flag, const double* lhs)
  {
    ...
    for (ipass = 0; ipass < npass; ipass++)
    {
      ...
      printElement("Rank");
      printElement("    ");
      for (j = ideb; j < ifin; j++) printElement(j + 1);
      message("\n");
      ...
      if (flag != nullptr)
      {
        printElement("    ");
        printElement("Flag");
        for (j = ideb; j < ifin; j++) printElement(rel[j]);
        message("\n");
      }
      ...
      for (i = 0; i < nred; i++)
      {
        printElement(i + 1);
        printElement(rel[i]);
        for (j = ideb; j < ifin; j++) printElement(LHS_C(i, j));
        message("\n");
      }
    }
  }
    ...
    double* rhs)
  {
...

    {
      switch (KOPTION.calcul.toEnum())
      {
        ...
        case EKrigOpt::E_DGM: message("DGM Estimation\n"); break;
      }
    }
    message("\n");
    ...
    for (Id ivar = 0; ivar < nvar; ivar++) printElement(ivar + 1);
    message("\n");
    ...
    for (Id i = 0; i < nred; i++)
    {
      ...
      for (Id ivar = 0; ivar < nvar; ivar++) printElement(RHS_C(i, ivar));
      message("\n");
    }
  }
   ...
   **
   *****************************************************************************/
  static void st_krige_wgt_print(
    Id status,
    Id nvar,
    Id nvar_m,
    Id nfeq,
    ...
    const double* wgt)
  {
    ...
    if (DBIN->getNLoc(ELoc::V) > 0) printElement("Err.");
    ...
    for (jvar_m = lec = cumflag = 0; jvar_m < nvar_m; jvar_m++)
    {
      ...
      for (iech = 0; iech < nech; iech++, lec++)
      {
        ...
        if (KOPTION.flag_data_disc)
        {
          for (idim = 0; idim < ndim; idim++)
            printElement(
              DBIN->getLocVariable(ELoc::BLEX, nbgh_ranks[iech], idim));
        }
        if (icase < 0)
        ...
        for (ivar = 0; ivar < nvar; ivar++)
        {
          ...
          value =
            (wgt != nullptr && status == 0 && flag_value) ? wgt[iwgt] : TEST;
  ...
  static void st_result_kriging_print(Id flag_xvalid, Id nvar, Id status)
  {
    ...
    for (ivar = 0; ivar < nvar; ivar++)
    {
      if (flag_xvalid != 0)
      {
        ...
        message(
          "Printout for Cross-validation should not be performed anymore\n");
      }
      else
      {
        message("Variable Z%-2d\n", ivar + 1);
        ...
        {
          value =
            (status == 0) ? DBOUT->getArray(IECH_OUT, IPTR_EST + ivar) : TEST;
        ...
        {
          value =
            (status == 0) ? DBOUT->getArray(IECH_OUT, IPTR_STD + ivar) : TEST;
        ...
        {
          value =
            (status == 0) ? DBOUT->getArray(IECH_OUT, IPTR_VARZ + ivar) : TEST;
    ...
    double* cvtrans)
  {
    ...
    {
      messerr(
        "The transitive global estimation is implemented for 1 and 2 space "
        "only");
      return 1;
    }
    if (model->getNVar() != 1)
   ...
   *****************************************************************************/
  static Id
    st_get_limits(DbGrid* db, double top, double bot, Id* ideb, Id* ifin)
  {
    Id ndim, nz, iad;
    double z0, dz;
   ...
   *****************************************************************************/
  static double
    st_cov_exp(Id dist, const double* cov, Id cov_radius, Id flag_sym)
  {
   ...
   *****************************************************************************/
  static double
    st_estim_exp(Db* db, const VectorDouble& wgt, Id nbefore, Id nafter)
  {
    Id i;
    double result;
    ...
    Id nfeq)
  {
    ...
    {
      messerr(
        "The radius of the neighborhood (%d) must be smaller or equal",
        neigh_radius);
    ...
    for (IECH_OUT = 0; IECH_OUT < nech; IECH_OUT++)
    {
      ...
      if (nbefore_mem != nbefore || nafter_mem != nafter || OptDbg::force())
      {
        ...
        if (OptDbg::query(EDbg::KRIGING))
          krige_lhs_print(
            nech, neq, neq, flag_global.data(), lhs_global.getValues().data());
        ...
        if (OptDbg::query(EDbg::KRIGING))
          krige_rhs_print(
            nvarin, nech, neq, neq, flag_global.data(), rhs_global.data());
      ...
      if (OptDbg::query(EDbg::RESULTS))
        st_result_kriging_print(0, nvarin, status);
    }
   ...
   **
   *****************************************************************************/
  static void st_calculate_covres(
    DbGrid* db,
    Model* model,
   ...
   **
   *****************************************************************************/
  static void st_calculate_covtot(
    DbGrid* db,
    Id ix0,
    Id iy0,
    Id flag_sym,
   ...
   **
   *****************************************************************************/
  static VectorInt st_neigh_find(
    DbGrid* db,
    Id ix0,
    Id iy0,
    Id iz0,
    ...
    Id dbg_iy)
  {
    ...
    st_calculate_covres(
      db, model, cov_ref, cov_radius, flag_sym, cov_ss, cov_nn, cov_res);
    ...
    for (ix = 0; ix < db->getNX(0); ix++)
      for (iy = 0; iy < db->getNX(1); iy++)
      {
        ...
        st_calculate_covtot(
          db, ix, iy, flag_sym, cov_ss, cov_nn, num_tot, cov_tot);
        ...
        {
          mes_process(
            "3-D Factorial Kriging Analysis", DBOUT->getNSample(), ecr);
          indg[0] = ix;
          indg[1] = iy;
          indg[2] = iz;
          ...
          flag_new =
            flag_col || st_neigh_diff(nei_ss, nei_nn, nei_ref, nei_cur);
          ...
          if (flag_new || OptDbg::force())
          {
            ...
            st_lhs_exp_3D(
              nech, nfeq, nei_ss, nei_nn, cov_ss, cov_nn, nei_cur, cov_tot,
              nugget);
            if (OptDbg::query(EDbg::KRIGING))
              krige_lhs_print(
                nech, neq, neq, flag_global.data(),
                lhs_global.getValues().data());
            ...
            st_rhs_exp_3D(
              nech, nfeq, nei_ss, nei_nn, cov_ss, cov_nn, nei_cur, cov_res);
            if (OptDbg::query(EDbg::KRIGING))
              krige_rhs_print(
                nvarin, nech, neq, neq, flag_global.data(), rhs_global.data());
    ...
    const NamingConvention& namconv)
  {
    ...
    {
      messerr(
        "This procedure requires one Variable with Locator SUM in the Output "
        "Db");
      messerr(
        "The number of such variable is currently equal to %d",
    ...
    for (Id iech_out = 0; iech_out < dbout->getNSample(); iech_out++)
    {
      ...
      while (!correct)
      {
        ...
        {
          messerr(
            "The sum of scaling terms is zero. No correction is possible");
          return 1;
        }
        ...
        for (Id ivar = 0; ivar < nvar; ivar++)
        {
          ...
          if (!active[ivar])
            estim =
              (dbout->getArray(iech_out, iptr_est + ivar)
   ...
   *****************************************************************************/
  static VectorInt
    st_ranks_other(Id nech, const VectorInt& ranks1, const VectorInt& ranks2)
  {
    VectorInt rother(nech, 0);
    for (Id i = 0; i < nech; i++) rother[i] = i;
   ...
   **
   *****************************************************************************/
  static Id st_sampling_krige_data(
    Db* db,
    Model* model,
    double beta,
    VectorInt& ranks1,
    ...
    MatrixSquare& invsig)
  {
    ...
    if (nsize2 > 0)
    {
      ...
      auto* tn2 = dynamic_cast<MatrixSymmetric*>(
      ...
      MatrixSquare* eigvec = eigenvectors.getEigenVectors().clone();

      eigvec->prodByDiagInPlace(3, eigval);
      auto* spart = dynamic_cast<MatrixDense*>(
      ...
      if (beta > 0.)
      {
        ...
        for (i = 0; i < nmax; i++)
          for (j = 0; j < nsize2; j++) spart->setValue(isort[i], j, 0.);
      }
    ...
    for (i = ecr = 0; i < ndat; i++)
    {
      ...
      rutil[ecr] = ralls[i];
      for (j = 0; j < ntot; j++) tutil.setValue(ecr, j, UTAB(i, j));
      ecr++;
    }
    ...
    VectorDouble& data_var)
  {
    ...
    if (st_sampling_krige_data(
          db, model, beta, ranks1, ranks2, rother, &ntot, &nutil, rutil, tutil,
          invsig))
      return 1;
    ...
    for (Id i = 0; i < nutil; i++) datm[i] = data[rutil[i]] - model->getMean(0);
    ...
    VectorDouble& crit)
  {
    ...
    for (Id iech = ecr = 0; iech < ndat; iech++)
    {
      ...
      value = 0.;
      for (Id i = 0; i < nutil; i++) value += cs1[i] * cs1[i];
    ...
    Id verbose)
  {
    ...
    while (nsize1 < nsize1_max)
    {
      ...
      {
        if (st_krige_data(
              db, model, beta, ranks1, ranks2, rother, 1, data_est, data_var))
          return 1;
      ...
      if (verbose)
        message(
          "Exact Pivots (%3d/%3d): Rank = %3d - value = %lf\n", nsize1 + 1,
    ...
    while (nsize2 < nsize2_max)
    {
      ...
      if (verbose)
        message(
          "ACP   Pivots (%3d/%3d): Rank = %3d - value = %lf\n", nsize2 + 1,
    ...
    Id verbose)
  {
    ...
    if (st_sampling_krige_data(
          dbin, model, beta, ranks1, ranks2, rother, &ntot, &nutil, rutil,
          tutil, invsig))
      return 1;
    ...
    VectorDouble s;
    VectorDouble c00;
    if (FLAG_STD) aux4.resize(ntot, 0);
    ...
    bool verbose)
  {
    ...
    switch (method)
    {
      ...
      default: messerr("Not yet implemented"); return 1;
    }
   ...
   **
   *****************************************************************************/
  static MatrixDense st_calcul_covmat(
    const char* title,
    Db* db1,
    Id test_def1,
    Db* db2,
    ...
    Model* model)
  {
    ...
    n1 =
      (test_def1) ? db1->getNSampleActiveAndDefined(0) : db1->getNSample(true);
    n2 =
      (test_def2) ? db2->getNSampleActiveAndDefined(0) : db2->getNSample(true);
   ...
   *****************************************************************************/
  static VectorDouble
    st_calcul_drfmat(const char* title, Db* db1, Id test_def1, Model* model)
  {
    Id i1, n1, nbfl;
    VectorDouble drftab;
    ...
    for (Id ii1 = 0; ii1 < db1->getNSample(); ii1++)
    {
      ...
      VectorDouble drfloc =
        model->evalDriftBySample(db1, ii1, ECalcMember::LHS);
   ...
   **
   *****************************************************************************/
  static VectorDouble st_calcul_distmat(
    const char* title,
    Db* db1,
    Id test_def1,
    Db* db2,
   ...
   **
   *****************************************************************************/
  static VectorDouble st_calcul_product(
    const char* title,
    Id n1,
    Id ns,
   ...
   **
   *****************************************************************************/
  static MatrixDense st_inhomogeneous_covpp(
    Db* dbdat,
    Db* dbsrc,
    Model* model_dat,
    ...
    const VectorDouble& prodps)
  {
    ...
    auto covpp =
      st_calcul_covmat("Covariance P-P", dbdat, 1, dbdat, 1, model_dat);
    ...
    for (Id ip = 0; ip < np; ip++)
      for (Id jp = ip; jp < np; jp++)
        ...
        {
          covpp.setValue(
            ip, jp, covpp.getValue(ip, jp) + DISTPS(ip, is) * PRODPS(jp, is));
          if (jp > ip) covpp.setValue(jp, ip, covpp.getValue(ip, jp));
        }
   ...
   **
   *****************************************************************************/
  static MatrixDense st_inhomogeneous_covgp(
    Db* dbdat,
    Db* dbsrc,
    Db* dbout,
    Id flag_source,
    ...
    const VectorDouble& prodgs)
  {
    ...
    auto covgp =
      st_calcul_covmat("Covariance G-P", dbout, 0, dbdat, 1, model_dat);
    ...
    if (!flag_source)
    {
      for (Id ig = 0; ig < ng; ig++)
        for (Id ip = 0; ip < np; ip++)
          for (Id is = 0; is < ns; is++)
            covgp.setValue(
              ig, ip, covgp(ig, ip) + DISTPS(ip, is) * PRODGS(ig, is));
    }
    else
    {
      for (Id ig = 0; ig < ng; ig++)
        for (Id ip = 0; ip < np; ip++) covgp.setValue(ig, ip, PRODPS(ip, ig));
    }
   ...
   **
   *****************************************************************************/
  static VectorDouble st_inhomogeneous_covgg(
    Db* dbsrc,
    Db* dbout,
    Id flag_source,
    Model* model_dat,
    ...
    const VectorDouble& prodgs)
  {
    ...
    {
      for (Id ig = 0; ig < ng; ig++) covgg[ig] = c00;
    }
    return covgg;
  }
    ...
    MatrixSquare& zmat)
  {
    ...
    for (Id il = 0; il < nbfl; il++)
      ...
      {
        value = 0.;
        for (Id ip = 0; ip < np; ip++) value += YMAT(ip, il) * DRFTAB(ip, jl);
    ...
    VectorDouble& mu)
  {
    ...
    for (Id ip = 0; ip < np; ip++)
      for (Id il = 0; il < nbfl; il++) lambda[ip] -= YMAT(ip, il) * mu[il];
  }
    ...
    Model* model_src)
  {
    ...
    covgp = st_inhomogeneous_covgp(
      dbdat, dbsrc, dbout, flag_source, model_dat, distps, prodps, prodgs);
    ...
    covgg = st_inhomogeneous_covgg(
      dbsrc, dbout, flag_source, model_dat, distgs, prodgs);
    ...
    for (IECH_OUT = 0; IECH_OUT < DBOUT->getNSample(); IECH_OUT++)
    {
      ...
      if (OptDbg::force())
        krige_rhs_print(nvar, np, neq, nred, NULL, rhs.data());

      /* Fill the drift at Target point (optional) */

      model_dat->evalDriftBySampleInPlace(
        dbout, IECH_OUT, ECalcMember::LHS, driftg);
      ...
      if (OptDbg::force())
        st_krige_wgt_print(
          0, nvar, nvar, nfeq, nbgh_ranks, nred, -1, NULL, lambda.data());
      ...
      if (nbfl > 0) stdev += mu.innerProduct(maux);
