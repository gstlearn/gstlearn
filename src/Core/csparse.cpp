...
csn *cs_chol (const cs *A, const css *S)
  ...
  int *cs_counts (const cs *A, const int *parent, const int *post, int ata)
  {
	  ...
	  *head = NULL, *next = NULL, *colcount, *w, *first, *delta ;
      ...
      s = 4*n + (ata ? (n+m+1) : 0) ;
      w = (int *) 
        cs_malloc (s, sizeof (int)) ; first = w+3*n ;	/* get workspace */
...
int cs_ltsolve ( const cs *L, double *x )
 ...
 cs *cs_transpose (const cs *A, int values)
 {
     ...
     if (!C || !w || m <= 0 || n <= 0) 
     {
       messerr("Problem when transposing a matrix in CSparse Library (%d x %d)",
               m,n);
...
String toStringRange(const String& title, const cs *C)
{
  ...
  sstr << " Descr: m=" << C->m << " - n=" << C->n <<
      " - nzmax=" << C->nzmax << std::endl;
  sstr << " Range: [" << mini << " ; " << maxi << "] (" <<
      nvalid << " / " << number << ")" << std::endl;
...
int cs_sort_i(cs *C)
{
   ...
   for (j = 0; j < n; j++)
   {
     for (i=C->p[j], ecr=0; i<C->p[j+1]; i++) rank[ecr++] = C->i[i];
     ut_sort_int(0,ecr,NULL,rank);
     for (i=C->p[j], ecr=0; i<C->p[j+1]; i++) C->i[i] = rank[ecr++];
   }
...
void cs_print_nice (const char *title, const cs *A, int maxrow, int maxcol)
 {
   ...
   for (int ipass=0; ipass<npass; ipass++)
   {
     ...
     message("      ");
     for (j = jdeb; j < jfin; j++) message("    [,%3d]",j+1);
     message("\n");
...
double *cs_col_sumrow(const cs *A,
		      ...
		      int *nrow)
 {
   ...
   *ncol = *nrow = 0;
   ...
   {
     sumval = 0.;
     for (int p = Ap [j] ; p < Ap [j+1] ; p++) sumval += Ax[p];
     vect[j] = sumval;
   }
...
void cs_mulvec(const cs *A, int nout, const double *x, double *y)
{
 ...
 static double func0(double x){return(x);}
 static double func1(double x){return(1./x);}
 static double func2(double x){return(1/sqrt(x));}
 static double func3(double x){return(sqrt(x));}
...
static int st_coarse_type0(cs  *Q,
			   ...
			   cs **Ltret)
 {
  ...
  label_end:
   lambda   = (int *) mem_free((char *) lambda);
   Ltriplet = cs_spfree(Ltriplet);
   if (error) 
   {
     L	= cs_spfree(L);
		   ...
		   cs  **L_ret)
 {
   ...
   for (int i=0; i<n; i++) indUd[i] = indCo[i];
			   ...
			   int path_type)
{
  ...
  switch (path_type)
  {
    case 1:			// V type
      ...
      ecr = 0;
      for (int i=0; i<nlevels; i++) mgs->path[ecr++] =	1;
      for (int i=0; i<nlevels; i++) mgs->path[ecr++] = -1;
      break;
	...
	mgs->npath = niw;
	mgs->path  = (int *) mem_alloc(sizeof(int) * mgs->npath,1);
	for (int i=0; i<niw; i++) mgs->path[i] = iw[i];
	...
	for (int ilevel=2; ilevel<nlevels; ilevel++)
	{
	  ...
	  mgs->npath = 2 + 2 * n;
	  mgs->path  = (int *) 
	    mem_realloc((char *) mgs->path,sizeof(int) * mgs->npath,1);
	  for (int i=n-1; i>=0; i--)
	  {
    ...
    case 3:			// F type
      mgs->npath = nlevels;
      mgs->path	 = (int *) mem_alloc(sizeof(int) * mgs->npath,1);
      for (int i=0; i<nlevels; i++) mgs->path[i] = 1;
	...
	for (int i=0; i<ilevel; i++)
	{
      ...
      mgs->path	  = (int *) 
	mem_realloc((char *) mgs->path,sizeof(int) * mgs->npath,1);
      for (int i=0; i<nlevels; i++) mgs->path[i+n] = -1;
      break;
  }
}
			...
			int rank)
{
  ...
  if (mg->IhH != NULL)
    cs_print_range("Range of IhH" ,mg->IhH);
  if (mg->A != NULL)
    cs_print_range("Range of A"	  ,mg->A->Q);
  if (mg->sumrow != NULL)
...
void cs_multigrid_print(cs_MGS *mgs)
{
  ...
  for (int i=0; i<=mgs->nlevels; i++) st_mg_print(mgs,i);
}
                                 ...
                                 double	  *work)
{
  ...
  for (int icur=0; icur<mg->nh; icur++) work[icur] = rhsin[icur] - work[icur];
			  ...
			  double   *work)
{
  ...
  if (mode > 0)
  {
    ...
    if (level != 0)
      for (int icur=0; icur<mg->nh; icur++) xcr[icur] = 0.;

    for (int i=0; i<mgs->ngs; i++)
    {
      cs_mulvec_uptri(mg->A->Q, mg->nh, xcr, work, 0);
      for (int icur=0; icur<mg->nh; icur++) xcr[icur] = rhs[icur] - work[icur];
      cs_lsolve_lowtri(mg->A->Q,xcr,work);    
      for (int icur=0; icur<mg->nh; icur++) xcr[icur] = work[icur];
    }
  }
  else
  {
                                     ...
                                     double *work)
{
  ...
  for (int icur=0; icur<ncur; icur++) x[icur] = XCR(0,icur);
                                   ...
                                   double *work)
{
  ...
  for (int icur=0; icur<mg->nh; icur++) resid[icur] = b[icur] - work[icur];
  for (int icur=0; icur<ncur; icur++) z[icur] = 0.;
  st_multigrid_kriging_prec(mgs,0,z,resid,work);
  matrix_product(1,ncur,1,resid,z,&sn);
  for (int icur=0; icur<ncur; icur++) p[icur] = z[icur];
  ...
  for (int iter=0; iter<mgs->ngc; iter++)
  {
    ...
    scores[niter++] = score;
    if (verbose) message("Iteration Gradient %3d -> Score = %15.10lf\n",
			 iter+1,score);
                       ...
                       double **sel_arg)
{
 ...
 label_end:
  if (error) sel = (double *) mem_free((char *) sel);
  indCo = (int *) mem_free((char *) indCo);
  L	= cs_spfree(L);
  return(error);
}
                         ...
                         int	*temp)
{
  ...
  for (int j=0; j<ncolor; j++) temp[j] = 0;
