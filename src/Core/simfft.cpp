...
**
*****************************************************************************/
static void st_simfft_init(Db     *db,
                           ST_FFT *simu)
{
  int i;
                             ...
                             int flag_amplitude)
{
  ...
  for (kbound=0; kbound<=kbmax; kbound++)
  {
    ...
    kb2   = (ndim >= 2) ? kbound : 0;
    kb3   = (ndim >= 3) ? kbound : 0;
    for (i=0; i<simu->sizes_alloc; i++) cplx[i] = cply[i] = 0.;
    ...
    for (iz=0; iz<simu->dims[2]; iz++)
      for (iy=0; iy<simu->dims[1]; iy++)
        ...
        {
          jnd[0] = (ix <= simu->dim2[0]) ? ix : ix - simu->dims[0];
          jnd[1] = (iy <= simu->dim2[1]) ? iy : iy - simu->dims[1];
          jnd[2] = (iz <= simu->dim2[2]) ? iz : iz - simu->dims[2];
          for (i=0; i<3; i++)
          {
            proj = 0.;
            for (j=0; j<3; j++) proj += jnd[j] * xyz1[j][i];
            xyz[i] = (i < ndim) ? proj : 0.;
          }
          for (k1=-kb1; k1<=kb1; k1++)
            for (k2=-kb2; k2<=kb2; k2++)
    ...
    if (DEBUG)
      print_matrix("Discretized Covariance",0,1,
    ...
    if (flag_amplitude)
      for (i=0; i<simu->sizes_alloc; i++) cplx[i] = sqrt(cplx[i] / 2.);
    ...
    if (DEBUG)
      print_matrix("Discretized (square root of half-) Spectrum",
                   0,1,simu->dims[0],simu->dims[1],NULL,cplx);
...
**
*****************************************************************************/
static void st_set_variance(ST_FFT *simu,
                            int ix,
                            int iy,
                            int iz)
{
  ...
  simu->v[ind] = 0.;
  if (DEBUG) 
    message("Element (%d,%d,%d) variance updated\n",ix,iy,iz);
}
...
**
*****************************************************************************/
static void st_set_zero(ST_FFT *simu,
                        int ix,
                        int iy,
                        int iz)
{
  ...
  simu->v[ind] = 0.;
  if (DEBUG) 
    message("Element (%d,%d,%d) has no imaginary part\n",ix,iy,iz);
}
...
**
*****************************************************************************/
static int st_total_count(int  ndim,
                          int *nxyz)
{
  int idim,total;

  total = 1;
  for (idim=0; idim<ndim; idim++) total *= nxyz[idim];
  return (total);
}
...
**
*****************************************************************************/
static void st_grid_dilate(Db     *db,
                           Model  *model,
                           ST_FFT *simu,
                           double  percent)
{
  double xyz0[3],xyz[3][3];
  int i,j,idx,idy,idz,ndx,ndy,ndz,correct,not_ok,not_ok_dir,ndim,indg[3];
...
**
*****************************************************************************/
static int st_simfft_alloc(Db     *db,
                           Model  *model,
                           double  percent,
                           ST_FFT *simu)
{
...
**
*****************************************************************************/
static int st_check_simfft_environment(Db     *db,
                                       ST_FFT *simu,
                                       Model  *model)
{
...
**
*****************************************************************************/
static void st_print(ST_FFT *simu,
                     int iz)
{
  if (! DEBUG) return;
...
**
*****************************************************************************/
static void st_simfft_sym_2(ST_FFT *simu,
                            int iz0)
{
  int ix,iy,jx,jy;
...
**
*****************************************************************************/
static double st_rho_sigma(ST_FFT *simu,
                           double  sigma,
                           int     ix,
                           int     iy,
                           int     iz)
...
**
*****************************************************************************/
static double st_support_1(ST_FFT *simu,
                           double  sigma)
{
  int ix,iix;
  double value,rho;
...
**
*****************************************************************************/
static double st_support_2(ST_FFT *simu,
                           double  sigma)
{
  int ix,iy,iix,iiy;
  double value,rho;
  ...
  for (ix=-simu->nx[0]; ix<=simu->nx[0]; ix++)
    ...
    {
      iix = (ix < 0) ? simu->dims[0] + ix : ix;
      iiy = (iy < 0) ? simu->dims[1] + iy : iy;
      rho = st_rho_sigma(simu,sigma,iix,iiy,0);
      value += ((simu->nx[0] - ABS(ix)) *
                (simu->nx[1] - ABS(iy)) *
                rho);
    }
  return(value);
...
**
*****************************************************************************/
static double st_support_3(ST_FFT *simu,
                           double  sigma)
{
...
**
*****************************************************************************/
static double st_support(ST_FFT *simu,
                         double  sigma)
{
  double value,scale;
  int    idim;
                               ...
                               double *coeffs)
{
  ...
  if (DEBUG)
    print_matrix("Discretized Covariance (after scaling)",0,1,
                 simu.dims[0],simu.dims[1],NULL,simu.cmat);
