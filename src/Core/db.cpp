...
{
  if (get_NDIM(db1) <= get_NDIM(db2)) return (1);
  messerr("The Space Dimension of the First Db (%d)", get_NDIM(db1));
...

{
  ...
  ndim1 = std::min(get_NDIM(db1), get_NDIM(db2));
  for (i = 0; i < ndim1; i++)
  {
                                  ...
                                  double *dist_vect)
{
  ...
  ndim = MIN(get_NDIM(db1), get_NDIM(db2));
                                 ...
                                 int flag_nerr)
{
  ...
  if (flag_ndim) for (idim = 0; idim < get_NDIM(db); idim++)
  {
                             ...
                             int *cols)
{
  if (!(db1->isGrid() && db2->isGrid()))
  {
    messerr("This command requires two Dbs organized as regular grids");
    return (1);
  }
  ...
  for (int iech = 0; iech < get_NECH(db2); iech++)
  {
    ...
    for (int idim = 0; idim < get_NDIM(db2); idim++)
    {
      if (ind2[idim] > 0)
...
GEOSLIB_API int db_prop_read(Db *db, int ix, int iy, double *props)
{
  ...
  if (!db->isGrid() || get_NDIM(db) != 3) return (1);
                               ...
                               double vmax)
{
  ...
  label_end: if (error) ss_grid = db_delete(ss_grid);
...
GEOSLIB_API VectorDouble db_get_grid_axis(Db *dbgrid, int idim)
{
  ...
  if (idim < 0 || idim >= get_NDIM(dbgrid)) return (vect);
