...
namespace gstlrn
{
  ...
  Id compat_NDIM(Db* db1, Db* db2)
  {
    ...
    messerr(
      "must not be smaller than the Space Dimension of the Second Db",
      db2->getNDim());
    return (0);
  }
  ...
  Id get_LOCATOR_NITEM(const Db* db, const ELoc& locatorType)
  {
    ...
    if (db->isGrid() && locatorType == ELoc::X) return (db->getNDim());
    ...
    double* dist_vect)
  {
    ...
    if (iech1 == iech2)
    {
      if (dist_vect != nullptr)
        for (Id idim = 0; idim < db->getNDim(); idim++) dist_vect[idim] = 0.;
      return (0.);
    }
  ...
  Id db_center(Db* db, double* center)
  {
    ...
    if (db->hasLocVariable(ELoc::SEL)) sel = db->getColumnByLocator(ELoc::SEL);
    if (db->hasLocVariable(ELoc::W)) wgt = db->getColumnByLocator(ELoc::W);
    ...
    double* tabout)
  {
    ...
    ntotal = 1;
    for (idim = 0; idim < ndim; idim++) ntotal *= nx[idim];
   ...
   ** \param[in]  nshift  Vector of dilation/compression defined in cell
   *number
   ...
   ** \param[in]  coor          array of coordinates of the point
   ** \param[in]  flag_outside  value returned for the point outside the
   *grid
   ** \li                       1 the index is set to the closest grid
   *node
  ...
  /*****************************************************************************/
  /*!
   **  Converts from point coordinates to index of the bench to which it
   *belongs
   **
   ** \return  Error return code
   ** \return   0 if the point is inside the grid
   ...
   **
   ** \remarks The bench corresponds to the third dimension of the grid
   *provided
    ...
    double* coor)
  {
    ...
    if (point_to_grid(dbout, coor, flag_outside, iwork1.data()) < 0)
      return (-1);
   ...
   ** \param[in]  nfac1max  Maximum number of facies for the first
   *variable
   ** \param[in]  nfac2max  Maximum number of facies for the second
   *variable
   **
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
    {
      if (nmax[ivar] <= 0)
      {
        ...
        ut_facies_statistics(
          nech, tab.data(), sel.data(), &nval, &mini, &nmax[ivar]);
      }
    }
    Id nclass = nmax.prod();
    ...
    Id flag_locnew)
  {
    ...
    else
    {
      ...
      for (Id i = 0; i < number; i++)
      {
        ...
        {
          if (locatorIdentify(
                strings[current[j] - 1], &ref_type, &ref_item, &nmult))
            continue;
    ...
    while (it.hasNext())
    {
      if (*it != ELoc::UNDEFINED)
      {
        ...
        for (Id i = 0; i < number; i++)
        {
          if (locatorIdentify(strings[i], &cur_type, &cur_item, &nmult))
            continue;
  ...
  /****************************************************************************/
  /*!
   **  Transform a set of gradients defined by (modulus,angle) into
   *(gx,gy)
    ...
    double vmax)
  {
    ...
    nx = indmax;
    x0 = coor;
    ss_grid = DbGrid::create(
      nx, db_grid->getDXs(), x0, db_grid->getAngles(), ELoadBy::COLUMN,
    ...
    Id verbose)
  {
    ...
    if (verbose)
    {
      ...
      for (Id idim = 0; idim < ndim; idim++)
        message(
          "- Dimension %d: NX_S =%4d - NX_G =%4d - Shift =%4d\n", idim + 1,
