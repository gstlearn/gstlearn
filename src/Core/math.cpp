...
#define NELEM(ctables)                                                         \
  ((ctables->flag_cumul) ? ctables->ndisc + 1 : ctables->ndisc)
...
namespace gstlrn
{
   ...
   **
   *****************************************************************************/
  static void st_tableone_manage(
    CTables* ctables,
    Id mode,
    Id rank,
    Id* nb_used,
    ...
    *nb_max = size;
    ...
    if (mode > 0)
    {
      ...
      if (ctables->res[rank].empty())
      {
        ...
        for (Id i = 0; i < size; i++) INTRESX(rank, i) = TEST;
        return;
      }
    }
    else
   ...
   *****************************************************************************/
  double
    ct_INTRES3(CTables* ctables, Id iconf0, Id idisc0, Id jdisc0, Id kdisc0)
  {
  ...
  void ct_tables_print(CTables* ctables, Id flag_print)
  {
    ...
    message(
      "Lower Bound of Covariance Discretization    = %lf\n", ctables->cmin);
    message(
      "Upper Bound of Covariance Discretization    = %lf\n", ctables->cmax);
    ...
    message(
      "Covariance is discretized between %lf and %lf\n", ctables->cmin,
      ctables->cmax);
    ...
    if (!ctables->v.empty())
      printMatrix(
        ctables->v, 1, ctables->ndisc + 1, "List of Gaussian Thresholds", 0, 1);
    ...
    if (flag_print > 0)
    {
      ...
      for (Id iconf = 0; iconf < ctables->nconf; iconf++)
      {
        ...
        if (flag_print == 2)
          printMatrix(ctables->res[iconf], nelem, nelem, String(), 0, 1);
      }
      message("\n");
    }
  }
    ...
    CTables* ctables_old)
  {
    ...
    if (mode > 0)
    {
      ...
      if (verbose)
        message(
          "Allocating CTables (%dx%d) for %d possible configurations\n", ndisc,
          ndisc, nconf);
      ctables = new CTables;
      ...
      ctables->cmax = cmax;
      ctables->dc =
        (ctables->cmax - ctables->cmin) / static_cast<double>(nconf - 1);
      ...
      for (Id idisc = 0; idisc < ndisc; idisc++)
        ctables->v[idisc] =
          law_invcdf_gaussian(static_cast<double>(idisc) * ctables->dp);
    }
    else
    {
      ...
      for (Id iconf = 0; iconf < ctables->nconf; iconf++)
      {
        ...
        if (nb_used > 0)
        {
          if (verbose)
            message(
              "Configuration %3d - Number of items used: %d/%d\n", iconf + 1,
              nb_used, nb_max);
          n_used++;
        }
      }
   ...
   **
   *****************************************************************************/
  static void st_tableone_getrank(
    const CTables* ctables,
    double low,
    double up,
    Id* indmin,
    Id* indmax)
    ...
    double* ups)
  {
    ...
    else
    {
      ...
      st_tableone_getrank(ctables, lows[1], ups[1], &i2min, &i2max);
      result =
        (ct_INTRES2(ctables, iconf0, i1max, i2max)
   ...
   **
   *****************************************************************************/
  double ct_tableone_calculate_by_rank(
    CTables* ctables,
    Id iconf0,
    double* rklows,
    double* rkups)
  {
    ...
    else
    {
      ...
      result =
        (ct_INTRES2(
           ctables, iconf0, static_cast<Id>(rkups[0]),
           static_cast<Id>(rkups[1]))
         - ct_INTRES2(
           ctables, iconf0, static_cast<Id>(rklows[0]),
           static_cast<Id>(rkups[1]))
         - ct_INTRES2(
           ctables, iconf0, static_cast<Id>(rkups[0]),
           static_cast<Id>(rklows[1]))
         + ct_INTRES2(
           ctables, iconf0, static_cast<Id>(rklows[0]),
           static_cast<Id>(rklows[1])));
    }
