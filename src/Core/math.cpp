...
namespace gstlrn
{
  ...
  void ct_tables_print(CTables* ctables, Id flag_print)
  {
    ...
    if (!ctables->v.empty())
      printMatrix(ctables->v,
                  1,
                  ctables->ndisc + 1,
                  "List of Gaussian Thresholds",
                  0,
                  1);
                                       ...
                                       double* rkups)
  {
    ...
    if (!ctables->flag_cumul)
    {
      ...
      for (Id idisc = static_cast<Id>(rklows[0]);
           idisc < static_cast<Id>(rkups[0]);
           idisc++)
        for (Id jdisc = static_cast<Id>(rklows[1]);
             jdisc < static_cast<Id>(rkups[1]);
             jdisc++)
          result += ct_INTRES2(ctables, iconf0, idisc, jdisc);
    }
    else
    {
      ...
      result = (ct_INTRES2(ctables,
                           iconf0,
                           static_cast<Id>(rkups[0]),
                           static_cast<Id>(rkups[1]))
                - ct_INTRES2(ctables,
                             iconf0,
                             static_cast<Id>(rklows[0]),
                             static_cast<Id>(rkups[1]))
                - ct_INTRES2(ctables,
                             iconf0,
                             static_cast<Id>(rkups[0]),
