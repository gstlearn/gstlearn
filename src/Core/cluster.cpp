                                 ...
                                 double *errors)
{
  ...
  for (int i = 0; i < nclusters; i++) errors[i] = DBL_MAXIMUM;
                              ...
                              int     verbose)
{
  ...
  for (int ipass=0; ipass<npass; ipass++)
  {
    ...
    while(1)
    {
      ...
      /* Find the center */
      if (mode == 0)
        st_getclustermeans(data,nvar,nech,nclusters,tclusterid,cdata,cmask);
      else
        st_getclustermedian(data,nvar,nech,nclusters,tclusterid,cdata,cache);
...
label_end:
  if (error || ifound <= 0) cdata = (double *) mem_free((char *) cdata);
