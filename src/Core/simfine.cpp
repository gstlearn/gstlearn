...
**
*****************************************************************************/
static void st_merge_data(Db  *db1,
                          int  iatt1,
                          Db  *db2,
                          int  iatt2)
{
...
**
*****************************************************************************/
static void st_truncate_result(Db  *db2,
                               int  iatt2,
                               Db  *db1,
                               int  iatt1)
{
                               ...
                               int iz0)
{
  ...
  if (iz0 == 0)
  {
    ...
    for (i=0; i<4; i++)
      value += (WGT[type][0][i] * 
                st_read(db,iatt,
                        ix0,iy0,iz0,IX[type][i],IY[type][i],IZ[type][i]));
...
**
*****************************************************************************/
static int st_kriging_solve(int type,
                            int rank,
                            int nb,
                            Model *model)
{
  ...
  if (! FLAG_KS)
  {
    for (i=0; i<nb; i++) LHS(i,nb) = LHS(nb,i) = 1.;
    LHS(nb,nb) = 0;
    RHS(nb)    = 1.;
  }
  ...
  STDV[type][rank] = (variance > 0) ? sqrt(variance) : 0.;
  ...
  if (debug_query("kriging"))
  {
    ...
    for (i=0; i<nb; i++)
      message("X=%10.3lf Y=%10.3lf Z=%10.3lf W=%10.6lf\n",
              XN[type][i],YN[type][i],ZN[type][i],WGT[type][rank][i]);
