...
namespace gstlrn
{
                        ...
                        double* t2max)
  {
    ...
    if (flag_check && !isNA(facies) && rule->getModeRule() == ERule::STD)
    {
      ...
      {
        messerr("The presence of facies (%d) at sample (%d) is not consistent "
                "with the zero proportion",
                facies,
                iech + 1);
        if (!propdef->case_stat)
                    ...
                    Id nfacies)
  {
    ...
    for (igrf = 0; igrf < 2; igrf++)
    {
      ...
      if (iptr < 0)
      {
        ...
        if (iptr < 0)
        {
          messerr("The variable containing the simulation of the GRF %d is "
                  "missing in the Db",
                  igrf + 1);
          goto label_end;
        }
              ...
              const NamingConvention& namconv)
  {
    ...
    if (nbsimu != ngrf && nvar != ngrf)
    {
      ...
      messerr("The input 'db' should have one variable per GRF with locator "
              "'SIMU' or 'Z'");
      goto label_end;
    }
    ...
    if (mode > 0)
    {
      ...
      if (flag_facies)
      {
        ...
        if (!flag_stat)
        {
          ...
          {
            messerr("In the non-stationary case, the number of proportion "
                    "variables (%d)",
                    db_loc->getNLoc(ELoc::P));
            messerr("must be equal to the number of facies (%d) in the "
                    "Lithotype Rule",
                    nfacprod);
            goto label_end;
          }
