...
**
*****************************************************************************/
static void st_pot_env_manage(int mode,
                              int verbose,
                              Pot_Env *pot_env)
{
  /* Dispatch */
...

{
  ...
  for (int iech=0; iech<nech; iech++)
  {
    ...
    if (found < 0)
    {
      layval = (int *) mem_realloc((char *) layval,sizeof(int) * (nlayers+1),1);
      laycnt = (int *) mem_realloc((char *) laycnt,sizeof(int) * (nlayers+1),1);
      layval[nlayers] = ival;
      laycnt[nlayers] = 1;
      nlayers++;
    }
...
label_end:
  layval = (int *) mem_free((char *) layval);
                              ...
                              Pot_Env *pot_env)
{
  ...
  pot_env->rank_grd = (int *) 
    mem_realloc((char *) pot_env->rank_grd,ngrd * sizeof(int),0);
  pot_env->ngrd = ngrd;
...

{
  ...
  pot_env->rank_tgt = (int *) 
    mem_realloc((char *) pot_env->rank_tgt,ntgt * sizeof(int),0);
  pot_env->ntgt = ntgt;
...

{
  ...
  pot_env->nequa = (pot_env->size_iso + 
                    pot_env->size_grd + 
                    pot_env->size_tgt + 
                    pot_env->size_drf + 
                    pot_env->size_ext);
...
**
*****************************************************************************/
static void set_lhs(double *lhs,
                    int     nequa,
                    int     i,
                    int     j,
                    double  value)
...
**
*****************************************************************************/
static double get_lhs(double *lhs,
                      int     nequa,
                      int     i,
                      int     j)
{
...
**
*****************************************************************************/
static int st_extdrift_neigh(Db      *dbgrid,
                             Pot_Ext *pot_ext)
{
  int    ecr,iech;
  double drift;
                            ...
                            double  *extgrd)
{
  ...
  *extval = result[0];
  for (int idim=0; idim<pot_ext->ndim; idim++) extgrd[idim] = result[1+idim];
                        ...
                        double  *lhs)
{
  ...
  for (int it=0; it<pot_env->ntgt; it++) 
  {
    ...
    set_lhs(lhs,nequa,TGT(it),TGT(it),
            matrix_UAV(ndim,covGG,
                       TGT_VAL(it,0),TGT_VAL(it,1),TGT_VAL(it,2),
                       TGT_VAL(it,0),TGT_VAL(it,1),TGT_VAL(it,2)) + nugget_tgt);
  }
  ...
  for (int it=0; it<pot_env->ntgt; it++) 
  {
    ...
    if (pot_env->order >= 2) 
    {
      ...
      set_lhs(lhs,nequa,DRF(6),TGT(it),
              (TGT_COO(it,1) * TGT_VAL(it,0) + 
               TGT_COO(it,0) * TGT_VAL(it,1)));
      set_lhs(lhs,nequa,DRF(7),TGT(it),
              (TGT_COO(it,2) * TGT_VAL(it,0) +
               TGT_COO(it,0) * TGT_VAL(it,2)));
      set_lhs(lhs,nequa,DRF(8),TGT(it),
              (TGT_COO(it,2) * TGT_VAL(it,1) +
  ...
  for (int ic1=0; ic1<pot_env->nlayers; ic1++) 
  {
    for (int j=1; j<pot_env->nb_per_layer[ic1]; j++) 
    {
      ...
      for (iext=0; iext<pot_env->next; iext++)
      {
        if (st_extdrift_eval("Iso-potential",
                             ISO_COO(ic1,j,0),ISO_COO(ic1,j,1),ISO_COO(ic1,j,2),
                             dbout, pot_ext, &extval2, extgrd)) return(1);
        if (st_extdrift_eval("Iso-potential",
                             ISO_COO(ic1,0,0),ISO_COO(ic1,0,1),ISO_COO(ic1,0,2),
                             dbout, pot_ext, &extval1, extgrd)) return(1);
                                    ...
                                    double  *zval)
{
  ...
  for (int i=0; i<nequa * nbsimu; i++) zval[i] = 0.;
                         ...
                         double  *rhs)
{
  ...
  for (int i=0; i<3; i++) ccor[i] = coor[i] - center[i];
                          ...
                          double  *result)
{
  ...
  st_build_rhs(pot_env,pot_ext,flag_grad,flag_part,
               dbiso,dbgrd,dbtgt,dbgrid,model,coor,rhs);
                                  ...
                                  double *result)
{
  ...
  {
    if (result[0] > minval && 
        result[0] <= (POTVAL(isimu,i) - potref)) ilayer = i;
                        ...
                        double  *potval)
{
  ...
  for (int iech=0; iech<dbout->getSampleNumber(); iech++)
  {
    ...
    st_calc_point(pot_env,pot_ext,flag_grad,flag_part,
                  dbiso,dbgrd,dbtgt,dbout,model,zdualk,rhs,dbout,iech,
                  result);
                            ...
                            double  *dist_geo)
{
  ...
  nsol  = 1 + pot_env->ndim;
  icol0 = ISC(ic0,j0);
  for (int idim=0; idim<pot_env->ndim; idim++) deuc[idim] = dgeo[idim] = 0.;
  ...
  st_build_rhs(pot_env,pot_ext,0,0,dbiso,dbgrd,dbtgt,dbiso,model,coor0,rhs);
  ...
  st_build_rhs(pot_env,pot_ext,1,0,dbiso,dbgrd,dbtgt,dbiso,model,coor0,rhs);
  matrix_manage(nequa,nsol,-1,0,&icol0,NULL,rhs,rhs);
  matrix_product(1,neqm1,nsol,zdualk,rhs,result);
  if (debug_query("converge"))
  {
    message("Sample:%2d/%2d Iter:%2d Potential:%lf",j0+1,ic0+1,0,result[0]);
    for (int idim=0; idim<pot_env->ndim; idim++) message(" %lf",coor0[idim]);
    message("\n");
  }
  ...
  for (int iter=0; iter<niter_max; iter++)
  {
    ...
    st_build_rhs(pot_env,pot_ext,1,0,dbiso,dbgrd,dbtgt,dbiso,model,coor,rhs);
    matrix_manage(nequa,nsol,-1,0,&icol0,NULL,rhs,rhs);
    matrix_product(1,neqm1,nsol,zdualk,rhs,result);
    if (debug_query("converge"))
    {
      message("Sample:%2d/%2d Iter:%2d Potential:%lf",
              j0+1,ic0+1,iter,result[0]);
      for (int idim=0; idim<pot_env->ndim; idim++) message(" %lf",coor[idim]);
      message("\n");
    }
  }
                      ...
                      double  *zdualk)
{
  ...
  for (int ic=0; ic<pot_env->nlayers; ic++)
  {
    ...
    {
      iech0 = IAD_ISO(ic,j);
      mes_process("Potential Estimation on Iso-Potential %d of %d",
                  j+1,ic+1);
      debug_index(iech0+1);
      ...
      {
        message("Sample %d/%d (%d): Error=%lf - Variance=%lf",
                j+1,ic+1,iech0+1,result[0],variance);
        if (flag_dist_conv)
                       ...
                       double  *rhs)
{
  ...
  for (int iech=0; iech<dbout->getSampleNumber(); iech++)
  {
    ...
    if (! FFFF(dist_tempere))
    {
      ...
      st_calc_point(pot_env,pot_ext,1,0,dbiso,dbgrd,dbtgt,dbout,
                    model,zdualk,rhs,dbout,iech,resest);
                          ...
                          double  *rhs)
{
  ...
  if (VERBOSE)
    mestitle(0,"Checking Exactness of Information reproduction");
  ...
  if (dbiso != (Db *) NULL)
  {
    ...
    for (int iech=0; iech<dbiso->getSampleNumber(); iech++)
    {
      ...
      if (flag_save)
        st_save_manage(0,"Isopotential",dbiso,iech,result);
  ...
  if (dbgrd != (Db *) NULL)
  {
    ...
    for (int iech=0; iech<dbgrd->getSampleNumber(); iech++)
    {
      ...
      if (flag_save)
        st_save_manage(0,"Gradient",dbgrd,iech,result);
  ...
  if (dbtgt != (Db *) NULL)
  {
    ...
    for (int iech=0; iech<dbtgt->getSampleNumber(); iech++)
    {
      ...
      if (flag_save)
        st_save_manage(0,"Tangent",dbtgt,iech,result);
...
label_end:
  if (error) pot_ext->model = model_free(pot_ext->model);
  return(error);
}
...
**
*****************************************************************************/
static int st_extdrift_create_db(Db      *dbout,
                                 Pot_Ext *pot_ext)
{
  int    error,nech;
  VectorInt nx;
...
**
*****************************************************************************/
static int st_extdrift_calc_init(Db      *dbout,
                                 Pot_Ext *pot_ext)
{
  int     number,error;
  double *a,*b;
                                  ...
                                  int    verbose)
{
  ...
  if (st_build_lhs(&pot_env,&pot_ext,
                   dbiso,dbgrd,dbtgt,dbout,
                   model,nugget_grd,nugget_tgt,lhs)) goto label_end;
  ...
  refpot = st_evaluate_refpot(&pot_env,&pot_ext,dbiso,dbgrd,dbtgt,dbout,model,
                              zdualk,rhs);

  // Check that the information is fulfilled correctly

  st_check_data(&pot_env,&pot_ext,dbiso,dbgrd,dbtgt,dbout,model,
                -1,0,refpot,zdualk,rhs);

  // Get the Potential value at the iso-potential samples

  st_evaluate_potval(&pot_env,&pot_ext,dbiso,dbgrd,dbtgt,dbout,model,refpot,
                     -1,0,zdualk,rhs,potval);
  
  // Perform the estimation on the grid

  st_estimate(&pot_env,&pot_ext,flag_grad,flag_trans,flag_part,
              dbiso,dbgrd,dbtgt,dbout,model,refpot,zdualk,rhs,potval);
                                   ...
                                   int    verbose)
{
  ...
  double *lhs,*zval,*zduals,*zdualk,*rhs,*potsim,*potval,range,delta,refpot;
  Pot_Env pot_env;
  Pot_Ext pot_ext;
  static int nring = 1;
  ...
  if (simtub_potential(dbiso,dbgrd,dbtgt,dbout,model,
                       nbsimu,nbtuba,delta)) goto label_end;
  ...
  if (st_build_lhs(&pot_env,&pot_ext,dbiso,dbgrd,dbtgt,dbout,
                   model,nugget_grd,nugget_tgt,lhs)) goto label_end;
  ...
  if (flag_tempere)
  {
    ...
    st_estimate(&pot_env,&pot_ext,0,0,0,
                dbiso,dbgrd,dbtgt,dbout,model,refpot,zdualk,rhs,potval);
  ...
  for (int isimu=0; isimu<nbsimu; isimu++)
  {
    ...
    st_check_data(&pot_env,&pot_ext,dbiso,dbgrd,dbtgt,dbout,model,
                  isimu,nbsimu,refpot,&ZDUALS(isimu,0),rhs);

    // Calculate the simulated iso-value
    
    st_evaluate_potval(&pot_env,&pot_ext,dbiso,dbgrd,dbtgt,dbout,model,refpot,
                       isimu,nbsimu,&ZDUALS(isimu,0),rhs,&POTSIM(isimu,0));
  }
  
  // Perform the conditional simulations on the grid

  st_simcond(&pot_env,&pot_ext,dist_tempere,flag_trans,nbsimu,
             dbiso,dbgrd,dbtgt,dbout,model,refpot,potsim,
             zdualk,zduals,rhs);
                                 ...
                                 int    verbose)
{
  ...
  st_xvalid(&pot_env,&pot_ext,dbiso,dbgrd,dbtgt,model,lhs,
...

{
  ...
  *n1 = *n2 = 0;
      ...
      *n1 = ndim;
      switch (type2)
      {
        ...
        case -1:                // 2-Gradient-Target
          for (idim=0; idim<ndim; idim++) dd[idim] = x1p[idim] - x2p[idim];
        ...
        case 2:                 // 2-Tangent
          ...
          {
            i = 3 * idim;
            covtab[idim] = matrix_UV(ndim,tx2[0],tx2[1],tx2[2],
                                     covGG[i+0],covGG[i+1],covGG[i+2]);
          }
          *n2 = 1;
          break;
        ...
        case 3:                 // 2-IsoPotential
          ...
          for (idim=0; idim<ndim; idim++) dd[idim] = x1p[idim] - x20[idim];
          st_cov(model,1,dd[0],dd[1],dd[2],&covar,covGp,covGG);
          for (idim=0; idim<ndim; idim++)
      ...
      *n1 = 1;
      switch(type2)
      {
        ...
        case -1:                // 2-Gradient-Target
          ...
          {
            i = 3 *idim;
            covtab[idim] = matrix_UV(ndim,tx1[0],tx1[1],tx1[2],
                                     covGG[i+0],covGG[i+1],covGG[i+2]);
          }
          *n2 = ndim;
          break;
        ...
        case 2:                 // 2-Tangent
          for (idim=0; idim<ndim; idim++) dd[idim] = x2p[idim] - x1p[idim];
          st_cov(model,1,dd[0],dd[1],dd[2],&covar,covGp,covGG);
          covtab[0] = matrix_UAV(ndim,covGG,
                                 tx1[0],tx1[1],tx1[2],tx2[0],tx2[1],tx2[2]);
          *n2 = 1;
          break;
    ...
    case 3:                     // 1-IsoPotential
      *n1 = 1;
      switch(type2)
      {
        case 1:                 // 2-Gradient
        case -1:                 // 2-Gradient-Target
          for (idim=0; idim<ndim; idim++) dd[idim] = x10[idim] - x2p[idim];
          st_cov(model,1,dd[0],dd[1],dd[2],&covar,covGp,covGG);
          for (idim=0; idim<ndim; idim++) dd[idim] = x1p[idim] - x2p[idim];
          st_cov(model,1,dd[0],dd[1],dd[2],&covar,cov2Gp,covGG);
          for (idim=0; idim<ndim; idim++)
        ...
        case 3:                 // 2-IsoPotential
          ...
          for (idim=0; idim<ndim; idim++) dd[idim] = x2p[idim] - x10[idim];
          st_cov(model,0,dd[0],dd[1],dd[2],&covar2,covGp,covGG);
          for (idim=0; idim<ndim; idim++) dd[idim] = x20[idim] - x1p[idim];
          st_cov(model,0,dd[0],dd[1],dd[2],&covar3,covGp,covGG);
          for (idim=0; idim<ndim; idim++) dd[idim] = x20[idim] - x10[idim];
  ...
  if (verbose)
    print_matrix("Covariance",0,1,*n2,*n1,NULL,covtab);
