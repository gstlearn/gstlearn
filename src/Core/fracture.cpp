...

{
  ...
  for (int i=0; i<frac_environ->nfaults; i++)
  {
    ...
    for (int j=0; j<frac_environ->nfamilies; j++)
    {
      mestitle(2,"Fault #%d - Family #%d/%d",
               i+1,j+1,frac_environ->nfamilies);
...
**
*****************************************************************************/
static double st_fault_abscissae(const Frac_Fault& fault,
                                 double      cote)
{
                               ...
                               double *totdens)
{
  ...
  total = 0.;
  for (idisc=0; idisc<NDISC; idisc++) total += denstab[idisc];
                                ...
                                double *denstab)
{
  ...
  for (idisc=0; idisc<NDISC; idisc++) denstab[idisc] = 0.;
...
**
*****************************************************************************/
static int st_get_discretized_rank(double  cumdens,
                                   double *denstab)
{
  double local;
  int    idisc;
                                        ...
                                        int           ifrac0)
{
  ...
  for (int ifrac=0; ifrac<frac_list->nfracs; ifrac++)
  {
    ...
    if (segment_intersect(xd1,yd1,xe1,ye1,xd2,yd2,xe2,ye2,&x,&y)) continue;
...
**
*****************************************************************************/
static void st_update_repulsion(double  x0,
                                double  range,
                                double *denstab)
{
...
**
*****************************************************************************/
static double st_fracture_extension(Frac_Desc& desc,
                                    double     cote,
                                    double     dcote)
{
  ...
  for (i=0; i<desc.npoint-1; i++)
  {
    ...
    disty = YYF(i+1) - YYF(i);
    if (! FFFF(cote) && 
        (YYF(i) < cote - dcote || YYF(i+1) < cote - dcote)) continue;
                                  ...
                                  Frac_List    *frac_list)
{
  ...
  propsur = (ntotal > 0) ? (double) next / (double) ntotal : 0.;
  if (VERBOSE && ntotal > 0)
  {
...
{
  double *thicks_loc;
  double  mean,stdev,var,total,thick,cote_up,cote_down,thick_min,theta,k;
  ...
  {
    thick = (flag_rnd) ? theta * law_gamma(k) : mean;
    ...
    if (number >= nquant * quantum)
    {
      nquant++;
      thicks_loc = (double *) 
        mem_realloc((char *) thicks_loc,sizeof(double) * nquant * quantum,1);
    }
  ...
  thicks_loc = (double *)
    mem_realloc((char *) thicks_loc,sizeof(double) * number,1);
  *y0      = cote_down;
  *nlayers = number;
                                  ...
                                  Frac_List    *frac_list)
{
  ...
  for (i=0; i<ninfos*np1; i++) locinfo[i] = 0.;
  ...
  {
    if (VERBOSE) mestitle(0,"Processing Family #%d/%d",
                          family+1,frac_environ->nfamilies);
    ...
    for (ilayer=0; ilayer<(*nlayers); ilayer++)
    {
      ...
      propsur = st_extend_fractures(frac_environ,family,cote,thick,
                                    denstab,frac_list);
...
label_suite:
  denstab  = (double *) mem_free((char *) denstab);
...

{
  ...
  double coord,thetal,thetar,rangel,ranger,mean,stdev,deltax,deltay,range;
  double orient,dorient,xmax,ymax,theta0,prop1,prop2,aterm,bterm,alpha,ratcst;
  ...
  nfamilies = (flag_def) ? frac_def->nfamilies : 0;
  nfamilies = _lire_int("Number of fracture families",flag_def,nfamilies,1,ITEST);

  mestitle(1,"Scene Geometry");
  xmax      = (flag_def) ? frac_def->xmax : 0.;
  xmax      = _lire_double("Field Extension along X-axis",flag_def,xmax,
                           0.,TEST);
  ymax      = (flag_def) ? frac_def->ymax : 0.;
  ymax      = _lire_double("Field Extension along Y-axis",flag_def,ymax,
                           0.,TEST);
  deltax    = (flag_def) ? frac_def->deltax : 0.;
  deltax    = _lire_double("Field Dilation along X-axis ",flag_def,deltax,
                           0.,TEST);
  deltay    = (flag_def) ? frac_def->deltay : 0.;
  deltay    = _lire_double("Field Dilation along Y-axis ",flag_def,deltay,
                           0.,TEST);
  mean      = (flag_def) ? frac_def->mean : 1.;
  mean      = _lire_double("Mean of thickness law       ",1,mean,
                           0.,TEST);
  stdev     = (flag_def) ? frac_def->stdev : 1.;
  stdev     = _lire_double("St. dev. of thickness law   ",1,stdev,
                           0.,TEST);
  ...
  for (family=0; family<frac_environ->nfamilies; family++)
  {
    ...
    orient    = (flag_def_new) ? fam_def.orient : 0.;
    orient    = _lire_double("Mean Fracture Orientation      ",1,orient,
                             -89.,89.);
    dorient   = (flag_def_new) ? fam_def.dorient : 0.;
    dorient   = _lire_double("Tolerance for Orientation      ",1,dorient,
                             0.,45.);
    theta0    = (flag_def_new) ? fam_def.theta0 : 1.;
    theta0    = _lire_double("Reference Poisson intensity    ",1,theta0,
                             0.,TEST);
    alpha     = (flag_def_new) ? fam_def.alpha : 1.;
    alpha     = _lire_double("Intensity from thick. exponent ",1,alpha,
                             0.,1.);
    ratcst    = (flag_def_new) ? fam_def.ratcst : 0.;
    ratcst    = _lire_double("Intensity Constant/Shaped ratio",flag_def,
                             ratcst,0.,1.);
    prop1     = (flag_def_new) ? fam_def.prop1 : 0.;
    prop1     = _lire_double("Survival constant probability  ",flag_def,prop1,
                             0.,1.);
    prop2     = (flag_def_new) ? fam_def.prop2 : 0.;
    prop2     = _lire_double("Survival length-dependent proba",flag_def,prop2,
                             0.,1.);
    aterm     = (flag_def_new) ? fam_def.aterm : 0.;
    aterm     = _lire_double("Survival cumul. length exponent",flag_def,aterm,
                             0.,TEST);
    bterm     = (flag_def_new) ? fam_def.bterm : 0.;
    bterm     = _lire_double("Survival thickness exponent    ",flag_def,bterm,
                             0.,TEST);
    range     = (flag_def_new) ? fam_def.range : 0.;
    range     = _lire_double("Fracture repulsion area Range  ",flag_def,range,
                             0.,TEST);

    fracture_update_family(frac_environ,family,orient,dorient,
                           theta0,alpha,ratcst,prop1,prop2,aterm,bterm,range);
  }
  ...
  while (flag_new)
  {
    ...
    flag_new = _lire_logical("Add a new Deterministic Fault",
                             flag_def,flag_def_new);
    if (! flag_new) break;

    coord   = (flag_def_new) ? 
      frac_def->frac_faults[frac_environ->nfaults].coord : 0.;
    coord   = _lire_double("Fault Location   ",flag_def_new,coord,0.,xmax);
    orient  = (flag_def_new) ? 
      frac_def->frac_faults[frac_environ->nfaults].orient : 0.;
    ...
    for (family=0; family<frac_environ->nfamilies; family++)
    {
      ...
      thetal = (flag_def_new && family < frac_def->nfamilies) ?
        fault_def.thetal[family] : 0.;
      thetal = _lire_double("Intensity max. value (left)    ",
                            flag_def_new,thetal,0.,TEST);
      rangel = (flag_def_new && family < frac_def->nfamilies) ?
        fault_def.rangel[family] : 0.;
      rangel = _lire_double("Intensity max. distance (left) ",
                            flag_def_new,rangel,0.,TEST);
      thetar = (flag_def_new && family < frac_def->nfamilies) ?
        fault_def.thetar[family] : 0.;
      thetar = _lire_double("Intensity max. value (right)   ",
                            flag_def_new,thetar,0.,TEST);
      ranger = (flag_def_new && family < frac_def->nfamilies) ?
        fault_def.ranger[family] : 0.;
      ranger = _lire_double("Intensity max. distance (right)",
                            flag_def_new,ranger,0.,TEST);
      fracture_update_fault(frac_environ,ifault,family,
                            thetal,thetar,rangel,ranger);
    }
  }
                                 ...
                                 double** fracs_arg)
{
  ...
  frac_segs = (double *) 
    mem_realloc((char *) frac_segs,sizeof(double) * nbyfrac * nfracs,1);
                                       ...
                                       double *frac_segs)
{
  ...
  {
    messerr("The number of values read (%d) should be a multiple of %d",
            nval,nbyfrac);
    return(frac_list);
  }
  nseg = nval / nbyfrac;
...
static void st_plunge_segment_gradual(Db     *dbgrid,
                                      ...
                                      double  range)
{
  double deltax,deltay,dist,incr,incx,incy,total,x1,y1,x2,y2,perm,h,red;
  double coor[2];
  int number,i,iech;
                                  ...
                                  int           ndisc)
{
  ...
  if (perm_bench > 0)
  {
    for (ilayer=0; ilayer<n_layers; ilayer++)
    {
      ...
      st_plunge_segment(dbgrid,iptr,indg,delta,perm_bench,
                        0.,cote,xmax,cote);
    }
  }
...
**
*****************************************************************************/
static void st_traj_add(double *traj,
                        double  x,
                        double  y,
                        int    *ntraj)
{
                                       ...
                                       int           verbose)
{
  ...
  if (! IFFFF(col_perm))
    db_attribute_copy(dbgrid,col_perm,iptr_perm);

  if (flag_fluid)
  {
    iptr_fluid = dbgrid->addFields(1,0.);
    if (! IFFFF(col_fluid))
      db_attribute_copy(dbgrid,col_fluid,iptr_fluid);
  }
  ...
  if (flag_fluid)
  {
    for (int iw=0; iw<nw_xy-1; iw++)
    {
      ...
      x2 = WELLOUT(iw+1,0);
      y2 = WELLOUT(iw+1,1);
      st_plunge_segment(dbgrid,iptr_fluid,indg,delta,val_fluid,x1,y1,x2,y2);
    }
  }
  ...
  for (int iw=0; iw<nw_xy; iw++)
  {
    ...
    if (ifrac >= frac_list->nfracs)
    {
      ...
      messerr("- Fracture number (%d) should lie within [1,%d]",
              ifrac+1,frac_list->nfracs);
      continue;
    }
    ...
    for (int i=0; i<npoint-1; i++)
      st_plunge_segment(dbgrid,iptr_perm,indg,delta,perm2,
                        XXF(i),YYF(i),XXF(i+1),YYF(i+1));
