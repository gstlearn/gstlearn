  ...
  if (code == 1 || code == 5) 
  {
    ...
    if (verbOption >= 2)
      message("Min(%6d) = %11.0f - Max(%6d) = %11.0f\n", 
              minsamp, fminvalue, maxsamp, fmaxvalue);
  }
  ...
  if (code == 2 || code == 3) 
  {
    ...
    if (verbOption >= 2)
      message("Min(%6d) = %11d - Max(%6d) = %11d\n", 
              minsamp, iminvalue, maxsamp, imaxvalue);
  }
                       ...
                       double *czbot)
{
  ...
  *cztop = *czbot = TEST;
  ...
  if (iatt_top >= 0)
    *cztop = surfaces->getArray(rank,iatt_top);

  // Check if the top has been defined

  if (iatt_bot >= 0)
    *czbot = surfaces->getArray(rank,iatt_bot);
...
static void st_grid_from_3refpt(RefPt     refpt[3],
                                ...
                                GridC&    def_grid)
{
  double dx12,dy12,di12,dj12,dx13,dy13,di13,dj13,mat[4],a[2],di10,dj10;
                         ...
                         RefStats& refstats)
{
  int iz1, iz2;
  *nbvalues = 0;
  for (int i = 0; i < nPerTrace; i++) writes[i] = TEST;
  ...
  for (int i = 0; i < nPerTrace; i++)
  {
    ...
    if (! FFFF(dval))
    {
      ...
      if (! FFFF(refstats.modif_scale))
        dval /= refstats.modif_scale;
    }

    // Dispatch

    if (st_vertical_limits(z0, delta, cztop, czbot, cote, option, nz,
                           &iz1, &iz2)) continue;
    ...
    for (int iz = iz1; iz <= iz2; iz++) writes[iz] = dval;
  }
                               ...
                               RefStats& refstats)
{
  ...
  if (iline  < refstats.ilming || IFFFF(refstats.ilming)) refstats.ilming = iline;
  if (iline  > refstats.ilmaxg || IFFFF(refstats.ilmaxg)) refstats.ilmaxg = iline;
  if (xline  < refstats.xlming || IFFFF(refstats.xlming)) refstats.xlming = xline;
  if (xline  > refstats.xlmaxg || IFFFF(refstats.xlmaxg)) refstats.xlmaxg = xline;
  refstats.nbtrace_in++;
}
                              ...
                              double modif_scale)
{
  ...
  while (1)
  {
    ...
    if (st_read_trace(file, code, refstats.nbtrace, nPerTrace, delta, verbOption,
                      values, cotes)) continue;
    ...
    if (st_load_trace(nPerTrace, nz, option, z0, delta, czbot, cztop, cotes, values,
                      writes, &nbvalues, refstats)) continue;
    ...
    for (int iz = 0; iz < nz; iz++) local[iz] = writes[iz];
