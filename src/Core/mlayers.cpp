...

{
  ...
  if (lmlayers->flag_z)
    message("Results are converted into Depth\n");
                               ...
                               double   *props)
{
  ...
  for (ilayer=0; ilayer<lmlayers->nlayers; ilayer++) props[ilayer] = 0.;
  ...
  else
  {
    ...
    for (ilayer=0; ilayer<ilayer0; ilayer++) props[ilayer] = 1.;
  }
  return(0);
}
                             ...
                             double   *props)
{
  ...
  if (st_locate_sample_in_output(lmlayers,dbin,dbout,iech,&igrid)) return(1);
                  ...
                  double   *b)
{
  ...
  for (i=0; i<lmlayers->neq; i++) b[i] = 0.;
                  ...
                  double   *acov)
{
  ...
  for (iech=iiech=0; iech<dbin->getSampleNumber(); iech++)
  {
    ...
    for (ifois=0; ifois<seltab[iech]; ifois++, iiech++)
    {
      ...
      if (st_get_props_data(lmlayers,dbin,dbout,iech,ilayer,prop1)) return(1);
                           ...
                           double   *zval)
{
  ...
  igrid   = 0;
  nlayers = lmlayers->nlayers;
  for (i=0; i<lmlayers->neq; i++) zval[i] = 0.;
                              ...
                              double *stdev)
{
  ...
  *estim = *stdev = TEST;
  ...
  for (int iech=0; iech<nech; iech++) c2[iech] = rhs[iech] + fsf0[iech];
  ...
  *estim = estim1 + estim2;
  ...
  if (flag_std)
  {
    ...
    for (int ipar=0; ipar<npar; ipar++) temp[ipar] -= ff0[ipar];
                        ...
                        double   *post_S)
{
  ...
  for (iechout=0; iechout<dbout->getSampleNumber(); iechout++)
  {
    ...
    if (lmlayers->colrefb >= 0) 
    {
      ...
      if (flag_correc)
      {
        if (st_collocated_prepare(lmlayers,iechout,coor,dbin,dbout,model,seltab,
                                  a,zval,prop1,prop2,covtab,b2,baux,
                                  &ratio)) continue;
      }
    }
    ...
    for (ilayer=0; ilayer<nlayers; ilayer++)
    {
      ...
      if (lmlayers->flag_cumul)
      {
        if (st_get_props_result(lmlayers,dbout,iechout,ilayer+1,
                                prop1)) continue;
      }
      ...
      if (st_rhs(lmlayers,dbin,dbout,model,coor,seltab,iechout,ilayer+1,
                 prop1,prop2,covtab,b)) continue;
      if (debug_query("kriging"))
      ...
      if (flag_std) 
        dbout->setVariable(iechout,nlayers+ilayer,stdv);
                                        ...
                                        int *seltab)
{
  ...
  for (iech=0; iech<dbin->getSampleNumber(); iech++)
  {
    ...
    if (lmlayers->colrefb >= 0)
    {
      ...
      if (ilayer < lmlayers->nlayers &&
          st_get_close_sample(lmlayers,dbin,iech,coor)) newval = 2;
    }
                          ...
                          double *post_S)
{
  ...
  for (int i=0; i<npar2; i++) post_S[i] = invS[i] + invH[i];
  ...
  for (int i=0; i<npar; i++) fm1z[i] += post_mean[i];
  ...
  for (int i=0; i<npar2; i++) invS[i] = post_S[i];
  if (matrix_invert(invS,npar,-1)) goto label_end;
  for (int i=0; i<npar2; i++) gg[i] = invH[i] + invS[i];
  ...
  for (int i=0; i<nech2; i++) cc[i] = acov[i] - cc[i];
  for (int i=0; i<npar2; i++) gs[i] = invH[i] + invS[i];
                                    ...
                                    int     verbose)
{
  ...
  if (is_debug_reference_defined() ||
      debug_query("kriging")) krige_lhs_print(nech,neq,neq,NULL,atot);
  ...
  if (flag_bayes)
  {
    ...
    if (st_drift_bayes(lmlayers,verbose,prior_mean,prior_vars,a,zval,fftab,
                       a0,cc,ss,gs,post_mean,post_S)) goto label_end;
  }
  else
  {
                           ...
                           double *sill)
{
  ...
  for (int i=0; i<nhalf; i++) btab[i] = 0.;
  for (int i=0; i<nhalf * nhalf; i++) atab[i] = 0.;
  for (int i=0; i<nlayers * nlayers; i++) stat[i] = 0;
                           ...
                           Vario       *vario)
{
  ...
  for (ipas=0; ipas<vario->getLagNumber(idir); ipas++)
  {
    ...
    if (debug_query("variogram"))
      print_trimat("C(h)",2,nlayers,sill);
...
label_end:
  phia = (double *) mem_free((char *) phia);
                                  ...
                                  int      verbose)
{
  ...
  {
    if (st_varioexp_chh(lmlayers,verbose,dbin,dbout,vorder,zval,
                        idir,vario)) goto label_end;
  }
                        ...
                        double *vars)
{
  ...
  for (int i=0; i<npar*npar; i++) vars[i] = 0.;
  ...
  for (int iech=0; iech<nech; iech++)
  {
    for (int i=0; i<npar; i++) btab[i] = btab0[i];
    for (int i=0; i<size; i++) atab[i] = atab0[i];
  ...
  for (int ipar=0; ipar<npar; ipar++) mean[ipar] /= nech;
  for (int ipar=0; ipar<npar; ipar++) 
    for (int jpar=0; jpar<npar; jpar++)
                                      ...
                                      double **vars)
{
  ...
  lmlayers = lmlayers_alloc(flag_same,flag_vel,0,flag_ext,1,
                            colrefd,colreft,colrefb,
                            irf_rank,match_time,nlayers);
