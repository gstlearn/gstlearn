...
GEOSLIB_API void time_chunk_add(const char *call_name)
{
  ...
  {
    tchunk = TimeStat[i];
    if (! strncmp(tchunk->call_name,call_name,STORE_NAME_LENGTH-1)) found = i;
  }
...
GEOSLIB_API void time_report(void)
{
  ...
  for (int i=0; i<NB_TIME_CHUNK; i++)
  {
    ...
    message("Time %s : %d s (%d calls)\n",
            tchunk->call_name,tchunk->msec,tchunk->ncalls);
  }
}
  ...
  if (MEMORY_DEBUG > 1 && size > minsize)
  {
    if (oper > 0)
      message("%s (%15s : %5d): +%5d Nbytes - Still allocated (%6d)\n",
              format,call_file,call_line,size,MEMORY_TOTAL);
    else
      message("%s (%15s : %5d): -%5d Nbytes - Still allocated (%6d)\n",
              format,call_file,call_line,size,MEMORY_TOTAL);
  }
