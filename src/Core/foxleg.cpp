                        ...
                        VectorDouble& tabmod2)
{
  ...
  for (ipar=0; ipar<NPAR; ipar++)
  {
    ...
    for (jpar=0; jpar<NPAR; jpar++) param1[jpar] = param2[jpar] = param[jpar];

    param1[ipar] = param[ipar] + epsloc;
    if (! FFFF(upper[ipar]))
      param1[ipar] = MIN(upper[ipar], param1[ipar]);
    param2[ipar] = param[ipar] - epsloc;
    if (! FFFF(lower[ipar]))
      param2[ipar] = MAX(lower[ipar], param2[ipar]);

    ratio1 = ratio2 = epsloc;
    if (! FFFF(upper[ipar])) 
      ratio1 = MIN(epsloc, upper[ipar] - param[ipar]);
    if (! FFFF(lower[ipar])) 
      ratio2 = MIN(epsloc, param[ipar] - lower[ipar]);
...
**
*****************************************************************************/
static void st_determine_gauss(VectorDouble& Jr,
                               VectorDouble& gauss)
{
  int    idat,ipar,jpar;
  double value;
                                          ...
                                          VectorDouble& temp)
{
  ...
  for (ipar2=0; ipar2<NPAR2; ipar2++) flag1[ipar2] = 1;
                     ...
                     const VectorDouble& acont)
{
  ...
  for (icont=0; icont<NCONT; icont++)
  {
    ...
    if (ABS(temp) > 1e-3)
      messageAbort("The constraints are not fulfilled. This should never happen");
  }
  return;
}
                                             ...
                                             const VectorDouble& acont)
{
  ...
  for (iparac=0; iparac<NPARAC; iparac++) hgnadm[iparac] = 0.;
  ...
  while (! sortie && SOUSITER + 1 <= nitermax)
  {
    ...
    if (st_establish_minimization(nactive,ind_util,flag_active,
                                  bords_red,ai_red,grad_red,gauss_red,
                                  &lambda_neg,hgnc,a,b1,temp)) return(1);
    ...
    if (nactaux > 0)
    {
      ...
      matrix_product(NPARAC2,NPARAC,1,ai_red.data(),hgnadm.data(),b1.data());
      ...
      if (lambda_neg >= 0) 
      {
        ...
        if (VERBOSE_GQO && debug_query("converge"))
          message("GQO(%3d) : Gain for feasible case     = %lg\n",
                  SOUSITER,-st_essai(hgnadm,grad_red,gauss_red));
      }
      else
      {
        sortie = 1;
                                  ...
                                  VectorDouble& tabmod1)
{
  ...
  while (1)
  {
    ...
    for (ipar=0; ipar<NPAR && flag_ok; ipar++) 
    {
      ...
      if ((shift < 0 && ABS(BORDS(0,ipar)) < TOLVAL) ||
          (shift > 0 && ABS(BORDS(1,ipar)) < TOLVAL)) shift = 0.;
      ...
      if (shift < BORDS(0,ipar) - TOLVAL ||
          shift > BORDS(1,ipar) + TOLVAL) flag_ok = 0;
    }
    if (! flag_ok) continue;
  ...
  while (flag_cont)
  {
    ...
    if (flag_moved)
    {
      if (st_suppress_unused_constraints(bords,ai,grad,gauss,invhess,hgnc,
                                         ind_util,
                                         bords_red,ai_red,grad_red,gauss_red,
                                         flag_active,flag_actaux,temp)) break;
    }
    else
    {
    ...
    if (st_minimization_under_constraints(ind_util,
                                          bords_red,ai_red,grad_red,
                                          gauss_red,consts,hgnc,hgnadm,
                                          flag_active,flag_actaux,
                                          a,b1,b2,b3,temp,acont))
    {
    ...
    if (ITERATION >= mauto.getMaxiter() || delta < mauto.getEpsdelta()) flag_cont = 0;
    arret = 0.;
