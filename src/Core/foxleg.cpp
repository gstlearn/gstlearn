...
#define AI_RED(ic, iparac, jparac)                                             \
  (ai_red[(jparac) * NPARAC2 + IADAC(ic, iparac)])
...
namespace gstlrn
{
   ...
   **
   *****************************************************************************/
  static double st_essai(
    const VectorDouble& hgnadm,
    const VectorDouble& grad_red,
    MatrixSquare& gauss_red)
  {
    double v1 = hgnadm.innerProduct(grad_red);
   ...
   **
   *****************************************************************************/
  static void st_fill_constraints(
  ...
  {
    st_gradient(
      param, lower, upper, scale, tabwgt, Jr, param1, param2, tabmod1, tabmod2);
    ...
    VectorDouble& temp)
  {
    ...
    for (Id ic = 0; ic < 2; ic++)
      ...
      {
        flag_imposs =
          ((ABS(bords.getValue(ic, ipar)) < EPSILON9)
   ...
   **
   *****************************************************************************/
  static Id st_define_constraints(
    Id mode,
    ...
    VectorDouble& temp)
  {
    ...
    for (Id ic = 0; ic < 2; ic++)
      for (Id iparac = 0; iparac < NPARAC; iparac++, iparac2++)
      {
        consts.setValue(
          ic, iparac,
          (temp[iparac2] - bords_red.getValue(ic, iparac)) * SIGNE(ic));
        if (ABS(consts.getValue(ic, iparac)) < EPSILON9)
          consts.setValue(ic, iparac, 0.);
      }
    ...
    for (Id ic = 0; ic < 2; ic++)
      for (Id iparac = 0; iparac < NPARAC; iparac++, iparac2++)
      {
        switch (mode)
        {
          ...
          case -1: flag_loc = (consts.getValue(ic, iparac) < 0); break;
        }
  ...
  **
  *****************************************************************************/
  static void st_minimum(
    const VectorInt& /*ind_util*/,
    ...
    VectorDouble& hgnadm)
  {
    ...
    if (doit())
    {
      ...
      printVector(hgnadm, "hgnadm avant", true, true);
    }
    ...
    {
      message("alpha_in=%lf\n", alpha_inf);
      printVector(hgnadm, "hgnadm apres", true, true);
    }
  }
   ...
   **
   *****************************************************************************/
  static Id st_suppress_unused_constraints(
    MatrixDense& bords,
    MatrixDense& ai,
    VectorDouble& grad,
    MatrixSquare& gauss,
    ...
    VectorDouble& temp)
  {
    ...
    for (Id ipar2 = 0; ipar2 < NPAR2; ipar2++) flag1[ipar2] = 1;
    ...
    do
    {
      ...
      for (Id ic = 0; ic < 2; ic++)
        for (Id ipar = 0, iparac = 0; ipar < NPAR; ipar++)
        {
          ...
          for (Id jpar = 0, jparac = 0; jpar < NPAR; jpar++)
          {
            if (!POSSIBLE(jpar)) continue;
            ai_red.setValue(
              IADAC(ic, iparac), jparac, ai.getValue(IAD(ic, ipar), jpar));
            jparac++;
          }
          iparac++;
        }
      ...
      if (n_imposs > 0)
      {
        ...
        n_imposs =
          st_possibilities(NPARAC, bords_red, ai_red, hgnc, flag2, temp);
  ...
  **
  *****************************************************************************/
  static Id st_establish_minimization(
    Id nactive,
    ...
    VectorDouble& temp)
  {
    ...
    *lambda_neg = -1;
    ...
    for (iparac = 0; iparac < NPARAC; iparac++) hgnc[iparac] = temp[iparac];
    for (ic = 0; ic < 2; ic++)
      for (iparac = 0; iparac < NPARAC; iparac++)
        if (FLAG_ACTIVE(ic, iparac))
          hgnc[iparac] = bords_red.getValue(ic, iparac);
    ...
    const MatrixDense& acont)
  {
    ...
    for (icont = 0; icont < NCONT; icont++)
    {
      ...
      for (iparac = ipar = 0; ipar < NPAR; ipar++)
        if (POSSIBLE(ipar))
          temp += acont.getValue(ipar, icont) * hgnc[iparac++];
   ...
   **
   *****************************************************************************/
  static Id st_minimization_under_constraints(
    VectorInt& ind_util,
    MatrixDense& bords_red,
    MatrixDense& ai_red,
    VectorDouble& grad_red,
    ...
    const MatrixDense& acont)
  {
    ...
    nactaux = st_define_constraints(
      -1, bords_red, ai_red, hgnc, consts, flag_actaux, temp);
    if (nactaux <= 0)
    {
      for (iparac = 0; iparac < NPARAC; iparac++) hgnadm[iparac] = hgnc[iparac];
    ...
    st_minimum(
      ind_util, flag_actaux, bords_red, VectorDouble(), b1, hgnc, hgnadm);
    st_check(ind_util, hgnadm, acont);

    /* Calculate the constraints vector */

    nactive = st_define_constraints(
      0, bords_red, ai_red, hgnadm, consts, flag_active, temp);
    ...
    while (!sortie && SOUSITER + 1 <= nitermax)
    {
      ...
      if (st_establish_minimization(
            nactive, flag_active, bords_red, ai_red, grad_red, gauss_red,
            &lambda_neg, hgnc, a, b1, temp))
        return (1);
      ...
      if (nactaux > 0)
      {
        ...
        nactive = st_define_constraints(
          1, bords_red, ai_red, hgnadm, consts, flag_active, temp);
        min_adm_cur = st_essai(hgnadm, grad_red, gauss_red);
        if (VERBOSE_GQO && OptDbg::query(EDbg::CONVERGE))
          message(
            "GQO(%3d) : Gain for infeasible case   = %lg\n", SOUSITER,
            -min_adm_cur);
      ...
      else
      {
        ...
        if (lambda_neg >= 0)
        {
          ...
          if (VERBOSE_GQO && OptDbg::query(EDbg::CONVERGE))
            message(
              "GQO(%3d) : Gain for feasible case     = %lg\n", SOUSITER,
  ...
  static void st_constraints_init(VectorInt& ind_util, MatrixDense& ai)
  {
    ...
    for (ipar = 0; ipar < NPCT; ipar++) ind_util[ipar] = 1;
   ...
   *****************************************************************************/
  static void
    st_foxleg_debug_current(double mscur, double delta, VectorDouble& param)
  {
    if (!OptDbg::query(EDbg::CONVERGE)) return;
    printElement(ITERATION);
    printElement(mscur, String(), 1, 1, false, true);
    printElement(delta, String(), 1, 1, true, false);
    for (Id ipar = 0; ipar < NPAR; ipar++) printElement(param[ipar]);
    message("\n");
  }
   ...
   **
   *****************************************************************************/
  static void st_linear_interpolate(
    double mscur,
    VectorDouble& param,
    const MatrixDense& acont,
    const VectorDouble& tabexp,
    ...
    while (1)
    {
      ...
      for (ipar = 0; ipar < NPAR && flag_ok; ipar++)
      {
        ...
        if ((shift < 0 && ABS(bords.getValue(0, ipar)) < EPSILON9)
            || (shift > 0 && ABS(bords.getValue(1, ipar)) < EPSILON9))
          shift = 0.;
    ...
    const VectorDouble& upper)
  {
    ...
    if (!lower.empty() && !upper.empty())
      for (ipar = 0; ipar < NPAR; ipar++)
      {
        ...
        messerr(
          "Error in parameter %d: lower bound (%lf) > upper bound (%lf)",
          ipar + 1, lower[ipar], upper[ipar]);
        return (1);
      }

    /* Check lower bounds vs default values */

    if (!lower.empty() && !param.empty())
      for (ipar = 0; ipar < NPAR; ipar++)
      {
    ...
    VectorDouble& tabwgt)
  {
    ...
    for (Id ipar = 0; ipar < NPAR; ipar++) paramaux[ipar] = param[ipar];
    ...
    double mscur = ms0;
    if (st_calcul0(
          param, lower, upper, scale, acont, tabwgt, residuals, Jr, grad, gauss,
          hgnc, param1, param2, tabmod1, tabmod2))
      return 1;
    ...
    while (flag_cont)
    {
      ...
      if (flag_moved)
      {
        if (st_suppress_unused_constraints(
              bords, ai, grad, gauss, hgnc, ind_util, bords_red, ai_red,
              grad_red, gauss_red, flag_active, flag_actaux, temp))
          break;
      }
      else
      {
      ...
      if (st_minimization_under_constraints(
            ind_util, bords_red, ai_red, grad_red, gauss_red, consts, hgnc,
            hgnadm, flag_active, flag_actaux, a, b1, b2, b3, temp, acont))
      {
      ...
      if (ITERATION >= mauto.getMaxiter() || delta < mauto.getEpsdelta())
        flag_cont = false;
      arret = 0.;
      for (iparac = 0; iparac < NPARAC; iparac++) arret += ABS(hgnc[iparac]);
      ...
      if (denom > 0)
        st_linear_interpolate(
          mscur, param, acont, tabexp, tabwgt, bords, grad, &msaux, paramaux,
          residuals, tabmod1);
      else
        msaux = st_residuals(paramaux, tabexp, tabwgt, tabmod1, residuals);
      ...
      if (msaux < mscur)
      {
        SOUSITER++;
        mscur = msaux;
        for (Id ipar = 0; ipar < NPAR; ipar++) param[ipar] = paramaux[ipar];
        if (st_calcul0(
              param, lower, upper, scale, acont, tabwgt, residuals, Jr, grad,
              gauss, hgnc, param1, param2, tabmod1, tabmod2))
          return 1;
        st_constraints_init(ind_util, ai);
        flag_moved = true;
        if (denom < 0 && rho > 0.75)
