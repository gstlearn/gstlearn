...
namespace gstlrn
{
                             ...
                             VectorDouble& temp)
  {
    ...
    for (Id ic = 0; ic < 2; ic++)
      ...
      {
        flag_imposs = ((ABS(bords.getValue(ic, ipar)) < EPSILON9)
                                  ...
                                  VectorDouble& temp)
  {
    ...
    for (Id ic = 0; ic < 2; ic++)
      for (Id iparac = 0; iparac < NPARAC; iparac++, iparac2++)
      {
        consts.setValue(ic,
                        iparac,
                        (temp[iparac2] - bords_red.getValue(ic, iparac))
                          * SIGNE(ic));
    ...
    do
    {
      ...
      for (Id ic = 0; ic < 2; ic++)
        for (Id ipar = 0, iparac = 0; ipar < NPAR; ipar++)
        {
          ...
          {
            if (!POSSIBLE(jpar)) continue;
            ai_red.setValue(IADAC(ic, iparac),
                            jparac,
                            ai.getValue(IAD(ic, ipar), jpar));
            jparac++;
          }
          iparac++;
        }
  ...
  *****************************************************************************/
                                              ...
                                              const MatrixDense& acont)
  {
    ...
    nactaux = st_define_constraints(-1,
                                    bords_red,
                                    ai_red,
                                    hgnc,
                                    consts,
    ...
    nactive = st_define_constraints(0,
                                    bords_red,
                                    ai_red,
                                    hgnadm,
                                    consts,
    ...
    while (!sortie && SOUSITER + 1 <= nitermax)
    {
      ...
      if (st_establish_minimization(nactive,
                                    flag_active,
                                    bords_red,
                                    ai_red,
                                    grad_red,
      ...
      if (nactaux > 0)
      {
        ...
        nactive = st_define_constraints(1,
                                        bords_red,
                                        ai_red,
                                        hgnadm,
                                        consts,
                           ...
                           const VectorDouble& upper)
  {
    ...
    if (!lower.empty() && !upper.empty())
      for (ipar = 0; ipar < NPAR; ipar++)
      {
        ...
        messerr("Error in parameter %d: lower bound (%lf) > upper bound (%lf)",
                ipar + 1,
                lower[ipar],
                upper[ipar]);
        return (1);
              ...
              VectorDouble& tabwgt)
  {
    ...
    while (flag_cont)
    {
      ...
      if (flag_moved)
      {
        if (st_suppress_unused_constraints(bords,
                                           ai,
                                           grad,
                                           gauss,
                                           hgnc,
      ...
      if (st_minimization_under_constraints(ind_util,
                                            bords_red,
                                            ai_red,
                                            grad_red,
                                            gauss_red,
