...
#define VERBOSE messages > 1
  ...
  static Id pre_init(void), init_point(void), /* routine modified in Dec 2005 */
    recursive_init(void), propagate_point(Id), x_side(Id, Id, Id, Id, Id, Id),
    y_side(Id, Id, Id, Id, Id, Id), z_side(Id, Id, Id, Id, Id, Id),
    scan_x_ff(Id, Id, Id, Id, Id, Id, Id),
    scan_x_fb(Id, Id, Id, Id, Id, Id, Id),
    scan_x_bf(Id, Id, Id, Id, Id, Id, Id),
    scan_x_bb(Id, Id, Id, Id, Id, Id, Id),
    scan_y_ff(Id, Id, Id, Id, Id, Id, Id),
    scan_y_fb(Id, Id, Id, Id, Id, Id, Id),
    scan_y_bf(Id, Id, Id, Id, Id, Id, Id),
    scan_y_bb(Id, Id, Id, Id, Id, Id, Id),
    scan_z_ff(Id, Id, Id, Id, Id, Id, Id),

    scan_z_fb(Id, Id, Id, Id, Id, Id, Id),
    scan_z_bf(Id, Id, Id, Id, Id, Id, Id),
...
#define ERR_BOX (-9)
    ...
    Id MSG)
  {
    ...
    messerr(
      "Args hs,t are seen as arrays[nx][ny][nz], \
i.e. arrays[%d][%d][%d])",
      NX, NY, NZ);
    messerr(
      "Licit src coordinate ranges: xs in [0.,%d] ys in [0.,%d] \
zs in [0.,%d]",
      NX - 1, NY - 1, NZ - 1);
    messerr("Requested source location xs=%g ys=%g zs=%g", XS, YS, ZS);
    messerr("Other Args: EPS_INIT=%g MESSAGES=%d", HS_EPS_INIT, MSG);
    messerr(
      "First elements of input arrays: HS[0][0][0]=%g, \
T[0][0][0]=%g",
      *HS, *T);
    ...
    by = BY;
    bz = BZ;
    if ((bx * by * bz != bx && bx * by * bz != by && bx * by * bz != bz)
        || bx * by * bz <= 0)
    {
      error(ERR_BOX);
      return ERR_BOX;
...

  {
    ...
    if (fxs >= 0.0 && fxs <= nx - 1 && fys >= 0 && fys <= ny - 1 && fzs >= 0
        && fzs <= nz - 1)
      for (x = 0, pf = t_buf; x < nt; x++) *pf++ = T3D_INF;
  ...
  {
    Id signal = ERROR_FREE, x, y, z, xsc, ysc,
       zsc, /* three variables added : patches of December 2005 */
...
Nearest node [%d,%d,%d].",
    ...
    if (init_stage >= INIT_RECURS_LIMIT || ((X0 == 0 || (xs - X0) >= INIT_MIN) && (Y0 == 0 || (ys - Y0) >= INIT_MIN) && (Z0 == 0 || (zs - Z0) >= INIT_MIN) && (X1 == nmesh_x || (X1 - xs) >= INIT_MIN) && (Y1 == nmesh_y || (Y1 - ys) >= INIT_MIN) && (Z1 == nmesh_z || (Z1 - zs) >= INIT_MIN)))
    {
       ...
       */
      if ((X1 - X0) * (Y1 - Y0) * (Z1 - Z0) == 1) /* end 261205[6] */
        init_nearest();
      else
        for (x = X0; x <= X1; x++)
          for (y = Y0; y <= Y1; y++)
...

  {
    ...
    {
      hs3 =
        MIN3(hs[xl - 1][yl][zl], hs[xl][yl - 1][zl], hs[xl - 1][yl - 1][zl]);
      if (hs3 < hsc)
      {
        vv = sqrt(vx * vx + vy * vy);
    ...
    {
      hs3 =
        MIN3(hs[xl][yl - 1][zl], hs[xl][yl][zl - 1], hs[xl][yl - 1][zl - 1]);
      if (hs3 < hsc)
      {
        vv = sqrt(vy * vy + vz * vz);
    ...
    {
      hs3 =
        MIN3(hs[xl][yl][zl - 1], hs[xl - 1][yl][zl], hs[xl - 1][yl][zl - 1]);
      if (hs3 < hsc)
      {
        vv = sqrt(vz * vz + vx * vx);
...

  {
    ...
    for (i = 0; i < N_INIT; i++) HS[i] = T[i] = T3D_INF;
    nx = ny = nz = N_INIT_X;
    ...
    signal = time_3db(
      HS, T, nx, ny, nz, bx, by, bz, fxs, fys, fzs, hs_eps_init, messages);
  ...
  static double
    exact_delay(double vx, double vy, double vz, Id xm, Id ym, Id zm)
...
#define t_3d_part2(x, y, z, a, b, c, d, e) t_3d_(x, y, z, a, b, c, d, e, 1)
  ...
  static Id
    t_3d_part1(Id x, Id y, Id z, double t0, double tl, double tr, double hs0)
...

  {
    ...
    for (y = y_begin; y <= y_end; y++)
    {
      for (z = z_begin; z <= z_end; z++)
      {
        ...
        if (y < y_end && z < z_end)
          updated += t_3d_part1(
            x, y, z, t[x0][y][z], t[x0][y + 1][z], t[x0][y][z + 1], hs_ff);
        if (y > y_begin && z < z_end)
          updated += t_3d_part1(
            x, y, z, t[x0][y][z], t[x0][y - 1][z], t[x0][y][z + 1], hs_bf);
        if (y > y_begin && z > z_begin)
          updated += t_3d_part1(
            x, y, z, t[x0][y][z], t[x0][y - 1][z], t[x0][y][z - 1], hs_bb);
        if (y < y_end && z > z_begin)
          updated += t_3d_part1(
            x, y, z, t[x0][y][z], t[x0][y + 1][z], t[x0][y][z - 1], hs_fb);
        ...
        if (y < y_end && t[x0][y][z] <= t[x0][y + 1][z])
        {
          ...
          updated +=
            t_2d(x, y + 1, z, t[x0][y][z], t[x0][y + 1][z], hs_ff, hs_fb);
        }
        if (y > y_begin && t[x0][y][z] <= t[x0][y - 1][z])
        {
          ...
          updated +=
            t_2d(x, y - 1, z, t[x0][y][z], t[x0][y - 1][z], hs_bf, hs_bb);
        }
        if (z < z_end && t[x0][y][z] <= t[x0][y][z + 1])
        {
          ...
          updated +=
            t_2d(x, y, z + 1, t[x0][y][z], t[x0][y][z + 1], hs_ff, hs_bf);
        }
        if (z > z_begin && t[x0][y][z] <= t[x0][y][z - 1])
        {
          ...
          updated +=
            t_2d(x, y, z - 1, t[x0][y][z], t[x0][y][z - 1], hs_bb, hs_fb);
        }
        ...
        if (sign_ff == 2)
        {
          ...
          updated +=
            edge_diff(x, y + 1, z + 1, t[x0][y][z], t[x0][y + 1][z], hs_ff);
          updated +=
            edge_diff(x, y + 1, z + 1, t[x0][y][z], t[x0][y][z + 1], hs_ff);
          updated += t_3d_part2(
            x, y + 1, z + 1, t[x0][y][z], t[x0][y + 1][z], t[x0][y][z + 1],
            t[x0][y + 1][z + 1], hs_ff);
        }
        if (sign_bf == 2)
        {
          ...
          updated +=
            edge_diff(x, y - 1, z + 1, t[x0][y][z], t[x0][y - 1][z], hs_bf);
          updated +=
            edge_diff(x, y - 1, z + 1, t[x0][y][z], t[x0][y][z + 1], hs_bf);
          updated += t_3d_part2(
            x, y - 1, z + 1, t[x0][y][z], t[x0][y - 1][z], t[x0][y][z + 1],
            t[x0][y - 1][z + 1], hs_bf);
        }
        if (sign_bb == 2)
        {
          ...
          updated +=
            edge_diff(x, y - 1, z - 1, t[x0][y][z], t[x0][y - 1][z], hs_bb);
          updated +=
            edge_diff(x, y - 1, z - 1, t[x0][y][z], t[x0][y][z - 1], hs_bb);
          updated += t_3d_part2(
            x, y - 1, z - 1, t[x0][y][z], t[x0][y - 1][z], t[x0][y][z - 1],
            t[x0][y - 1][z - 1], hs_bb);
        }
        if (sign_fb == 2)
        {
          ...
          updated +=
            edge_diff(x, y + 1, z - 1, t[x0][y][z], t[x0][y + 1][z], hs_fb);
          updated +=
            edge_diff(x, y + 1, z - 1, t[x0][y][z], t[x0][y][z - 1], hs_fb);
          updated += t_3d_part2(
            x, y + 1, z - 1, t[x0][y][z], t[x0][y + 1][z], t[x0][y][z - 1],
            t[x0][y + 1][z - 1], hs_fb);
        }
    ...
    do
    {
      ...
      if (flag_fb)
      {
        ...
        updated +=
          scan_x_fb(y_start_fb, y_end, z_begin, z_start_fb, x0, x, x_s);
      }
      if (flag_bb)
      {
        test++;
        ...
        updated +=
          scan_x_bb(y_begin, y_start_bb, z_begin, z_start_bb, x0, x, x_s);
      }
      if (flag_bf)
      {
        test++;
        ...
        updated +=
          scan_x_bf(y_begin, y_start_bf, z_start_bf, z_end, x0, x, x_s);
      }
    } while (test);
  ...
  static Id
    scan_x_ff(Id y_start, Id y_end, Id z_start, Id z_end, Id x0, Id x, Id x_s)
    ...
    for (y = y_start; y < y_end; y++)
    {
      for (z = z_start; z < z_end; z++)
      {
        ...
        alert0 = edge_diff(x, y + 1, z + 1, t[x0][y][z], t[x][y][z], hs_bb)
               + t_3d(
                   x, y + 1, z + 1, t[x0][y][z], t[x0][y + 1][z], t[x][y][z],
                   t[x][y + 1][z], hs_bb)
               + t_3d(
                   x, y + 1, z + 1, t[x0][y][z], t[x0][y][z + 1], t[x][y][z],
                   t[x][y][z + 1], hs_bb);
        if (alert0)
        {
          updated += alert0;
        ...
        /* interface waves along y_side: 1 1D transmission and 1 2D transmission */
        alert1 =
          t_1d(x, y + 1, z + 1, t[x][y][z + 1], hs_bb, hs_bf, hs_ubb, hs_ube);
        alert0 =
          t_2d(x, y + 1, z + 1, t[x0][y][z + 1], t[x][y][z + 1], hs_bb, hs_bf);
        if (alert0 + alert1)
        {
          updated += alert0 + alert1;
        ...
        /* interface waves along z_side: 1 1D transmission and 1 2D transmission */
        alert1 =
          t_1d(x, y + 1, z + 1, t[x][y + 1][z], hs_bb, hs_fb, hs_ubb, hs_ueb);
        alert0 =
          t_2d(x, y + 1, z + 1, t[x0][y + 1][z], t[x][y + 1][z], hs_bb, hs_fb);
        if (alert0 + alert1)
        {
          updated += alert0 + alert1;
        ...
        alert1 =
          diff_2d(x, y + 1, z + 1, t[x][y][z], hs_bb, hs_ubb)
  ...
  static Id
    scan_x_bf(Id y_begin, Id y_start, Id z_start, Id z_end, Id x0, Id x, Id x_s)

  {
    ...
    for (y = y_start; y > y_begin; y--)
    {
      for (z = z_start; z < z_end; z++)
      {
        ...
        /* interface waves along y_side: 1 1D transmission and 1 2D transmission */
        alert1 =
          t_1d(x, y - 1, z + 1, t[x][y][z + 1], hs_ff, hs_fb, hs_uee, hs_ueb);
        alert0 =
          t_2d(x, y - 1, z + 1, t[x0][y][z + 1], t[x][y][z + 1], hs_ff, hs_fb);
        if (alert0 + alert1)
        {
          updated += alert0 + alert1;
        ...
        /* interface waves along x_side : 2 2D transmission and 1 2D diffraction */
        alert1 =
          diff_2d(x, y - 1, z + 1, t[x][y][z], hs_fb, hs_ueb)
...

  {
    ...
    for (y = y_start; y > y_begin; y--)
    {
      for (z = z_start; z > z_begin; z--)
      {
        ...
        /* interface waves along z_side: 1 1D transmission and 1 2D transmission */
        alert1 =
          t_1d(x, y - 1, z - 1, t[x][y - 1][z], hs_ff, hs_bf, hs_uee, hs_ube);
        alert0 =
          t_2d(x, y - 1, z - 1, t[x0][y - 1][z], t[x][y - 1][z], hs_ff, hs_bf);
        if (alert0 + alert1)
        {
          updated += alert0 + alert1;
          flag_fb++; /* this scan must be (re-)examined */
        ...
        /* interface waves along x_side : 2 2D transmission and 1 2D diffraction */
        alert1 =
          diff_2d(x, y - 1, z - 1, t[x][y][z], hs_ff, hs_uee)
  ...
  static Id
    scan_x_fb(Id y_start, Id y_end, Id z_begin, Id z_start, Id x0, Id x, Id x_s)

  {
    ...
    for (y = y_start; y < y_end; y++)
    {
      for (z = z_start; z > z_begin; z--)
      {
        ...
        /* interface waves along x_side : 2 2D transmission and 1 2D diffraction */
        alert1 =
          diff_2d(x, y + 1, z - 1, t[x][y][z], hs_bf, hs_ube)
...

  {
    ...
    for (x = x_begin; x <= x_end; x++)
    {
      for (z = z_begin; z <= z_end; z++)
      {
        ...
        if (x < x_end && z < z_end)
          updated += t_3d_part1(
            x, y, z, t[x][y0][z], t[x + 1][y0][z], t[x][y0][z + 1], hs_ff);
        if (x > x_begin && z < z_end)
          updated += t_3d_part1(
            x, y, z, t[x][y0][z], t[x - 1][y0][z], t[x][y0][z + 1], hs_bf);
        if (x > x_begin && z > z_begin)
          updated += t_3d_part1(
            x, y, z, t[x][y0][z], t[x - 1][y0][z], t[x][y0][z - 1], hs_bb);
        if (x < x_end && z > z_begin)
          updated += t_3d_part1(
            x, y, z, t[x][y0][z], t[x + 1][y0][z], t[x][y0][z - 1], hs_fb);
        ...
        if (x < x_end && t[x][y0][z] <= t[x + 1][y0][z])
        {
          ...
          updated +=
            t_2d(x + 1, y, z, t[x][y0][z], t[x + 1][y0][z], hs_ff, hs_fb);
        }
        if (x > x_begin && t[x][y0][z] <= t[x - 1][y0][z])
        {
          ...
          updated +=
            t_2d(x - 1, y, z, t[x][y0][z], t[x - 1][y0][z], hs_bf, hs_bb);
        }
        if (z < z_end && t[x][y0][z] <= t[x][y0][z + 1])
        {
          ...
          updated +=
            t_2d(x, y, z + 1, t[x][y0][z], t[x][y0][z + 1], hs_ff, hs_bf);
        }
        if (z > z_begin && t[x][y0][z] <= t[x][y0][z - 1])
        {
          ...
          updated +=
            t_2d(x, y, z - 1, t[x][y0][z], t[x][y0][z - 1], hs_bb, hs_fb);
        }
        ...
        if (sign_bf == 2)
        {
          flag_bf = 1;
          updated +=
            point_diff(x - 1, y, z + 1, t[x][y0][z], hs_bf)
        ...
        if (sign_bb == 2)
        {
          flag_bb = 1;
          updated +=
            point_diff(x - 1, y, z - 1, t[x][y0][z], hs_bb)
        ...
        if (sign_fb == 2)
        {
          flag_fb = 1;
          updated +=
            point_diff(x + 1, y, z - 1, t[x][y0][z], hs_fb)
    ...
    do
    {
      ...
      if (flag_fb)
      {
        ...
        if (VERBOSE) message("fb ");
        updated +=
          scan_y_fb(x_start_fb, x_end, z_begin, z_start_fb, y0, y, y_s);
      }
      if (flag_bb)
      {
        test++;
        if (VERBOSE) message("bb ");
        updated +=
          scan_y_bb(x_begin, x_start_bb, z_begin, z_start_bb, y0, y, y_s);
      }
      if (flag_bf)
      {
        test++;
        if (VERBOSE) message("bf ");
        updated +=
          scan_y_bf(x_begin, x_start_bf, z_start_bf, z_end, y0, y, y_s);
      }
    } while (test);
  ...
  static Id
    scan_y_ff(Id x_start, Id x_end, Id z_start, Id z_end, Id y0, Id y, Id y_s)
    ...
    for (x = x_start; x < x_end; x++)
    {
      for (z = z_start; z < z_end; z++)
      {
        ...
        alert0 = edge_diff(x + 1, y, z + 1, t[x][y0][z], t[x][y][z], hs_bb)
               + t_3d(
                   x + 1, y, z + 1, t[x][y0][z], t[x + 1][y0][z], t[x][y][z],
                   t[x + 1][y][z], hs_bb)
               + t_3d(
                   x + 1, y, z + 1, t[x][y0][z], t[x][y0][z + 1], t[x][y][z],
                   t[x][y][z + 1], hs_bb);
        if (alert0)
        {
          updated++;
        ...
        alert1 =
          t_1d(x + 1, y, z + 1, t[x][y][z + 1], hs_bb, hs_bf, hs_ubb, hs_ube);
        alert0 =
          t_2d(x + 1, y, z + 1, t[x][y0][z + 1], t[x][y][z + 1], hs_bb, hs_bf);
        if (alert0 + alert1)
        {
          updated += alert0 + alert1;
          flag_fb++; /* this scan must be (re-)examined */
        ...
        /* interface waves along z_side: 1 1D transmission and 1 2D transmission */
        alert1 =
          t_1d(x + 1, y, z + 1, t[x + 1][y][z], hs_bb, hs_fb, hs_ubb, hs_ueb);
        alert0 =
          t_2d(x + 1, y, z + 1, t[x + 1][y0][z], t[x + 1][y][z], hs_bb, hs_fb);
        if (alert0 + alert1)
        {
          updated += alert0 + alert1;
          flag_bf++; /* this scan must be (re-)examined */
        ...
        alert1 =
          diff_2d(x + 1, y, z + 1, t[x][y][z], hs_bb, hs_ubb)
  ...
  static Id
    scan_y_bf(Id x_begin, Id x_start, Id z_start, Id z_end, Id y0, Id y, Id y_s)

  {
    ...
    for (x = x_start; x > x_begin; x--)
    {
      for (z = z_start; z < z_end; z++)
      {
        ...
        /* interface waves along x_side: 1 1D transmission and 1 2D transmission */
        alert1 =
          t_1d(x - 1, y, z + 1, t[x][y][z + 1], hs_ff, hs_fb, hs_uee, hs_ueb);
        alert0 =
          t_2d(x - 1, y, z + 1, t[x][y0][z + 1], t[x][y][z + 1], hs_ff, hs_fb);
        if (alert0 + alert1)
        {
          updated += alert0 + alert1;
          flag_bb++; /* this scan must be (re-)examined */
        ...
        /* interface waves along y_side : 2 2D transmission and 1 2D diffraction */
        alert1 =
          diff_2d(x - 1, y, z + 1, t[x][y][z], hs_fb, hs_ueb)
...

  {
    ...
    for (x = x_start; x > x_begin; x--)
    {
      for (z = z_start; z > z_begin; z--)
      {
        ...
        /* interface waves along z_side: 1 1D transmission and 1 2D transmission */
        alert1 =
          t_1d(x - 1, y, z - 1, t[x - 1][y][z], hs_ff, hs_bf, hs_uee, hs_ube);
        alert0 =
          t_2d(x - 1, y, z - 1, t[x - 1][y0][z], t[x - 1][y][z], hs_ff, hs_bf);
        if (alert0 + alert1)
        {
          updated += alert0 + alert1;
          flag_fb++; /* this scan must be (re-)examined */
        ...
        /* interface waves along y_side : 2 2D transmission and 1 2D diffraction */
        alert1 =
          diff_2d(x - 1, y, z - 1, t[x][y][z], hs_ff, hs_uee)
  ...
  static Id
    scan_y_fb(Id x_start, Id x_end, Id z_begin, Id z_start, Id y0, Id y, Id y_s)

  {
    ...
    for (x = x_start; x < x_end; x++)
    {
      for (z = z_start; z > z_begin; z--)
      {
        ...
        /* interface waves along y_side : 2 2D transmission and 1 2D diffraction */
        alert1 =
          diff_2d(x + 1, y, z - 1, t[x][y][z], hs_bf, hs_ube)
...

  {
    ...
    for (x = x_begin; x <= x_end; x++)
    {
      for (y = y_begin; y <= y_end; y++)
      {
        ...
        if (x < x_end && y < y_end)
          updated += t_3d_part1(
            x, y, z, t[x][y][z0], t[x + 1][y][z0], t[x][y + 1][z0], hs_ff);
        if (x > x_begin && y < y_end)
          updated += t_3d_part1(
            x, y, z, t[x][y][z0], t[x - 1][y][z0], t[x][y + 1][z0], hs_bf);
        if (x > x_begin && y > y_begin)
          updated += t_3d_part1(
            x, y, z, t[x][y][z0], t[x - 1][y][z0], t[x][y - 1][z0], hs_bb);
        if (x < x_end && y > y_begin)
          updated += t_3d_part1(
            x, y, z, t[x][y][z0], t[x + 1][y][z0], t[x][y - 1][z0], hs_fb);
        ...
        if (x < x_end && t[x][y][z0] <= t[x + 1][y][z0])
        {
          ...
          updated +=
            t_2d(x + 1, y, z, t[x][y][z0], t[x + 1][y][z0], hs_ff, hs_fb);
        }
        if (x > x_begin && t[x][y][z0] <= t[x - 1][y][z0])
        {
          ...
          updated +=
            t_2d(x - 1, y, z, t[x][y][z0], t[x - 1][y][z0], hs_bf, hs_bb);
        }
        if (y < y_end && t[x][y][z0] <= t[x][y + 1][z0])
        {
          ...
          updated +=
            t_2d(x, y + 1, z, t[x][y][z0], t[x][y + 1][z0], hs_ff, hs_bf);
        }
        if (y > y_begin && t[x][y][z0] <= t[x][y - 1][z0])
        {
          ...
          updated +=
            t_2d(x, y - 1, z, t[x][y][z0], t[x][y - 1][z0], hs_bb, hs_fb);
        }
        ...
        if (sign_bf == 2)
        {
          flag_bf = 1;
          updated +=
            point_diff(x - 1, y + 1, z, t[x][y][z0], hs_bf)
        ...
        if (sign_bb == 2)
        {
          flag_bb = 1;
          updated +=
            point_diff(x - 1, y - 1, z, t[x][y][z0], hs_bb)
        ...
        if (sign_fb == 2)
        {
          flag_fb = 1;
          updated +=
            point_diff(x + 1, y - 1, z, t[x][y][z0], hs_fb)
    ...
    do
    {
      ...
      if (flag_fb)
      {
        ...
        if (VERBOSE) message("fb ");
        updated +=
          scan_z_fb(x_start_fb, x_end, y_begin, y_start_fb, z0, z, z_s);
      }
      if (flag_bb)
      {
        test++;
        if (VERBOSE) message("bb ");
        updated +=
          scan_z_bb(x_begin, x_start_bb, y_begin, y_start_bb, z0, z, z_s);
      }
      if (flag_bf)
      {
        test++;
        if (VERBOSE) message("bf ");
        updated +=
          scan_z_bf(x_begin, x_start_bf, y_start_bf, y_end, z0, z, z_s);
      }
    } while (test);
  ...
  static Id
    scan_z_ff(Id x_start, Id x_end, Id y_start, Id y_end, Id z0, Id z, Id z_s)
    ...
    for (x = x_start; x < x_end; x++)
    {
      for (y = y_start; y < y_end; y++)
      {
        ...
        alert0 = edge_diff(x + 1, y + 1, z, t[x][y][z0], t[x][y][z], hs_bb)
               + t_3d(
                   x + 1, y + 1, z, t[x][y][z0], t[x + 1][y][z0], t[x][y][z],
                   t[x + 1][y][z], hs_bb)
               + t_3d(
                   x + 1, y + 1, z, t[x][y][z0], t[x][y + 1][z0], t[x][y][z],
                   t[x][y + 1][z], hs_bb);
        if (alert0)
        {
          updated += alert0;
        ...
        /* interface waves along x_side: 1 1D transmission and 1 2D transmission */
        alert1 =
          t_1d(x + 1, y + 1, z, t[x][y + 1][z], hs_bb, hs_bf, hs_ubb, hs_ube);
        alert0 =
          t_2d(x + 1, y + 1, z, t[x][y + 1][z0], t[x][y + 1][z], hs_bb, hs_bf);
        if (alert0 + alert1)
        {
          updated += alert0 + alert1;
          flag_fb++; /* this scan must be (re-)examined */
        ...
        /* interface waves along y_side: 1 1D transmission and 1 2D transmission */
        alert1 =
          t_1d(x + 1, y + 1, z, t[x + 1][y][z], hs_bb, hs_fb, hs_ubb, hs_ueb);
        alert0 =
          t_2d(x + 1, y + 1, z, t[x + 1][y][z0], t[x + 1][y][z], hs_bb, hs_fb);
        if (alert0 + alert1)
        {
          updated += alert0 + alert1;
          flag_bf++; /* this scan must be (re-)examined */
        ...
        alert1 =
          diff_2d(x + 1, y + 1, z, t[x][y][z], hs_bb, hs_ubb)
  ...
  static Id
    scan_z_bf(Id x_begin, Id x_start, Id y_start, Id y_end, Id z0, Id z, Id z_s)

  {
    ...
    for (x = x_start; x > x_begin; x--)
    {
      for (y = y_start; y < y_end; y++)
      {
        ...
        /* interface waves along x_side: 1 1D transmission and 1 2D transmission */
        alert1 =
          t_1d(x - 1, y + 1, z, t[x][y + 1][z], hs_ff, hs_fb, hs_uee, hs_ueb);
        alert0 =
          t_2d(x - 1, y + 1, z, t[x][y + 1][z0], t[x][y + 1][z], hs_ff, hs_fb);
        if (alert0 + alert1)
        {
          updated += alert0 + alert1;
          flag_bb++; /* this scan must be (re-)examined */
        ...
        /* interface waves along z_side : 2 2D transmission and 1 2D diffraction */
        alert1 =
          diff_2d(x - 1, y + 1, z, t[x][y][z], hs_fb, hs_ueb)
...

  {
    ...
    for (x = x_start; x > x_begin; x--)
    {
      for (y = y_start; y > y_begin; y--)
      {
        ...
        /* interface waves along y_side: 1 1D transmission and 1 2D transmission */
        alert1 =
          t_1d(x - 1, y - 1, z, t[x - 1][y][z], hs_ff, hs_bf, hs_uee, hs_ube);
        alert0 =
          t_2d(x - 1, y - 1, z, t[x - 1][y][z0], t[x - 1][y][z], hs_ff, hs_bf);
        if (alert0 + alert1)
        {
          updated += alert0 + alert1;
          flag_fb++; /* this scan must be (re-)examined */
        ...
        /* interface waves along z_side : 2 2D transmission and 1 2D diffraction */
        alert1 =
          diff_2d(x - 1, y - 1, z, t[x][y][z], hs_ff, hs_uee)
  ...
  static Id
    scan_z_fb(Id x_start, Id x_end, Id y_begin, Id y_start, Id z0, Id z, Id z_s)

  {
    ...
    for (x = x_start; x < x_end; x++)
    {
      for (y = y_start; y > y_begin; y--)
      {
        ...
        /* interface waves along z_side : 2 2D transmission and 1 2D diffraction */
        alert1 =
          diff_2d(x + 1, y - 1, z, t[x][y][z], hs_bf, hs_ube)
