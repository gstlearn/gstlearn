                                 ...
                                 VectorDouble& codir)
{
  ...
  if (norme <= 0.)
  {
    for (int idim=0; idim<ndim; idim++) codir[idim] = 0.;
    codir[0] = 1.;
  }
  else
  {
    norme = sqrt(norme);
    for (int idim=0; idim<ndim; idim++) codir[idim] /= norme;
  }
}
...
**
*****************************************************************************/
static int st_get_variable_order(int    nvar,
                                 int    ivar0,
                                 int    jvar0)
{
  int rank,ivar,jvar;
...
**
*****************************************************************************/
static void st_manage_drift_removal(int    type,
                                    Db    *db,
                                    Model *model)
{
  int nbfl,nech,i;
...
/****************************************************************************/
/*!
**  Return the maximum distance involved in variogram/direction calculation
**
...
GEOSLIB_API double variogram_maximum_distance(const Dir& dir)
{
  ...
  if (dir.getLagRegular())
    maxdist = dir.getDPas() * (dir.getNPas() + dir.getTolDist());
  else
    maxdist = dir.getBreaks(dir.getNPas());
  return(maxdist);
}
...
**
*****************************************************************************/
static void st_variogram_stats(Db     *db,
                               Vario  *vario)
{
  double z1,z2,ww;
  ...
  for (int ivar = 0; ivar < db->getVariableNumber(); ivar++)
    for (int jvar = 0; jvar <= ivar; jvar++)
    {
      ...
      {
        vario->setVars(ivar,jvar,s12wzz / s12w - (s12wz1 / s12w) * (s12wz2 / s12w));
        vario->setVars(jvar,ivar,s12wzz / s12w - (s12wz1 / s12w) * (s12wz2 / s12w));
      }
    }
      ...
      for (int jvar=0; jvar<db->getVariableNumber(); jvar++)
        if (ivar != jvar)
          vario->setVars(ivar,jvar,
                         ...
                         sqrt(vario->getVars(ivar,ivar) *
                              vario->getVars(jvar,jvar)));
  }
  return;
}
...
**
*****************************************************************************/
static void st_variovect_stats(Db    *db,
                               Vario *vario,
                               int    ncomp)
{
  ...
  for (ivar=0; ivar<vario->getVariableNumber(); ivar++)
    for (jvar=0; jvar<=ivar; jvar++)
    {
      ...
      vario->setVars(ivar,jvar,(s12ww > 0) ? s12wzz / s12ww : 0.);
      vario->setVars(jvar,ivar,(s12ww > 0) ? s12wzz / s12ww : 0.);
    }
                                  ...
                                  double *dist)
{
  ...
  if (dir.getLagRegular())
  {
    ilag = (int) floor((*dist) / dir.getDPas() + 0.5);
    if (ABS((*dist) - ilag * dir.getDPas()) > dir.getTolDist() * dir.getDPas())
      return(ITEST);
  }
  else
  {
    for (k=0, ilag= -1; k<dir.getNPas() && ilag<0; k++)
      if ((*dist) >  dir.getBreaks()[k] &&
          (*dist) <= dir.getBreaks()[k+1]) ilag = k;
  }
  if (ilag < 0 || ilag >= dir.getNPas()) return(ITEST);
                                 ...
                                 int idir)
{
  ...
  for (int ivar=0; ivar<NVAR; ivar++)
    for (int jvar=0; jvar<=ivar; jvar++)
    {
      for (int i=0; i<dir.getLagTotalNumber(); i++,ecr++)
      {
      ...
      case 3:
        ...
        while (diff > tol && nfois < maxiter)
        {
          ...
          for (ipair=ifirst; ipair<ilast; ipair++)
          {
            ...
            sval  = st_s(db,iech,jech);
            gval  = st_g(db,iech,jech);
            value = sval + dir.getGg(ipas);
            wgt   = dir.getGg(0,0,ipas) / value;
            sumt += wgt * gval;
            sumb += 1.;
          }
          vario->setGg(idir, 0,0,ipas,sumt / sumb);
...
**
*****************************************************************************/
static int st_get_relative_sample_rank(Db *db,
                                       int iech0)
{
  int iech,iiech;
...
**
*****************************************************************************/
static double st_get_bias_value(Db  *db,
                                int  nbfl,
                                int  iiech,
                                int  jjech)
{
...
**
*****************************************************************************/
static void st_calculate_bias_global(Db     *db,
                                     VectorDouble d1)
{
...
** \param[in]  vario      Vario structure
** \param[in]  vorder     Vario_Order structure
** \param[in]  flag_model 1 if the underlying Model is provided; 0 otherwise
  ...
  for (iiech=0; iiech<nech-1; iiech++)
  {
    ...
    for (jjech=ideb; jjech<nech; jjech++)
    {
      ...
      if (code_comparable(db,db,iech,jech,
                          dir.getOptionCode(),(int) dir.getTolCode())) continue;

      /* Check if the pair must be kept (Date criterion) */

      if (date_comparable(vario,db,db,iech,jech,dir.getIdate())) continue;
...
**
*****************************************************************************/
static int st_variogram_calcul2(Db    *db,
                                Vario *vario,
                                int    idir,
                                int   *rindex)
{
  ...
  for (iiech=0; iiech<nech; iiech++)
  {
    ...
    for (i=0; i<size; i++)
    {
      if (dir.getSw(i) <= 0) continue;
      sw_sum[i] += w1;
      gg_sum[i] += w1 * dir.getGg(i) / dir.getSw(i);
      hh_sum[i] += w1 * dir.getHh(i) / dir.getSw(i);
    }
  }
...
label_end:
  gg_sum = (double *) mem_free((char *) gg_sum);
...
**
*****************************************************************************/
GEOSLIB_API void vardir_print(Vario *vario,
                              int    idir,
                              int    verbose)
{
...
**
*****************************************************************************/
static int st_estimate_drift_coefficients(Db *db,
                                          int verbose)
                                ...
                                int verbose)
{
  ...
  if (flag_ku)
  {
    if (st_update_variogram_ku(db,vario,vorder,flag_model,
                           ...
                           NamingConvention namconv)
{
  ...
  for (jech1=0; jech1<nech; jech1++)
  {
    ...
    for (jech2=jech1; jech2<nech; jech2++)
    {
      ...
      for (idim=0; idim<ndim; idim++) delta[idim] = -delta[idim];
      if (point_to_grid(dbmap,delta,0,indg0)) continue;
      for (int in=0; in<nbmax; in++)
      {
        iech0 = st_find_neigh_cell(dbmap,indg0,neigh.data(),in,indg1);
                                        ...
                                        const   VectorInt&    grincr)
{
  ...
  dir.init(vario->getDimensionNumber(), npas, dpas, toldis, tolang, vario->getFlagAsym(),
           opt_code, idate, bench, cylrad, tolcode, breaks, codir, grincr);
  vario->addDirs(dir);
  return(0);
}
...
/****************************************************************************/
/*!
**  Initializes the Vario structure
**
** \return  Pointer to the newly allocated Vario structure
**
** \param[in]  calcul_name   Type of calculation
** \param[in]  scale      scaling factor for the transitive covariogram
** \param[in]  dates      Array defining the Date intervals
**                        (Dimension: 2 * ndate)
**
** \remark The following code shows how to create a variogram structure for 
** \remark a 2-D data set containing one variable.
** \remark See documentation of vario_extract to retrieve calculation results
**
*****************************************************************************/
GEOSLIB_API Vario *variogram_init(const std::string& calcul_name,
                                  double  scale,
                                  const VectorDouble& dates)
{
  Vario* vario = new (Vario);
  vario->setCalculName(calcul_name);
  vario->setScale(scale);
  vario->setDates(dates);
...
**
*****************************************************************************/
static void st_final_discretization_grid(Db    *db,
                                         int    iptr)
{
  int    iech,nech;
  double value;
...
**
*****************************************************************************/
static int st_update_discretization_grid(Db    *db,
                                         double x,
                                         double y)
{
  ...
  else
  {
    ...
    for (iech=0; iech<nech-1; iech++)
    {
      ...
      for (jech=iech+1; jech<nech; jech++)
      {
        ...
        if (code_comparable(db1,db1,iech,jech,
                            opt_code,tolcode)) continue;

        /* Check if the pair must be kept */

        dist = distance_intra(db1,iech,jech,NULL);
        if (variogram_reject_pair(db1,iech,jech,dist,psmin,
                                  bench,cylrad,codir,&ps)) continue;
                                       ...
                                       Polygons *polygon)
{
  ...
  for (iech=0; iech<nech; iech++) rank[iech] = iech;
  ...
  for (iech=0; iech<nech-1; iech++)
  {
    ...
    for (jech=ideb; jech<nech; jech++)
    {
      ...
      if (variogram_reject_pair(db,iech,jech,dist,psmin,
                                dir.getBench(),dir.getCylRad(),
                                dir.getCodir(),&ps)) continue;
      if (floor(dist / dir.getDPas() + 0.5) >= dir.getNPas()) continue;
                           ...
                           double *correl)
{
  ...
  *variance = *varres = *correl = TEST;
  for (int i=0; i<size; i++) coeff[i] = TEST;
  ...
  switch (flag_mode)
  {
    case 0:
      ...
      for (i=0; i<ncol; i++) 
      {
        ...
        {
          messerr("The regression requires a valid auxiliary variable (#%d)",i+1);
          goto label_end;
        }
      }
      break;
    ...
    case 1:
      if (nvar != 1 || nfex <= 0)
      {
        ...
        messerr("The Db contains %d variables and %d drift variables",
                nvar,nfex);
        goto label_end;
      }
      break;
  ...
  for (iech=number=0; iech<nech; iech++)
  {
    ...
    switch(flag_mode)
    {
      ...
      case 1:
        ...
        if (flag_one) x[ecr++] = 1.;
        for (i=0; i<nfex; i++) x[ecr++] = db2->getExternalDrift(iech,i);
        break;
      case 2:
        value = st_get_IVAR(db1,iech,0);
  ...
  *varres = prod / number;
  ...
  for (iech=0; iech<nech; iech++)
  {
    ...
    if (db1->isActive(iech))
    {
      ...
      {
        drift = 0.;
        for (i=0; i<size; i++) drift += x[i] * coeff[i];
        nb    += 1.;
        m1    += value;
        m2    += drift;
        v1    += value * value;
...
**
*****************************************************************************/
GEOSLIB_API int vario_get_rank(Vario *vario,
                               int    idir,
                               int    idate)
{
  int ndir,ndate,rank;
...
/****************************************************************************/
/*!
**  Ask the characteristics of the Dir structure
**
** \return  Error returned code
**
** \param[in]  dir    Dir structure
** \param[in]  ndim   Space dimension
**
** \param[out]  flag_regular lag definition mode
** \li                        1 when using regular lags
** \li                        0 when irregular lags are given in 'breaks'
** \param[out]  npas         Number of lags
** \param[out]  npatot       Number of values per variable pair
** \param[out]  opt_code     Code selection option
** \li                       0 : no use of the code selection
** \li                       1 : codes must be close enough
** \li                       2 : codes must be different
** \param[out]  size         Dimension of variogram arrays
** \param[out]  dlag         Lag value
** \param[out]  toldis       tolerance on distance (percentage of the lag)
** \param[out]  tolang       angular tolerance (in degrees)
** \param[out]  bench        Slice bench
** \param[out]  cylrad       Slice radius
** \param[out]  tolcode      Tolerance on the code
** \param[out]  codir        calculation direction
** \param[out]  grincr       direction increment (for grid)
**
                               ...
                               double *grincr)
{
  ...
  *flag_regular = dir.getLagRegular();
  *npas         = dir.getNPas();
  *npatot       = dir.getLagTotalNumber();
  *opt_code     = dir.getOptionCode();
  *size         = dir.getSize();
  *dlag         = dir.getDPas();
  *toldis       = dir.getTolDist();
  *tolang       = dir.getTolAngle();
  *bench        = dir.getBench();
  *cylrad       = dir.getCylRad();
  *tolcode      = dir.getTolCode();
  for (idim=0; idim<ndim; idim++) codir[idim]  = dir.getCodir(idim);
  for (idim=0; idim<ndim; idim++) grincr[idim] = dir.getGrincr(idim);
...
/****************************************************************************/
/*!
**  Ask the dimension of the arrays of the Dir structure
**
                            ...
                            double *tab[2])
{
  ...
  for (i=0; i<size; i++)
  {
    tab[0][i] += coef * (tab1[0][i] * tab2[0][i] + 
                         tab1[1][i] * tab2[1][i]);
    tab[1][i] += coef * (tab1[0][i] * tab2[1][i] - 
                         tab1[1][i] * tab2[0][i]);
  }
}
...
**
*****************************************************************************/
static void st_vmap_blank(int     size,
                          double *tab[2])
{
  int i,ic;
  ...
  for (ix=ecr=0; ix<dims[0]; ix++)
    for (iy=0; iy<dims[1]; iy++)
      for (iz=0; iz<dims[2]; iz++,ecr++)
      {
        ...
        if (z1i1 != NULL) z1i1[0][ecr] = (ind1) ? val1 : 0.;
        if (i2i2 != NULL) i2i2[0][ecr] = (ind2);
        if (z2i2 != NULL) z2i2[0][ecr] = (ind2) ? val2 : 0.;
        if (i1i2 != NULL) i1i2[0][ecr] = (ind1 && ind2);
        if (z1i2 != NULL) z1i2[0][ecr] = (ind1 && ind2) ? val1 : 0.;
        if (z2i1 != NULL) z2i1[0][ecr] = (ind1 && ind2) ? val2 : 0.;
        if (z1z2 != NULL) z1z2[0][ecr] = (ind1 && ind2) ? val1 * val2 : 0.;
      }
...
**
*****************************************************************************/
static int st_complex_array_alloc(int     size,
                                  double *tab[2])
{
  int ic;
                            ...
                            NamingConvention namconv)
{
  ...
  for (i=0; i<ndim; i++) dinv[i] = dims[ndim-i-1];
  if (verbose)
  {
    mestitle(0,"Simulation of a grid using FFT");
    message("Grid dimension:");
    for (idim=0; idim<ndim; idim++) message(" %4d",nxgrid[idim]);
    message("\n");
    message("Variogram Map :");
    for (idim=0; idim<ndim; idim++) message(" %4d",nxmap[idim]);
    message("\n");
    message("Working array :");
    for (idim=0; idim<ndim; idim++) message(" %4d",dinv[idim]);
    message("\n");
  }
...
GEOSLIB_API void vardir_copy(Vario *vario_in,
                             ...
                             int    idir_out)
{
  if (vario_in == (Vario *) NULL) return;
  if (idir_in < 0 || idir_in >= vario_in->getDirectionNumber()) return;
  if (vario_out == (Vario *) NULL) return;
  if (idir_out < 0 || idir_out >= vario_in->getDirectionNumber()) return;

  Dir dir_in  = vario_in->getDirs(idir_in);
  Dir dir_out = vario_out->getDirs(idir_out);
  dir_out.copy(dir_in);
}
                                       ...
                                       VectorDouble& sigma)
{
  ...
  if (flag_normalize)
  {
    ...
    for (int ivar=0; ivar<nvar; ivar++) mean[ivar] = sigma[ivar] = 0.;
                                       ...
                                       double *data2)
{
  ...
  for (int iech=0; iech<nech; iech++)
  {
    ...
    for (int jech=0; jech<iech; jech++)
    {
      ...
      if (code_comparable(db,db,iech,jech,opt_code,(int) tolcode)) continue;
      if (variogram_reject_pair(db,iech,jech,dist,psmin,
                                bench,cylrad,codir,&ps)) continue;
                           ...
                           PCA    *pca)
{
  ...
  if (matrix_invert_copy(pca->getZ2F().data(),nvar,
                         pcaf2z.data())) goto label_end;
                        ...
                        int    verbose)
{
  ...
  if (nvar != pca->getNVar())
  {
    messerr("The number of variables in PCA (%d) does not match the number of Z-variables in the Db (%d)",
            pca->getNVar(),nvar);
    goto label_end;
  }
  mean.resize(nvar);
  ...
  {
    cols.resize(nvar);
    for (ivar=0; ivar<nvar; ivar++) cols[ivar] = iptr + ivar;
...
**
*****************************************************************************/
static double st_linear_interpolate(int     n,
                                    double *x,
                                    double *y,
                                    double  x0)
{
...
GEOSLIB_API void variogram_trans_cut(Vario *vario, int nh, double ycut)
{
  ...
  for (idisc=0; idisc<ndisc; idisc++) ro[idisc] = disc * idisc - 1.;
  ...
  variance = 0.;
  for (ih=1; ih<nh; ih++) variance += fncoeff[ih] * fncoeff[ih];
...
**
*****************************************************************************/
GEOSLIB_API int variogram_y2z(Vario *vario,
                              Anam   *anam,
                              Model  *model)
{
  int     error,idir,ndim;
  ...
  for (idir=0; idir<vario->getDirectionNumber(); idir++)
  {
    ...
    {
      for (int idim=0; idim<ndim; idim++)
        d1[idim] = (ipas+1) * dir.getDPas() * dir.getCodir(idim);
    ...
    for (int iclass=0; iclass<nclass; iclass++)
    {
      if (ncond[iclass] > 0)
        message("Class %2d : V1=%lf V2=%lf\n" ,
                iclass+1,xcond[iclass],ycond[iclass]);
    }
  }
}
