...
#define OUT_TO_MARK(pt_out) *(pt_out - Offset_mark_out)
   ...
   **
   *****************************************************************************/
  static void st_get_coordinates(
    const double* pt_out,
    Id* ix,
    Id* iy,
   ...
   *****************************************************************************/
  static void
    st_dump(bool flagMain, const String& title, double* pt_out, SPIMG* image)
  {
    String STRING;
    ...
    message(
      "Spill'#' Queue'?'(%d) Unknown' '(%d) Out'.'(%d) In'*'(%d) Below'-'(%d) "
      "Heap(%d)\n",
  ...
  {
    for (Id iy = 0; iy < SY; iy++)
      for (Id ix = 0; ix < SX; ix++) BITMAP(image, ix, iy) = 0.;
  }
...

  {
    ...
    while (i < Hsize / 2)
    {
      ...
      is = i;
      if ((il < Hsize)
          && SIGNE * (OUT_TO_IN(Heap[il]) - OUT_TO_IN(Heap[is])) > 0)
        is = il;
      if ((ir < Hsize)
          && SIGNE * (OUT_TO_IN(Heap[ir]) - OUT_TO_IN(Heap[is])) > 0)
        is = ir;
  ...
  static void st_final_stats(double hspill, Id ix0, Id iy0)
  {
    ...
    message(
      "INSIDE:  Topography within [%lf ; %lf] (%d)\n", min_inside, max_inside,
      num_inside);
    message(
      "OUTSIDE: Topography within [%lf ; %lf] (%d)\n", min_outside, max_outside,
      num_outside);
    message(
      "UNKNOWN: Topography within [%lf ; %lf] (%d)\n", min_else, max_else,
      num_else);
    ...
    Id* iy0)
  {
    ...
    if (ind_depth < 0 || ind_depth > dbgrid->getNColumn() || ind_data < 0
        || ind_data > dbgrid->getNColumn())
    {
      messerr(
        "Error in the ranks of the height (%d) and data (%d) variables",
        ind_depth, ind_data);
      return (1);
    }
    ...
    Offset_out_in =
      static_cast<Id>(SPIMG_IN->bitmap.data() - SPIMG_OUT->bitmap.data());
    Offset_mark_out =
      static_cast<Id>(SPIMG_OUT->bitmap.data() - SPIMG_MARK->bitmap.data());
    ...
    while (Hsize > 0)
    {
      ...
      for (k = 0; k < n; k++) (void)st_traite(pt_out, &NBGH(x[k], y[k]));
      st_change(pt_out, SURFACE_INSIDE);
