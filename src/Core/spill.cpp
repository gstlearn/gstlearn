...
**
*****************************************************************************/
static void st_htop_evaluate(SPIMG *data,
                             SPIMG *input)
{
  double value;
  int    ix,iy;
                       ...
                       double  hspill)
{
  ...
  for (ix=0; ix<SX; ix++)
    for (iy=0; iy<SY; iy++)
    {
      ...
      th = (SIGNE > 0) ? IN_OUT(pt_out) - hspill : hspill - IN_OUT(pt_out);
      if (th < 0.)
      {
      ...
      {
        if (*pt_out == SURFACE_INSIDE ||
            *pt_out == SURFACE_OUTSIDE) continue;
...

{
  ...
  while (i < Hsize/2) 
  {
    ...
    is = i;
    if ((il < Hsize) &&	
        SIGNE * (IN_OUT(Heap[il]) - IN_OUT(Heap[is])) > 0) is = il;
    if ((ir < Hsize) && 
        SIGNE * (IN_OUT(Heap[ir]) - IN_OUT(Heap[is])) > 0) is = ir;
    ...
    if (*pt_out == INQUEUE)
    {
      ...
      *pt_out = *pt_vois;
      if (*pt_vois == SURFACE_INSIDE)
      {
        ...
        if (! FFFF(HMAX))
        {
          th = (SIGNE > 0) ? HTOP - value : value - HTOP;
          if (th > HMAX) 
            return(2);
        }
      }
...
**
*****************************************************************************/
static void st_dump(const char *title,
                    double *pt_out,
                    SPIMG  *out)

{
  ...
  {
    message(
      "For the DEBUGGING print, we use the following conventions\n"
                            ...
                            int     *iy0)
{
  ...
  if (ind_depth < 0 || ind_depth > dbgrid->getFieldNumber() ||
      ind_data  < 0 || ind_data  > dbgrid->getFieldNumber())
  {
  ...
  thick = (SIGNE > 0) ? HTOP - hspill :  hspill - HTOP;
