...
**
*****************************************************************************/
static int st_polyset_inside_3D(double  zz,
                                double  zmin,
                                double  zmax)
{
  if (FFFF(zz)) return(1);
  ...
  if (flag_nested)
  {
    ...
    for (int ipol = 0; ipol < polygon->getPolySetNumber(); ipol++)
    {
      ...
      if (st_polyset_inside(xx, yy, polyset.getNVertices(),
                            polyset.getX().data(),
                            polyset.getY().data())) number++;
      if (number % 2 != 0 &&
          st_polyset_inside_3D(zz, polyset.getZmin(),polyset.getZmax())) return (1);
    }
  }
  else
  {
...
label_loop:
  ...
  {
    if (nvert > 0)
      flag_stop = ! _lire_logical("Add another Vertex",1,1);
...
label_end:
  if (error) polygon = polygon_free(polygon);
  return(polygon);
}
...

{
  ...
  for(;;)
  {
    ...
    {
      if (! db->isActive(i)) continue;
      if ((db->getCoordinate(i,0) - x1) * y21 -
          (db->getCoordinate(i,1) - y1) * x21 <= 0.) continue;
