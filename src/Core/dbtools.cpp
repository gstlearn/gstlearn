...
namespace gstlrn
{
    ...
    double* gtab)
  {
    ...
    for (Id iech = 0; iech < db_point->getNSample(); iech++) dtab[iech] = 0.;
   ...
   **
   *****************************************************************************/
  static VectorDouble st_point_init_homogeneous(
    Id number,
    ...
    double beta)
  {
    ...
    VectorDouble extend = coormax - coormin;
   ...
   **
   *****************************************************************************/
  static VectorDouble st_point_init_inhomogeneous(
    Id number,
    DbGrid* dbgrid,
    bool flag_repulsion,
    double range,
    double beta)
  {
    ...
    bool flag_region =
      (ndim == 2 && dbgrid->getNLoc(ELoc::NOSTAT) == (ndim + 1));
        ...
        for (Id jp = 0; jp < ecr && !flag_drop; jp++)
        {
          ...
          else
          {
            ...
            for (Id idim = 0; idim < ndim; idim++)
              radius[idim] = 2.
                           / (radip[idim]
                              + dbgrid->getFromLocator(
                                ELoc::NOSTAT, indjp, idim));
            double angle =
              (angip + dbgrid->getFromLocator(ELoc::NOSTAT, indjp, ndim)) / 2.;
            Tensor tensor(ndim);
    ...
    double* surf)
  {
    ...
    if (FFFF(surf_loc) || st_is_undefined(dbgrid, iptr_grad, knd_loc))
      return (1);
    ...
    VectorDouble& line)
  {
    ...
    if (use_grad)
    {
      if (dbgrid->getNLoc(ELoc::G) != ndim)
      {
        ...
        messerr(
          "the number of gradients should be %d (%d)", ndim,
    ...
    for (Id iech = 0; iech < dbpoint->getNSample(); iech++)
    {
      ...
      for (idim = ecr = 0; idim < ndim; idim++) LINE(nbline, ecr++) = TEST;
  ...
  Db* db_regularize(Db* db, DbGrid* dbgrid, Id flag_center)
  {
    ...
    {
      messerr(
        "This function requires the 'dbgrid' to be defined in 3D or more");
      return dbnew;
    }
    ...
    for (Id icode = 0; icode < ncode; icode++)
      for (iz = 0; iz < nz; iz++)
      {
        ...
        if (ratio <= 0) continue;
        for (Id idim = 0; idim < ndim; idim++) WCOR(iz, icode, idim) /= ratio;
        if (flag_center)
          WCOR(iz, icode, 2) = dbgrid->getX0(2) + (0.5 + iz) * dbgrid->getDX(2);
        for (Id ivar = 0; ivar < nvar; ivar++) WTAB(iz, icode, ivar) /= ratio;
        nech++;
      }
    ...
    dbnew = Db::createFromSamples(
      nech, ELoadBy::SAMPLE, wecr, VectorString(), VectorString(), 0);
    ...
    VectorDouble& data)
  {
    ...
    *nech_ret = 0;
    ...
    ntotal = 1;
    for (Id idim = 0; idim < ndim; idim++) ntotal *= npacks[idim];
    ...
    if (ndim == 1)
    {
      for (Id ixcell = 0; ixcell < dbgrid->getNX(0); ixcell += npacks[0])
      {
        ...
        for (Id i = 0; i < nech; i++) rndval[i] = law_uniform(0., 1.);
        VH::arrangeInPlace(0, ranks, rndval, true, nech);
        for (Id i = 0; i < npcell; i++) retain[nret++] = ranks[i];
      }
    }
    else if (ndim == 2)
    {
    ...
    bool flagAddSampleRank)
  {
    ...
    if (number > 0)
    {
      if (dbgrid == nullptr)
      {
        tab = st_point_init_homogeneous(
          number, locmin, locmax, flag_repulsion, range, beta);
      }
      else
      {
        tab = st_point_init_inhomogeneous(
          number, dbgrid, flag_repulsion, range, beta);
      }
    }
    ...
    db = Db::createFromSamples(
      number, ELoadBy::SAMPLE, tab, VectorString(), VectorString(),
      flagAddSampleRank);
    ...
    const NamingConvention& namconv)
  {
    ...
    VectorVectorDouble props =
      Oc.minimize(facies, splits, propGlob, verbose, niter);
