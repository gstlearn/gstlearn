...
namespace gstlrn
{
          ...
          else
          {
            ...
            for (Id idim = 0; idim < ndim; idim++)
              radius[idim] =
                2.
                / (radip[idim]
                   + dbgrid->getFromLocator(ELoc::NOSTAT, indjp, idim));
            double angle =
                   ...
                   VectorDouble& line)
  {
    ...
    if (use_grad)
    {
      if (dbgrid->getNLoc(ELoc::G) != ndim)
      {
        ...
        messerr("the number of gradients should be %d (%d)",
                ndim,
  ...
  Db* db_regularize(Db* db, DbGrid* dbgrid, Id flag_center)
  {
    ...
    dbnew = Db::createFromSamples(nech,
                                  ELoadBy::SAMPLE,
                                  wecr,
                                  VectorString(),
                                  VectorString(),
                    ...
                    bool flagAddSampleRank)
  {
    ...
    if (number > 0)
    {
      if (dbgrid == nullptr)
      {
        tab = st_point_init_homogeneous(number,
                                        locmin,
                                        locmax,
                                        flag_repulsion,
                                        range,
      ...
      {
        tab = st_point_init_inhomogeneous(number,
                                          dbgrid,
                                          flag_repulsion,
                                          range,
                                          beta);
                            ...
                            const NamingConvention& namconv)
  {
    ...
    for (Id i = 0; i < ncat; i++)
    {
      ...
      if (i == 0) iptr0 = iptr;
      namconv.setNamesAndLocators(nullptr,
                                  VectorString(),
                                  ELoc::UNDEFINED,
                                  -1,
                                  dbout,
