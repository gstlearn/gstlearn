...
namespace gstlrn
{
  ...
  static Id st_parid_alloc(StrMod* strmod, Id npar0)
  {
    ...
    for (Id imod = 0; imod < strmod->nmodel; imod++)
    {
      ...
      {
        model_cova_characteristics(model->getCovType(jcov),
                                   cov_name,
                                   &flag_range,
                                   &flag_param,
                                   &min_order,
           ...
           ilag++, ipadir++)
      {
        ...
        for (Id ivar = 0; ivar < nvar; ivar++)
          for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
          {
            ...
            if (lec > sizein)
            {
              messerr("st_compress_array: lec (%d) exceeds maximum (%d)",
                      lec,
                      sizein);
              continue;
            }
            ...
            if (ecr > sizeout)
            {
              messerr("st_compress_array: ecr (%d) exceeds maximum (%d)",
                      ecr,
                      sizeout);
              continue;
            }
                                   ...
                                   VectorDouble& scale)
  {
    ...
    for (Id ntot = 0; ntot < npar; ntot++)
    {
      st_parid_decode(strmod->parid[ntot], &imod, &icov, &icons, &ivar, &jvar);
      Model* model = strmod->models[imod];
      model_cova_characteristics(model->getCovType(icov),
                                 cov_name,
                                 &flag_range,
                                 &flag_param,
                                 &min_order,
                                              ...
                                              VectorDouble& upper)
  {
    ...
    for (Id ipar = 0; ipar < npar; ipar++)
    {
      ...
      param_loc = constraints_get(constraints,
                                  EConsType::DEFAULT,
                                  imod,
                                  icov,
                                  icons,
                                  ivar,
                                  jvar);
      lower_loc = constraints_get(constraints,
                                  EConsType::LOWER,
                                  imod,
                                  icov,
                                  icons,
                                  ivar,
                                  jvar);
      upper_loc = constraints_get(constraints,
                                  EConsType::UPPER,
                                  imod,
                                  icov,
                                  icons,
                                   ...
                                   VectorDouble& upper)
  {
    ...
    for (Id ntot = 0; ntot < npar; ntot++)
    {
      ...
      model_cova_characteristics(type,
                                 cov_name,
                                 &flag_range,
                                 &flag_param,
                                 &min_order,
...

  {
    ...
    else
    {
      for (Id ivar = 0; ivar < nvar; ivar++)
        for (Id jvar = 0; jvar <= ivar; jvar++)
          matcoru[icov0].setValue(ivar,
                                  jvar,
                                  matcor[icov0].getValue(ivar, jvar));
    }
          ...
          Birk_v.setValue(k,
                          ...
                          Birk_v.getValue(k, ivar)
                            + alpha[icov].getValue(ivar0, ivar)
                                * (ge[icov].getValue(0, k)
                                   - value * ge[icov0].getValue(0, k))
                                / Nir_v[ivar]);
        }
      }
                                    ...
                                    std::vector<MatrixSymmetric>& matcor)
  {
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
      for (Id jvar = 0; jvar <= ivar; jvar++)
        matcor[icov0].setValue(ivar,
                               jvar,
                               matcor[icov0].getValue(ivar, jvar) * norme1[ivar]
                                 * norme1[jvar]);
    ...
    for (iter = 0; iter < mauto.getMaxiter(); iter++)
    {
      for (Id icov0 = 0; icov0 < ncova; icov0++)
      {
        for (Id ivar0 = 0; ivar0 < nvar; ivar0++)
        {
          ...
          st_updateCurrentSillGoulard(icov0,
                                      ivar0,
                                      npadir,
                                      nvar,
                                      ncova,
                                        ...
                                        std::vector<MatrixSymmetric>& sill)
  {
    ...
    if (!flag_positive)
    {
      ...
      (void)st_optimize_under_constraints(nvar,
                                          ncova,
                                          npadir,
                                          consSill,
                                          mauto,
                                      ...
                                      std::vector<MatrixSymmetric>& sill1)
  {
    ...
    for (Id iter = 0; iter < mauto.getMaxiter(); iter++)
    {
      ...
      if (st_goulard_without_constraint(mauto_new,
                                        nvar,
                                        1,
                                        npadir,
                                        wt,
                               ...
                               const Option_AutoFit& mauto)
  {
    ...
    if (!optvar.getFlagIntrinsic())
    {
      ...
      if (FFFF(constraints.getConstantSillValue()))
      {
        ...
        status = st_goulard_without_constraint(mauto,
                                               nvar,
                                               ncova,
                                               RECINT.npadir,
                                               RECINT.wt,
      ...
      else
      {
        ...
        status = st_goulard_with_constraints(constraints.getConstantSills(),
                                             mauto,
                                             model->getNVar(),
                                             model->getNCov(),
                                             RECINT.npadir,
      ...
      st_goulard_sill_to_model(model->getNVar(),
                               model->getNCov(),
                               RECINT.sill,
                               model);
    }
    ...
    {
      status = st_sill_fitting_intrinsic(model,
                                         mauto,
                                         RECINT.npadir,
                                         RECINT.wt,
                                         RECINT.gg,
                                        ...
                                        Option_AutoFit& mauto)
  {
    ...
    for (imod = 0; imod < strmod->nmodel; imod++)
    {
      ...
      {
        FLAG_COMPRESS(imod, icov) = st_structure_reduce(strmod,
                                                        imod,
                                                        icov,
                                                        hmax,
                                                        gmax,
        ...
        if (FLAG_COMPRESS(imod, icov))
        {
          ...
          if (optvar.getLockSamerot())
          {
            ...
            {
              lost_rank = rank;
              st_parid_decode(strmod->parid[lost_rank],
                              &lost_imod,
                              &lost_icov,
                              &icons,
                              &ivar,
    ...
    if (ncovleft <= 0)
    {
      message("Due to the tolerance (%lf (percent)), no basic structure would "
              "be left\n",
              mauto.getTolsigma());
  ...
  label_compress:
    ...
    for (imod = 0; imod < strmod->nmodel; imod++)
    {
      ...
      for (icov = 0; icov < strmod->models[imod]->getNCov(); icov++)
      {
        ...
        for (ntot = 0; ntot < nparloc; ntot++)
        {
          st_parid_decode(strmod->parid[ntot],
                          &jmod,
                          &kcov,
                          &icons,
                          &ivar,
  ...
  static Id st_model_define(Model* model, const Option_VarioFit& optvar)
  {
    ...
    for (Id jcov = 0; jcov < model->getNCov(); jcov++)
    {
      ...
      model_cova_characteristics(cova->getType(),
                                 cov_name,
                                 &flag_range,
                                 &flag_param,
                                 &min_order,
                                ...
                                VectorDouble& upper)
  {
    ...
    for (Id imod = 0; imod < 2; imod++)
    {
      ...
      for (Id jcov = 0; jcov < model->getNCov(); jcov++)
      {
        ...
        {
          messerr("The structure '%s' is limited to dimension (%d)",
                  cov_name,
                  max_ndim);
                    ...
                    const Option_VarioFit& optvar_arg)
  {
    ...
    npar0 = st_model_auto_count(vario,
                                model,
                                nullptr,
                                constraints,
                                optvar,
    ...
    strmod = st_model_auto_strmod_alloc(model,
                                        NULL,
                                        npar0,
                                        norder,
                                        hmax,
    ...
    st_model_auto_constraints_apply(strmod,
                                    npar,
                                    constraints,
                                    param,
                                    lower,
    ...
    do
    {
      ...
      st_model_auto_strmod_print(0,
                                 ...
                                 npar,
                                 nbexp);
      flag_reduce = st_model_auto_strmod_reduce(strmod,
                                                &npar,
                                                hmax,
                                                gmax,
                                                param,
                   ...
                   const Option_VarioFit& optvar_arg)
  {
    ...
    npar0 = st_vmap_auto_count(dbmap,
                               model,
                               constraints,
                               optvar,
                               param,
