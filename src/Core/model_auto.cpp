...
/*! \cond */
#define TAKE_ROT                                                               \
  ((optvar.getLockSamerot() && first_covrot < 0) || !optvar.getLockSamerot())
...
#define DD(idim, ijvar, ipadir)                                                \
  dd[IJDIR(ijvar, ipadir) + (idim) * nvs2 * npadir]
...
#define CORRECT(idir, k)                                                       \
  (!isZero(vario->getHhByIndex(idir, k))                                       \
   ...
   && !FFFF(vario->getSwByIndex(idir, k))                                      \
   && !FFFF(vario->getGgByIndex(idir, k)))
#define INCORRECT(idir, k)                                                     \
  (isZero(vario->getHhByIndex(idir, k)) || FFFF(vario->getHhByIndex(idir, k))  \
   || isZero(vario->getSwByIndex(idir, k))                                     \
   || FFFF(vario->getSwByIndex(idir, k))                                       \
   || FFFF(vario->getGgByIndex(idir, k)))
...
namespace gstlrn
{
  ...
  {
    const auto* covanam =
      dynamic_cast<const CovLMCAnamorphosis*>(model->getCovAnisoList());
    if (covanam != nullptr)
    {
   ...
   *****************************************************************************/
  static Id
    st_parid_encode(Id imod, Id icov, const EConsElem& icons, Id ivar, Id jvar)
  {
    Id value = imod;
    ...
    for (Id imod = 0; imod < strmod->nmodel; imod++)
    {
      ...
      for (Id jcov = 0; jcov < model->getNCov(); jcov++)
      {
        model_cova_characteristics(
          model->getCovType(jcov), cov_name, &flag_range, &flag_param,
          &min_order, &max_ndim, &flag_int_1d, &flag_int_2d, &flag_aniso,
          &flag_rotation, &scalfac, &parmax);
        ...
        if (DEFINE_AIC)
          for (Id ivar = 0; ivar < nvar; ivar++)
            for (Id jvar = 0; jvar <= ivar; jvar++)
              strmod->parid[ntot++] =
                st_parid_encode(imod, jcov, EConsElem::SILL, ivar, jvar);

        /* Third parameter */
        if (DEFINE_THIRD)
        {
          strmod->parid[ntot++] =
            st_parid_encode(imod, jcov, EConsElem::PARAM, 0, 0);
        }
        ...
        {
          strmod->parid[ntot++] =
            st_parid_encode(imod, jcov, EConsElem::RANGE, 0, 0);
        }
        ...
        if (DEFINE_ANICOEF)
        {
          ...
          {
            strmod->parid[ntot++] =
              st_parid_encode(imod, jcov, EConsElem::RANGE, 1, 0);
          }
          else if (ndim == 3)
          {
            ...
            {
              strmod->parid[ntot++] =
                st_parid_encode(imod, jcov, EConsElem::RANGE, 2, 0);
            }
          }
          else
          {
            ...
            {
              strmod->parid[ntot++] =
                st_parid_encode(imod, jcov, EConsElem::RANGE, idim, 0);
            }
          }
        }
        ...
        if (DEFINE_ANIROT)
        {
          if ((ndim == 2) || (ndim == 3 && optvar.getLockRot2d()))
          {
            if (TAKE_ROT)
            {
              first_covrot = jcov;
              strmod->parid[ntot++] =
                st_parid_encode(imod, jcov, EConsElem::ANGLE, 0, 0);
            }
          }
          else
          {
            if (TAKE_ROT)
            {
              ...
              {
                strmod->parid[ntot++] =
                  st_parid_encode(imod, jcov, EConsElem::ANGLE, idim, 0);
              }
            }
          }
        }
        ...
        {
          strmod->parid[ntot++] =
            st_parid_encode(imod, 0, EConsElem::T_RANGE, 0, 0);
        }
      }
    }
    return (ntot);
   ...
   **
   *****************************************************************************/
  static StrMod* st_model_auto_strmod_alloc(
    Model* model1,
    Model* model2,
    Id npar0,
    Id norder,
   ...
   **
   *****************************************************************************/
  static Id st_get_vmap_dimension(
    const Db* dbmap,
    Id nvar,
    Id* nbexp_ret,
    Id* npadir_ret)
   ...
   *****************************************************************************/
  static void
    st_mauto_rescale(Id nvar, VectorDouble& varchol, Option_AutoFit& mauto)
  {
    ...
    VectorDouble& tabout)
  {
    ...
    for (Id idir = 0, ndir = vario->getNDir(); idir < ndir; idir++)
      for (Id ilag = 0, nlag = vario->getNLag(idir); ilag < nlag;
           ilag++, ipadir++)
      {
        Id ijvar = 0;
        for (Id ivar = 0; ivar < nvar; ivar++)
          for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
          {
            ...
            {
              messerr(
                "st_compress_array: lec (%d) exceeds maximum (%d)", lec,
                sizein);
              continue;
            }
            ...
            {
              messerr(
                "st_compress_array: ecr (%d) exceeds maximum (%d)", ecr,
                sizeout);
              continue;
            }
            tabout[ecr++] = value;
...

  {
    ...
    CovCalcMode mode(
      ECalcMember::RHS); // to allow selecting individual structures
   ...
   *****************************************************************************/
  static void
    st_load_wt(const Vario* vario, Id wmode, Id npadir, VectorDouble& wt)
  {
    Id ipadir;
        ...
        for (Id idir = 0; idir < ndir; idir++)
        {
               ...
               ilag++, ipadir++)
          {
            ...
            for (Id ijvar = 0; ijvar < nvs2; ijvar++)
            {
              ...
              {
                Id iad = shift + ilag;
                if (CORRECT(idir, iad)) WT(ijvar, ipadir) = flag[idir];
              }
            }
          }
        }
        ...
        for (Id idir = 0; idir < ndir; idir++)
        {
               ...
               ilag++, ipadir++)
          {
            ...
            for (Id ijvar = 0; ijvar < nvs2; ijvar++)
            {
              ...
              {
                Id iad = shift + ilag;
                if (CORRECT(idir, iad)) WT(ijvar, ipadir) = 1.;
              }
            }
          }
        }
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
      ...
      {
        double ratio =
          (vario->getVar(ivar, ivar) > 0 && vario->getVar(jvar, jvar) > 0)
            ? sqrt(vario->getVar(ivar, ivar) * vario->getVar(jvar, jvar))
            : 1.;
        ipadir = 0;
        for (Id idir = 0; idir < ndir; idir++)
        {
  ...
  static void st_goulard_debug_title(Id nvar, Id ncova)
  {
    ...
    message(
      "(Sti(V1-V2) : Sill for structure 'i' for variables 'V1' and 'V2'\n");
    ...
    for (Id icov = 0; icov < ncova; icov++)
      for (Id ivar = 0; ivar < nvar; ivar++)
        ...
        {
          (void)
            gslSPrintf(loc_string, "St%d(%d-%d)", icov + 1, ivar + 1, jvar + 1);
          printElement(loc_string);
        }
    message("\n");
  }
   ...
   **
   *****************************************************************************/
  static void st_keypair_results(
    Id mode,
    Id icov,
    Id nvar,
    double* valpro,
   ...
   *****************************************************************************/
  static void
    st_sill_reset(Id nvar, Id ncova, std::vector<MatrixSymmetric>& sill)
  {
    for (Id icov = 0; icov < ncova; icov++)
      for (Id ivar = 0; ivar < nvar; ivar++)
        for (Id jvar = 0; jvar <= ivar; jvar++)
   ...
   **
   *****************************************************************************/
  static Id st_goulard_without_constraint(
    ...
    for (Id icov = 0; icov < ncova; icov++)
    {
      ...
      for (Id ivar = 0; ivar < nvar; ivar++)
        for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
        {
          ...
          aicLocal[icov].setValue(
            ivar, jvar, sum1 * alphak[icov].getValue(ivar, jvar));
        }
    }
      ...
      for (Id icov = 0; icov < ncova; icov++)
      {
        ...
        for (Id ivar = 0; ivar < nvar; ivar++)
        {
          for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
          {
            ...
            value = aicLocal[icov].getValue(ivar, jvar)
          ...
          for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
          {
            ...
            else
            {
              ...
              for (kvar = 0; kvar < nvar; kvar++)
                sum +=
                  (MAX(valpro[kvar], 0.) * vecpro->getValue(ivar, kvar)
   ...
   **
   *****************************************************************************/
  static void st_model_auto_strmod_print(
    Id flag_title,
    StrMod* strmod,
    ...
    Id nbexp)
  {
    ...
    for (Id ntot = 0; ntot < npar; ntot++)
    {
      ...
      switch (icons.toEnum())
      {
        ...
        default: messerr("Unknown constraint!\n"); break;
      }
    }
  }
   ...
   **
   *****************************************************************************/
  static void st_model_auto_scldef(
    StrMod* strmod,
    Id npar,
    double hmax,
    ...
    VectorDouble& scale)
  {
    ...
    for (Id ntot = 0; ntot < npar; ntot++)
    {
      ...
      Model* model = strmod->models[imod];
      model_cova_characteristics(
        model->getCovType(icov), cov_name, &flag_range, &flag_param, &min_order,
        &max_ndim, &flag_int_1d, &flag_int_2d, &flag_aniso, &flag_rotation,
        &scalfac, &parmax);
      switch (icons.toEnum())
      {
        case EConsElem::E_SILL:
   ...
   **
   *****************************************************************************/
  static void st_model_auto_constraints_apply(
    StrMod* strmod,
    Id npar,
    const Constraints& constraints,
    VectorDouble& param,
    ...
    VectorDouble& upper)
  {
    ...
    for (Id ipar = 0; ipar < npar; ipar++)
    {
      ...
      param_loc = constraints_get(
        constraints, EConsType::DEFAULT, imod, icov, icons, ivar, jvar);
      lower_loc = constraints_get(
        constraints, EConsType::LOWER, imod, icov, icons, ivar, jvar);
      upper_loc = constraints_get(
        constraints, EConsType::UPPER, imod, icov, icons, ivar, jvar);
   ...
   **
   *****************************************************************************/
  static void st_model_auto_pardef(
    StrMod* strmod,
    Id npar,
    double hmax,
    VectorDouble& varchol,
    ...
    VectorDouble& upper)
  {
    ...
    for (Id ntot = 0; ntot < npar; ntot++)
    {
      ...
      model_cova_characteristics(
        type, cov_name, &flag_range, &flag_param, &min_order, &max_ndim,
        &flag_int_1d, &flag_int_2d, &flag_aniso, &flag_rotation, &scalfac,
        &parmax);
   ...
   **
   *****************************************************************************/
  static void st_model_auto_strmod_define(
    StrMod* strmod,
    Id npar,
   ...
   **
   *****************************************************************************/
  static Id st_truncate_negative_eigen(
    Id nvar,
    Id icov0,
...

  {
    ...
    else
    {
      for (Id ivar = 0; ivar < nvar; ivar++)
        for (Id jvar = 0; jvar <= ivar; jvar++)
          matcoru[icov0].setValue(
            ivar, jvar, matcor[icov0].getValue(ivar, jvar));
    }
   ...
   *****************************************************************************/
  static double
    st_sum_sills(Id ivar0, Id ncova, std::vector<MatrixSymmetric>& alpha)
  {
    double Sr = 0;
    for (Id icov = 0; icov < ncova; icov++)
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
      for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
      {
        ...
        for (Id ipadir = 0; ipadir < npadir; ipadir++)
        {
          ...
          if (FFFF(dd)) continue;
          for (Id icov = 0; icov < ncova; icov++)
            dd -= matcor[icov].getValue(ivar, jvar)
                * ge[icov].getValue(ijvar, ipadir);
    ...
    const VectorDouble& consSill)
  {
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
    {
      ...
      for (Id k = 0; k < npadir; k++)
        Nir_v[ivar] +=
          WT(irl, k) * ge[icov0].getValue(0, k) * ge[icov0].getValue(0, k);
    }
    ...
    for (Id k = 0; k < npadir; k++)
    {
      ...
      for (Id jcov = 0; jcov < ncova; jcov++)
      {
        ...
        Mrr_v[k] += alpha[jcov].getValue(ivar0, ivar0)
                  * (ge[jcov].getValue(0, k) - ge[icov0].getValue(0, k));
      }
    }
    ...
    for (Id k = 0; k < npadir; k++)
      for (Id ivar = 0; ivar < nvar; ivar++)
      {
        ...
        for (Id icov = 0; icov < ncova; icov++)
        {
          ...
          for (Id l = 0; l < npadir; l++)
            value +=
              WT(irl, l) * ge[icov].getValue(0, l) * ge[icov].getValue(0, l);
          Birk_v.setValue(
            k, ivar,
            Birk_v.getValue(k, ivar)
              + alpha[icov].getValue(ivar0, ivar)
                  * (ge[icov].getValue(0, k) - value * ge[icov0].getValue(0, k))
                  / Nir_v[ivar]);
        }
      }
    ...
    a = 0.;
    for (Id k = 0; k < npadir; k++) a += WT(irr, k) * Mrr_v[k] * Mrr_v[k];
  ...
  {
    double srm =
      st_sum_sills(ivar0, ncova, alpha) - alpha[icov0].getValue(ivar0, ivar0);
   ...
   **
   ******************************************************************************/
  static void st_updateOtherSills(
    Id icov0,
    Id ivar0,
    Id ncova,
    Id nvar,
   ...
   **
   *****************************************************************************/
  static void st_updateCurrentSillGoulard(
    Id icov0,
    Id ivar0,
    Id npadir,
    Id nvar,
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
    {
      ...
      for (Id ilagdir = 0; ilagdir < npadir; ilagdir++)
      {
        ...
        for (Id icov = 0; icov < ncova; icov++)
        {
          if (icov == icov0) continue;
          mv[ilagdir] +=
            matcor[icov].getValue(ivar0, ivar) * ge[icov].getValue(0, ilagdir);
        }
      }
   ...
   **
   ******************************************************************************/
  static void st_updateAlphaNoDiag(
    Id icov0,
    Id ivar0,
    Id nvar,
    VectorDouble& xr,
    ...
    std::vector<MatrixSymmetric>& alpha)
  {
    for (Id ivar = 0; ivar < nvar; ivar++)
    {
      if (ivar == ivar0 && !FFFF(consSill[ivar0])) continue;
      double value =
        matcor[icov0].getValue(ivar0, ivar) / (xr[ivar0] * xr[ivar]);
   ...
   **
   ****************************************************************************/
  static void st_updateCurrentSillDiag(
    Id icov0,
    Id ivar0,
    std::vector<MatrixSymmetric>& alpha,
    VectorDouble& xr,
   ...
   **
   *****************************************************************************/
  static Id st_makeDefinitePositive(
    Id icov0,
    Id nvar,
    const VectorDouble& consSill,
    std::vector<MatrixSymmetric>& matcor)
   ...
   **
   *****************************************************************************/
  static Id st_optimize_under_constraints(
    Id nvar,
    Id ncova,
    Id npadir,
    const VectorDouble& consSill,
    ...
    double* score)
  {
    ...
    for (Id icov = 0; icov < ncova; icov++) alpha[icov] = matcor[icov];
    ...
    for (iter = 0; iter < mauto.getMaxiter(); iter++)
    {
      for (Id icov0 = 0; icov0 < ncova; icov0++)
      {
        ...
        {
          double denom = st_sum_sills(ivar0, ncova, alpha)
                       - alpha[icov0].getValue(ivar0, ivar0);
          if (!FFFF(consSill[ivar0]) && denom < 1e-30) continue;
          if (!FFFF(consSill[ivar0]))
          {
            xrmax = sqrt(consSill[ivar0] / denom);
            xr[ivar0] = st_minimize_P4(
              icov0, ivar0, ncova, nvar, npadir, xrmax, xr, alpha, wt, gg, ge,
              consSill);
          ...
          st_updateCurrentSillGoulard(
            icov0, ivar0, npadir, nvar, ncova, wt, ge, gg, consSill, matcor);
          ...
          for (Id ivar = 0; ivar < nvar; ivar++)
            st_updateAlphaNoDiag(
              icov0, ivar, nvar, xr, consSill, matcor, alpha);
        }
      }
   ...
   **
   *****************************************************************************/
  static Id st_initialize_goulard(
    Id nvar,
    Id ncova,
    Id npadir,
    VectorDouble& wt,
      ...
      for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
      {
        ...
        else
        {
          retcode = aa.minimizeWithConstraintsInPlace(
            bb, MatrixDense(), VectorDouble(), MatrixDense(), VectorDouble(),
            res);
        }
        ...
        if (retcode)
        {
          for (Id icov = 0; icov < ncova; icov++) res[icov] = (ivar == jvar);
          if (ivar == jvar && !consSill.empty() && !FFFF(consSill[ivar]))
          {
            double temp = consSill[ivar] / ncova;
            for (Id icov = 0; icov < ncova; icov++) res[icov] = temp;
          }
        }
   ...
   **
   *****************************************************************************/
  static void st_goulard_sill_to_model(
    Id nvar,
    Id ncova,
    std::vector<MatrixSymmetric>& sill,
    Model* model)
   ...
   **
   *****************************************************************************/
  static Id st_goulard_with_constraints(
    const VectorDouble& consSill,
    const Option_AutoFit& mauto,
    Id nvar,
    Id ncova,
    ...
    std::vector<MatrixSymmetric>& sill)
  {
    ...
    st_initialize_goulard(nvar, ncova, npadir, wt, gg, ge, consSill, matcor);
    ...
    if (!flag_positive)
    {
      ...
      (void)st_optimize_under_constraints(
        nvar, ncova, npadir, consSill, mauto, wt, gg, ge, matcor, &crit);
    }
   ...
   **
   *****************************************************************************/
  static Id st_sill_fitting_intrinsic(
    Model* model,
    const Option_AutoFit& mauto,
    Id npadir,
    VectorDouble& wt,
    ...
    std::vector<MatrixSymmetric>& sill1)
  {
    ...
    for (Id iter = 0; iter < mauto.getMaxiter(); iter++)
    {
      ...
      for (Id ipadir = 0; ipadir < npadir; ipadir++)
      {
        ...
        for (Id ivar = 0; ivar < nvar; ivar++)
        {
          for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
          {
            ...
            for (Id icov = 0; icov < ncova; icov++)
              sum +=
                alphau[icov].getValue(0, 0) * ge[icov].getValue(ijvar, ipadir);
      ...
      if (st_goulard_without_constraint(
            mauto_new, nvar, 1, npadir, wt, gg, ge1, sill1, &crit))
        goto label_end;
      ...
      for (Id ivar = 0; ivar < nvar; ivar++)
        for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
        {
          ...
          {
            GG2(ijvar, ipadir) =
              (isZero(pivot)) ? 0. : GG(ijvar, ipadir) / pivot;
            WT2(ijvar, ipadir) = WT(ijvar, ipadir) * pivot * pivot;
            for (Id icov = 0; icov < ncova; icov++)
              ge2[icov].setValue(
                ijvar, ipadir, ge[icov].getValue(ijvar, ipadir));
          }
        }
      ...
      if (st_goulard_without_constraint(
            mauto_new, 1, ncova, npadir * nvs2, wt2, gg2, ge2, alphau, &crit))
        goto label_end;

      /* Stopping criterion */

      if (ABS(crit) < mauto_new.getTolred()
          || ABS(crit - crit_mem) / ABS(crit) < mauto_new.getTolred())
        break;
      crit_mem = crit;
    }
    ...
    const Option_AutoFit& mauto)
  {
    ...
    if (flag_title) st_goulard_debug_title(model->getNVar(), model->getNCov());
    ...
    if (!optvar.getFlagIntrinsic())
    {
      ...
      if (FFFF(constraints.getConstantSillValue()))
      {
        ...
        status = st_goulard_without_constraint(
          mauto, nvar, ncova, RECINT.npadir, RECINT.wt, RECINT.gg, RECINT.ge,
          RECINT.sill, &crit);
      }
      else
      {
      ...
      st_goulard_sill_to_model(
        model->getNVar(), model->getNCov(), RECINT.sill, model);
    }
    else
    {
      status = st_sill_fitting_intrinsic(
        model, mauto, RECINT.npadir, RECINT.wt, RECINT.gg, RECINT.ge,
        RECINT.wt2, RECINT.ge1, RECINT.ge2, RECINT.gg2, RECINT.alphau,
        RECINT.sill1);
    }
   ...
   **
   *****************************************************************************/
  static Id st_model_auto_strmod_reduce(
    StrMod* strmod,
    Id* npar,
    double hmax,
    double gmax,
    ...
    Option_AutoFit& mauto)
  {
    ...
    if (optvar.getFlagGoulardUsed())
      for (imod = 0; imod < strmod->nmodel; imod++)
      {
        ...
        (void)st_goulard_fitting(
          1, STRMOD->models[imod], constraints, optvar, mauto);
      }
    ...
    for (imod = 0; imod < strmod->nmodel; imod++)
    {
      ...
      {
        FLAG_COMPRESS(imod, icov) = st_structure_reduce(
          strmod, imod, icov, hmax, gmax, mauto.getTolsigma());
        ...
        if (FLAG_COMPRESS(imod, icov))
        {
          ...
          if (mauto.getVerbose() || OptDbg::query(EDbg::CONVERGE))
          {
            ...
            message(
              "Structure '%s' in model #%d is suppressed\n",
          ...
          {
            rank = st_parid_match(
              strmod, nparloc, imod, icov, EConsElem::ANGLE, -1, -1);
            if (rank >= 0 && lost_rank < 0)
            {
              lost_rank = rank;
              st_parid_decode(
                strmod->parid[lost_rank], &lost_imod, &lost_icov, &icons, &ivar,
                &jvar);
              if (mauto.getVerbose() || OptDbg::query(EDbg::CONVERGE))
              {
    ...
    {
      message(
        "Due to the tolerance (%lf (percent)), no basic structure would be "
        "left\n",
        mauto.getTolsigma());
    ...
    if (lost_rank >= 0)
    {
      for (imod = 0; imod < strmod->nmodel; imod++)
      {
        ...
        for (icov = 0; icov < model->getNCov(); icov++)
        {
          ...
          model_cova_characteristics(
            model->getCovType(icov), cov_name, &flag_range, &flag_param,
            &min_order, &max_ndim, &flag_int_1d, &flag_int_2d, &flag_aniso,
            &flag_rotation, &scalfac, &parmax);
          ...
          {
            message(
              "The Rotation is swapped to Structure '%s' in model #%d\n",
              model->getCovName(icov).c_str(), imod + 1);
          }
          ...
          {
            rank = st_parid_match(
              strmod, nparloc, lost_imod, lost_icov, EConsElem::ANGLE, ivar,
              -1);
            if (rank < 0) goto label_compress;
            strmod->parid[rank] =
              st_parid_encode(imod, icov, EConsElem::ANGLE, ivar, 0);
            ivar++;
          }
        }
      }
  ...
  label_compress:
    ...
    for (imod = 0; imod < strmod->nmodel; imod++)
    {
      ...
      for (icov = 0; icov < strmod->models[imod]->getNCov(); icov++)
      {
        ...
        {
          st_parid_decode(
            strmod->parid[ntot], &jmod, &kcov, &icons, &ivar, &jvar);
          if (jmod == imod && kcov == icov)
            strmod->parid[ntot] =
              st_parid_encode(jmod, jcov, icons, ivar, jvar);
        }
        jcov++;
      }
    }
  ...
  static Id st_model_define(Model* model, const Option_VarioFit& optvar)
  {
    ...
    for (Id jcov = 0; jcov < model->getNCov(); jcov++)
    {
      ...
      model_cova_characteristics(
        cova->getType(), cov_name, &flag_range, &flag_param, &min_order,
        &max_ndim, &flag_int_1d, &flag_int_2d, &flag_aniso, &flag_rotation,
        &scalfac, &parmax);
   ...
   **
   *****************************************************************************/
  static Id st_alter_model_optvar(
    const Vario* vario,
    Model* model,
    Constraints& constraints,
   ...
   **
   *****************************************************************************/
  static Id st_alter_vmap_optvar(
    const Db* dbmap,
    Model* model,
    Constraints& constraints,
    Option_VarioFit& optvar)
    ...
    VectorDouble& upper)
  {
    ...
    for (Id imod = 0; imod < 2; imod++)
    {
      ...
      for (Id jcov = 0; jcov < model->getNCov(); jcov++)
      {
        ...
        {
          messerr(
            "The structure '%s' is limited to dimension (%d)", cov_name,
            max_ndim);
        ...
        if (DEFINE_ANICOEF)
        {
          ...
          {
            for (Id idim = 1; idim < ndim; idim++) ntot++;
          }
        }
        ...
        if (DEFINE_ANIROT)
        {
          ...
          else
          {
            ...
            {
              first_covrot = jcov;
              for (Id idim = 0; idim < ndim; idim++) ntot++;
            }
          }
        }
   ...
   **
   *****************************************************************************/
  static void st_strmod_vario_evaluate(
    Id nbexp,
    Id npar,
    const VectorDouble& param,
    VectorDouble& tabge)
  {
    ...
    if (STRMOD->optvar.getFlagGoulardUsed())
      for (Id imod = 0; imod < STRMOD->nmodel; imod++)
      {
        ST_PREPAR_GOULARD(imod);
        (void)st_goulard_fitting(
          0, STRMOD->models[imod], CONSTRAINTS, OPTVAR, MAUTO);
      }
    st_goulard_verbose(1, MAUTO);
   ...
   **
   *****************************************************************************/
  static void st_strmod_vmap_evaluate(
    Id nbexp,
    Id npar,
    const VectorDouble& param,
    VectorDouble& tabge)
   ...
   **
   *****************************************************************************/
  static void st_vario_varchol_manage(
    const Vario* vario,
    Model* model,
    VectorDouble& varchol)
  {
    ...
    if (model->getCovMode() != EModelProperty::NONE)
    {
      ...
      for (Id ivar = 0; ivar < nvar; ivar++)
        for (Id jvar = 0; jvar <= ivar; jvar++)
        {
          ...
          double value =
            (ABS(auxval) > 0.) ? vario->getVar(ivar, jvar) / auxval : 0.;
    ...
    if (!matChol.isReady())
    {
      ...
      if (!matChol.isReady())
        messageAbort(
          "Error in st_vario_varchol_manage(): This should never happen");
    }
   ...
   *****************************************************************************/
  static void
    st_model_post_update(StrMod* strmod, const Option_VarioFit& optvar)
  {
    for (Id imod = 0; imod < strmod->nmodel; imod++)
    {
      Model* model = strmod->models[imod];
    ...
    const Option_VarioFit& optvar_arg)
  {
    ...
    if (flag_regular)
    {
      ...
      REGULARIZE.ndisc[0] =
        static_cast<Id>(get_keypone("Data_Discretization_NX", 1.));
      REGULARIZE.ndisc[1] =
        static_cast<Id>(get_keypone("Data_Discretization_NY", 1.));
      REGULARIZE.ndisc[2] =
        static_cast<Id>(get_keypone("Data_Discretization_NZ", 1.));
    ...
    vario->getExtension(
      0, 0, -1, 0, 1, TEST, TEST, TEST, TEST, &flag_hneg, &flag_gneg, &c0,
      &hmin, &hmax, &gmin, &gmax);
    angles.resize(ndim);
    ...
    if (st_manage_recint(optvar, 1, ndim, nvar, nbexp, ncova, npadir))
      goto label_end;

    /* Generate the default values */

    npar0 = st_model_auto_count(
      vario, model, nullptr, constraints, optvar, param, lower, upper);

    /* Create the Model structures */

    strmod = st_model_auto_strmod_alloc(
      model, NULL, npar0, norder, hmax, angles, optvar, &npar);
    ...
    st_model_auto_pardef(
      strmod, npar, hmax, varchol, angles, param, lower, upper);
    st_model_auto_scldef(strmod, npar, hmax, varchol, scale);
    st_model_auto_constraints_apply(
      strmod, npar, constraints, param, lower, upper);
    ...
    do
    {
      st_model_auto_strmod_print(
        1, strmod, constraints, mauto, param, lower, upper, npar, nbexp);
      if (npar > 0)
      {
        status = foxleg_f(
          nbexp, npar, 0, MatrixDense(), param, lower, upper, scale, mauto, 0,
          st_strmod_vario_evaluate, RECINT.ggc, RECINT.wtc);
      }
      else
      {
      ...
      st_model_auto_strmod_print(
        0, strmod, constraints, mauto, param, lower, upper, npar, nbexp);
      flag_reduce = st_model_auto_strmod_reduce(
        strmod, &npar, hmax, gmax, param, lower, upper, constraints, mauto);

      /* Reset the parameters to their default values */

      if (status < 0)
      {
        for (Id i = 0; i < npar; i++) param[i] = lower[i] = upper[i] = TEST;
        st_model_auto_pardef(
          strmod, npar, hmax, varchol, angles, param, lower, upper);
      }
      st_model_auto_scldef(strmod, npar, hmax, varchol, scale);
    ...
    VectorDouble& upper)
  {
    ...
    {
      messerr(
        "The number of items in the Db Grid for Variogram maps (%d)",
        dbmap->getNLoc(ELoc::Z));
      messerr(
        "is not compatible with the number of variables in the Model (%d)",
        nvar);
      return (-1);
    }
    ...
    {
      model_cova_characteristics(
        model->getCovType(jcov), cov_name, &flag_range, &flag_param, &min_order,
        &max_ndim, &flag_int_1d, &flag_int_2d, &flag_aniso, &flag_rotation,
        &scalfac, &parmax);
      if (max_ndim > 0 && ndim > max_ndim)
      {
    ...
    const Option_VarioFit& optvar_arg)
  {
    ...
    {
      messerr(
        "Number of variables in Db (%d) must match the one in Model (%d)",
    ...
    if (st_manage_recint(optvar, 0, ndim, nvar, 0, ncova, npadir))
      goto label_end;
    st_load_vmap(npadir, RECINT.gg, RECINT.wt);

    /* Generate the default values */

    npar0 = st_vmap_auto_count(
      dbmap, model, constraints, optvar, param, lower, upper);
    ...
    do
    {
      ...
      status = foxleg_f(
        nbexp, npar, 0, MatrixDense(), param, lower, upper, scale, mauto, 0,
        st_strmod_vmap_evaluate, RECINT.gg, RECINT.wt);
      if (status > 0) goto label_end;
