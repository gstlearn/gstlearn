...
**
*****************************************************************************/
static void st_blank(VectorDouble& tab,
                     int     ntab)
{
  int i;

  for (i=0; i<ntab; i++) tab[i] = TEST;
}
  ...
  for (imod=0; imod<strmod->nmodel; imod++)
  {
    ...
    for (jcov=0; jcov<model->getCovaNumber(); jcov++)
    {
      ...
      if (DEFINE_AIC)
        for (ivar=0; ivar<nvar; ivar++)
          for (jvar=0; jvar<=ivar; jvar++)
            strmod->parid[ntot++] = st_parid_encode(imod,jcov,CONS_SILL,ivar,jvar);
      
      /* Third parameter */
      if (DEFINE_THIRD)
      {
        strmod->parid[ntot++] = st_parid_encode(imod,jcov,CONS_PARAM,0,0);
      }
      
      /* Range */
      if (DEFINE_RANGE)
      {
        strmod->parid[ntot++] = st_parid_encode(imod,jcov,CONS_RANGE,0,0);
      }
      ...
      if (DEFINE_ANICOEF)
      {
        if (ndim == 2)
        {
          strmod->parid[ntot++] = st_parid_encode(imod,jcov,CONS_RANGE,1,0);
        }
        else if (ndim == 3)
        {
          ...
          {
            strmod->parid[ntot++] = st_parid_encode(imod,jcov,CONS_RANGE,2,0);
          }
        }
        else
        {
          ...
          {
            strmod->parid[ntot++] = st_parid_encode(imod,jcov,CONS_RANGE,idim,0);
          }
        }
      }
      ...
      if (DEFINE_ANIROT)
      {
        if (ndim == 2)
        {
          ...
          {
            first_covrot = jcov;
            strmod->parid[ntot++] = st_parid_encode(imod,jcov,CONS_ANGLE,0,0);
          }
        }
        ...
        else
        {
          if (TAKE_ROT)
          {
            ...
            {
              strmod->parid[ntot++] = st_parid_encode(imod,jcov,CONS_ANGLE,idim,0);
            }
          }
        }
      }
      ...
      if (DEFINE_T_RANGE)
      {
        strmod->parid[ntot++] = st_parid_encode(imod,0,CONS_T_RANGE,0,0);
      }
    }
  }
                       ...
                       VectorDouble& wt)
{
  ...
  for (ivar=ijvar=0; ivar<nvar; ivar++)
    for (jvar=0; jvar<=ivar; jvar++, ijvar++)
    {
      ratio = (vario->getVars(ivar,jvar) > 0 && vario->getVars(jvar,ivar) > 0) ?
                                         ...
                                         double       *crit_arg)
{
  ...
  for (int i = 0; i< npadir * nvs2 * ncova; i++) fk[i] = 0.;
                      ...
                      VectorDouble& upper)
{
  ...
  else if (! FFFF(lower_val)) 
    lower[rank] = MAX(lower_val,lower[rank]);

  /* Upper bound */

  if (FFFF(upper[rank]))
    upper[rank] = upper_val;
  else if (! FFFF(upper_val))
    upper[rank] = MIN(upper_val,upper[rank]);

  /* Initial parameter */
  if (FFFF(param[rank]))
    param[rank] = (! FFFF(def_val)) ? def_val : 0.;
                                        ...
                                        VectorDouble& param)
{
  ...
  for (ntot=0; ntot<npar; ntot++)
  {
    ...
    switch (type)
    {
      ...
      case CONS_T_RANGE:
        Tapering* tape = model->getModTrans().getTape();
  ...
  if (strmod->optvar.getLockSamerot())
  {
    for (imod=0; imod<strmod->nmodel; imod++)
    {
      ...
      for (icov=0, found=-1; icov<model->getCovaNumber() && found<0; icov++)
      {
                              ...
                              VectorDouble& tabge)
{
  ...
  for (int i=0; i<nbexp; i++)
  {
    ...
    for (int idim = 0; idim < ndim; idim++) d0[idim] = strexps[i].dd[idim];
...
static int st_check_definite_positive(Model *model)
{
  ...
  for (int icov=0; icov<model->getCovaNumber(); icov++)
  {
    ...
    if (! is_matrix_definite_positive(nvar,cc.data(),valpro.data(),vecpro.data(),1))
      return 1;
  }
                                         ...
                                         double    *score)
{
  ...
  for (ivar=0; ivar<nvar; ivar++)
  {
    if(FFFF(mauto.getConstantSills(ivar)))
      ...
      xr[ivar] = sqrt(mauto.getConstantSills(ivar) /
                      st_sum_sills(ivar,ncova,nvar,alpha));
  }
    ...
    { 
      for(ivar0=0;ivar0<nvar;ivar0++)
      {
        ...
        if(!FFFF(mauto.getConstantSills(ivar0)))
        {
          ...
          xr[ivar0]=st_minimize_P4(icov0,ivar0,ncova,nvar,
                                   npadir,xrmax,xr,alpha,wt,gg,
                                   ge,mauto.getConstantSills());
        ...
        if(!FFFF(mauto.getConstantSills(ivar0)))
          st_updateAlphaDiag(icov0,ivar0,ncova,nvar,xr,alpha,mauto.getConstantSills());
        ...
        st_updateCurrentSillGoulard(icov0,ivar0,npadir,nvar,ncova,
                                    wt,ge,gg,mauto.getConstantSills(),matcor);
        ...
        (void) st_makeDefinitePositive(icov0,nvar,mauto.getConstantSills(),matcor);
                                 ...
                                 VectorDouble& matcor)
{
  ...
  for (ivar=ijvar=0; ivar<nvar; ivar++)
    for (jvar=0; jvar<=ivar; jvar++,ijvar++)
    {
      ...
      {
        bb[icov] = 0.;
        for (jcov=0; jcov<ncova; jcov++) AA(icov,jcov) = 0.;
      }
                                       ...
                                       VectorDouble& sill)
{
  ...
  st_initialize_goulard(nvar,nvs2,ncova,npadir,wt,gg,ge,mauto.getConstantSills(),
                        matcor);
                               ...
                               VectorDouble& sill)
{
  ...
  nvs2   = nvar * (nvar + 1) / 2;
  crit_mem = 1.e30;
  for (icov=0; icov<ncova; icov++) alphau[icov] = 1. / (double) ncova;
                              ...
                              Option_AutoFit& mauto)
{
  ...
  if (flag_reset) st_sill_reset(model->getVariableNumber(),model->getCovaNumber(),RECINT.sill);

  /* Print the debug title (optional) */

  if (flag_title) st_goulard_debug_title(model->getVariableNumber(),model->getCovaNumber());
  ...
  if (! mauto.getFlagIntrinsic())
  {
    ...
    {      
      ...
      status = st_goulard_without_constraint(mauto,model->getVariableNumber(),model->getCovaNumber(),
                                             RECINT.npadir,
                                             RECINT.wt,RECINT.gg,
    ...
    { 
      ...
      status = st_goulard_with_constraints(mauto,
                                           model->getVariableNumber(),model->getCovaNumber(),
                                           RECINT.npadir,
                                           RECINT.wt,RECINT.gg,
                                           RECINT.ge,RECINT.sill);
    }
    ...
    st_goulard_sill_to_model(model->getVariableNumber(),model->getCovaNumber(),RECINT.sill,model);
  }
  else
  {
                                       ...
                                       Option_AutoFit& mauto)
{
  ...
  for (imod=0; imod<strmod->nmodel; imod++)
  {
    ...
    for (icov=0; icov<model->getCovaNumber(); icov++)
    {
      FLAG_COMPRESS(imod,icov) =
        st_structure_reduce(strmod,imod,icov,hmax,gmax,mauto.getTolsigma());
      ...
      if (FLAG_COMPRESS(imod,icov)) 
      {
        ...
        {
          rank = st_parid_match(strmod,nparloc,imod,icov,CONS_ANGLE,-1,-1);
  ...
  if (lost_rank >= 0)
  {
    for (imod=0; imod<strmod->nmodel; imod++)
    {
      ...
      for (icov=0; icov<model->getCovaNumber(); icov++)
      {
        ...
        while (1)
        {
          rank = st_parid_match(strmod,nparloc,lost_imod,lost_icov,CONS_ANGLE,
                                ivar,-1);
          if (rank < 0) goto label_compress;
          strmod->parid[rank] = st_parid_encode(imod,icov,CONS_ANGLE,ivar,0);
          ivar++;
        }
      }
    }
                           ...
                           const Option_VarioFit& optvar)
{
  ...
  for (jcov=0; jcov<model->getCovaNumber(); jcov++)
  {
    ...
    model_cova_characteristics(cova->getType(),cov_name,
                               &flag_range,&flag_param,&min_order,&max_ndim,
                               &flag_int_1d,&flag_int_2d,
                               &flag_aniso,&flag_rotation,&scalfac,&parmax);
                                 ...
                                 Option_VarioFit& optvar)
{
  ...
  {
    message("Goulard option is switched OFF due to presence of Sills in Constraints\n");
  ...
  for (imod=0; imod<2; imod++)
  {
    ...
    for (jcov=0; jcov<model->getCovaNumber(); jcov++)
    {
      ...
      if (DEFINE_ANICOEF)
      {
        ...
        {
          for (idim=1; idim<ndim; idim++) ntot++;
        }
      }
      ...
      if (DEFINE_ANIROT)
      {
        ...
        else
        {
          ...
          {
            first_covrot = jcov;
            for (idim=0; idim<ndim; idim++) ntot++;
          }
        }
      }
                                    ...
                                    VectorDouble& varchol)
{
  ...
  if (model->getModTransMode() != MODEL_PROPERTY_NONE)
  {
    ...
    for (i = 0; i < nvar2; i++)
      aux[i] = vario->getVars(i) / aux[i];
  }
  else
  {
    for (i = 0; i < nvar2; i++)
      aux[i] = vario->getVars(i);
  }
                               ...
                               Option_VarioFit    optvar)
{
  ...
  variogram_extension(vario,0,0,-1,0,1,TEST,TEST,TEST,TEST,
  ...
  do {
    st_model_auto_strmod_print(1,strmod,mauto,param,lower,upper,npar,nbexp);
    if (npar > 0)
    {
      status = foxleg_f(nbexp,npar,0,VectorDouble(),param,lower,upper,scale,mauto,0,
    ...
    st_model_auto_strmod_print(0,strmod,mauto,param,lower,upper,npar,nbexp);
    flag_reduce = st_model_auto_strmod_reduce(strmod,&npar,hmax,gmax,
                                              param,lower,upper,mauto);
                              ...
                              Option_VarioFit optvar)
{
  ...
  if (ndim > 3)
  {
    messerr("Procedure cannot be used for space dimension (%d) larger than 3",ndim);
    goto label_end;
  }
  nvar = model->getVariableNumber();
  ...
  angles.resize(ndim);
  for (idim=0; idim<ndim; idim++) angles[idim] = 0.;
                                   ...
                                   int iv2)
{
  ...
  for (i=0; i<(int) constraints.getConsItemNumber(); i++)
  {
    ...
    {
      if (icase == -1 || icase == 1) return(item->getValue());
    }
    else
    {
