                         ...
                         double *result)
{
  ...
  for (icol1=0; icol1<ncol; icol1++)
    for (icol2=0; icol2<ncol; icol2++)
    {
      ...
      else
      {
        ...
        if (oper == "corr")
        {
          v12[iad] = (v1[iad] > 0 && v2[iad] > 0) ? 
            v12[iad] / sqrt(v1[iad] * v2[iad]) : 0.;
        }
      }
    }
  ...
  if (flag_verbose)
  {
    message("\n");
    if (oper == "num")
      print_matrix("Matrix of Number of defined samples",0,1,nx,ny,NULL,num);
    else if (oper == "mean")
      ...
      print_matrix("Matrix of Variable Standard Deviations",0,1,nx,ny,NULL,v12);
    else if (oper == "mini")
      print_matrix("Matrix of Variable Minima",0,1,nx,ny,NULL,mini);
    else if (oper == "maxi")
      print_matrix("Matrix of Variable Maxima",0,1,nx,ny,NULL,maxi);
    else if (oper == "plus")
      print_matrix("Matrix of Number of positive samples",0,1,nx,ny,NULL,plus);
    else if (oper == "moins")
      print_matrix("Matrix of Number of negative samples",0,1,nx,ny,NULL,moins);
    else if (oper == "zero")
      print_matrix("Matrix of Number of zero samples",0,1,nx,ny,NULL,zero);
    else
                              ...
                              int           radius)
{
  ...
  for (icol=0; icol<ncol; icol++)
  {
    ...
    else if (! strcmp(oper,"med"))
    {
      for (i=0; i<nxyz; i++)
      {
        value = (nmed > 0) ? medtab[nmed / 2] : TEST;
        dbgrid->setArray(i,iptr,value);
      }
    }
    else if (! strcmp(oper,"mini"))
...
label_end:
  ...
  if ((! strcmp(oper,"mean")  ||
       ! strcmp(oper,"var")   || 
       ! strcmp(oper,"stdv")) && iptn > 0)
    dbgrid->deleteFieldByAttribute(iptn);
  if ((! strcmp(oper,"var")   || 
       ! strcmp(oper,"stdv")) && iptm > 0)
                                    ...
                                    double *tab)
{
  ...
  double *coor,*nn,*s1,*s2,*v1,*v2,*v12,*mini,*maxi,*cutval,z1,z2,ratio;
  ...
  for (int i=0; i<nxyz; i++)
  {
    ...
    if (ratio <= 0)
    {
      ...
      if (flag_maxi) maxi[i] = TEST;
      if (flag_t || flag_q) 
        for (int icut=0; icut<ncut; icut++) cutval[icut + i * ncut] = TEST;
    }
    else
    {
...
**
*****************************************************************************/
static void st_update_prop(Db     *dbin,
                           int    *indg,
                           int     nfacies,
                           double *prop)
{
                            ...
                            double *trans)
{
  ...
  if (ifac1 < 1 || ifac1 > nfacies || 
      ifac2 < 1 || ifac2 > nfacies) return;
                                ...
                                double *tab)
{
  ...
  total = 0.;
  for (ifac=0; ifac<nitem; ifac++) total += tab[ifac];
...
{
  double *prop;
  int    *indg,ndim,ngrid,error,iptr,iech,ifac,aux1,aux2,bux1,bux2,ishift,i1,i2;
  ...
  {
    for (ifac=0; ifac<nfacies; ifac++) prop[ifac] = 0.;
...
label_end:
  prop = (double *) mem_free((char *) prop);
  indg = db_indg_free(indg);
  return(error);
}
                                 ...
                                 int     orient)
{
  double *trans;
  int    *indg;
  int    ndim,ngrid,error,iptr,iech,item,aux1,aux2,bux1,bux2,i1,i2,ishift,nitem;
  ...
  {
    indg[pos] = iech;
    for (item=0; item<nitem; item++) trans[item] = 0.;
...
label_end:
  trans = (double *) mem_free((char *) trans);
  indg  = db_indg_free(indg);
  return(error);
}
                                 ...
                                 double *valtab1)
{
  ...
  if (verbose)
  {
    ...
    for (int idim=1; idim<ndim; idim++) message("x%d",nxyz[idim]);
    ...
    for (iy=0; iy<nxyz2[1]; iy++)
      for (ix=0; ix<nxyz2[0]; ix++)
      {
        ...
        V2_TAB(ix,iy,iz) = (N2_TAB(ix,iy,iz) > 0) ?
          V2_TAB(ix,iy,iz) / N2_TAB(ix,iy,iz) : TEST;
      }
  return;
}
    ...
    for (iy=0; iy<nxyz2[1]; iy++)
      for (ix=0; ix<nxyz2[0]; ix++)
      {
        ...
        V2_TAB(ix,iy,iz) = (ABS(V2_TAB(ix,iy,iz)) > 1.e-10) ?
          N2_TAB(ix,iy,iz) / V2_TAB(ix,iy,iz) : TEST;
      }
  return;
}
                           ...
                           int     verbose)
{
  ...
  if (! st_is_subgrid(verbose,"Upscaling",
                      dbgrid1,dbgrid2,ind0,nxyz,&ntot)) goto label_end;
    ...
    if (dbgrid2->isActive(iech))
    {
      ...
      if (probtot > 0)
      {
        ...
        st_upscale(orient,nxyz,flag_save,
                   numtab0,numtab1,numtab2,valtab0,valtab1,valtab2,
                   &result1,&result2);
                            ...
                            int    *loccur)
{
  ...
  if (probtot > 0)
  {
    ...
    for (int idim=0; idim<ndim; idim++) loccur[idim] += NBGH(ivois,idim);
  }
}
                      ...
                      double *trsave)
{
  ...
  for (int iseed=0; iseed<nseed; iseed++)
  {
    ...
    for (int idim=0; idim<ndim; idim++) TABCUR(iseed,idim) = TABINI(iseed,idim);
  ...
  for (int iter=0; iter<niter; iter++)
  {
    ...
    for (int iseed=0; iseed<nseed; iseed++)
    {
      ...
      st_migrate_seed(ndim,n_nbgh,nxyz,nbgh,valwrk,valtab0,
                      tabwrk,&TABCUR(iseed,0));
      ...
      d2 = st_squared_distance(orient,ndim,&TABINI(iseed,0),&TABCUR(iseed,0));
...
static double st_get_diff_coeff(int     niter,
                                ...
                                double *cvsave)
{
  double slope,origin,slope_ref,origin_ref,sum_x,sum_y,sum_xy,sum_xx,count;
  ...
  for (int jter=0; jter<niter; jter++)
  {
    ...
    st_update_regression((double ) (iter+1),cvdist2[iter],
  ...
  if (verbose)
    message("- Slope=%lf Origin=%lf\n",slope_ref,origin_ref);

  /* Optional saving */

  if (flag_save)
    set_keypair("Diffusion.Converge",1,niter,3,cvsave);
                             ...
                             int     verbose)
{
  ...
  if (! st_is_subgrid(verbose,"Diffusion Coefficient",
                      dbgrid1,dbgrid2,ind0,nxyz,&ntot)) goto label_end;
  ...
  for (iech=0; iech<nech; iech++)
  {
    ...
    if (dbgrid2->isActive(iech))
    {
      ...
      probtot = st_extract_subgrid(verbose,1,iech,nech,ntot,
                                   dbgrid1,ind0,ixyz,nxyz,numtab0,valtab0);
                                ...
                                const String& radix)
{
  ...
  for (icol=ijcol=0; icol<ncol; icol++)
  {
    ...
    maxi[icol] = -1.e30;
    if (flag_correl)
      for (jcol=0; jcol<ncol; jcol++,ijcol++) cov[ijcol] = 0.;
  }
  ...
  for (icol=0; icol<ncol; icol++)
  {
    st_get_rowname(radix,ncol,icol,db_name_get_by_att(db,iatts[icol]),string);
