...
namespace gstlrn
{
   ...
   **
   *****************************************************************************/
  static double st_extract_subgrid(
    Id verbose,
    Id flag_ffff,
    Id iech0,
    Id nech0,
    ...
    VectorDouble& valtab)
  {
    ...
    for (iz = 0; iz < nxyz[2]; iz++)
    {
      ...
      message(string.data());
      VectorDouble valsub(
        valtab.begin() + shift * iz, valtab.begin() + shift * (iz + 1));
      printMatrix(valsub, nxyz[0], nxyz[1]);
      (void)gslSPrintf(string, "%s Counts (iz=%d)\n", subtitle, iz + 1);
      message(string.data());
      VectorDouble numsub(
        numtab.begin() + shift * iz, numtab.begin() + shift * (iz + 1));
    ...
    Id* ntot)
  {
    ...
    for (Id idim = 0; idim < ndim; idim++)
    {
      ...
      {
        messerr(
          "The origin of the Output Grid does not coincide with a node of the "
          "Input Grid");
        return (0);
      }
      ...
      {
        messerr(
          "The grid cell of the Output Grid is not a multiple of the grid cell "
          "of the Input Grid");
        return (0);
      }
  ...
  Id db_upscale(DbGrid* dbgrid1, DbGrid* dbgrid2, Id orient, Id verbose)
  {
    ...
    {
      messerr(
        "Inconsistency between Orientation (%d) and Space dimension (%d)",
        orient, ndim);
      goto label_end;
    }
    orient--;
    ...
    if (!st_is_subgrid(
          verbose, "Upscaling", dbgrid1, dbgrid2, ind0, nxyz, &ntot))
      goto label_end;
    ...
    for (iech = 0; iech < dbgrid2->getNSample(); iech++)
    {
      ...
      if (dbgrid2->isActive(iech))
      {
        ...
        probtot = st_extract_subgrid(
          verbose, 0, iech, dbgrid2->getNSample(), ntot, dbgrid1, ind0,
          ixyz.data(), nxyz, numtab0.data(), valtab0.data());
        ...
        if (probtot > 0)
        {
          ...
          st_upscale(
            orient, nxyz, flag_save, numtab0, numtab1, numtab2, valtab0,
    ...
    double* trsave)
  {
    ...
    for (Id iter = 0; iter < niter; iter++)
    {
      ...
      for (Id iseed = 0; iseed < nseed; iseed++)
      {
        ...
        st_migrate_seed(
          ndim, n_nbgh, nxyz, nbgh, valwrk, valtab0, tabwrk, &TABCUR(iseed, 0));
        ...
        d2 = st_squared_distance(
          orient, ndim, &TABINI(iseed, 0), &TABCUR(iseed, 0));
   ...
   **
   *****************************************************************************/
  static void st_update_regression(
    double x,
    double y,
    double* count,
    double* sum_x,
   ...
   **
   *****************************************************************************/
  static double st_get_diff_coeff(
    Id niter,
    Id verbose,
    double pmid,
    Id flag_save,
    ...
    double* cvsave)
  {
    ...
    for (Id jter = 0; jter < niter; jter++)
    {
      ...
      st_update_regression(
        static_cast<double>(iter + 1), cvdist2[iter], &count, &sum_x, &sum_y,
        &sum_xx, &sum_xy);
      if (count > 1)
      {
        mx = sum_x / count;
      ...
      {
        message(
          "  Rank=%5d Slope=%lf Origin=%lf (Count=%d)", iter + 1, slope, origin,
    ...
    Id verbose)
  {
    ...
    {
      messerr(
        "Argument 'orient' (%d) can be 0 or one of the space dimension",
        orient);
      goto label_end;
    }
    if (!dbgrid1->isNVarComparedTo(1)) goto label_end;
    ...
    if (!st_is_subgrid(
          verbose, "Diffusion Coefficient", dbgrid1, dbgrid2, ind0, nxyz,
          &ntot))
      goto label_end;
    ...
    for (iech = 0; iech < nech; iech++)
    {
      ...
      if (dbgrid2->isActive(iech))
      {
        ...
        probtot = st_extract_subgrid(
          verbose, 1, iech, nech, ntot, dbgrid1, ind0, ixyz.data(), nxyz,
        ...
        if (probtot > 0)
        {
          ...
          st_updiff(
            orient, ndim, ntot, nseed, niter, n_nbgh, flag_save, probtot, nxyz,
            nbgh.data(), tabini.data(), tabcur.data(), tabwrk.data(),
            valwrk.data(), valtab0.data(), verbose, cvdist2.data(),
            trsave.data());

          /* Derive the diffusion coefficient */

          diff_coeff = st_get_diff_coeff(
            niter, verbose, pmid, flag_save, cvdist2.data(), cvsave.data());
    ...
    double* Q)
  {
    ...
    if (verbose)
    {
      ...
      for (icut = 0; icut < ncut; icut++)
        message(
          "Cutoff %2d (above %lf) - Tonnage = %lf - Metal = %lf\n", icut + 1,
          zcut[icut], T[icut], Q[icut]);
    }
