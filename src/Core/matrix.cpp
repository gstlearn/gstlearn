...
**
*****************************************************************************/
static int st_matrix_solve(double *at,
                           double *b,
                           double *x,
                           int     neq,
                           int     nrhs)

{
  ...
  for (l=0; l<nrhs; l++)
  {
    ...
    {
      ratio = BS(k,l);
      for (j=k+1; j<neq; j++) ratio -= AT(k,j) * XS(j,l);
      XS(k,l) = ratio / AT(k,k);
    }
  }
    ...
    for (i2=neq-1; i2>0 && n2==0; i2--)
    {
      ...
      for (i1=i2; i1>0 && n1==0; i1--)
        if(ABS(work[2][i1]) <= EpsMatrix * neq *
           (ABS(work[3][i1-1]) + ABS(work[3][i1]))) n1 = i1;
      if (n1 != i2) n2 = i2;
    }
    if (n2 < 1) break;
                                ...
                                double *v3)
{
  ...
  for (i4=0; i4<n1*n3; i4++) v4[i4] = 0.;

  for (i3=0; i3<n3; i3++)
    for (i1=0; i1<n1; i1++)
      for (i2=0; i2<n2; i2++) V4(i1,i3) += V1(i1,i2) * V2(i2,i3);

  for (i4=0; i4<n1*n3; i4++) v3[i4] = v4[i4];
...
GEOSLIB_API void matrix_transpose_in_place(int n1, int n2, double *v1)
{
  ...
  for (int i=0; i<n1 * n2; i++) v1[i] = w1[i];
                             ...
                             int    *pivot)
{
  ...
  if (mode == 0)
    for (i=0; i<loc_lhs; i++) LHS_TAB[i] = a[i];
  else
    matrix_tl2tu(neq,a,LHS_TAB);
  for (i=0; i<loc_rhs; i++) RHS_TAB[i] = b[i];
                                            ...
                                            int     verbose)
{
  ...
  if (verbose) 
    print_matrix("Matrix to be checked",0,1,neq,neq,NULL,a);
                                   ...
                                   int     verbose)
{
  ...
  for (i=0; i<neq; i++)
    for (j=0; j<neq; j++)
    {
      ...
      {
        if (verbose) messerr("The element (A*At)[%d,%d] = %lf (should be %lf)",
                             i+1,j+1,prod,comp);
        return(0);
      }
    }
                                      ...
                                      double *x)
{
  ...
  {
    sum = r[i];
    for (j=i+1; j<neq; j++) sum -= x[j] * TL(j,i);
    x[i] = sum / TL(i,i);
  }
                                          ...
                                          double *xl)
{
  ...
  for (k=0; k<neq; k++) 
  {
    ...
    {
      sum = r[i];
      for (j=i+1; j<neq; j++) sum -= res[j] * TL(j,i);
      res[i] = sum / TL(i,i);
      XL(i,k) = res[i];
    }
  }
                              ...
                              double *cond)
{
  ...
  for (i=0; i<neq; i++)
    for (j=0; j<neq; j++)
    {
      ...
      for (k=0; k<neq; k++)
      {
        if (ABS(eigval[k]) > valcond * TolInvgen)
...
GEOSLIB_API void vector_translate(int ndim,
                                  ...
                                  double *b)
{
  for (int i=0; i<ndim; i++) b[i] = a[i] + v[i];
}
                  ...
                  int neq)
{
  ...
  for (i=0; i<neq; i++) 
  { 
    for (j=0; j<neq; j++) A2(neq+i,j) = 0.0; 
    A2(neq+i,i) = 1.0; 
  }
...
**
*****************************************************************************/
static int st_match_index(int  index,
                          int  nitem,
                          int *items)
{
  int i;
                               ...
                               double *b)
{
  ...
  for (i1=0; i1<neq; i1++)
    ...
    {
      value = 0.;
      for (i2=0; i2<neq; i2++) value += A(i1,i2) * A(i3,i2);
      B(i1,i3) = value;
    }
...
**
*****************************************************************************/
static int st_count_active(int     nai,
                           int *active)
{
  int number,i;
                          ...
                          double *tabout)
{
  ...
  for (i=0; i<nae; i++)
  {
    for (j=0; j<neq; j++) TABOUT(j,number) = TABEMT(j,i);
    number++;
  }
  ...
  for (i=0; i<nai; i++)
  {
    ...
    for (j=0; j<neq; j++) TABOUT(j,number) = TABIMT(j,i);
    number++;
  }
  return(number);
}
                           ...
                           int    *flag)
{
  ...
  for (i=0; i<nai; i++)
  {
    ...
    value = 0.;
    for (j=0; j<neq; j++) value += AIMT(j,i) * xmat[j];
                            ...
                            double *xmat)
{
  ...
  if (matrix_qoc(0,neq,hmat,gmat,nae,aemat,bemat,xcand,lambda)) goto label_end;
  if (nai <= 0) 
  {
    for (i=0; i<neq; i++) xmat[i] = xcand[i];
    error = 0;
    return(0);
  }
  ...
  while(! sortie)
  {
    ...
    else
    {
      ...
      for (i=0; i<nai; i++)
      {
        ...
        value = 0.;
        for (j=0; j<neq; j++) value += AIMT(j,i) * (xcand[j] - xmat[j]);
        omega = vmat[i] / value;
      ...
      for (i=0; i<neq; i++) xmat[i] += omin * (xcand[i] - xmat[i]);
      active[first] = 1;
    }
  }
...
label_end:
  if (error) a = (double *) mem_free((char *) a);
  return(a);
}
                               ...
                               int     neq)
{
  ...
  for (int i=0; i<neq * neq; i++) mat[i] /= det;
...
label_end:
  cofac = (double *) mem_free((char *) cofac);
  return(error);
}
