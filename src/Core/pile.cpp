...

{
  ...
  for (i=0; i<MAX_COUNT[type]; i++) piles[type][i] = NULL;
...

{
  ...
  if (ival < 0) 
  {
    messerr("No %s Slot available: all entries are used (%d)",
            PILE_NAME[type],MAX_COUNT[type]);
                             ...
                             int mode)
{
  ...
  switch (mode)
  {
    case 1:                     /* The element must be already allocated */
      ...
      messerr("Error: the element (%d) of the pile (%s) is not allocated",
              rank,PILE_NAME[type]);
      break;
    ...
    for (i=0; i<MAX_COUNT[type]; i++)
    {
      if (piles[type][i] != NULL) 
        message("Type %9s : Slot %2d (out of %2d) allocated\n",
                PILE_NAME[type],i+1,MAX_COUNT[type]);
      else
