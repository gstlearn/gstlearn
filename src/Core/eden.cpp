...
label_end:
  if (error) stats = st_stats_undefine(stats);
  return(stats);
}
...
**
*****************************************************************************/
static void st_stats_print(const char *title,
                           Eden_Stats *stats)
...
**
*****************************************************************************/
static void set_FACIES(int iech,
                       int ifacies)
{
...
**
*****************************************************************************/
static void set_FLUID(int iech,
                      int ifluid)
{
...
**
*****************************************************************************/
static void set_DATE(int iech,
                     int idate)
{
  double value;
  
  value = (IFFFF(idate)) ? TEST : idate;
...
**
*****************************************************************************/
static int st_check_max(Eden_Stats *stats,
                        double number_max,
                        double volume_max)
{
  int    totnum,number;
...

{
  ...
  for (iech=0; iech<NXYZ; iech++)
  {
    ...
    if (ifacies == SHALE || perm <= 0)
    {
      ...
      {
        if (VERBOSE)
          messerr("Cell %d: Inconsistent Fluid (%d) with Facies (%d) or Perm (%d) -> set to %d",
...
**
*****************************************************************************/
static void st_update_results(int reset_facies,
                              int show_fluid)
...

{
  ...
  for (iech=0; iech<NXYZ; iech++)
  {
    ...
    if (ifluid > 0)
      DB->updArray(iech,IPTR_STAT_FLUID+ifluid-1,0,1);
...
**
*****************************************************************************/
static int st_fluid_modify(Skin *skin,
                           int   ipos,
                           int  *ref_fluid_loc)
{
    ...
    if (skin_grid_shift(skin,ipos,dir,&ecr))
    {
      if (ALREADY_FILLED(ecr))
      {
        ...
        if (ref_fluid == UNDEF_FLUID)
        {
          if (dir == DIR_DOWN)
          {
            if (WT(get_FACIES(ecr),get_FLUID(ecr),get_PERM(ecr),
                   invdir[dir]) > 0) ref_fluid = fluid;
          }
          else if (dir == DIR_UP)
          {
            if (WT(get_FACIES(ecr),get_FLUID(ecr),get_PERM(ecr),
  ...
  if (ref_fluid == UNDEF_FLUID) 
    messageAbort("Undefined replacement Fluid");
...

{
  ...
  for (ifluid=0; ifluid<NFLUIDS; ifluid++)
    for (ifacies=0; ifacies<NFACIES; ifacies++)
      for (idir=0; idir<6; idir++)
      {
        ...
        {
          messerr("The Propagation Directional Speed is zero for: Fluid=%d - Facies=%d - Direction='%s'",
  ...
  for (ifacies=0; ifacies<NFACIES; ifacies++)
    for (ifluid=1; ifluid<NFLUIDS; ifluid++)
    {
      ...
      if (WT(ifacies+1,jfluid+1,1,DIR_UP) < WT(ifacies+1,ifluid+1,1,DIR_UP))
      {
        messerr("Error for the Z+ Propagation Directional Speed for Facies=%d:",
                ifacies + 1);
        messerr("Speed for Fluid=%d [%d] must not be smaller than Speed for Fluid=%d [%d]",
                jfluid+1, WT(ifacies+1,jfluid+1,1,DIR_UP),
                ifluid+1, WT(ifacies+1,ifluid+1,1,DIR_UP));
        error = 1;
      }
      
      /* Z- Speed */
      
      if (WT(ifacies+1,jfluid+1,1,DIR_DOWN) > WT(ifacies+1,ifluid+1,1,DIR_DOWN))
      {
        messerr("Error for the Z- Propagation Directional Speed for Facies=%d:",
                ifacies + 1);
        messerr("Speed for Fluid=%d [%d] must not be larger than Speed for Fluid=%d  [%d]",
                jfluid+1, WT(ifacies+1,jfluid+1,1,DIR_DOWN),
                ifluid+1, WT(ifacies+1,ifluid+1,1,DIR_DOWN));
        error = 1;
      }
    }
  return(error);
                                  ...
                                  double  volume_max)
{
  ...
  if (ind_facies < 0 || ind_facies > dbgrid->getFieldNumber() ||
      ind_fluid  < 0 || ind_fluid  > dbgrid->getFieldNumber())
  {
                              ...
                              double  *tab)
{
  ...
  if (ind_facies < 0 || ind_facies > dbgrid->getFieldNumber() ||
      ind_fluid  < 0 || ind_fluid  > dbgrid->getFieldNumber() ||
      ind_date   < 0 || ind_fluid  > dbgrid->getFieldNumber())
  {
