...
namespace gstlrn
{
  ...
  PPMT::~PPMT()
  {
    ...
    if (!_anams.empty())
    {
      for (Id i = 0; i < static_cast<Id>(_anams.size()); i++) delete _anams[i];
    }
    if (!_initAnams.empty())
    {
  ...
  {
    return new PPMT(
      ndir, flagPreprocessing, methodDir, methodTrans, nbpoly, alpha);
  }
  ...
  String PPMT::toString(const AStringFormat* strfmt) const
  {
    ...
    if (getMethodDir() == EDirGen::VDC)
      sstr << "- Using Van der Corput method for generating Directions"
           << std::endl;
    else
  ...
  double PPMT::_gaussianizeForward(
    double Yi,
    Id rank,
  ...
  double PPMT::_getGaussianDistance(
    const VectorDouble& Yi,
    const VectorInt& Ri,
    ...
    const VectorDouble& N0) const
  {
    ...
    for (Id ip = 0; ip < np; ip++)
    {
      ...
      {
        double value =
          Y->getValue(ip, idim) + scale * _dirmat->getValue(id, idim);
        Y->setValue(ip, idim, value);
      }
    }
  }

  void PPMT::_shiftBackward(
    AMatrix* Y,
    Id id,
    const AnamHermite* anam,
  ...
  void PPMT::_iterationForward(AMatrix* Y, const VectorDouble& N0, Id iter)
  {
    Id np = Y->getNRows();
    Id idmax = _serieAngle[iter];
    VectorDouble Y0(np, TEST);
  ...
  void PPMT::_iterationBackward(AMatrix* Y, const VectorDouble& N0, Id iter)
  {
    DECLARE_UNUSED(N0);
    Id np = Y->getNRows();
    Id idmax = _serieAngle[iter];
  ...
  Id PPMT::fitFromMatrix(AMatrix* Y, Id niter, bool verbose)
  {
    if (Y == nullptr)
    {
      messerr(
        "Input Argument 'Y' (matrix) should be provided. Nothing is done");
    }
    _ndim = Y->getNCols();
    _niter = niter;
    ...
    const NamingConvention& namconv)
  {
    ...
    {
      messerr(
        "This Multivariate Transform requires several variables to be defined");
      return 1;
    }
    ...
    const NamingConvention& namconv)
  {
    ...
    // Loop on the iterations
    for (Id iter = 0; iter < niter; iter++) _iterationForward(&Y, N0, iter);
    ...
    const NamingConvention& namconv)
  {
    ...
    {
      messerr(
        "This Multivariate Back-Transform requires several variables to be "
        "defined");
      return 1;
    }
    if (getMethodTrans() != EGaussInv::HMT)
    {
      messerr(
        "The PPMT back-trasform is only available when methodTrans = "
        "'hermite'");
      return 1;
    }
    if (!isFitted())
    ...
    for (Id iter = niter - 1; iter >= 0; iter--)
