...
namespace gstlrn
{
                         ...
                         const NamingConvention& namconv)
  {
    ...
    if (Y.empty())
    {
      messerr("This Multivariate Back-Transform requires several variables to "
              "be defined");
      return 1;
    }
    if (getMethodTrans() != EGaussInv::HMT)
    {
      messerr("The PPMT back-trasform is only available when methodTrans = "
              "'hermite'");
      return 1;
    }
    if (!isFitted())
