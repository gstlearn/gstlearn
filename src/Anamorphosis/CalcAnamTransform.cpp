...
namespace gstlrn
{
  ...
  bool CalcAnamTransform::_hasAnam(bool flagAny, const EAnam& anamType) const
  {
    ...
    {
      messerr("The argument 'anam' should be of type %s",
              anamType.getKey().data());
      return false;
    }
    return true;
  }
    ...
    if (_flagDisjKrig)
    {
      ...
      for (Id i = 0; i < nsel; i++)
        _renameVariable(1,
                        _iattSel + i,
                        ELoc::UNDEFINED,
                        _selectivity->getVariableName(i),
                        1);
      return true;
    }
  ...
  bool CalcAnamTransform::_run()
  {
    ...
    if (_flagCondExp)
    {
      return (!_conditionalExpectation(getDb(),
                                       _anam,
                                       _selectivity,
                                       _iattSel,
                                       _iptrEst[0],
    ...
    if (_flagUniCond)
    {
      ...
      return (!_uniformConditioning(getDb(),
                                    anam_hermite,
                                    _selectivity,
                                    _iattSel,
                                    _iptrEst[0],
  ...
  bool CalcAnamTransform::_FactorsToSelectivity()
  {
    ...
    switch (_anam->getType().toEnum())
    {
      case EAnam::E_HERMITIAN:
        anam_hermite->factor2Selectivity(getDb(),
                                         _selectivity,
                                         _iptrEst,
                                         _iptrStd,
                                         _iattSel);
      ...
      case EAnam::E_DISCRETE_DD:
        anam_discrete_DD->factor2Selectivity(getDb(),
                                             _selectivity,
                                             _iptrEst,
                                             _iptrStd,
                                             _iattSel);
      ...
      case EAnam::E_DISCRETE_IR:
        anam_discrete_IR->factor2Selectivity(getDb(),
                                             _selectivity,
                                             _iptrEst,
                                             _iptrStd,
                                             _iattSel);
                                                ...
                                                Id nbsimu)
  {
    ...
    if (!selectivity->isUsed(ESelectivity::T))
      db->deleteColumnsByUIDRange(selectivity->getAddressQTEst(ESelectivity::T,
                                                               iptr0),
                                  selectivity->getNQTEst(ESelectivity::T));
    if (!selectivity->isUsed(ESelectivity::Q))
      db->deleteColumnsByUIDRange(selectivity->getAddressQTEst(ESelectivity::Q,
                                                               iptr0),
                                             ...
                                             Id col_var)
  {
    ...
    for (Id iech = 0; iech < db->getNSample(); iech++)
    {
      ...
      for (Id icut = 0; icut < ncuts; icut++)
      {
        ...
        double ore =
          1.
          - law_cdf_gaussian((yc - yv * sv / r_coef)
   ...
   **
   *****************************************************************************/
  Id anamPointToBlock(AAnam* anam,
                      Id verbose,
                      double cvv,
                      double coeff,
                      double mu)
