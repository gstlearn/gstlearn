...
namespace gstlrn
{
  ...
  bool CalcAnamTransform::_hasAnam(bool flagAny, const EAnam& anamType) const
  {
    ...
    {
      messerr(
        "The argument 'anam' should be of type %s", anamType.getKey().data());
      return false;
    }
    return true;
  }

  bool CalcAnamTransform::_hasInputVarDefined(Id mode) const
  {
  ...
  bool CalcAnamTransform::_check()
  {
    ...
    if (_flagToFactors)
    {
      ...
      for (Id ifac = 0; ifac < nfact; ifac++)
        ...
        {
          messerr(
            "Error in the rank of the factor(%d): it should lie in [1,%d]",
            _ifacs[ifac], nmax);
          return false;
        }
      return true;
  ...
  bool CalcAnamTransform::_postprocess()
  {
    ...
    if (_flagDisjKrig)
    {
      ...
      for (Id i = 0; i < nsel; i++)
        _renameVariable(
          1, _iattSel + i, ELoc::UNDEFINED, _selectivity->getVariableName(i),
          1);
      return true;
    }
    ...
    if (_flagCondExp)
    {
      ...
      for (Id i = 0; i < nsel; i++)
        _renameVariable(
          1, _iattSel + i, ELoc::Z, _selectivity->getVariableName(i), 1);
      return true;
    }
  ...
  bool CalcAnamTransform::_run()
  {
    ...
    if (_flagCondExp)
    {
      return (!_conditionalExpectation(
        getDb(), _anam, _selectivity, _iattSel, _iptrEst[0], _iptrStd[0],
        _flagOK, _proba, _nbsimu));
    }
  ...
  bool CalcAnamTransform::_ZToFactors()
  {
    ...
    for (Id iech = 0; iech < getDb()->getNSample(); iech++)
    {
  ...
  bool CalcAnamTransform::_FactorsToSelectivity()
  {
    ...
    switch (_anam->getType().toEnum())
    {
      case EAnam::E_HERMITIAN:
        anam_hermite->factor2Selectivity(
          getDb(), _selectivity, _iptrEst, _iptrStd, _iattSel);
        return true;

      case EAnam::E_DISCRETE_DD:
        anam_discrete_DD->factor2Selectivity(
          getDb(), _selectivity, _iptrEst, _iptrStd, _iattSel);
        return true;

      case EAnam::E_DISCRETE_IR:
        anam_discrete_IR->factor2Selectivity(
          getDb(), _selectivity, _iptrEst, _iptrStd, _iattSel);
        return true;
   ...
   **
   *****************************************************************************/
  Id CalcAnamTransform::_conditionalExpectation(
    Db* db,
    AAnam* anam,
    ...
    Id nbsimu)
  {
    ...
    {
      if (_ceZ(
            db, anam_hermite, selectivity, iptr0, col_est, col_std, nbsimu,
            flag_OK))
        return 1;
    }
    ...
    {
      if (_ceT(
            1, db, selectivity, iptr0, col_est, col_std, ycuts, nbsimu,
            flag_OK))
        return 1;
    }
    ...
    {
      if (_ceQ(
            db, anam_hermite, selectivity, iptr0, col_est, col_std, ycuts,
            nbsimu, flag_OK))
        return 1;
    }
    ...
    {
      if (_ceT(
            2, db, selectivity, iptr0, col_est, col_std, ycuts, nbsimu,
            flag_OK))
        return 1;
    }
    ...
    {
      if (_ceQuant(
            db, anam_hermite, selectivity, iptr0, col_est, col_std, proba,
            flag_OK))
        return 1;
    }
   ...
   **
   *****************************************************************************/
  Id CalcAnamTransform::_uniformConditioning(
    Db* db,
    AnamHermite* anam,
    Selectivity* selectivity,
    Id iptr0,
   ...
   **
   *****************************************************************************/
  void CalcAnamTransform::_correctForOK(
    Db* db,
    Id iech,
    Id col_est,
    Id col_std,
   ...
   **
   *****************************************************************************/
  void CalcAnamTransform::_getVectorsForCE(
    Db* db,
    Id col_est,
    Id col_std,
    bool flag_OK,
    ...
    VectorDouble& krigstd)
  {
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      if (!db->isActive(iech)) continue;
      _correctForOK(
        db, iech, col_est, col_std, flag_OK, &krigest[iech], &krigstd[iech]);
    }
  }
    ...
    bool flag_OK)
  {
    ...
    Id iptrEst = selectivity->getAddressQTEst(ESelectivity::Z, iptr0);
    Id iptrStd = selectivity->getAddressQTStd(ESelectivity::Z, iptr0);
    for (Id iech = 0; iech < db->getNSample(); iech++)
    {
      if (!db->isActive(iech)) continue;
    ...
    bool flag_OK)
  {
    ...
    for (Id icut = 0; icut < selectivity->getNCuts(); icut++)
    {
      _getVectorsForCE(db, col_est, col_std, flag_OK, krigest, krigstd);
      ...
      Id iptrEst = selectivity->getAddressQTEst(ESelectivity::T, iptr0, icut);
      Id iptrStd = selectivity->getAddressQTStd(ESelectivity::T, iptr0, icut);
      Id iptrProba =
    ...
    bool flag_OK)
  {
    ...
    Id iptrQuant = selectivity->getAddressQTEst(ESelectivity::QUANT, iptr0);
    ...
    bool flag_OK)
  {
    ...
    for (Id icut = 0; icut < selectivity->getNCuts(); icut++)
    {
      ...
      Id iptrEst = selectivity->getAddressQTEst(ESelectivity::Q, iptr0, icut);
      Id iptrStd = selectivity->getAddressQTStd(ESelectivity::Q, iptr0, icut);
      for (Id iech = 0; iech < db->getNSample(); iech++)
      {
        if (!db->isActive(iech)) continue;
    ...
    const VectorDouble& ycuts)
  {
    ...
    for (Id icut = 0; icut < selectivity->getNCuts(); icut++)
    {
      Id iptrT = selectivity->getAddressQTEst(ESelectivity::T, iptr0, icut);
      Id iptrQ = selectivity->getAddressQTEst(ESelectivity::Q, iptr0, icut);
      Id iptrB = selectivity->getAddressQTEst(ESelectivity::B, iptr0, icut);
   ...
   **
   *****************************************************************************/
  Id CalcAnamTransform::_ceM(Db* db, const Selectivity* selectivity, Id iptr0)
  {
    double t, q;
    ...
    for (Id icut = 0; icut < selectivity->getNCuts(); icut++)
    {
      Id iptrT = selectivity->getAddressQTEst(ESelectivity::T, iptr0, icut);
      Id iptrQ = selectivity->getAddressQTEst(ESelectivity::Q, iptr0, icut);
      Id iptrM = selectivity->getAddressQTEst(ESelectivity::M, iptr0, icut);
    ...
    double mu)
  {
    ...
    {
      messerr(
        "Change of support coefficient (%lf) must lie between 0 and 1.", coeff);
      return 1;
    }
      ...
      case EAnam::E_HERMITIAN:
        ...
        if (FFFF(coeff))
        {
          ...
          if (verbose)
          {
            ...
            message(
              "Change of support coefficient = %lf\n",
