...
#define QT_FLAG(j) (QT_VARS(QT_EST, j) > 0 || QT_VARS(QT_STD, j) > 0)
...
namespace gstlrn
{
  ...
  Id AAnam::fitFromLocator(Db* db, const ELoc& locatorType)
  {
    ...
    {
      messerr(
        "The number of items for locator(%d) is %d. It should be 1",
   ...
   **
   *****************************************************************************/
  Id AAnam::rawToFactor(Db* db, Id nfactor, const NamingConvention& namconv)
  {
    CalcAnamTransform transfo(this);
    transfo.setDb(db);
    transfo.setFlagToFactors(true);
