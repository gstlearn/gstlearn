...
namespace gstlrn
{
                                  ...
                                  const VectorDouble& /*wt*/)
  {
    ...
    for (Id iclass = 0; iclass < nclass; iclass++)
    {
      ...
      else
      {
        ...
        setIRStatR(iclass,
                   (tprev > 0 && tcur > 0) ? 1. / tcur - 1. / tprev : 0.);
      }
                                        ...
                                        Id iptr0)
  {
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      {
        selloc->setQest(ncleff - 1,
                        getIRStatZ(ncleff) * selloc->getTest(ncleff - 1));
        for (Id ivar = ncleff - 2; ivar >= 0; ivar--)
        {
          selloc->setQest(ivar,
                          selloc->getQest(ivar + 1)
                            + getIRStatZ(ivar + 1)
                                * (selloc->getTest(ivar)
                                   - selloc->getTest(ivar + 1)));
        }
      }
