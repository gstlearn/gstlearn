...
namespace gstlrn
{
  ...
  AnamDiscreteIR*
    AnamDiscreteIR::createFromNF(const String& NFFilename, bool verbose)
  {
  ...
  Id AnamDiscreteIR::fitFromArray(
    ...
    const VectorDouble& /*wt*/)
  {
    ...
    if (_stats_residuals(
          false, nech, tab, &nsorted, &mean, residuals.data(), T.data(),
          Q.data()))
      return 1;
  ...
  Id AnamDiscreteIR::_stats_residuals(
    Id verbose,
    Id nech,
    const VectorDouble& tab,
    Id* nsorted,
  ...
  double AnamDiscreteIR::computeVariance(double sval) const
  {
    ...
    for (Id iclass = 0; iclass < nclass - 1; iclass++)
    {
      ...
      double resid = (tprev > 0 && tcur > 0)
                     ? 1. / pow(tcur, sval) - 1. / pow(tprev, sval)
                     : 0.;
      var += b * b * resid;
    }
    return (var);
   ...
   **
   *****************************************************************************/
  Id AnamDiscreteIR::factor2Selectivity(
    Db* db,
    ...
    Id iptr0)
  {
    ...
    {
      messerr(
        "Number of factors (%d) must be smaller or equal to Number of Classes",
        nfactor, getNClass());
      return 1;
    }
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      {
        selloc->setQest(
          ncleff - 1, getIRStatZ(ncleff) * selloc->getTest(ncleff - 1));
        for (Id ivar = ncleff - 2; ivar >= 0; ivar--)
        {
          selloc->setQest(
            ivar, selloc->getQest(ivar + 1)
                    + getIRStatZ(ivar + 1)
      ...
      if (selloc->isUsedStD(ESelectivity::Q))
      {
        for (Id ivar = 0; ivar < ncleff; ivar++)
        {
          ...
          double prod =
            getIRStatB(ivar + 1) + getIRStatZ(ivar + 1) * getIRStatT(ivar + 1);
          total *= prod * prod;
      ...
      if (selloc->isUsedStD(ESelectivity::Z))
      {
        ...
        {
          double prod =
            db->getArray(iech, cols_std[ivar]) * getIRStatB(ivar + 1);
          total += prod * prod;
        }
        zstdev = sqrt(total);
      }
