...
namespace gstlrn
{
  ...
  String AnamDiscrete::toString(const AStringFormat* /*strfmt*/) const
  {
    ...
    if (!FFFF(_mean)) sstr << "Mean              = " << _mean << std::endl;
    if (!FFFF(_variance))
      sstr << "Variance          = " << _variance << std::endl;

    sstr << std::endl;
    sstr << toStrMatrix(
      "Cutoffs", VectorString(), VectorString(), true, _nCut, 1, _zCut);

    sstr << toStrMatrix(
      String(), VectorString(), VectorString(), true, getNClass(), getNElem(),
      getStats().getValues());
  ...
  bool AnamDiscrete::_serializeAscii(std::ostream& os) const
  {
    ...
    ret = ret
       && _tableWrite(
            os, "DD Stats", getNClass() * getNElem(), getStats().getValues());
    return ret;
  }
  ...
  void AnamDiscrete::setStats(const VectorDouble& stats)
  {
    ...
    {
      messerr(
        "Argument 'stats' incorrect. Its dimension (%d) should be %d * %d",
  ...
  bool AnamDiscrete::serializeH5(H5::Group& grp) const
  {
    ...
    ret =
      ret && SerializeHDF5::writeVec(anamG, "Stats", getStats().getValues());
