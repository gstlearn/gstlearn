...
namespace gstlrn
{
  ...
  MatrixDense AnamDiscreteDD::factors_maf(bool verbose)
  {
    ...
    for (Id icut = 0; icut < ncut; icut++)
      for (Id iclass = 0; iclass < nclass; iclass++)
      {
        ...
        tab.setValue(iclass,
                     icut,
                     ((bval - cval) - prop) / sqrt(prop * (1. - prop)));
      }
                                          ...
                                          VectorDouble& eigval)
  {
    ...
    for (Id iclass = 0; iclass < nclass; iclass++)
    {
      if (veca[iclass] < 0)
      {
        messerr("Diffusion hypothesis invalid: superdiagonal term (class=%d) "
                "is not positive",
                iclass + 1);
        return chi;
      }
      if (vecb[iclass] < 0)
      {
        messerr("Diffusion hypothesis invalid: subdiagonal term (class=%d) is "
                "not positive",
                iclass + 1);
        return chi;
      }
  ...
  void AnamDiscreteDD::_lambdaToMul()
  {
    ...
    for (Id iclass = 0; iclass < nclass; iclass++)
      setDDStatMul(iclass,
                   pow(mu / (mu + getDDStatLambda(iclass)), scoef / 2.));
  }
  ...
  VectorDouble AnamDiscreteDD::factors_mod()
  {
    ...
    for (Id iclass = 0; iclass < nclass; iclass++)
      for (Id jclass = 0; jclass < nclass; jclass++)
      {
        ...
        ptab.setValue(iclass,
                      jclass,
                      pow(getDDStatLambda(jclass), iclass) / sqrt(value));
      }
      ...
      for (Id jclass = 0; jclass < nclass; jclass++) switch (mode)
        {
          ...
          case 2:
            mati.setValue(iclass,
                          jclass,
                          (jclass >= iclass) * getDDStatZmoy(jclass));
            break;
  ...
  bool AnamDiscreteDD::_serializeAscii(std::ostream& os) const
  {
    ...
    ret = ret
       && _tableWrite(os,
                      "PCA Z2Y",
                      getNCut() * getNCut(),
                      getPcaZ2Fs().getValues());
    ret = ret
       && _tableWrite(os,
                      "PCA Y2Z",
                      getNCut() * getNCut(),
                      getPcaF2Zs().getValues());
    return ret;
  }
                                        ...
                                        Id iptr0)
  {
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      {
        selloc->setQest(nclass - 1,
                        getDDStatZmoy(ncleff - 1)
                          * selloc->getTest(ncleff - 1));
        for (Id iclass = ncleff - 2; iclass >= 0; iclass--)
          selloc->setQest(iclass,
                          selloc->getQest(iclass + 1)
                            + getDDStatZmoy(iclass)
                                * (selloc->getTest(iclass)
                                   - selloc->getTest(iclass + 1)));
      }
