...
namespace gstlrn
{
  ...
  String Regression::toString(const AStringFormat* strfmt) const
  {
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
      sstr << "- Explanatory Variable #" << ivar + 1 << " = " << _coeffs[ecr++]
           << std::endl;
    ...
    const Model* model)
  {
    ...
    switch (mode)
    {
      ...
      case 2:
        ...
        {
          messerr(
            "The number of Drift equations in the Model should be positive");
          return false;
        }
    }
    return true;
   ...
   */
  VectorDouble
    regressionDeming(const VectorDouble& x, const VectorDouble& y, double delta)
  {
    ...
    const Model* model)
  {
    ...
    switch (mode)
    {
      ...
      case 1:
        ...
        if (flagCst) size++;
        break;
      case 2: size = model->getNDrift(); break;
    }
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      if (_regressionLoad(
            db1, db2, iech, icol0, icols, mode, flagCst, model, &value, x))
        continue;
    ...
    {
      prod -= 2. * x[i] * b[i];
      for (Id j = 0; j < size; j++) prod += x[i] * x[j] * a.getValue(i, j);
    }
    ...
    const Model* model) const
  {
    ...
    for (Id iech = 0; iech < db1->getNSample(); iech++)
    {
      if (db1->isActive(iech))
      {
        ...
        {
          for (Id i = 0; i < size; i++) value -= x[i] * getCoeff(i);
        }
      }
