...
namespace gstlrn
{
      ...
      for (Id i = 0; i < noper; i++)
      {
        if (neff > 0)
        {
          if (opers[i] == EStatOption::NUM)
            ...
            tab.push_back(static_cast<double>(nperc)
                          / static_cast<double>(neff));
          else if (opers[i] == EStatOption::QUANT)
            tab.push_back(_getQuantile(local, neff, proba));
          else if (opers[i] == EStatOption::Q)
            tab.push_back(metal / static_cast<double>(neff));
          else if (opers[i] == EStatOption::M)
            tab.push_back((nperc > 0) ? metal / static_cast<double>(nperc)
                                      : TEST);
          else if (opers[i] == EStatOption::B)
            tab.push_back((!FFFF(vmin))
                            ? (metal - vmin) / static_cast<double>(neff)
                            : TEST);
   ...
   */
  Table dbStatisticsCorrel(Db* db,
                           const VectorString& names,
                           bool flagIso,
                           const String& title)
  {
                          ...
                          Id radius)
  {
    ...
    if (pos < 0 || pos >= ndim)
    {
      messerr("The rank of the 'montee' axis should lie between 1 and %d",
              ndim);
      return 1;
    }
