...
namespace gstlrn
{
    ...
    VectorDouble& trans)
  {
    ...
    Id ifac1 = static_cast<Id>(
      dbin->getZVariable(dbin->getGrid().indiceToRank(indg), 0));
    indg[pos] += orient;
    Id ifac2 = static_cast<Id>(
      dbin->getZVariable(dbin->getGrid().indiceToRank(indg), 0));
    indg[pos] -= orient;
   ...
   **
   *****************************************************************************/
  void _scaleAndAffect(Db* dbout, Id iptr, Id iech, Id nitem, VectorDouble& tab)
  {
    double value;

    double total = 0.;
    for (Id ifac = 0; ifac < nitem; ifac++) total += tab[ifac];
   ...
   *****************************************************************************/
  bool
    _operExists(const std::vector<EStatOption>& opers, const EStatOption& refe)
  {
  ...
  void _refactor(Id ncol, VectorDouble& tab)
  {
    ...
    for (ix = 0; ix < ncol; ix++)
      for (iy = 0; iy < ncol; iy++) tab[ix * ncol + iy] = tab[iy * ncol + iy];
  }
   ...
   *****************************************************************************/
  void
    _copyResults(Id nx, Id ny, const VectorDouble& tabin, VectorDouble& tabout)
  {
    Id ix, iy, lec;

    for (ix = lec = 0; ix < nx; ix++)
      for (iy = 0; iy < ny; iy++, lec++) tabout[lec] = tabin[lec];
  }
      ...
      for (Id i = 0; i < noper; i++)
      {
        ...
        if (neff > 0)
        {
          if (opers[i] == EStatOption::NUM)
            ...
            tab =
              (!FFFF(vmin)) ? (metal - vmin) / static_cast<double>(neff) : TEST;
          else
            return;
        }
    ...
    Id radius)
  {
    ...
    {
      messerr(
        "The rank of the 'montee' axis should lie between 1 and %d", ndim);
      return 1;
    }
    ...
    const String& radix)
  {
    ...
    if (_operExists(opers, EStatOption::NUM)) printElement("Number");
    if (_operExists(opers, EStatOption::MINI)) printElement("Minimum");
    if (_operExists(opers, EStatOption::MAXI)) printElement("Maximum");
    if (_operExists(opers, EStatOption::MEAN)) printElement("Mean");
    if (_operExists(opers, EStatOption::STDV)) printElement("St. Dev.");
    if (_operExists(opers, EStatOption::VAR)) printElement("Variance");
    message("\n");
   ...
   *
   */
  VectorDouble dbStatisticsPerCell(
    Db* db,
    DbGrid* dbgrid,
    const EStatOption& oper,
    const String& name1,
    ...
    const VectorDouble& cuts)
  {
    ...
    VectorDouble maxi;
    VectorDouble cutval;
    if (flag_s1) s1.resize(nxyz, 0.);
    if (flag_s2) s2.resize(nxyz, 0.);
    if (flag_v1) v1.resize(nxyz, 0.);
    if (flag_v2) v2.resize(nxyz, 0.);
    if (flag_v12) v12.resize(nxyz, 0.);
    if (flag_mini) mini.resize(nxyz, TEST);
    if (flag_maxi) maxi.resize(nxyz, TEST);
    if (flag_t || flag_q)
    {
      if (ncut <= 0)
      {
    ...
    for (Id i = 0; i < nxyz; i++)
    {
      ...
      if (ratio <= 0)
      {
        ...
        if (flag_t || flag_q)
          for (Id icut = 0; icut < ncut; icut++) cutval[icut + i * ncut] = TEST;
      }
      else
      {
    ...
    const String& title)
  {
    ...
    else if (oper == EStatOption::VAR || oper == EStatOption::CORR
    ...
    Id iptr0)
  {
    ...
    Id count =
      static_cast<Id>(pow(2. * radius + 1., static_cast<double>(ndim)));
    ...
    for (Id iuid = 0; iuid < nuid; iuid++)
    {
      ...
      for (Id iech = 0; iech < db->getNSample(); iech++)
      {
        ...
        if (!db->isActive(iech)) continue;
        db->getCoordinatesInPlace(coor, iech);
        if (dbgrid->getGrid().coordinateToIndicesInPlace(coor, indg0, true))
          continue;
        double value = db->getArray(iech, juid);
        if (FFFF(value)) continue;
   ...
   **
   *****************************************************************************/
  VectorVectorInt correlationPairs(
    Db* db1,
    Db* db2,
    const String& name1,
    const String& name2,
   ...
   *****************************************************************************/
  Id
    correlationIdentify(Db* db1, Db* db2, Id icol1, Id icol2, Polygons* polygon)
  {
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      message(
        "Sample #%d - Variable #1=%lf - Variable #2=%lf\n", iech + 1, val1,
        val2);
      number++;
    }
