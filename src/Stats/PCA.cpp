...
namespace gstlrn
{
  ...
  void PCA::_mafFunctions(bool verbose)
  {
    ...
    A.invert();
    for (Id ifac = 0; ifac < nvar; ifac++)
      for (Id ivar = 0; ivar < nvar; ivar++) _setF2Z(ivar, ifac, A(ivar, ifac));
  ...
  String PCA::toString(const AStringFormat* strfmt) const
  {
    ...
    if (dsf.getflagStats())
    {
      ...
      if (_gh.size() > 0) sstr << toStrMatrix("Variogram Matrix at lag h", _gh);
  ...
  Id PCA::dbF2Z(Db* db, bool verbose, const NamingConvention& namconv)
  {
    if (db == nullptr)
    {
      messerr("You must define 'Db'");
   ...
   **
   *****************************************************************************/
  void PCA::_calculateNormalization(
    const Db* db,
    ...
    bool flag_nm1)
  {
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      for (Id ivar = 0; ivar < nvar; ivar++)
        for (Id jvar = 0; jvar <= ivar; jvar++)
          _c0.setValue(
            jvar, ivar, _c0.getValue(jvar, ivar) + data1[ivar] * data1[jvar]);
    }
    ...
    if (verbose) printMatrix(_c0, "Variance-Covariance matrix for distance 0");
  }
    ...
    bool flag_scale)
  {
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
    {
      if (flag_center) data[ivar] -= mean[ivar];
      if (flag_scale && sigma[ivar] > 0.) data[ivar] /= sigma[ivar];
    }
  }
    ...
    bool flag_scale)
  {
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
    {
      ...
      if (flag_scale) data[ivar] *= sigma[ivar];
      if (flag_center) data[ivar] += mean[ivar];
    }
  }
  ...
  VectorString PCA::_getNames(const Db* db)
  {
    ...
    for (Id ivar = 0, nvar = db->getNLoc(ELoc::Z); ivar < nvar; ivar++)
    {
      for (Id iversion = 0; iversion < db->getNVersionsByLocator(ELoc::Z, ivar);
           iversion++)
      {
        String name = db->getNameByLocator(ELoc::Z, ivar, iversion, true);
