...
namespace gstlrn
{
  ...
  Id PCA::dbZ2F(Db* db, bool verbose, const NamingConvention& namconv)
  {
    ...
    if (nvar != _nVar)
    {
      messerr("The number of Z variables (%d) does not match the number of "
              "variables in PCA (%d)",
              nvar,
              _nVar);
      return 1;
    ...
    if (verbose)
    {
      ...
      dbStatisticsPrint(db,
                        names,
                        {},
                        true,
                        true,
                        "Statistics on Factors",
                        "Factor");
    }
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      for (Id ivar = 0; ivar < nvar; ivar++)
        for (Id jvar = 0; jvar <= ivar; jvar++)
          _c0.setValue(jvar,
                       ivar,
                       _c0.getValue(jvar, ivar) + data1[ivar] * data1[jvar]);
    }
