...
#define IMAGES(ix, iy, iz)                                                     \
  ((option == BLOCK) ? imagtmp.getValue(ix, iy, iz)                            \
                     : imagin.getValue(ix, iy, iz))
...
#define COMPNUM(i, j, k) (compnum[GRID_ADD(i, j, k)])
   ...
   **
   *****************************************************************************/
  Id _st_morpho_label_size(
    const VectorDouble& compnum,
    Id nbcomp,
    VectorInt& sizes)
  {
    Id total = 0;
    ...
    bool verbose)
  {
    ...
    VectorDouble compnum;
    Id id[26][3] = {{-1, 00, 00}, {00, -1, 00}, {00, 00, -1}, {01, 00, 00},
                    {00, 01, 00}, {00, 00, 01}, {-1, -1, -1}, {00, -1, -1},
                    {01, -1, -1}, {-1, 00, -1}, {-1, 01, -1}, {00, 01, -1},
                    {01, 00, -1}, {01, 01, -1}, {-1, -1, 00}, {-1, 01, 00},
                    {01, -1, 00}, {01, 01, 00}, {-1, -1, 01}, {00, -1, 01},
                    {01, -1, 01}, {-1, 00, 01}, {-1, 01, 01}, {00, 01, 01},
                    {01, 00, 01}, {01, 01, 01}};
    Id ndel[2] = {6, 26};
    Id quantum = 100;
    ...
    bool verbose)
  {
    ...
    if (verbose)
      message("Union: %d and %d -> %d\n", nbin1, nbin2, morpho_count(imagout));
  }
   ...
   */
  void morpho_double2imageInPlace(
    const VectorInt& nx,
    ...
    bool verbose)
  {
    VectorInt NX = imagin.getNDimsExt(3);
    auto nxyz = imagin.getNPixels();
    if (verbose) message("Translation: %d / %d\n", morpho_count(imagin), nxyz);
    ...
    for (Id iz = 0; iz < imagin.getNDims(2); iz++)
      for (Id iy = 0; iy < imagin.getNDims(1); iy++)
        for (Id ix = 0; ix < imagin.getNDims(0); ix++)
        {
          ...
          switch (mode)
          {
            case 0: TAB(ix, iy, iz) = value; break;

            case 1: TAB(ix, iy, iz) += value; break;
    ...
    bool verbose)
  {
    ...
    for (Id i = 0; i < nxyz; i++) dist[i] = ABS(dist[i]);
  }
    ...
    bool verbose)
  {
    ...
    DbGrid* grid = DbGrid::create(
      nx, dx, x0, VectorDouble(), ELoadBy::SAMPLE, VectorDouble(),
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      for (Id idim = 0; idim < ndim; idim++) indret[ecr++] = indg1[idim];
    }
    ...
    if (verbose && nvois > 0)
    {
      ...
      for (Id i = 0; i < nvois; i++)
      {
        ...
        for (Id idim = 0; idim < ndim; idim++) message(" %2d", indret[ecr++]);
        message("\n");
      }
    }
        ...
        for (Id jx = -RADIUS[0]; jx <= RADIUS[0]; jx++)
          for (Id jy = -RADIUS[1]; jy <= RADIUS[1]; jy++)
          {
            ...
            a.updValue(
              0, 1, EOperator::ADD,
              xi
                * yi); // Do not perform upd(1,0) as it is already done due to symmetric matrix
    ...
    double hmax)
  {
    ...
    Id error = spill_point(
      dbgrid, ind_depth, ind_data, option, flag_up, verbose_step, hmax, &h, &th,
      &ix0, &iy0);
