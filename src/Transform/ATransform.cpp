...
namespace gstlrn
{
  ...
  {
    double x =
      y; // initialisation naturelle (si la fonction est proche de l’identité)
  ...
  void ATransform::condExpVec(constvect mu, constvect sigma, vect out, Id power)
    const
  {
  ...
  VectorDouble ATransform::condExpVec(
  ...
  {
    Id n = static_cast<Id>(in.size());
    for (Id i = 0; i < n; i++) out[i] = transform(in[i]);
  }

  void ATransform::inverseTransformVec(constvect in, vect out) const
  {
    Id n = static_cast<Id>(in.size());
    for (Id i = 0; i < n; i++) out[i] = inverseTransform(in[i]);
  }
