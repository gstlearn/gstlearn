...
namespace gstlrn
{
  ...
  *****************************************************************************/
   ...
   * @return
   */
  Id Rule::resetFromNumericalCoding(const VectorInt& n_type,
  ...
  Id Rule::setMainNodeFromNodNames(const VectorInt& nodes)
  {
    ...
    for (Id inode = 0; inode < nb_node; inode++)
    {
      ...
      if ((NODE_TYPE(inode) == THRESH_Y1
           && n1tab[NODE_RANK(inode) - 1] != nullptr)
          || (NODE_TYPE(inode) == THRESH_Y2
              && n2tab[NODE_RANK(inode) - 1] != nullptr))
      {
        messerr("Error: Node #%d (%s%d) has already been created",
                inode + 1,
                symbol[NODE_TYPE(inode)].c_str(),
                NODE_RANK(inode));
        return 1;
      }
      ...
      if (inode > 0)
      {
        ...
        if (found < 0)
        {
          messerr("Node #%d (%s%d) refers to unknown parent (%s%d)",
                  inode + 1,
                  symbol[NODE_TYPE(inode)].c_str(),
                  NODE_RANK(inode),
                  symbol[FROM_TYPE(inode)].c_str(),
                  FROM_RANK(inode));
          return 1;
        }
      }
  ...
  *****************************************************************************/
  ...
  VectorDouble Rule::getThreshFromRectangle(Id rect, Id* facies)
  {
    ...
    if (!_mainNode
           ->getThresh(2, rect, &rank, facies, &t1min, &t1max, &t2min, &t2max))
      return bounds;
    bounds.resize(4);
    bounds[0] = t1min;
    bounds[1] = t1max;
  ...
  *****************************************************************************/
  ...
  VectorInt Rule::getNodes() const
  {
    ...
    Id ny2_tot = 0;
    double prop_tot = 0.;
    _mainNode->getStatistics(&nb_node,
                             &nfac_tot,
                             &ny1_tot,
                             &ny2_tot,
                             &prop_tot);
  ...
  bool Rule::_serializeAscii(std::ostream& os) const
  {
    ...
    ret = ret
       && _recordWrite<double>(os,
                               "Correlation coefficient between GRFs",
                               getRho());
