...
namespace gstlrn
{
  ...
  Id RuleShift::resetFromNames(const VectorString& nodnames,
  ...
  bool RuleShift::_deserializeAscii(std::istream& is)
  {
    ...
    ret = ret
       && _recordRead<double>(is, "Lower Threshold for Shadow Rule", _shDown);
    ret = ret
       && _recordRead<double>(is, "Upper Threshold for Shadow Rule", _shDsup);
    ret =
      ret && _recordRead<double>(is, "Shift along first direction", _shift[0]);
    ret =
      ret && _recordRead<double>(is, "Shift along second direction", _shift[1]);
    ret =
      ret && _recordRead<double>(is, "Shift along third direction", _shift[2]);
    return ret;
  }
  ...
  bool RuleShift::_serializeAscii(std::ostream& os) const
  {
    ...
    ret = ret
       && _recordWrite<double>(os, "Lower Threshold for Shadow Rule", shdown);
    ret = ret
       && _recordWrite<double>(os, "Upper Threshold for Shadow Rule", shdsup);
    ret = ret
       && _recordWrite<double>(os, "Shift along first direction", shiftloc[0]);
    ret = ret
       && _recordWrite<double>(os, "Shift along second direction", shiftloc[1]);
    ret = ret
       && _recordWrite<double>(os, "Shift along third direction", shiftloc[2]);
    return ret;
  }
  ...
  String RuleShift::displaySpecific() const
  {
    ...
    sstr << "(With the 'Shift' option, only the first GRF is used)"
         << std::endl;
    return sstr.str();
  }
  ...
  *****************************************************************************/
                                ...
                                Id /*flag_stat*/) const
  {
    ...
    for (Id idim = 0; idim < ndim; idim++) wxyz[idim] = _xyz[idim];
  ...
  Id RuleShift::_st_shift_on_grid(Db* db, Id ndim, Id flag_grid_check) const
  {
    ...
    Id ntot = 0;
    for (Id idim = 0; idim < ndim; idim++) ntot += ABS(_ind1[idim]);
    if (ntot <= 0)
    {
                               ...
                               Id nbsimu) const
  {
    ...
    for (iech = 0; iech < nech; iech++)
    {
      ...
      if (rule_thresh_define(propdef,
                             ...
                             &t2max))
        return (1);
      dbin->setLocVariable(ELoc::L,
                           iech,
                           get_rank_from_propdef(propdef, ipgs, igrf),
                           t1min);
      dbin->setLocVariable(ELoc::U,
                           iech,
                           get_rank_from_propdef(propdef, ipgs, igrf),
                           t1max);
