...
namespace gstlrn
{
  ...
  void RuleShadow::_st_shadow_max(
    const Db* dbprop,
    Id flag_stat,
    double* sh_dsup_max,
   ...
   **
   *****************************************************************************/
  double RuleShadow::_st_grid_eval(
    DbGrid* dbgrid,
    Id isimu,
    Id icase,
    Id nbsimu,
    ...
    Id nbsimu) const
  {
    ...
    check_mandatory_attribute(
      "rule_gaus2fac_result_shadow", dbout, ELoc::FACIES);
      ...
      if (facies == SHADOW_SHADOW)
      {
        ...
        while (!valid)
        {
          ...
          for (idim = 0; idim < dbin->getNDim(); idim++)
          {
            ...
            dbin->setCoordinate(
              jech, idim, dbin->getCoordinate(iech, idim) - dval);
            dist += dval * dval;
          }
          dist = sqrt(dist);
