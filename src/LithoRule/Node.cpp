...
namespace gstlrn
{
  ...
  Node::Node(const Node& m)
    ...
    , _cdf2max(m._cdf2max)
  {
    if (m._r1 != nullptr) _r1 = new Node(*m._r1);
    if (m._r2 != nullptr) _r2 = new Node(*m._r2);
  }
  ...
  *****************************************************************************/
  ...
  {
    std::stringstream sstr;
    if (_r1 != nullptr) sstr << _r1->nodePrint(flagProp, flagThresh);
    if (_r2 != nullptr) sstr << _r2->nodePrint(flagProp, flagThresh);
  ...
  *****************************************************************************/
  ...
  {
    std::stringstream sstr;
    if (_r1 != nullptr) sstr << _r1->nodePrintShadow(flagProp, flagThresh);
    if (_r2 != nullptr) sstr << _r2->nodePrintShadow(flagProp, flagThresh);

    switch (_orient)
    {
      case THRESH_IDLE:
        if (_facies == SHADOW_ISLAND) sstr << "Node " << _nodnam << " - Island";
        if (_facies == SHADOW_WATER) sstr << "Node " << _nodnam << " - Water";
        if (_facies == SHADOW_SHADOW) sstr << "Node " << _nodnam << " - Shadow";
  ...
  *****************************************************************************/
    ...
    double* prop_tot)
  {
    ...
    switch (_orient)
    {
      ...
      case THRESH_Y1: (*ny1_tot) += 1; break;

      case THRESH_Y2: (*ny2_tot) += 1; break;
    }
  }
  ...
  *****************************************************************************/
  Id Node::isValid(VectorInt& facies)
  {
    ...
    {
      messerr(
        "Error in the facies rank (%d) at node %s: it should lie within [1,%d]",
  ...
  *****************************************************************************/
  ...
  Id Node::proportionDefine(const VectorDouble& props)
  {
    ...
    if (
      _orient
      == THRESH_IDLE) // [FO] 21/09/13 => nothing to do in splitting nodes (facies=0)
    {
      Id facies = _facies;
      ...
      if (!FFFF(propval))
      {
        ...
        {
          messerr(
            "Wrong proportion for facies %d (%lf): it should lie in [0,1]",
            facies, propval);
          return (1);
        }
  ...
  *****************************************************************************/
  double Node::_threshDichotomy(double rho) const
  {
    ...
    do
    {
      ...
      mvndst(
        2, low, sup, infin, &rho, maxpts, abseps, releps, &error, &prop, &ier);
