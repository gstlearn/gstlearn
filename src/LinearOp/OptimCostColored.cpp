...
namespace gstlrn
{
  ...
  **
  *****************************************************************************/
  VectorVectorDouble OptimCostColored::minimize(
    const VectorDouble& facies,
    ...
    double eps)
  {
    ...
    try
    {
      ...
      for (Id level = 0; level < nlevel; level++)
      {
        ...
        if (propfac.empty()) my_throw("Error in 'OptimCostBinary'");
   ...
   */
  void OptimCostColored::_getFaciesToIndic(
    const VectorDouble& facies,
  ...
  *****************************************************************************/
  Id OptimCostColored::_checkFacies(const VectorDouble& facies) const
  {
    ...
    for (Id i = 0; i < npoint; i++)
    {
      ...
      {
        messerr(
          "Error: At sample #%d - Facies (%d) should be in [1,%d]", i + 1,
          static_cast<Id>(facies[i]), _nprop);
        nerr++;
      }
    }
    return (nerr > 0);
  ...
  *****************************************************************************/
  Id OptimCostColored::_checkSplits(const VectorVectorInt& splits)
  {
    ...
    for (Id level = 0; level < nlevel; level++)
    {
      for (Id ip = 0; ip < _nprop; ip++)
      {
        if (splits[level][ip] != 0 && splits[level][ip] != 1
            && splits[level][ip] != 2)
        {
          messerr(
            "For Level=%d/%d and Facies=%d/%d, argument 'splits' is invalid "
            "(%d)",
    ...
    for (Id level = 0; level < nlevel; level++)
    {
      ...
      if (none <= 0 || ntwo <= 0)
      {
        messerr("At level #%d, there must be at least a 1 and a 2", level + 1);
        nerr++;
      }
    }
    if (nerr > 0) goto label_error;
    ...
    for (Id level = 1; level < nlevel; level++)
    {
      ...
      for (Id ip = 0; ip < _nprop; ip++)
      {
        ...
        else
        {
          ...
          {
            messerr(
              "Non-zero values at level #%d should share same value previous "
              "level",
              level + 1);
            nerr++;
          }
  ...
  *****************************************************************************/
  Id OptimCostColored::_checkMeanProportions(const VectorDouble& meanprops)
  {
    ...
    double total = 0.;
    for (Id ip = 0; ip < _nprop; ip++) total += meanprops[ip];
  ...
  **
  *****************************************************************************/
  void OptimCostColored::_copyMultProportions(
    Id level,
    Id ip,
