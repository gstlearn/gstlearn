  ...
  *****************************************************************************/
  ...
  Id ASPDEOp::_addToDest(const constvect inv, vect outv) const
  {
    ...
    Id status = _QKriging->addToDest(
      inv,
      outv); // TODO: find why outv is set to zero in multistructure case
                                   ...
                                   const VectorDouble& driftCoeffs)
  {
    ...
    {
      messerr("Error in number of Rows of drift matrix (%d) and size of data "
              "vector (%d)",
              nrows,
              Z.size());
      return 1;
    ...
    {
      messerr("Error in number of Columns of drift matrix (%d) and size of "
              "drift coefficients (%d)",
              ncols,
              driftCoeffs.size());
      return 1;
    }
  ...
  void ASPDEOp::_preparePoly(Chebychev& logPoly, Id ndiscr) const
  {
    ...
    logPoly.fit([](double val) { return log(val); },
                a,
                b,
                2 * EPSILON4 / (a + b));
  }
