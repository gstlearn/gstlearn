...
{
  ASPDEOp::ASPDEOp(
    const IPrecisionOp* const popKriging,
  ...
  *****************************************************************************/
  ...
  VectorDouble
    ASPDEOp::kriging(const VectorDouble& dat, const IProj* proj) const
  {
  ...
  Id ASPDEOp::centerDataByDriftMat(
    VectorDouble& Z,
    ...
    const VectorDouble& driftCoeffs)
  {
    ...
    {
      messerr(
        "Error in number of Rows of drift matrix (%d) and size of data vector "
        "(%d)",
        nrows, Z.size());
      return 1;
    }
    ...
    {
      messerr(
        "Error in number of Columns of drift matrix (%d) and size of drift "
        "coefficients (%d)",
  ...
  Id ASPDEOp::centerDataByMeanVec(VectorDouble& Z, const VectorDouble& meanVec)
  {
    if (static_cast<Id>(Z.size()) != static_cast<Id>(meanVec.size()))
    {
      messerr(
        "Error in size of data vector (%d) and size of mean vector (%d)",
        Z.size(), meanVec.size());
      return 1;
    }
    ...
    bool verbose) const
  {
    ...
    // Optional printout
    if (verbose) toStrVector("Drift coefficients", result);
  ...
  void ASPDEOp::_preparePoly(Chebychev& logPoly, Id ndiscr) const
  {
    ...
    logPoly.fit(
      [](double val) { return log(val); }, a, b, 2 * EPSILON4 / (a + b));
  }
  ...
  double ASPDEOp::computeTotalLogDet(Id nMC, Id seed) const
  {
    ...
    double result = TEST;
    if (!FFFF(a1) && !FFFF(a2) && !FFFF(a3))
      result = a1 - a2 - a3; // -a3 since a3 is log|invNoise|
    return result;
  }
} // namespace gstlrn
