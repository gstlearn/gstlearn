...
namespace gstlrn
{
  ...
  MultiGridSPDE::MultiGridSPDE(
    const CovAniso* cov,
    const DbGrid* gridfine,
  ...
  std::pair<DbGrid, bool> MultiGridSPDE::buildNextGrid(const DbGrid* dbfine)
  {
    ...
    next_db.reset(
      nxs, dxs, x0, angles, ELoadBy::fromKey("SAMPLE"), {}, {}, {}, false,
      false);
  ...
  void MultiGridSPDE::buildGridHierarchy(Id nlevels_max, Id n_rings)
  {
    ...
    for (Id i = 0; i < nlevels_max - 1; i++)
    {
      ...
      _solver->addLevel(
        std::move(pop),
        std::make_unique<const ProjMatrix>(std::move(prolongator)));
  ...
  ProjMatrix MultiGridSPDE::buildProlongator(
    const DbGrid* dbfine,
...
#pragma omp parallel for firstprivate(                                         \
    ...
    for (Id i = 0; i < dbfine->getNSample(); i++)
    {
      ...
      all_parents.erase(
        std::unique(all_parents.begin(), all_parents.end()), all_parents.end());
      ...
      cova.evalCovMatOptimInPlace(
        C, dbcoarse, *rkh, EKrigOpt::POINT, ECalcMember::LHS, tabwork);
      c0.resize(all_parents.size(), 1);

      cova.evalCovVecRHSInPlace(
        c0.getViewOnColumnModify(0), *rkh, i, EKrigOpt::POINT, pin, pout,
        tabwork);
...
#pragma omp parallel num_threads(nbthread)
  ...
  MultiGridSolver* createMultiGridSolverSPDE(
    const CovAniso* cov,
    DbGrid* grid,
    Id nlevels_max,
    Id n_rings,
