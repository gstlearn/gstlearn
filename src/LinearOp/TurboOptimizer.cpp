...
namespace gstlrn
{
  ...
  {
    static Id S2D[2][3][2] = {{{0, 0}, {1, 0}, {0, 1}},
                              {{0, 1}, {1, 0}, {1, 1}}};
    return S2D[icas][icorn][idim0];
  }

  void TurboOptimizer::_rankToIndice(Id rank, VectorInt& indice, bool minusOne)
    const
  {
  ...
  double
    TurboOptimizer::_indiceToCoordinate(Id idim0, const VectorInt& indice) const
  {
  ...
  void TurboOptimizer::_printVector(
  ...
  void TurboOptimizer::_printMatrix(
    const std::string& title,
    Id nrow,
    Id ncol,
    VectorDouble& uu,
  ...
  void
    TurboOptimizer::_invert_3x3(VectorDouble& uu, VectorDouble& vv, double tol)
  {
   ...
   */
  void TurboOptimizer::_prodMatrix(
    Id size,
  ...
  void TurboOptimizer::_prodMatrixVector(
    Id size,
    const VectorDouble& aa,
    const VectorDouble& bb,
    VectorDouble& cc)
  {
    ...
    {
      double value = 0.;
      for (Id j = 0; j < size; j++) value += aa[GETADR(size, i, j)] * bb[j];
      cc[i] = value;
    }
  }
  ...
  Id TurboOptimizer::_determineInternalGrid(bool verbose)
  {
    ...
    if (verbose)
    {
      ...
      std::cout << "- Dimension of Internal Square Grid = " << _nxred
                << std::endl;
    }
  ...
  void TurboOptimizer::run(bool verbose)
  {
    ...
    if (verbose) std::cout << "Scale = " << _scale << std::endl;
    ...
    if (verbose)
      _printMatrix("Template S Matrix", nvertex_red, nvertex_red, _S_T, 7);

    // Build Q matrix
    _Q_T = _buildQ(_S_T, _Blin, _Lambda_T);
    if (verbose)
      _printMatrix("Template Q Matrix", nvertex_red, nvertex_red, _Q_T, 6);
  ...
  VectorDouble TurboOptimizer::_buildTildeC() const
  {
    ...
    for (Id ip = 0; ip < nvertex; ip++) TildeC[ip] /= TO_ncorner;
  ...
  VectorDouble TurboOptimizer::_buildBlin() const
  {
    ...
    double gammap = 0.;
    for (Id i = 1; i < _param; i++) gammap += log(static_cast<double>(i));
    gammap = exp(gammap);
    double gammaa = 0.;
  ...
  VectorDouble TurboOptimizer::_buildQ(
    const VectorDouble& ss,
    ...
    const VectorDouble& lambda) const
  {
    ...
    // First term
    for (Id i = 0; i < nvertex; i++) qq[GETADR(nvertex, i, i)] = blin[0];
    for (Id i = 0; i < nvertex * nvertex; i++) bi[i] = ss[i];

    // Loop on the different terms
    for (Id iterm = 1; iterm < nblin; iterm++)
    {
      for (Id i = 0; i < nvertex * nvertex; i++) qq[i] += bi[i] * blin[iterm];
      if (iterm < nblin - 1)
      {
  ...
  void TurboOptimizer::printMeshes() const
  {
    ...
    std::cout << "Number of Vertices         = " << _getNVertices()
              << std::endl;
  ...
  void TurboOptimizer::_getRankInTemplate(
    VectorInt& indice1,
  ...
  VectorDouble
    TurboOptimizer::_getVectorFromTemplate(const VectorDouble& vecin) const
  {
    auto nvertex = _getNVertices();
    VectorDouble vecout(nvertex, 0.);
    if (!_isCalculated) my_throw("You must use the method 'run' beforehand");
  ...
  TripletND TurboOptimizer::_getMatrixFromTemplate(
    const VectorDouble& matin,
    Id nperline) const
  {
  ...
  Id TurboOptimizer::_coordinateToIndice(double x, double y, VectorInt& indice)
    const
  {
    if (static_cast<Id>(indice.size()) < 2)
      my_throw("Argument indice should have the correct size");
    ...
    VectorDouble& lambda) const
  {
    ...
    for (Id icorner = 0; icorner < TO_ncorner; icorner++)
    {
      ...
      for (Id idim = 0; idim < TO_ndim; idim++)
        lhs[GETADR(TO_ncorner, idim, icorner)] =
          _indiceToCoordinate(idim, indgg);
   ...
   */
  TripletND TurboOptimizer::interpolate(
   ...
   */
  VectorDouble TurboOptimizer::_expandTripletToMatrix(
    Id row_begin,
    Id row_end,
    Id col_begin,
    Id col_end,
    const TripletND& triplet) const
  {
    ...
    for (Id i = 0; i < size; i++)
    {
      ...
      if (irow >= row_begin && irow <= row_end && icol >= col_begin
          && icol <= col_end)
        mat[GETADR(nrows, irow - row_begin, icol - col_begin)] =
          triplet.values[i];
    }
    return mat;
  }
    ...
    Id col_end) const
  {
    ...
    if (row_end <= 0 || row_end >= _getNVertices())
      row_end = _getNVertices() - 1;
    if (col_end <= 0 || col_end >= _getNVertices())
      col_end = _getNVertices() - 1;

    VectorDouble temp =
      _expandTripletToMatrix(row_begin, row_end, col_begin, col_end, getS());
    Id nrows = row_end - row_begin + 1;
    Id ncols = col_end - col_begin + 1;
    _printMatrix(
      "Matrix S", nrows, ncols, temp, nper_batch, row_begin, col_begin);
  }
    ...
    Id col_end) const
  {
    ...
    VectorDouble temp =
      _expandTripletToMatrix(row_begin, row_end, col_begin, col_end, getQ());
    Id nrows = row_end - row_begin + 1;
    Id ncols = col_end - col_begin + 1;
    _printMatrix(
      "Matrix Q", nrows, ncols, temp, nper_batch, row_begin, col_begin);
  }
} // namespace gstlrn
