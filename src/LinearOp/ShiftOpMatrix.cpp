...
namespace gstlrn
{
    ...
    if (igparam < ndim)
    {
      ...
      for (Id idim = 0; idim < ndim; idim++)
      {
        ...
        else
        {
          temp.setValue(idim,
                        idim,
                        2. * _getCovAniso()->getScale(idim) / number);
        }
      }
  ...
  Id ShiftOpMatrix::_buildS(const AMesh* amesh, double tol)
  {
    ...
    for (Id imesh = 0; imesh < nmeshes; imesh++)
    {
      ...
      if (!flagSphere)
      {
        if (_prepareMatricesSVariety(amesh,
                                     imesh,
                                     coords,
                                     matM,
                                     matMtM,
                                     ...
                                     double tol)
  {
    ...
    ret = tab.insert(std::pair<std::pair<Id, Id>, double>(
      key,
      value)); // ret.second = false if key is already in the map
    if (!ret.second) ret.first->second += value;
  }
