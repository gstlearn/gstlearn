...
namespace gstlrn
{
  ...
  Id
    InvNuggetOp::_addSimulateToDest(const constvect whitenoise, vect outv) const
  {
    return _cholNuggetMatrix->addToDest(whitenoise, outv);
  }

  static void
    _addVerrConstant(MatrixSymmetric& sills, const VectorDouble& verrDef)
  {
  ...
  static void
    _checkMinNugget(MatrixSymmetric& sills, const VectorDouble& minNug)
  {
  ...
  static MatrixSymmetric _buildSillPartialMatrix(
    const MatrixSymmetric& sillsRef,
    Id nvar,
    Id ndef,
    ...
    else
    {
      ...
      for (Id idef = 0; idef < ndef; idef++)
        for (Id jdef = 0; jdef <= idef; jdef++)
          sills.setValue(
            idef, jdef, sillsRef.getValue(identity[idef], identity[jdef]));
    }
    return sills;
  }

  static Id _loadPositions(
    Id iech,
  ...
  double InvNuggetOp::_updateQuantities(MatrixSymmetric& sillsinv)
  {
    ...
    return std::accumulate(
      eigenvals.begin(), eigenvals.end(), 0.,
      [](const double acc, const double x) { return acc + std::log(x); });
  }
   ...
   */
  void InvNuggetOp::_buildInvNugget(
    const Db* db,
    Model* model,
    const SPDEParam& params)
  {
    ...
    if (!hasnugget)
    {
      ...
      cova = CovAniso::createIsotropicMulti(
        *model->getContext(), ECov::NUGGET, 0, sills);
    }
    ...
    if (flag_nostat_sill) cova->informDbInForSills(db);
    ...
    if (flag_verr)
    {
      for (Id iverr = 0; iverr < nverr && flag_uniqueVerr; iverr++)
      {
        ...
        if (static_cast<Id>(VH::unique(verr).size()) > 1)
          flag_uniqueVerr = false;
    ...
    VectorInt lastPositions(
      nvar, 0); // Track last found positions for optimization
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      ndef = _loadPositions(
        iech, index1, cumul, position, identity, &rank, lastPositions);
      if (ndef <= 0) continue;
      ...
      if (flag_constant)
      {
        ...
        {
          sillsInv[rank] =
            _buildSillPartialMatrix(sillsRef, nvar, ndef, identity);
          cholcache[rank].setMatrix(sillsInv[rank]);

          if (sillsInv[rank].invert() != 0) return;
          if (_flagEigVals)
          {
            if (FFFF(_logDeterminant)) _logDeterminant = 0.;
        ...
        _updateMatrix(
          sillsInv[rank], cholcache[rank], ndef, position, identity);
      }
      else
      {
    ...
    if (!hasnugget) delete cova;
  }

  void InvNuggetOp::_updateMatrix(
    MatrixSymmetric& invsill,
  ...
  {
    {
      for (Id idef = 0; idef < ndef; idef++)
        for (Id jdef = 0; jdef < ndef; jdef++)
        {
          ...
          if (idef >= jdef)
          {
            _cholNuggetMatrix->setValue(
              pidef, pjdef, cholsill.getLowerTriangle(idef, jdef));
          }
        }
    }
  }

  std::shared_ptr<MatrixSparse>
    buildInvNugget(Db* dbin, Model* model, const SPDEParam& params)
  {
