...
namespace gstlrn
{
  ...
  InvNuggetOp::InvNuggetOp(const Db* dbin,
                           Model* model,
  ...
  static MatrixSymmetric
    _buildSillPartialMatrix(const MatrixSymmetric& sillsRef,
                            Id nvar,
                            Id ndef,
    ...
    else
    {
      ...
      for (Id idef = 0; idef < ndef; idef++)
        for (Id jdef = 0; jdef <= idef; jdef++)
          sills.setValue(idef,
                         jdef,
                         sillsRef.getValue(identity[idef], identity[jdef]));
    }
    return sills;
  }
  ...
  double InvNuggetOp::_updateQuantities(MatrixSymmetric& sillsinv)
  {
    ...
    return std::accumulate(eigenvals.begin(),
                           eigenvals.end(),
                           0.,
   ...
   */
  void InvNuggetOp::_buildInvNugget(const Db* db,
                                    Model* model,
                                    const SPDEParam& params)
  {
    CholeskyDense chol;
    ...
    if (!hasnugget)
    {
      ...
      cova = CovAniso::createIsotropicMulti(*model->getContext(),
                                            ECov::NUGGET,
                                            0,
                                            sills);
    }
    ...
    VectorInt lastPositions(nvar,
                            0); // Track last found positions for optimization
    for (Id iech = 0; iech < nech; iech++)
    {
      if (!db->isActive(iech)) continue;

      // Count the number of variables for which current sample is valid
      ndef = _loadPositions(iech,
                            index1,
                            cumul,
                            position,
                            identity,
  ...
  void InvNuggetOp::_updateMatrix(MatrixSymmetric& invsill,
  ...
  {
    {
      for (Id idef = 0; idef < ndef; idef++)
        for (Id jdef = 0; jdef < ndef; jdef++)
        {
          ...
          if (idef >= jdef)
          {
            _cholNuggetMatrix->setValue(pidef,
                                        pjdef,
                                        cholsill.getLowerTriangle(idef, jdef));
          }
        }
    }
  }
