...
namespace gstlrn
{
  ...
  void ALinearOpMulti::evalInverse(
    const VectorVectorDouble& vecin,
    VectorVectorDouble& vecout) const
  {
    ...
    while (niter < _nIterMax && crit > _eps)
    {
      ...
      VectorHelper::linearCombinationVVDInPlace(
        1., vecout, alpha, _p, vecout); // x = x + alpha * p
      ...
      if (_nIterRestart > 0 && (niter + 1) % _nIterRestart == 0)
      {
        ...
        if (OptDbg::query(EDbg::CONVERGE))
          message(
            "Recomputing exact residuals after %d iterations (max=%d)\n", niter,
            _nIterMax);
      }
      else
        VectorHelper::linearCombinationVVDInPlace(
          1., _r, -alpha, _temp, _r); // r = r - alpha * Ap
      ...
      if (_precondStatus)
      {
        ...
        VectorHelper::linearCombinationVVDInPlace(
          1., _temp, rsnew / rsold, _p, _p); // p = z+beta p
      }
      else
      {
        rsnew = _r.innerProduct(_r);
        VectorHelper::linearCombinationVVDInPlace(
          1., _r, rsnew / rsold, _p, _p); // p = r+beta p
      }
      crit = rsnew / nb;
    ...
    {
      message(
        "-- Conjugate Gradient (precond=%d) : %d iterations (max=%d) "
        "(eps=%lg)\n",
