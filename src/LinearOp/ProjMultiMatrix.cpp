...
namespace gstlrn
{
  ...
  template<typename T>
  static std::vector<std::vector<const IProj*>>
    castToBase(const std::vector<std::vector<T>>& vec)
  {
   ...
   */
  ProjMultiMatrix*
    ProjMultiMatrix::createFromDbAndMeshes(const Db* db,
      ...
      for (Id imesh = 0; imesh < nmesh; imesh++)
        for (Id jvar = 0; jvar < nvar; jvar++)
        {
          ...
          if (ivar != jvar)
            ...
            stocker[ivar].emplace_back(
              ProjMatrix(db, meshes(imesh), kvar, verbose));
        }
    }
    return new ProjMultiMatrix(std::move(stocker));
  }

  std::vector<std::vector<const ProjMatrix*>>
    ProjMultiMatrix::create(std::vector<const ProjMatrix*>& vectproj,
                            Id nvariable)
  {
    ...
    for (Id i = 1; i < nlatent; i++)
    {
      if (vectproj[i]->getNPoint() != npoint)
      {
        ...
        messerr("Element %d has %d Point instead of %d.",
                i,
                vectproj[i]->getNPoint(),
                npoint);
        return result;
      }
    }
  ...
  ProjMultiMatrix::ProjMultiMatrix(ProjsStore&& proj, bool silent)
    : ProjMulti(castToBase(proj), silent)
  ...
  void ProjMultiMatrix::init()
  {
    ...
    for (Id i = 0; i < getNVariable(); i++)
    {
      ...
      for (Id j = 0; j < getNLatent(); j++)
      {
        if (_projs[i][j])
        {
          MatrixSparse::glueInPlace(&currentrow,
                                    &static_cast<const ProjMatrix&>(
                                      _projs[i][j].value().get()),
                                    0,
                                    1);
        }
        else
  ...
  {
    AMatrix::productInPlace(outv, _Proj, inv, true, false);
    return 0;
  }

  Id ProjMultiMatrix::_addMesh2point(const constvect inv, vect outv) const
  {
    AMatrix::productInPlace(outv, _Proj, inv, false, false);
    return 0;
  }
} // namespace gstlrn
