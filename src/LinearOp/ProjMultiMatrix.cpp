...
namespace gstlrn
{
    ...
    bool verbose)
  {
    ...
    {
      messerr(
        "Inconsistent number of meshes (%d) and structures (%d)", nmeshes,
        ncov);
      return nullptr;
    }
  ...
  std::vector<std::vector<const ProjMatrix*>> ProjMultiMatrix::create(
    std::vector<const ProjMatrix*>& vectproj,
    Id nvariable)
  {
    ...
    for (Id i = 1; i < nlatent; i++)
    {
      if (vectproj[i]->getNPoint() != npoint)
      {
        ...
        messerr(
          "Element %d has %d Point instead of %d.", i, vectproj[i]->getNPoint(),
          npoint);
        return result;
      }
    }
      ...
      for (Id j = 0; j < getNLatent(); j++)
      {
        if (_projs[i][j])
        {
          MatrixSparse::glueInPlace(
            &currentrow,
            &static_cast<const ProjMatrix&>(_projs[i][j].value().get()), 0, 1);
        }
        else
        {
