...
namespace gstlrn
{
    ...
    try
    {
      ...
      while (flagContinue)
      {
        ...
        if (OptDbg::query(EDbg::CONVERGE))
          message(
            "Iteration #%d (max=%d) - Cost=%lf - NormGrad=%lf (eps=%lg)\n",
  ...
  **
  *****************************************************************************/
  void OptimCostBinary::calculateGradient(
  ...
  *****************************************************************************/
  void OptimCostBinary::toggleSeismic(bool status)
  {
    ...
    if (_projSeis == nullptr || _projSeis->getNPoint() <= 0) return;
  ...
  **
  *****************************************************************************/
  double OptimCostBinary::_evaluateCost(
    const VectorDouble& indic,
    const VectorDouble& lambda)
  {
    double result = 0.;
  ...
  **
  *****************************************************************************/
  void OptimCostBinary::_evaluateGrad(
    const VectorDouble& indic,
    const VectorDouble& lambda,
    double* normgrad)
  {
  ...
  *****************************************************************************/
  void OptimCostBinary::_contributeSeismic(const VectorDouble& lambda)
  {
    ...
    for (Id i = 0; i < _projSeis->getNPoint(); i++) _works[i] -= _propSeis[i];
  }
