...
namespace gstlrn
{
  ...
  MatrixSparse
    PrecisionOpMultiMatrix::_prepareMatrixNoStat(Id icov,
                                                 const MatrixSparse* Q) const
  {
  ...
  void PrecisionOpMultiMatrix::_prepareMatrix()
  {
    ...
    for (Id istruct = 0; istruct < _getNCov(); istruct++)
    {
      const MatrixSparse* Q =
        static_cast<const PrecisionOpMatrix*>(_pops[istruct])->getQ();
  ...
  Id
    PrecisionOpMultiMatrix::_addToDest(const constvect vecin, vect vecout) const
  {
