...
namespace gstlrn
{
    ...
    bool flagStDev) const
  {
    ...
    if (flagStDev)
      for (Id iech = 0, ntarget = static_cast<Id>(vcur.size()); iech < ntarget;
           iech++)
  ...
  double CholeskySparse::computeLogDeterminant() const
  {
    ...
    const auto& diag =
      _factor
        ->vectorD(); // Diagonal of the LDL^t decomposition (don't multiply by 2.!)
    for (Id i = 0; i < _size; ++i) det += log(diag[i]);
    return det;
  }
   ...
   */
  Id CholeskySparse::_stdev(VectorDouble& vcur, const MatrixSparse* proj) const
  {
    ...
    SpRowMat P =
      Pcol; // copie creuse -> row‑major ; coût négligeable si P très creuse

    // Alias ----------------------------------------------------------------------
    using SpRowMat =
      Eigen::SparseMatrix<double, Eigen::RowMajor>; // P (row‑major)
