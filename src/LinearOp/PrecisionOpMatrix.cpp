...
namespace gstlrn
{
  ...
  void PrecisionOpMatrix::gradYQX(
    const constvect X,
    const constvect Y,
    vect result,
    const EPowerPT& power)
  {
    ...
    for (Id igparam = 0; igparam < soptmat->getNCovAnisoGradParam(); igparam++)
    {
  ...
  void PrecisionOpMatrix::gradYQXOptim(
    const constvect X,
    const constvect Y,
    vect result,
    const EPowerPT& power)
  ...
  Id PrecisionOpMatrix::_addSimulateToDest(
    const constvect whitenoise,
    vect outv) const
  {
  ...
  void
    PrecisionOpMatrix::evalInverse(const constvect vecin, VectorDouble& vecout)
  {
  ...
  void PrecisionOpMatrix::evalDerivOptim(
    vect outv,
    Id iapex,
    Id igparam,
    const EPowerPT& power)
