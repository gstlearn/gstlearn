...
namespace gstlrn
{
  ...
  void LogStats::statsShow(void) const
  {
    ...
    if (_choleskyNumber > 0)
      message(
        "Statistics - Cholesky decomposition (*%d) : %lf ms\n", _choleskyNumber,
        _choleskyTime);
    if (_directNumber > 0)
      message(
        "Statistics - Direct evaluation (*%d) : %lf ms\n", _directNumber,
        _directTime);
    if (_inverseCGNumber > 0)
      message(
        "Statistics - Inverse using Conjugate Gradient (*%d) : %lf ms (%d "
        "iterations)\n",
        _inverseCGNumber, _inverseCGTime, _inverseCGNIter);
    if (_inverseCholNumber > 0)
      message(
        "Statistics - Inverse using Cholesky decomposition (*%d) : %lf ms\n",
        _inverseCholNumber, _inverseCholTime);
    if (_inversePolyNumber > 0)
      message(
        "Statistics - Inverse using Polynomial expansion (*%d) : %lf ms\n",
        _inversePolyNumber, _inversePolyTime);
    if (_simulateNumber > 0)
      message(
        "Statistics - Simulation evaluation (*%d) : %lf ms\n", _simulateNumber,
        _simulateTime);
  }
