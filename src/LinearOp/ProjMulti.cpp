...
namespace gstlrn
{
  ...
  bool ProjMulti::_checkArg(const ProjVect& projs) const
  {
    if (projs.empty())
    {
      if (!_silent) messerr("projs is empty.");
      return true;
    }
    ...
    for (Id i = 1; i < nvariable; i++)
    {
      if (static_cast<Id>(projs[i].size()) != nlatent)
      {
        ...
        messerr(
          "Line %d has %d elements instead of %d.", i, projs[i].size(),
          nlatent);
        return true;
      }
    }
      ...
      for (Id j = fcol + 1; j < nlatent; j++)
      {
        if (projs[i][j])
        {
          if (projs[i][j]->get().getNPoint() != npoints)
          {
            ...
            messerr(
              "Element [%d,%d] should have Point Number = %d  instead of %d.",
      ...
      for (Id i = frow + 1; i < nvariable; i++)
      {
        if (projs[i][j])
        {
          if (projs[i][j]->get().getNApex() != nvertex)
          {
            ...
            messerr(
              "Element [%d,%d] should have Apex Number = %d  instead of %d.", i,
              j, nvertex, projs[i][j]->get().getNApex());
            return true;
          }
        }
      }
  ...
  static ProjMulti::ProjVect
    convert(const std::vector<std::vector<const IProj*>>& projs)
  {
  ...
  ProjMulti::ProjMulti(
    const std::vector<std::vector<const IProj*>>& projs,
    bool silent)
