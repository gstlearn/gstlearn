...
namespace gstlrn
{
  ...
  void CholeskyDense::solveMatInPlace(const MatrixDense& mat, MatrixDense& res)
    const
  {
    if (!isReady()) return;
   ...
   **
   *****************************************************************************/
  void CholeskyDense::matProductInPlace(
    Id mode,
    const MatrixDense& a,
    MatrixDense& x)
  {
    ...
    if (mode == 0)
    {
      ...
      for (Id irhs = 0; irhs < nrhs; irhs++)
        ...
        {
          val = 0.;
          for (Id j = i; j < neq; j++) val += _TL(j, i) * a.getValue(j, irhs);
    ...
    else if (mode == 1)
    {
      ...
      for (Id irhs = 0; irhs < nrhs; irhs++)
        ...
        {
          val = 0.;
          for (Id j = 0; j <= i; j++) val += _TL(i, j) * a.getValue(j, irhs);
          x.setValue(i, irhs, val);
        }
    }
    else if (mode == 2)
    {
      ...
      for (Id irhs = 0; irhs < nrhs; irhs++)
        ...
        {
          val = 0.;
          for (Id j = 0; j <= i; j++) val += a.getValue(irhs, j) * _TL(i, j);
    ...
    else if (mode == 3)
    {
      ...
      for (Id irhs = 0; irhs < nrhs; irhs++)
        ...
        {
          val = 0.;
          for (Id j = i; j < neq; j++) val += a.getValue(irhs, j) * _TL(j, i);
          x.setValue(irhs, i, val);
        }
    }
    else if (mode == 4)
   ...
   **
   *****************************************************************************/
  void CholeskyDense::normMatInPlace(
    Id mode,
    Id neq,
