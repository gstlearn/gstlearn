        ...
        {                                                                      \
          ...
          {                                                                    \
            ...
            VectorHelper::addMultiplyConstantInPlace(                          \
              TAB##Stat[icov].getmat(IVAR, JVAR),                              \
              i1,                                                              \
              i2,                                                              \
              iad_y);                                                          \
          }                                                                    \
...
namespace gstlrn
{
  ...
  *******************************************************************************/
  ...
  bool PrecisionOpMulti::_matchModelAndMeshes() const
  {
    ...
    {
      messerr("The number of meshes (%d) and the number of covariances (%d) do "
              "not match",
              _getNMesh(),
              _getNCov());
      return false;
  ...
  Id PrecisionOpMulti::_addToDest(const constvect vecin, vect vecout) const
  {
    ...
    if (_getNVar() > 1)
    {
      ...
      EVALOP(vecin,
             _workTot,
             _invCholSills,
             getUpperTriangleInverse,
             addToDest,
             iad_struct + jvar * napices,
             false,
             x,
             jvar,
             nvar,
  ...
  Id PrecisionOpMulti::_addSimulateToDest(const constvect vecin,
                                          vect vecout) const
  {
    if (!_checkReady()) return 1;
    EVALOP(vecin,
