...
namespace gstlrn
{
  ...
  bool CalcGlobal::_check()
  {
    ...
    {
      messerr(
        "This method requires the model to be a 'Model' (not a ModelGeneric)");
      return false;
    }
    ...
    {
      messerr(
        "The target variable (%d) must lie between 1 and the number of "
        "variables (%d)",
  ...
  Id CalcGlobal::_globalKriging()
  {
    ...
    VectorDouble Z = dbin->getValuesByRanks(
      sampleRanks, _modelLocal->getMeans(), !_modelLocal->hasDrift());
    if (_modelLocal->evalCovMatSymInPlaceFromIdx(
          Sigma, dbin, sampleRanks, &mode, false))
      return 1;
    if (_modelLocal->evalDriftMatByRanksInPlace(
          X, dbin, sampleRanks, ECalcMember::LHS))
      return 1;
    ...
    for (Id iech = 0, nech = dbout->getNSample(); iech < nech; iech++)
    {
      ...
      if (_modelLocal->evalCovMatRHSInPlaceFromIdx(
            Sigma0, dbin, dbout, sampleRanks, iech, krigopt, false))
        return 1;
      if (_modelLocal->evalDriftMatByTargetInPlace(X0, dbout, iech, krigopt))
        return 1;
  ...
  Id CalcGlobal::_globalArithmetic()
  {
    ...
    double cxx = _modelLocal->evalAverageDbToDb(
      getDbin(), getDbin(), _ivar0, _ivar0, 0., 0);
    ...
    double cvv = _modelLocal->evalAverageDbToDb(
      dbgrid, dbgrid, _ivar0, _ivar0, dbgrid->getExtensionDiagonal() / 1.e3, 0);
