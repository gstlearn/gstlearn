 ...
 */
Id KrigingAlgebra::setColCokUnique(const VectorDouble* Zp, const VectorInt* rankColCok)
{
  _resetLinkedToColCok();
...
Id KrigingAlgebra::_needZstar()
{
  ...
  if (_flagDual)
  {
    ...
    _Zstar = AMatrix::prodMV(*_Sigma0, _bDual, true);
    // _Zstar = _Sigma0->prodMatVec(_bDual, true);
    if (_nbfl > 0)
    {
      if (_needX0()) return 1;
      VectorDouble ext = AMatrix::prodMV(*_X0, _cDual);
  ...
  if (_flagSK || _flagBayes)
  {
    ...
    _Zstar = AMatrix::prodVM(*_Z, _LambdaSK, false);
    ...
    if (_flagBayes)
    {
      ...
      VectorDouble mean = AMatrix::prodMV(_Y0, _Beta);
  ...
  else
  {
    ...
    _Zstar = AMatrix::prodVM(*_Z, _LambdaUK, false);
  ...
  if (_ncck > 0)
  {
    ...
    VectorDouble Zstar0 = AMatrix::prodMV(_Lambda0, _Z0p, true);
...
Id KrigingAlgebra::_patchColCokVarianceZstar(MatrixSymmetric* varZK)
{
  ...
  AMatrix::linearCombination(*varZK, 0., 1., *varZK, -1., L0tCLK, 1., L0tCL0);
  return 0;
}
...
Id KrigingAlgebra::_needStdv()
{
  ...
  else
  {
    ...
    AMatrix::linearCombination(_Stdv, 0., 1., _Stdv, -1., p1, 1, p2);
    ...
    if (_ncck > 0)
    {
      ...
      AMatrix::linearCombination(_Stdv, 0., 1., _Stdv, -1., p3);
    }
  }
...
Id KrigingAlgebra::_needSigmac()
{
  ...
  if (_flagBayes)
  {
    ...
    AMatrix::linearCombination(_Sigmac, 0., 1., _Sigmac, 1., _InvPriorCov);
  }
...
Id KrigingAlgebra::_needBeta()
{
  ...
  VectorDouble XtInvCZ = AMatrix::prodMV(_XtInvSigma, *_Z);
...
Id KrigingAlgebra::_needY0()
{
  ...
  AMatrix::linearCombination(_Y0, 0., 1., *_X0, -1., LambdaSKtX);
  return 0;
}
...
Id KrigingAlgebra::_needY0p()
{
  ...
  AMatrix::linearCombination(_Y0p, 0., 1., _X0p, -1., _Y0p);
  return 0;
}
...
Id KrigingAlgebra::_needMuUK()
{
  ...
  if (_ncck > 0)
  {
    ...
    AMatrix::linearCombination(LtY, 0., 1., _Y0, -1., LtY);
...
Id KrigingAlgebra::_patchRHSForXvalidUnique()
{
  ...
  AMatrix::linearCombination(omega, 0., 1., S00, -1., InvAlpha);
  ...
  if (_nbfl > 0)
  {
    ...
    AMatrix::linearCombination(a3, 0., 1., X0, 1., epsilon);
    ...
    AMatrix::linearCombination(a2, 0., 1., a2, -1., p3);
    if (a2.invert()) return 1;

    // Compute omega
    MatrixSymmetric p4(_nxvalid);
    p4.prodNormMatMatInPlace(&a3, &a2);
    AMatrix::linearCombination(omega, 0., 1., omega, -1., p4);
...
Id KrigingAlgebra::_needLambdaSK()
{
  ...
  if (_ncck > 0)
  {
    ...
    AMatrix::linearCombination(S, 0., 1., *_Sigma0, -1., S);
...
Id KrigingAlgebra::_needLambdaUK()
{
  ...
  AMatrix::linearCombination(_LambdaUK, 0., 1., _LambdaSK, 1., p1);
...
Id KrigingAlgebra::_needDual()
{
  ...
  _bDual = AMatrix::prodMV(_InvSigma, *_Z, true);
  ...
  if (_nbfl > 0)
  {
    ...
    VectorDouble wp = AMatrix::prodMV(_XtInvSigma, *_Z, false);
    // VectorDouble wp = _XtInvSigma.prodMatVec(*_Z, false);
    _cDual = AMatrix::prodMV(_Sigmac, wp, false);
    // _cDual = _Sigmac.prodMatVec(wp, false);

    VectorDouble p1 = AMatrix::prodMV(_XtInvSigma, _cDual, true);
...
Id KrigingAlgebra::_needLambda0()
{
  ...
  AMatrix::linearCombination(bot, 0., 1., bot, -1., bot1, (_nbfl > 0) ? 1. : 0., bot2);
  ...
  AMatrix::linearCombination(top, 0., 1., top, -1., top1, (_nbfl > 0) ? 1. : 0., top2);
