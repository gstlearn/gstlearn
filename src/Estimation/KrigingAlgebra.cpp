...
namespace gstlrn
{
   ...
   * @note kept unchanged (even if its contents may have been updated)
   */
  Id KrigingAlgebra::setLHS(const MatrixSymmetric* Sigma, const MatrixDense* X)
  {
    _resetLinkedToLHS();
  ...
  Id KrigingAlgebra::setVariance(const MatrixSymmetric* Sigma00)
  {
    if (Sigma00 != nullptr)
    {
  ...
  Id KrigingAlgebra::setRHS(const MatrixDense* Sigma0, const MatrixDense* X0)
  {
    _resetLinkedToRHS();
  ...
  bool KrigingAlgebra::_checkDimensionVD(
    const String& name,
    ...
    Id* sizeRef)
  {
    ...
    {
      messerr(
        "Dimension of %s (%d) incorrect: it should be (%d)", name.c_str(), size,
        *sizeRef);
      return false;
    }
  ...
  bool KrigingAlgebra::_checkDimensionVI(
    const String& name,
    const VectorInt* vec,
    Id* sizeRef)
  {
  ...
  bool KrigingAlgebra::_checkDimensionVVI(
    const String& name,
    ...
    Id* size2Ref)
  {
    ...
    {
      messerr(
        "First dimension of %s (%d) incorrect: it should be (%d)", name.c_str(),
        count, *size1Ref);
      return false;
    }
    ...
    {
      messerr(
        "Second dimension of %s (%d) incorrect: it should be (%d)",
        name.c_str(), size, *size2Ref);
      return false;
    }
  ...
  bool KrigingAlgebra::_checkDimensionMatrix(
    const String& name,
    const AMatrix* mat,
    Id* nrowsRef,
    Id* ncolsRef)
  {
    ...
    {
      messerr(
        "Number of Rows of %s (%d) incorrect: it should be (%d)", name.c_str(),
        nrows, *nrowsRef);
      return false;
    }
    ...
    {
      messerr(
        "Number of Columns of %s (%d) incorrect: it should be (%d)",
   ...
   */
  Id KrigingAlgebra::setColCokUnique(
   ...
   */
  Id KrigingAlgebra::setXvalidUnique(
    const VectorInt* rankXvalidEqs,
  ...
  Id KrigingAlgebra::setBayes(
    const VectorDouble* PriorMeans,
    const MatrixSymmetric* PriorCovs)
  {
    ...
    if (!_checkDimensionMatrix("PriorCovs", PriorCovs, &_nbfl, &_nbfl))
      return 1;
  ...
  Id KrigingAlgebra::_needInvSigma()
  {
  ...
  Id KrigingAlgebra::_needInvPriorCov()
  {
  ...
  Id KrigingAlgebra::_needZstar()
  {
    ...
    if (_flagDual)
    {
      ...
      _Zstar = AMatrix::product(*_Sigma0, _bDual, true);
      if (_nbfl > 0)
      {
        if (_needX0()) return 1;
        VectorDouble ext = AMatrix::product(*_X0, _cDual);
    ...
    if (_flagSK || _flagBayes)
    {
      ...
      _Zstar = AMatrix::product(*_Z, _LambdaSK, false);
      ...
      if (_flagBayes)
      {
        ...
        VectorDouble mean = AMatrix::product(_Y0, _Beta);
    ...
    else
    {
      ...
      _Zstar = AMatrix::product(*_Z, _LambdaUK, false);
    }
    ...
    if (_ncck > 0)
    {
      ...
      VectorDouble Zstar0 = AMatrix::product(_Lambda0, _Z0p, true);
      VH::linearCombinationInPlace(1., _Zstar, 1., Zstar0, _Zstar);
    }
    return 0;
  }

  Id KrigingAlgebra::_patchColCokVarianceZstar(MatrixSymmetric* varZK)
  {
    ...
    AMatrix::linearCombinationInPlace(
      *varZK, 0., 1., *varZK, -1., L0tCLK, 1., L0tCL0);
    return 0;
  }

  Id KrigingAlgebra::_needVarZSK()
  {
  ...
  Id KrigingAlgebra::_needXtInvSigma()
  {
  ...
  Id KrigingAlgebra::_needSigmac()
  {
    ...
    if (_flagBayes)
    {
      ...
      AMatrix::linearCombinationInPlace(
        _Sigmac, 0., 1., _Sigmac, 1., _InvPriorCov);
    }
  ...
  Id KrigingAlgebra::_needSigma00p()
  {
  ...
  Id KrigingAlgebra::_needSigma00pp()
  {
  ...
  Id KrigingAlgebra::_needSigma0p()
  {
  ...
  Id KrigingAlgebra::_needBeta()
  {
    ...
    VectorDouble XtInvCZ = AMatrix::product(_XtInvSigma, *_Z);

    if (_flagBayes)
    {
      if (_needPriorMean()) return 1;
      if (_needInvPriorCov()) return 1;
      VectorDouble InvSMBayes = AMatrix::product(_InvPriorCov, *_PriorMeans);
      VH::linearCombinationInPlace(1., XtInvCZ, 1., InvSMBayes, XtInvCZ);
    }

    _Beta = AMatrix::product(_Sigmac, XtInvCZ);
    return 0;
  }

  Id KrigingAlgebra::_needY0()
  {
  ...
  Id KrigingAlgebra::_needInvSigmaSigma0()
  {
  ...
  Id KrigingAlgebra::_patchRHSForXvalidUnique()
  {
    _resetLinkedToRHS();
    _resetLinkedtoVar0();
    ...
    AMatrix::linearCombinationInPlace(omega, 0., 1., S00, -1., InvAlpha);
    ...
    if (_nbfl > 0)
    {
      ...
      MatrixDense beta;
      MatrixDense::sample(
        beta, _InvSigma, *_rankXvalidEqs, *_rankXvalidEqs, false, true);
      ...
      AMatrix::linearCombinationInPlace(a3, 0., 1., X0, 1., epsilon);
      ...
      AMatrix::linearCombinationInPlace(a2, 0., 1., a2, -1., p3);
      if (a2.invert()) return 1;

      // Compute omega
      MatrixSymmetric p4(_nxvalid);
      p4.prodNormMatMatInPlace(&a3, &a2);
      AMatrix::linearCombinationInPlace(omega, 0., 1., omega, -1., p4);
    ...
    MatrixDense::sample(
      _C_RHS, *_Sigma, VectorInt(), *_rankXvalidEqs, false, false);
  ...
  Id KrigingAlgebra::_needLambdaSK()
  {
  ...
  Id KrigingAlgebra::_needLambdaUK()
  {
  ...
  Id KrigingAlgebra::_needDual()
  {
    if (!_flagDual) return 1;
    if (_needZ()) return 1;
    if (_needInvSigma()) return 1;

    _bDual = AMatrix::product(_InvSigma, *_Z, true);
    if (_nbfl > 0)
    {
      if (_needSigmac()) return 1;
      if (_needXtInvSigma()) return 1;
  ...
  bool KrigingAlgebra::_isPresentVector(
    const String& name,
    const VectorDouble* vec)
  {
  ...
  bool
    KrigingAlgebra::_isPresentIVector(const String& name, const VectorInt* vec)
  {
    if (vec != nullptr) return true;
    messerr(">>> Vector %s is missing (required)", name.c_str());
  ...
  bool KrigingAlgebra::_isPresentIIVector(
    const String& name,
    const VectorVectorInt* vec)
  {
    if (vec != nullptr) return true;
  ...
  Id KrigingAlgebra::_needLambda0()
  {
    ...
    AMatrix::linearCombinationInPlace(
      bot, 0., 1., bot, -1., bot1, (_nbfl > 0) ? 1. : 0., bot2);
    ...
    AMatrix::linearCombinationInPlace(
      top, 0., 1., top, -1., top1, (_nbfl > 0) ? 1. : 0., top2);
  ...
  void KrigingAlgebra::dumpLHS(Id nbypas) const
  {
    Id size = _neq;
