...
namespace gstlrn
{
  ...
  bool CalcImage::_filterImage(DbGrid* dbgrid, const ModelGeneric* modelGeneric)
  {
    ...
    if (!_flagFFT)
    {
      retcode = conv.ConvolveSparse(_iattOut,
                                    ranks,
                                    wgt,
                                    means,
                                    static_cast<Id>(_verbose));
    }
    else
    {
      DbGrid* marpat =
                                  ...
                                  Id optionVerbose)
  {
    ...
    for (Id ineigh = 0; ineigh < nbneigh; ineigh++)
    {
      ...
      for (Id ivar = 0; ivar < nvar; ivar++)
        for (Id jvar = 0; jvar < nvar; jvar++, ecr++)
          dbgrid->setArray(iadd,
                           iuid + ecr,
                           wgt.getValue(nbneigh * jvar + ineigh, ivar));
    }
    ...
    if (optionVerbose)
    {
      ...
      if (optionVerbose == 1)
      {
        Table table = dbStatisticsMono(dbgrid,
                                       {"Weights*"},
      ...
      {
        DbStringFormat* dbfmt = DbStringFormat::createFromFlags(false,
                                                                false,
                                                                false,
                                                                false,
                                                                true,
