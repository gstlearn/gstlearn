...
namespace gstlrn
{
  ...
  bool CalcImage::_postprocess()
  {
    ...
    if (_flagFilter)
      _renameVariable(
        2, VectorString(), ELoc::Z, getDbin()->getNLoc(ELoc::Z), _iattOut,
        String(), 1);

    if (_flagMorpho)
      _renameVariable(
        2, VectorString(), ELoc::Z, 1, _iattOut, String{_oper.getKey()},
        _nvarMorpho);
  ...
  bool CalcImage::_filterImage(DbGrid* dbgrid, const ModelGeneric* modelGeneric)
  {
    ...
    KrigingSystem ksys(dblocal, target, modelGeneric, neighU, getKrigopt());
    ...
    if (!_flagFFT)
    {
      retcode = conv.ConvolveSparse(
        _iattOut, ranks, wgt, means, static_cast<Id>(_verbose));
    }
    else
    {
      DbGrid* marpat =
        _buildMarpat(neighI, ranks, wgt, static_cast<Id>(_verbose));
    ...
    Id optionVerbose)
  {
    ...
    VectorInt nx(ndim);
    for (Id i = 0; i < ndim; i++) nx[i] = 2 * neigh->getImageRadius(i) + 1;
    ...
    for (Id ineigh = 0; ineigh < nbneigh; ineigh++)
    {
      ...
      for (Id ivar = 0; ivar < nvar; ivar++)
        for (Id jvar = 0; jvar < nvar; jvar++, ecr++)
          dbgrid->setArray(
            iadd, iuid + ecr, wgt.getValue(nbneigh * jvar + ineigh, ivar));
    }
    ...
    if (optionVerbose)
    {
      ...
      if (optionVerbose == 1)
      {
        Table table = dbStatisticsMono(
          dbgrid, {"Weights*"}, {EStatOption::MINI, EStatOption::MAXI});
        for (Id ivar = 0, irow = 0; ivar < nvar; ivar++)
          for (Id jvar = 0; jvar < nvar; jvar++, irow++)
            table.setRowName(
              irow, "Weight of Z" + std::to_string(jvar + 1) + " for Z*"
                      + std::to_string(ivar + 1));
        table.display();
      }
      else
      {
        DbStringFormat* dbfmt = DbStringFormat::createFromFlags(
          false, false, false, false, true, false, {"Weights*"});
  ...
  bool CalcImage::_run()
  {
    ...
    if (_flagFilter)
    {
      if (!_filterImage(dbgrid, getModelGeneric())) return false;
    }

    if (_flagMorpho)
    {
      if (db_morpho_calc(
            dbgrid, _iattOut, _oper, _vmin, _vmax, _option, _radius, _distErode,
            _verbose))
        return false;
    }
  ...
  **
  *****************************************************************************/
  void CalcImage::_image_smoother(
    DbGrid* dbgrid,
    const NeighImage* neigh,
    Id type,
    double range,
