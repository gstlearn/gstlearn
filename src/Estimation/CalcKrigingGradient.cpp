...
namespace gstlrn
{
  ...
  bool CalcKrigingGradient::_check()
  {
    ...
    {
      messerr(
        "The Depth and Gradient method is limited to the Monovariate case");
      return false;
    }
    ...
    {
      messerr(
        "Number of Gradient components (%d) must be equal to the Space "
        "dimension (%d)",
        ngrad, ndim);
      return false;
    }
...

  {
    ...
    else
    {
      ...
      {
        messerr(
          "The Gradient Model requires a single Covariance to be defined");
        return 1;
      }
    ...
    DriftList* drifts = DriftFactory::createDriftListForGradients(
      getModelGeneric()->getDriftList(), ctxt);
  ...
  {
    (void)kriging(
      _dbGradient, getDbout(), _modelGradient, getNeigh(), _flagEst, _flagStd,
      0, KrigOpt(), getNamingConvention());
    return true;
  }
