...
namespace gstlrn
{
  ...
  Likelihood* Likelihood::createForOptim(
    ModelGeneric* model,
    const Db* db,
    bool reml,
    bool verbose)
  ...
  void Likelihood::evalGrad(vect res)
  {
    ...
    _gradCovMatTimesInvCov.resize(
      static_cast<Id>(_Yc.size()), static_cast<Id>(_Yc.size()));
    ...
    for (size_t iparam = 0; iparam < gradcov.size(); iparam++)
    {
      ...
      AMatrix::productInPlace(_temp, _gradCovMat, _Cm1Yc);
      ...
      double dlogdet = MatrixDense::traceProd(
        invcov, _gradCovMat); // Warning: _gradCovMat is modified so the line
      ...
      for (const auto j: indsj)
      {
        ...
        for (Id ivar = 0; ivar < _model->getNVar(); ivar++)
        {
          ...
          for (const auto i: indsi)
          {
            ...
            _gradCovMat.setValue(
              icur, jcur, gradcov(p1, p2, ivar, jvar, nullptr));
            icur++;
          }
        }
        jcur++;
