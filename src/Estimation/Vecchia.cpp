...
namespace gstlrn
{
  ...
  Id Vecchia::computeLower(const MatrixT<Id>& Ranks, bool verbose)
  {
    ...
    if (verbose)
      mestitle(
        1,
        "Processing Neighborhood for constructing Lower Matrix for Vecchia");
    ...
    if (_db1 != nullptr)
    {
      _NumberRel1 = _db1->getListOfSampleIndicesInPlace(nvar,
                                                        _cumulRanks1,
                                                        _varRanks1,
                                                        true);
    ...
    if (_db2 != nullptr)
    {
      _NumberRel2 = _db2->getListOfSampleIndicesInPlace(nvar,
                                                        _cumulRanks2,
                                                        _varRanks2,
                                                        true);
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
    {
      ...
      for (Id isample = 0; isample < nsample; isample++)
      {
        ...
        Id nitems = _buildNeighborhood(Ranks,
                                       ndim,
                                       icaseDb,
                                       isample,
                                       ivar,
