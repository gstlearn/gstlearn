...
namespace gstlrn
{
                              ...
                              bool flag_per_cell)
  {
    ...
    if (calcul == EKrigOpt::BLOCK)
    {
      ...
      if (ndiscs.empty())
      {
        messerr("In case of BLOCK kriging, you must define the discretization "
                "parameters");
        messerr("i.e. a vector (dimension: Space Dimension) filled with "
                "positive numbers");
        return 1;
      }
      _ndiscs = ndiscs;
  ...
  void KrigOpt::blockDiscretize(Id iechout, bool flagRandom, Id seed) const
  {
    ...
    if (flagRandom)
      _disc2 = _dbgrid->getDiscretizedBlock(_ndiscs,
                                            iechout,
                                            _flagPerCell,
                                            true,
                                            seed);
  ...
  bool KrigOpt::_isValidCalcul(const Db* dbout, const ANeigh* neigh) const
  {
    ...
    if (_calcul == EKrigOpt::BLOCK)
    {
      ...
      if (_ndiscs.empty())
      {
        messerr("In case of BLOCK kriging, you must define the discretization "
                "coefficients");
        messerr("i.e. a vector (dimension equal Space Dimension) filled with "
                "positive numbers");
        return false;
      }
