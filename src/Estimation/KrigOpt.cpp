...
namespace gstlrn
{
    ...
    bool flag_per_cell)
  {
    ...
    if (calcul == EKrigOpt::BLOCK)
    {
      ...
      {
        messerr(
          "In case of BLOCK kriging, you must define the discretization "
          "parameters");
        messerr(
          "i.e. a vector (dimension: Space Dimension) filled with positive "
          "numbers");
        return 1;
      }
      _ndiscs = ndiscs;
    ...
    if (calcul == EKrigOpt::DGM) _flagDGM = true;
  ...
  {
    _disc1 =
      _dbgrid->getDiscretizedBlock(_ndiscs, iechout, _flagPerCell, false);

    if (flagRandom)
      _disc2 = _dbgrid->getDiscretizedBlock(
        _ndiscs, iechout, _flagPerCell, true, seed);
  }
  ...
  VectorVectorDouble KrigOpt::getDisc1VVD() const
  {
    ...
    for (Id idisc = 0; idisc < _nDiscNumber; idisc++)
      vecvec[idisc] = getDisc1VD(idisc);
    return vecvec;
  }
  ...
  VectorVectorDouble KrigOpt::getDisc2VVD() const
  {
    VectorVectorDouble vecvec(_nDiscNumber);
    for (Id idisc = 0; idisc < _nDiscNumber; idisc++)
      vecvec[idisc] = getDisc2VD(idisc);
    return vecvec;
  }

  void KrigOpt::setOptionDGM(bool flag_dgm)
  {
    if (flag_dgm) _calcul = EKrigOpt::DGM;
  }
    ...
    if (_calcul == EKrigOpt::BLOCK)
    {
      ...
      if (_ndiscs.empty())
      {
        messerr(
          ...
          "coefficients");
        messerr(
          "i.e. a vector (dimension equal Space Dimension) filled with "
          "positive numbers");
        return false;
      }
  ...
  bool KrigOpt::_isValidMatLC(const ModelGeneric* model) const
  {
    ...
    if (nrows > nvar)
    {
      ...
      messerr(
        "should be smaller than the number of variables in the model (%d)",
        nvar);
      return false;
    }
    if (ncols != nvar)
    {
      ...
      messerr(
        "should be equal to the number of variables in the model (%d)", nvar);
      return false;
    }
    return true;
  }
  ...
  bool KrigOpt::isCorrect(
    const Db* dbout,
    const ANeigh* neigh,
    const ModelGeneric* model) const
  {
