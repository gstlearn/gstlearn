...
namespace gstlrn
{
    ...
    , _flagNoStat(false)
  {
    ...
    if (model != nullptr) _flagNoStat = _model->isNoStat();

    // Reset the neighborhood
    if (neigh != nullptr) neigh->reset();
  ...
  Id KrigingSystem::_getNVar() const
  {
    Id nvar = 0;
    if (_model != nullptr)
    {
  ...
  Id KrigingSystem::_getNVarCL() const
  {
    if (!_krigopt.hasMatLC()) return _getNVar();
    return _krigopt.getMatLCNRows();
  }

  Id KrigingSystem::_getNbfl() const
  {
    if (_model == nullptr) return 0;
    return _model->getNDrift();
  }

  Id KrigingSystem::_getNFeq() const
  {
    if (_model == nullptr) return 0;
    return _model->getNDriftEquation();
  }

  Id KrigingSystem::_getNeq() const
  {
    Id neq = _nvar * _nech + _nfeq;
    return neq;
  }
   ...
   **
   *****************************************************************************/
  double KrigingSystem::_continuousMultiplier(Id rank1, Id rank2, double eps)
  {
    ...
    for (Id idim = 0; idim < _ndim; idim++)
      dd[idim] =
        _dbin->getCoordinate(rank1, idim) - _dbout->getCoordinate(rank2, idim);
   ...
   **
   *****************************************************************************/
  void KrigingSystem::_simulateCalcul(Id status)
  {
    Id ecr = 0;
    for (Id isimu = ecr = 0; isimu < _nbsimu; isimu++)
      for (Id ivar = 0; ivar < _nvar; ivar++, ecr++)
      {
        ...
        {
          if (_flagBayes)
            simu = _model->evalDriftVarCoef(
              _dbout, _iechOut, ivar, _postSimu.getColumn(isimu));
          ...
          for (Id jvar = 0; jvar < _nvar; jvar++)
            for (Id iech = 0; iech < _nech; iech++)
            {
              ...
              double diff = _dbin->getSimvar(
                ELoc::SIMU, jech, isimu, jvar, _rankPGS, _nbsimu, _nvar);
        ...
        _dbout->updSimvar(
          ELoc::SIMU, _iechOut, isimu, ivar, _rankPGS, _nbsimu, _nvar,
          EOperator::ADD, simu);
      }
  }
   ...
   **
   *****************************************************************************/
  void KrigingSystem::_estimateCalcul(Id status)
  {
    if (_flagEst) _estimateEstim(status);

    /* Variance of the estimation error */

    if (_flagStd) _estimateStdv(status);

    /* Variance of the estimator */

    if (_flagVarZ != 0) _estimateVarZ(status);
    ...
    if (_neigh->getFlagXvalid())
    {
      for (Id ivarCL = 0; ivarCL < _nvarCL; ivarCL++)
      {
        ...
        double estim =
          (_flagEst) ? _dbout->getArray(_iechOut, _iptrEst + ivarCL) : TEST;
        double stdv =
          (_flagStd) ? _dbout->getArray(_iechOut, _iptrStd + ivarCL) : TEST;
        ...
        if (_flagStd)
        {
          if (_xvalidStdev)
          {
            stdv = (FFFF(estim) || FFFF(valdat) || stdv <= 0.)
                   ? TEST
                   : (estim - valdat) / stdv;
   ...
   **
   *****************************************************************************/
  void KrigingSystem::_estimateEstim(Id status)
  {
   ...
   **
   *****************************************************************************/
  void KrigingSystem::_estimateStdv(Id status)
  {
    VectorDouble local(_nvarCL, TEST);
    if (!status)
    {
   ...
   **
   *****************************************************************************/
  void KrigingSystem::_estimateVarZ(Id status)
  {
    VectorDouble local(_nvarCL, TEST);
    if (!status)
    {
  ...
  Id KrigingSystem::resetData()
  {
    ...
    if (_model->evalCovMatSymInPlaceFromIdx(
          _Sigma, _dbin, _sampleRanks, &calcmode, false))
      return 1;
    if (_model->evalDriftMatByRanksInPlace(
          _X, _dbin, _sampleRanks, ECalcMember::LHS))
      return 1;
  ...
  bool KrigingSystem::isReady()
  {
    ...
    _meansTarget = _means;
    if (_krigopt.hasMatLC())
      _meansTarget = AMatrix::product(*_krigopt.getMatLC(), _means);
    ...
    {
      _iechOut = 0;
      if (_model->evalCovMat0InPlace(_Sigma00, _dbout, _iechOut, _krigopt))
        return false;
  ...
  void KrigingSystem::conclusion()
  {
    ...
    if (cova != nullptr) cova->optimizationPostProcess();
  }
   ...
   * @return
   */
  Id KrigingSystem::estimate(Id iech_out)
  {
    if (!_isReady)
    {
    ...
    bool caseXvalidUnique =
      (_neigh->getType() == ENeigh::UNIQUE && _neigh->getFlagXvalid());
    ...
    {
      if (_model->evalCovMatRHSInPlaceFromIdx(
            _Sigma0, _dbin, _dbout, _sampleRanks, iech_out, _krigopt, false))
        return 1;
      if (_model->evalDriftMatByTargetInPlace(_X0, _dbout, iech_out, _krigopt))
        return 1;
  ...
  Id KrigingSystem::_updateForColCokMoving()
  {
    ...
    {
      for (Id i = 0, n = static_cast<Id>(_sampleRanks[ivar].size()); i < n;
           i++, lec++)
        adds[ecr++] = 1 + lec;
  ...
  VectorInt KrigingSystem::_xvalidUniqueIndices() const
  {
    VectorInt ranks;
    Id lec = 0;
    for (Id ivar = 0, nvar = static_cast<Id>(_sampleRanks.size()); ivar < nvar;
         ivar++)
    {
      for (Id i = 0, n = static_cast<Id>(_sampleRanks[ivar].size()); i < n;
           i++, lec++)
   ...
   **
   *****************************************************************************/
  void KrigingSystem::_dumpKrigingResults(Id status)
  {
    if (_neigh->getFlagXvalid())
    ...
    for (Id ivar = 0; ivar < _nvarCL; ivar++)
    {
      if (_neigh->getFlagXvalid())
      {
        ...
        {
          double trueval =
            (status == 0) ? _dbin->getZVariable(_iechOut, ivar) : TEST;
          double estim =
            (status == 0) ? _dbout->getArray(_iechOut, _iptrEst + ivar) : TEST;
        ...
        {
          double stdev =
            (status == 0) ? _dbout->getArray(_iechOut, _iptrStd + ivar) : TEST;
      ...
      else
      {
        ...
        {
          double value =
            (status == 0) ? _dbout->getArray(_iechOut, _iptrVarZ + ivar) : TEST;
  ...
  void KrigingSystem::_dumpSimulationResults(Id status)
  {
   ...
   */
  Id KrigingSystem::updKrigOptEstim(
    Id iptrEst,
    Id iptrStd,
    Id iptrVarZ,
    bool forceNoDual)
  ...
  Id KrigingSystem::updKrigOptNeighOnly(Id iptrNeigh)
  {
    _isReady = false;
    if (iptrNeigh < 0)
    {
  ...
  Id KrigingSystem::setKrigOpt(const KrigOpt& krigopt)
  {
    _krigopt = krigopt;
    return 0;
  }

  Id KrigingSystem::setKrigOptCalcul(
   ...
   */
  Id KrigingSystem::setKrigOptXValid(
    bool flag_xvalid,
    bool flag_kfold,
  ...
  Id KrigingSystem::setKrigOptBayes(bool flag_bayes)
  {
    ...
    if (flag_bayes)
    {
      ...
      if (_algebra.setBayes(&_model->getPriorMeans(), &_model->getPriorCovs()))
        return 1;
    }
  ...
  Id KrigingSystem::setKrigOptFlagSimu(bool flagSimu, Id nbsimu, Id rankPGS)
  {
    _isReady = false;
    _flagSimu = flagSimu;
    _nbsimu = nbsimu;
  ...
  Id KrigingSystem::setKrigOptFlagGlobal(bool flag_global)
  {
    _isReady = false;
  ...
  Id KrigingSystem::setKrigOptFlagLTerm(bool flag_lterm)
  {
    _isReady = false;
  ...
  Id KrigingSystem::setKrigOptFactorKriging(bool flag_factor_kriging)
  {
    _isReady = false;
    ...
    else
    {
      CovLMCAnamorphosis* covAnam =
        dynamic_cast<CovLMCAnamorphosis*>(_model->_getCovModify());
      if (covAnam == nullptr)
      {
        messerr(
          "Your Model should contain a CovLMCAnamorphosis covariance item");
        return 1;
      }
      if (!covAnam->hasAnam())
      {
        messerr(
          "You may not use this option as there is no Anamorphosis defined");
        return 1;
      }
  ...
  Id KrigingSystem::updKrigOptIclass(Id index_class, Id nclasses)
  {
    if (!_flagFactorKriging)
    {
      messerr(
  ...
  bool KrigingSystem::_isCorrect()
  {
    ...
    if (_model != nullptr)
    {
      ...
      ndim = static_cast<Id>(_model->getNDim());
    }
    if (_neigh != nullptr)
    {
    ...
    if (_model != nullptr)
    {
      ...
      {
        messerr(
          "The Model should contain some Covariances defined before Kriging");
        return false;
      }
    }
    ...
    if (_model != nullptr)
    {
      ...
      if (_dbin != nullptr) _dbin->getExtensionInPlace(db_mini, db_maxi, true);
    ...
    if (_neigh != nullptr)
    {
      ...
      {
        messerr(
          "The Image neighborhood may not be used in KrigingSystem anymore");
        messerr("Use 'krimage' instead");
        return false;
      }

      if (_neigh->getType() == ENeigh::UNIQUE && _neigh->getFlagXvalid()
          && nvar > 1)
      {
   ...
   **
   *****************************************************************************/
  Id KrigingSystem::_bayesPreCalculations()
  {
   ...
   */
  Id KrigingSystem::_setInternalShortCutVariablesNeigh()
  {
    _nech = getNech();
    _neq = _getNeq();
    return (_nech <= 0);
