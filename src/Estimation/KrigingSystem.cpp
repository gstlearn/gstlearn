...
namespace gstlrn
{
  ...
  void KrigingSystem::_simulateCalcul(Id status)
  {
    ...
    for (Id isimu = ecr = 0; isimu < _nbsimu; isimu++)
      for (Id ivar = 0; ivar < _nvar; ivar++, ecr++)
      {
        ...
        {
          if (_flagBayes)
            simu = _model->evalDriftVarCoef(_dbout,
                                            _iechOut,
                                            ivar,
          ...
          for (Id jvar = 0; jvar < _nvar; jvar++)
            for (Id iech = 0; iech < _nech; iech++)
            {
              ...
              double diff = _dbin->getSimvar(ELoc::SIMU,
                                             jech,
                                             isimu,
                                             jvar,
                                             _rankPGS,
  ...
  Id KrigingSystem::resetData()
  {
    ...
    if (_model->evalCovMatSymInPlaceFromIdx(_Sigma,
                                            _dbin,
                                            _sampleRanks,
                                            &calcmode,
                                            false))
      return 1;
    if (_model->evalDriftMatByRanksInPlace(_X,
                                           _dbin,
                                           _sampleRanks,
                                           ECalcMember::LHS))
      return 1;
  ...
  Id KrigingSystem::estimate(Id iech_out)
  {
    ...
    {
      if (_model->evalCovMatRHSInPlaceFromIdx(_Sigma0,
                                              _dbin,
                                              _dbout,
                                              _sampleRanks,
                                              iech_out,
  ...
  Id KrigingSystem::updKrigOptIclass(Id index_class, Id nclasses)
  {
    if (!_flagFactorKriging)
    {
      messerr("Setting the Class Index only makes sense if 'flagFactorKriging' "
              "is ON");
