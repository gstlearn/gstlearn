...
namespace gstlrn
{
  ...
  Id KrigingSystemSimpleCase::_getNVar() const
  {
    Id nvar = 0;
    if (_model != nullptr)
    {
  ...
  Id KrigingSystemSimpleCase::_getNbfl() const
  {
    if (_model == nullptr) return 0;
    return _model->getNDrift();
  }

  Id KrigingSystemSimpleCase::_getNFeq() const
  {
    if (_model == nullptr) return 0;
    return _model->getNDriftEquation();
  }

  Id KrigingSystemSimpleCase::_getNeq(Id nech) const
  {
    Id neq = (_nvar * nech) + _nfeq;
    return neq;
  }
  ...
  void
    KrigingSystemSimpleCase::_rhsDump(KrigingAlgebraSimpleCase& algebra) const
  {
   ...
   **
   *****************************************************************************/
  void KrigingSystemSimpleCase::_estimateCalcul(
    Id status,
    Id iechout,
    KrigingAlgebraSimpleCase& algebra) const
  {
    Id nech = static_cast<Id>(algebra.getSampleRanksByVariable(0)->size());
    if (_flagEst) _estimateEstim(status, algebra, iechout);

    /* Variance of the estimation error */

    if (_flagStd) _estimateStdv(status, iechout, algebra);

    /* Variance of the estimator */

    if (_flagVarZ != 0) _estimateVarZ(status, iechout, algebra);

    /* Kriging weights (stored in _dbin) */

    if (_flagWeights != 0)
    {
      for (Id ivarCL = 0; ivarCL < 1; ivarCL++)
      {
  ...
  void KrigingSystemSimpleCase::_neighCalcul(
    Id status,
   ...
   **
   *****************************************************************************/
  void KrigingSystemSimpleCase::_estimateEstim(
    Id status,
   ...
   **
   *****************************************************************************/
  void KrigingSystemSimpleCase::_estimateStdv(
    Id status,
    Id iechout,
    KrigingAlgebraSimpleCase& algebra) const
  {
    ...
    for (Id ivarCL = 0; ivarCL < 1; ivarCL++)
      _dbout->setArray(
        iechout, _iptrStd + ivarCL, status ? TEST : local[ivarCL]);
  }
   ...
   **
   *****************************************************************************/
  void KrigingSystemSimpleCase::_estimateVarZ(
    Id status,
    Id iechout,
    KrigingAlgebraSimpleCase& algebra) const
  {
    ...
    for (Id ivarCL = 0; ivarCL < 1; ivarCL++)
      _dbout->setArray(
        iechout, _iptrVarZ + ivarCL, algebra.getVarianceZstar(ivarCL));
  }

  void KrigingSystemSimpleCase::updateLHS(
    KrigingAlgebraSimpleCase& algebra,
    ModelGeneric& model,
    VectorDouble& tabwork)
  {
    ...
    if (model.evalCovMatOptimInPlace(
          *Sigma, _dbin, *rkh, _krigopt, ECalcMember::LHS, tabwork))
      return;
    if (model.evalDriftMatByRanksInPlace(
          *X, _dbin, *sampleRanks, ECalcMember::LHS))
      return;
  ...
  bool KrigingSystemSimpleCase::isReady(VectorDouble& tabwork)
  {
    ...
    if (_flagStd)
    {
      ...
      if (_model->evalCovMat0InPlace(*Sigma00, _dbout, 0, _krigopt))
        return false;
    }
  ...
  Id KrigingSystemSimpleCase::estimate(
    Id iechout,
    SpacePoint& pin,
    SpacePoint& pout,
    VectorDouble& tabwork,
    ...
    ANeigh* neigh)
  {
    ...
    else
    {
      ...
      if (nbgh->size() <= 0) return 0;
      if (static_cast<Id>(nbgh->size()) < _getNbfl())
      {
        messerr(
          "Not enough data (%d) to perform kriging (with %d Drift Functions) "
          "at target %d",
    ...
    model.evalCovVecRHSInPlace(
      Sigma0->getViewOnColumnModify(0), *algebra.getRankHandler(), iechout,
      _krigopt, pin, pout, tabwork);
    if (_getNbfl() > 0)
      model.evalDriftMatByTargetInPlace(*X0, _dbout, iechout, _krigopt);
    algebra.updateRHS();
   ...
   **
   *****************************************************************************/
  void KrigingSystemSimpleCase::_dumpKrigingResults(
    Id status,
    Id iechout,
    KrigingAlgebraSimpleCase* algebra) const
  {
    ...
    for (Id ivar = 0; ivar < 1; ivar++)
    {
      ...
      {
        double value =
          (status == 0) ? _dbout->getArray(iechout, _iptrEst + ivar) : TEST;
      ...
      {
        double value =
          (status == 0) ? _dbout->getArray(iechout, _iptrStd + ivar) : TEST;
      ...
      {
        double value =
          (status == 0) ? _dbout->getArray(iechout, _iptrVarZ + ivar) : TEST;
   ...
   */
  Id KrigingSystemSimpleCase::updKrigOptEstim(
    Id iptrEst,
    Id iptrStd,
    Id iptrVarZ,
    bool forceNoDual)
  ...
  Id
    KrigingSystemSimpleCase::setKrigOptDataWeights(Id iptrWeights, bool flagSet)
  {
    _isReady = false;
    auto nvar = _getNVar();
  ...
  Id KrigingSystemSimpleCase::setKrigOptCalcul(const EKrigOpt& calcul)
  {
    _isReady = false;
  ...
  Id KrigingSystemSimpleCase::setKrigOptFlagGlobal(bool flag_global)
  {
    _isReady = false;
  ...
  Id KrigingSystemSimpleCase::setKrigOptFlagLTerm(bool flag_lterm)
  {
    _isReady = false;
  ...
  bool KrigingSystemSimpleCase::_isCorrect()
  {
    ...
    if (_neigh != nullptr)
    {
      ...
      {
        messerr(
          "The Image neighborhood may not be used in KrigingSystemSimpleCase "
          "anymore");
   ...
   */
  VectorVectorDouble KrigingSystemSimpleCase::getSampleCoordinates(
    KrigingAlgebraSimpleCase& algebra,
    Id iechout) const
  {
    Id nech = static_cast<Id>(algebra.getSampleRanksByVariable(0)->size());
    ...
    for (Id idim = 0; idim < _ndim; idim++)
    {
      ...
      {
        Id jech = (*algebra.getSampleRanksByVariable(0))[iech];
        if (jech >= 0)
  ...
  MatrixDense
    KrigingSystemSimpleCase::getWeights(KrigingAlgebraSimpleCase& algebra)
  {
