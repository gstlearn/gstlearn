...
namespace gstlrn
{
  ...
  void KrigingSystemSimpleCase::_neighCalcul(Id status,
   ...
   *****************************************************************************/
  void
    KrigingSystemSimpleCase::_estimateEstim(Id status,
    ...
    KrigingAlgebraSimpleCase& algebra) const
  {
    ...
    if (local.size() <= 0) return;
    for (Id ivarCL = 0; ivarCL < 1; ivarCL++)
      _dbout->setArray(iechout,
                       _iptrStd + ivarCL,
                       status ? TEST : local[ivarCL]);
  }
    ...
    KrigingAlgebraSimpleCase& algebra) const
  {
    ...
    for (Id ivarCL = 0; ivarCL < 1; ivarCL++)
      _dbout->setArray(iechout,
                       _iptrVarZ + ivarCL,
                       algebra.getVarianceZstar(ivarCL));
  }
                                          ...
                                          VectorDouble& tabwork)
  {
    ...
    if (model.evalCovMatOptimInPlace(*Sigma,
                                     _dbin,
                                     *rkh,
                                     _krigopt,
                                     ECalcMember::LHS,
                                     tabwork))
      return;
    if (model.evalDriftMatByRanksInPlace(*X,
                                         _dbin,
                                         *sampleRanks,
                                         ECalcMember::LHS))
      return;
                                       ...
                                       ANeigh* neigh)
  {
    ...
    else
    {
      ...
      {
        messerr("Not enough data (%d) to perform kriging (with %d Drift "
                "Functions) at target %d",
                nbgh->size(),
                _getNbfl(),
                iechout + 1);
        return 1;
  ...
  bool KrigingSystemSimpleCase::_isCorrect()
  {
    ...
    if (_neigh != nullptr)
    {
      ...
      {
        messerr("The Image neighborhood may not be used in "
                "KrigingSystemSimpleCase anymore");
