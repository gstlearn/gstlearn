...
namespace gstlrn
{
    ...
    , _trace(false)
  {
    ...
    _opt->setGradient(
      [this](vect grad) { this->evalGrad(grad); }, _params->getDispatch(),
      _params->getDispatchIndex());
    resetIter();
    _opt->setMaxIter(10000);
  };

  void AModelOptim::setEnvironment(
    ...
    double max)
  {
    ...
    _opt->setLowerBounds(
      _params->getMinValues(epsilon), _params->getDispatch());
    _opt->setUpperBounds(
      _params->getMaxValues(epsilon), _params->getDispatch());
    _x = _params->getOptimizableValues();
  }
