...
namespace gstlrn
{
  AModelOptim::AModelOptim(ModelGeneric* model, bool verbose)
    ...
    , _trace(false)
  {
    if (_model == nullptr) throw std::invalid_argument("Model cannot be null");
    ...
    _opt->setObjective([this](const VectorDouble& x) { return this->eval(x); });
    _opt->setGradient([this](vect grad) { this->evalGrad(grad); },
                      _params->getDispatch(),
  ...
  void AModelOptim::setEnvironment(const MatrixSymmetric& vars,
                                   double href,
                                   double epsilon,
                                   double min,
                                   double max)
  {
    ...
    _opt->setLowerBounds(_params->getMinValues(epsilon),
                         _params->getDispatch());
    _opt->setUpperBounds(_params->getMaxValues(epsilon),
                         _params->getDispatch());
    _x = _params->getOptimizableValues();
  }
  ...
  void AModelOptim::evalGrad(vect res) { DECLARE_UNUSED(res) };
