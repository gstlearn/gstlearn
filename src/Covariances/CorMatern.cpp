...
namespace gstlrn
{
  CorMatern::CorMatern(const CovContext& ctxt,
                       const ECov& type,
    ...
    , _corRef(std::shared_ptr<const CorAniso>(CorAniso::createAnisotropic(
...

  {
    ...
    if (static_cast<Id>(params.size()) != _nVar)
    {
      ...
      messerr("CorMatern: params size = %d, context size = %d",
              params.size(),
              _nVar);
      ierr = 1;
    }
    if (static_cast<Id>(kappas.size()) != _nVar)
    {
      ...
      messerr("CorMatern: coeffScales size = %d, context size = %d",
              kappas.size(),
              _nVar);
      ierr = 2;
    }
  ...
  CorMatern* CorMatern::create(const CovContext& ctxt,
                               const ECov& type,
                               const VectorDouble& params,
                               const VectorDouble& kappas,
                               const VectorDouble& ranges,
  ...
  double
    CorMatern::evalSpectrum(const VectorDouble& freq, Id ivar, Id jvar) const
  {
  ...
  double CorMatern::evalSpectrumRatio(const VectorDouble& freq,
                                      Id ivar,
                                      Id jvar,
    ...
    for (Id ib = 0; ib < ns; ib++)
    {
      ...
      else
      {
        ...
        for (Id ivar = 0; ivar < nvar; ivar++)
        {
          ...
          {
            double ratioIS =
              evalSpectrum(freq, ivar, jvar) / _corRef->evalSpectrum(freq);
  ...
  void
    CorMatern::_optimizationPreProcess(Id mode,
