...
namespace gstlrn
{
  CorMatern::CorMatern(
    ...
    , _corRef(
        std::shared_ptr<const CorAniso>(CorAniso::createAnisotropic(
...

  {
    ...
    if (static_cast<Id>(params.size()) != _nVar)
    {
      ...
      messerr(
        "CorMatern: params size = %d, context size = %d", params.size(), _nVar);
      ierr = 1;
    }
    if (static_cast<Id>(kappas.size()) != _nVar)
    {
      ...
      messerr(
        "CorMatern: coeffScales size = %d, context size = %d", kappas.size(),
        _nVar);
      ierr = 2;
    }
    ...
    for (Id ivar = 0; ivar < _nVar; ivar++)
    {
      ...
      for (Id jvar = ivar + 1; jvar < _nVar; jvar++)
      {
        ...
        double tau =
          pow(scalei, nui) * pow(scalej, nuj) / pow(scaleij, 2. * nuij);
        val *= tau;
    ...
    bool flagRange)
  {
    ...
    {
      messerr(
        "Mismatch in Space Dimension between 'ctxt'(%d) and 'ranges'(%d)", ndim,
        ranges.length());
      return nullptr;
    }
    ...
    {
      messerr(
        "Mismatch in Space Dimension between 'ctxt'(%d) and 'angles'(%d)", ndim,
        angles.length());
      return nullptr;
    }
    ...
    {
      messerr(
        "Mismatch in number of variables between 'ctxt'(%d) and 'params'(%d)",
        nvar, params.length());
      return nullptr;
    }
    ...
    {
      messerr(
        "Mismatch in number of variables between 'ctxt'(%d) and 'kappas'(%d)",
        nvar, kappas.length());
      return nullptr;
    }

    auto* cov =
      new CorMatern(ctxt, type, params, kappas, ranges, angles, flagRange);
    return cov;
  }
  ...
  double CorMatern::evalSpectrumOnRN(const VectorDouble& freq, Id ivar, Id jvar)
    const
  {
  ...
  double CorMatern::evalSpectrumRatio(
  ...
  SpectrumOnRN* CorMatern::simulateOnRN(Id ns) const
  {
    ...
    for (Id ib = 0; ib < ns; ib++)
    {
      ...
      for (Id ivar = 0; ivar < nvar; ivar++)
      {
        ...
        {
          double ratioIS = evalSpectrumOnRN(freq, ivar, jvar)
                         / _corRef->evalSpectrumOnRN(freq);
  ...
  void CorMatern::_optimizationPreProcess(
    Id mode,
