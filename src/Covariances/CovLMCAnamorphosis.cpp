...
namespace gstlrn
{
  ...
  Id CovLMCAnamorphosis::init(const VectorInt& anam_strcnt)
  {
    ...
    if (type == EAnam::DISCRETE_IR)
    {
      ...
      {
        messerr("Argument 'anam_strcnt' must be dimensioned to the number of "
                "factors (%d)",
                nfact);
        return 1;
      }
      ...
      for (Id i = 0; i < nfact; i++)
      {
        ...
        {
          messerr("Argument 'anam_strcnt' must contain ranks of covariances of "
                  "each factor");
          messerr("This value (%d) should lie within [1,ncov[ where ncov=%d",
                  anam_strcnt[i],
                  ncov);
          return 1;
        }
