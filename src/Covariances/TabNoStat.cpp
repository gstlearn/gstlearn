...
namespace gstlrn
{
  ...
  bool TabNoStat::isValid(const EConsElem& econs) const
  {
    ...
    {
      messerr(
        "%s is an invalid parameter for this covariance structure",
    ...
    Id iv2)
  {
    ...
    {
      messerr(
        "Warning, this non stationarity was already specified. It has been "
        "replaced");
  ...
  bool TabNoStat::variableExistsInDb(const String& namecol) const
  {
    ...
    {
      messerr(
        "The Name of the Non-stationary variable (%s) does not exist in the "
        "Reference Db",
        namecol.c_str());
      return false;
    }
    ...
    for (const auto& [paramId, noStatPtr]: _items)
    {
      ...
      ret = ret
         && SerializeHDF5::writeValue(
              itemG, "Type", paramId.getType().getValue());
