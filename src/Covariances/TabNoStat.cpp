...
namespace gstlrn
{
  ...
  std::shared_ptr<ANoStat>
    TabNoStat::getElem(const EConsElem& econs, Id iv1, Id iv2)
  {
    ParamId conselem(econs, iv1, iv2);
  ...
  String TabNoStat::toStringInside(const AStringFormat* strfmt, Id i) const
  {
    ...
    std::vector<std::pair<std::string, std::string>> sortedTable(
      getTable().size());
    size_t j{};
  ...
  Id TabNoStat::addElem(std::shared_ptr<ANoStat>& nostat,
                        const EConsElem& econs,
                        Id iv1,
                        Id iv2)
  {
    if (!isValid(econs)) return 0;
    ParamId param(econs, iv1, iv2);
    Id res = static_cast<Id>(_items.count(param));
    _items[param] = nostat;
    if (res == 1)
    {
      messerr("Warning, this non stationarity was already specified. It has "
              "been replaced");
  ...
  void TabNoStat::informMeshByMesh(const AMesh* amesh,
                                   const EConsElem& econs) const
  {
    for (const auto& e: _items)
    {
      if (e.first.getType() == econs) e.second->informMeshByMesh(amesh);
    }
  }
                                   ...
                                   const EConsElem& econs) const
  {
    for (const auto& e: _items)
    {
      if (e.first.getType() == econs) e.second->informMeshByApex(amesh);
    }
  }
  ...
  void TabNoStat::informDbIn(const Db* dbin, const EConsElem& econs) const
  {
    for (const auto& e: _items)
    {
      if (e.first.getType() == econs) e.second->informDbIn(dbin);
    }
  }
  ...
  void TabNoStat::informDbOut(const Db* dbout, const EConsElem& econs) const
  {
    for (const auto& e: _items)
    {
      if (e.first.getType() == econs) e.second->informDbOout(dbout);
    }
  }
  ...
  bool TabNoStat::variableExistsInDb(const String& namecol) const
  {
    ...
    {
      messerr("The Name of the Non-stationary variable (%s) does not exist in "
              "the Reference Db",
              namecol.c_str());
      return false;
    }
