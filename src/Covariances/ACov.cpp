...
namespace gstlrn
{
  ...
  void ACov::_optimizationPreProcessForTarget(const Db* db,
  ...
  void ACov::optimizationPreProcess(Id mode,
   ...
   */
  void ACov::_optimizationPreProcess(Id mode,
                                     const std::vector<SpacePoint>& ps) const
  {
    if (mode == 1)
    {
  ...
  bool ACov::checkAndManageNoStatDb(const Db* db, const String& namecol)
  {
    ...
    {
      messerr("You have to define a Db (with attachNoStatDb or by specifying a "
              "Db here)");
      return false;
    }
  ...
  void ACov::_attachNoStatDb(const Db* db){DECLARE_UNUSED(
    db)} VectorDouble ACov::informCoords(const VectorVectorDouble& coords,
    ...
    else
    {
      ...
      for (Id ivar = 0; ivar < nvar; ivar++)
        for (Id jvar = 0; jvar <= ivar; jvar++)
          mat.setValue(ivar,
                       jvar,
                       evalAverageIncrToIncr(d1, d2, ivar, jvar, &mode));
    };
                              ...
                              const CovCalcMode* mode) const
  {
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
      for (Id jvar = 0; jvar < nvar; jvar++)
        mat.setValue(ivar,
                     jvar,
                     evalCxv(p1, ext, ndisc, angles, x0, ivar, jvar, mode));
    return mat;
  }
                               ...
                               bool cleanOptim) const
  {
    ...
    Id error = evalCovMatInPlace(mat,
                                 db1,
                                 db2,
                                 ivar0,
                                 jvar0,
                             ...
                             bool cleanOptim) const
  {
    ...
    return evalCovMatInPlaceFromIdx(mat,
                                    db1,
                                    db2,
                                    index1,
                                    index2,
                                ...
                                const ECalcMember& calcMember) const
  {
    ...
    return addEvalCovVecRHSInPlace(vec,
                                   rank.getSampleRanks(0),
                                   iech2,
                                   krigopt,
                                   pin,
                                   pout,
  ...
  Id ACov::makeElemNoStat(const EConsElem& econs,
                          Id iv1,
                          Id iv2,
  ...
  Id ACov::_makeElemNoStat(const EConsElem& econs,
                           Id iv1,
                           Id iv2,
                           const AFunctional* func,
                           const Db* db,
   ...
   **
   *****************************************************************************/
  Id ACov::buildVmapOnDbGrid(DbGrid* dbgrid,
  ...
  double ACov::evalCovOnSphere(double alpha,
                               Id degree,
  ...
  VectorDouble ACov::evalSpectrumOnSphere(Id n,
                                          bool scaleDistanceByRadius,
  ...
  double ACov::evalSpectrumRatio(const VectorDouble& freq,
                                 Id ivar,
                                 Id jvar,
