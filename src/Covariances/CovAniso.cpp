...
namespace gstlrn
{
  ...
  double CovAniso::evalCovOnSphere(
    double alpha,
    Id degree,
    bool scaleDistanceByRadius,
    const CovCalcMode* mode) const
  {
    double value = getCorAniso()->evalCovOnSphere(
      alpha, degree, scaleDistanceByRadius, mode);
    return value * _getSillValue(0, 0, mode);
  }

  VectorDouble CovAniso::evalSpectrumOnSphere(
    Id n,
    bool scaleDistanceByRadius,
    bool flagScale) const
  {
    if (!getCorAniso()->isValidForSimulation(ESimuType::SPECTRAL))
      return VectorDouble();
    return getCorAniso()->evalSpectrumOnSphere(
      n, scaleDistanceByRadius, flagScale);
  }

  double
    CovAniso::evalSpectrumOnRN(const VectorDouble& freq, Id ivar, Id jvar) const
  {
    if (!getCorAniso()->isValidForSimulation(ESimuType::SPECTRAL)) return TEST;
    return _sillCur.getValue(ivar, jvar)
         * getCorAniso()->evalSpectrumOnRN(freq, ivar, jvar);
  }
  ...
  VectorDouble CovAniso::evalCovOnSphereVec(
  ...
  String CovAniso::toString(const AStringFormat* strfmt) const
  {
    ...
    if (getCorAniso()->hasRange() >= 0)
    {
      ...
      {
        sstr << toStrMatrix(
          "- Sill matrix:", VectorString(), VectorString(), 0, getNVar(),
          getNVar(), _sillCur.getValues());
      }
      else
      {
        sstr << "- Sill         = " << toStr(_sillCur.getValue(0, 0))
             << std::endl;
      }
    }
    else
    {
      ...
      if (getNVar() > 1)
      {
        ...
        sstr << toStrMatrix(
          "- Slope matrix:", VectorString(), VectorString(), 0, getNVar(),
          getNVar(), slopes.getValues());
      }
      else
      {
  ...
  {
    return _sillCur.getValue(0, 0)
         * getCorAniso()->getIntegralRange(ndisc, hmax);
  }

  CovAniso* CovAniso::createIsotropic(
    const CovContext& ctxt,
    const ECov& type,
    double range,
    double sill,
  ...
  CovAniso* CovAniso::createAnisotropic(
    const CovContext& ctxt,
    const ECov& type,
  ...
  CovAniso* CovAniso::createIsotropicMulti(
    const CovContext& ctxt,
    const ECov& type,
    double range,
    ...
    bool flagRange)
  {
    ...
    {
      messerr(
        "Mismatch in the number of variables between 'sills'(%d) and "
        "'ctxt'(%d)",
        nvar, ctxt.getNVar());
      return nullptr;
    }
  ...
  CovAniso* CovAniso::createAnisotropicMulti(
    const CovContext& ctxt,
    const ECov& type,
    const VectorDouble& ranges,
    const MatrixSymmetric& sills,
  ...
  CovAniso* CovAniso::createFromParam(
    const ECov& type,
    double range,
    double sill,
    double param,
    ...
    bool flagRange)
  {
    ...
    {
      messerr(
        "Mismatch between the space dimension in 'space' (%d)",
  ...
  Array
    CovAniso::evalCovFFT(const VectorDouble& hmax, Id N, Id ivar, Id jvar) const
  {
  ...
  CovAniso* CovAniso::createReduce(const VectorInt& validVars) const
  {
    ...
    auto* newsill = dynamic_cast<MatrixSymmetric*>(
      MatrixFactory::createReduce(&_sillCur, validVars, validVars));
