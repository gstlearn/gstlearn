...
namespace gstlrn
{
  ...
  CovAniso::CovAniso(const CovContext& ctxt,
                     const ECov& type,
                     double param,
                     double sill,
                     double range,
  ...
  double CovAniso::evalCovOnSphere(double alpha,
                                   ...
                                   const CovCalcMode* mode) const
  {
    double value = getCorAniso()->evalCovOnSphere(alpha,
                                                  degree,
                                                  scaleDistanceByRadius,
                                                  mode);
    return value * _getSillValue(0, 0, mode);
  }

  VectorDouble CovAniso::evalSpectrumOnSphere(Id n,
                                              bool scaleDistanceByRadius,
                                              bool flagScale) const
  {
    if (!getCorAniso()->isValidForSimulation(ESimuType::SPECTRAL))
      return VectorDouble();
    return getCorAniso()->evalSpectrumOnSphere(n,
                                               scaleDistanceByRadius,
                                               flagScale);
  }

  double
    CovAniso::evalSpectrum(const VectorDouble& freq, Id ivar, Id jvar) const
  {
    if (!getCorAniso()->isValidForSimulation(ESimuType::SPECTRAL)) return TEST;
    return _sillCur.getValue(ivar, jvar)
         * getCorAniso()->evalSpectrum(freq, ivar, jvar);
  }
                                           ...
                                           bool flagRange)
  {
    ...
    {
      messerr("Mismatch in the number of variables between 'sills'(%d) and "
              "'ctxt'(%d)",
              nvar,
              ctxt.getNVar());
      return nullptr;
