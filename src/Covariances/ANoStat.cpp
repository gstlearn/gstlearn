...
namespace gstlrn
{
  ...
  {
    if (icas == 1) return getValueOnDbIn(iech);
    if (icas == 2) return getValueOnDbOut(iech);
    return TEST;
  }

  double ANoStat::getValueOnMesh(Id iapex, bool center) const
  {
    if (center) return getValueOnMeshByMesh(iapex);
    return getValueOnMeshByApex(iapex);
  }

  bool ANoStat::getValuesOnDb(Id icas1,
                              Id iech1,
                              double* val1,
                              Id icas2,
                              Id iech2,
