...
namespace gstlrn
{
  ...
  void AKernel::setParam(double param, Id ipar)
  {
    ...
    if (ipar >= static_cast<Id>(_params.size())) _params.resize(ipar + 1, TEST);
  ...
  {
    if (ipar >= static_cast<Id>(_params.size())) return TEST;
    return _params[ipar];
  }

  void AKernel::setField(double field)
  {
    if (isZero(field)) my_throw("Cannot scale with zero");
  ...
  double
    AKernel::evaluateCovOnSphere(double alpha, double scale, Id degree) const
  {
    return _evaluateCovOnSphere(alpha, scale, degree);
  }

  VectorDouble
    AKernel::evaluateSpectrumOnSphere(Id n, double scale, bool flagScale) const
  {
  ...
  void AKernel::setMarkovCoeffs(const VectorDouble& coeffs)
  {
    ...
    messerr(
      "This covariance should have a method giving the Markov coefficients");
  ...
  double
    AKernel::_evaluateCovOnSphere(double alpha, double scale, Id degree) const
  {
    double s = 0.;
  ...
  VectorDouble
    AKernel::_evaluateSpectrumOnSphere(Id n, double scale, bool flagScale) const
  {
    DECLARE_UNUSED(n);
  ...
  Array AKernel::_evalCovFFT(const VectorDouble& hmax, Id N) const
  {
    ...
    VectorInt nxs(ndim);
    for (Id idim = 0; idim < ndim; idim++) nxs[idim] = N;
    Array array(nxs);
    ...
    for (Id iad = 0; iad < ntotal; iad++)
    {
      ...
      {
        double temp =
          a[idim] * (static_cast<double>(indices[idim]) / (N - 1) - 0.5);
        s += temp * temp;
      }
    ...
    VectorInt nxs2(ndim);
    for (Id idim = 0; idim < ndim; idim++) nxs2[idim] = N / 2;
    Array result(nxs2);
    ...
    for (Id iad = 0; iad < ntotal; iad++)
    {
      ...
      for (Id idim = 0; idim < ndim; idim++)
      {
        if (indices[idim] < (nxs2[idim] / 2)
            || indices[idim] >= (3 * nxs2[idim] / 2))
        {
          cont = false;
          continue;
        }
  ...
  VectorInt getAllMaxDimension()
  {
    ...
    Id ndim =
      1; // We set the dimension to 1 to get the maximum count of availabme kernels.
    VectorInt maxdims;
