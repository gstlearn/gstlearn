...
namespace gstlrn
{
  ...
  struct DerivCache
  {
               ...
               const CovCalcMode* mode) const
    {
      ...
      if (useCache) // check coordinates
      {
        ...
        bool equal1 = std::all_of(incr1.begin(),
                                  incr1.end(),
                                  [](double val) { return val == 0.0; });
        auto incr2 = cachedP2.getIncrement(p2);
        bool equal2 = std::all_of(incr2.begin(),
                                  incr2.end(),
                                  [](double val) { return val == 0.0; });
  ...
  CorAniso::CorAniso(const CovContext& ctxt,
                     const ECov& type,
                     double param,
                     double range,
                     bool flagRange)
                                            ...
                                            const VectorDouble& scales) const
  {
    ...
    if (!ranges.empty())
    {
      ...
      for (size_t i = 0; i < ranges.size(); i++)
      {
        if (ranges[i] <= EPSILON10)
        {
          messerr("The range in Space dimension (%d) should not be too small",
                  i);
        }
      }
      scales_local = ranges;
  ...
  SpectrumRN CorAniso::simulateSpectrumRN(Id ns, const ACov* cov0) const
  {
    ...
    else // Importance sampling using the auxiliary the spectral measure of cov0
    {
      ...
      for (Id ib = 0; ib < ns; ib++)
      {
        ...
        double ratioIS =
          evalSpectrum(freq, 0, 0) / cov0->evalSpectrum(freq, 0, 0);
   ...
   */
  void CorAniso::addEvalCorFromHVect(constvect h,
                                     vect res,
  ...
  {
    if (!isOptimEnabled())
      return ACov::addEvalCovVecRHSInPlace(vec,
                                           index1,
                                           iech2,
                                           krigopt,
                                           pin,
  ...
  VectorDouble CorAniso::evalSpectrumOnSphere(Id n,
                                              bool scaleDistanceByRadius,
  ...
  double
    CorAniso::evalSpectrum(const VectorDouble& freq, Id ivar, Id jvar) const
  {
    DECLARE_UNUSED(ivar, jvar)
    if (!_corfunc->isValidForSimulation(ESimuType::SPECTRAL)) return TEST;
  ...
  double CorAniso::evalSpectrumRatio(const VectorDouble& freq,
                                     Id ivar,
                                     Id jvar,
                                     const ACov* cov0) const
  {
    ...
    if (cov0 != nullptr)
      ratio =
        evalSpectrum(freq, ivar, jvar) / cov0->evalSpectrum(freq, ivar, jvar);
    ...
    if (_corfunc->hasRange() > 0)
    {
      ...
      else
      {
        ...
        if (!_aniso.getRotation().isIdentity())
        {
          ...
          sstr << toStrMatrix("- Rotation Matrix",
                              VectorString(),
                              VectorString(),
                              true,
                              getNDim(),
                              getNDim(),
                              getAnisoRotMat().getValues());
        }
      }
    }
  ...
  void
    CorAniso::_optimizationPreProcess(Id mode,
  ...
  Id CorAniso::makeElemNoStatOnMesh(const EConsElem& econs,
                                    Id iv1,
                                    Id iv2,
                                    const Db* db,
  ...
  Id CorAniso::makeElemNoStat(const EConsElem& econs,
                              Id iv1,
                              Id iv2,
  ...
  bool CorAniso::_checkTensor() const
  {
    ...
    {
      messerr("You have already defined non stationarity by using Tensor "
              "specifications");
      messerr("Use makeTensorStationary before specifying other non stationary "
              "parameters");
  ...
  bool CorAniso::_checkRotation() const
  {
    if (isNoStatForRotation())
    {
      ...
      messerr("Make these parameters stationary (e.g by makeRangeStationary) "
              "before specifying");
    ...
    if (isNoStatForTensor())
    {
      for (Id idim = 0; idim < ndim; idim++)
      {
        for (Id jdim = 0; jdim < ndim; jdim++)
          if (getTabNoStatCovAniso().isElemDefined(EConsElem::TENSOR,
                                                   idim,
                                                   jdim))
          {
            auto noStat =
                              ...
                              std::vector<covmaptype>* gradFuncs)
  {
    ...
    for (auto& sc: _scales)
    {
      gradFuncs->emplace_back(
        [this, &sc, i, derivCache](const SpacePoint& p1,
                                   const SpacePoint& p2,
                                   Id ivar,
                                   Id jvar,
                                   const CovCalcMode* mode) -> double
        {
          ...
          if (std::all_of(incr.begin(),
                          incr.end(),
                          [](double v) { return isZero(v); }))
            return 0.;
    ...
    {
      gradFuncs->emplace_back(
        [this, i, derivCache](const SpacePoint& p1,
                              const SpacePoint& p2,
                              Id ivar,
                              Id jvar,
                              const CovCalcMode* mode) -> double
  ...
  bool CorAniso::serializeH5(H5::Group& grp) const
  {
    ...
    // Anisotropy
    ret = ret
       && SerializeHDF5::writeValue(grp,
                                    "FlagAniso",
                                    static_cast<Id>(getFlagAniso()));
    if (getFlagAniso())
    {
      ...
      ret = ret
         && SerializeHDF5::writeValue(grp,
                                      "FlagRotation",
                                      static_cast<Id>(getFlagRotation()));
      if (getFlagRotation())
        ret = ret
           && SerializeHDF5::writeVec(grp,
                                      "Rotation",
                                      getAnisoRotMat().getValues());
    }
