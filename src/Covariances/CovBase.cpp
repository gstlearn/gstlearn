...
namespace gstlrn
{
  ...
  {
    return ParamInfo(
      String("Cholesky sill"), TEST, {-INF, INF},
  ...
  CovBase::CovBase(ACov* cor, const MatrixSymmetric& sill)
    ...
    , _sillCur(sill)
    , _cor(
        std::dynamic_pointer_cast<ACov>(
          cor == nullptr ? nullptr : cor->cloneShared()))
  ...
  SpectrumOnRN* CovBase::simulateOnRN(Id ns) const
  {
    ...
    {
      messerr(
        "Inconsistent number of variables: nvar = %d vs. nvar_cor = %d", nvar,
        nvar_cor);
      return nullptr;
    }
  ...
  Id CovBase::addEvalCovVecRHSInPlace(
    vect vec,
    ...
    const ECalcMember& calcMember) const
  {
    ...
    return _cor->addEvalCovVecRHSInPlace(
      vec, index1, iech2, krigopt, pin, pout, tabwork, getSill(0, 0),
      calcMember);
  }

  double CovBase::_eval(
    const SpacePoint& p1,
    const SpacePoint& p2,
    Id ivar,
    Id jvar,
   ...
   */
  void CovBase::_optimizationPreProcess(
    Id mode,
  ...
  VectorDouble CovBase::informCoords(
  ...
  void CovBase::_multiplyCorDerivativesBySills(
    Id oldSize,
    std::vector<covmaptype>* gradFuncs)
  {
    ...
    for (size_t i = oldSize; i < newSize; ++i)
    {
      ...
      (*gradFuncs)[i] = [f, this](
                          const SpacePoint& p1, const SpacePoint& p2, Id ivar,
                          Id jvar, const CovCalcMode* mode)
      {
