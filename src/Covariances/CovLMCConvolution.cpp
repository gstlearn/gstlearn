...
namespace gstlrn
{
    ...
    Id conv_ndisc)
  {
    ...
    {
      case 0: navail[0] = (ndim >= 1) ? _convDiscNumber : 0; break;

      case 1: navail[1] = (ndim >= 2) ? _convDiscNumber : 0; break;

      case 2: navail[2] = (ndim >= 3) ? _convDiscNumber : 0; break;
      ...
      default:
        messerr(
          "This CovLMCConvolution direction (%d) does not exist",
          _convDir.getValue());
        return 1;
    }
    ...
    _convNumber = 1;
    for (Id i = 0; i < 3; i++) _convNumber *= 2 * navail[i] + 1;
  ...
  {
    static Def_Convolution DEF_CONVS[] = {{"Uniform", 1., _conv_uniform},
                                          {"Exponential", 2.995732,
                                           _conv_exponential},
                                          {"Gaussian", 1.730818,
                                           _conv_gaussian},
  ...
  double
    CovLMCConvolution::eval0(Id ivar, Id jvar, const CovCalcMode* mode) const
  {
    double cov0 = 0.;
    SpacePoint p11(getSpace());
    SpacePoint p22(getSpace());
  ...
  double CovLMCConvolution::_eval(
    const SpacePoint& p1,
    const SpacePoint& p2,
    Id ivar,
    Id jvar,
