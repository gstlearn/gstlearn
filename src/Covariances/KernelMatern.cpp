...
namespace gstlrn
{
  ...
  {
    if (ipar != 0) my_throw("KernelMatern: only one parameter is expected");
    if (param == 0.5)
    {
  ...
  double KernelMatern::_evaluateCovFirstDerivative(double h) const
  {
    ...
    double term2 =
      0.5 * pow(h, nu) * (_besselK(nu - 1, h) + _besselK(nu + 1, h));
  ...
  double KernelMatern::simulateTurningBand(
    double t0,
    TurningBandOperate& operTB) const
  {
    if (getParam() > 0.5) return operTB.cosineOne(t0);
  ...
  VectorDouble
    KernelMatern::_evaluateSpectrumOnSphere(Id n, double scale, bool flagScale)
      const
  {
    ...
    for (Id k = 0; k <= n; k++)
      sp[k] =
        (2. * k + 1.) / (4. * GV_PI) / pow(1. + (scale2 * k * (k + 1.)), alpha);
