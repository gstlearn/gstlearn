...
namespace gstlrn
{
    ...
    : ACov(r)
  {
    ...
    {
      _covs.push_back(
        std::dynamic_pointer_cast<ACov>(r._covs[ifac]->cloneShared()));
      _projs.push_back(
        std::dynamic_pointer_cast<MatrixDense>(r._projs[ifac]->cloneShared()));
    ...
    if (this != &r)
    {
      ...
      for (Id ifac = 0; ifac < nfac; ifac++)
      {
        _covs.push_back(
          std::dynamic_pointer_cast<ACov>(r._covs[ifac]->cloneShared()));
        _projs.push_back(
          std::dynamic_pointer_cast<MatrixDense>(
            r._projs[ifac]->cloneShared()));
        //  _subspaces.addSpaceComponent(r._subspaces.getComponent(ifac));
      }
    }
    return *this;
  ...
  void CorFactorized::addFactor(const ACov& cov, const MatrixDense& proj)
  {
    ...
    {
      messerr(
        "Error: the number of variables is inconsistent (nv = %d != %d)",
        cov.getNVar(), nvar);
    ...
    {
      messerr(
        "Error: dimension of covariance inconsistent (d = %d > %d)",
        cov.getNDim(), ndim);
      messerr("Operation is cancelled");
    }
    ...
    {
      messerr(
        "Error: subspace dimension inconsistent (d = %d != %d)",
        proj.getNRows(), ndim);
      messerr("Operation is cancelled");
    }
    if (ndim_sub > ndim)
    {
      messerr(
        "Error: subspace dimension inconsistent (d = %d > %d)", proj.getNCols(),
        ndim);
      messerr("Operation is cancelled");
    }
    _covs.push_back(std::dynamic_pointer_cast<ACov>(cov.cloneShared()));
    _projs.push_back(
      std::dynamic_pointer_cast<MatrixDense>(proj.cloneShared()));
  ...
  bool CorFactorized::isValidForSimulation(const ESimuType& simuType) const
  {
    ...
    for (int ifac = 0; ifac < getNFac(); ifac++)
    {
      ...
      {
        messerr(
          "The structure %d# is not valid for %s Simulation on RN", ifac,
          simuType.getKey());
        return false;
      }
    }
  ...
  SpectrumOnRN* CorFactorized::simulateOnRN(Id ns) const
  {
    ...
    for (Id ivar = 0; ivar < nvar; ivar++) gamma.setColumnToConstant(ivar, 1.0);
    ...
    for (Id ifac = 0; ifac < nfac; ifac++)
    {
      ...
      {
        messerr(
          "Inconsistent number of variables for factor #%d (nvar = %d != %d)",
          ifac, cova->getNVar(), getNVar());
        continue;
      }
