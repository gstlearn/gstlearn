...
namespace gstlrn
{
   ...
   */
  double CovGradientAnalytic::_eval(
    const SpacePoint& p1,
    const SpacePoint& p2,
    Id ivar,
    Id jvar,
  ...
  {
    DECLARE_UNUSED(mode);
    if (_onePointHasChanged(p1, p2)) _evalZAndGradients(p1, p2);
    ...
    if (ivar == 0)
    {
      if (jvar == 0) return _covpp;

      return -_covGp[jdim];
    }
    if (jvar == 0) return _covGp[idim];
    return _covGG[3 * idim + jdim];
  }

  bool CovGradientAnalytic::_onePointHasChanged(
    const SpacePoint& p1,
  ...
  void CovGradientAnalytic::_calculateTrTtr() const
  {
    ...
    const MatrixSquare& mat =
      _getCovRefAniso()->getAniso().getRotation().getMatrixDirect();
  ...
  void CovGradientAnalytic::_evalZAndGradients(
    const SpacePoint& p1,
    const SpacePoint& p2) const
  {
    _covpp = 0.;
    ...
    _calculateTrTtr();
    double dcovsr =
      covref->getSill(0, 0) * covfunc->evalCovFirstDerivativeOverH(hh);
    ...
    if (hh < EPSGRAD)
    {
      if (_flagCalculateGG)
      {
        for (Id ecr = 0; ecr < 9; ecr++) _covGG[ecr] -= dcovsr * _trttr[ecr];
      }
    }
    else
    {
      //  Calculate covariance between point and gradient
      for (Id ecr = 0; ecr < 3; ecr++) _covGp[ecr] += _uF[ecr] * dcovsr;

      //  Calculate the covariance between gradient and gradient
      if (_flagCalculateGG)
      {
        double d2cov =
          covref->getSill(0, 0) * covfunc->evalCovDerivative(2, hh);
