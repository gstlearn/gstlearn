...
namespace gstlrn
{
    ...
    , _scaleGneiting(1.0)
  {
    ...
    if (static_cast<Id>(params.size()) != _nVar)
    {
      ...
      messerr(
        "CorGaussianMixture: params size = %d, context size = %d",
        params.size(), _nVar);
      ierr = 2;
    }
    if (static_cast<Id>(kappas.size()) != _nVar)
    {
      ...
      messerr(
        "CorGaussianMixture: coeffScales size = %d, context size = %d",
        kappas.size(), _nVar);
      ierr = 3;
    }
    if (ierr == 0)
    ...
    bool flagRange)
  {
    ...
    auto* cov = new CorGaussianMixture(
      ctxt, type, params, kappas, ranges, angles, flagRange);
    return cov;
  }
  ...
  double CorGaussianMixture::_eval(
    const SpacePoint& p1,
    const SpacePoint& p2,
    Id ivar,
    Id jvar,
