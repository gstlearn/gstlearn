...
namespace gstlrn
{
  CorGaussianMixture::CorGaussianMixture(const CovContext& ctxt,
                                         const ECov& type,
  ...
  {
    int ierr = 0;
    if ((type != ECov::CAUCHY) && (type != ECov::MATERN)
        && (type != ECov::GAUSSIAN))
    {
    ...
    if (static_cast<Id>(params.size()) != _nVar)
    {
      ...
      messerr("CorGaussianMixture: params size = %d, context size = %d",
              params.size(),
              _nVar);
      ierr = 2;
    }
    if (static_cast<Id>(kappas.size()) != _nVar)
    {
      ...
      messerr("CorGaussianMixture: coeffScales size = %d, context size = %d",
              kappas.size(),
              _nVar);
      ierr = 3;
    }
  ...
  CorGaussianMixture* CorGaussianMixture::create(const CovContext& ctxt,
                                                 const ECov& type,
                                                 const VectorDouble& params,
                                                 const VectorDouble& kappas,
                                                 const VectorDouble& ranges,
                                                 ...
                                                 bool flagRange)
  {
    ...
    auto* cov = new CorGaussianMixture(ctxt,
                                       type,
                                       params,
                                       kappas,
                                       ranges,
  ...
  SpectrumRN
    CorGaussianMixture::simulateSpectrumRN(Id ns, const ACov* cov0) const
  {
    DECLARE_UNUSED(cov0)
    Id nvar = getNVar();
    Id ndim = getNDim();
