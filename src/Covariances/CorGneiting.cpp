...
namespace gstlrn
{
  ...
  CorGneiting::CorGneiting(const CorGaussianMixture* covS,
                           const CorAniso* covT,
                           double separability)
    : ACov()
    , _corS(std::shared_ptr<const CorGaussianMixture>(
        std::dynamic_pointer_cast<const CorGaussianMixture>(
          covS->cloneShared())))
    , _corT(std::shared_ptr<const CorAniso>(
  ...
  CorGneiting* CorGneiting::create(const CovContext& ctxt, // spatial context
                                   const ECov& type,
                                   double alpha,
                                   double beta,
                                   double timeRange,
                                   ...
                                   bool flagRange)
  {
    ...
    {
      messerr("Inconsistent number of shape parameters: nvar = %d vs. "
              "params.length = %d",
              nvar,
              params.length());
      return nullptr;
    ...
    {
      messerr("Inconsistent number of rate parameters: nvar = %d vs. "
    ...
    {
      messerr("Gneiting-Gaussian model requires alpha in (0,2]:  alpha = %f",
              alpha);
      return nullptr;
    }
    if ((beta <= 0.0) || (beta > 1.0))
    {
      messerr("Gneiting-Gaussian model requires beta in (0,2]: beta = %f",
              beta);
      return nullptr;
    }
    ...
    auto* corS = new CorGaussianMixture(CovContext(nvar, ndim),
  ...
  SpectrumRN CorGneiting::simulateSpectrumRN(Id ns, const ACov* cov0) const
  {
    ...
    double beta = _corT->getCorFunc()->getParam(1) * 2
                / ndim; // the second parameter of the Cauchy is beta*ndim/2
