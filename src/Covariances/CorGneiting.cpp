...
namespace gstlrn
{
  ...
  CorGneiting::CorGneiting(
    const CorGaussianMixture* covS,
    const CorAniso* covT,
    double separability)
    : ACov()
    , _corS(
        std::shared_ptr<const CorGaussianMixture>(
          std::dynamic_pointer_cast<const CorGaussianMixture>(
            covS->cloneShared())))
    , _corT(
        std::shared_ptr<const CorAniso>(
          std::dynamic_pointer_cast<const CorAniso>(covT->cloneShared())))
    , _cor(*covS)
  ...
  {
    Id id_spatial = 0;
    if ((type == ECov::GNEITING_G) || (type == ECov::GNEITING_C)
        || (type == ECov::GNEITING_M))
    {
    ...
    {
      messerr(
        "Inconsistent number of shape parameters: nvar = %d vs. params.length "
        "= %d",
        nvar, params.length());
      return nullptr;
    }
    ...
    {
      messerr(
        "Inconsistent number of rate parameters: nvar = %d vs. "
        "coeffScales.length = %d",
        nvar, kappas.length());
      return nullptr;
    }
    ...
    {
      messerr(
        "Gneiting-Gaussian model requires alpha in (0,2]:  alpha = %f", alpha);
      return nullptr;
    }
    if ((beta <= 0.0) || (beta > 1.0))
    {
      messerr(
        "Gneiting-Gaussian model requires beta in (0,2]: beta = %f", beta);
      return nullptr;
    }
    ...
    auto* corS = new CorGaussianMixture(
      CovContext(nvar, SpaceRN::create(ndim)), ECov::fromValue(id_spatial),
      params, kappas, ranges, angles, flagRange);
    // creation of the temporal covariance
    auto* corT =
      new CorAniso(CovContext(1, SpaceRN::create(1)), ECov::CAUCHY_GEN);
  ...
  SpectrumOnRN* CorGneiting::simulateOnRN(Id ns) const
  {
    ...
    else
    {
      ...
      double beta =
        _corT->getCorFunc()->getParam(1) * 2 * _separability
        / (ndim - 1); // the second parameter of the Cauchy is beta*ndim/2
      double timeRange = _corT->getScale(0);
      MatrixDense omega0 =
        sp->getOmega0(); // spatial frequency without anisotropy
