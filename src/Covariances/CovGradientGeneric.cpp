...
namespace gstlrn
{
   ...
   */
  double CovGradientGeneric::_eval(
    const SpacePoint& p1,
    const SpacePoint& p2,
    Id ivar,
    Id jvar,
    const CovCalcMode* mode) const
  {
    ...
    if (ivar == 0) return -_evalZGradientNumeric(p1, p2, jdim, mode);
    if (jvar == 0) return +_evalZGradientNumeric(p1, p2, idim, mode);
    if (jdim == idim)
  ...
  double CovGradientGeneric::_evalZGradientNumeric(
    const SpacePoint& p1,
    const SpacePoint& p2,
    Id idim,
    const CovCalcMode* mode) const
  ...
  double CovGradientGeneric::_evalGradientGradientNumeric(
    const SpacePoint& p1,
    const SpacePoint& p2,
    Id idim,
    Id jdim,
