...
namespace gstlrn
{
  bool _isValid(AKernel* cova, const CovContext& ctxt)
  {
    return static_cast<Id>(cova->getMaxNDim()) <= 0
        || static_cast<Id>(ctxt.getNDim())
             <= static_cast<Id>(cova->getMaxNDim());
  }
  ...
  AKernel* CovFactory::duplicateCovFunc(const AKernel& cov)
  {
    switch (cov.getType().toEnum())
    {
      ...
      case ECov::E_NUGGET:
        return new KernelNugget(dynamic_cast<const KernelNugget&>(cov));
      case ECov::E_EXPONENTIAL:
        return new KernelExponential(
          dynamic_cast<const KernelExponential&>(cov));
      case ECov::E_SPHERICAL:
        return new KernelSpherical(dynamic_cast<const KernelSpherical&>(cov));
      case ECov::E_GAUSSIAN:
        return new KernelGaussian(dynamic_cast<const KernelGaussian&>(cov));
      case ECov::E_CUBIC:
        return new KernelCubic(dynamic_cast<const KernelCubic&>(cov));
      case ECov::E_SINCARD:
        return new KernelSincard(dynamic_cast<const KernelSincard&>(cov));
      case ECov::E_BESSELJ:
        return new KernelBesselJ(dynamic_cast<const KernelBesselJ&>(cov));
      case ECov::E_MATERN:
        return new KernelMatern(dynamic_cast<const KernelMatern&>(cov));
      case ECov::E_GAMMA:
        return new KernelGamma(dynamic_cast<const KernelGamma&>(cov));
      case ECov::E_CAUCHY:
        return new KernelCauchy(dynamic_cast<const KernelCauchy&>(cov));
      case ECov::E_CAUCHY_GEN:
        return new KernelCauchyGen(dynamic_cast<const KernelCauchyGen&>(cov));
      case ECov::E_STABLE:
        return new KernelStable(dynamic_cast<const KernelStable&>(cov));
      case ECov::E_LINEAR:
        return new KernelLinear(dynamic_cast<const KernelLinear&>(cov));
      case ECov::E_POWER:
        return new KernelPower(dynamic_cast<const KernelPower&>(cov));
      case ECov::E_ORDER1_GC:
        return new KernelGC1(dynamic_cast<const KernelGC1&>(cov));
      case ECov::E_SPLINE_GC:
        return new KernelGCspline(dynamic_cast<const KernelGCspline&>(cov));
      case ECov::E_ORDER3_GC:
        return new KernelGC3(dynamic_cast<const KernelGC3&>(cov));
      case ECov::E_ORDER5_GC:
        return new KernelGC5(dynamic_cast<const KernelGC5&>(cov));
      case ECov::E_COSINUS:
        return new KernelCosinus(dynamic_cast<const KernelCosinus&>(cov));
      case ECov::E_TRIANGLE:
        return new KernelTriangle(dynamic_cast<const KernelTriangle&>(cov));
      case ECov::E_COSEXP:
        return new KernelCosExp(dynamic_cast<const KernelCosExp&>(cov));
      case ECov::E_REG1D:
        return new KernelReg1D(dynamic_cast<const KernelReg1D&>(cov));
      case ECov::E_PENTA:
        return new KernelPenta(dynamic_cast<const KernelPenta&>(cov));
      case ECov::E_SPLINE2_GC:
        return new KernelGCspline2(dynamic_cast<const KernelGCspline2&>(cov));
      case ECov::E_STORKEY:
        return new KernelStorkey(dynamic_cast<const KernelStorkey&>(cov));
      case ECov::E_WENDLAND0:
        return new KernelWendland0(dynamic_cast<const KernelWendland0&>(cov));
      case ECov::E_WENDLAND1:
        return new KernelWendland1(dynamic_cast<const KernelWendland1&>(cov));
      case ECov::E_WENDLAND2:
        return new KernelWendland2(dynamic_cast<const KernelWendland2&>(cov));
      case ECov::E_MARKOV:
        return new KernelMarkov(dynamic_cast<const KernelMarkov&>(cov));
      case ECov::E_POISSON:
        return new KernelPoisson(dynamic_cast<const KernelPoisson&>(cov));
      case ECov::E_GEOMETRIC:
        return new KernelGeometric(dynamic_cast<const KernelGeometric&>(cov));
      case ECov::E_LINEARSPH:
        return new KernelLinearSph(dynamic_cast<const KernelLinearSph&>(cov));
      default: break;
    }
  ...
  VectorString CovFactory::getCovList(const CovContext& ctxt, Id order)
  {
    ...
    while (it.hasNext())
    {
      if (*it != ECov::FUNCTION)
      {
        ...
        if (_isValid(cova, ctxt))
        {
          if (cova->getMinOrder() <= order) names.push_back(cova->getCovName());
        }
        delete cova;
      }
      it.toNext();
