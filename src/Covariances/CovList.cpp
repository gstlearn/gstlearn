...
namespace gstlrn
{
  ...
  void CovList::addCov(const CovBase& cov)
  {
    ...
    else
    {
      ...
      {
        messerr("Error: Covariances in the same CovList should share the same "
                "Context");
  ...
  bool CovList::_isNoStat() const
  {
    ...
    return std::any_of(_covs.cbegin(),
                       _covs.cend(),
  ...
  void CovList::makeSillNoStatFunctional(Id icov,
                                         const AFunctional* func,
                                         Id ivar,
                                         Id jvar)
  {
                                      ...
                                      const ECalcMember& calcMember) const
  {
    ...
    for (const auto& j: list.getVector())
    {
      if (_covs[j]->isOptimEnabled())
        _covs[j]->addEvalCovVecRHSInPlace(vec,
                                          index1,
                                          iech2,
                                          krigopt,
                                          pin,
        ...
        _covs[j]->ACov::addEvalCovVecRHSInPlace(vec,
                                                index1,
                                                iech2,
                                                krigopt,
                                                pin,
  ...
  void CovList::_optimizationPreProcess(Id mode,
