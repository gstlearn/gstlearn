...
namespace gstlrn
{
  ...
  {
    return "C(h)=\\int_{R^d} "
           "\\frac{e^{-i\\omega^t.h}}{P(||\\omega||^2)}d\\omega";
  }

  VectorDouble
    KernelMarkov::_evaluateSpectrumOnSphere(Id n, double scale, bool flagScale)
      const
  {
    ...
    {
      double nnp1 =
        scale * scale * static_cast<double>(j) * (static_cast<double>(j) + 1.);
      double s = 0.;
