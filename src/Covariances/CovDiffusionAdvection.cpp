...
namespace gstlrn
{
  ...
  CovDiffusionAdvection::~CovDiffusionAdvection()
  {
    ...
    if (_destroyMarkovL) delete _markovL;
    if (_destroyMarkovR) delete _markovR;
  }

  CovDiffusionAdvection* CovDiffusionAdvection::create(
    CovAniso* markovL,
    CovAniso* markovR,
    double scaleTime,
  ...
  void CovDiffusionAdvection::_init()
  {
    ...
    {
      my_throw(
        "At least one of the covariances has to be defined to make a valid "
    ...
    if (_markovL == nullptr)
    {
      ...
      _markovL = CovAniso::createAnisotropic(
        _ctxt, ECov::MARKOV, temp, 1., 1., temp, false);
    ...
    if (_markovR == nullptr)
    {
      _markovR = CovAniso::createAnisotropic(
        _ctxt, ECov::MARKOV, temp, 1., 1., temp, false);
      _destroyMarkovR = true;
      _markovRdefined = false;
      correcR =
        1. / _markovL->getFullCorrec()
        * pow(
          2.,
          -.5 * ndim); // _markovR has not tensor so we use _markovL determinant
    }
    else
    {
  ...
  void CovDiffusionAdvection::_computeSpatialTrace()
  {
    ...
    _spatialTrace = CovAniso::createAnisotropic(
      _ctxt, ECov::MARKOV, scales, _sigma2, 1., angles, false);

    VectorDouble coeffsL = _markovL->getMarkovCoeffs();
    VectorDouble coeffsR = _markovR->getMarkovCoeffs();
    Id degree =
      (static_cast<Id>(coeffsL.size()) + static_cast<Id>(coeffsR.size()) - 2);
    VectorDouble coeffs;
    ...
    double time) const
  {
    ...
    if (_markovLdefined) s1 = 1. / (_markovL->evalSpectrumOnRN(freq));

    double s2 = 1.;

    if (_markovRdefined) s2 = 1. / (_markovR->evalSpectrumOnRN(freq));

    std::complex<double> a(
      -_scaleTime * ABS(time * s1), -_scaleTime * velinner * time);
