...
namespace gstlrn
{
  ...
  Id Polygons::resetFromDb(const Db* db, double dilate, bool verbose)
  {
    ...
    Id nrow_max)
  {
    ...
    if (csv_table_read(
          filename, csv, verbose, ncol_max, nrow_max, &ncol, &nrow, names, tab))
    {
   ...
   * @return Polygons*
   */
  Polygons* Polygons::createFromVD(const VectorDouble& x, const VectorDouble& y)
  {
    if (x.size() != y.size())
    {
      messerr(
        "In Polygons::createFromVD: x and y vectors must have the same size");
      return nullptr;
    }
    auto* polygons = new Polygons();
   ...
   */
  Polygons* Polygons::createFillRandom(
    Id nseg,
    double ratio,
    double mini,
    double maxi,
    Id seed)
  {
    ...
    {
      messerr(
        "In Polygons::createFillRandom: nseg must be at least equal to 3");
      return nullptr;
    }
    ...
    for (Id idim = 0; idim < 2; idim++) center[idim] = middle;
    ...
    Id nrow_max)
  {
    ...
    if (csv.getFlagHeader())
    {
      ...
      while (gslSafeGetline(file, line) && i < csv.getNSkip()) i++;
      // Header too short
      if (i < csv.getNSkip()) return 1;
    }
    // Parse next lines
    while (std::getline(file, line))
    {
      ...
      do
      {
        ...
        if (found != std::string::npos) poly = poly.substr(found + 5);
  ...
  PolyElem Polygons::_extractFromTab(
    Id ideb,
    Id ifin,
    Id ncol,
  ...
  PolyElem Polygons::_extractFromWKT(const CSVformat& csv, String& polye)
  {
    ...
    do
    {
      ...
      found2 = coords.find_first_of(' ');
      if (found2 != std::string::npos) coords = coords.substr(0, found2);
      y.push_back(fromStr<double>(coords, csv.getCharDec()));
      if (found != std::string::npos) polye = polye.substr(found + 1);
    } while (found != std::string::npos);
  ...
  bool Polygons::_deserializeAscii(std::istream& is)
  {
    ...
    bool ret = true;
    ret = ret && _recordRead<Id>(is, "Number of Polygons", npol);
  ...
  Polygons* Polygons::createFromCSV(
    const String& filename,
    const CSVformat& csv,
    Id verbose,
    Id ncol_max,
  ...
  Polygons* Polygons::createFromWKT(
    const String& filename,
    const CSVformat& csv,
    Id verbose,
    Id ncol_max,
  ...
  const PolyElem& Polygons::getPolyElem(Id ipol) const
  {
    if (!_isValidPolyElemIndex(ipol)) return _emptyElem;
    return _polyelems[ipol];
  }

  PolyElem Polygons::getClosedPolyElem(Id ipol) const
  {
  ...
  const VectorDouble& Polygons::getX(Id ipol) const
  {
    if (!_isValidPolyElemIndex(ipol)) return _emptyVec;
    return _polyelems[ipol].getX();
  }

  const VectorDouble& Polygons::getY(Id ipol) const
  {
    if (!_isValidPolyElemIndex(ipol)) return _emptyVec;
    return _polyelems[ipol].getY();
  }

  void Polygons::setX(Id ipol, const VectorDouble& x)
  {
    if (!_isValidPolyElemIndex(ipol)) return;
    return _polyelems[ipol].setX(x);
  }

  void Polygons::setY(Id ipol, const VectorDouble& y)
  {
    if (!_isValidPolyElemIndex(ipol)) return;
    return _polyelems[ipol].setY(y);
  }

  bool Polygons::_isValidPolyElemIndex(Id ipol) const
  {
    auto npol = getNPolyElem();
    if (ipol < 0 || ipol >= npol)
    {
      messerr(
        "PolyElem Index %d is not valid. It should lie in [0,%d[", ipol, npol);
      return false;
    }
    return true;
  }
    ...
    const NamingConvention& namconv)
  {
    ...
    double distmax = 0.;
    double distmin = TEST;
    for (Id iset = 0; iset < polygon->getNPolyElem(); iset++)
    {
    ...
    if (scale != 0 || polin != 0)
    {
      ...
      for (Id iech = 0; iech < nech; iech++)
      {
        ...
        if (polin != 0)
        {
          Id inside = polygon->inside(
            db->getCoordinate(iech, 0), db->getCoordinate(iech, 1));
          if (polin > 0)
          {
            if (!inside) continue;
          }
   ...
   *****************************************************************************/
  VectorInt
    Polygons::_getHullIndices(const VectorDouble& x, const VectorDouble& y)
  {
  ...
  void Polygons::_polygonHullPrint(
    ...
    const VectorDouble& y)
  {
    ...
    message(
      "Ranks (1-based) and coordinates of the Active Samples included in the "
      "Convex Hull\n");
   ...
   */
  void
    Polygons::_getExtend(double ext, VectorDouble& x, VectorDouble& y, Id nsect)
  {
   ...
   **
   *****************************************************************************/
  Id Polygons::_buildHull(const Db* db, double dilate, bool verbose)
  ...
  Polygons Polygons::reduceComplexity(double distmin) const
  {
    ...
    for (Id ipol = 0, npol = getNPolyElem(); ipol < npol; ipol++)
    {
    ...
    const NamingConvention& namconv)
  {
    ...
    {
      mes_process(
        "Checking if sample belongs to a polygon", db->getNSample(), iech);
      Id selval = 0;
    ...
    const NamingConvention& namconv)
  {
    ...
    Id ntotal = db2->getNSample();
    Id nactive = 0;
    Id nout = 0;
    Id nin = 0;

    VectorDouble coor(3, TEST);
    for (Id iech = 0; iech < ntotal; iech++)
    {
