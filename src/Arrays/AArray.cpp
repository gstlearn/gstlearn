...
namespace gstlrn
{
  ...
  String AArray::toString(const AStringFormat* /*strfmt*/) const
  {
    ...
    {
      sstr << "- Dimension #" << idim + 1 << " : " << getNDims(idim)
           << std::endl;
    }
    return sstr.str();
  }
  ...
  Id AArray::indiceToRank(const VectorInt& indice) const
  {
    ...
    if (ival < 0 || ival >= _ndims[ndim - 1]) return (-1);
    for (Id idim = ndim - 2; idim >= 0; idim--)
    {
      if (indice[idim] < 0 || indice[idim] >= _ndims[idim]) return (-1);
  ...
  void AArray::rankToIndice(Id rank, VectorInt& indices) const
  {
    ...
    {
      messerr("Argument indices should have the correct size. Output argument "
              "'indices' not modified");
      return;
    }
  ...
  {
    if (idim < getNDim()) return _ndims[idim];
    return 1;
  }
  ...
  bool AArray::_isValidIndice(const VectorInt& indice) const
  {
    ...
    {
      messerr("Argument 'indice' does not have the correct dimension (%d)",
              static_cast<Id>(indice.size()));
