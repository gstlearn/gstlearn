...
namespace gstlrn
{
  ...
  bool SPDE::_isValidProjection(const Db* db,
  ...
  VectorMeshes SPDE::_defineMeshesFromDbs(bool flagKrige)
  {
    ...
    for (Id jcov = 0, icov = 0; jcov < ncovtot; jcov++)
    {
      ...
      meshes.replace(icov++,
                     MeshETurbo::createFromCova(*cova,
                                                localDomain,
                                                refine,
                                                _params.getBorder(),
                                                _params.isPolarized(),
  ...
  void SPDE::_printMeshesDetails(const VectorMeshes& meshes)
  {
    ...
    for (Id imesh = 0; imesh < nmesh; imesh++)
    {
      message("- Mesh #%d/%d: NMeshes = %d, NApices = %d\n",
              imesh + 1,
              nmesh,
              meshes(imesh)->getNMeshes(),
              meshes(imesh)->getNApices());
    }
  }
  ...
  {
    const ProjMultiMatrix* dest = flagIn ? (flagForKrige ? _AinK : _AinS)
  ...
  Id SPDE::_defineProjection(bool flagIn, bool flagForKrige, bool verbose)
  {
    ...
    const ProjMultiMatrix* src =
      flagIn ? (flagForKrige ? _projInKInit : _projInSInit)
  ...
  Id SPDE::defineSpdeOperator(bool verbose)
  {
    ...
    if (_flagCholesky)
    {
      ...
      {
        messerr(
          "You must provide 'invnoise' as a InvNuggetOp for Cholesky case");
        return 1;
      }
                         ...
                         bool verbose)
  {
    ...
    if (verbose)
      mestitle(1,
               "Trend in SPDE framework (Cholesky=%ld)",
                 ...
                 const NamingConvention& namconv)
  {
    ...
    if (verbose)
      mestitle(1,
               "Kriging in SPDE framework (Cholesky=%d)",
               static_cast<Id>(spde.getFlagCholesky()));
                  ...
                  const NamingConvention& namconv)
  {
    ...
    if (verbose)
      mestitle(1,
               "Simulation in SPDE framework (Conditional=%d, Cholesky=%d)",
               dbin != nullptr,
               static_cast<Id>(spde.getFlagCholesky()));
                           ...
                           bool verbose)
  {
    ...
    if (verbose)
      mestitle(1,
               "Log-likelhood calculation in SPDE framework( Cholesky=%d)",
               static_cast<Id>(spde.getFlagCholesky()));
