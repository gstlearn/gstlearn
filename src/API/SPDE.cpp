...
namespace gstlrn
{
   ...
   */
  void
    SPDE::_defineFlagCholesky(Id useCholesky, const Model* model, bool verbose)
  {
    if (useCholesky == -1)
    {
  ...
  Id SPDE::setMeshes(bool flagForKrig, const VectorMeshes* meshes)
  {
    ...
    if (nmesh != 1 && nmesh != ncov)
    {
      ...
      messerr(
        "or equal to the number of covariances (%d) contained in the Model",
        ncov);
  ...
  Id SPDE::setProjIn(bool flagForKrig, const ProjMulti* proj)
  {
    ...
    if (flagForKrig)
    {
      if (_meshesKInit == nullptr)
      {
        ...
        messerr(
          "as you have not defined specific Meshes for Kriging beforehand");
        return 1;
      }
    ...
    else
    {
      if (_meshesSInit == nullptr)
      {
        ...
        messerr(
          "as you have not defined specific Meshes for Simulations beforehand");
        return 1;
      }
  ...
  bool SPDE::_isValidProjection(
    const Db* db,
    const VectorMeshes* meshes,
    const ProjMulti* proj)
  {
    ...
    if (ncol != napices)
    {
      ...
      for (Id icov = 0; icov < nvar; icov++)
        messerr(
          "- Number of apices for Meshing (%ld) = %d", icov + 1,
          (*meshes)(icov)->getNApices());
      return false;
    }
    ...
    if (nrow != npoint)
    {
      ...
      for (Id ivar = 0; ivar < nvar; ivar++)
        messerr(
          "- Number of samples for variable %ld = %l\n", ivar,
          db->getNSampleActiveAndDefined(ivar));
      return false;
    }
    return true;
  }
  ...
  VectorMeshes SPDE::_duplicateMeshes(bool flagForKrige)
  {
    ...
    for (Id icov = 0; icov < ncov; icov++) meshes.replace(icov, mesh);
    return meshes;
  }
  ...
  VectorMeshes SPDE::_defineMeshesFromDbs(bool flagKrige)
  {
    ...
    for (Id jcov = 0, icov = 0; jcov < ncovtot; jcov++)
    {
      ...
      meshes.replace(
        icov++, MeshETurbo::createFromCova(
                  *cova, localDomain, refine, _params.getBorder(),
                  _params.isPolarized(), true, _params.getNxMax()));
    }
  ...
  void SPDE::_printMeshesDetails(const VectorMeshes& meshes)
  {
    ...
    {
      message(
        "- Mesh #%d/%d: NMeshes = %d, NApices = %d\n", imesh + 1, nmesh,
        meshes(imesh)->getNMeshes(), meshes(imesh)->getNApices());
    }
  }
  ...
  void SPDE::_defineMeshes(bool flagForKrige, bool verbose)
  {
    ...
    if (nmesh == 1 && nmesh != ncov)
    {
      ...
      {
        message(
          "Duplicating the Input Mesh for each one of the %d covariance(s)\n",
          ncov);
        _printMeshesDetails(dest);
      }
      return;
  ...
  {
    const ProjMulti* dest = flagIn ? (flagForKrige ? _AinK : _AinS)
                                   : (flagForKrige ? _AoutK : _AoutS);
    auto& createProj = flagIn ? (flagForKrige ? _createAinK : _createAinS)
  ...
  Id SPDE::_defineProjection(bool flagIn, bool flagForKrige, bool verbose)
  {
    ...
    Id nvar = _model->getNVar();
    if (verbose)
      message("Projection of %s for %s: ", file.c_str(), option.c_str());
    ...
    {
      *dest =
        ProjMultiMatrix::createFromDbAndMeshes(db, meshes, ncov, nvar, flagIn);
      createProj = true;
      if (verbose)
      {
  ...
  Id SPDE::defineSpdeOperator(bool verbose)
  {
    ...
    if (_flagCholesky)
    {
      ...
      const auto* invnoise =
        dynamic_cast<const ASimulableMatrix*>(_invnoiseobj);
      if (invnoise == nullptr)
      {
        messerr(
          "You must provide 'invnoise' as a ASimulableMatrix for Cholesky "
          "case");
        return 1;
      }
      ...
      {
        messerr(
          "You must provide 'AinK' as a ProjMultiMatrix for Cholesky case");
        return 1;
      }
      if (dynamic_cast<const ASimulableMatrix*>(_invnoiseobj) == nullptr)
      {
        messerr(
          "The provided 'invnoise' should be a ASimulableMatrix for Cholesky "
          "case");
        return 1;
      }
  ...
  Id SPDE::centerDataByDriftInPlace(VectorDouble& Z, bool verbose)
  {
    ...
    if (mustEvaluateDrift)
    {
      ...
      if (verbose)
        printVector(_driftCoeffs, "Drift coefficients = ", true, true);
    }
    else
    {
  ...
  void SPDE::addNuggetToResult(VectorDouble& result) const
  {
    ...
    for (Id ivar = 0, ecr = 0; ivar < nvar; ivar++)
    {
      ...
      for (Id iech = 0, nech = static_cast<Id>(result.size()); iech < nech;
           iech++, ecr++)
    ...
    bool verbose)
  {
    ...
    if (verbose)
      mestitle(
        1, "Trend in SPDE framework (Cholesky=%ld)",
    ...
    const NamingConvention& namconv)
  {
    ...
    if (verbose)
      mestitle(
        1, "Kriging in SPDE framework (Cholesky=%d)",
        static_cast<Id>(spde.getFlagCholesky()));
    ...
    const NamingConvention& namconv)
  {
    ...
    if (verbose)
      mestitle(
        1, "Simulation in SPDE framework (Conditional=%d, Cholesky=%d)",
    ...
    const NamingConvention& namconv)
  {
    ...
    if (verbose)
      mestitle(
        1,
        "PluriGaussian Simulation in SPDE framework (Conditional=%d, "
        "Cholesky=%d)",
        dbin != nullptr, static_cast<Id>(spde.getFlagCholesky()));
    ...
    bool verbose)
  {
    ...
    if (verbose)
      mestitle(
        1, "Log-likelhood calculation in SPDE framework( Cholesky=%d)",
        static_cast<Id>(spde.getFlagCholesky()));
  ...
  const MatrixSparse* SPDE::getQ() const
  {
    ...
    const auto* mat =
      dynamic_cast<const PrecisionOpMultiMatrix*>(spdeopmatrix->getQKriging());
    return mat->getQ();
  }
  ...
  const MatrixSparse* SPDE::getProj() const
  {
    ...
    const auto* mat =
      dynamic_cast<const ProjMultiMatrix*>(spdeopmatrix->getProjKriging());
    return mat->getProj();
  }
  ...
  VectorDouble SPDE::stdev(const VectorDouble& Z) const
  {
    ...
    VectorDouble result =
      getSPDEOp()->stdev(Z, getNMC(), getSeed(), getAoutK(), getAoutS());
    return result;
  }
