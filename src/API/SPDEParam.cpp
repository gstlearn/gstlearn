...
namespace gstlrn
{
  ...
  SPDEParam* SPDEParam::create(
    ...
    const CGParam& cgparams)
  {
    return new SPDEParam(
      refineK, refineS, border, flag_polarized, nxmax, epsNugget, useStencil,
      nMC, seedMC, cgparams);
  }
  ...
  String SPDEParam::toString(const AStringFormat* strfmt) const
  {
    ...
    sstr << "Default option for no Cholesky (only used in stationary case for "
            "Turbo Meshing) = "
         << _useStencil << std::endl;
    sstr
      << "Number of Monte-Carlo simulations (used for Variance and logdet) = "
      << _nMC << std::endl;
    sstr << "Seed for the random number generator (used for Variance and "
            "logdet) = "
         << _seedMC << std::endl;
