...
namespace gstlrn
{
                                  ...
                                  VectorInt& uid_grad) const
  {
    ...
    if (_flagGrad)
    {
      Id uid = db->addColumnsByConstant(nvar * _ndim,
                                        value,
                                        "Gradients",
                                        loctype_grad);
  ...
  void Potential::_fillDualSimulation(Id nbsimu, MatrixDense& zvals)
  {
    ...
    for (Id isimu = 0; isimu < nbsimu; isimu++)
    {
      ...
      {
        if (_ndim >= 1)
          zvals.setValue(GRX(ig),
                         isimu,
                         _dbgrd->getSimvar(ELoc::SIMU,
                                           IAD_GRD(ig),
                                           isimu + 0 * nbsimu,
                                           0,
                                           0,
      ...
      for (Id ic = 0; ic < _nlayers; ic++)
        for (Id j = 1; j < _nbPerLayer[ic]; j++)
        {
          zvals.setValue(
            ...
            isimu,
            _dbiso
                ->getSimvar(ELoc::SIMU, IAD_ISO(ic, j), isimu, 0, 0, nbsimu, 1)
              - _dbiso->getSimvar(ELoc::SIMU,
                                  IAD_ISO(ic, 0),
                                  isimu,
                                  0,
                                  0,
                                   ...
                                   double* dist_geo)
  {
    ...
    lhs_aux = dynamic_cast<MatrixSymmetric*>(
      MatrixFactory::createReduceOne(&lhs_orig_arg,
                                     icol0,
                                     icol0,
                                     false,
                                     false));
    ...
    for (Id iech = 0; iech < dbout->getNSample(); iech++)
    {
      ...
      for (Id isimu = 0; isimu < nbsimu; isimu++)
      {
        ...
        for (Id idim = 0; idim < ndim; idim++)
          result[1 + idim] = (_dbgrd->getSimvar(ELoc::SIMU,
                                                iech,
                                                isimu + idim * nbsimu,
                                                0,
                                                0,
                                     ...
                                     double* potval)
  {
    ...
    for (Id ic = 0; ic < _nlayers; ic++)
    {
      ...
      if (nbsimu > 0)
        result[0] = (_dbiso->getSimvar(ELoc::SIMU, ip1, isimu, 0, 0, nbsimu, 1)
                     - result[0]);
                         ...
                         Id nbtuba)
  {
    ...
    /* Processing the non-conditional simulation over the iso-values */
    {
      ...
      if (situba_new
            .simulatePotential(_dbiso, _dbgrd, _dbtgt, dbout, _model, delta))
        return 1;
    }
