...
namespace gstlrn
{
      ...
      for (Id idir = 0; idir < ndir; idir++)
      {
        if (_dirparams[idir].isDefinedForGrid() != definedFromGrid)
        {
          ...
          if (_dirparams[idir].isDefinedForGrid())
            ...
            messerr(
              "Element(%d) is defined NOT using Grid definition", idir + 1);
   ...
   * @return
   */
  VarioParam* VarioParam::createFromSpaceDimension(
    Id nlag,
    double dlag,
    double toldis,
    double tolang,
  ...
  String VarioParam::toStringMain(const AStringFormat* /*strfmt*/) const
  {
    ...
    if (hasDate())
    {
      ...
      sstr << toStrMatrix(
        "Matrix of Bounds for Data Intervals", VectorString(), VectorString(),
