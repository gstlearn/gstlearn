...
namespace gstlrn
{
      ...
      for (Id idir = 0; idir < ndir; idir++)
      {
        if (_dirparams[idir].isDefinedForGrid() != definedFromGrid)
        {
          ...
          if (_dirparams[idir].isDefinedForGrid())
            ...
            messerr("Element(%d) is defined NOT using Grid definition",
                    idir + 1);
  ...
  {
    DirParam* dir = DirParam::createOmniDirection(nlag,
                                                  dlag,
                                                  toldis,
                                                  opt_code,
                                                  idate,
                                         ...
                                         const ASpaceSharedPtr& space)
  {
    std::vector<DirParam> dirs =
      DirParam::createMultiple(ndir, nlag, dlag, toldis, angref, space);
                                                 ...
                                                 Id ndimax)
  {
    ...
    for (Id idim = 0; idim < ncalc; idim++)
    {
      ...
      grincr[idim] = 1;
      DirParam* dirparam =
        DirParam::createFromGrid(dbgrid, nlag, grincr, space);
  ...
  {
    std::vector<DirParam> dirs =
      DirParam::createSeveral2D(angles, nlag, dlag, toldis, tolang, space);
    if (dirs.empty()) return nullptr;
    auto* varioparam = new VarioParam(scale, dates);
    varioparam->addMultiDirs(dirs);
    return varioparam;
  ...
  String VarioParam::toStringMain(const AStringFormat* /*strfmt*/) const
  {
    ...
    if (hasDate())
    {
      ...
      sstr << toStrMatrix("Matrix of Bounds for Data Intervals",
                          VectorString(),
                          VectorString(),
                          false,
                          getNDate(),
  ...
  Id VarioParam::getNDim() const
  {
    ...
    return static_cast<Id>(_dirparams[0].getNDim());
  }
  ...
  Db* buildDbFromVarioParam(Db* db, const VarioParam& varioparam)
  {
    ...
    {
      messerr(
        "This function can only be calculated in dimension equal to 2 or 3");
      return nullptr;
    }
    ...
    {
      messerr(
        "This function requires some direction to be defined in 'VarioParam'");
      return nullptr;
    }
