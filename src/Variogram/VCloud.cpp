...
namespace gstlrn
{
  ...
  Id VCloud::selectFromPolygon(Db* db, Polygons* polygon, Id idir)
  {
    ...
    mestitle(0,
             "Samples in variogram cloud (by decreasing order of occurence)");
   ...
   */
  void VCloud::dumpStorage(Id mode,
                           double distmax,
                           double varmin,
                           Id npairmax,
                           Id ndatamax)
    ...
    if (mode == 0)
    {
      ...
      for (Id ipair = 0; ipair < npairs; ++ipair)
      {
        ...
        message("Pair (%3d, %3d) - Dist: %lf - Value: %lf\n",
                static_cast<Id>(record[0]),
                static_cast<Id>(record[1]),
                record[2],
                record[3]);
      }
    }
