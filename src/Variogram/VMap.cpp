...
#define OPP(idim, i) (dims[idim] - i - 1)
  ...
  double VMap::_getIVAR(const Db* db, Id iech, Id ivar) const
  {
                   ...
                   const NamingConvention& namconv)
  {
    ...
    if (_IPTW >= 0)
      namconv.setNamesAndLocators(db,
                                  VectorString(),
                                  ELoc::Z,
                                  -1,
                                  _dbmap,
  ...
  Id VMap::_grid_fft(DbGrid* dbgrid, const NamingConvention& namconv)
  {
    ...
    for (Id idim = 0; idim < _dbmap->getNDim(); idim++)
    {
    ...
    for (Id i = 0; i < ndim; i++) dinv[i] = dims[ndim - i - 1];
    if (verbose)
    {
      mestitle(0, "Simulation of a grid using FFT");
      message("Grid dimension:");
      for (Id idim = 0; idim < ndim; idim++) message(" %4d", nxgrid[idim]);
      message("\n");
      message("Variogram Map :");
      for (Id idim = 0; idim < ndim; idim++) message(" %4d", nxmap[idim]);
      message("\n");
      message("Working array :");
      for (Id idim = 0; idim < ndim; idim++) message(" %4d", dinv[idim]);
      message("\n");
    }
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
      for (Id jvar = 0; jvar <= ivar; jvar++, ijvar++)
      {
        ...
        if (getCalcul() == ECalcVario::VARIOGRAM)
        {
          ...
          if (fftn(ndim, dinv, ztab[0].data(), ztab[1].data(), -1, -1.))
            continue;
   ...
   **
   *****************************************************************************/
  void VMap::_extract(const Id* nxmap,
                      const Id* nxgrid,
                      Id* dims,
                      VectorDouble& tabin,
                      VectorDouble& tabout)
    ...
    for (Id ix = 0; ix < nxloc - 1; ix++)
      for (Id iy = 0; iy < nyloc - 1; iy++)
        for (Id iz = 0; iz < nzloc; iz++)
        {
          ...
          Id jy = nys2 - iy - 1;
          Id jz = nzs2 + iz;
          tabout[ADD(jx, jy, jz, nxmap)] =
            tabin[ADD(OPP(0, ix), OPP(1, iy), iz, dims)];
        }
    ...
    for (Id ix = 0; ix < nxloc - 1; ix++)
      for (Id iy = 0; iy < nyloc; iy++)
        for (Id iz = 0; iz < nzloc - 1; iz++)
        {
          ...
          Id jz = nzs2 - iz - 1;
          tabout[ADD(jx, jy, jz, nxmap)] =
            tabin[ADD(OPP(0, ix), iy, OPP(2, iz), dims)];
        }
    ...
    for (Id ix = 0; ix < nxloc; ix++)
      for (Id iy = 0; iy < nyloc - 1; iy++)
        for (Id iz = 0; iz < nzloc - 1; iz++)
        {
          ...
          Id jz = nzs2 - iz - 1;
          tabout[ADD(jx, jy, jz, nxmap)] =
            tabin[ADD(ix, OPP(1, iy), OPP(2, iz), dims)];
        }
    ...
    for (Id ix = 0; ix < nxloc - 1; ix++)
      for (Id iy = 0; iy < nyloc - 1; iy++)
        for (Id iz = 0; iz < nzloc - 1; iz++)
        {
          ...
          Id jz = nzs2 - iz - 1;
          tabout[ADD(jx, jy, jz, nxmap)] =
            tabin[ADD(OPP(0, ix), OPP(1, iy), OPP(2, iz), dims)];
        }
  }
   ...
   **
   *****************************************************************************/
  Id VMap::_vmap_general(Db* db, Id radius, const NamingConvention& namconv)
  {
    DECLARE_UNUSED(namconv);
    Id flag_out, iech0, iech1, iech2;
    double x0;
    ...
    for (Id jech1 = 0; jech1 < nech; jech1++)
    {
      ...
      for (Id jech2 = jech1; jech2 < nech; jech2++)
      {
        iech2 = ind1[jech2];
        ...
        for (Id idim = 0; idim < ndim; idim++) delta[idim] = -delta[idim];
        if (point_to_grid(_dbmap, delta.data(), 0, indg0.data())) continue;
        for (Id in = 0; in < nbmax; in++)
        {
          iech0 = _findNeighCell(indg0, neigh, in, indg1);
  ...
  Id VMap::_vmap_grid(DbGrid* dbgrid, const NamingConvention& namconv)
  {
    ...
    for (Id iech1 = 0; iech1 < dbgrid->getNSample(); iech1++)
    {
      ...
      for (Id iech2 = 0; iech2 < dbgrid->getNSample(); iech2++)
      {
        if (!dbgrid->isActive(iech2)) continue;
        dbgrid->rankToIndice(iech2, ind2);

        for (Id idim = flag_out = 0; idim < ndim && flag_out == 0; idim++)
        {
          delta = ind1[idim] - ind2[idim];
          Id moitie = (_dbmap->getNX(idim) - 1) / 2;
   ...
   **
   *****************************************************************************/
  Id VMap::_get_variable_order(Id nvar, Id ivar0, Id jvar0)
  {
    Id rank = 0;
    for (Id ivar = 0; ivar < nvar; ivar++)
      for (Id jvar = 0; jvar <= ivar; jvar++, rank++)
      {
   ...
   **
   *****************************************************************************/
  void VMap::_complexArrayAlloc(Id size, VectorVectorDouble& tab)
  {
    tab.resize(2);
    for (Id ic = 0; ic < 2; ic++) tab[ic].resize(size);
  }
   ...
   **
   *****************************************************************************/
  Id VMap::_vmap_load_simple(DbGrid* dbgrid,
                             Id ndim,
                             Id sizetot,
                             const Id* dims,
                             Id* dinv,
                             ...
                             VectorVectorDouble& z2z1)
  {
    ...
    for (Id ix = 0; ix < dims[0]; ix++)
      for (Id iy = 0; iy < dims[1]; iy++)
        for (Id iz = 0; iz < dims[2]; iz++, ecr++)
        {
          ...
          indice[1] = iy;
          indice[2] = iz;
          Id iech = dbgrid->indiceToRank(indice);
    ...
    if (!i1i2.empty()
        && fftn(ndim, dinv, i1i2[0].data(), i1i2[1].data(), 1, 1.))
      return (1);
    if (!z1i2.empty()
        && fftn(ndim, dinv, z1i2[0].data(), z1i2[1].data(), 1, 1.))
      return (1);
    if (!z2i1.empty()
        && fftn(ndim, dinv, z2i1[0].data(), z2i1[1].data(), 1, 1.))
      return (1);
    if (!z2z1.empty()
        && fftn(ndim, dinv, z2z1[0].data(), z2z1[1].data(), 1, 1.))
      return (1);
    return (0);
  }
   ...
   **
   *****************************************************************************/
  Id VMap::_vmap_load_cross(DbGrid* dbgrid,
                            Id ndim,
                            Id sizetot,
                            const Id* dims,
                            Id* dinv,
                            ...
                            VectorVectorDouble& z2i2)
  {
    ...
    if (!i1i1.empty()
        && fftn(ndim, dinv, i1i1[0].data(), i1i1[1].data(), 1, 1.))
      return (1);
    if (!z1i1.empty()
        && fftn(ndim, dinv, z1i1[0].data(), z1i1[1].data(), 1, 1.))
      return (1);
    if (!i2i2.empty()
        && fftn(ndim, dinv, i2i2[0].data(), i2i2[1].data(), 1, 1.))
      return (1);
    if (!z2i2.empty()
        && fftn(ndim, dinv, z2i2[0].data(), z2i2[1].data(), 1, 1.))
      return (1);
    return (0);
  }
   ...
   **
   *****************************************************************************/
  void VMap::_vmap_rescale(double scale, VectorDouble& tab1, VectorDouble& tab2)
  {
   ...
   **
   *****************************************************************************/
  void VMap::_vmap_store(VectorDouble& tab, Id iptr)
  {
    Id ndim = _dbmap->getNDim();
    VectorInt indice(3, 0);
    VectorDouble dims(3);
    ...
    for (Id ix = 0; ix < dims[0]; ix++)
      for (Id iy = 0; iy < dims[1]; iy++)
        for (Id iz = 0; iz < dims[2]; iz++, ecr++)
        {
          ...
          indice[1] = iy;
          indice[2] = iz;
          Id iech = _dbmap->indiceToRank(indice);
   ...
   **
   *****************************************************************************/
  Id VMap::_findNeighCell(const VectorInt& indg0,
   ...
   **
   *****************************************************************************/
  void VMap::_vmap_normalize(Id nv2)
  {
    for (Id iech = 0; iech < _dbmap->getNSample(); iech++)
    {
