...
#define OPP(idim, i) (dims[idim] - i - 1)
    ...
    const NamingConvention& namconv)
  {
    ...
    for (Id idim = 0; idim < _dbmap->getNDim(); idim++)
    {
      ...
      {
        messerr(
          "The grid mesh in the direction %d (dx=%lf)", idim + 1,
          dbgrid->getDX(idim));
        messerr(
          "must match the mesh of the Variogram Map grid (dx=%lf)",
          _dbmap->getDX(idim));
        return (1);
      }
    }
  ...
  Id VMap::_vmap_general(Db* db, Id radius, const NamingConvention& namconv)
  {
    ...
    VectorDouble coor = db->getVecCoordinate(0);
