...
#include "ExternalTools/triangle.hpp"
...
//#define FILENAMESIZE 2048 (defined in tetrahedron.hpp)
/* Maximum number of characters in a line read from a file (including the    */
/*   null).                                                                  */

//#define INPUTLINESIZE 1024 (define in tetrahedron.hpp)
...
typedef DREAL **triangle; /* Really:  typedef triangle *triangle   */
...
typedef DREAL **subseg; /* Really:  typedef subseg *subseg   */
...
/* The vertex data structure.  Each vertex is actually an array of DREALs.    */
/*   The number of DREALs is unknown until runtime.  An integer boundary      */
/*   marker, and sometimes a pointer to a triangle, is appended after the    */
/*   DREALs.                                                                  */

typedef DREAL *vertex;
...
struct badtriang
{
  ...
  DREAL key; /* cos^2 of smallest (apical) angle. */
...
struct event
{
  DREAL xkey, ykey; /* Coordinates of the event. */
  VOID_D *eventptr; /* Can be a vertex or the location of a circle event. */
...
struct memorypool
{
  VOID_D **firstblock, **nowblock;
  VOID_D *nextitem;
  VOID_D *deaditemstack;
  VOID_D **pathblock;
  VOID_D *pathitem;
  int alignbytes;
  int itembytes;
...
DREAL splitter; /* Used to split DREAL factors for exact multiplication. */
DREAL epsilon; /* Floating-point machine epsilon. */
DREAL resulterrbound;
DREAL ccwerrboundA, ccwerrboundB, ccwerrboundC;
DREAL iccerrboundA, iccerrboundB, iccerrboundC;
DREAL o3derrboundA, o3derrboundB, o3derrboundC;
...
struct mesh
{
  ...
  DREAL xmin, xmax, ymin, ymax; /* x and y bounds. */
  DREAL xminextreme; /* Nonexistent x value used as a flag in sweepline. */
...
struct behavior
{
  ...
  int nobisect;
  int steiner;
  DREAL minangle, goodangle, offconstant;
  DREAL maxarea;
...
/*  triangles are four-byte-aligned and that the `UNSIGNED_LONG' type is     */
...
#define elemattribute(otri, attnum)                                           \
  ((DREAL *) (otri).tri)[m->elemattribindex + (attnum)]

#define setelemattribute(otri, attnum, value)                                 \
  ((DREAL *) (otri).tri)[m->elemattribindex + (attnum)] = value

/* Check or set a triangle's maximum area bound.                             */

#define areabound(otri)  ((DREAL *) (otri).tri)[m->areaboundindex]

#define setareabound(otri, value)                                             \
  ((DREAL *) (otri).tri)[m->areaboundindex] = value
...
int triunsuitable(vertex triorg, vertex tridest, vertex triapex, DREAL /*area*/)
...
DREAL area;                                      /* The area of the triangle. */
#endif /* not ANSI_DECLARATORS */

{
  DREAL dxoa, dxda, dxod;
  DREAL dyoa, dyda, dyod;
  DREAL oalen, dalen, odlen;
  DREAL maxlen;
  ...
  maxlen = (dalen > oalen) ? dalen :
                             oalen;
  maxlen = (odlen > maxlen) ? odlen :
                              maxlen;
...
{
  messageAbort("Error during Triangulation");
}

#ifdef ANSI_DECLARATORS
VOID_D* trimalloc(int size)
#else /* not ANSI_DECLARATORS */
VOID_D *trimalloc(size)
int size;
#endif /* not ANSI_DECLARATORS */

{
  ...
  memptr = (VOID_D*) mem_alloc((unsigned int ) size, 1);
  return (memptr);
}

#ifdef ANSI_DECLARATORS
void trifree(VOID_D *memptr)
#else /* not ANSI_DECLARATORS */
void trifree(memptr)
VOID_D *memptr;
#endif /* not ANSI_DECLARATORS */
...
#ifdef CDT_ONLY
#ifdef REDUCED
  message("triangle [-pAcjevngBPNEIOXzo_lQVh] input_file\n");
#else /* not REDUCED */
  message("triangle [-pAcjevngBPNEIOXzo_iFlCQVh] input_file\n");
#endif /* not REDUCED */
#else /* not CDT_ONLY */
#ifdef REDUCED
  message("triangle [-prq__a__uAcDjevngBPNEIOXzo_YS__lQVh] input_file\n");
#else /* not REDUCED */
  message("triangle [-prq__a__uAcDjevngBPNEIOXzo_YS__iFlsCQVh] input_file\n");
#endif /* not REDUCED */
#endif /* not CDT_ONLY */

  message("    -p  Triangulates a Planar Straight Line Graph (.poly file).\n");
#ifndef CDT_ONLY
  message("    -r  Refines a previously generated mesh.\n");
  message("    -q  Quality mesh generation.  A minimum angle may be specified.\n");
  message("    -a  Applies a maximum triangle area constraint.\n");
  message("    -u  Applies a user-defined triangle constraint.\n");
#endif /* not CDT_ONLY */
  message("    -A  Applies attributes to identify triangles in certain regions.\n");
  message("    -c  Encloses the convex hull with segments.\n");
#ifndef CDT_ONLY
  message("    -D  Conforming Delaunay:  all triangles are truly Delaunay.\n");
#endif /* not CDT_ONLY */
  message("    -j  Jettison unused vertices from output .node file.\n");
  message("    -e  Generates an edge list.\n");
  message("    -v  Generates a Voronoi diagram.\n");
  message("    -n  Generates a list of triangle neighbors.\n");
  message("    -g  Generates an .off file for Geomview.\n");
  message("    -B  Suppresses output of boundary information.\n");
  message("    -P  Suppresses output of .poly file.\n");
  message("    -N  Suppresses output of .node file.\n");
  message("    -E  Suppresses output of .ele file.\n");
  message("    -I  Suppresses mesh iteration numbers.\n");
  message("    -O  Ignores holes in .poly file.\n");
  message("    -X  Suppresses use of exact arithmetic.\n");
  message("    -z  Numbers all items starting from zero (rather than one).\n");
  message("    -o2 Generates second-order subparametric elements.\n");
#ifndef CDT_ONLY
  message("    -Y  Suppresses boundary segment splitting.\n");
  message("    -S  Specifies maximum number of added Steiner points.\n");
#endif /* not CDT_ONLY */
#ifndef REDUCED
  message("    -i  Uses incremental method, rather than divide-and-conquer.\n");
  message("    -F  Uses Fortune's sweepline algorithm, rather than d-and-c.\n");
#endif /* not REDUCED */
  message("    -l  Uses vertical cuts only, rather than alternating cuts.\n");
#ifndef REDUCED
#ifndef CDT_ONLY
  message(
    "    -s  Force segments into mesh by splitting (instead of using CDT).\n");
#endif /* not CDT_ONLY */
  message("    -C  Check consistency of final mesh.\n");
#endif /* not REDUCED */
  message("    -Q  Quiet:  No terminal output except errors.\n");
  message("    -V  Verbose:  Detailed information on what I'm doing.\n");
  message("    -h  Help:  Detailed instructions for Triangle.\n");
  triexit(0);
}
...
"Copyright 1993, 1995, 1997, 1998, 2002, 2005 Jonathan Richard Shewchuk\n");
  message("2360 Woolsey #H / Berkeley, California 94705-1927\n");
  message("Bugs/comments to jrs@cs.berkeley.edu\n");
  message(
"Created as part of the Quake project (tools for earthquake simulation).\n");
  message(
"Supported in part by NSF Grant CMS-9318163 and an NSERC 1967 Scholarship.\n");
  message("There is no warranty whatsoever.  Use at your own risk.\n");
#ifdef SINGLE
  message("This executable is compiled for single precision arithmetic.\n\n\n");
#else /* not SINGLE */
  message("This executable is compiled for double precision arithmetic.\n\n\n");
#endif /* not SINGLE */
  message(
...
"and possibly a .poly file and an .area file as well.  The formats of these\n"
  ...
  message("files are described below.\n\n");
...
"        triunsuitable().  In either case, the -u switch causes the user-\n");
  message("        defined test to be applied to every triangle.\n");
  message(
...
"        .poly file to be written containing the boundary edges of the mesh\n"
  ...
  message("        (useful if no .poly file was read).\n");
  message(
...
"    -g  Outputs the mesh to an Object File Format (.off) file, suitable for\n"
  ...
  message("        viewing with the Geometry Center's Geomview package.\n");
  message(
...
"        to maintain constraining segments on later refinements of the mesh.\n"
  ...
  message("    -N  No output .node file.\n");
  message("    -E  No output .ele file.\n");
  message(
...
"        using a .node file for input, because no .node file is written, so\n"
  ...
  message("        there is no record of any added Steiner points.\n");
  message("    -O  No holes.  Ignores the holes in the .poly file.\n");
  message(
...
"        improvement in speed and creates the possibility that Triangle will\n"
  ...
  message("        fail to produce a valid mesh.  Not recommended.\n");
  message(
...
"        always adds enough vertices to ensure that all the segments of the\n"
  ...
  message("        PLSG are recovered, ignoring the limit if necessary.\n");
  message(
...
"        construct a Delaunay triangulation.  Try it if the divide-and-\n");
  message("        conquer algorithm fails.\n");
  message(
...
"        triangulation.  Warning:  does not use exact arithmetic for all\n");
  message("        calculations.  An exact result is not guaranteed.\n");
  message(
...
"        prints so much that Triangle runs much more slowly.  `-VVVV' gives\n"
  ...
  message("        information only a debugger could love.\n");
  message("    -h  Help:  Displays these instructions.\n");
  message("\n");
  message("Definitions:\n");
  message("\n");
  message(
...
"  inside the circumscribing circle (circle that passes through all three\n");
  message("  vertices) of any triangle in the triangulation.\n\n");
  message(
...
"  to some input vertex than to any other input vertex.  The Voronoi diagram\n"
  ...
  message("  is a geometric dual of the Delaunay triangulation.\n\n");
  message(
...
"  Segments may intersect each other only at their endpoints.  The file\n");
  message("  format for PSLGs (.poly files) is described below.\n\n");
  message(
...
"  cover the convex hull of the PSLG, or it might cover only a segment-\n");
  message("  bounded region (e.g. a polygon).\n\n");
  message(
...
"  The new vertices are frequently necessary to maintain the Delaunay\n");
  message("  property while ensuring that every segment is represented.\n\n");
  message(
...
"  triangulation of a PSLG whose triangles are constrained Delaunay.  New\n");
  message("  vertices may appear, and input segments may be subdivided into\n");
  message(
...
"  another program, use the -z switch if you wish to number objects from\n");
  message("  zero.)  Examples of these file formats are given below.\n\n");
  message("  .node files:\n");
  message(
...
"    -a, -u, -D, or -s is selected, each new Steiner point added to the mesh\n"
  ...
  message("    has attributes assigned to it by linear interpolation.\n\n");
  message(
...
"    .node file produced by Triangle contains boundary markers in the last\n");
  message("    column unless they are suppressed by the -B switch.\n\n");
  message("  .ele files:\n");
  message(
...
"    around each triangle.  (The remaining nodes, if any, depend on the type\n"
  ...
  message("    of finite element used.)\n\n");
  message(
...
"    among nearby triangles as the triangulation is refined.  Attributes do\n"
  ...
  message("    not diffuse across segments, so attributes used to identify\n");
  message("    segment-bounded regions remain intact.\n\n");
  message(
...
"    Following lines:  <vertex #> <x> <y> [attributes] [boundary marker]\n");
  message("    One line:  <# of segments> <# of boundary markers (0 or 1)>\n");
  message(
"    Following lines:  <segment #> <endpoint> <endpoint> [boundary marker]\n");
  message("    One line:  <# of holes>\n");
  message("    Following lines:  <hole #> <x> <y>\n");
  message(
...
"    means that you must include its endpoints in the vertex list.  Each\n");
  message("    segment, like each point, may have a boundary marker.\n\n");
  message(
...
"    segment itself is also eaten.  Do not place a hole directly on a\n");
  message("    segment; if you do, Triangle chooses one side of the segment\n");
  message("    arbitrarily.\n\n");
  message(
...
"    simultaneously and wish to assign an attribute to some region without\n");
  message("    imposing an area constraint, use a negative maximum area.\n\n");
  message(
...
"    the segment, and you'll have a needle-sharp triangle in your output--or\n"
  ...
  message("    a lot of tiny triangles if you're generating a quality mesh.\n");
  message("\n");
  message(
...
"    mesh, and don't want segments to be missing in later triangulations.\n");
  message("\n");
  message("  .area files:\n");
  message("    First line:  <# of triangles>\n");
  message("    Following lines:  <triangle #> <maximum area>\n");
  message("\n");
  message(
...
"    triangle may be left unconstrained by assigning it a negative maximum\n");
  message("    area.\n\n");
  message("  .edge files:\n");
  message("    First line:  <# of edges> <# of boundary markers (0 or 1)>\n");
  message(
...
"    infinite ray with only one endpoint.  For these edges, a different\n");
  ...
  message("        <edge #> <endpoint> -1 <direction x> <direction y>\n\n");
  message(
"    The `direction' is a floating-point vector that indicates the direction\n"
);
  message("    of the infinite ray.\n\n");
  message("  .neigh files:\n");
  message(
...
"    boundary).  The first neighbor of triangle i is opposite the first\n");
  message("    corner of triangle i, and so on.\n\n");
  message(
...
"  finite element mesh.  You can prevent boundary markers from being written\n"
  ...
  message("  into files produced by Triangle by using the -B switch.\n\n");
  message(
"  The boundary marker associated with each segment in an output .poly file\n"
);
  message("  and each edge in an output .edge file is chosen as follows:\n");
  message(
...
"      (even the boundary of a hole), then the edge is assigned the marker\n");
  message("      one (1).\n");
  message("    - Otherwise, the edge is assigned the marker zero (0).\n");
  message(
"  The boundary marker associated with each vertex in an output .node file\n");
  message("  is chosen as follows:\n");
  message(
...
"      vertex is assigned the same marker.  If the vertex lies on several\n");
  message("      such segments, one of the markers is chosen arbitrarily.\n");
  message(
"    - Otherwise, if the vertex occurs on a boundary of the triangulation,\n");
  message("      then the vertex is assigned the marker one (1).\n");
  message("    - Otherwise, the vertex is assigned the marker zero (0).\n");
  message("\n");
  message(
...
"  all) in your input files.  In the output files, all boundary vertices,\n");
  message("  edges, and segments will be assigned the value one.\n\n");
  message("Triangulation Iteration Numbers:\n\n");
  message(
...
"  triangulation, and output the files mesh.4.node, mesh.4.ele, and\n");
  message("  mesh.4.poly.  Files with no iteration number are treated as if\n");
  message(
...
"  points, a .node file is written.  This can be quite convenient for\n");
  message("  computing CDTs or quality meshes.)\n\n");
  message("Examples of How to Use Triangle:\n\n");
  message(
...
"  mesh to object.1.node and object.1.ele.  Each segment may be broken up\n");
  message("  into multiple subsegments; these are written to object.1.poly.\n");
  message("\n");
  message(
...
"    # A box with eight vertices in 2D, no attributes, one boundary marker.\n"
  ...
  message("    8 2 0 1\n");
  message("     # Outer box has these vertices:\n");
  message("     1   0 0   0\n");
  message("     2   0 3   0\n");
  message("     3   3 0   0\n");
  message("     4   3 3   33     # A special marker for this vertex.\n");
  message("     # Inner square has these vertices:\n");
  message("     5   1 1   0\n");
  message("     6   1 2   0\n");
  message("     7   2 1   0\n");
  message("     8   2 2   0\n");
  message("    # Five segments with boundary markers.\n");
  message("    5 1\n");
  message("     1   1 2   5      # Left side of outer box.\n");
  message("     # Square hole has these segments:\n");
  message("     2   5 7   0\n");
  message("     3   7 8   0\n");
  message("     4   8 6   10\n");
  message("     5   6 5   0\n");
  message("    # One hole in the middle of the inner square.\n");
  message("    1\n");
  message("     1   1.5 1.5\n");
  message("\n");
  message(
...
"  other vertices but 4 have been marked to indicate that they lie on a\n");
  ...
  message("      11    3   1.5    1\n");
  message("      12    1.5   3    1\n");
  message("    # Generated by triangle -pqc box.poly\n");
  message("\n");
  message("  Here is the output file `box.1.ele', with twelve triangles.\n");
  message("\n");
  message("    12  3  0\n");
  message("       1     5   6   9\n");
  message("       2    10   3   7\n");
...
"  newly added vertices.  Note also that <# of vertices> is set to zero to\n");
  message("  indicate that the vertices should be read from the .node file.\n");
  message("\n");
  message("    0  2  0  1\n");
  message("    12  1\n");
  message("       1     1   9     5\n");
  ...
  message("    # Generated by triangle -pqc box.poly\n");
  message("\n");
  message("Refinement and Area Constraints:\n");
  message("\n");
  message(
...
"  consider instead using the -u switch and writing a C procedure that\n");
  message("  determines which triangles are too large.\n\n");
  message(
...
"  the nodes at the corners of an element; the midpoint nodes on each edge\n");
  message("  are discarded before the mesh is refined.\n\n");
  message(
...
"  are typically based on a posteriori error estimates resulting from a\n");
  message("  finite element simulation on that mesh.\n\n");
  message(
...
"  x.2' creates a sequence of successively finer meshes x.1, x.2, and x.3,\n");
  message("  suitable for multigrid.\n\n");
  message("Convex Hulls and Mesh Boundaries:\n\n");
  message(
...
"  There are faster algorithms for finding a two-dimensional convex hull\n");
  message("  than triangulation, of course, but this one comes for free.\n\n");
  message(
...
"  If you also use the -p switch, the output .poly file contains all the\n");
  message("  segments from the input .poly file as well.\n\n");
  message("Voronoi Diagrams:\n\n");
  message(
...
"  rays.  (The choice of filenames makes it easy to run the set of Voronoi\n");
  message("  vertices through Triangle, if so desired.)\n\n");
  message(
...
"  construct a conforming Delaunay triangulation instead of a CCDT, so the\n");
  message("  Voronoi diagram will be valid.\n\n");
...
"  output files with the recollection that the Delaunay triangulation and\n");
  message("  the Voronoi diagram are planar duals.\n\n");
  message(
...
"  vertex j of the corresponding .v.node file.  Voronoi cell k is the dual\n");
  message("  of vertex k of the corresponding .node file.\n\n");
  message(
...
"  respectively.  To find which Voronoi cells are adjacent to each other,\n");
  message("  just read the list of Delaunay edges.\n\n");
  message(
...
"  Voronoi vertices attached to it.  You might find this more convenient\n");
  message("  than the .v.edge file.\n\n");
  message("Quadratic Elements:\n\n");
  message(
...
"  and sixth nodes appearing opposite the first, second, and third corners\n");
  ...
  message("Domains with Small Angles:\n\n");
  message(
...
"  is reduced to memory precision.  Triangle does this if it is compiled\n");
  message("  correctly.  See the makefile for details.\n\n");
  message(
...
"  inputs most likely to cause errors if you use the -X switch.  Hence, the\n"
  ...
  message("  -X switch is not recommended.\n\n");
  message(
...
"  files.)  Similarly, exact arithmetic is not used to compute the vertices\n"
  ...
  message("  of the Voronoi diagram.\n\n");
  message(
...
"  being performed exactly, while overflow typically causes a floating\n");
  message("  exception.\n\n");
  message("Calling Triangle from Another Program:\n\n");
  message("  Read the file triangle.h for details.\n\n");
  message("Troubleshooting:\n\n");
  message("  Please read this section before mailing me bugs.\n\n");
  message("  `My output mesh has no triangles!'\n\n");
  message(
...
"    have placed a hole badly, thereby eating all your triangles.  To test\n");
  message("    these possibilities, try again with the -c and -O switches.\n");
  message(
"    Alternatively, all your input vertices may be collinear, in which case\n"
);
  message("    you can hardly expect to triangulate them.\n\n");
  message("  `Triangle doesn't terminate, or just crashes.'\n\n");
  message(
...
"    between 6247133 and 6247134, you're not leaving much floating-point\n");
  message("    precision for Triangle to work with.\n\n");
  message(
...
"    above.)  Again, I recommend compiling Triangle for double (rather\n");
  message("    than single) precision arithmetic.\n\n");
  message(
...
"    don't use the -X switch.  If you still have problems, please report the\n"
  ...
  message(
"  `Triangle executes without incident, but when I look at the resulting\n");
  message("  Voronoi diagram, it has overlapping edges or other geometric\n");
...
"    holes, and you use the -D switch to construct a conforming Delaunay\n");
  message("    triangulation (instead of a CDT or CCDT).\n\n");
  message(
...
"  doesn't, and you'll have to connect each adjacent pair of vertices with a\n"
  ...
  message("  separate segment.  If you don't like it, tough.\n\n");
  message(
...
"  endpoints, try not to let the intersections fall extremely close to PSLG\n"
  ...
  message("  vertices or each other.\n\n");
  message(
...
"  triangles constrained Delaunay?  Triangle's Delaunay refinement algorithm\n"
  ...
  message("  assumes that it starts with a CDT.\n\n");
  message("Show Me:\n\n");
  message(
...
"  you have the X Windows system.  Sorry, Microsoft Windows users.\n");
  message("\n");
  message("Triangle on the Web:\n");
  message("\n");
  message("  To see an illustrated version of these instructions, check out\n");
  message("\n");
  message("    http://www.cs.cmu.edu/~quake/triangle.html\n");
  message("\n");
  message("A Brief Plea:\n");
  message("\n");
  message(
...
"  you.  Also, I can put you on a list to receive email whenever a new\n");
  message("  version of Triangle is available.\n\n");
  message(
...
"  Schachter, Micha Sharir, Peter W. Shor, Daniel D. Sleator, Jorge Stolfi,\n"
  ...
  message("  Robert E. Tarjan, Alper Ungor, Christopher J. Van Wyk, Noel J.\n");
  message(
"  Walkington, and Binhai Zhu.  See the comments at the beginning of the\n");
  message("  source code for references.\n\n");
  triexit(0);
}
...
void internalerror()
{
  messerr("  Please report this bug to jrs@cs.berkeley.edu");
  messerr("  Include the message above, your input data set, and the exact");
  messerr("    command line you used to run Triangle.");
  triexit(1);
}
...
#define STARTINDEX 1
  ...
  if (b->weighted && (b->poly || b->quality))
  {
    ...
    if (!b->quiet)
    {
      message("Warning:  weighted triangulations (-w, -W) are incompatible\n");
      message(
  ...
  {
    message("Warning:  -j and -N switches are somewhat incompatible.\n");
    message("  If any vertices are jettisoned, you will need the output\n");
    message("  .node file to reconstruct the new node indices.");
  }
...

{
  ...
  message("triangle x%lx with orientation %d:\n", (UNSIGNED_LONG) t->tri,
          t->orient);
  decode(t->tri[0], printtri);
  if (printtri.tri == m->dummytri)
  {
    message("    [0] = Outer space\n");
  }
  else
  {
  ...
  if (printvertex == (vertex) NULL)
    message("    Origin[%d] = NULL\n", (t->orient + 1) % 3 + 3);
  else
    message("    Origin[%d] = x%lx  (%.12g, %.12g)\n", (t->orient + 1) % 3 + 3,
            (UNSIGNED_LONG) printvertex, printvertex[0], printvertex[1]);
  dest(*t, printvertex);
  if (printvertex == (vertex) NULL)
    message("    Dest  [%d] = NULL\n", (t->orient + 2) % 3 + 3);
  else
    message("    Dest  [%d] = x%lx  (%.12g, %.12g)\n", (t->orient + 2) % 3 + 3,
            (UNSIGNED_LONG) printvertex, printvertex[0], printvertex[1]);
  apex(*t, printvertex);
  if (printvertex == (vertex) NULL)
    message("    Apex  [%d] = NULL\n", t->orient + 3);
  else
    message("    Apex  [%d] = x%lx  (%.12g, %.12g)\n", t->orient + 3,
            (UNSIGNED_LONG) printvertex, printvertex[0], printvertex[1]);
...

{
  ...
  message("subsegment x%lx with orientation %d and mark %d:\n",
          (UNSIGNED_LONG) s->ss, s->ssorient, mark(*s));
  sdecode(s->ss[0], printsh);
  if (printsh.ss == m->dummysub)
  {
    message("    [0] = No subsegment\n");
  }
  else
  {
    message("    [0] = x%lx  %d\n", (UNSIGNED_LONG) printsh.ss,
  ...
  if (printvertex == (vertex) NULL)
    message("    Segment origin[%d] = NULL\n", 4 + s->ssorient);
  else
    message("    Segment origin[%d] = x%lx  (%.12g, %.12g)\n", 4 + s->ssorient,
            (UNSIGNED_LONG) printvertex, printvertex[0], printvertex[1]);
  segdest(*s, printvertex);
  if (printvertex == (vertex) NULL)
    message("    Segment dest  [%d] = NULL\n", 5 - s->ssorient);
  else
    message("    Segment dest  [%d] = x%lx  (%.12g, %.12g)\n", 5 - s->ssorient,
            (UNSIGNED_LONG) printvertex, printvertex[0], printvertex[1]);
}
...
{
  pool->firstblock = (VOID_D**) NULL;
  pool->nowblock = (VOID_D**) NULL;
  pool->nextitem = (VOID_D*) NULL;
  pool->deaditemstack = (VOID_D*) NULL;
  pool->pathblock = (VOID_D**) NULL;
  pool->pathitem = (VOID_D*) NULL;
  pool->alignbytes = 0;
  pool->itembytes = 0;
...

{
  ...
  /* Find the first item in the pool.  Increment by the size of (VOID_D *). */
  alignptr = (UNSIGNED_LONG) (pool->nowblock + 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (VOID_D*) (alignptr + (UNSIGNED_LONG) pool->alignbytes
...

{
  ...
  /*   - sizeof(VOID_D *), so the stack of dead items can be maintained */
  /*       without unaligned accesses.                                */
  if ((int) alignment > (int) sizeof(VOID_D*))
  {
    pool->alignbytes = alignment;
  }
  else
  {
    pool->alignbytes = sizeof(VOID_D*);
  }
  ...
  pool->firstblock = (VOID_D**) trimalloc(
      pool->itemsfirstblock * pool->itembytes + (int) sizeof(VOID_D*)
      + pool->alignbytes);
...
{
  while (pool->firstblock != (VOID_D**) NULL)
  {
    pool->nowblock = (VOID_D**) *(pool->firstblock);
    trifree((VOID_D*) pool->firstblock);
...
#ifdef ANSI_DECLARATORS
VOID_D* poolalloc(struct memorypool *pool)
#else /* not ANSI_DECLARATORS */
VOID_D *poolalloc(pool)
struct memorypool *pool;
#endif /* not ANSI_DECLARATORS */

{
  ...
  if (pool->deaditemstack != (VOID_D*) NULL)
  {
    newitem = pool->deaditemstack; /* Take first item in list. */
    pool->deaditemstack = *(VOID_D**) pool->deaditemstack;
  }
  else
  {
    ...
    if (pool->unallocateditems == 0)
    {
      ...
      if (*(pool->nowblock) == (VOID_D*) NULL)
      {
        /* Allocate a new block of items, pointed to by the previous block. */
        newblock = (VOID_D**) trimalloc(
            pool->itemsperblock * pool->itembytes + (int) sizeof(VOID_D*)
            + pool->alignbytes);
        *(pool->nowblock) = (VOID_D*) newblock;
        /* The next block pointer is NULL. */
        *newblock = (VOID_D*) NULL;
      }

      /* Move to the new block. */
      pool->nowblock = (VOID_D**) *(pool->nowblock);
      /* Find the first item in the block.    */
      /*   Increment by the size of (VOID_D *). */
      alignptr = (UNSIGNED_LONG) (pool->nowblock + 1);
      /* Align the item on an `alignbytes'-byte boundary. */
      pool->nextitem = (VOID_D*) (alignptr + (UNSIGNED_LONG) pool->alignbytes
          - (alignptr % (UNSIGNED_LONG) pool->alignbytes));
    ...
    pool->nextitem = (VOID_D*) ((char*) pool->nextitem + pool->itembytes);
...
#ifdef ANSI_DECLARATORS
void pooldealloc(struct memorypool *pool, VOID_D *dyingitem)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  *((VOID_D**) dyingitem) = pool->deaditemstack;
...

{
  ...
  /* Find the first item in the block.  Increment by the size of (VOID_D *). */
  alignptr = (UNSIGNED_LONG) (pool->pathblock + 1);
  /* Align with item on an `alignbytes'-byte boundary. */
  pool->pathitem = (VOID_D*) (alignptr + (UNSIGNED_LONG) pool->alignbytes
      - (alignptr % (UNSIGNED_LONG) pool->alignbytes));
...
#ifdef ANSI_DECLARATORS
VOID_D* traverse(struct memorypool *pool)
#else /* not ANSI_DECLARATORS */
VOID_D *traverse(pool)
struct memorypool *pool;
#endif /* not ANSI_DECLARATORS */

{
  ...
  {
    /* Find the next block. */
    pool->pathblock = (VOID_D**) *(pool->pathblock);
    /* Find the first item in the block.  Increment by the size of (VOID_D *). */
    alignptr = (UNSIGNED_LONG) (pool->pathblock + 1);
    /* Align with item on an `alignbytes'-byte boundary. */
    pool->pathitem = (VOID_D*) (alignptr + (UNSIGNED_LONG) pool->alignbytes
  ...
  pool->pathitem = (VOID_D*) ((char*) pool->pathitem + pool->itembytes);
  pool->pathitemsleft--;
  return newitem;
}
...

{
  ...
  m->dummytribase = (triangle*) trimalloc(
      trianglebytes + m->triangles.alignbytes);
  /* Align `dummytri' on a `triangles.alignbytes'-byte boundary. */
  alignptr = (UNSIGNED_LONG) m->dummytribase;
  m->dummytri = (triangle*) (alignptr + (UNSIGNED_LONG) m->triangles.alignbytes
  ...
  if (b->usesegments)
  {
    ...
    m->dummysub = (subseg*) (alignptr + (UNSIGNED_LONG) m->subsegs.alignbytes
...

{
  ...
  m->vertexmarkindex = ((m->mesh_dim + m->nextras) * sizeof(DREAL) + sizeof(int)
      - 1)
                       / sizeof(int);
...

{
  ...
  /*   where the index is measured in DREALs.                           */
  m->elemattribindex = (trisize + sizeof(DREAL) - 1) / sizeof(DREAL);
  /* The index within each triangle at which the maximum area constraint  */
  /*   is found, where the index is measured in DREALs.  Note that if the  */
  ...
  if (b->vararea)
  {
    trisize = (m->areaboundindex + 1) * sizeof(DREAL);
  }
  else if (m->eextras + b->regionattrib > 0)
  {
    trisize = m->areaboundindex * sizeof(DREAL);
  }
  /* If a Voronoi diagram or triangle neighbor graph is requested, make    */
  /*   sure there's room to store an integer index in each triangle.  This */
  /*   integer index can occupy the same space as the subsegment pointers  */
  /*   or attributes or area constraint or extra nodes.                    */
  if ((b->voronoi || b->neighbors) && ((int) trisize
      < 6 * (int) sizeof(triangle) + (int) sizeof(int)))
  {
...

{
  ...
  pooldealloc(&m->triangles, (VOID_D*) dyingtriangle);
}
...

{
  ...
  pooldealloc(&m->subsegs, (VOID_D*) dyingsubseg);
}
...

{
  ...
  pooldealloc(&m->vertices, (VOID_D*) dyingvertex);
}
...

{
  ...
  pooldealloc(&m->badsubsegs, (VOID_D*) dyingseg);
}
...

{
  ...
  trifree((VOID_D*) m->dummytribase);
  if (b->usesegments)
  {
    pooldeinit(&m->subsegs);
    trifree((VOID_D*) m->dummysubbase);
  }
...
#ifdef ANSI_DECLARATORS
static int scale_expansion_zeroelim(int elen, DREAL *e, DREAL b, DREAL *h)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  INEXACT DREAL c;
  INEXACT DREAL abig;
  DREAL ahi, alo, bhi, blo;
  DREAL err1, err2, err3;
...
#ifdef ANSI_DECLARATORS
DREAL estimate(int elen, DREAL *e)
#else /* not ANSI_DECLARATORS */
DREAL estimate(elen, e)
int elen;
DREAL *e;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
DREAL counterclockwiseadapt(vertex pa, vertex pb, vertex pc, DREAL detsum)
#else /* not ANSI_DECLARATORS */
DREAL counterclockwiseadapt(pa, pb, pc, detsum)
vertex pa;
vertex pb;
vertex pc;
DREAL detsum;
...
{
  INEXACT DREAL acx, acy, bcx, bcy;
  DREAL acxtail, acytail, bcxtail, bcytail;
  INEXACT DREAL detleft, detright;
  DREAL detlefttail, detrighttail;
  DREAL det, errbound;
  DREAL B[4], C1[8], C2[12], D[16];
  INEXACT DREAL B3;
...
#ifdef ANSI_DECLARATORS
DREAL counterclockwise(struct mesh *m,
                       struct behavior *b,
                       vertex pa,
                       vertex pb,
                       vertex pc)
#else /* not ANSI_DECLARATORS */
DREAL counterclockwise(m, b, pa, pb, pc)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
{
  DREAL detleft, detright, det;
  DREAL detsum, errbound;
...
#ifdef ANSI_DECLARATORS
static DREAL incircleadapt(vertex pa,
                           vertex pb,
                           vertex pc,
                           vertex pd,
                           DREAL permanent)
#else /* not ANSI_DECLARATORS */
static DREAL incircleadapt(pa, pb, pc, pd, permanent)
vertex pa;
vertex pb;
vertex pc;
vertex pd;
...
{
  INEXACT DREAL adx, bdx, cdx, ady, bdy, cdy;
  DREAL det, errbound;

  INEXACT DREAL bdxcdy1, cdxbdy1, cdxady1, adxcdy1, adxbdy1, bdxady1;
  DREAL bdxcdy0, cdxbdy0, cdxady0, adxcdy0, adxbdy0, bdxady0;
  DREAL bc[4], ca[4], ab[4];
  INEXACT DREAL bc3, ca3, ab3;
  DREAL axbc[8], axxbc[16], aybc[8], ayybc[16], adet[32];
  int axbclen, axxbclen, aybclen, ayybclen, alen;
  DREAL bxca[8], bxxca[16], byca[8], byyca[16], bdet[32];
  int bxcalen, bxxcalen, bycalen, byycalen, blen;
  DREAL cxab[8], cxxab[16], cyab[8], cyyab[16], cdet[32];
  int cxablen, cxxablen, cyablen, cyyablen, clen;
  DREAL abdet[64];
  int ablen;
  DREAL fin1[1152], fin2[1152];
  DREAL *finnow, *finother, *finswap;
  int finlength;

  DREAL adxtail, bdxtail, cdxtail, adytail, bdytail, cdytail;
  INEXACT DREAL adxadx1, adyady1, bdxbdx1, bdybdy1, cdxcdx1, cdycdy1;
  DREAL adxadx0, adyady0, bdxbdx0, bdybdy0, cdxcdx0, cdycdy0;
  DREAL aa[4], bb[4], cc[4];
  INEXACT DREAL aa3, bb3, cc3;
  INEXACT DREAL ti1, tj1;
  DREAL ti0, tj0;
  DREAL u[4], v[4];
  INEXACT DREAL u3, v3;
  DREAL temp8[8], temp16a[16], temp16b[16], temp16c[16];
  DREAL temp32a[32], temp32b[32], temp48[48], temp64[64];
  int temp8len, temp16alen, temp16blen, temp16clen;
  int temp32alen, temp32blen, temp48len, temp64len;
  DREAL axtbb[8], axtcc[8], aytbb[8], aytcc[8];
  int axtbblen, axtcclen, aytbblen, aytcclen;
  DREAL bxtaa[8], bxtcc[8], bytaa[8], bytcc[8];
  int bxtaalen, bxtcclen, bytaalen, bytcclen;
  DREAL cxtaa[8], cxtbb[8], cytaa[8], cytbb[8];
  int cxtaalen, cxtbblen, cytaalen, cytbblen;
  DREAL axtbc[8], aytbc[8], bxtca[8], bytca[8], cxtab[8], cytab[8];
  int axtbclen, aytbclen, bxtcalen, bytcalen, cxtablen, cytablen;
  DREAL axtbct[16], aytbct[16], bxtcat[16], bytcat[16], cxtabt[16], cytabt[16];
  int axtbctlen, aytbctlen, bxtcatlen, bytcatlen, cxtabtlen, cytabtlen;
  DREAL axtbctt[8], aytbctt[8], bxtcatt[8];
  DREAL bytcatt[8], cxtabtt[8], cytabtt[8];
  int axtbcttlen, aytbcttlen, bxtcattlen, bytcattlen, cxtabttlen, cytabttlen;
  DREAL abt[8], bct[8], cat[8];
  int abtlen, bctlen, catlen;
  DREAL abtt[4], bctt[4], catt[4];
  int abttlen, bcttlen, cattlen;
  INEXACT DREAL abtt3, bctt3, catt3;
  DREAL negate;
  ...
  axtbctlen = aytbctlen = bxtcattlen = bytcattlen = cxtabttlen = cytabttlen = 0;
  cxtablen = cytablen = bxtcalen = bytcalen = axtbclen = aytbclen = 0;
  adx = (DREAL) (pa[0] - pd[0]);
  bdx = (DREAL) (pb[0] - pd[0]);
  cdx = (DREAL) (pc[0] - pd[0]);
  ady = (DREAL) (pa[1] - pd[1]);
...
DREAL incircle(struct mesh *m,
               ...
               vertex pd)
#else /* not ANSI_DECLARATORS */
DREAL incircle(m, b, pa, pb, pc, pd)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
{
  DREAL adx, bdx, cdx, ady, bdy, cdy;
  DREAL bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
  DREAL alift, blift, clift;
  DREAL det;
  DREAL permanent, errbound;
...
#ifdef ANSI_DECLARATORS
DREAL orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd,
DREAL aheight,
                    DREAL bheight, DREAL cheight, DREAL dheight,
                    DREAL permanent)
#else /* not ANSI_DECLARATORS */
DREAL orient3dadapt(pa, pb, pc, pd,
...
{
  INEXACT DREAL adx, bdx, cdx, ady, bdy, cdy, adheight, bdheight, cdheight;
  DREAL det, errbound;
  ...
  DREAL bc[4], ca[4], ab[4];
  INEXACT DREAL bc3, ca3, ab3;
  DREAL adet[8], bdet[8], cdet[8];
  int alen, blen, clen;
  DREAL abdet[16];
  int ablen;
  DREAL *finnow, *finother, *finswap;
  DREAL fin1[192], fin2[192];
  int finlength;

  DREAL adxtail, bdxtail, cdxtail;
  DREAL adytail, bdytail, cdytail;
  DREAL adheighttail, bdheighttail, cdheighttail;
  INEXACT DREAL at_blarge, at_clarge;
  INEXACT DREAL bt_clarge, bt_alarge;
  INEXACT DREAL ct_alarge, ct_blarge;
  DREAL at_b[4], at_c[4], bt_c[4], bt_a[4], ct_a[4], ct_b[4];
  int at_blen, at_clen, bt_clen, bt_alen, ct_alen, ct_blen;
  INEXACT DREAL bdxt_cdy1, cdxt_bdy1, cdxt_ady1;
  INEXACT DREAL adxt_cdy1, adxt_bdy1, bdxt_ady1;
  DREAL bdxt_cdy0, cdxt_bdy0, cdxt_ady0;
  DREAL adxt_cdy0, adxt_bdy0, bdxt_ady0;
  INEXACT DREAL bdyt_cdx1, cdyt_bdx1, cdyt_adx1;
  INEXACT DREAL adyt_cdx1, adyt_bdx1, bdyt_adx1;
  DREAL bdyt_cdx0, cdyt_bdx0, cdyt_adx0;
  DREAL adyt_cdx0, adyt_bdx0, bdyt_adx0;
  DREAL bct[8], cat[8], abt[8];
  int bctlen, catlen, abtlen;
  INEXACT DREAL bdxt_cdyt1, cdxt_bdyt1, cdxt_adyt1;
  INEXACT DREAL adxt_cdyt1, adxt_bdyt1, bdxt_adyt1;
  DREAL bdxt_cdyt0, cdxt_bdyt0, cdxt_adyt0;
  DREAL adxt_cdyt0, adxt_bdyt0, bdxt_adyt0;
  DREAL u[4], v[12], w[16];
  INEXACT DREAL u3;
  int vlength, wlength;
  DREAL negate;
  ...
  bdy = (DREAL) (pb[1] - pd[1]);
  cdy = (DREAL) (pc[1] - pd[1]);
  adheight = (DREAL) (aheight - dheight);
  bdheight = (DREAL) (bheight - dheight);
  cdheight = (DREAL) (cheight - dheight);
...
DREAL orient3d(struct mesh *m,
               ...
               DREAL dheight)
#else /* not ANSI_DECLARATORS */
DREAL orient3d(m, b, pa, pb, pc, pd, aheight, bheight, cheight, dheight)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
{
  DREAL adx, bdx, cdx, ady, bdy, cdy, adheight, bdheight, cdheight;
  DREAL bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
  DREAL det;
  DREAL permanent, errbound;
...
#ifdef ANSI_DECLARATORS
DREAL nonregular(struct mesh *m,
                 struct behavior *b,
                 vertex pa,
                 vertex pb,
                 vertex pc,
                 vertex pd)
#else /* not ANSI_DECLARATORS */
DREAL nonregular(m, b, pa, pb, pc, pd)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
{
  DREAL xdo, ydo, xao, yao;
  DREAL dodist, aodist, dadist;
  DREAL denominator;
  DREAL dx, dy, dxoff, dyoff;
  ...
  else
  {
    if (offcenter && (b->offconstant > 0.0))
    {
      ...
      /*   circumcenter, use the off-center instead.             */
      if (dxoff * dxoff + dyoff * dyoff < (dx - xdo) * (dx - xdo)
          + (dy - ydo) * (dy - ydo))
      {
        dx = xdo + dxoff;
        dy = ydo + dyoff;
...

{
  ...
  if (!b->quiet)
  {
    message("  Checking consistency of mesh...\n");
  }
  horrors = 0;
  ...
  while (triangleloop.tri != (triangle*) NULL)
  {
        ...
        triangleloop.orient++)
    {
      ...
      if (oppotri.tri != m->dummytri)
      {
        ...
        if ((triangleloop.tri != oppooppotri.tri) || (triangleloop.orient
            != oppooppotri.orient))
        {
          message("  !! !! Asymmetric triangle-triangle bond:\n");
          ...
          message("    First ");
          printtriangle(m, b, &triangleloop);
          message("    Second (nonreciprocating) ");
        ...
        if ((triorg != oppodest) || (tridest != oppoorg))
        {
          ...
          message("    First mismatched ");
          printtriangle(m, b, &triangleloop);
          message("    Second mismatched ");
          printtriangle(m, b, &oppotri);
          horrors++;
        }
      }
  ...
  if (horrors == 0)
  {
    if (!b->quiet)
    {
      message("  In my studied opinion, the mesh appears to be consistent.\n");
    }
  }
  else if (horrors == 1)
  {
    message("  !! !! !! !! Precisely one festering wound discovered.\n");
  }
  else
  {
    message("  !! !! !! !! %d abominations witnessed.\n", horrors);
  }
...

{
  ...
  if (!b->quiet)
  {
    message("  Checking Delaunay property of mesh...\n");
  }
  horrors = 0;
  /* Run through the list of triangles, checking each one. */
  traversalinit(&m->triangles);
  ...
  while (triangleloop.tri != (triangle*) NULL)
  {
        ...
        triangleloop.orient++)
    {
      ...
      shouldbedelaunay = (oppotri.tri != m->dummytri) && !deadtri(oppotri.tri)
      ...
      if (shouldbedelaunay)
      {
        if (nonregular(m, b, triorg, tridest, triapex, oppoapex) > 0.0)
        {
          if (!b->weighted)
          {
            message("  !! !! Non-Delaunay pair of triangles:\n");
            message("    First non-Delaunay ");
            printtriangle(m, b, &triangleloop);
            message("    Second non-Delaunay ");
          }
          else
          {
            message("  !! !! Non-regular pair of triangles:\n");
            message("    First non-regular ");
            printtriangle(m, b, &triangleloop);
            message("    Second non-regular ");
          }
          printtriangle(m, b, &oppotri);
          horrors++;
        }
  ...
  {
    message("  !! !! !! !! %d obscenities viewed with horror.\n", horrors);
  }
  /* Restore the status of exact arithmetic. */
  b->noexact = saveexact;
}
...

{
  ...
  if (b->verbose > 2)
  {
    message("  Queueing bad triangle:\n");
    message("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
...

{
  ...
  if (neighbortri.tri != m->dummytri)
  {
    ...
    if (dotproduct < 0.0)
    {
      if (b->conformdel || (dotproduct * dotproduct
          >= (2.0 * b->goodangle - 1.0) * (2.0 * b->goodangle - 1.0)
             * ((eorg[0] - eapex[0]) * (eorg[0] - eapex[0]) + (eorg[1]
                 - eapex[1])
                                                              * (eorg[1] - eapex[1]))
             * ((edest[0] - eapex[0]) * (edest[0] - eapex[0]) + (edest[1]
                 - eapex[1])
                                                                * (edest[1] - eapex[1]))))
      {
        encroached = 1;
...

{
  ...
  vertex joinvertex;
  DREAL dxod, dyod, dxda, dyda, dxao, dyao;
  DREAL dxod2, dyod2, dxda2, dyda2, dxao2, dyao2;
  DREAL apexlen, orglen, destlen, minedge;
  DREAL angle;
  DREAL area;
  DREAL dist1, dist2;
  subseg sptr; /* Temporary variable used by tspivot(). */
...

{
  ...
  if (b->verbose)
  {
    message("    Constructing mapping from vertices to triangles.\n");
  }
  traversalinit(&m->triangles);
  triangleloop.tri = triangletraverse(m);
  while (triangleloop.tri != (triangle*) NULL)
...

{
  ...
  if (b->verbose > 2)
  {
    message("  Searching for point (%.12g, %.12g).\n", searchpoint[0],
            searchpoint[1]);
  }
  /* Where are we? */
  org(*searchtri, forg);
  ...
  while (1)
  {
    if (b->verbose > 2)
    {
      message("    At (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", forg[0],
...

{
  ...
  if (b->verbose > 2)
  {
    message("  Randomly sampling for a triangle near point (%.12g, %.12g).\n",
  ...
  if (b->verbose > 2)
  {
    message("    Boundary triangle has origin (%.12g, %.12g).\n", torg[0],
            torg[1]);
  }
  ...
  if (m->recenttri.tri != (triangle*) NULL)
  {
    if (!deadtri(m->recenttri.tri))
    {
      ...
      if (dist < searchdist)
      {
        ...
        if (b->verbose > 2)
        {
          message("    Choosing recent triangle with origin (%.12g, %.12g).\n",
                  torg[0], torg[1]);
        }
      }
    }
  ...
  while (totalsamplesleft > 0)
  {
    ...
    firsttri = (char*) (alignptr + (UNSIGNED_LONG) m->triangles.alignbytes
        - (alignptr % (UNSIGNED_LONG) m->triangles.alignbytes));
    ...
    do
    {
      ...
      if (!deadtri(sampletri.tri))
      {
        ...
        if (dist < searchdist)
        {
          ...
          if (b->verbose > 2)
          {
            message("    Choosing triangle with origin (%.12g, %.12g).\n",
                    torg[0], torg[1]);
          }
        }
      }
    ...
    {
      sampleblock = (VOID_D**) *sampleblock;
  ...
  else if (ahead == 0.0)
  {
    ...
    if (((torg[0] < searchpoint[0]) == (searchpoint[0] < tdest[0])) && ((torg[1]
        < searchpoint[1])
...

{
  ...
  if (top.tri == m->dummytri) {
    message("Internal error in flip():  Attempt to flip on boundary.\n");
    lnextself(*flipedge);
    return;
  }
  if (m->checksegments) {
    ...
    if (toplsubseg.ss != m->dummysub) {
      message("Internal error in flip():  Attempt to flip a segment.\n");
      lnextself(*flipedge);
      return;
    }
  }
  ...
  if (b->verbose > 2)
  {
    message("  Edge flip results in left ");
...

{
...
#ifdef SELF_CHECK
  if (top.tri == m->dummytri) {
    message("Internal error in unflip():  Attempt to flip on boundary.\n");
    lnextself(*flipedge);
    return;
  }
  if (m->checksegments) {
    tspivot(*flipedge, toplsubseg);
    if (toplsubseg.ss != m->dummysub) {
      message("Internal error in unflip():  Attempt to flip a subsegment.\n");
      lnextself(*flipedge);
      return;
    }
  }
  ...
  if (b->verbose > 2)
  {
    message("  Edge unflip results in left ");
    printtriangle(m, b, flipedge);
    message("  and right ");
    printtriangle(m, b, &top);
  }
...

{
  ...
  if ((intersect == ONEDGE) || (intersect == OUTSIDE))
  {
    ...
    if (counterclockwise(m, b, rightvertex, leftvertex, botvertex) < 0.0) {
      messerr("Internal error in insertvertex():");
      messerr(
    ...
    if (mirrorflag) {
      if (counterclockwise(m, b, leftvertex, rightvertex, topvertex) < 0.0) {
        messerr("Internal error in insertvertex():");
        messerr("  Clockwise triangle prior to edge vertex insertion (top).");
      }
      ...
      if (counterclockwise(m, b, topvertex, leftvertex, newvertex) < 0.0) {
        mess("Internal error in insertvertex():");
        mess(
    ...
    if (b->verbose > 2)
    {
      message("  Updating bottom left ");
      printtriangle(m, b, &botright);
      if (mirrorflag)
      {
        message("  Updating top left ");
        printtriangle(m, b, &topright);
        message("  Creating top right ");
        printtriangle(m, b, &newtopright);
      }
      message("  Creating bottom right ");
  ...
  else
  {
    ...
    if (counterclockwise(m, b, rightvertex, leftvertex, botvertex) < 0.0) {
      messerr("Internal error in insertvertex():");
      messerr("  Clockwise triangle prior to vertex insertion.");
    }
    if (counterclockwise(m, b, rightvertex, leftvertex, newvertex) < 0.0) {
      messerr("Internal error in insertvertex():");
      messerr("  Clockwise triangle after vertex insertion (top).");
    }
    if (counterclockwise(m, b, leftvertex, botvertex, newvertex) < 0.0) {
      messerr("Internal error in insertvertex():");
      messerr("  Clockwise triangle after vertex insertion (left).");
    }
    if (counterclockwise(m, b, botvertex, rightvertex, newvertex) < 0.0) {
      messerr("Internal error in insertvertex():");
      messerr("  Clockwise triangle after vertex insertion (right).");
    }
#endif /* SELF_CHECK */
    if (b->verbose > 2)
    {
      ...
      message("  Creating left ");
      printtriangle(m, b, &newbotleft);
      message("  Creating right ");
      printtriangle(m, b, &newbotright);
    }
  }
      ...
      else
      {
            ...
            || (leftvertex == m->infvertex3))
        {
          ...
          doflip = counterclockwise(m, b, newvertex, rightvertex, farvertex)
            ...
            || (rightvertex == m->infvertex2) || (rightvertex == m->infvertex3))
        {
        ...
        else
        {
          ...
          doflip = incircle(m, b, leftvertex, newvertex, rightvertex, farvertex)
              > 0.0;
        }
        if (doflip)
        {
          ...
          if (newvertex != (vertex) NULL) {
            if (counterclockwise(m, b, leftvertex, newvertex, rightvertex) <
                0.0) {
              messerr("Internal error in insertvertex():");
              messerr("  Clockwise triangle prior to edge flip (bottom).");
            }
...
/*
            if (counterclockwise(m, b, rightvertex, farvertex, leftvertex) <
                0.0) {
              messerr("Internal error in insertvertex():");
              messerr("  Clockwise triangle prior to edge flip (top).");
            }
*/
            if (counterclockwise(m, b, farvertex, leftvertex, newvertex) <
                0.0) {
              messerr("Internal error in insertvertex():");
              messerr("  Clockwise triangle after edge flip (left).");
            }
            if (counterclockwise(m, b, newvertex, rightvertex, farvertex) <
                0.0) {
              messerr("Internal error in insertvertex():");
              messerr("  Clockwise triangle after edge flip (right).");
            }
          }
#endif /* SELF_CHECK */
          if (b->verbose > 2)
...

{
  ...
  if (b->verbose > 2)
  {
    message("  Triangulating interior polygon at edge\n");
    message("    (%.12g, %.12g) (%.12g, %.12g)\n", leftbasevertex[0],
  ...
  if (b->verbose > 2)
  {
    message("    Connecting edge to (%.12g, %.12g)\n", bestvertex[0],
            bestvertex[1]);
  }
  if (bestnumber > 1)
  {
...

{
  ...
  while (!otriequal(*deltri, countingtri))
  {
    ...
    if (countingtri.tri == m->dummytri) {
      messerr("Internal error in deletevertex():");
      messerr("  Attempt to delete boundary vertex.");
      internalerror();
    }
#endif /* SELF_CHECK */
    edgecount++;
...

{
  ...
  {
    /* Recursive base case. */
    if ((sortarray[0][0] > sortarray[1][0]) || ((sortarray[0][0]
        == sortarray[1][0])
  ...
  while (left < right)
  {
    ...
    while ((left <= right)
        && ((sortarray[left][0] < pivotx) || ((sortarray[left][0] == pivotx)
    ...
    while ((left <= right)
        && ((sortarray[right][0] > pivotx) || ((sortarray[right][0] == pivotx)
...

{
  ...
  {
    /* Recursive base case. */
    if ((sortarray[0][axis] > sortarray[1][axis]) || ((sortarray[0][axis]
        == sortarray[1][axis])
                                                      && (sortarray[0][1 - axis] > sortarray[1][1
                                                          - axis])))
    {
      temp = sortarray[1];
      sortarray[1] = sortarray[0];
  ...
  while (left < right)
  {
    ...
    while ((left <= right)
        && ((sortarray[left][axis] < pivot1) || ((sortarray[left][axis]
            == pivot1)
    ...
    while ((left <= right)
        && ((sortarray[right][axis] > pivot1) || ((sortarray[right][axis]
            == pivot1)
...

{
  ...
  if (b->verbose > 2)
  {
    message("  Creating base bounding ");
...

{
  ...
  if (b->verbose > 2)
  {
    message("  Triangulating %d vertices.\n", vertices);
  }
  if (vertices == 2)
  {
  ...
  else
  {
    ...
    if (b->verbose > 1)
    {
      message("  Joining triangulations with %d and %d vertices.\n", divider,
              vertices - divider);
    }
...

{
  ...
  if (b->verbose)
  {
    message("  Removing ghost triangles.\n");
  }
...

{
  ...
  {
    if ((sortarray[i][0] == sortarray[j][0]) && (sortarray[i][1]
        == sortarray[j][1]))
    {
      if (!b->quiet)
      {
...

{
  ...
  if (b->verbose)
  {
    message("  Creating triangular bounding box.\n");
  }
...

{
  ...
  if (b->verbose)
  {
    message("  Removing triangular bounding box.\n");
  }
  /* Find a boundary triangle. */
  ...
  trifree((VOID_D*) m->infvertex1); /* Deallocate the bounding box vertices. */
  trifree((VOID_D*) m->infvertex2);
  trifree((VOID_D*) m->infvertex3);
...

{
  ...
  if (b->verbose)
  {
    message("  Incrementally inserting vertices.\n");
  }
  traversalinit(&m->vertices);
...

{
  ...
  while (notdone)
  {
    if ((heap[leftchild]->ykey < eventy) || ((heap[leftchild]->ykey == eventy)
    ...
    {
      if ((heap[rightchild]->ykey < heap[smallest]->ykey) || ((heap[rightchild]->ykey
          == heap[smallest]->ykey)
...

{
  ...
  *eventheap = (struct event**) trimalloc(
      maxevents * (int) sizeof(struct event*));
  *events = (struct event*) trimalloc(maxevents * (int) sizeof(struct event));
  traversalinit(&m->vertices);
  for (i = 0; i < m->invertices; i++)
  {
    thisvertex = vertextraverse(m);
    (*events)[i].eventptr = (VOID_D*) thisvertex;
  ...
  *freeevents = (struct event*) NULL;
  ...
  {
    (*events)[i].eventptr = (VOID_D*) *freeevents;
    *freeevents = *events + i;
  }
}
...
#ifdef ANSI_DECLARATORS
DREAL circletop(struct mesh *m, vertex pa, vertex pb, vertex pc, DREAL ccwabc)
#else /* not ANSI_DECLARATORS */
DREAL circletop(m, pa, pb, pc, ccwabc)
struct mesh *m;
vertex pa;
vertex pb;
vertex pc;
...
{
  DREAL xac, yac, xbc, ybc, xab, yab;
  DREAL aclen2, bclen2, ablen2;
  ...
  return pc[1]
      + (xac * bclen2 - xbc * aclen2 + sqrt(aclen2 * bclen2 * ablen2)) / (2.0
          * ccwabc);
}
...

{
  ...
  if (eventvertex != (vertex) NULL)
  {
    ...
    deadevent->eventptr = (VOID_D*) *freeevents;
...

{
  ...
  else
  {
    ...
    pooldealloc(&m->splaynodes, (VOID_D*) splaytree);
...

{
  ...
  if (b->verbose)
  {
    message("  Placing vertices in event heap.\n");
  }
  ...
  eventheap[0]->eventptr = (VOID_D*) freeevents;
  ...
  do
  {
    if (heapsize == 0)
    {
      messerr("Error:  Input vertices are all identical.");
      triexit(1);
    }
    ...
    eventheapdelete(eventheap, heapsize, 0);
    heapsize--;
    if ((firstvertex[0] == secondvertex[0]) && (firstvertex[1]
        == secondvertex[1]))
    {
      if (!b->quiet)
      {
  ...
  while (heapsize > 0)
  {
    ...
    nextevent->eventptr = (VOID_D*) freeevents;
    ...
    if (check4events)
    {
      ...
      if (lefttest > 0.0)
      {
        ...
        newevent->ykey = circletop(m, leftvertex, midvertex, rightvertex,
                                   lefttest);
        newevent->eventptr = (VOID_D*) encode(lefttri);
      ...
      if (righttest > 0.0)
      {
        ...
        newevent->ykey = circletop(m, leftvertex, midvertex, rightvertex,
                                   righttest);
        newevent->eventptr = (VOID_D*) encode(farrighttri);
        eventheapinsert(eventheap, heapsize, newevent);
        heapsize++;
...

{
  ...
  if (!b->quiet)
  {
    ...
    if (b->incremental)
    {
      message("by incremental method.\n");
    }
    else if (b->sweepline)
    {
      message("by sweepline method.\n");
    }
    else
    {
      message("by divide-and-conquer method.\n");
    }
  }
  if (b->incremental)
  {
...
int reconstruct(struct mesh *m,
                struct behavior *b,
                int *trianglelist,
                DREAL *triangleattriblist,
                DREAL *trianglearealist,
                int elements,
                int corners,
                int attribs,
                int *segmentlist,
...

{
  ...
  if (incorners < 3)
  {
    messerr("Error:  Triangles must have at least 3 vertices.");
    triexit(1);
  }
  m->eextras = attribs;
#else /* not TRILIBRARY */
  ...
  if (*stringptr == '\0') {
    ...
    if (incorners < 3) {
      messerr("Error:  Triangles in %s must have at least 3 vertices.",
             elefilename);
      triexit(1);
    }
  }
  ...
  if (b->vararea) {
    ...
    if (areaelements != m->inelements) {
      messerr("Error:  %s and %s disagree on number of triangles.",
  ...
  while (triangleloop.tri != (triangle*) NULL)
  {
    ...
    for (j = 0; j < 3; j++)
    {
      ...
      if ((corner[j] < b->firstnumber) || (corner[j]
          >= b->firstnumber + m->invertices))
      {
        messerr("Error:  Triangle %ld has an invalid vertex index.",
                elementnumber);
        triexit(1);
      }
    }
    ...
    for (j = 0; j < 3; j++) {
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        messerr("Error:  Triangle %ld is missing vertex %d in %s.",
...
#endif /* not TRILIBRARY */
      if ((killvertexindex >= b->firstnumber) && (killvertexindex
          < b->firstnumber + m->invertices))
      {
  ...
  /* Read the triangle's attributes. */
    for (j = 0; j < m->eextras; j++)
    {
      ...
      if (*stringptr == '\0') {
        ...
        setelemattribute(triangleloop, j,
                         (DREAL) strtod(stringptr, &stringptr));
      }
#endif /* not TRILIBRARY */
    }
    ...
    if (b->vararea)
    {
      ...
      if (*stringptr == '\0') {
        ...
        area = (DREAL) strtod(stringptr, &stringptr);
      }
#endif /* not TRILIBRARY */
  ...
  if (b->poly)
  {
    if (b->verbose)
    {
      message("  Marking segments in triangulation.\n");
    }
    ...
    while (subsegloop.ss != (subseg*) NULL)
    {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        messerr("Error:  Segment %ld has no endpoints in %s.", segmentnumber,
               polyfilename);
        triexit(1);
      } else {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        messerr("Error:  Segment %ld is missing its second endpoint in %s.",
      ...
      for (j = 0; j < 2; j++)
      {
        if ((end[j] < b->firstnumber) || (end[j]
            >= b->firstnumber + m->invertices))
        {
          messerr("Error:  Segment %ld has an invalid vertex index.",
                  segmentnumber);
          triexit(1);
        }
      }
...

{
  ...
  while (leftflag)
  {
    ...
    if (searchtri->tri == m->dummytri)
    {
      messerr("Internal error in finddirection():  Unable to find a");
      messerr("  triangle leading from (%.12g, %.12g) to", startvertex[0],
              startvertex[1]);
      messerr("  (%.12g, %.12g).", searchpoint[0], searchpoint[1]);
      internalerror();
    }
    apex(*searchtri, leftvertex);
    rightccw = leftccw;
...

{
  ...
  if (denom == 0.0)
  {
    messerr("Internal error in segmentintersection():");
    messerr("  Attempt to find intersection of parallel segments.");
    internalerror();
  }
  ...
  {
    messerr("Internal error in segmentintersection():");
    messerr("  Failure to split a segment.");
    internalerror();
  }
  ...
  {
    messerr("Internal error in segmentintersection():");
    messerr("  Topological inconsistency after splitting a segment.");
    internalerror();
  }
...

{
  ...
  dest(*searchtri, rightvertex);
  apex(*searchtri, leftvertex);
  if (((leftvertex[0] == endpoint2[0]) && (leftvertex[1] == endpoint2[1])) || ((rightvertex[0]
      == endpoint2[0])
...

{
  ...
  if (b->verbose > 2)
  {
    message("Forcing segment into triangulation by recursive splitting:\n");
  ...
  if (success == DUPLICATEVERTEX)
  {
    if (b->verbose > 2)
    {
      message("  Segment intersects existing vertex (%.12g, %.12g).\n",
              newvertex[0], newvertex[1]);
    }
  ...
  else
  {
    if (success == VIOLATINGVERTEX)
    {
      if (b->verbose > 2)
      {
        message("  Two segments intersect at (%.12g, %.12g).\n", newvertex[0],
                newvertex[1]);
      }
      ...
      if (success != SUCCESSFULVERTEX)
      {
        messerr("Internal error in conformingedge():");
        messerr("  Failure to split a segment.");
        internalerror();
      }
    }
...

{
  ...
  if (b->verbose > 1)
  {
    message("  Connecting (%.12g, %.12g) to (%.12g, %.12g).\n", endpoint1[0],
  ...
  if (checkvertex != endpoint1)
  {
    ...
    if (locate(m, b, endpoint1, &searchtri1) != ONVERTEX)
    {
      ...
      messerr("  (%.12g, %.12g) in triangulation.", endpoint1[0], endpoint1[1]);
      internalerror();
    }
  }
  ...
  if (checkvertex != endpoint2)
  {
    ...
    if (locate(m, b, endpoint2, &searchtri2) != ONVERTEX)
    {
      messerr(
          "Internal error in insertsegment():  Unable to locate PSLG vertex");
      messerr("  (%.12g, %.12g) in triangulation.", endpoint2[0], endpoint2[1]);
      internalerror();
    }
  }
  /* Remember this triangle to improve subsequent point location. */
...

{
  ...
  if (b->poly)
  {
    ...
    if (m->insegments > 0)
    {
      ...
      if (b->verbose)
      {
        message("  Recovering PSLG segments.\n");
      }
    }
    ...
    for (i = 0; i < m->insegments; i++)
    {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        messerr("Error:  Segment %d has no endpoints in %s.",
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        messerr("Error:  Segment %d is missing its second endpoint in %s.",
               b->firstnumber + i, polyfilename);
        triexit(1);
      } else {
      ...
      if ((end1 < b->firstnumber) || (end1 >= b->firstnumber + m->invertices))
      {
        if (!b->quiet)
        {
          message("Warning:  Invalid first endpoint of segment %d in %s.\n",
                  b->firstnumber + i, polyfilename);
        }
      }
          ...
          || (end2 >= b->firstnumber + m->invertices))
      {
        if (!b->quiet)
        {
          message("Warning:  Invalid second endpoint of segment %d in %s.\n",
                  b->firstnumber + i, polyfilename);
        }
      }
      else
      {
        ...
        if ((endpoint1[0] == endpoint2[0]) && (endpoint1[1] == endpoint2[1]))
        {
          if (!b->quiet)
          {
            message("Warning:  Endpoints of segment %d are coincident in %s.\n",
                    b->firstnumber + i, polyfilename);
          }
        }
        else
  ...
  if (b->convex || !b->poly)
  {
    ...
    if (b->verbose)
    {
      message("  Enclosing convex hull with segments.\n");
    }
    markhull(m, b);
  }
}
...

{
  ...
  if (b->verbose)
  {
    message("  Marking concavities (external triangles) for elimination.\n");
  }
  /* Find a triangle handle on the hull. */
  hulltri.tri = m->dummytri;
  hulltri.orient = 0;
...

{
  ...
  if (b->verbose)
  {
    message("  Marking neighbors of marked triangles.\n");
  }
  ...
  while (virusloop != (triangle**) NULL)
  {
    ...
    if (b->verbose > 2)
    {
      ...
      message("    Checking (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
  ...
  if (b->verbose)
  {
    message("  Deleting marked triangles.\n");
  }
    ...
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
    {
      ...
      if (testvertex != (vertex) NULL)
      {
        ...
        if (killorg)
        {
          if (b->verbose > 1)
          {
            message("    Deleting vertex (%.12g, %.12g)\n", testvertex[0],
                    testvertex[1]);
          }
...

{
    ...
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
    {
          ...
          && (neighborsubseg.ss == m->dummysub))
      {
        if (b->verbose > 2)
        {
          ...
          message("    Marking (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
                  regionorg[0], regionorg[1], regiondest[0], regiondest[1],
                  regionapex[0], regionapex[1]);
        }
        /* Infect the neighbor. */
  ...
  if (b->verbose > 1)
  {
    message("  Unmarking marked triangles.\n");
  }
  traversalinit(&m->viri);
  virusloop = (triangle**) traverse(&m->viri);
  while (virusloop != (triangle**) NULL)
...
#ifdef ANSI_DECLARATORS
void carveholes(struct mesh *m, struct behavior *b, DREAL *holelist, int holes,
DREAL *regionlist,
                int regions)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  {
    message("Removing unwanted triangles.\n");
    if (b->verbose && (holes > 0))
    {
      message("  Marking holes for elimination.\n");
    }
  }
  ...
  if (regions > 0)
  {
    if (!b->quiet)
    {
      if (b->regionattrib)
      {
        if (b->vararea)
        {
          message("Spreading regional attributes and area constraints.\n");
        }
        else
        {
          message("Spreading regional attributes.\n");
        }
      }
      else
      {
        message("Spreading regional area constraints.\n");
      }
    }
...
void precisionerror()
{
  message("Try increasing the area criterion and/or reducing the minimum\n");
  message("  allowable angle so that tiny triangles are not created.\n");
#ifdef SINGLE
  message("Alternatively, try recompiling me with double precision\n");
  message("  arithmetic (by removing \"#define SINGLE\" from the\n");
  message("  source file or \"-DSINGLE\" from the makefile).\n");
#endif /* SINGLE */
}
...

{
  ...
  vertex newvertex;
  enum insertvertexresult success;
  DREAL segmentlength, nearestpoweroftwo;
  DREAL split;
  DREAL multiplier, divisor;
  ...
  while ((m->badsubsegs.items > 0) && (m->steinerleft != 0))
  {
    ...
    while ((encloop != (struct badsubseg*) NULL) && (m->steinerleft != 0))
    {
          ...
          && (edest == encloop->subsegdest))
      {
        ...
        {
          segmentlength = sqrt(
              (edest[0] - eorg[0]) * (edest[0] - eorg[0]) + (edest[1] - eorg[1])
                  * (edest[1] - eorg[1]));
        ...
        if (((newvertex[0] == eorg[0]) && (newvertex[1] == eorg[1])) || ((newvertex[0]
            == edest[0])
                                                                         && (newvertex[1] == edest[1])))
        {
          messerr("Error:  Ran out of precision at (%.12g, %.12g).",
                  newvertex[0], newvertex[1]);
          messerr("I attempted to split a segment to a smaller size than");
          messerr("  can be accommodated by the finite precision of");
          messerr("  floating point arithmetic.");
          precisionerror();
          triexit(1);
        }
        ...
        {
          messerr("Internal error in splitencsegs():");
          messerr("  Failure to split a segment.");
          internalerror();
        }
        if (m->steinerleft > 0)
...

{
  ...
  if (b->verbose)
  {
    message("  Making a list of bad triangles.\n");
  }
  traversalinit(&m->triangles);
  triangleloop.orient = 0;
  triangleloop.tri = triangletraverse(m);
...

{
      ...
      && (bdest == badtri->triangdest) && (bapex == badtri->triangapex))
  {
    if (b->verbose > 1)
    {
      message("  Splitting this triangle at its circumcenter:\n");
      message("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", borg[0],
    ...
    if (((newvertex[0] == borg[0]) && (newvertex[1] == borg[1])) || ((newvertex[0]
        == bdest[0])
                                                                     && (newvertex[1] == bdest[1]))
    ...
    else
    {
      ...
      success = insertvertex(m, b, newvertex, &badotri, (struct osub*) NULL, 1,
                             1);
      if (success == SUCCESSFULVERTEX)
      {
        if (m->steinerleft > 0)
    ...
    if (errorflag)
    {
      if (b->verbose)
      {
        message("  The new vertex is at the circumcenter of triangle\n");
        message("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", borg[0],
                borg[1], bdest[0], bdest[1], bapex[0], bapex[1]);
      }
      message("This probably means that I am trying to refine triangles\n");
      message("  to a smaller size than can be accommodated by the finite\n");
      message("  precision of floating point arithmetic.  (You can be\n");
      message("  sure of this if I fail to terminate.)\n");
      precisionerror();
    }
  }
}
...

{
  ...
  if (!b->quiet)
  {
    message("Adding Steiner points to enforce quality.\n");
  }
  ...
  if (b->verbose)
  {
    message("  Looking for encroached subsegments.\n");
  }
  /* Test all segments to see if they're encroached. */
  tallyencs(m, b);
  if (b->verbose && (m->badsubsegs.items > 0))
  {
    message("  Splitting encroached subsegments.\n");
  }
  ...
  if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest)
  {
    ...
    if (b->verbose)
    {
      message("  Splitting bad triangles.\n");
    }
    while ((m->badtriangles.items > 0) && (m->steinerleft != 0))
    {
      ...
      else
      {
        ...
        pooldealloc(&m->badtriangles, (VOID_D*) badtri);
      }
    }
  }
      ...
      && (m->steinerleft == 0))
  {
    ...
    message("  Delaunay.  If the Delaunay property is important to you,\n");
    message("  try increasing the number of Steiner points (controlled by\n");
    message("  the -S switch) slightly and try again.\n\n");
  }
}
...

{
  ...
  if (!b->quiet)
  {
    message("Adding vertices for second-order triangles.\n");
  }
  ...
  m->vertices.deaditemstack = (VOID_D*) NULL;
...

{
  ...
  if (m->invertices < 3) {
    messerr("Error:  Input must have at least three input vertices.");
    triexit(1);
  }
  if (m->mesh_dim != 2) {
    messerr("Error:  Triangle only works with two-dimensional meshes.");
    triexit(1);
  }
  if (m->nextras == 0) {
    b->weighted = 0;
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      messerr("Error:  Vertex %d has no x coordinate.", b->firstnumber + i);
      triexit(1);
    }
    x = (DREAL) strtod(stringptr, &stringptr);
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      messerr("Error:  Vertex %d has no y coordinate.", b->firstnumber + i);
      triexit(1);
    }
    y = (DREAL) strtod(stringptr, &stringptr);
    vertexloop[0] = x;
    vertexloop[1] = y;
    ...
    for (j = 2; j < 2 + m->nextras; j++) {
      ...
      if (*stringptr == '\0') {
        ...
        vertexloop[j] = (DREAL) strtod(stringptr, &stringptr);
      }
    }
    if (nodemarkers) {
...
void readholes(struct mesh *m, struct behavior *b,
               FILE *polyfile, char *polyfilename, DREAL **hlist, int *holes,
               DREAL **rlist, int *regions)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  *holes = (int) strtol(stringptr, &stringptr, 0);
  if (*holes > 0) {
    holelist = (DREAL *) trimalloc(2 * *holes * (int) sizeof(DREAL));
    *hlist = holelist;
    for (i = 0; i < 2 * *holes; i += 2) {
      stringptr = readline(inputline, polyfile, polyfilename);
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        messerr("Error:  Hole %d has no x coordinate.",
               b->firstnumber + (i >> 1));
        triexit(1);
      } else {
        holelist[i] = (DREAL) strtod(stringptr, &stringptr);
      }
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        messerr("Error:  Hole %d has no y coordinate.",
               b->firstnumber + (i >> 1));
        triexit(1);
      } else {
        holelist[i + 1] = (DREAL) strtod(stringptr, &stringptr);
      }
    }
  } else {
    *hlist = nullptr;
  ...
  if ((b->regionattrib || b->vararea) && !b->refine) {
    ...
    *regions = (int) strtol(stringptr, &stringptr, 0);
    if (*regions > 0) {
      regionlist = (DREAL *) trimalloc(4 * *regions * (int) sizeof(DREAL));
      *rlist = regionlist;
      index = 0;
      for (i = 0; i < *regions; i++) {
        stringptr = readline(inputline, polyfile, polyfilename);
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
          messerr("Error:  Region %d has no x coordinate.",
                 b->firstnumber + i);
          triexit(1);
        } else {
          regionlist[index++] = (DREAL) strtod(stringptr, &stringptr);
        }
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
          messerr("Error:  Region %d has no y coordinate.",
                 b->firstnumber + i);
          triexit(1);
        } else {
          regionlist[index++] = (DREAL) strtod(stringptr, &stringptr);
...
#ifdef ANSI_DECLARATORS
void writenodes(struct mesh *m, struct behavior *b, DREAL **pointlist,
DREAL **pointattriblist,
                int **pointmarkerlist)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  {
    *pointattriblist = (DREAL*) trimalloc(
        (int) (outvertices * m->nextras * sizeof(DREAL)));
  }
  ...
  while (vertexloop != (vertex) NULL)
  {
    if (!b->jettison || (vertextype(vertexloop)!= UNDEADVERTEX))
    {
      ...
      fmessage(outfile, "%4d    %.17g  %.17g", vertexnumber, vertexloop[0],
      vertexloop[1]);
      for (i = 0; i < m->nextras; i++)
      {
        /* Write an attribute. */
        fmessage(outfile, "  %.17g", vertexloop[i + 2]);
      }
      if (b->nobound)
      {
        fmessage(outfile, "\n");
...

{
  ...
  {
    *triangleattriblist = (DREAL*) trimalloc(
        (int) (m->triangles.items * m->eextras * sizeof(DREAL)));
  }
  tlist = *trianglelist;
  ...
  while (triangleloop.tri != (triangle*) NULL)
  {
    ...
    if (b->order == 1)
    {
      ...
      fmessage(outfile, "%4ld    %4d  %4d  %4d", elementnumber,
    ...
    else
    {
      ...
      fmessage(outfile, "%4ld    %4d  %4d  %4d  %4d  %4d  %4d", elementnumber,
...
#else /* not TRILIBRARY */
    for (i = 0; i < m->eextras; i++) {
      fmessage(outfile, "  %.17g", elemattribute(triangleloop, i));
    }
    fmessage(outfile, "\n");
#endif /* not TRILIBRARY */
...
void writepoly(struct mesh *m, struct behavior *b, char *polyfilename,
               DREAL *holelist, int holes, DREAL *regionlist, int regions,
               int argc, char **argv)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  {
    *segmentmarkerlist = (int*) trimalloc(
        (int) (m->subsegs.items * sizeof(int)));
  }
  slist = *segmentlist;
  ...
  /*   and number of boundary markers (zero or one).                    */
  fmessage(outfile, "%d  %d  %d  %d\n", 0, m->mesh_dim, m->nextras,
          1 - b->nobound);
  /* Number of segments, number of boundary markers (zero or one). */
  fmessage(outfile, "%ld  %d\n", m->subsegs.items, 1 - b->nobound);
#endif /* not TRILIBRARY */
  ...
  while (subsegloop.ss != (subseg*) NULL)
  {
    ...
    if (b->nobound) {
      fmessage(outfile, "%4ld    %4d  %4d\n", subsegnumber,
              vertexmark(endpoint1), vertexmark(endpoint2));
    } else {
      fmessage(outfile, "%4ld    %4d  %4d    %4d\n", subsegnumber,
  ...
  if (regions > 0) {
    ...
    for (regionnumber = 0; regionnumber < regions; regionnumber++) {
      ...
      fmessage(outfile, "%4ld   %.17g  %.17g  %.17g  %.17g\n",
...

{
  ...
  {
    *vpointlist = (DREAL*) trimalloc(
        (int) (m->triangles.items * 2 * sizeof(DREAL)));
  }
  /* Allocate memory for Voronoi vertex attributes if necessary. */
  if (*vpointattriblist == nullptr)
  {
    *vpointattriblist = (DREAL*) trimalloc(
        (int) (m->triangles.items * m->nextras * sizeof(DREAL)));
  }
  *vpointmarkerlist = nullptr;
  ...
  while (triangleloop.tri != (triangle*) NULL)
  {
    ...
    fmessage(outfile, "%4ld    %.17g  %.17g", vnodenumber, circumcenter[0],
            circumcenter[1]);
    for (i = 2; i < 2 + m->nextras; i++) {
      /* Interpolate the vertex attributes at the circumcenter. */
      fmessage(outfile, "  %.17g", torg[i] + xi * (tdest[i] - torg[i])
                                         + eta * (tapex[i] - torg[i]));
    }
    fmessage(outfile, "\n");
#endif /* not TRILIBRARY */
        ...
        triangleloop.orient++)
    {
      ...
      if ((triangleloop.tri < trisym.tri) || (trisym.tri == m->dummytri))
      {
        ...
        if (trisym.tri == m->dummytri)
        {
          ...
          fmessage(outfile, "%4ld   %d  %d   %.17g  %.17g\n", vedgenumber,
...

{
  ...
  DREAL cossquare;
  DREAL triarea;
  DREAL shortest, longest;
  DREAL trilongest2;
  DREAL smallestarea, biggestarea;
  DREAL triminaltitude2;
  DREAL minaltitude;
  DREAL triaspect2;
  DREAL worstaspect;
  DREAL smallestangle, biggestangle;
  DREAL radconst, degconst;
  int angletable[18];
  int aspecttable[16];
  int aspectindex;
  int tendegree;
  ...
  message("Mesh quality statistics:\n\n");
  radconst = PI / 18.0;
  degconst = 180.0 / PI;
  for (i = 0; i < 8; i++)
  {
    cossquaretable[i] = cos(radconst * (DREAL) (i + 1));
  ...
  message("  Smallest area: %16.5g   |  Largest area: %16.5g\n", smallestarea,
          biggestarea);
  message("  Shortest edge: %16.5g   |  Longest edge: %16.5g\n", shortest,
          longest);
  message("  Shortest altitude: %12.5g   |  Largest aspect ratio: %8.5g\n\n",
          minaltitude, worstaspect);

  message("  Triangle aspect ratio histogram:\n");
  message("  1.1547 - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d\n",
          ratiotable[0], aspecttable[0], ratiotable[7], ratiotable[8],
          aspecttable[8]);
  for (i = 1; i < 7; i++)
  {
    message("  %6.6g - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d\n",
            ratiotable[i - 1], ratiotable[i], aspecttable[i], ratiotable[i + 7],
            ratiotable[i + 8], aspecttable[i + 8]);
  }
  message("  %6.6g - %-6.6g    :  %8d    | %6.6g -            :  %8d\n",
          ratiotable[6], ratiotable[7], aspecttable[7], ratiotable[14],
          aspecttable[15]);
  message("  (Aspect ratio is longest edge divided by shortest altitude)\n\n");

  message("  Smallest angle: %15.5g   |  Largest angle: %15.5g\n\n",
          smallestangle, biggestangle);

  message("  Angle histogram:\n");
  for (i = 0; i < 9; i++)
  {
    message("    %3d - %3d degrees:  %8d    |    %3d - %3d degrees:  %8d\n",
...
{
  message("\nStatistics:\n\n");
  message("  Input vertices: %d\n", m->invertices);
  if (b->refine)
  {
    message("  Input triangles: %d\n", m->inelements);
  }
  if (b->poly)
  {
    message("  Input segments: %d\n", m->insegments);
    if (!b->refine)
    {
      message("  Input holes: %d\n", m->holes);
    }
  }

  message("\n  Mesh vertices: %ld\n", m->vertices.items - m->undeads);
  message("  Mesh triangles: %ld\n", m->triangles.items);
  message("  Mesh edges: %ld\n", m->edges);
  message("  Mesh exterior boundary edges: %ld\n", m->hullsize);
  if (b->poly || b->refine)
  {
    message("  Mesh interior boundary edges: %ld\n",
            m->subsegs.items - m->hullsize);
    message("  Mesh subsegments (constrained edges): %ld\n", m->subsegs.items);
  }
  message("\n");
  ...
  if (b->verbose)
  {
    ...
    message("Memory allocation statistics:\n\n");
    message("  Maximum number of vertices: %ld\n", m->vertices.maxitems);
    message("  Maximum number of triangles: %ld\n", m->triangles.maxitems);
    if (m->subsegs.maxitems > 0)
    {
      message("  Maximum number of subsegments: %ld\n", m->subsegs.maxitems);
    }
    if (m->viri.maxitems > 0)
    {
      message("  Maximum number of viri: %ld\n", m->viri.maxitems);
    }
    if (m->badsubsegs.maxitems > 0)
    {
      message("  Maximum number of encroached subsegments: %ld\n",
              m->badsubsegs.maxitems);
    }
    if (m->badtriangles.maxitems > 0)
    {
      message("  Maximum number of bad triangles: %ld\n",
              m->badtriangles.maxitems);
    }
    if (m->flipstackers.maxitems > 0)
    {
      message("  Maximum number of stacked triangle flips: %ld\n",
              m->flipstackers.maxitems);
    }
    if (m->splaynodes.maxitems > 0)
    {
      message("  Maximum number of splay tree nodes: %ld\n",
              m->splaynodes.maxitems);
    }
    message(
        "  Approximate heap memory use (bytes): %ld\n\n",
        m->vertices.maxitems * m->vertices.itembytes + m->triangles.maxitems
    ...
    message("Algorithmic statistics:\n\n");
    if (!b->weighted)
    {
      message("  Number of incircle tests: %ld\n", m->incirclecount);
    }
    else
    {
      message("  Number of 3D orientation tests: %ld\n", m->orient3dcount);
    }
    message("  Number of 2D orientation tests: %ld\n", m->counterclockcount);
    if (m->hyperbolacount > 0)
    {
      message("  Number of right-of-hyperbola tests: %ld\n", m->hyperbolacount);
    }
    if (m->circletopcount > 0)
    {
      message("  Number of circle top computations: %ld\n", m->circletopcount);
    }
    if (m->circumcentercount > 0)
    {
      message("  Number of triangle circumcenter computations: %ld\n",
              m->circumcentercount);
    }
    message("\n");
  }
...

{
  struct mesh m;
  struct behavior b;
  DREAL *holearray; /* Array of holes. */
  DREAL *regionarray; /* Array of regional attributes and area constraints. */
#ifndef TRILIBRARY
  FILE *polyfile;
#endif /* not TRILIBRARY */
//#ifndef NO_TIMER
...
//#endif /* not NO_TIMER */
  ...
  if (!b.quiet)
  {
    ...
    message(
        " milliseconds:  %ld\n",
        1000l * (tv2.tv_sec - tv1.tv_sec) + (tv2.tv_usec - tv1.tv_usec)
            / 1000l);
  }
#endif /* not NO_TIMER */
  ...
  if (!b.quiet)
  {
    ...
    if (b.usesegments && !b.refine)
    {
      message(
          ...
          1000l * (tv3.tv_sec - tv2.tv_sec) + (tv3.tv_usec - tv2.tv_usec)
              / 1000l);
    }
  }
#endif /* not NO_TIMER */
  ...
  if (!b.quiet)
  {
    ...
    if (b.poly && !b.refine)
    {
      message(
          ...
          1000l * (tv4.tv_sec - tv3.tv_sec) + (tv4.tv_usec - tv3.tv_usec)
              / 1000l);
    }
  }
#endif /* not NO_TIMER */
  ...
  if (!b.quiet)
  {
    ...
    if (b.quality)
    {
      message(
          ...
          1000l * (tv5.tv_sec - tv4.tv_sec) + (tv5.tv_usec - tv4.tv_usec)
              / 1000l);
    }
#endif /* not CDT_ONLY */
  }
  ...
  if (b.nonodewritten || (b.noiterationnum && m.readnodefile))
  {
    ...
    {
#ifdef TRILIBRARY
      message("NOT writing vertices.\n");
#else /* not TRILIBRARY */
      message("NOT writing a .node file.\n");
#endif /* not TRILIBRARY */
    }
  ...
  if (b.noelewritten)
  {
    ...
    {
#ifdef TRILIBRARY
      message("NOT writing triangles.\n");
#else /* not TRILIBRARY */
      message("NOT writing an .ele file.\n");
#endif /* not TRILIBRARY */
    }
  }
  else
  ...
  if (b.poly || b.convex)
  {
    ...
    if (b.nopolywritten || b.noiterationnum)
    {
      ...
      {
#ifdef TRILIBRARY
        message("NOT writing segments.\n");
#else /* not TRILIBRARY */
        message("NOT writing a .poly file.\n");
#endif /* not TRILIBRARY */
      }
    }
    else
...
#ifndef CDT_ONLY
  if (m.regions > 0) {
    trifree((VOID_D *) regionarray);
  }
#endif /* not CDT_ONLY */
  if (m.holes > 0) {
    trifree((VOID_D *) holearray);
  }
  if (b.geomview) {
  ...
  if (!b.quiet)
  {
    ...
    message(
        ...
        1000l * (tv6.tv_sec - tv5.tv_sec) + (tv6.tv_usec - tv5.tv_usec)
            / 1000l);
    message(
        "Total running milliseconds:  %ld\n",
        1000l * (tv6.tv_sec - tv0.tv_sec) + (tv6.tv_usec - tv0.tv_usec)
            / 1000l);
#endif /* not NO_TIMER */
