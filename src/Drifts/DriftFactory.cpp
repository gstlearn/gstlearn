...
namespace gstlrn
{
  ...
  ADrift* DriftFactory::createDriftByIdentifier(const String& driftname)
  {
    ...
    if (drift == nullptr) drift = DriftF::createByIdentifier(driftname);
    ...
    DriftList* drifts)
  {
    ...
    for (Id i = startVar; i < ndim; ++i)
    {
      for (Id d = 1; d <= remainingDegree; ++d)
      {
        exps[i] = d;
        _generateRecursive(
          ndim, order, i + 1, remainingDegree - d, exps, drifts);
        exps[i] = 0;
      }
    }
  }
   ...
   */
  DriftList* DriftFactory::createDriftListFromIRF(
    Id order,
    Id nfex,
    const CovContext& ctxt)
  {
    ...
    if (nfex > 0)
    {
      for (Id ifex = 0; ifex < nfex; ifex++) drifts->addDrift(new DriftF(ifex));
    }
   ...
   * @return
   */
  DriftList* DriftFactory::createDriftListForGradients(
    const DriftList* olddrifts,
    const CovContext& ctxt)
  {
    DriftM drft;
