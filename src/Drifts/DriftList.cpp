...
namespace gstlrn
{
  ...
  bool DriftList::isValid() const
  {
    ...
    for (Id il = 0; il < nbfl; il++)
    {
      ...
      for (Id jl = 0; jl < il; jl++)
      {
        ...
        if (str_il == str_jl)
        {
          messerr("Set of drift functions is invalid: %d and %d are similar",
                  il + 1,
                  jl + 1);
          return false;
        }
  ...
  void DriftList::setDriftCLByPart(Id ivar, Id ib, const VectorDouble& coef)
  {
    ...
    {
      messerr("The dimension of 'vec' (%d) is not equal to the number of drift "
              "functions (%d)",
              static_cast<Id>(coef.size()),
              nbfl);
      return;
    }
                                  ...
                                  const VectorDouble& coeffs) const
  {
    ...
    if (nbfl != ncoeff)
    {
      messerr("Dimension of 'coeffs' (%d) should match number of drift "
              "functions (%d)",
              ncoeff,
              nbfl);
      return TEST;
  ...
  VectorDouble
    DriftList::evalMeanVecByRanks(const Db* db,
  ...
  void DriftList::setPriorMeans(const VectorDouble& priorMeans)
  {
    ...
    {
      messerr("Dimension of 'priorMeans' (%d) does not match number of drift "
              "equations (%d)",
              static_cast<Id>(priorMeans.size()),
              nfeq);
      return;
    }
  ...
  void DriftList::setPriorCovs(const MatrixSymmetric& priorCovs)
  {
    ...
    {
      messerr("Dimensions of 'priorCovs' (%d, %d) does not match number of "
              "drift equations (%d)",
              priorCovs.getNRows(),
              priorCovs.getNCols(),
              nfeq);
