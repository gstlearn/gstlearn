...
namespace gstlrn
{
  ...
  void DriftList::_update()
  {
    ...
    if (static_cast<Id>(_mean.size()) != nvar) _mean = VectorDouble(nvar, 0.);
  ...
  String DriftList::toString(const AStringFormat* /*strfmt*/) const
  {
    ...
    if (getNDrift() <= 0) sstr << toStrVector("Known Mean(s)", getMeans());
    ...
    for (Id i = 0, nbfl = getNDrift(); i < nbfl; i++)
    {
      ...
      if (_filtered[i]) sstr << " (This component is filtered)";
      sstr << std::endl;
    }
    return sstr.str();
  }
  ...
  bool DriftList::isValid() const
  {
    ...
    for (Id il = 0; il < nbfl; il++)
    {
      ...
      for (Id jl = 0; jl < il; jl++)
      {
        ...
        {
          messerr(
            "Set of drift functions is invalid: %d and %d are similar", il + 1,
            jl + 1);
          return false;
        }
      }
  ...
  void DriftList::setDriftCLByPart(Id ivar, Id ib, const VectorDouble& coef)
  {
    ...
    {
      messerr(
        "The dimension of 'vec' (%d) is not equal to the number of drift "
        "functions (%d)",
        static_cast<Id>(coef.size()), nbfl);
      return;
    }
    for (Id il = 0; il < nbfl; il++) _setDriftCL(ivar, il, ib, coef[il]);
   ...
   */
  bool DriftList::isDriftSampleDefined(
    const Db* db,
    Id ib,
    Id nech,
      ...
      for (Id il = 0; il < nbfl; il++)
        for (Id ivar = 0; ivar < nvar; ivar++)
        {
          ...
          for (Id iech = 0; iech < nech; iech++)
            if (!FFFF(db->getLocVariable(loctype, nbgh[iech], ivar)))
              return true;
        }
    }
    else
  ...
  double
    DriftList::evalDriftCoef(const Db* db, Id iech, const VectorDouble& coeffs)
      const
  {
    auto nbfl = getNDrift();
    ...
    {
      messerr(
        "Dimension of 'coeffs' (%d) should match number of drift functions "
        "(%d)",
        ncoeff, nbfl);
      return TEST;
    }
   ...
   * @return
   */
  VectorDouble DriftList::evalDriftCoefs(
    const Db* db,
    ...
    bool useSel) const
  {
    ...
    {
      messerr(
        "'coeffs' dimension (%d) should match number of drift functions (%d)",
        ncoeff, nbfl);
      return vec;
    }
   ...
   * @return
   */
  bool DriftList::isDriftDifferentDefined(const VectorInt& powers, Id rank_fex)
    const
  {
    for (Id il = 0, nbfl = getNDrift(); il < nbfl; il++)
    {
   ...
   **
   *****************************************************************************/
  MatrixDense DriftList::evalDriftMat(
    const Db* db,
    const VectorInt& nbgh,
  ...
  Id DriftList::evalDriftMatInPlace(
    MatrixDense& mat,
    const Db* db,
    const VectorInt& nbgh,
    const ECalcMember& member) const
   ...
   */
  Id DriftList::evalDriftMatByRanksInPlace(
    MatrixDense& mat,
    const Db* db,
   ...
   * @return MatrixDense
   */
  MatrixDense DriftList::evalDriftMatByRanks(
    const Db* db,
    const VectorVectorInt& sampleRanks,
    const ECalcMember& member) const
  {
  ...
  VectorDouble DriftList::evalMeanVecByRanks(
    const Db* db,
   ...
   **
   *****************************************************************************/
  Id DriftList::evalDriftMatByTargetInPlace(
    MatrixDense& mat,
    const Db* db,
    Id iech2,
    const KrigOpt& krigopt) const
  {
    ...
    const VectorVectorInt& index =
      db->getSampleRanks(ivars, viech2, true, false, false);
  ...
  VectorDouble DriftList::evalDriftBySample(
    const Db* db,
    Id iech,
    const ECalcMember& member) const
  {
  ...
  void DriftList::evalDriftBySampleInPlace(
    const Db* db,
    Id iech,
  ...
  void DriftList::setPriorMeans(const VectorDouble& priorMeans)
  {
    ...
    {
      messerr(
        "Dimension of 'priorMeans' (%d) does not match number of drift "
        "equations (%d)",
  ...
  void DriftList::setPriorCovs(const MatrixSymmetric& priorCovs)
  {
    ...
    {
      messerr(
        "Dimensions of 'priorCovs' (%d, %d) does not match number of drift "
        "equations (%d)",
   ...
   **
   *****************************************************************************/
  double DriftList::evalDriftVarCoef(
    const Db* db,
    Id iech,
    Id ivar,
    const VectorDouble& coeffs) const
  {
    ...
    for (Id ib = 0, nfeq = getNDriftEquation(); ib < nfeq; ib++)
      drift +=
        evalDriftValue(db, iech, ivar, ib, ECalcMember::LHS) * coeffs[ib];
    return drift;
  }
  ...
  VectorDouble DriftList::evalDriftVarCoefs(
    const Db* db,
    const VectorDouble& coeffs,
    bool useSel) const
  {
  ...
  bool DriftList::serializeH5(H5::Group& grp) const
  {
    ...
    for (Id ibfl = 0; ret && ibfl < nbfl; ibfl++)
    {
      ...
      ret =
        ret && SerializeHDF5::writeValue(driftG, "Name", drift->getDriftName());
    }
