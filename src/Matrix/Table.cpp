...
namespace gstlrn
{
                                 ...
                                 const VectorString& colnames)
  {
    ...
    if (colLocal.empty()) colLocal = generateMultipleNames("Col", ncol);
    table->setColumnNames(colLocal);

    VectorString rowLocal = rownames;
    if (rowLocal.empty()) rowLocal = generateMultipleNames("Row", nrow);
  ...
  String Table::toString(const AStringFormat* /*strfmt*/) const
  {
    ...
    if (!_skipTitle)
    {
      if (!_title.empty()) sstr << toStrTitle(1, _title.c_str());
    }
  ...
  void Table::setColumnNames(const VectorString& colNames)
  {
    ...
    {
      messerr(
        "The size of 'colNames' (%d) does not match the number of columns (%d)",
        static_cast<Id>(colNames.size()),
        getNCols());
      return;
    }
    _colNames = colNames;
  ...
  void Table::setColumnName(Id icol, const String& name)
  {
    ...
    auto ncols = getNCols();
    if (_colNames.empty()) _colNames.resize(ncols, "  ");
    _colNames[icol] = name;
  }
  ...
  void Table::setRowName(Id irow, const String& name)
  {
    ...
    auto nrows = getNRows();
    if (_rowNames.empty()) _rowNames.resize(nrows, "  ");
    _rowNames[irow] = name;
  }
  ...
  void Table::setRowNames(const VectorString& rowNames)
  {
    ...
    {
      messerr(
        "The size of 'rowNames' (%d) does not match the number of rows (%d)",
        static_cast<Id>(rowNames.size()),
        getNRows());
      return;
    }
    _rowNames = rowNames;
