...
namespace gstlrn
{
   ...
   */
  void
    AMatrix::resetFromArray(Id nrows, Id ncols, const double* tab, bool byCol)
  {
    reset(nrows, ncols);
    if (byCol)
    {
  ...
  bool AMatrix::isSquare(bool printWhyNot) const
  {
    ...
    if (_nRows != _nCols)
    {
      if (printWhyNot)
        messerr(
          "The number of rows (%d) should match the number of columns (%d)",
          _nRows, _nCols);
      return false;
    }
    return true;
  ...
  bool AMatrix::isValid(Id irow, Id icol, bool printWhyNot) const
  {
    ...
    if (irow < 0 || irow >= getNRows())
    {
      if (printWhyNot)
        messerr(
          "Argument 'irow' invalid: it should lie in [0;%d[", irow, getNRows());
      return false;
    }
    if (icol < 0 || icol >= getNCols())
    {
      if (printWhyNot)
        messerr(
          "Argument 'icol' invalid: it should lie in [0;%d[", icol, getNCols());
      return false;
    }
    return true;
  }
    ...
    for (Id irow = 0; irow < _nRows; irow++)
      for (Id icol = 0; icol < _nCols; icol++)
      {
        if (ABS(getValue(irow, icol) - getValue(icol, irow)) > eps)
        {
          if (printWhyNot)
            messerr(
              "Elements (%d;%d)=%lf and (%d;%d)=%kf should be equal", irow,
              icol, getValue(irow, icol), icol, irow, getValue(icol, irow));
          return false;
        }
      }
    return true;
    ...
    for (Id irow = 0; irow < getNRows(); irow++)
      for (Id icol = 0; icol < getNCols(); icol++)
      {
        ...
        if (ABS(getValue(irow, icol) - refval) > EPSILON10)
        {
          if (printWhyNot)
            messerr(
              "The term (%d,%d) should be equal to %lf (%lf)", irow + 1,
 ...
 */
  ...
  void AMatrix::resize(Id nrows, Id ncols)
  {
    ...
    if (!_matrixNeedToReset(nrows, ncols)) return;
   ...
   */
  void AMatrix::prodMatMatInPlace(
    const AMatrix* x,
    const AMatrix* y,
    bool transposeX,
    bool transposeY)
  {
    if (getFlagMatrixCheck()
        && !_isMatrixCompatible(
          "AMatrix::prodMatMatInPlace", x, 0, transposeX, y, 0, transposeY))
      return;
    ...
    for (Id irow = 0; irow < ni1; irow++)
    {
      for (Id icol = 0; icol < nm2; icol++)
      {
        ...
        {
          double v1 =
            (transposeX) ? x->getValue(k, irow) : x->getValue(irow, k);
          double v2 =
            (transposeY) ? y->getValue(icol, k) : y->getValue(k, icol);
          value += v1 * v2;
        }
        setValue(irow, icol, value);
      }
   ...
   */
  void AMatrix::prodNormMatMatInPlace(
    const AMatrix* a,
    const AMatrix* m,
    bool transpose)
  {
    if (getFlagMatrixCheck()
        && !_isMatrixCompatible(
          "AMatrix::prodNormMatMatInPlace", a, 0, transpose, m, 0, false, a, 0,
          !transpose))
      return;
  ...
  void AMatrix::prodNormMatVecInPlace(
    const AMatrix* a,
    ...
    bool transpose)
  {
    if (getFlagMatrixCheck()
        && !_isMatrixCompatible(
          "AMatrix::prodNormMatVecInPlace", a, 0, transpose, a, 0, !transpose))
      return;
  ...
  void AMatrix::prodNormMatInPlace(const AMatrix* a, bool transpose)
  {
    if (getFlagMatrixCheck()
        && !_isMatrixCompatible(
          "AMatrix::prodNormMatInPlace", a, 0, transpose, a, 0, !transpose))
      return;
  ...
  double
    AMatrix::prodVecMatVec(const VectorDouble& x, const VectorDouble& y) const
  {
    if (getFlagMatrixCheck()
        && !_isMatrixCompatible(
          "AMatrix::quadraticMatrix", nullptr, static_cast<Id>(x.size()), true,
          this, 0, false, nullptr, static_cast<Id>(y.size()), false))
      return TEST;
  ...
  Id AMatrix::solve(const VectorDouble& b, VectorDouble& x) const
  {
    ...
    if (static_cast<Id>(b.size()) != _nRows
        || static_cast<Id>(x.size()) != _nRows)
    {
  ...
  String AMatrix::toString(const AStringFormat* strfmt) const
  {
    ...
    sstr << toStrMatrix(
      String(), VectorString(), VectorString(), true, _nRows, _nCols,
  ...
  bool AMatrix::_isMatrixConsistent(const VectorDouble& tab) const
  {
    ...
    {
      messerr(
        "The argument 'tab'(%d) does not have correct dimension (%d)",
  ...
  bool AMatrix::_identifyRowAndCol(
    const AMatrix* mat,
    Id vsize,
    bool transpose,
    Id* nrow,
    Id* ncol)
  {
    ...
    if (mat != nullptr)
    {
      ...
      {
        messerr(
          "Both Matrix and VectorDouble are defined. Only one should be used");
        return false;
      }
      nr = mat->getNRows();
      nc = mat->getNCols();
   ...
   */
  bool AMatrix::_isMatrixCompatible(
    const String& name,
    const AMatrix* mat1,
    Id vsize1,
    bool transpose1,
    ...
    bool transpose3)
  {
    ...
    Id nrow1 = -1;
    Id ncol1 = -1;
    if (!_identifyRowAndCol(mat1, vsize1, transpose1, &nrow1, &ncol1))
      return false;
    ...
    if (_identifyRowAndCol(mat2, vsize2, transpose2, &nrow2, &ncol2))
    {
      ...
      {
        messerr(
          "Linkage error called from %s: Level = 1 - Nrow = %d - Ncol = %d",
      ...
      if (_identifyRowAndCol(mat3, vsize3, transpose3, &nrow3, &ncol3))
      {
        ...
        {
          messerr(
            "Linkage error called from %s: Level = 2 - Nrow = %d - Ncol = %d",
  ...
  {
    if (icol < 0 || icol >= getNCols()) my_throw("Incorrect argument 'icol'");
  ...
  {
    if (irow < 0 || irow >= getNRows()) my_throw("Incorrect argument 'irow'");
    ...
    for (Id irow = 0; irow < _nRows; irow++)
      for (Id icol = 0; icol < _nCols; icol++)
      {
        ...
        if (value < 0.)
        {
          if (verbose)
            messerr(
              "The matrix term (%d,%d) is not non-negative (%lf)", irow, icol,
              value);
          return false;
        }
      }
  ...
  void AMatrix::dumpRange(const char* title)
  {
    ...
    StatResults stats =
      ut_statistics(static_cast<Id>(elements.size()), elements.data());
    ...
    message(
      " Descr: m=%d n=%d nnz=%d\n", getNRows(), getNCols(),
      _getMatrixPhysicalSize());
    if (elements.size() > 0)
      message(
        " Range: [%lf ; %lf] (%d/%d)\n", stats.mini, stats.maxi, stats.nvalid,
        elements.size());
    else
