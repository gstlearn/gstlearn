...
namespace gstlrn
{
   ...
   */
  void AMatrix::resetFromVD(Id nrows,
                            Id ncols,
  ...
  bool AMatrix::isIdentity(bool printWhyNot) const
  {
    for (Id irow = 0; irow < getNRows(); irow++)
      for (Id icol = 0; icol < getNCols(); icol++)
      {
        ...
        {
          if (printWhyNot)
            messerr("The term (%d,%d) should be equal to %lf (%lf)",
                    irow + 1,
                    icol + 1,
                    refval,
                    getValue(irow, icol));
 ...
 */
  ...
  void AMatrix::prodNormMatVecInPlace(const AMatrix* a,
  ...
  bool AMatrix::isNonNegative(bool verbose) const
  {
    for (Id irow = 0; irow < _nRows; irow++)
      for (Id icol = 0; icol < _nCols; icol++)
      {
        ...
        {
          if (verbose)
            messerr("The matrix term (%d,%d) is not non-negative (%lf)",
                    irow,
                    icol,
                    value);
          return false;
