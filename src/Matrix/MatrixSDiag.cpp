...
MatrixSDiag::MatrixSDiag(int nrows, bool sparse)
...
MatrixSDiag::MatrixSDiag(const MatrixSDiag &r) 
  : AMatrixSquare(r)
{
  _recopy(r);
}

MatrixSDiag& MatrixSDiag::operator= (const MatrixSDiag &r)
{
  if (this != &r)
  {
    _deallocate();
...
MatrixSDiag::~MatrixSDiag()
{
  _deallocate();
}

IClonable* MatrixSDiag::clone() const
{
  return new MatrixSDiag(*this);
}

double MatrixSDiag::_getValue(int irow, int icol) const
{
  if (irow == icol)
...
double MatrixSDiag::_getValue(int irank) const
{
  _isRankValid(irank);
  return _diagMatrix[irank];
}

void MatrixSDiag::_setValue(int irow, int icol, double value)
{
  if (irow == icol)
  {
...
void MatrixSDiag::_setValue(int irank, double value)
{
  _isRankValid(irank);
  _diagMatrix[irank] = value;
}

void MatrixSDiag::_prodVector(const double *in, double *out) const
{
  int nrow = getNRows();
...
void MatrixSDiag::transposeInPlace()
{
 ...
 */
void MatrixSDiag::_setValues(const double* values, bool byCol)
{
  int ecr = 0;
...
double MatrixSDiag::_determinant() const
{
  double deter = 1.;
...
double& MatrixSDiag::_getValueRef(int irow, int icol)
{
...
void MatrixSDiag::_deallocate()
{
}

void MatrixSDiag::_recopy(const MatrixSDiag &r)
{
  _diagMatrix = r._diagMatrix;
}

void MatrixSDiag::_allocate()
{
  _diagMatrix.resize(_getMatrixSize());
}

bool MatrixSDiag::_isIndexValid(int irow, int icol) const
{
...
int MatrixSDiag::_getMatrixSize() const
{
  return (getNRows());
}

bool MatrixSDiag::isValid(int irow, int icol, bool printWhyNot) const
{
...
void MatrixSDiag::addScalar(double v)
{
  my_throw("This function does not make sense for Diagonal Matrix");
}

int MatrixSDiag::_solve(const VectorDouble& b, VectorDouble& x) const
{
  for (int irow=0; irow<getNRows(); irow++)
  {
...
void MatrixSDiag::setColumn(int icol, const VectorDouble& tab)
{
  my_throw("This function does not make sense for Diagonal Matrix");
}

/*! Set the contents of a Row */
void MatrixSDiag::setRow(int irow, const VectorDouble& tab)
{
  my_throw("This function does not make sense for Diagonal Matrix");
}

String MatrixSDiag::toString(int level) const
{
