...
namespace gstlrn
{
  ...
  void NF_Triplet::force(Id nrow, Id ncol)
  {
    ...
    if (nrow_max < nrow || ncol_max < ncol) add(nrow - 1, ncol - 1, 0.);
  }
  ...
  NF_Triplet NF_Triplet::createFromEigen(const EigenSparseMatrix& mat,
                                         Id shiftRow,
                                         Id shiftCol)
  {
    NF_Triplet NF_T;
  ...
  VectorDouble NF_Triplet::getValues() const
  {
    auto n = getNElements();
    VectorDouble vec(n);
    for (Id i = 0; i < n; i++) vec[i] = _eigenT[i].value();
    return vec;
  }
  ...
  VectorInt NF_Triplet::getRows(bool flag_from_1) const
  {
    ...
    VectorInt vec(n);
    for (Id i = 0; i < n; i++) vec[i] = _eigenT[i].row() + shift;
    return vec;
  }
  ...
  VectorInt NF_Triplet::getCols(bool flag_from_1) const
  {
    ...
    Id shift = (flag_from_1) ? 1 : 0;
    VectorInt vec(n);
    for (Id i = 0; i < n; i++) vec[i] = _eigenT[i].col() + shift;
    return vec;
  }
