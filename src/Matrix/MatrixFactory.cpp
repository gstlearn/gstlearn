...
namespace gstlrn
{
   ...
   */
  AMatrix* MatrixFactory::createReduce(
    const AMatrix* x,
    ...
    bool flagKeepCols)
  {
    ...
    else
    {
      ...
      if (!flagKeepRows)
        localSelRows = VH::complement(VH::sequence(nrows), localSelRows);
    }
    ...
    else
    {
      ...
      if (!flagKeepCols)
        localSelCols = VH::complement(VH::sequence(ncols), localSelCols);
    }
    ...
    else if (mxrg != nullptr)
    {
      ...
      messageAbort(
        "CreateReduce cannot be called for such matrix. This should never "
        "happen");
    ...
    bool flagKeepCol)
  {
    ...
    return MatrixFactory::createReduce(
      x, localSelRows, localSelCols, flagKeepRow, flagKeepCol);
  }
    ...
    bool flagShiftCol)
  {
    ...
    if ((a1->isSparse() && !a2->isSparse())
        || (!a1->isSparse() && a2->isSparse()))
    {
