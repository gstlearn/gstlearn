...
namespace gstlrn
{
        ...
        if (im1 >= 0)
        {
          for (Id k = 0; k <= im1; k++)
          {
            tus.setValue(i,
                         j,
                         tus.getValue(i, j)
                           - tls.getValue(i, k) * tus.getValue(k, j));
          }
        }
      }
      if (ip1 < neq)
          ...
          if (im1 >= 0)
          {
            for (Id k = 0; k <= im1; k++)
            {
              tls.setValue(j,
                           i,
                           tls.getValue(j, i)
                             - tls.getValue(j, k) * tus.getValue(k, i));
            }
          }
   ...
   **
   *****************************************************************************/
  Id MatrixSquare::_forwardLU(const MatrixSquare& tls,
                              const double* b,
                              double* x,
                              double eps)
  {
   ...
   **
   *****************************************************************************/
  Id MatrixSquare::_backwardLU(const MatrixSquare& tus,
                               const double* b,
                               double* x,
                               double eps)
  {
  ...
  MatrixSquare* prodNormMatVec(const MatrixDense& a,
                                                    ...
                                                    const VectorDouble& vecsup)
  {
    ...
    {
      messerr("Arguments 'vecdiag'(%d), 'vecinf'(%d) and 'vecsup'(%d) should "
              "have same dimension",
              vecdiag.size(),
              vecinf.size(),
              vecsup.size());
