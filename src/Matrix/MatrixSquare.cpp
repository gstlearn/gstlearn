...
namespace gstlrn
{
  ...
  {
    double res = 0.;
    for (Id irow = 0; irow < getNSize(); irow++) res += getValue(irow, irow);
    return res;
  }
  ...
  bool MatrixSquare::invert3x3(MatrixSquare& res) const
  {
    ...
    const auto em = this->eigenMat();
    const auto det =
      (em(0, 0) * em(1, 1) * em(2, 2)) + (em(0, 1) * em(1, 2) * em(2, 0))
      + (em(1, 0) * em(2, 1) * em(0, 2)) - (em(2, 0) * em(1, 1) * em(0, 2))
      - (em(1, 0) * em(0, 1) * em(2, 2)) - (em(2, 1) * em(1, 2) * em(0, 0));
    if (det == 0.)
    {
      return false;
    }
    ...
    for (Id irow = 0; irow < n; irow++)
    {
      for (Id icol = 0; icol < n; icol++)
      {
        ...
        for (Id k = 0; k < n; k++)
        {
          ...
          {
            value +=
              r1.getValue(k, irow) * x.getValue(k, l) * r2.getValue(l, icol);
            value +=
              r2.getValue(k, irow) * x.getValue(k, l) * r1.getValue(l, icol);
          }
        }
  ...
  double MatrixSquare::determinant(void) const
  {
    ...
    switch (neq)
    {
      case 1: deter = getValue(0, 0); break;

      case 2:
        deter =
          (getValue(0, 0) * getValue(1, 1) - getValue(1, 0) * getValue(0, 1));
        break;
      ...
      default:
        Id neqm1 = neq - 1;
        MatrixSquare c(
          neqm1); // TODO Merge MatrixSquare and MatrixSquare into MatrixSquare
  ...
  bool MatrixSquare::_isNumbersValid(Id nrows, Id ncols) const
  {
    ...
    {
      messerr(
        "Arguments 'nrows' and 'ncols' should be equal for Square Matrices");
      return false;
    }
    return true;
  }
  ...
  MatrixSquare* MatrixSquare::createFromVD(
   ...
   */
  Id MatrixSquare::decomposeLU(MatrixSquare& tls, MatrixSquare& tus, double eps)
  {
    auto neq = getNRows();
    tls.fill(0.);
    tus.fill(0.);

    for (Id i = 0; i < neq; i++) tls.setValue(i, i, 1.);
      ...
      for (Id j = 0; j < neq; j++)
      {
        ...
        if (im1 >= 0)
        {
          for (Id k = 0; k <= im1; k++)
          {
            tus.setValue(
              i, j,
              tus.getValue(i, j) - tls.getValue(i, k) * tus.getValue(k, j));
          }
        }
      }
      if (ip1 < neq)
        ...
        for (Id j = ip1; j < neq; j++)
        {
          ...
          if (im1 >= 0)
          {
            for (Id k = 0; k <= im1; k++)
            {
              tls.setValue(
                j, i,
                tls.getValue(j, i) - tls.getValue(j, k) * tus.getValue(k, i));
            }
          }
  ...
  Id MatrixSquare::_invertLU()
  {
    ...
    for (Id i = 0; i < neq; i++)
    {
      ...
      for (Id j = 0; j < neq; j++) ais.setValue(i, j, x[j]);
    }
  ...
  Id MatrixSquare::_solveLU(
    const MatrixSquare& tus,
    ...
    double eps)
  {
    ...
    {
      double tmp = b[i];
      for (Id j = 0; j < i; j++) tmp -= tls.getValue(i, j) * x[j];
    ...
    double eps)
  {
    ...
    {
      double tmp = b[i];
      for (Id j = i + 1; j < neq; j++) tmp -= tus.getValue(i, j) * x[j];
  ...
  MatrixSquare*
    prodNormMatMat(const MatrixDense* a, const MatrixDense* m, bool transpose)
  {
   ...
   */
  MatrixSquare* MatrixSquare::createFromTridiagonal(
    const VectorDouble& vecdiag,
    ...
    const VectorDouble& vecsup)
  {
    ...
    {
      messerr(
        "Arguments 'vecdiag'(%d), 'vecinf'(%d) and 'vecsup'(%d) should have "
        "same dimension",
    ...
    for (Id i = 0; i < neq; i++)
    {
      ...
      if (i > 0) res->setValue(i, i - 1, vecinf[i]); // sous-diagonale
      if (i < neq - 1) res->setValue(i, i + 1, vecsup[i]); // sur-diagonale
    }
    return res;
  }
