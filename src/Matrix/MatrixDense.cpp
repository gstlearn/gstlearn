...
namespace gstlrn
{
  ...
  void
    MatrixDense::updValue(Id irow, Id icol, const EOperator& oper, double value)
  {
    if (getFlagMatrixCheck() && !_isIndexValid(irow, icol)) return;
    double result = modifyOperator(oper, eigenMat()(irow, icol), value);
    eigenMat().coeffRef(irow, icol) = result;
    eigenMat()(irow, icol) = result;
    if (mustBeSymmetric() && irow != icol) eigenMat()(icol, irow) = result;
  }
  ...
  Id MatrixDense::_invert()
  {
    ...
    if (!eigenMat().allFinite()) return 1;
    return 0;
  }
  ...
  Id MatrixDense::invert2(MatrixDense& res) const
  {
    ...
    if (!res.eigenMat().allFinite()) return 1;
    return 0;
  }
  ...
  void MatrixDense::addMatNoCheck(
    const MatrixDense& y,
    const double cx,
    const double cy)
  {
  ...
  void MatrixDense::_productGeneral(
    MatrixDense& res,
    ...
    bool transpose2)
  {
    ...
    if (transpose1)
    {
      if (transpose2)
      {
        res.eigenMat().noalias() =
          other1.eigenMat().transpose() * other2.eigenMat().transpose();
      }
      else
      {
        res.eigenMat().noalias() =
          other1.eigenMat().transpose() * other2.eigenMat();
      }
    }
    else
    {
      ...
      {
        res.eigenMat().noalias() =
          other1.eigenMat() * other2.eigenMat().transpose();
      }
      else
      {
   ...
   */
  void MatrixDense::_prodVecAddGeneral(
    VectorDouble& res,
  ...
  void MatrixDense::prodMatMatInPlace(
    const AMatrix* x,
    const AMatrix* y,
    bool transposeX,
    bool transposeY)
  {
    if (getFlagMatrixCheck()
        && !_isMatrixCompatible(
          "MatrixDense::prodMatMatInPlace", x, 0, transposeX, y, 0, transposeY))
      return;
    ...
    else
    {
      ...
      if (transposeX)
      {
        if (transposeY)
        {
          ...
          eigenMat().noalias() =
            xm->eigenMat().transpose() * ym->eigenMat().transpose();
        }
        else
        {
   ...
   */
  void MatrixDense::prodNormMatMatInPlace(
    const AMatrix* a,
    const AMatrix* m,
    bool transpose)
  {
   ...
   */
  void MatrixDense::prodNormMatVecInPlace(
    const AMatrix* a,
    const VectorDouble& vec,
    bool transpose)
  {
    if (getFlagMatrixCheck()
        && !_isMatrixCompatible(
          "MatrixDense::prodNormMatVecInPlace", a, 0, transpose, a, 0,
          !transpose))
      return;
    ...
    else
    {
      if (transpose)
      {
        Eigen::Map<const Eigen::VectorXd> vecm(vec.data(), vec.size());
        eigenMat().noalias() =
          am->eigenMat().transpose() * vecm * am->eigenMat();
      }
      else
      {
        Eigen::Map<const Eigen::VectorXd> vecm(vec.data(), vec.size());
        eigenMat().noalias() =
          am->eigenMat() * vecm * am->eigenMat().transpose();
      }
    }
  ...
  void MatrixDense::prodNormMatInPlace(const AMatrix* a, bool transpose)
  {
    if (getFlagMatrixCheck()
        && !_isMatrixCompatible(
          "MatrixDense::prodNormMatInPlace", a, 0, transpose, a, 0, !transpose))
      return;
  ...
  MatrixDense* MatrixDense::createFromVD(
    ...
    bool invertColumnOrder)
  {
    ...
    {
      messerr(
        "Inconsistency between arguments 'nrow'(%d) and 'ncol'(%d)", nrow,
        ncol);
      messerr(
        "and the dimension of the input Vector (%d)",
        static_cast<Id>(X.size()));
    }
    auto* mat = new MatrixDense(nrow, ncol);
    ...
    bool flagShiftCol)
  {
    ...
    Id nrows = (flagShiftRow) ? A1->getNRows() + A2->getNRows()
                              : MAX(A1->getNRows(), A2->getNRows());
    Id ncols = (flagShiftCol) ? A1->getNCols() + A2->getNCols()
    ...
    for (Id irow = 0; irow < A2->getNRows(); irow++)
      for (Id icol = 0; icol < A2->getNCols(); icol++)
        mat->setValue(
          irow + shiftRow, icol + shiftCol, A2->getValue(irow, icol));
    ...
    bool flagInvertCol)
  {
    ...
    for (Id icol = 0; icol < ncols; icol++)
    {
      if (!checkArg("Selected Column index", cols[icol], ncoltotal))
        return false;
    }
    ...
    bool flagInvertCol)
  {
    ...
    for (Id irow = 0; irow < nrows; irow++)
    {
      if (!checkArg("Selected Row index", rows[irow], getNRows())) return;
    }
    for (Id icol = 0; icol < ncols; icol++)
    {
      if (!checkArg("Selected Column index", cols[icol], getNCols())) return;
    }
   ...
   */
  MatrixDense
    MatrixDense::compressMatLC(const MatrixDense& matLC, bool transpose)
  {
    auto nrows = getNRows();
    auto ncols = getNCols();
    ...
    if (!transpose)
    {
      ...
      {
        messerr(
          "Number of Columns (%d) should match number of columns of 'matLC' "
          "(%d)",
          ncols, ncolCL);
        return mat;
      }
    ...
    else
    {
      ...
      {
        messerr(
          "Number of Rows (%d) should match number of Columns of 'matLC' (%d)",
          nrows, ncolCL);
        return mat;
      }
  ...
  void MatrixDense::sum(
    const MatrixDense* mat1,
  ...
  void MatrixDense::_prodnormGeneral(
    MatrixDense& res,
    const MatrixDense& a,
    const MatrixDense& m,
    bool transpose)
  {
    ...
    else
    {
      ...
      {
        res.eigenMat().noalias() =
          a.eigenMat().transpose() * m.eigenMat() * a.eigenMat();
      }
      else
      {
        res.eigenMat().noalias() =
          a.eigenMat() * m.eigenMat() * a.eigenMat().transpose();
      }
    }
  }
  ...
  bool MatrixDense::_areIdenticalGeneral(
    const MatrixDense& a,
    const MatrixDense& b,
    bool verbose)
  {
    ...
    for (Id icol = 0; icol < ncols; icol++)
      for (Id irow = 0; irow < nrows; irow++)
      {
        ...
        if (ABS(v1 - v2) > EPSILON6)
        {
          if (verbose)
            messerr(
              "Matrices are not identical at position (%d, %d)", irow, icol);
          return false;
        }
      }
    return true;
