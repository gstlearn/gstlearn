...
namespace gstlrn
{
  ...
  void MatrixDense::addMatNoCheck(const MatrixDense& y,
                                  const double cx,
                                  const double cy)
  {
    eigenMat() *= cx;
   ...
   */
  void MatrixDense::prodNormMatVecInPlace(const AMatrix* a,
                                          const VectorDouble& vec,
                                          bool transpose)
  {
    if (getFlagMatrixCheck()
                                         ...
                                         bool invertColumnOrder)
  {
    ...
    {
      messerr("Inconsistency between arguments 'nrow'(%d) and 'ncol'(%d)",
              nrow,
              ncol);
      messerr("and the dimension of the input Vector (%d)",
                                 ...
                                 bool flagShiftCol)
  {
    ...
    for (Id irow = 0; irow < A2->getNRows(); irow++)
      for (Id icol = 0; icol < A2->getNCols(); icol++)
        mat->setValue(irow + shiftRow,
                      icol + shiftCol,
                      A2->getValue(irow, icol));
    ...
    MatrixDense::compressMatLC(const MatrixDense& matLC, bool transpose)
  {
    ...
    if (!transpose)
    {
      if (ncols != ncolCL)
      {
        messerr("Number of Columns (%d) should match number of columns of "
                "'matLC' (%d)",
                ncols,
                ncolCL);
        return mat;
  ...
  bool MatrixDense::_areIdenticalGeneral(const MatrixDense& a,
                                         const MatrixDense& b,
                                         bool verbose)
  {
    ...
    for (Id icol = 0; icol < ncols; icol++)
      for (Id irow = 0; irow < nrows; irow++)
      {
        ...
        {
          if (verbose)
            messerr("Matrices are not identical at position (%d, %d)",
                    irow,
                    icol);
          return false;
        }
