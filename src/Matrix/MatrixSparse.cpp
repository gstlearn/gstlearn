...
namespace gstlrn
{
  ...
  void MatrixSparse::resetFromArray(
    Id nrows,
    Id ncols,
    const double* tab,
    bool byCol)
  ...
  void MatrixSparse::fill(double value)
  {
    ...
    NF_Triplet NF_T;
    for (Id irow = 0; irow < nrow; irow++)
      for (Id icol = 0; icol < ncol; icol++) NF_T.add(irow, icol, value);
 ...
 */
  ...
  MatrixSparse* MatrixSparse::createFromTriplet(
    const NF_Triplet& NF_T,
    Id nrow,
    Id ncol,
    Id nrowmax)
  {
  ...
  MatrixSparse* MatrixSparse::createFillRandom(
    Id nrow,
    Id ncol,
    double zeroPercent,
    Id seed)
  ...
  MatrixSparse* MatrixSparse::addMatMat(
    const MatrixSparse* x,
  ...
  {
    return (Eigen::RowVectorXd::Ones(eigenMat().rows()) * eigenMat().cwiseAbs())
      .maxCoeff();
  }

  void MatrixSparse::getStats(Id* nrows, Id* ncols, Id* count, double* percent)
    const
  {
    *nrows = getNRows();
    *ncols = getNCols();
  ...
  Id MatrixSparse::addVecInPlaceEigen(
    const Eigen::Map<const Eigen::VectorXd>& xm,
  ...
  void MatrixSparse::_productGeneral(
    MatrixSparse& res,
    ...
    bool transpose2)
  {
    if (transpose1)
    {
      if (transpose2)
        res.eigenMat() =
          other1.eigenMat().transpose() * other2.eigenMat().transpose();
      else
   ...
   */
  void MatrixSparse::_prodVecAddGeneral(
    VectorDouble& res,
   ...
   */
  void MatrixSparse::prodMatMatInPlace(
    const AMatrix* x,
    const AMatrix* y,
    bool transposeX,
    bool transposeY)
  {
    if (getFlagMatrixCheck()
        && !_isMatrixCompatible(
          "MatrixSparse::prodMatMatInPlace", x, 0, transposeX, y, 0,
          transposeY))
      return;
  ...
  MatrixSparse*
    prodNormMatMat(const MatrixSparse* a, const MatrixSparse* m, bool transpose)
  {
   ...
   */
  void MatrixSparse::prodNormDiagVecInPlace(
    const VectorDouble& vec,
    Id oper_choice)
  {
    if (!isSquare())
  ...
  void MatrixSparse::prodNormMatVecInPlace(
    const AMatrix* a,
    const VectorDouble& vec,
    bool transpose)
  {
    if (getFlagMatrixCheck())
    {
      ...
      if (!_isMatrixCompatible(
            "MatrixSparse::prodNormMatVecInPlace", a, 0, transpose, a, 0,
            !transpose))
        return;
    }
    ...
    else
    {
      if (transpose)
      {
        Eigen::Map<const Eigen::VectorXd> vecm(vec.data(), vec.size());
        eigenMat() =
          am->eigenMat().transpose() * vecm.asDiagonal() * am->eigenMat();
      }
      else
      {
        Eigen::Map<const Eigen::VectorXd> vecm(vec.data(), vec.size());
        eigenMat() =
          am->eigenMat() * vecm.asDiagonal() * am->eigenMat().transpose();
      }
    }
  ...
  void MatrixSparse::prodNormMatInPlace(const AMatrix* a, bool transpose)
  {
    if (getFlagMatrixCheck()
        && !_isMatrixCompatible(
          "MatrixSparse::prodNormMatInPlace", a, 0, transpose, a, 0,
          !transpose))
      return;
  ...
  void MatrixSparse::prodNormMatMatInPlace(
    const AMatrix* a,
    const AMatrix* m,
    bool transpose)
  {
    if (getFlagMatrixCheck()
        && !_isMatrixCompatible(
          "MatrixSparse::prodNormMatMatInPlace", a, 0, transpose, m, 0, false,
          a, 0, !transpose))
      return;
    ...
    else
    {
      ...
      {
        eigenMat() =
          (am->eigenMat().transpose() * mm->eigenMat()) * am->eigenMat();
      }
      else
      {
        eigenMat() =
          (am->eigenMat() * mm->eigenMat()) * am->eigenMat().transpose();
      }
    }
  ...
  {
    if (!isSquare()) my_throw("Invert method is restricted to Square matrices");
    auto n = getNCols();
  ...
  {
    if (!isSquare()) my_throw("Invert method is limited to Square Matrices");
    if (static_cast<Id>(b.size()) != getNRows()
        || static_cast<Id>(x.size()) != getNRows())
  ...
  void MatrixSparse::_allocate(Id nrow, Id ncol, Id ncolmax)
  {
    ...
    {
      eigenMat().reserve(
        Eigen::VectorXi::Constant(nrow, static_cast<I32>(ncolmax)));
    }
    _nColMax = ncolmax;
  }
  ...
  void MatrixSparse::_allocate()
  {
    eigenMat() = EigenSparseMatrix(getNRows(), getNCols());
    {
      ...
      {
        eigenMat().reserve(
          Eigen::VectorXi::Constant(getNCols(), static_cast<I32>(_nColMax)));
      }
  ...
  {
    return MatrixSparse::createFromTriplet(
      matin->getMatrixToTriplet(), matin->getNRows(), matin->getNCols(), -1);
  }

  Id MatrixSparse::_eigen_findColor(
    Id imesh,
    Id ncolor,
    VectorInt& colors,
    ...
    bool flagShiftCol)
  {
    ...
    Id nrow = (flagShiftRow) ? A1->getNRows() + A2->getNRows()
                             : MAX(A1->getNRows(), A2->getNRows());
    Id ncol = (flagShiftCol) ? A1->getNCols() + A2->getNCols()
                             : MAX(A1->getNCols(), A2->getNCols());
  ...
  MatrixSparse* MatrixSparse::glue(
    const MatrixSparse* A1,
    const MatrixSparse* A2,
    bool flagShiftRow,
    bool flagShiftCol)
  {
  ...
  MatrixSparse* MatrixSparse::extractSubmatrixByRanks(
    const VectorInt& rank_rows,
  ...
  MatrixSparse* MatrixSparse::extractSubmatrixByColor(
  ...
  {
    *yk = 0.;
    for (EigenSparseMatrix::InnerIterator it(eigenMat(), iech); it; ++it)
    {
  ...
  /*! Extract a Column */
  MatrixSparse*
    MatrixSparse::getColumnAsMatrixSparse(Id icol, double coeff) const
  {
    auto nrows = getNRows();
  ...
  EigenSparseMatrix AtMA(const EigenSparseMatrix& A, const EigenSparseMatrix& M)
  {
    return A.transpose() * M * A;
  }

  Id MatrixSparse::forwardLU(
    const VectorDouble& b,
    VectorDouble& x,
    bool flagLower) const
  {
  ...
  void MatrixSparse::_prodGeneral(
    MatrixSparse& res,
    const MatrixSparse& other,
    double cst)
  {
  ...
  void MatrixSparse::_prodnormGeneral(
    MatrixSparse& res,
    const MatrixSparse& a,
  ...
  bool MatrixSparse::_areIdenticalGeneral(
    const MatrixSparse& a,
    ...
    bool verbose)
  {
    ...
    if (!flag && verbose) messerr("Matrices are not identical ");
    return flag;
  }
