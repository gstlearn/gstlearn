...
namespace gstlrn
{
  ...
  bool NeighMoving::_deserializeAscii(std::istream& is)
  {
    ...
    ret = true;
    ret = ret
       && _recordRead<Id>(is, "NeighMovingborhood sector search", flag_sector);
    ret = ret && _recordRead<Id>(is, "Minimum Number of samples", _nMini);
    ret = ret && _recordRead<Id>(is, "Maximum Number of samples", _nMaxi);
    ret = ret
       && _recordRead<Id>(is, "Optimum Number of samples per sector", _nSect);
    ret = ret
       && _recordRead<Id>(is, "Maximum Number of samples per sector", _nSMax);
    ...
    if (flag_aniso)
    {
      ...
      for (size_t idim = 0; ret && idim < ndim; idim++)
        ret = ret
           && _recordRead<double>(
                is, "Anisotropy Coefficient", nbgh_coeffs[idim]);
      ret =
        ret && _recordRead<Id>(is, "Anisotropy Rotation Flag", flag_rotation);
      if (flag_rotation)
      {
        ...
        for (size_t idim = 0; ret && idim < ndim; idim++)
          for (size_t jdim = 0; ret && jdim < ndim; jdim++, lec++)
            ret = ret
               && _recordRead<double>(
                    is, "Anisotropy Rotation Matrix", nbgh_rotmat[lec]);
      }
    }
    if (!ret) return ret;
    ...
    if (!nbgh_rotmat.empty()) _biPtDist->setAnisoRotMat(nbgh_rotmat);
  ...
  bool NeighMoving::_serializeAscii(std::ostream& os) const
  {
    ...
    ret = ret
       && _recordWrite<double>(
            os, "Maximum distance radius", _biPtDist->getRadius());
    ret =
      ret && _recordWrite<Id>(os, "Anisotropy Flag", _biPtDist->getFlagAniso());
    ...
    if (_biPtDist->getFlagAniso())
    {
      for (Id idim = 0; ret && idim < ndim; idim++)
        ret =
          ret && _recordWrite<double>(os, "", _biPtDist->getAnisoCoeff(idim));
      ret = ret && _commentWrite(os, "Anisotropy Coefficients");
      ret = ret
         && _recordWrite<Id>(
              os, "Anisotropy Rotation Flag", _biPtDist->getFlagRotation());
      if (_biPtDist->getFlagRotation())
      {
        ...
        for (Id idim = 0; ret && idim < ndim; idim++)
          for (Id jdim = 0; ret && jdim < ndim; jdim++)
            ret =
              ret
              && _recordWrite<double>(os, "", _biPtDist->getAnisoRotMat(ecr++));
  ...
  NeighMoving* NeighMoving::create(
    bool flag_xvalid,
    Id nmaxi,
    double radius,
    Id nmini,
    ...
    const ASpaceSharedPtr& space)
  {
    return new NeighMoving(
      flag_xvalid, nmaxi, radius, nmini, nsect, nsmax, coeffs, angles,
      useBallTree, leaf_size, space);
  }
  ...
  bool NeighMoving::_getAnisotropyElements(
    double* rx,
    double* ry,
    double* theta,
    double* cosp,
  ...
  VectorVectorDouble
    NeighMoving::getEllipsoid(const VectorDouble& target, Id count) const
  {
    double rx, ry, theta, cosp, sinp;
    if (!_getAnisotropyElements(&rx, &ry, &theta, &cosp, &sinp))
  ...
  VectorVectorDouble
    NeighMoving::getZoomLimits(const VectorDouble& target, double percent) const
  {
    double rx, ry, theta, cosp, sinp;
    if (!_getAnisotropyElements(&rx, &ry, &theta, &cosp, &sinp))
      return VectorVectorDouble();
  ...
  bool NeighMoving::deserializeH5(H5::Group& grp)
  {
    ...
    if (flag_aniso)
    {
      ...
      ret =
        ret && SerializeHDF5::readValue(*neighG, "RotationFlag", flag_rotation);
      if (flag_rotation)
        ret =
          ret && SerializeHDF5::readVec(*neighG, "AnisoRotmat", nbgh_rotmat);
    }
  ...
  bool NeighMoving::serializeH5(H5::Group& grp) const
  {
    ...
    ret = ret
       && SerializeHDF5::writeValue(
            neighG, "UseSectors", static_cast<Id>(getFlagSector()));
    ...
    // Store information from the Bipoint Checker based on distances
    ret = ret
       && SerializeHDF5::writeValue(neighG, "Radius", _biPtDist->getRadius());
    ret = ret
       && SerializeHDF5::writeValue(
            neighG, "AnisoFlag", _biPtDist->getFlagAniso());

    if (_biPtDist->getFlagAniso())
    {
      ret = ret
         && SerializeHDF5::writeVec(
              neighG, "AnisoCoeff", _biPtDist->getAnisoCoeffs());
      ret = ret
         && SerializeHDF5::writeValue(
              neighG, "RotationFlag", _biPtDist->getFlagRotation());
      if (_biPtDist->getFlagRotation())
        ret = ret
           && SerializeHDF5::writeVec(
                neighG, "AnisoRotmat", _biPtDist->getAnisoRotMats());
    }
    return ret;
  }
#endif
