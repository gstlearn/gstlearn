...
namespace gstlrn
{
  ...
  NeighMoving* NeighMoving::create(bool flag_xvalid,
                                   ...
                                   const ASpaceSharedPtr& space)
  {
    return new NeighMoving(flag_xvalid,
                           nmaxi,
                           radius,
                           nmini,
                           nsect,
  ...
  bool NeighMoving::_getAnisotropyElements(double* rx,
                                           double* ry,
                                           double* theta,
                                           double* cosp,
                                           double* sinp) const
  ...
  bool NeighMoving::serializeH5(H5::Group& grp) const
  {
    ...
    ret = ret
       && SerializeHDF5::writeValue(neighG,
                                    "UseSectors",
                                    static_cast<Id>(getFlagSector()));
    ...
    ret = ret
       && SerializeHDF5::writeValue(neighG,
                                    "AnisoFlag",
                                    _biPtDist->getFlagAniso());

    if (_biPtDist->getFlagAniso())
    {
      ret = ret
         && SerializeHDF5::writeVec(neighG,
                                    "AnisoCoeff",
                                    _biPtDist->getAnisoCoeffs());
      ret = ret
         && SerializeHDF5::writeValue(neighG,
                                      "RotationFlag",
                                      _biPtDist->getFlagRotation());
