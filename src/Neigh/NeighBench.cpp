...
namespace gstlrn
{
  ...
  bool NeighBench::_deserializeAscii(std::istream& is)
  {
    ...
    _biPtBench = BiTargetCheckBench::create(
      -1, width); // idim_bench will be updated in 'attach'
  ...
  bool NeighBench::_serializeAscii(std::ostream& os) const
  {
    ...
    ret =
      ret && _recordWrite<double>(os, "Bench Width", _biPtBench->getWidth());
    return ret;
  }

  NeighBench* NeighBench::create(
    bool flag_xvalid,
    double width,
    bool useBallTree,
    Id leaf_size,
  ...
  Id NeighBench::getNSampleMax(const Db* db) const
  {
    ...
    VectorDouble vec = db->getVecCoordinate(ndim - 1, useSel);
  ...
  bool NeighBench::_isSameTargetBench(Id iech_out) const
  {
    ...
    {
      Id nval = 1;
      for (Id idim = 0; idim < ndim - 1; idim++) nval *= _dbgrid->getNX(idim);
      if ((iech_out / nval) != (_iechMemo / nval)) return false;
    }
    else
    {
      if (_dbout->getCoordinate(iech_out, ndim - 1)
          != _dbout->getCoordinate(_iechMemo, ndim - 1))
        return false;
    }
    return true;
  }
  ...
  bool NeighBench::serializeH5(H5::Group& grp) const
  {
    ...
    ret =
      ret && SerializeHDF5::writeValue(neighG, "Bench", _biPtBench->getWidth());
