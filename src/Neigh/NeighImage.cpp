...
namespace gstlrn
{
  ...
  String NeighImage::toString(const AStringFormat* strfmt) const
  {
    ...
    sstr << toStrMatrix(
      "Image radius :", VectorString(), VectorString(), true, 1,
      static_cast<Id>(getNDim()), _imageRadius);
  ...
  bool NeighImage::_deserializeAscii(std::istream& is)
  {
    ...
    for (Id idim = 0; ret && idim < static_cast<Id>(getNDim()); idim++)
    {
      ...
      ret = ret
         && _recordRead<double>(
              is, "Image NeighImageborhood Radius", loc_radius);
  ...
  bool NeighImage::_serializeAscii(std::ostream& os) const
  {
    ...
    for (Id idim = 0; ret && idim < static_cast<Id>(getNDim()); idim++)
      ret = ret
         && _recordWrite<double>(
              os, "", static_cast<double>(getImageRadius(idim)));
    ret = ret && _commentWrite(os, "Image NeighImageborhood parameters");
    return ret;
  }

  NeighImage* NeighImage::create(
    const VectorInt& radius,
    Id skip,
    const ASpaceSharedPtr& space)
  {
