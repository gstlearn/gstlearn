...
namespace gstlrn
{
  NeighImage::NeighImage(const VectorInt& radius,
                         Id skip,
  ...
  String NeighImage::toString(const AStringFormat* strfmt) const
  {
    ...
    sstr << toStrMatrix("Image radius :",
                        VectorString(),
                        VectorString(),
                        true,
                        1,
                        static_cast<Id>(getNDim()),
                        _imageRadius);
    ...
    for (Id idim = 0; ret && idim < static_cast<Id>(getNDim()); idim++)
    {
      ...
      ret = ret
         && _recordRead<double>(is,
                                "Image NeighImageborhood Radius",
                                loc_radius);
    ...
    for (Id idim = 0; ret && idim < static_cast<Id>(getNDim()); idim++)
      ret = ret
         && _recordWrite<double>(os,
                                 "",
                                 static_cast<double>(getImageRadius(idim)));
