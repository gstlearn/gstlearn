...
namespace gstlrn
{
  PolyLine2D::PolyLine2D(const VectorDouble& x, const VectorDouble& y)
    : AStringable()
    , ASerializable()
    , _x(x)
    , _y(y)
  ...
  String PolyLine2D::toString(const AStringFormat* strfmt) const
  {
    ...
    if (sf.getLevel() > 2)
    {
      ...
      sstr << toStrMatrix(
        "Line Vertex Coordinates", VectorString(), VectorString(), true,
        npoints, 2, tab);
    }
    return sstr.str();
  }
  ...
  bool PolyLine2D::_serializeAscii(std::ostream& os) const
  {
    ...
    bool ret = true;
    ret = ret
       && _recordWrite<Id>(os, "Number of Points", static_cast<Id>(_x.size()));
  ...
  bool PolyLine2D::_deserializeAscii(std::istream& is)
  {
    ...
    {
      messerr(
        "Something wrong in your file: the Number of Points read is equal to "
        "%d",
        np);
      return false;
    }
  ...
  PolyPoint2D PolyLine2D::getPLIndex(const VectorDouble& xy0) const
  {
    ...
    {
      dist = GH::distancePointToSegment(
        xy0[0], xy0[1], getX(i), getY(i), getX(i + 1), getY(i + 1), &xx, &yy,
        &nint);
   ...
   **
   *****************************************************************************/
  double PolyLine2D::distanceAlongPolyline(
  ...
  double
    PolyLine2D::angleAtPolyline(const PolyPoint2D& pldist, Id nb_neigh) const
  {
    Id rfrom, rto;
    ...
    const NamingConvention& namconv)
  {
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      if (!flagMask || (pl_top.dist <= distinter && pl_bot.dist <= distinter))
      {
        ...
        angle =
          (angle_top * dist_bot + angle_bot * dist_top) / (dist_bot + dist_top);
      }
      else
      {
        distmin = TEST;
