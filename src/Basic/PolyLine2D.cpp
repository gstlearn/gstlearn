...
namespace gstlrn
{
  ...
  String PolyLine2D::toString(const AStringFormat* strfmt) const
  {
    ...
    if (sf.getLevel() > 2)
    {
      ...
      sstr << toStrMatrix("Line Vertex Coordinates",
                          VectorString(),
                          VectorString(),
                          true,
                          npoints,
  ...
  bool PolyLine2D::_deserializeAscii(std::istream& is)
  {
    ...
    if (np < 0)
    {
      messerr("Something wrong in your file: the Number of Points read is "
              "equal to %d",
              np);
      return false;
    }
  ...
  PolyPoint2D PolyLine2D::getPLIndex(const VectorDouble& xy0) const
  {
    ...
    {
      dist = GH::distancePointToSegment(xy0[0],
                                        xy0[1],
                                        getX(i),
                                        getY(i),
                                        getX(i + 1),
                    ...
                    const NamingConvention& namconv)
  {
    ...
    namconv
      .setNamesAndLocators(dbout, VectorString(), ELoc::Z, -1, dbout, iptr);
