...
namespace gstlrn
{
  ...
  void ParamInfo::setMinValue(double value)
  {
    ...
    else
    {
      ...
      messerr(
        "Minimum value %f is less than the minimum authorized value %f", value,
        _absoluteBounds[0]);
  ...
  void ParamInfo::setValue(double value)
  {
    ...
    if (value < _userBounds[0] || value > _userBounds[1])
    {
      ...
      if (_userBounds[0] == -INF) parenthesisleft = "(";
      if (_userBounds[1] == INF) parenthesisright = ")";
      messerr("Warning for parameter: %s", _name.c_str());
      messerr("Input value: %f", value);
      messerr(
        " is out of user bounds: %s %f, %f %s", parenthesisleft.c_str(),
        _userBounds[0], _userBounds[1], parenthesisright.c_str());
  ...
  void ParamInfo::setMaxValue(double value)
  {
    ...
    else
    {
      messerr("Warning for parameter: %s", _name.c_str());
      messerr(
        "Maximum value %f is greater than the maximum authorized value %f",
        value, _absoluteBounds[1]);
