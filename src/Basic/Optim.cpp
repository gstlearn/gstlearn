...
namespace gstlrn
{
  Optim::Optim(opt_algorithm algo, Id dim)
    : _opt(nlopt_create(
        static_cast<nlopt_algorithm>(algo),
  ...
  void Optim::setGradient(
    std::function<void(vect)> gradient,
  ...
  void Optim::setLowerBounds(
    const VectorDouble& lb,
    const std::vector<size_t>& dispatch)
  {
    if (dispatch.empty())
    {
  ...
  void Optim::setUpperBounds(
    const VectorDouble& ub,
    const std::vector<size_t>& dispatch)
  {
    if (dispatch.empty())
    {
  ...
  double Optim::minimize(VectorDouble& x)
  {
    ...
    {
      messerr(
        "Dimension mismatch: initial guess has size %d but optimizer expects "
        "%d\n",
        x.size(), dim);
    }
    ...
    for (size_t i = 0; i < dim; i++)
    {
      ...
      {
        messerr(
          "Initial guess x[%d]=%lf is out of bounds [%lf, %lf]\n", i, x[i],
          lb[i], ub[i]);
      }
    }
