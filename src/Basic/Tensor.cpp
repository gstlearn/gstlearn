...
namespace gstlrn
{
  ...
  String Tensor::toString(const AStringFormat* /*strfmt*/) const
  {
    ...
    if (!_rotation.isIdentity()) sstr << _rotation.toString() << std::endl;
    return sstr.str();
  }

  void Tensor::setRadiusIsotropic(double radius)
  {
    if (isZero(radius)) my_throw("Ellipsoid radius cannot be null");
  ...
  void Tensor::setRadiusVec(const VectorDouble& radius)
  {
    ...
    {
      if (ABS(r) < EPSILON20) my_throw("Radius cannot be null");
    }
    _radius = radius;
    _updateIsotropic();
    _fillTensors();
  ...
  {
    if (idim >= _nDim) my_throw("Wrong index of dimension");
    if (isZero(radius)) my_throw("Radius cannot be null");
    _radius[idim] = radius;
    _updateIsotropic();
    _fillTensors();
  }
  ...
  void Tensor::setRotationAngles(const VectorDouble& angles)
  {
    ...
    if (_nDim > 2 && angles.size() != _nDim)
      my_throw(
        "Dimension of argument 'angles' should match the Space Dimension");
  ...
  {
    if ((_nDim == 2 && idim != 0) || (_nDim > 2 && idim >= _nDim))
   ...
   */
  void Tensor::setRotationAnglesAndRadius(
  ...
  void Tensor::applyInverseInPlace(const VectorDouble& vec, VectorDouble& out)
    const
  {
  ...
  void Tensor::applyInverse2InPlace(const VectorDouble& vec, VectorDouble& out)
    const
  {
  ...
  void
    Tensor::applyDirectInPlace(const VectorDouble& vec, VectorDouble& out) const
  {
    AMatrix::productInPlace(out, _tensorDirect, vec);
  }

  void Tensor::applyDirectSwapInPlace(
