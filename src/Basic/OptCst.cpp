...
namespace gstlrn
{
  ...
  std::map<Id, double> OptCst::_cst = {
    {ECst::E_NTCAR, 10.}, {ECst::E_NTDEC, 3.},   {ECst::E_NTROW, 7.},
    {ECst::E_NTCOL, 7.},  {ECst::E_NTBATCH, 7.}, {ECst::E_NTNAME, 12.},
    {ECst::E_NTRANK, 3.}, {ECst::E_NPROC, 0.},   {ECst::E_LOCMOD, 1.}};
  ...
  double OptCst::queryByKey(const String& name)
  {
    for (auto e: _cst)
    {
      if (ECst::fromValue(e.first).getKey() == toUpper(name)) return e.second;
    }
    return TEST;
  }
  ...
  void OptCst::display()
  {
    ...
    for (auto e: _cst)
    {
      ...
      sstr << std::setw(50) << ec.getDescr() << " [" << std::setw(7)
           << ec.getKey() << "]" << " : " << e.second << std::endl;
    }
