...
namespace gstlrn
{
  ...
  bool SerializeNeutralFile::fileOpenWrite(
  ...
  bool SerializeNeutralFile::fileOpenRead(
    const ASerializable& parent,
    const String& filename,
    std::ifstream& is,
    bool verbose)
  {
    ...
    if (type != parent.getNFName())
    {
      if (verbose)
        messerr(
          "The file %s has the wrong type (read: %s, expected: %s)",
          filepath.c_str(), type.c_str(), parent.getNFName().c_str());
      is.close();
      return false;
    }
  ...
  bool
    SerializeNeutralFile::commentWrite(std::ostream& os, const String& comment)
  {
    if (os.good())
    {
      if (comment.empty())
  ...
  bool SerializeNeutralFile::tableWrite(
    std::ostream& os,
    const String& string,
    Id ntab,
  ...
  bool SerializeNeutralFile::tableRead(
    std::istream& is,
    const String& string,
    Id ntab,
    double* tab)
