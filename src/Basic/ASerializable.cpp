...
namespace gstlrn
{
                               ...
                               const EFormatNF& format) const
  {
    ...
    messerr("Aserializable::dumpToNF: No Format is defined (enum value: %d)",
   ...
   * @return
   */
  String ASerializable::buildFileName(Id status,
  ...
  String ASerializable::getFileIdentity(const String& filename, bool verbose)
  {
    ...
    if (ret_type < 0)
    {
      ...
      if (H5::H5File::isHdf5(filepath))
      {
        ...
        {
          messerr("File %s doesn't contain Gstlearn metadata…",
                  filepath.c_str());
          return String();
        }
        ret_type = 1;
