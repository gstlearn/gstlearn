...
namespace gstlrn
{
    ...
    const VectorBool& incmaxi)
  {
    ...
    {
      messerr(
        "Arguments 'mini' and 'maxi' should have the same dimension. Limits "
        "empty");
      return;
    }
    ...
    {
      messerr(
        "You must define at least one item in 'mini' and 'maxi'. Limits empty");
      return;
    }
    if (incmini.size() != 0 && static_cast<Id>(incmini.size()) != nclass)
    {
      messerr(
        "Arguments 'incmini' and 'mini' should have the same dimension. Limits "
        "empty");
      return;
    }
    ...
    {
      messerr(
        "Arguments 'incmaxi' and 'maxi' should have the same dimension. Limits "
        "empty");
      return;
    }
  ...
  String Limits::toString(const AStringFormat* /*strfmt*/) const
  {
    ...
    for (Id i = 0; i < static_cast<Id>(_bounds.size()); i++)
      sstr << "Bound( " << i + 1 << " ) : " << _bounds[i].toString()
           << std::endl;
  ...
  VectorDouble Limits::getBound(Id iclass, Id mode) const
  {
    ...
    if (mode == 0 || mode == 1) bounds.push_back(_bounds[iclass].getVmin());
    if (mode == 0 || mode == 2) bounds.push_back(_bounds[iclass].getVmax());
    return bounds;
  }
  ...
  VectorDouble Limits::getLowerBounds() const
  {
    ...
    for (Id i = 0; i < nclass; i++) lower[i] = _bounds[i].getVmin();
    return lower;
  }
  ...
  VectorDouble Limits::getUpperBounds() const
  {
    ...
    for (Id i = 0; i < nclass; i++) upper[i] = _bounds[i].getVmax();
    return upper;
  }
  ...
  VectorBool Limits::getLowerIncluded() const
  {
    ...
    for (Id i = 0; i < nclass; i++) mininc[i] = _bounds[i].getMinIncluded();
    return mininc;
  }
  ...
  VectorBool Limits::getUpperIncluded() const
  {
    ...
    for (Id i = 0; i < nclass; i++) maxinc[i] = _bounds[i].getMaxIncluded();
    return maxinc;
  }
  ...
  Id Limits::toCategoryByAttribute(
    Db* db,
    Id iatt,
    const NamingConvention& namconv) const
  {
    return _computeCategory(
      db, iatt, getLowerBounds(), getUpperBounds(), getLowerIncluded(),
      getUpperIncluded(), namconv);
  }

  Id Limits::toCategory(
    Db* db,
    const String& name,
    const NamingConvention& namconv) const
  {
    ...
    const NamingConvention& namconv) const
  {
    Id iatt = db->getUID(name);
    if (iatt < 0) return 1;
    return toIndicatorByAttribute(
      db, iatt, OptionIndicator, flagBelow, flagAbove, namconv);
  }

  Id Limits::toIndicatorByAttribute(
    Db* db,
    Id iatt,
    Id OptionIndicator,
    bool flagBelow,
    ...
    const NamingConvention& namconv) const
  {
    return _computeIndicator(
      db, iatt, OptionIndicator, getLowerBounds(), getUpperBounds(),
      getLowerIncluded(), getUpperIncluded(), flagBelow, flagAbove, namconv);
  }
    ...
    bool flagAbove) const
  {
    Id iatt = db->getUID(name);
    if (iatt < 0) return 1;
    return _computeLimitStatistics(
      db, iatt, getLowerBounds(), getUpperBounds(), getLowerIncluded(),
      getUpperIncluded(), optionStat, flagBelow, flagAbove);
  }
    ...
    const NamingConvention& namconv)
  {
    ...
    for (Id iclass = 0; iclass < nclass; iclass++)
    {
      ...
      flagmin[iclass] =
        (incmini.empty()) ? 1 : static_cast<Id>(incmini[iclass]);
      flagmax[iclass] =
        (incmaxi.empty()) ? 0 : static_cast<Id>(incmaxi[iclass]);
    }
    Id nbelow = 0;
    Id nabove = 0;
    double mbelow = 0.;
    ...
    for (Id iech = 0; iech < db->getNSample(); iech++)
    {
      ...
      for (Id iclass = 0; iclass < nclass; iclass++)
      {
        ...
        if (flag_indic) db->setArray(iech, iptr_indic + iclass, belong);
      ...
      if (flag_indic)
      {
        if (flagBelow) db->setArray(iech, iptr_below, value < zmini);

        if (flagAbove) db->setArray(iech, iptr_above, value > zmaxi);
      }
   ...
   **
   *****************************************************************************/
  VectorDouble Limits::_computeLimitStatistics(
    Db* db,
    Id iatt,
    const VectorDouble& mini,
    const VectorDouble& maxi,
    ...
    bool flagAbove)
  {
    ...
    if (optionStat == 1)
    {
      if (flagBelow)
        stats.push_back(
          static_cast<double>(nbelow) / static_cast<double>(nactive));
      for (Id iclass = 0; iclass < nclass; iclass++)
        stats.push_back(
          static_cast<double>(count[iclass]) / static_cast<double>(nactive));
      if (flagAbove)
        stats.push_back(
          static_cast<double>(nabove) / static_cast<double>(nactive));
    }
    else
    {
   ...
   **
   *****************************************************************************/
  Id Limits::_check_bound_consistency(
    const VectorDouble& mini,
    const VectorDouble& maxi,
    const VectorBool& incmini,
    const VectorBool& incmaxi,
    Id* nclass_arg)
  {
    ...
    if (!mini.empty())
    {
      ...
      {
        messerr(
          "Wrong dimension of 'mini'(%d). It should be %d",
    ...
    if (!maxi.empty())
    {
      ...
      {
        messerr(
          "Wrong dimension of 'maxi'(%d). It should be %d",
    ...
    if (!incmini.empty())
    {
      ...
      {
        messerr(
          "Wrong dimension of 'incmini'(%d). It should be %d", incmini.size(),
          nclass);
        return 1;
      }
    ...
    if (!incmaxi.empty())
    {
      ...
      {
        messerr(
          "Wrong dimension of 'incmaxi'(%d). It should be %d", incmaxi.size(),
          nclass);
        return 1;
      }
