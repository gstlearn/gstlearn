...
namespace gstlrn
{
                 ...
                 const VectorBool& incmaxi)
  {
    ...
    {
      messerr("Arguments 'mini' and 'maxi' should have the same dimension. "
              "Limits empty");
      return;
    }
    ...
    {
      messerr("Arguments 'incmini' and 'mini' should have the same dimension. "
              "Limits empty");
      return;
    }
    if (incmaxi.size() != 0 && static_cast<Id>(incmaxi.size()) != nclass)
    {
      messerr("Arguments 'incmaxi' and 'maxi' should have the same dimension. "
              "Limits empty");
      return;
    }
                         ...
                         const NamingConvention& namconv) const
  {
    Id iatt = db->getUID(name);
    if (iatt < 0) return 1;
    return toIndicatorByAttribute(db,
                                  iatt,
                                  OptionIndicator,
                                  flagBelow,
                                  flagAbove,
                                  ...
                                  bool flagAbove) const
  {
    Id iatt = db->getUID(name);
    if (iatt < 0) return 1;
    return _computeLimitStatistics(db,
                                   iatt,
                                   getLowerBounds(),
                                   getUpperBounds(),
                                   getLowerIncluded(),
                                               ...
                                               bool flagAbove)
  {
    ...
    {
      if (flagBelow)
        stats.push_back(static_cast<double>(nbelow)
                        / static_cast<double>(nactive));
      for (Id iclass = 0; iclass < nclass; iclass++)
        stats.push_back(static_cast<double>(count[iclass])
                        / static_cast<double>(nactive));
      if (flagAbove)
        stats.push_back(static_cast<double>(nabove)
                        / static_cast<double>(nactive));
    }
    else
    {
