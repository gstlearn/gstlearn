...
namespace gstlrn
{
  ...
  NamingConvention* NamingConvention::create(const String& prefix,
                                             ...
                                             bool cleanSameLocator)
  {
    return new NamingConvention(prefix,
                                flag_varname,
                                flag_qualifier,
                                flag_locator,
                                locatorOutType,
    ...
    Id locatorShift) const
  {
    ...
    if (flagSetLocator)
    {
      if (_flagLocator && _locatorOutType != ELoc::UNDEFINED)
      {
        ...
        for (Id i = 0; i < ntotal; i++)
          dbout->setLocatorByUID(iattout_start + i,
                                 _locatorOutType,
                                 i + locatorShift);
      }
    }
                                     ...
                                     Id locatorShift) const
  {
    ...
    for (Id ecr = 0; ecr < nvar * nitems; ecr++)
      dbout->setLocatorByUID(iattout_start + ecr,
                             _locatorOutType,
                             ecr + locatorShift);
  }
                                              ...
                                              Id nitems) const
  {
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
    {
      ...
      for (Id item = 0; item < nitems; item++)
      {
        ...
        String name = concatenateStrings(_delim,
                                         _prefix,
                                         loc_varname,
                                         loc_qualifier,
                                         loc_number);
   ...
   */
  VectorString
    NamingConvention::_createSimulationNames(const VectorString& names,
                                             Id nvar,
                                             Id nbsimu,
