...
namespace gstlrn
{
  ...
  void VectorHelper::dumpStats(const String& title,
  ...
  void VectorHelper::dumpRange(const String& title,
                               const VectorDouble& vectin,
                               Id nmax)
  {
    constvect vec(vectin);
  ...
  void VectorHelper::dumpNNZ(const String& title,
    ...
    VectorHelper::normalScore(const VectorDouble& data, const VectorDouble& wt)
  {
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      if (!FFFF(data[iech]))
      {
        ...
        if (wtloc < 0.)
        {
          messerr("The weight of sample (%d) is negative (%lf)",
                  iech + 1,
                  wtloc);
          return VectorDouble();
        }
  ...
  bool VectorHelper::isEqual(const VectorDouble& v1,
  ...
  VectorDouble VectorHelper::simulateBernoulli(Id n,
                                               double proba,
                                               double vone,
                                               double velse)
  {
  ...
  void VectorHelper::concatenateInPlace(VectorDouble& veca,
                                        const VectorDouble& vecb)
  {
                              ...
                              double addval)
  {
    ...
    {
      messerr("Arguments 'veca' and 'vecb' should have the same dimension. "
              "Nothing is done");
      return;
    }
                                        ...
                                        Id nmax)
  {
    ...
    if (nmax > 0 && ninvalid > nmax)
      message("Found %d (out of %d) samples. Print limited to the %d most "
              "important ones.\n",
              ninvalid,
              nsize,
              nmax);
   ...
   */
  VectorInt VectorHelper::sampleRanks(Id ntotal,
                                      double proportion,
                                      Id number,
                                      Id seed,
                                      Id optSort)
  ...
  void VectorHelper::extractInPlace(const VectorDouble& vecin,
                                    ...
                                    Id start)
  {
    std::copy(vecin.begin() + start,
              vecin.begin() + start + vecout.size(),
              vecout.begin());
  }
    ...
    VectorHelper::sample(const VectorDouble& vecin, const VectorInt& indKeep)
  {
    ...
    {
      if (!checkArg("Selected index",
                    indices[i],
                    static_cast<Id>(vecin.size())))
        return vecout;
    }
  ...
  void VectorHelper::capInPlaceVVD(VectorVectorDouble& vec,
                                   double vmin,
                                   double vmax)
  {
    for (Id i = 0, n = static_cast<Id>(vec.size()); i < n; i++)
