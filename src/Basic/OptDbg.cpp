...
namespace gstlrn
{
  ...
  bool OptDbg::query(const EDbg& option, bool discardForce)
  {
    ...
    return std::any_of(
      _dbg.cbegin(), _dbg.cend(),
      [&option](const auto& e) { return e == option; });
#else
    return std::ranges::any_of(
      _dbg, [&option](const auto& e) { return e == option; });
#endif
  }
  ...
  void OptDbg::display()
  {
    ...
    {
      EDbg e = *it;
      sstr << std::setw(30) << e.getDescr() << "[ " << std::setw(9)
           << e.getKey() << "]" << " : " << query(e, true) << std::endl;
      it.toNext();
    }

    if (_reference >= 0)
      sstr << "Index of the reference target under DEBUG = " << _reference
           << std::endl;

    sstr << "Use 'OptDbg::define' to modify the previous values" << std::endl;
    sstr << "Use 'OptDbg::setReference' to define the target index where all "
            "flags are turned ON"
         << std::endl;
