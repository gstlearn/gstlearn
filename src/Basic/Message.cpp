...
namespace gstlrn
{
  ...
  bool checkArg(const char* title, Id current, Id nmax)
  {
    if (nmax >= 0)
    {
      ...
      {
        messerr(
          "Error in %s. The index (%d) should be non-negative and lower than "
          "%d",
          title, current, nmax);
        return false;
      }
    ...
    else
    {
      ...
      {
        messerr(
          "Error in %s. The index (%d) should be non-negative", title, current);
        return false;
      }
    }
    return true;
  ...
  void messerrNotImplemented(const String& className, const String& methodName)
  {
    if (!className.empty())
      messerr(
        "Function '%s' not implemented for class '%s'", methodName.c_str(),
        className.c_str());
    else
  ...
  void mes_process(const char* string, Id ntot, Id iech)
  {
    ...
    double ratio =
      nproc * static_cast<double>(jech) / static_cast<double>(ntot);
    ...
    bool flagScientific)
  {
    if (!title.empty()) message("%s", title.c_str());
    String string =
      toStr(value, justification, 0, roundZero, ncol, flagScientific);
   ...
   **
   *****************************************************************************/
  void printElement(Id value, const String& title, Id ncol, Id justification)
  {
    if (!title.empty()) message("%s", title.c_str());
  ...
  void printMatrix(
    ...
    Id bycol)
  {
    String string = toStrMatrix(
      title, VectorString(), VectorString(), bycol, ny, nx, tab,
      flag_limit == 0, false);
    message(string.c_str());
  }
  ...
  void printMatrix(const AMatrix& mat, const String& title, Id flag_limit)
  {
    printMatrix(
      mat.getValues(), mat.getNCols(), mat.getNRows(), title, flag_limit, true);
  }
  ...
  {
    String string =
      toStrVector(title, tab, flagIgnoreMaxNCols, newLineAfterTitle);
    message(string.c_str());
  }
