...
namespace gstlrn
{
  ...
  bool checkArg(const char* title, Id current, Id nmax)
  {
    if (nmax >= 0)
    {
      ...
      {
        messerr("Error in %s. The index (%d) should be non-negative and lower "
                "than %d",
                title,
                current,
                nmax);
  ...
  {
    if (!className.empty())
      messerr("Function '%s' not implemented for class '%s'",
              methodName.c_str(),
              className.c_str());
    else
  ...
  void printMatrix(const VectorDouble& tab,
                   ...
                   Id bycol)
  {
    String string = toStrMatrix(title,
                                VectorString(),
                                VectorString(),
                                bycol,
                                ny,
