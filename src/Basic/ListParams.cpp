...
namespace gstlrn
{
  ...
  struct DSU
  {
    ...
    Id find(Id x) { return parent[x] == x ? x : parent[x] = find(parent[x]); }
  ...
  String ListParams::toString(const AStringFormat* strfmt) const
  {
    ...
    for (Id ipar = 0, jpar = 0, npar = static_cast<Id>(_dispatchIndex.size());
         ipar < npar; ipar++)
    {
      jpar++;
      result << jpar << " - " << _params[_dispatchIndex[ipar]].get().toString()
             << std::endl;
    }
    return result.str();
  }
