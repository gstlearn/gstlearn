...
#define CONSTS(iconst, ivar1) (consts[(iconst) * nvar1 + (ivar1)])
  ...
  void law_set_random_seed(Id seed)
  {
    ...
    {
      Random_value = seed;
      if (!Random_Old_Style) Random_gen.seed(static_cast<unsigned>(seed));
    }
  }
...

  {
    ...
    if (Random_Old_Style)
    {
      ...
      value = static_cast<double>(Random_value)
            / static_cast<double>(Random_congruent);
      ...
      if (alpha > 1.)
      {
        ...
        do
        {
          ...
          value = c1 + t;
                 ...
                 || law_uniform(0., 1.)
                      > exp(c1 * log(value / c1) - t + log(1 + t * t / c2)));
        return (value / rate);
      }
  ...
  {
    double res = 0.0;
    res = -rate * value + alpha * log(rate) + (alpha - 1) * log(value)
        - loggamma(alpha);
    if (!isLog)
    {
      res = exp(res);
  ...
  double law_df_IGamma(double value, double alpha, double rate, bool isLog)
  {
    double res = 0.0;
    res =
      -rate / value + alpha * log(rate / value) - log(value) - loggamma(alpha);
    if (!isLog)
    {
      res = exp(res);
    }
  ...
  {
    static double b[] = {0.319381530, -0.356563782, 1.781477937, -1.821255978,
                         1.330274429};
  ...
  label_norme:
    ...
    for (k = 0; k < n; k++)
    {
      ...
      if (ABS(aa - bb) > 0.) switch (type)
        {
          case 1: wgt = (exp(-aa * aa / 2.) - exp(-bb * bb / 2.)) / bb; break;
          case 2: wgt = sqe * (exp(bb) - exp(aa)); break;
          case 3: wgt = sqe * (exp(-aa) - exp(-bb)); break;
          case 4: wgt = (exp(-aa * aa / 2.) - exp(-bb * bb / 2.)) / aa; break;
        }
      total += wgt;
      ptab[k] = total;
    }
    ...
    while (ok)
    {
      ...
      switch (type)
      {
        ...
        case 2: x = log(exp(aa) * (1. - u) + exp(bb) * u); break;

        case 3: x = -log(exp(-aa) * (1. - u) + exp(-bb) * u); break;
    ...
    MatrixSymmetric& correl)
  {
    ...
    xc[0] = vec[0] - mean[0];
    xc[1] = vec[1] - mean[1];
    double detv = correl.getValue(0, 0) * correl.getValue(1, 1)
                - correl.getValue(0, 1) * correl.getValue(1, 0);
    double det2 =
      (correl.getValue(1, 1) * xc[0] * xc[0]
       - 2. * correl.getValue(0, 1) * xc[0] * xc[1]
       + correl.getValue(0, 0) * xc[1] * xc[1]);
  ...
  double law_df_quadgaussian(VectorDouble& vec, MatrixSymmetric& correl)
  {
    ...
    for (Id ivar = 0; ivar < nvar; ivar++) density -= 0.5 * log(eigval[ivar]);
...

  {
    ...
    for (Id i = 0; i < nvar; i++) density -= 0.5 * log(eigval[i]);
  ...
  VectorDouble law_multigaussian(const VectorDouble& mu, MatrixSymmetric& sigma)
  {
    ...
    for (Id ii = 0; ii < size; ii++) nn[ii] = law_gaussian();
    MatrixDense::productInPlace(
      res, lu, nn); // TODO check if cholesky can t be used instead

    for (Id ii = 0; ii < size; ii++) res[ii] += mu[ii];
      ...
      while (1)
      {
        ...
        if (k > 20 && k < 0.5 * n * p * q - 1.0)
        {
          ...
          if (A > xm * log(f1 / x1) + (n - m + 0.5) * log(z / w)
                    + (y - m) * log(w * p / (x1 * q))
                    + (13860.
                       - (462. - (132. - (99. - 140. / f2) / f2) / f2) / f2)
                        / f1 / 166320.
                    + (13860.
                       - (462. - (132. - (99. - 140. / z2) / z2) / z2) / z2)
                        / z / 166320.
                    + (13860.
                       - (462. - (132. - (99. - 140. / x2) / x2) / x2) / x2)
                        / x1 / 166320.
                    + (13860.
                       - (462. - (132. - (99. - 140. / w2) / w2) / w2) / w2)
                        / w / 166320.)
            continue;
          return y;
        }
    ...
    double percent)
  {
    ...
    for (Id ivar = 0; ivar < nvarin; ivar++)
    {
      ...
      stdv[ivar] =
        stdv[ivar] / static_cast<double>(nechin) - mean[ivar] * mean[ivar];
    ...
    for (Id iechout = 0; iechout < nechout; iechout++)
    {
      ...
      if (flag_cont)
      {
        ...
        {
          messerr(
            " %d - Mini=%lf - Maxi=%lf - Mean=%lf - Stdv=%lf", ivar + 1,
  ...
  Id sampleInteger(Id mini, Id maxi)
  {
    ...
    Id retval = (rand > 0) ? static_cast<Id>(trunc(rand + 0.5))
                           : static_cast<Id>(-trunc(-rand + 0.5));
    return retval;
  }
