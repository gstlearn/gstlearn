...

{
  ...
  if (fabs((double) parameter-1.) < 0.00001)
    ...
    if (parameter > 1.)
    {
      ...
      while ( res<0 || law_uniform(0.,1.) >
              exp(c1*log(res/c1) - t + log(1+t*t/c2)) );
      return (res);
    }
    else
    {
                                       ...
                                       double *corr)
{
  ...
  for (i=0; i<nvar; i++) density -= 0.5 * log(eigval[i]);
    ...
    if (flag_cont)
    {
      ...
      for (int ivar=0; ivar<nvar; ivar++)
      {
        messerr(" %d - Mini=%lf - Maxi=%lf - Mean=%lf - Stdv=%lf",
                ivar+1,mini[ivar],maxi[ivar],mean[ivar],stdv[ivar]);
      }
