...
namespace gstlrn
{
    ...
    Id optionVerbose)
  {
    ...
    {
      messerr(
        "The second dimension of 'ranks' (%d)",
    ...
    {
      messerr(
        "The number of columns in the weight matrix (%d)", wgt.getNCols());
    ...
    if (optionVerbose)
    {
      ...
      if (optionVerbose == 1)
      {
        ...
        for (Id ivar = 0, irow = 0; ivar < nvar; ivar++)
        {
          ...
          {
            VectorDouble vec = wgt.getColumnByRowRange(
              ivar, nbneigh * jvar, nbneigh * (jvar + 1));
            table.setRowName(
    ...
    const VectorDouble& means)
  {
    ...
    for (Id iv2 = 0; iv2 < nv2; iv2++)
      if (fftn(
            ndim, dims.data(), kernelRe[iv2].data(), kernelIm[iv2].data(), 1))
        return 1;
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
    {
      ...
      if (fftn(
            ndim, dims.data(), imageRe[ivar].data(), imageIm[ivar].data(), 1))
        return 1;
    }
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
    {
      ...
      for (Id jvar = 0; jvar < nvar; jvar++, iv2++)
      {
        ...
        VH::multiplyComplexInPlace(
          kernelRe[iv2], kernelIm[iv2], imageRe[jvar], imageIm[jvar], localRe,
          localIm);
