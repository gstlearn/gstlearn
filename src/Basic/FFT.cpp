...
namespace gstlrn
{
   ...
   */
  Array evalCovFFTTimeSlice(
    const VectorDouble& hmax,
    double time,
    Id N,
    const std::function<std::complex<double>(VectorDouble, double)>&
      funcSpectrum)
  {
  ...
  static Id
    _getIndex(Id ndim, const VectorInt& strides, const VectorInt& indices)
  {
    Id index = 0;
    for (Id idim = 0; idim < ndim; idim++)
