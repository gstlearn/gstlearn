...
#undef TL2
                                 ...
                                 const VectorString& oldList)
  {
    ...
    catList.insert(catList.end(),
                   newList.begin(),
                   newList.end()); // append contents of 'b'
    ...
    decodeGridSorting(const String& string, const VectorInt& nx, bool verbose)
  {
    ...
    while (ind < length)
    {
      ...
      if (orient != 0)
      {
        ...
        if (num > ndim)
        {
          messerr("'order' refers to 'x%d' while space dimension is %d",
                  num,
                  ndim);
          return VectorInt();
        }
