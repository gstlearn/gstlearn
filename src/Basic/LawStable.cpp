...
LawStable& LawStable::operator=(const LawStable& r)
{
  DECLARE_UNUSED(r)
  return *this;
}
...
double
LawStable::_sinc(const double& u)
{
  if (std::abs(u) > 0)
    return std::sin(u) / u;
  return 1.0;
...
double
LawStable::_A(const double& u, const double& alpha)
{
  double beta = 1 - alpha;
  double val  = 0.0;
  if (u == 0.0)
...
double
LawStable::_BB0(const double& u, const double& alpha)
{
  double beta = 1 - alpha;
  double val  = 0.0;
  if (u != 0.0)
...
double
LawStable::law_stable_unilateral(const double alpha)
{
  double val = 0.0;
  if (alpha == 1.0)
  {
...
double
LawStable::law_stable_bilateral(const double alpha)
{
  double val = 0.0;
  if (alpha == 1.0)
  {
  ...
  {
    double U    = GV_PI * law_uniform(-0.5, 0.5); // uniform distribution on [-pi/2, pi/2]
...
double
LawStable::law_stable_unilateral_exptilt(const double alpha, const double lambda, bool doubleRejection)
{
  double val = 0.0;
  if (doubleRejection)
  {
...
double
LawStable::_unilateral_exptilt_singleRejection(const double alpha, const double lambda)
{
  double val = 0.0;
  Id nbr    = 0.0;
  do
...
double
LawStable::_unilateral_exptilt_doubleRejection(const double alpha, const double lambda)
{
  /*
  ** ------------------------------------------------------------
  ** set-up
  ...
  */
  ...
  double psi   = std::exp(-0.125 * gamma * GV_PI * GV_PI) * (2.0 + sqrt(GV_PI / 2.0)) * std::sqrt(gamma * GV_PI) / GV_PI;
  ...
  */
  ...
  do
  {
    ...
    if (U < GV_PI)
    {
      ...
      double g1   = std::exp(std::pow(lambda, alpha) * (1 - 1 / std::pow(zeta, 2)));
