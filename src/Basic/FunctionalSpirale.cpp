...
namespace gstlrn
{
   ...
   */
  MatrixSquare
    FunctionalSpirale::getFunctionMatrix(const VectorDouble& coor) const
  {
    Id ndim = 2;
  ...
  VectorVectorDouble
    FunctionalSpirale::getFunctionVectors(const Db* db,
                                          const CovAniso* cova) const
  {
    if (db == nullptr) return VectorVectorDouble();
    if (getNdim() != db->getNDim())
    {
      messerr("You cannot evaluate the function on input Db: they do not have "
