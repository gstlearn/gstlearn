...
namespace gstlrn
{
  ...
  Id Rotation::setMatrixDirectVec(const VectorDouble& rotmat)
  {
    if (!rotmat.empty())
    {
      ...
      _rotMat = local;
      GH::rotationGetAnglesInPlace(static_cast<Id>(_nDim),
                                   rotmat.data(),
                                   _angles.data());
      _directToInverse();
      _checkRotForIdentity();
  ...
  String Rotation::toString(const AStringFormat* strfmt) const
  {
    ...
    if (sf.getLevel() > 0)
    {
      sstr << toStrMatrix("Direct Rotation Matrix",
                          VectorString(),
                          VectorString(),
                          true,
                          static_cast<Id>(_nDim),
                          static_cast<Id>(_nDim),
                          _rotMat.getValues());
      sstr << toStrMatrix("Inverse Rotation Matrix",
                          VectorString(),
                          VectorString(),
                          true,
                          static_cast<Id>(_nDim),
  ...
  bool Rotation::isMatrixRotation(const MatrixSquare& rotmat, bool verbose)
  {
    ...
    for (Id i = 0; i < neq; i++)
      for (Id j = 0; j < neq; j++)
      {
        ...
        {
          if (verbose)
            messerr("The element (A*At)[%d,%d] = %lf (should be %lf)",
                    i + 1,
                    j + 1,
                    prod,
                    comp);
