...
namespace gstlrn
{
  ...
  Id Rotation::setMatrixDirect(const MatrixSquare& rotmat)
  {
    if (!rotmat.empty())
    {
      if (!_rotMat.isSameSize(rotmat))
        my_throw(
          "The argument 'rotmat' does not have same dimension as 'this'");
  ...
  Id Rotation::setMatrixDirectVec(const VectorDouble& rotmat)
  {
    if (!rotmat.empty())
    {
      ...
      _rotMat = local;
      GH::rotationGetAnglesInPlace(
        static_cast<Id>(_nDim), rotmat.data(), _angles.data());
      _directToInverse();
      _checkRotForIdentity();
    }
    return 0;
  ...
  std::vector<MatrixSquare> Rotation::getDerivatives() const
  {
    ...
    {
      res.resize(3);
      for (Id i = 0; i < 3; i++) res[i].reset(3, 3);
    }
  ...
  String Rotation::toString(const AStringFormat* strfmt) const
  {
    ...
    {
      sstr << toStrMatrix(
        "Direct Rotation Matrix", VectorString(), VectorString(), true,
        static_cast<Id>(_nDim), static_cast<Id>(_nDim), _rotMat.getValues());
      sstr << toStrMatrix(
        "Inverse Rotation Matrix", VectorString(), VectorString(), true,
        static_cast<Id>(_nDim), static_cast<Id>(_nDim), _rotInv.getValues());
    }
    return sstr.str();
  }
  ...
  void
    Rotation::rotateInverse(const VectorDouble& inv, VectorDouble& outv) const
  {
    if (!_flagRot)
      outv = inv;
    else
    ...
    for (Id i = 0; i < neq; i++)
      for (Id j = 0; j < neq; j++)
      {
        ...
        if (ABS(prod - comp) > EPSILON6)
        {
          if (verbose)
            messerr(
              "The element (A*At)[%d,%d] = %lf (should be %lf)", i + 1, j + 1,
              prod, comp);
          return false;
        }
      }
