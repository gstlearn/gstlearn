...
namespace gstlrn
{
  ...
  std::vector<Plane> Plane::poissonPlanesGenerate(DbGrid* dbgrid, Id np)
  {
    ...
    for (Id ip = 0; ip < np; ip++)
    {
      ...
      if (d0 < 0)
      {
        for (Id idim = 0; idim < 3; idim++) ap[idim] = -ap[idim];
        d0 = -d0;
      }

      /* Storing the plane */

      for (Id idim = 0; idim < 3; idim++) planes[ip].setCoor(idim, ap[idim]);
  ...
  {
    if (idim < static_cast<Id>(_coor.size())) return _coor[idim];
    return 0.;
  }

  void Plane::setCoor(Id idim, double value)
  {
    if (idim < static_cast<Id>(_coor.size())) _coor[idim] = value;
  }
} // namespace gstlrn
