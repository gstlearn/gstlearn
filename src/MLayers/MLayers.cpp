...
namespace gstlrn
{
  ...
  String MLayers::toString(const AStringFormat* strfmt) const
  {
    ...
    if (_flagZ) sstr << "Results are converted into Depth" << std::endl;

    sstr << "Do the Input and Output Db coincide: " << NOK[_flagSame]
         << std::endl;
    sstr << "Using External Drift functions: " << NOK[_flagExt] << std::endl;
    sstr << "Is Time used as External Drift: %s" << NOK[_matchTime]
         << std::endl;
    if (_colrefD >= 0)
    ...
    sstr << "Number of active samples (including collocated duplicates) = "
         << _nech << std::endl;
  ...
  Id MLayers::_getNDrift() const
  {
    switch (_irfRank)
    {
      ...
      default: messageAbort("_irfRank must be -1, 0 or 1"); break;
    }
    return (0);
  }

  void MLayers::setGeneral(
    Db* dbin,
    DbGrid* dbout,
    Model* model,
    bool flag_std,
   ...
   **
   *****************************************************************************/
  Id MLayers::_getPropsResult(Id iech, Id ilayer0, VectorDouble& props) const
  {
    ...
    else
    {
      ...
      for (Id ilayer = 0; ilayer < ilayer0; ilayer++) props[ilayer] = 1.;
    }
    return (0);
  }
   ...
   **
   *****************************************************************************/
  Id MLayers::_getPropsData(Id iech, Id ilayer0, VectorDouble& props) const
  {
    ...
    VectorDouble& c00)
  {
    ...
    {
      for (Id k = 0; k < _nlayers; k++) c00[k] = covtab.getValue(k, k);
    }
  }
    ...
    VectorDouble& b)
  {
    ...
    for (Id jech = 0; jech < _dbin->getNSample(); jech++)
    {
      ...
      {
        Id jlayer = (jfois == 0)
                    ? static_cast<Id>(_dbin->getFromLocator(ELoc::LAYER, jech))
                    : _nlayers;
    ...
    MatrixSquare& acov)
  {
    ...
    for (Id iech = 0, iiech = 0; iech < _dbin->getNSample(); iech++)
    {
      ...
      {
        Id ilayer = (ifois == 0)
                    ? static_cast<Id>(_dbin->getFromLocator(ELoc::LAYER, iech))
                    : _nlayers;
        ...
        if (_computeLhsOne(seltab, iech, ilayer, coor, prop1, prop2, covtab, b))
          return (1);

        for (Id i = 0; i < _neq; i++) atot.setValue(iiech, i, b[i]);
      }
    }
  ...
  void MLayers::_getDataVector(VectorInt& seltab, VectorDouble& zval)
  {
    ...
    for (Id iech = 0, iiech = 0; iech < _dbin->getNSample(); iech++)
    {
      ...
      for (Id ifois = 0; ifois < seltab[iech]; ifois++, iiech++)
      {
        ...
        if (_colrefD >= 0) value -= _dbout->getArray(igrid, _colrefD);
        ...
        if (_flagVel)
        {
          ...
          if (_colrefT >= 0) dtime -= _dbout->getArray(igrid, _colrefT);
          value /= dtime;
        }
        zval[iiech] = value;
      }
   ...
   **
   *****************************************************************************/
  Id MLayers::_subtractOptimalDrift(
    bool verbose,
    VectorInt& seltab,
    VectorDouble& zval)
  {
      ...
      for (Id ifois = 0; ifois < seltab[iech]; ifois++, iiech++)
      {
        ...
        for (Id k1 = 0; k1 < ipos; k1++)
        {
          ...
          for (Id k2 = 0; k2 < ipos; k2++)
            atab.setValue(
              k1, k2, atab.getValue(k1, k2) + drift[k1] * drift[k2]);
        }
      }
    }
    ...
    atab.invert();
    AMatrix::productInPlace(coeff, atab, btab);

    /* Optional printout of the result */

    if (verbose) printMatrix(coeff, _nlayers, _nbfl, "Estimated Drift", 0, 1);
    ...
    for (Id iech = 0, iiech = 0; iech < _dbin->getNSample(); iech++)
    {
      ...
      for (Id ifois = 0; ifois < seltab[iech]; ifois++, iiech++)
      {
        ...
        if (flag_subtract)
          for (Id k1 = 0; k1 < ipos; k1++) zval[iiech] -= coeff[k1] * drift[k1];

        /* Print the residuals (optional) */

        if (OptDbg::query(EDbg::VARIOGRAM))
          message(
            "Sample %d (Layer %d) - Coor = %lf %lf - Residual = %lf\n",
   ...
   **
   *****************************************************************************/
  Id MLayers::_prepareCollocation(
    Id iechout,
    VectorDouble& coor,
    VectorInt& seltab,
    MatrixSquare* a,
    ...
    double* ratio)
  {
    ...
    if (_colrefD >= 0) botval -= _dbout->getArray(iechout, _colrefD);

    if (_getPropsResult(iechout, _nlayers, prop1)) return 1;
    double c0 =
      _getCIJ(_nlayers, prop1, _nlayers, prop1, VectorDouble(), covtab);
    if (FFFF(c0)) return 1;

    if (_computeLhsOne(
          seltab, iechout, _nlayers, coor, prop1, prop2, covtab, baux))
      return 1;
    AMatrix::productInPlace(b2, *a, baux);
    ...
    double& stdev) const
  {
    ...
    if (_flagStd)
    {
      ...
      {
        AMatrix::productInPlace(wgt, *a, b);
    ...
    double& stdev) const
  {
    ...
    AMatrix::productInPlace(fsf0, a0, ff0);
    for (Id iech = 0; iech < _nech; iech++) c2[iech] = b[iech] + fsf0[iech];
    AMatrix::productInPlace(wgt, cc, c2);
    ...
    if (_flagStd)
    {
      AMatrix::productInPlace(temp, b, ss);
      for (Id ipar = 0; ipar < _npar; ipar++) temp[ipar] -= ff0[ipar];
  ...
  *****************************************************************************/
    ...
    VectorDouble& post_mean)
  {
    ...
    double ratio = 0.;
    double stdv = 0.;
    if (_flagStd && !_flagCumul) _getC00(VectorDouble(), covtab, c00);
    ...
    for (Id iechout = 0; iechout < _dbout->getNSample(); iechout++)
    {
      ...
      if (_colrefB >= 0)
      {
        ...
        if (flag_correc)
        {
          if (_prepareCollocation(
                iechout, coor, seltab, a, zval, prop1, prop2, covtab, b2, baux,
                &ratio))
            continue;
        }
      }
      ...
      for (Id ilayer = 0; ilayer < _nlayers; ilayer++)
      {
        ...
        if (_flagCumul)
        {
          if (_getPropsResult(iechout, ilayer + 1, prop1)) continue;
        }

        /* Find the C00 terms */

        if (_flagStd && _flagCumul) _getC00(prop1, covtab, c00);

        /* Establish the R.H.S. */

        if (_computeRHS(
              coor, seltab, iechout, ilayer + 1, prop1, prop2, covtab, b))
          continue;
        ...
        if (_flagBayes)
          _estimateBayes(
            c00[ilayer], a, zval, b, wgt, post_mean, a0, cc, ss, gs, estim,
            stdv);
        else
        ...
        {
          double cx =
            _getCI0(_nlayers, prop1, ilayer + 1, VectorDouble(), covtab);
          if (FFFF(cx)) continue;
        ...
        if (_flagStd)
          _dbout->setLocVariable(ELoc::Z, iechout, _nlayers + ilayer, stdv);
  ...
  void MLayers::_checkAuxiliaryVariables(VectorInt& seltab)
  {
    ...
    for (Id iech = 0; iech < _dbin->getNSample(); iech++)
    {
      ...
      if (_colrefB >= 0)
      {
        ...
        if (ilayer < _nlayers && _getCloseSample(iech, coor)) newval = 2;
      }
  ...
  void MLayers::_convertResults()
  {
    ...
    for (Id iechout = 0; iechout < _dbout->getNSample(); iechout++)
    {
      ...
      double depth0 =
        (_colrefD >= 0) ? _dbout->getArray(iechout, _colrefD) : 0.;
   ...
   **
   *****************************************************************************/
  Id MLayers::_calculateDriftBayes(
    bool verbose,
    MatrixSquare* acov,
    VectorDouble& zval,
    MatrixDense& fftab,
    ...
    MatrixSquare& post_vars) const
  {
    ...
    for (Id ipar = 0; ipar < _npar; ipar++)
      for (Id jpar = 0; jpar < _npar; jpar++)
        invS.setValue(
          ipar, jpar, (ipar == jpar) ? _model->getPriorCov(ipar, jpar) : 0.);
    ...
    for (Id ipar = 0; ipar < _npar; ipar++)
      for (Id jpar = 0; jpar < _npar; jpar++)
        post_vars.setValue(
          ipar, jpar, invS.getValue(ipar, jpar) + invH.getValue(ipar, jpar));
    post_vars.invert();

    /* Calculate the Posterior Mean vector */

    AMatrix::productInPlace(post_mean, invS, _model->getPriorMeans());
    for (Id i = 0; i < _npar; i++) fm1z[i] += post_mean[i];
    AMatrix::productInPlace(post_mean, post_vars, fm1z);
    ...
    if (verbose)
    {
      ...
      printMatrix(
        post_vars.getValues(), _npar, _npar, "Posterior Variance", 0, 1);
    }
    ...
    for (Id ipar = 0; ipar < _npar; ipar++)
      for (Id jpar = 0; jpar < _npar; jpar++)
        gg.setValue(
          ipar, jpar, invH.getValue(ipar, jpar) + invS.getValue(ipar, jpar));
    gg.invert();
    ...
    for (Id ipar = 0; ipar < _npar; ipar++)
      for (Id jpar = 0; jpar < _npar; jpar++)
        cc.setValue(
          ipar, jpar, acov->getValue(ipar, jpar) - cc.getValue(ipar, jpar));
    for (Id ipar = 0; ipar < _npar; ipar++)
      for (Id jpar = 0; jpar < _npar; jpar++)
        gs.setValue(
          ipar, jpar, invH.getValue(ipar, jpar) + invS.getValue(ipar, jpar));
        ...
        for (Id jl1 = 0; jl1 <= il1; jl1++, ecr1++)
        {
          ...
          for (Id il2 = 0; il2 < _nlayers; il2++)
            for (Id jl2 = 0; jl2 <= il2; jl2++, ecr2++)
            {
              ...
              atab.setValue(
                ecr1, ecr2, atab.getValue(ecr1, ecr2) + fact1 * fact2);
            }
        }
    ...
    Vario* vario)
  {
    ...
    for (Id ilag = 0; ilag < vario->getNLag(idir); ilag++)
    {
      ...
      if (_evaluateLag(
            vorder, ifirst, ilast, zval, &nval, &distsum, stat, phia, phib,
            atab, btab))
        goto label_end;
      ...
      atab.invert();
      AMatrix::productInPlace(sill, btab, atab);
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      for (Id ipar = 0; ipar < npar; ipar++)
      {
        ...
        for (Id jpar = 0; jpar < npar; jpar++)
          vars.updValue(
            ipar, jpar, EOperator::ADD, result[ipar] * result[jpar]);
      }
    }

    /* Normalize the results */
    for (Id ipar = 0; ipar < npar; ipar++) mean[ipar] /= nech;
  ...
  bool MLayers::checkValid()
  {
    ...
    if (_flagExt && _nlayers != _dbout->getNLoc(ELoc::F))
    {
      ...
      messerr(
        "- the number of external drifts in the Output Db File (%d)",
    ...
    if (_flagVel && _nlayers != get_LOCATOR_NITEM(_dbout, _ptime))
    {
      ...
      messerr("- the number of variables in the Model (%d)", _nlayers);
      messerr(
        "- the number of time variables in the Output Db File (%d)",
    ...
    if (_flagBayes && dim_prior != _getNDrift() * _nlayers)
    {
      ...
      messerr(
        "must be equal to %d (nlayers) x %d (nbfl)", _nlayers, _getNDrift());
      return false;
    }
  ...
  Id MLayers::kriging(bool verbose)
  {
    ...
    if (_flagBayes)
    {
      ...
      if (_calculateDriftBayes(
            verbose, a, zval, fftab, a0, cc, ss, gs, postMean, postVars))
        return -1;
    }
    else
    {
      a->invert();
      AMatrix::productInPlace(dual, *a, zval);
    ...
    _estimate(
      seltab, a, zval, dual, prop1, prop2, covtab, b, b2, baux, wgt, c00, fftab,
      a0, cc, ss, gs, postMean);
    ...
    const NamingConvention& namconv)
  {
    ...
    lmlayers.setGeneral(
      dbin, dbout, model, flag_std, flag_same, flag_vel, flag_cumul, flag_ext,
      flag_Z, flag_bayes, match_time, irf_rank, namerefd, namereft, namerefb);
    ...
    bool verbose)
  {
    MLayers lmlayers = MLayers();
    lmlayers.setGeneral(
      dbin, dbout, nullptr, false, false, flag_vel, false, flag_ext, false,
      false, match_time, irf_rank, namerefd, namereft, String());
    ...
    const String& namerefb)
  {
    MLayers lmlayers = MLayers();
    lmlayers.setGeneral(
      dbin, dbout, model, false, flag_same, flag_vel, false, flag_ext, false,
      false, match_time, irf_rank, namerefd, namereft, namerefb);
