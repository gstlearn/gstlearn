      ...
      for (Id ifois = 0; ifois < seltab[iech]; ifois++, iiech++)
      {
        ...
        for (Id k1 = 0; k1 < ipos; k1++)
        {
          ...
          for (Id k2 = 0; k2 < ipos; k2++)
            atab.setValue(k1,
                          k2,
                          atab.getValue(k1, k2) + drift[k1] * drift[k2]);
        }
      }
    }
  ...
  *****************************************************************************/
                          ...
                          VectorDouble& post_mean)
  {
    ...
    for (Id iechout = 0; iechout < _dbout->getNSample(); iechout++)
    {
      ...
      if (_colrefB >= 0)
      {
        ...
        if (flag_correc)
        {
          if (_prepareCollocation(iechout,
                                  coor,
                                  seltab,
                                  a,
                                  zval,
                                   ...
                                   MatrixSquare& post_vars) const
  {
    ...
    for (Id ipar = 0; ipar < _npar; ipar++)
      for (Id jpar = 0; jpar < _npar; jpar++)
        invS.setValue(ipar,
                      jpar,
                      (ipar == jpar) ? _model->getPriorCov(ipar, jpar) : 0.);
    ...
    for (Id ipar = 0; ipar < _npar; ipar++)
      for (Id jpar = 0; jpar < _npar; jpar++)
        post_vars.setValue(ipar,
                           jpar,
                           invS.getValue(ipar, jpar)
                             + invH.getValue(ipar, jpar));
    post_vars.invert();
    ...
    if (verbose)
    {
      ...
      printMatrix(post_vars.getValues(),
                  _npar,
                  _npar,
                  "Posterior Variance",
                  0,
    ...
    for (Id ipar = 0; ipar < _npar; ipar++)
      for (Id jpar = 0; jpar < _npar; jpar++)
        gg.setValue(ipar,
                    jpar,
                    invH.getValue(ipar, jpar) + invS.getValue(ipar, jpar));
    gg.invert();
    ...
    for (Id ipar = 0; ipar < _npar; ipar++)
      for (Id jpar = 0; jpar < _npar; jpar++)
        cc.setValue(ipar,
                    jpar,
                    acov->getValue(ipar, jpar) - cc.getValue(ipar, jpar));
    for (Id ipar = 0; ipar < _npar; ipar++)
      for (Id jpar = 0; jpar < _npar; jpar++)
        gs.setValue(ipar,
                    jpar,
                           ...
                           VectorDouble& btab)
  {
    ...
    for (Id ipair = ifirst; ipair < ilast; ipair++)
    {
      ...
      vario_order_get_auxiliary(vorder,
                                ipair,
        ...
        for (Id jl1 = 0; jl1 <= il1; jl1++, ecr1++)
        {
          ...
          for (Id il2 = 0; il2 < _nlayers; il2++)
            for (Id jl2 = 0; jl2 <= il2; jl2++, ecr2++)
            {
              ...
              atab.setValue(ecr1,
                            ecr2,
                            atab.getValue(ecr1, ecr2) + fact1 * fact2);
            }
        }
      ...
      for (Id ipar = 0; ipar < npar; ipar++)
      {
        ...
        for (Id jpar = 0; jpar <= ipar; jpar++)
          atab0.updValue(ipar,
                         jpar,
                         EOperator::ADD,
                         fftab.getValue(ipar, iech)
                           * fftab.getValue(jpar, iech));
      }
  ...
  Id MLayers::kriging(bool verbose)
  {
    ...
    if (_flagBayes)
    {
      ...
      if (_calculateDriftBayes(verbose,
                               a,
                               zval,
                               fftab,
                               a0,
