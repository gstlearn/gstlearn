...
namespace gstlrn
{
  ...
  String AMesh::toString(const AStringFormat* strfmt) const
  {
    ...
    if (!_extendMin.empty() && !_extendMax.empty())
    {
      ...
      for (Id idim = 0; idim < _nDim; idim++)
        sstr << "Dim #" << idim + 1 << " - Min:" << _extendMin[idim]
             << " - Max:" << _extendMax[idim] << std::endl;
    }
  ...
  void
    AMesh::getCoordinatesPerMeshInPlace(Id imesh, Id rank, VectorDouble& coords)
      const
  {
  ...
  *****************************************************************************/
  Id AMesh::isCompatibleDb(const Db* db) const
  {
    ...
    messerr(
      "Inconsistent Space dimension between Meshing (%d) and Db (%d)",
      getNDim(), db->getNDim());
    return 1;
  }
  ...
  void AMesh::printMeshes(Id level, Id nline_max) const
  {
    ...
    if (level == 1) _printMeshListByIndices(nline_max);

    if (level == 2) _printMeshListByCoordinates(nline_max);
  }
  ...
  VectorDouble AMesh::getCoordinatesPerApex(Id idim) const
  {
    ...
    for (Id ip = 0; ip < np; ip++) coor[ip] = getApexCoor(ip, idim);
    return coor;
  }
  ...
  VectorVectorDouble AMesh::getAllCoordinates() const
  {
    ...
    for (Id idim = 0; idim < _nDim; idim++) coords[idim].resize(napices);

    for (Id ip = 0; ip < napices; ip++)
    {
      getApexCoordinatesInPlace(ip, local);
      for (Id idim = 0; idim < _nDim; idim++) coords[idim][ip] = local[idim];
    }
    return coords;
  }
  ...
  VectorVectorDouble AMesh::getAllCenterCoordinates() const
  {
    ...
    auto nmeshes = getNMeshes();
    VectorVectorDouble coords(_nDim);
    for (Id idim = 0; idim < _nDim; idim++) coords[idim].resize(nmeshes);
    ...
    for (Id imesh = 0; imesh < nmeshes; imesh++)
    {
      ...
      {
        double total = 0.;
        for (Id ic = 0; ic < ncorner; ic++) total += getCoor(imesh, ic, idim);
  ...
  VectorInt AMesh::getMeshByApexPair(Id apex1, Id apex2) const
  {
    ...
    for (Id imesh = 0; imesh < getNMeshes(); imesh++)
    {
      ...
      for (Id ic = 0; ic < ncorner; ic++)
      {
        ...
        if (apex0 == apex1) found++;
        if (apex0 == apex2) found++;
        if (found == 2)
        {
  ...
  *****************************************************************************/
  ...
  VectorDouble
    AMesh::getCoordinatesPerMesh(Id imesh, Id idim, bool flagClose) const
  {
    VectorDouble vec;
    auto ncorner = getNApexPerMesh();
   ...
   */
  void
    AMesh::getEmbeddedCoorPerMesh(Id imesh, Id ic, VectorDouble& coords) const
  {
  ...
  ProjMatrix*
    AMesh::createProjMatrix(const Db* db, Id rankZ, bool verbose) const
  {
   ...
   */
  void AMesh::getEmbeddedCoordinatesPerMeshInPlace(
    Id imesh,
  ...
  VectorVectorDouble AMesh::getEmbeddedCoordinatesPerMesh(Id imesh) const
  {
    ...
    for (auto& e: vec) e = VectorDouble(ndim);
  ...
  VectorVectorDouble AMesh::getEmbeddedCoordinatesPerApex() const
  {
    ...
    for (auto& e: vec) e = VectorDouble(napices);

    VectorDouble local(ndim);
    for (Id ip = 0; ip < napices; ip++)
    {
      getEmbeddedCoorPerApex(ip, local);
      for (Id idim = 0; idim < ndim; idim++) vec[idim][ip] = local[idim];
    }
    return vec;
  }
  ...
  VectorInt
    AMesh::getMeshApicesFromCoordinates(const VectorDouble& coords) const
  {
    auto ncorner = getNApexPerMesh();
  ...
  Id AMesh::getMeshAndInPlaceWeightsFromCoordinates(
    const VectorDouble& coords,
    VectorDouble& weights) const
  {
    auto nmeshes = getNMeshes();
    VectorDouble units(nmeshes, 0.);
    if (getVariety() != 1) units = _defineUnits();
    ...
    if (found < 0)
    {
      ...
      {
        messerr(
          "Point (%lf %lf %lf) does not belong to any mesh (nb_neigh=%d)",
          coords[0], coords[1], coords[2], nb_neigh);
      }
      else
      {
        messerr(
          "Point (%lf %lf) does not belong to any mesh (nb_neigh=%d)",
  ...
  bool AMesh::_deserializeAscii(std::istream& is)
  {
    ...
    ret =
      ret && _recordReadVec<double>(is, "Minimum Extension", _extendMin, _nDim);
    ret =
      ret && _recordReadVec<double>(is, "Maximum Extension", _extendMax, _nDim);
    return ret;
  }
  ...
  *****************************************************************************/
  ...
  void AMesh::getBarycenterInPlace(Id imesh, vect coord) const
  {
    ...
    {
      double local = 0.;
      for (Id ic = 0; ic < ncorner; ic++) local += coords[ic][idim];
  ...
  *****************************************************************************/
  void
    AMesh::resetProjFromDb(ProjMatrix* m, const Db* db, Id rankZ, bool verbose)
      const
  {
    auto ndim = getNDim();
