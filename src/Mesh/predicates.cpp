...
#include "Mesh/tetgen.h"        // Defines the symbol DREAL (float or double).
...
/* #define DREAL double */                      /* float or double */
...
static DREAL splitter;
static DREAL epsilon;         /* = 2^(-p).  Used to estimate roundoff errors. */
/* A set of coefficients used to calculate maximum roundoff errors.          */
static DREAL resulterrbound;
static DREAL ccwerrboundA, ccwerrboundB, ccwerrboundC;
static DREAL o3derrboundA, o3derrboundB, o3derrboundC;
static DREAL iccerrboundA, iccerrboundB, iccerrboundC;
static DREAL isperrboundA, isperrboundB, isperrboundC;
...
// Added by H. Si, 2012-08-23.
static DREAL o3dstaticfilter;
static DREAL ispstaticfilter;
...
int test_float(int verbose)
{
  ...
  if (verbose) {
    (void)message("  sizeof(float) = %2u\n", (unsigned int)sizeof(float));
#ifdef CPU86  // <float.h>
    (void)message("  FLT_MANT_DIG = %2d\n", FLT_MANT_DIG);
#endif
  }
  ...
  while (fstore((float)1.0 + x/(float)2.0) != (float)1.0)
    x /= (float)2.0;
  if (verbose)
    (void)message("  machine epsilon = %13.5e  ", x);

  if (x == (float)fppow2(-23)) {
    if (verbose)
      (void)message("[IEEE 754 32-bit macheps]\n");
  } else {
    (void)message("[not IEEE 754 conformant] !!\n");
    pass = 0;
  }
  ...
  while (fstore(x / (float)2.0) != (float)0.0)
    x /= (float)2.0;
  if (verbose)
    (void)message("  smallest positive number =  %13.5e  ", x);

  if (x == (float)fppow2(-149)) {
    if (verbose)
      (void)message("[smallest 32-bit subnormal]\n");
  } else if (x == (float)fppow2(-126)) {
    if (verbose)
      (void)message("[smallest 32-bit normal]\n");
  } else {
    (void)message("[not IEEE 754 conformant] !!\n");
    pass = 0;
  }
...
int test_double(int verbose)
{
  ...
  // (void)message("double:\n");
  if (verbose) {
    (void)message("  sizeof(double) = %2u\n", (unsigned int)sizeof(double));
#ifdef CPU86  // <float.h>
    (void)message("  DBL_MANT_DIG = %2d\n", DBL_MANT_DIG);
#endif
  }
  ...
  while (dstore(1.0 + x/2.0) != 1.0)
    x /= 2.0;
  if (verbose) 
    (void)message("  machine epsilon = %13.5le ", x);

  if (x == (double)fppow2(-52)) {
    if (verbose)
      (void)message("[IEEE 754 64-bit macheps]\n");
  } else {
    (void)message("[not IEEE 754 conformant] !!\n");
    pass = 0;
  }
  ...
  while (dstore(x / 2.0) != 0.0)
    x /= 2.0;
  //if (verbose)
  //  (void)message("  smallest positive number = %13.5le ", x);

  if (x == (double)fppow2(-1074)) {
    //if (verbose)
    //  (void)message("[smallest 64-bit subnormal]\n");
  } else if (x == (double)fppow2(-1022)) {
    //if (verbose)
    //  (void)message("[smallest 64-bit normal]\n");
  } else {
    (void)message("[not IEEE 754 conformant] !!\n");
    pass = 0;
  }
...
void exactinit(int verbose, int noexact, int nofilter, DREAL maxx, DREAL maxy, 
               DREAL maxz)
{
  DREAL half;
  ...
  if (verbose) {
    message("  Initializing robust predicates.\n");
  }

#ifdef USE_CGAL_PREDICATES
  if (cgal_pred_obj.Has_static_filters) {
    message("  Use static filter.\n");
  } else {
    message("  No static filter.\n");
  }
...
int grow_expansion(int elen, DREAL *e, DREAL b, DREAL *h)
...
int grow_expansion_zeroelim(int elen, DREAL *e, DREAL b, DREAL *h)
  /* e and h can be the same. */
{
  DREAL Q, hh;
  INEXACT DREAL Qnew;
...
int expansion_sum(int elen, DREAL *e, int flen, DREAL *f, DREAL *h)
...
int expansion_sum_zeroelim1(int elen, DREAL *e, int flen, DREAL *f, DREAL *h)
  /* e and h can be the same, but f and h cannot. */
{
  DREAL Q;
  INEXACT DREAL Qnew;
...
int expansion_sum_zeroelim2(int elen, DREAL *e, int flen, DREAL *f, DREAL *h)
  /* e and h can be the same, but f and h cannot. */
{
  DREAL Q, hh;
  INEXACT DREAL Qnew;
...
int fast_expansion_sum(int elen, DREAL *e, int flen, DREAL *f, DREAL *h)
  /* h cannot be e or f. */
{
  DREAL Q;
  INEXACT DREAL Qnew;
...
int fast_expansion_sum_zeroelim(int elen, DREAL *e, int flen, DREAL *f, DREAL *h)
  /* h cannot be e or f. */
{
  DREAL Q;
  INEXACT DREAL Qnew;
...
int linear_expansion_sum(int elen, DREAL *e, int flen, DREAL *f, DREAL *h)
  /* h cannot be e or f. */
{
  DREAL Q, q;
  INEXACT DREAL Qnew;
...
int linear_expansion_sum_zeroelim(int elen, DREAL *e, int flen, DREAL *f,
                                  DREAL *h)
  /* h cannot be e or f. */
{
  DREAL Q, q, hh;
...
int scale_expansion(int elen, DREAL *e, DREAL b, DREAL *h)
...
int scale_expansion_zeroelim(int elen, DREAL *e, DREAL b, DREAL *h)
  /* e and h cannot be the same. */
{
  INEXACT DREAL Q, sum;
  DREAL hh;
...
int compress(int elen, DREAL *e, DREAL *h)
...
static DREAL estimate(int elen, DREAL *e)
{
  DREAL Q;
  int eindex;
...
DREAL orient2dfast(DREAL *pa, DREAL *pb, DREAL *pc)
{
  DREAL acx, bcx, acy, bcy;
...
DREAL orient2dexact(DREAL *pa, DREAL *pb, DREAL *pc)
{
  INEXACT DREAL axby1, axcy1, bxcy1, bxay1, cxay1, cxby1;
  DREAL axby0, axcy0, bxcy0, bxay0, cxay0, cxby0;
  DREAL aterms[4], bterms[4], cterms[4];
  INEXACT DREAL aterms3, bterms3, cterms3;
  DREAL v[8], w[12];
  int vlength, wlength;
...
DREAL orient2dslow(DREAL *pa, DREAL *pb, DREAL *pc)
{
  INEXACT DREAL acx, acy, bcx, bcy;
  DREAL acxtail, acytail;
  DREAL bcxtail, bcytail;
  DREAL negate, negatetail;
  DREAL axby[8], bxay[8];
  INEXACT DREAL axby7, bxay7;
  DREAL deter[16];
  int deterlen;
  ...
  INEXACT DREAL c;
  INEXACT DREAL abig;
  DREAL a0hi, a0lo, a1hi, a1lo, bhi, blo;
  DREAL err1, err2, err3;
  INEXACT DREAL _i, _j, _k, _l, _m, _n;
  DREAL _0, _1, _2;
...
DREAL orient2dadapt(DREAL *pa, DREAL *pb, DREAL *pc, DREAL detsum)
{
  INEXACT DREAL acx, acy, bcx, bcy;
...
DREAL orient2d(DREAL *pa, DREAL *pb, DREAL *pc)
{
...
DREAL orient3dfast(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd)
{
  DREAL adx, bdx, cdx;
  DREAL ady, bdy, cdy;
  DREAL adz, bdz, cdz;
...
DREAL orient3dexact(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd)
{
  INEXACT DREAL axby1, bxcy1, cxdy1, dxay1, axcy1, bxdy1;
  INEXACT DREAL bxay1, cxby1, dxcy1, axdy1, cxay1, dxby1;
  DREAL axby0, bxcy0, cxdy0, dxay0, axcy0, bxdy0;
  DREAL bxay0, cxby0, dxcy0, axdy0, cxay0, dxby0;
  DREAL ab[4], bc[4], cd[4], da[4], ac[4], bd[4];
  DREAL temp8[8];
  int templen;
  DREAL abc[12], bcd[12], cda[12], dab[12];
  int abclen, bcdlen, cdalen, dablen;
  DREAL adet[24], bdet[24], cdet[24], ddet[24];
  int alen, blen, clen, dlen;
  DREAL abdet[48], cddet[48];
  int ablen, cdlen;
  DREAL deter[96];
  int deterlen;
  int i;
...
DREAL orient3dslow(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd)
{
  INEXACT DREAL adx, ady, adz, bdx, bdy, bdz, cdx, cdy, cdz;
  DREAL adxtail, adytail, adztail;
  DREAL bdxtail, bdytail, bdztail;
  DREAL cdxtail, cdytail, cdztail;
  DREAL negate, negatetail;
  INEXACT DREAL axby7, bxcy7, axcy7, bxay7, cxby7, cxay7;
  DREAL axby[8], bxcy[8], axcy[8], bxay[8], cxby[8], cxay[8];
  DREAL temp16[16], temp32[32], temp32t[32];
  int temp16len, temp32len, temp32tlen;
  DREAL adet[64], bdet[64], cdet[64];
  int alen, blen, clen;
  DREAL abdet[128];
  int ablen;
  DREAL deter[192];
...
DREAL orient3dadapt(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd, DREAL permanent)
{
  INEXACT DREAL adx, bdx, cdx, ady, bdy, cdy, adz, bdz, cdz;
  DREAL det, errbound;
...
DREAL orient3d(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd)
{
  return (DREAL) 
...
DREAL orient3d(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd)
{
  DREAL adx, bdx, cdx, ady, bdy, cdy, adz, bdz, cdz;
...
DREAL incirclefast(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd)
{
  DREAL adx, ady, bdx, bdy, cdx, cdy;
  DREAL abdet, bcdet, cadet;
...
DREAL incircleexact(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd)
{
  INEXACT DREAL axby1, bxcy1, cxdy1, dxay1, axcy1, bxdy1;
  INEXACT DREAL bxay1, cxby1, dxcy1, axdy1, cxay1, dxby1;
  DREAL axby0, bxcy0, cxdy0, dxay0, axcy0, bxdy0;
  ...
  DREAL abc[12], bcd[12], cda[12], dab[12];
  int abclen, bcdlen, cdalen, dablen;
  DREAL det24x[24], det24y[24], det48x[48], det48y[48];
  int xlen, ylen;
  DREAL adet[96], bdet[96], cdet[96], ddet[96];
  int alen, blen, clen, dlen;
  DREAL abdet[192], cddet[192];
  int ablen, cdlen;
  DREAL deter[384];
  int deterlen;
  int i;
...
DREAL incircleslow(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd)
{
  ...
  DREAL temp16[16];
  int temp16len;
  DREAL detx[32], detxx[64], detxt[32], detxxt[64], detxtxt[64];
  int xlen, xxlen, xtlen, xxtlen, xtxtlen;
  DREAL x1[128], x2[192];
  int x1len, x2len;
  DREAL dety[32], detyy[64], detyt[32], detyyt[64], detytyt[64];
  int ylen, yylen, ytlen, yytlen, ytytlen;
  DREAL y1[128], y2[192];
  int y1len, y2len;
  DREAL adet[384], bdet[384], cdet[384], abdet[768], deter[1152];
...
DREAL incircleadapt(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd, DREAL permanent)
{
  INEXACT DREAL adx, bdx, cdx, ady, bdy, cdy;
  DREAL det, errbound;
...
DREAL incircle(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd)
{
...
DREAL inspherefast(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd, DREAL *pe)
{
  DREAL aex, bex, cex, dex;
  DREAL aey, bey, cey, dey;
  DREAL aez, bez, cez, dez;
  DREAL alift, blift, clift, dlift;
  DREAL ab, bc, cd, da, ac, bd;
  DREAL abc, bcd, cda, dab;
...
DREAL insphereexact(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd, DREAL *pe)
{
  INEXACT DREAL axby1, bxcy1, cxdy1, dxey1, exay1;
  INEXACT DREAL bxay1, cxby1, dxcy1, exdy1, axey1;
  INEXACT DREAL axcy1, bxdy1, cxey1, dxay1, exby1;
  INEXACT DREAL cxay1, dxby1, excy1, axdy1, bxey1;
  DREAL axby0, bxcy0, cxdy0, dxey0, exay0;
  DREAL bxay0, cxby0, dxcy0, exdy0, axey0;
  DREAL axcy0, bxdy0, cxey0, dxay0, exby0;
  DREAL cxay0, dxby0, excy0, axdy0, bxey0;
  DREAL ab[4], bc[4], cd[4], de[4], ea[4];
  DREAL ac[4], bd[4], ce[4], da[4], eb[4];
  DREAL temp8a[8], temp8b[8], temp16[16];
  int temp8alen, temp8blen, temp16len;
  DREAL abc[24], bcd[24], cde[24], dea[24], eab[24];
  DREAL abd[24], bce[24], cda[24], deb[24], eac[24];
  int abclen, bcdlen, cdelen, dealen, eablen;
  int abdlen, bcelen, cdalen, deblen, eaclen;
  DREAL temp48a[48], temp48b[48];
  int temp48alen, temp48blen;
  DREAL abcd[96], bcde[96], cdea[96], deab[96], eabc[96];
  int abcdlen, bcdelen, cdealen, deablen, eabclen;
  DREAL temp192[192];
  DREAL det384x[384], det384y[384], det384z[384];
  int xlen, ylen, zlen;
  DREAL detxy[768];
  int xylen;
  DREAL adet[1152], bdet[1152], cdet[1152], ddet[1152], edet[1152];
  int alen, blen, clen, dlen, elen;
  DREAL abdet[2304], cddet[2304], cdedet[3456];
  int ablen, cdlen;
  DREAL deter[5760];
  int deterlen;
  int i;
...
DREAL insphereslow(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd, DREAL *pe)
{
  INEXACT DREAL aex, bex, cex, dex, aey, bey, cey, dey, aez, bez, cez, dez;
  DREAL aextail, bextail, cextail, dextail;
  DREAL aeytail, beytail, ceytail, deytail;
  DREAL aeztail, beztail, ceztail, deztail;
  DREAL negate, negatetail;
  INEXACT DREAL axby7, bxcy7, cxdy7, dxay7, axcy7, bxdy7;
  INEXACT DREAL bxay7, cxby7, dxcy7, axdy7, cxay7, dxby7;
  DREAL axby[8], bxcy[8], cxdy[8], dxay[8], axcy[8], bxdy[8];
  DREAL bxay[8], cxby[8], dxcy[8], axdy[8], cxay[8], dxby[8];
  DREAL ab[16], bc[16], cd[16], da[16], ac[16], bd[16];
  int ablen, bclen, cdlen, dalen, aclen, bdlen;
  DREAL temp32a[32], temp32b[32], temp64a[64], temp64b[64], temp64c[64];
  int temp32alen, temp32blen, temp64alen, temp64blen, temp64clen;
  DREAL temp128[128], temp192[192];
  int temp128len, temp192len;
  DREAL detx[384], detxx[768], detxt[384], detxxt[768], detxtxt[768];
  int xlen, xxlen, xtlen, xxtlen, xtxtlen;
  DREAL x1[1536], x2[2304];
  int x1len, x2len;
  DREAL dety[384], detyy[768], detyt[384], detyyt[768], detytyt[768];
  int ylen, yylen, ytlen, yytlen, ytytlen;
  DREAL y1[1536], y2[2304];
  int y1len, y2len;
  DREAL detz[384], detzz[768], detzt[384], detzzt[768], detztzt[768];
  int zlen, zzlen, ztlen, zztlen, ztztlen;
  DREAL z1[1536], z2[2304];
  int z1len, z2len;
  DREAL detxy[4608];
  int xylen;
  DREAL adet[6912], bdet[6912], cdet[6912], ddet[6912];
  int alen, blen, clen, dlen;
  DREAL abdet[13824], cddet[13824], deter[27648];
  int deterlen;
  int i;
...
DREAL insphereadapt(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd, DREAL *pe,
                    DREAL permanent)
{
  INEXACT DREAL aex, bex, cex, dex, aey, bey, cey, dey, aez, bez, cez, dez;
  DREAL det, errbound;

  INEXACT DREAL aexbey1, bexaey1, bexcey1, cexbey1;
  INEXACT DREAL cexdey1, dexcey1, dexaey1, aexdey1;
  INEXACT DREAL aexcey1, cexaey1, bexdey1, dexbey1;
  DREAL aexbey0, bexaey0, bexcey0, cexbey0;
  DREAL cexdey0, dexcey0, dexaey0, aexdey0;
  DREAL aexcey0, cexaey0, bexdey0, dexbey0;
  DREAL ab[4], bc[4], cd[4], da[4], ac[4], bd[4];
  INEXACT DREAL ab3, bc3, cd3, da3, ac3, bd3;
  DREAL abeps, bceps, cdeps, daeps, aceps, bdeps;
  DREAL temp8a[8], temp8b[8], temp8c[8], temp16[16], temp24[24], temp48[48];
  int temp8alen, temp8blen, temp8clen, temp16len, temp24len, temp48len;
  DREAL xdet[96], ydet[96], zdet[96], xydet[192];
  int xlen, ylen, zlen, xylen;
  DREAL adet[288], bdet[288], cdet[288], ddet[288];
  int alen, blen, clen, dlen;
  DREAL abdet[576], cddet[576];
  int ablen, cdlen;
  DREAL fin1[1152];
  int finlength;
...
DREAL insphere(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd, DREAL *pe)
{
  return (DREAL)
...
DREAL insphere(DREAL *pa, DREAL *pb, DREAL *pc, DREAL *pd, DREAL *pe)
{
  ...
  DREAL aey, bey, cey, dey;
  DREAL aez, bez, cez, dez;
  DREAL aexbey, bexaey, bexcey, cexbey, cexdey, dexcey, dexaey, aexdey;
  DREAL aexcey, cexaey, bexdey, dexbey;
  DREAL alift, blift, clift, dlift;
  DREAL ab, bc, cd, da, ac, bd;
  DREAL abc, bcd, cda, dab;
  DREAL det;
  ...
  DREAL aezplus, bezplus, cezplus, dezplus;
  DREAL aexbeyplus, bexaeyplus, bexceyplus, cexbeyplus;
  DREAL cexdeyplus, dexceyplus, dexaeyplus, aexdeyplus;
  DREAL aexceyplus, cexaeyplus, bexdeyplus, dexbeyplus;
  DREAL permanent, errbound;
...
DREAL orient4dexact(DREAL* pa, DREAL* pb, DREAL* pc, DREAL* pd, DREAL* pe,
                    ...
                    DREAL eheight)
{
  ...
  DREAL abcd[96], bcde[96], cdea[96], deab[96], eabc[96];
  int abcdlen, bcdelen, cdealen, deablen, eabclen;
  DREAL adet[192], bdet[192], cdet[192], ddet[192], edet[192];
  int alen, blen, clen, dlen, elen;
  DREAL abdet[384], cddet[384], cdedet[576];
  int ablen, cdlen;
  DREAL deter[960];
  int deterlen;
  int i;
...
DREAL orient4dadapt(DREAL* pa, DREAL* pb, DREAL* pc, DREAL* pd, DREAL* pe,
                    DREAL aheight, DREAL bheight, DREAL cheight, DREAL dheight, 
                    DREAL eheight, DREAL permanent)
{
  INEXACT DREAL aex, bex, cex, dex, aey, bey, cey, dey, aez, bez, cez, dez;
  INEXACT DREAL aeheight, beheight, ceheight, deheight;
  DREAL det, errbound;
  ...
  INEXACT DREAL ab3, bc3, cd3, da3, ac3, bd3;
  DREAL abeps, bceps, cdeps, daeps, aceps, bdeps;
  DREAL temp8a[8], temp8b[8], temp8c[8], temp16[16], temp24[24];
  int temp8alen, temp8blen, temp8clen, temp16len, temp24len;
  DREAL adet[48], bdet[48], cdet[48], ddet[48];
  int alen, blen, clen, dlen;
  DREAL abdet[96], cddet[96];
  int ablen, cdlen;
  DREAL fin1[192];
  int finlength;
  ...
  DREAL aeytail, beytail, ceytail, deytail;
  DREAL aeztail, beztail, ceztail, deztail;
  DREAL aeheighttail, beheighttail, ceheighttail, deheighttail;
  ...
  cez = (DREAL) (pc[2] - pe[2]);
  dez = (DREAL) (pd[2] - pe[2]);
  aeheight = (DREAL) (aheight - eheight);
  beheight = (DREAL) (bheight - eheight);
  ceheight = (DREAL) (cheight - eheight);
  deheight = (DREAL) (dheight - eheight);
...
DREAL orient4d(DREAL* pa, DREAL* pb, DREAL* pc, DREAL* pd, DREAL* pe, 
               DREAL aheight, DREAL bheight, DREAL cheight, DREAL dheight, 
               DREAL eheight)
{
  DREAL aex, bex, cex, dex;
  ...
  DREAL aexbey, bexaey, bexcey, cexbey, cexdey, dexcey, dexaey, aexdey;
  DREAL aexcey, cexaey, bexdey, dexbey;
  DREAL aeheight, beheight, ceheight, deheight;
  DREAL ab, bc, cd, da, ac, bd;
  DREAL abc, bcd, cda, dab;
  DREAL aezplus, bezplus, cezplus, dezplus;
  DREAL aexbeyplus, bexaeyplus, bexceyplus, cexbeyplus;
