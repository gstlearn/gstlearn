...
namespace gstlrn
{
  ...
  *****************************************************************************/
  ...
  {
    return GH::geodeticTriangleSurface(
      getCoor(imesh, 0, 0), getCoor(imesh, 0, 1), getCoor(imesh, 1, 0),
      getCoor(imesh, 1, 1), getCoor(imesh, 2, 0), getCoor(imesh, 2, 1));
  }
  ...
  *****************************************************************************/
   ...
   */
  MeshSpherical*
    MeshSpherical::createFromNF(const String& NFFilename, bool verbose)
  {
  ...
  MeshSpherical*
    MeshSpherical::create(const MatrixDense& apices, const MatrixInt& meshes)
  {
  ...
  **
  *****************************************************************************/
  bool MeshSpherical::_weightsInMesh(
  ...
  *****************************************************************************/
  ...
  void MeshSpherical::_getCoordOnSphere(
    double longitude,
    double latitude,
    VectorDouble& coords)
  {
    ...
    GH::convertSph2Cart(
      longitude, latitude, &coords.at(0), &coords.at(1), &coords.at(2), radius);
  }

  void
    MeshSpherical::getEmbeddedCoorPerMesh(Id imesh, Id ic, VectorDouble& coords)
      const
  {
  ...
  void
    MeshSpherical::getEmbeddedCoorPerApex(Id iapex, VectorDouble& coords) const
  {
  ...
  bool MeshSpherical::_deserializeAscii(std::istream& is)
  {
    ...
    if (ret)
    {
      ...
      ret = ret
         && _recordReadVec<double>(is, "Apices", apices_local, ndim * napices);
    ...
    if (ret)
    {
      ...
      ret = ret
         && _recordReadVec<Id>(
              is, "Meshes", meshes_local, nmeshes * napexpermesh);
  ...
  bool MeshSpherical::_serializeAscii(std::ostream& os) const
  {
    ...
    ret = ret
       && _recordWrite<Id>(os, "Number of Apices per Mesh", getNApexPerMesh());
  ...
  void MeshSpherical::getBarycenterInPlace(Id imesh, vect coord) const
  {
    ...
    {
      double local = 0.;
      for (Id ic = 0; ic < ncorner; ic++) local += coordE[ic][idimE];
      centerE[idimE] = local / ncorner;
    }

    // Turn the gravity center from embedded to long/lat coordinates
    GH::convertCart2Sph(
      centerE[0], centerE[1], centerE[2], &rlong, &rlat, TEST);
    coord[0] = rlong;
    coord[1] = rlat;
  }
  ...
  bool MeshSpherical::serializeH5(H5::Group& grp) const
  {
    ...
    ret =
      ret && SerializeHDF5::writeValue(meshG, "NPerMesh", getNApexPerMesh());
