...
namespace gstlrn
{
  ...
  **
  *****************************************************************************/
  Id MeshSphericalExt::resetFromDb(
    Db* dbin,
    Db* dbout,
  ...
  void MeshSphericalExt::_meshesSphLoadVertices(SphTriangle* t)
  {
    ...
    (void)trlist_(
      &t->n_nodes, t->sph_list.data(), t->sph_lptr.data(), t->sph_lend.data(),
      &nrow, &nt, ltri.data(), &error);
    if (error) return;
    ...
    for (Id i = 0; i < ntab; i++)
    {
      for (Id j = 0; j < natt; j++) itab[ecr + j] = ltri[lec + j] - 1;
      ecr += natt;
      lec += nrow;
    }
   ...
   **
   *****************************************************************************/
  AMesh* MeshSphericalExt::spde_mesh_load(
    Db* dbin,
    Db* dbout,
   ...
   **
   *****************************************************************************/
  AMesh* MeshSphericalExt::_load2DSph(
    bool verbose,
    Db* dbin,
    Db* dbout,
    const String& triswitch)
