...
namespace gstlrn
{
    ...
    , _gridIndirect(mode)
  {
    ...
    (void)initFromGridByMatrix(dbgrid->getNXs(),
                               dbgrid->getDXs(),
                               dbgrid->getX0s(),
                               dbgrid->getRotMat(),
                               sel,
  ...
  *****************************************************************************/
  ...
  void MeshETurbo::getCoordinatesPerMeshInPlace(Id imesh,
                                                Id rank,
                                             ...
                                             Id mode)
  {
    ...
    if (mesh->initFromGridByMatrix(grid->getNXs(),
                                   grid->getDXs(),
                                   grid->getX0s(),
                                   grid->getRotMat(),
                                   VectorDouble(),
  ...
  *****************************************************************************/
...

  {
    ...
    for (Id idim = 0; idim < ndim; idim++)
    {
      ...
      _grid.setNX(idim,
                  static_cast<Id>(ceil((getExtendMax(idim) - getExtendMin(idim))
                                       / cellsize[idim])
                                  + 1));
    }
                              ...
                              bool verbose)
  {
    ...
    initFromGridByMatrix(nx,
                         dx,
                         x0,
                         rot.getMatrixDirectVec(),
                         VectorDouble(),
                         isPolarized,
                         verbose);
    return 0;
  ...
  bool MeshETurbo::_deserializeAscii(std::istream& is)
  {
    ...
    if (ret)
      (void)initFromGridByMatrix(nx,
                                 dx,
                                 x0,
                                 rotmat,
                                 VectorDouble(),
                                 static_cast<bool>(flag_polarized),
                                 0);
  ...
  bool MeshETurbo::serializeH5(H5::Group& grp) const
  {
    ...
    if (nmesh_mask > 0)
      ret = ret
         && SerializeHDF5::writeVec(meshG,
                                    "MeshMask",
                                    _meshIndirect.getRelRanks());
