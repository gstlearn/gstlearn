...
namespace gstlrn
{
  ...
  MeshETurbo::MeshETurbo(
    ...
    , _gridIndirect(mode)
  {
    (void)initFromGridByAngles(
      nx, dx, x0, angles, VectorDouble(), flag_polarized, verbose);
  }
    ...
    , _gridIndirect(mode)
  {
    ...
    (void)initFromGridByMatrix(
      dbgrid->getNXs(), dbgrid->getDXs(), dbgrid->getX0s(), dbgrid->getRotMat(),
      sel, flag_polarized, verbose);
  }
  ...
  *****************************************************************************/
  Id MeshETurbo::getApex(Id imesh, Id rank) const
  {
    ...
    {
      messerr(
        "Problem for mesh=%d rank=%d grid=%d -> Mesh relative rank is negative",
  ...
  void MeshETurbo::getCoordinatesPerMeshInPlace(
    Id imesh,
    Id rank,
  ...
  Id MeshETurbo::initFromGridByAngles(
    const VectorInt& nx,
    const VectorDouble& dx,
    const VectorDouble& x0,
    const VectorDouble& angles,
  ...
  Id MeshETurbo::initFromGridByMatrix(
    const VectorInt& nx,
    const VectorDouble& dx,
    const VectorDouble& x0,
  ...
  Id MeshETurbo::_initFromGridInternal(
    const VectorDouble& sel,
    bool flag_polarized,
    bool verbose)
  {
  ...
  MeshETurbo* MeshETurbo::createFromGrid(
    const DbGrid* dbgrid,
    bool flag_polarized,
    bool verbose,
    Id mode)
  ...
  MeshETurbo* MeshETurbo::createFromGridInfo(
    const Grid* grid,
    bool flag_polarized,
    bool verbose,
    Id mode)
  {
    ...
    if (mesh->initFromGridByMatrix(
          grid->getNXs(), grid->getDXs(), grid->getX0s(), grid->getRotMat(),
          VectorDouble(), flag_polarized, verbose))
      return nullptr;
    return mesh;
  }

  MeshETurbo* MeshETurbo::createFromCova(
    const CovAniso& cova,
    const Db* field,
    double ratio,
    Id nbExt,
    ...
    bool verbose)
  {
    ...
    if (mesh->initFromCova(
          cova, field, ratio, nbExt, isPolarized, useSel, nxmax, verbose))
      return nullptr;
    return mesh;
  }
  ...
  *****************************************************************************/
  ...
  bool MeshETurbo::_addElementToTriplet(
    NF_Triplet& NF_T,
    Id iech,
  ...
  VectorInt
    MeshETurbo::getMeshApicesFromCoordinates(const VectorDouble& coords) const
  {
    auto ncorner = getNApexPerMesh();
  ...
  Id MeshETurbo::getMeshAndInPlaceWeightsFromCoordinates(
    const VectorDouble& coords,
   ...
   */
  Id MeshETurbo::getMeshFromCoordinates(
    const VectorDouble& coor,
    VectorInt& indices,
  ...
  *****************************************************************************/
  void MeshETurbo::resetProjFromDb(
    ProjMatrix* m,
    const Db* db,
    Id rankZ,
    bool verbose) const
  {
    ...
    for (Id jech = 0; jech < db->getNSample(); jech++)
    {
      ...
      {
        messerr(
          "Sample #%d does not belong to the Turbo Meshing internal Grid",
          jech + 1);
        iech++;
        continue;
      }
      ...
      if (verbose)
        message(
          "Sample %4d assigned to Grid Node %4d :", jech + 1,
    ...
    if (verbose && nout > 0)
      messerr(
        "%d / %d samples which do not belong to the Meshing", nout,
        db->getNSample(true));
  ...
  *****************************************************************************/
...

  {
    ...
    for (Id idim = 0; idim < ndim; idim++)
    {
      ...
      _grid.setNX(
        idim, static_cast<Id>(
                ceil((getExtendMax(idim) - getExtendMin(idim)) / cellsize[idim])
                + 1));
    }
    ...
    bool verbose) const
  {
    ...
    for (Id idim = 0; idim < ndim; idim++) _rhs[idim] = coor[idim];
    _rhs[ndim] = 1;
    ...
    bool verbose)
  {
    ...
    for (Id icorner = 0; icorner < nval; icorner++)
    {
      ...
      else
      {
        for (Id idim = 0; idim < ndim; idim++)
          corner1[idim] =
            (ic[idim] == 0) ? extremesData[idim][0] : extremesData[idim][1];
      }
      corner1 -= cornerRef;
    ...
    if (cova.isNoStatForAnisotropy())
    {
      ...
      for (Id idim = 0; idim < ndim; idim++) dx[idim] = dxmin;
    ...
    initFromGridByMatrix(
      nx, dx, x0, rot.getMatrixDirectVec(), VectorDouble(), isPolarized,
      verbose);
    return 0;
  }
  ...
  bool MeshETurbo::_deserializeAscii(std::istream& is)
  {
    ...
    if (ret)
      (void)initFromGridByMatrix(
        nx, dx, x0, rotmat, VectorDouble(), static_cast<bool>(flag_polarized),
        0);
    ...
    {
      ret =
        ret && _recordReadVec<Id>(is, "Mesh Masking", relranks, nmesh_active);
      if (ret)
        _meshIndirect.buildFromRankRInA(relranks, _nmeshInCompleteGrid());
    }
    ...
    {
      ret =
        ret && _recordReadVec<Id>(is, "Grid Masking", relranks, ngrid_active);
  ...
  bool MeshETurbo::_serializeAscii(std::ostream& os) const
  {
    ...
    if (nmesh_mask > 0)
      ret =
        ret
        && _recordWriteVec<Id>(os, "Mesh Masking", _meshIndirect.getRelRanks());
    ...
    if (ngrid_mask > 0)
      ret =
        ret
        && _recordWriteVec<Id>(os, "Grid Masking", _gridIndirect.getRelRanks());
  ...
  bool MeshETurbo::deserializeH5(H5::Group& grp)
  {
    ...
    ret =
      ret && SerializeHDF5::readValue(*meshG, "MeshCountActive", nmesh_active);
    ret = ret && SerializeHDF5::readValue(*meshG, "MeshCountMask", nmesh_mask);
    if (nmesh_mask > 0)
    {
      ret = ret && SerializeHDF5::readVec(*meshG, "MeshMask", nmesh_ranks);
      if (ret)
        _meshIndirect.buildFromRankRInA(nmesh_ranks, _nmeshInCompleteGrid());
    }
    ...
    ret =
      ret && SerializeHDF5::readValue(*meshG, "GridCountActive", ngrid_active);
  ...
  bool MeshETurbo::serializeH5(H5::Group& grp) const
  {
    ...
    ret =
      ret && SerializeHDF5::writeValue(meshG, "Mode", _meshIndirect.getMode());

    // Dumping the Mesh Masking map
    Id nmesh_mask = static_cast<Id>(_meshIndirect.getRelRanks().size());

    ret =
      ret && SerializeHDF5::writeValue(meshG, "MeshCountActive", getNMeshes());
    ret = ret && SerializeHDF5::writeValue(meshG, "MeshCountMask", nmesh_mask);
    if (nmesh_mask > 0)
      ret = ret
         && SerializeHDF5::writeVec(
              meshG, "MeshMask", _meshIndirect.getRelRanks());

    // Dumping the Grid Masking map
    Id ngrid_mask = static_cast<Id>(_gridIndirect.getRelRanks().size());

    ret =
      ret && SerializeHDF5::writeValue(meshG, "GridCountActive", getNApices());
    ret = ret && SerializeHDF5::writeValue(meshG, "GridCountMask", ngrid_mask);
    if (ngrid_mask > 0)
      ret = ret
         && SerializeHDF5::writeVec(
              meshG, "GridMask", _gridIndirect.getRelRanks());
