...
namespace gstlrn
{
  ...
  Id meshes_2D_sph_from_db(Db* db, SphTriangle* t)
  {
    ...
    {
      messerr(
        "In Spherical System, the Space Dimension of the data base Db must be "
        "2 (%d)\n",
        ndim);
      return (1);
    }
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      GH::convertSph2Cart(
        db->getCoordinate(iech, 0), db->getCoordinate(iech, 1), &xx, &yy, &zz);
      t->sph_x[ecr] = xx;
      t->sph_y[ecr] = yy;
      t->sph_z[ecr] = zz;
      ecr++;
  ...
  Id meshes_2D_sph_from_auxiliary(const String& triswitch, SphTriangle* t)
  {
    ...
    for (Id lec = 0; lec < npoint; lec++)
    {
      ...
      for (Id i = 0; i < t->n_nodes && found_close < 0; i++)
      {
        ...
        dist = 0.;
        for (Id j = 0; j < 3; j++) dist += (c1[j] - c2[j]) * (c1[j] - c2[j]);
  ...
  void meshes_2D_sph_print(SphTriangle* t, Id brief)
  {
    ...
    if (!brief && t->n_nodes > 0 && !t->sph_x.empty() && !t->sph_y.empty()
        && !t->sph_z.empty())
    {
      message(
      ...
      for (Id i = 0; i < t->n_nodes; i++)
      {
        ...
        message(
          " Cartesian=%8.3lf %8.3lf %8.3lf - Long-Lat=%8.3lf %8.3lf\n",
          t->sph_x[i], t->sph_y[i], t->sph_z[i], rlong, rlat);
      }
      message("\n");
    }
  ...
  Id meshes_2D_sph_create(Id verbose, SphTriangle* t)
  {
    ...
    if (!skip_rnd)
    {
      ...
      for (I32 i = 0; i < 3; i++)
      {
        ...
        memo[i][2] = t->sph_z[i];
        cste =
          (t->sph_x[i] * t->sph_x[i] + t->sph_y[i] * t->sph_y[i]
           + t->sph_z[i] * t->sph_z[i]);
        t->sph_x[i] += law_uniform(-ampli, ampli);
        t->sph_y[i] += law_uniform(-ampli, ampli);
        value =
          sqrt(cste - t->sph_x[i] * t->sph_x[i] - t->sph_y[i] * t->sph_y[i]);
    ...
    (void)trmesh_(
      &t->n_nodes, t->sph_x.data(), t->sph_y.data(), t->sph_z.data(),
      t->sph_list.data(), t->sph_lptr.data(), t->sph_lend.data(),
      loc_lnew.data(), loc_near.data(), loc_next.data(), loc_dist.data(),
      &error);
    ...
    switch (error)
    {
      ...
      case -2: messerr("The first three nodes may not be colinear"); break;
