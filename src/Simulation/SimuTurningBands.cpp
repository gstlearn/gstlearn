...
SimuTurningBands::SimuTurningBands(int nbsimu, int nbtuba, int seed)
...
bool SimuTurningBands::_resize()
{
  ...
  if (nbsimu > 0 && nbtuba > 0 && _model != nullptr)
  {
    int nvar  = _getNVar();
    int ncova = _getNCova();
...
int SimuTurningBands::_generateDirections(const Db* dbout)
{
  if (_model == nullptr)
  {
    messerr("The 'model' must have been defined beforehand (calling 'init')");
    return 1;
  }
  double x[2];
  int ndim = _model->getDimensionNumber();
  int ncova = _model->getCovaNumber();
  int nbsimu = getNbSimu();
  int nbands = getNDirs();
  ...
  for (int isimu = 0; isimu < nbsimu; isimu++)
    for (int is = 0; is < ncova; is++)
      ...
      {
        const CovAniso* cova = _model->getCova(is);
...
void SimuTurningBands::_simulateNugget(Db *db, const VectorDouble& aic, int icase)
{
  ...
  {
    if (_model->getCovaType(is) == ECov::NUGGET) flag_used = true;
  }
  if (!flag_used) return;
...
{
  _model = model;
  if (_model == nullptr || getNbSimu() <= 0 || getNBtuba() <= 0)
  {
...
bool SimuTurningBands::_check() const
{
  // TODO Check the condition of ACalcSimulation first
  int ndim = _model->getDimensionNumber();
 ...
 **
 *****************************************************************************/
//int SimuTurningBands::simtub(Db *dbin,
//                             Db *dbout,
//                             Model *model,
//                             ANeighParam *neighparam,
//                             int nbsimu,
