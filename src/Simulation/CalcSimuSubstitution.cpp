...
namespace gstlrn
{
   ...
   **
   *****************************************************************************/
  bool CalcSimuSubstitution::_simulateCalculate()
  {
    ...
    double eps)
  {
    ...
    {
      double total = 0.;
      for (Id j = 0; j < nfacies; j++) total += ABS(TRANS(i, j));

      if (total <= 0.)
        flag_error = 1;
      else
        for (Id j = 0; j < nfacies; j++) TRANS(i, j) = ABS(TRANS(i, j)) / total;
    }
    ...
    if (flag_error)
    {
      for (Id i = 0; i < nfacies; i++)
        for (Id j = 0; j < nfacies; j++) TRANS(i, j) = 1. / nfacies;
    }

    /* Initialize the proportion matrix */

    for (Id i = 0; i < nfacies; i++) props[i] = 1. / nfacies;
    ...
    while (diff > eps)
    {
      double w0 = 0.;
      diff = 0.;
      for (Id i = 0; i < nfacies; i++) propold[i] = props[i];
      for (Id i = 0; i < nfacies; i++)
      {
        double val = 0.;
        for (Id j = 0; j < nfacies; j++) val += TRANS(j, i) * propold[j];
        props[i] = val;
        w0 += val;
      }
      if (w0 == 0.) w0 = 1.;
    ...
    if (verbose) printMatrix(props, 1, nfacies, "Proportions", 0, 1);
  ...
  bool CalcSimuSubstitution::_preprocess()
  {
    ...
    _iattOut = _addVariableDb(2, 1, ELoc::SIMU, 0, 1, 1);
    return (_iattOut >= 0);
  }
