...
namespace gstlrn
{
   ...
   *****************************************************************************/
  void
    ACalcSimulation::_correctStationaryMean(Db* dbout, Id icase, bool flagBayes)
  {
    if (flagBayes) return;
    auto nbsimu = getNbSimu();
    auto nvar = getNVar();
      ...
      for (Id iech = 0; iech < dbin->getNSample(); iech++)
      {
        ...
        for (Id ivar = 0; ivar < nvar; ivar++)
        {
          ...
          for (Id isimu = 0; isimu < nbsimu; isimu++)
          {
            ...
            double simval =
              dbin
                ->getSimvar(ELoc::SIMU, iech, isimu, ivar, icase, nbsimu, nvar);
            if (flag_dgm)
            {
        ...
        for (Id isimu = 0; isimu < nbsimu; isimu++)
          for (Id ivar = 0; ivar < nvar; ivar++)
          {
            ...
            if (!flag_pgs)
              ...
              valdat =
                dbin->getSimvar(ELoc::GAUSFAC, ip, isimu, 0, icase, nbsimu, 1);
