...
namespace gstlrn
{
  ...
  String SimuSubstitutionParam::toString(const AStringFormat* /*strfmt*/) const
  {
    ...
    if (_flagOrient)
      sstr << toStrVector(
        "Vector orthogonal to desorientation layering", _vector);
    sstr
   ...
   **
   *****************************************************************************/
  void SimuSubstitutionParam::isValidOrientation(
    VectorDouble& vector,
    bool verbose)
  {
  ...
  bool SimuSubstitutionParam::_isValidTransition(bool verbose, double eps)
  {
    ...
    for (Id irow = 0; irow < _nfacies; irow++)
    {
      ...
      if (ABS(total - 1.) > eps)
      {
        if (verbose)
          messerr(
            "Transition: Sum of elements of row(%d) must be 1 (%lf)", irow + 1,
            total);
        return false;
      }
    }
