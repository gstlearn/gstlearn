...
namespace gstlrn
{
  ...
  MatrixDense SpectrumOnRN::computeToMatrix(Db* dbout)
  {
    auto nvar = getNVar();
    auto nech = dbout->getNSample();
    ...
    const VectorDouble& xi0)
  {
    ...
    {
      messerr(
        "Inconsistent dimensions of the frequency matrix: nrow = %d, ncol = %d "
        "(ns = %d, nv =  %d)",
    ...
    {
      messerr(
        "Inconsistent length of the phase vector: length = %d (ns = %d)",
        phi.length(), _ns);
      return false;
    }
    if ((omega0.getNRows() != 0)
        && ((omega0.getNRows() != _ns) || (omega0.getNCols() != _nd)))
    {
      messerr(
        "Inconsistent dimensions of the raw frequency matrix: nrow = %d, ncol "
        "= %d (ns = %d, nv =  %d)",
    ...
    {
      messerr(
        "Inconsistent length of the mixture coefficient vector: length = %d "
        "(ns = %d)",
        xi0.length(), _ns);
      return false;
    }
  ...
  void SpectrumOnRNSimple::_computeValues(
  ...
  SpectrumOnRNFactorized::SpectrumOnRNFactorized(
    const SpectrumOnRNFactorized& r)
    : SpectrumOnRN(r)
    , _nf(0)
    , _omegas()
    , _phis()
    ...
    const VectorDouble& xi0)
  {
    ...
    {
      messerr(
        "Inconsistent dimensions of the projection matrix: nrow = %d, ncol = "
        "%d (nd = %d)",
  ...
  void SpectrumOnRNFactorized::_computeValues(
    const VectorDouble& coor,
    VectorDouble& values)
  {
    for (Id is = 0; is < _ns; is++)
  ...
  SpectrumOnRNList::SpectrumOnRNList(Id nv, Id nd, Id ns)
    ...
    , _sps(){DECLARE_UNUSED(ns)} SpectrumOnRNList::SpectrumOnRNList(
        const SpectrumOnRNList& r)
    : SpectrumOnRN(r)
    , _sps()
  {
  ...
  bool SpectrumOnRNList::addSpectrum(std::unique_ptr<SpectrumOnRN> sp)
  {
    ...
    {
      messerr(
        "Inconsistent number of variables = %d (vs. nv = %d)", sp->getNVar(),
        getNVar());
      return false;
    }
    ...
    {
      messerr(
        "Inconsistent space dimension = %d (vs. nd = %d)", sp->getNDim(),
        getNDim());
      return false;
    }
    _ns += sp->getNs();
  ...
  void SpectrumOnRNList::_computeValues(
    const VectorDouble& coor,
    VectorDouble& values)
  {
