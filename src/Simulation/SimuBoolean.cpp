...
namespace gstlrn
{
  ...
  SimuBoolean::SimuBoolean(Id nbsimu, Id seed)
    ...
    Id iptr_rank)
  {
    for (Id iobj = 0; iobj < _getNObjects(); iobj++)
    {
      _objlist[iobj]->projectToGrid(
        dbout, iptr_simu, iptr_rank, static_cast<Id>(boolparam.getFacies()),
        iobj + 1);
    }
  }

  Id SimuBoolean::_countConditioningPore(const Db* db)
  {
  ...
  Id SimuBoolean::_countConditioningGrain(const Db* db)
  {
    if (db == nullptr) return 0;
  ...
  Id SimuBoolean::_getRankUncovered(const Db* db, Id rank) const
  {
    Id number = 0;
    for (Id iech = 0; iech < db->getNSample(); iech++)
    {
  ...
  Id SimuBoolean::_generatePrimary(
    Db* dbin,
    DbGrid* dbout,
    ...
    bool verbose)
  {
    ...
    Id nbpore = _countConditioningPore(dbin);
    Id nbgrain = _countConditioningGrain(dbin);
    ...
    while (draw_more)
    {
      ...
      {
        messerr(
          "Simulation of the initial objects failed after %d iterations",
      ...
      BooleanObject* object =
        BooleanObject::generate(dbout, cdgrain, tokens, boolparam);
  ...
  Id SimuBoolean::_generateSecondary(
    Db* dbin,
    DbGrid* dbout,
    const ModelBoolean* tokens,
    const SimuBooleanParam& boolparam,
    bool verbose)
  {
    ...
    while (tabtime < boolparam.getTmax())
    {
      ...
      if (law_uniform(0., 1.) <= ratio)
      {
        ...
        BooleanObject* object =
          BooleanObject::generate(dbout, VectorDouble(), tokens, boolparam);
        if (object == nullptr) continue;
  ...
  Id SimuBoolean::_getObjectRank(Id mode, Id rank)
  {
    Id nb_objects = _getNObjects();

    Id number = 0;
    for (Id iobj = 0; iobj < nb_objects; iobj++)
    {
   ...
   **
   *****************************************************************************/
  Id SimuBoolean::_deleteObject(Id mode, Db* dbin)
  {
  ...
  Id SimuBoolean::_getAverageCount(
    const DbGrid* dbout,
    const ModelBoolean* tokens,
    const SimuBooleanParam& boolparam)
  {
    ...
    const NamingConvention& namconv)
  {
    ...
    if (simubool.simulate(
          dbin, dbout, tokens, boolparam, iptr_simu, iptr_rank, iptr_cover,
          verbose))
      return 1;

    // Delete the working variable corresponding to the cover index

    if (iptr_cover >= 0) dbin->deleteColumnByUID(iptr_cover);

    namconv.setNamesAndLocators(
      dbin, VectorString(), ELoc::Z, 1, dbout, iptr_simu, "Facies", 1, false);
    namconv.setNamesAndLocators(
      dbin, VectorString(), ELoc::Z, 1, dbout, iptr_rank, "Rank", 1, false);
    return 0;
  }
