...
namespace gstlrn
{
   ...
   **
   *****************************************************************************/
  bool CalcSimuEden::_simulateCalculate()
  {
  ...
  {
    bool answer = _getFACIES(ipos) > 0 && _getPERM(ipos) > 0
...

  {
    ...
    for (Id iech = 0; iech < _nxyz; iech++)
    {
      ...
      if (ifluid > 0)
        dbgrid->updArray(iech, _iptrStatFluid + ifluid - 1, EOperator::ADD, 1);
  ...
  {
    Id count = 0;
    for (Id lec = 0; lec < _nxyz; lec++) count += isAlreadyFilled(lec);
    return count;
  }
  ...
  {
    Id count = 0;
    for (Id lec = 0; lec < _nxyz; lec++) count += isToBeFilled(lec);
    return count;
  }
  ...
  bool CalcSimuEden::_preprocess()
  {
    ...
    {
      _iptrStatFluid =
        _addVariableDb(2, 1, ELoc::UNDEFINED, 0, _nfluids, 1, 0.);
      if (_iptrStatFluid < 0) return false;
      _iptrStatCork = _addVariableDb(2, 1, ELoc::UNDEFINED, 0, 1, 1, 0.);
      if (_iptrStatCork < 0) return false;
    }

    /* Add the attributes for storing the Fluid and Data informations */

    _iptrFluid = _addVariableDb(2, 1, ELoc::UNDEFINED, 0, 1, 1, 0.);
    if (_iptrFluid < 0) return false;
    _iptrDate = _addVariableDb(2, 1, ELoc::UNDEFINED, 0, 1, 1, TEST);
    return (_iptrDate >= 0);
  }
