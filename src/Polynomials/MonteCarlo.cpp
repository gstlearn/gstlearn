...
namespace gstlrn
{
  ...
  double integralGaussHermite(double yc, double r, const VectorDouble& psi)
  {
    ...
    VectorDouble vec = hermitePolynomials(yc, 1., nbpoly);
    double value = hermiteSeries(vec, psi);
    return r * r * value;
  }
  ...
  double
    MCIndicatorElement(double yc, double krigest, double krigstd, Id nbsimu)
  {
    double proba = 0.;
    for (Id isimu = 0; isimu < nbsimu; isimu++)
    {
  ...
  double
    MCIndicatorStdElement(double yc, double krigest, double krigstd, Id nbsimu)
  {
