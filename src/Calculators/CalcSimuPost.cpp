...
namespace gstlrn
{
  ...
  bool CalcSimuPost::_check()
  {
    ...
    {
      messerr("The Space Dimension of Dbin(%d) must not be larger than the one "
              "of Dbout(%d)",
              getDbin()->getNDim(),
              getDbout()->getNDim());
      return false;
   ...
   */
  void CalcSimuPost::_readIn(Id iech,
  ...
  Id CalcSimuPost::_defineNames()
  {
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
    {
      ...
      if (nfois <= 0)
      {
        messerr("The variable (%s) does not seem to exist in the input Db",
                _names[ivar].c_str());
        return 1;
      }
      _nfact[ivar] = nfois;
      ...
      for (Id ifois = 0; ifois < nfois; ifois++)
      {
        ...
        {
          messerr("The variable (%s) does not have a propoer UID",
