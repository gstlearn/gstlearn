...
namespace gstlrn
{
  ...
  bool CalcGridToGrid::_check()
  {
    ...
    {
      messerr("This application requires %d variable(s) to be defined",
              nvar_requested);
      return false;
    }
    ...
    if (_flagExpand)
    {
      if (_compareInMinusOut() >= 0)
      {
        messerr("The Space dimension of 'dbout' should be larger then the one "
                "of 'dbin'");
        return false;
      }
    }
    ...
    if (_flagShrink)
    {
      if (_compareInMinusOut() <= 0)
      {
        messerr("The Space dimension of 'dbout' should be smaller then the one "
                "of 'dbin'");
        return false;
      }
    }
