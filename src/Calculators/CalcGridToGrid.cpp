...
namespace gstlrn
{
  ...
  bool CalcGridToGrid::_check()
  {
    ...
    {
      messerr(
        "This application requires %d variable(s) to be defined",
        nvar_requested);
      return false;
    }
    ...
    if (_flagExpand)
    {
      ...
      {
        messerr(
          "The Space dimension of 'dbout' should be larger then the one of "
          "'dbin'");
        return false;
      }
    }
    ...
    if (_flagShrink)
    {
      ...
      {
        messerr(
          "The Space dimension of 'dbout' should be smaller then the one of "
          "'dbin'");
        return false;
      }
    }
    ...
    if (_flagInter)
    {
      ...
      {
        messerr(
          "The argument 'nameBots' (%d) should be dimensioned to %d",
      ...
      {
        messerr(
          "The argument 'nameTops' (%d) should be dimensioned to %d",
  ...
  bool CalcGridToGrid::_preprocess()
  {
    ...
    if (_flagShrink)
    {
      _iattAux = _addVariableDb(2, 2, ELoc::UNDEFINED, 0, 1, 1, 0.);
  ...
  {
    if (_flagCopy) return _g2gCopy();

    if (_flagExpand) return _g2gExpand();

    if (_flagShrink) return _g2gShrink();

    if (_flagInter) return _g2gInter();

    return false;
  }

  Id dbg2gCopy(DbGrid* dbin, DbGrid* dbout, const NamingConvention& namconv)
  {
  ...
  Id dbg2gExpand(DbGrid* dbin, DbGrid* dbout, const NamingConvention& namconv)
  {
  ...
  Id dbg2gShrink(DbGrid* dbin, DbGrid* dbout, const NamingConvention& namconv)
  {
    CalcGridToGrid calcul;
    calcul.setDbin(dbin);
    calcul.setDbout(dbout);
   ...
   */
  void
    CalcGridToGrid::_reduceIndices(const VectorInt& indgIn, VectorInt& indgOut)
  {
    Id ndimOut = static_cast<Id>(indgOut.size());

    for (Id i = 0; i < ndimOut; i++) indgOut[i] = indgIn[i];
  }
   ...
   */
  double CalcGridToGrid::_interpolate(
    Id nvar,
    double valTop,
    double valBot,
  ...
  bool CalcGridToGrid::_loadExtrema(
    Id nvar,
    Id iech,
