...
namespace gstlrn
{
   ...
   **
   *****************************************************************************/
  static Id st_multilinear_evaluate(
    DbGrid* db_grid,
  ...
  static double st_distance_modify(
    DbGrid* dbgrid,
    Id ig,
    Db* dbpoint,
    Id ip,
    ...
    Id iatt_scalew)
  {
    ...
    if (iatt_time >= 0)
    {
      ...
      if (time > 0) dd = (FFFF(dd)) ? time : dd + time;
    }
    return dd;
  }
    ...
    VectorDouble& dvect)
  {
    ...
    {
      double dd2 = st_distance_modify(
        dbgrid, ig, dbpoint, ip, dvect, flag_aniso, iatt_time, iatt_angle,
   ...
   **
   *****************************************************************************/
  static Id st_locate_point_on_grid(
    const Db* db_point,
   ...
   **
   *****************************************************************************/
  static Id st_locate_coor_on_grid(
    Id np,
    ...
    VectorDouble& tab)
  {
    ...
    {
      tab[iech] = TEST;
      for (Id idim = 0; idim < ndim; idim++) local[idim] = coords[idim][iech];
    ...
    VectorDouble& tab2)
  {
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      ...
      else
      {
        ...
        for (Id idim = 0; idim < ndim; idim++)
          for (Id ifois = -1; ifois <= 1; ifois += 2)
            for (Id irad = 0; irad < radius; irad++)
            {
              for (Id jdim = 0; jdim < ndim; jdim++) indg[jdim] = indg0[jdim];
   ...
   **
   *****************************************************************************/
  Id CalcMigrate::_migrateGridToPoint(
    DbGrid* db_grid,
    Db* db_point,
    Id iatt,
    Id distType,
    ...
    VectorDouble& tab)
  {
    ...
    Id ndim_max = MAX(db_grid->getNDim(), db_point->getNDim());
  ...
  bool CalcMigrate::_check()
  {
    ...
    {
      messerr(
        "Argument 'dist_type'(%d)  should be 1 (for L1 distance) or 2 (for L2 "
        "distance",
        _distType);
      return false;
    }
  ...
  bool CalcMigrate::_postprocess()
  {
    ...
    auto nvar = _getNVar();
    _renameVariable(
      2, getDbin()->getNamesByUID(_iuids), ELoc::UNDEFINED, nvar, _iattOut,
      String(), 1);
    ...
    bool* flag_created)
  {
    ...
    for (Id info = 0; info < ninfo; info++)
    {
      Id jatt = dbin->getUIDByLocator(locatorType, info);
   ...
   * @remarks If checkOrder<0, if surf_N > surf_N-1 then surf_N = surf_N-1
   */
  VectorVectorDouble interpolateVariablesToPoint(
    DbGrid* dbgrid,
    ...
    bool flagCap)
  {
    ...
    for (Id isurf = 0; isurf < nsurf; isurf++)
      if (interpolateVariableToPoint(
            dbgrid, iatts[isurf], xp, yp, zp, tab[isurf]))
        break;
  ...
  VectorDouble interpolateOneVariableToPoint(
    DbGrid* dbgrid,
    const VectorDouble& xp,
    const VectorDouble& yp,
    const VectorDouble& zp,
    ...
    VectorDouble& tab)
  {
    ...
    if ((ndim >= 1 && xp.empty()) || (ndim >= 2 && yp.empty())
        || (ndim >= 3 && zp.empty()))
    {
   ...
   *****************************************************************************/
  VectorDouble dbgridLineSampling(
    DbGrid* dbgrid,
    ...
    VectorDouble& tab)
  {
    ...
    Id ng = db_grid->getNSample();
    Id np = db_point->getNSample(true);
    VectorDouble dvect(ndim_max);
    ...
    for (Id ig = 0; ig < ng; ig++)
    {
      ...
      for (Id ip = ip0; ip >= 0; ip--)
      {
        ...
        st_get_closest_sample(
          db_grid, ig, db_point, jp, flag_aniso, iatt_time, iatt_angle,
          iatt_scaleu, iatt_scalev, iatt_scalew, &jpmin, &ddmin, dvect);
      }
    ...
    VectorDouble& tab)
  {
    ...
    if (ndim != ndimp)
    {
      ...
      messerr(
        "must be equal to the Space Dimension of the coordinate arrays", ndimp);
      return 1;
    }
    ...
    Id iatt_scalew)
  {
    ...
    if (flag_index)
    {
      for (Id i = 0; i < dbgrid->getNSample(); i++) tab2[i] = tab1[i];
      goto label_suite;
    }
    ...
    for (Id iech = 0; iech < dbgrid->getNSample(); iech++)
    {
      ...
      lec = 0;
      for (Id ivois = 0; ivois < nvois; ivois++)
      {
        for (Id idim = 0; idim < ndim; idim++)
        ...
        else
        {
          ...
          tab2[iech] =
            (iatt_size >= 0) ? dbpoint->getArray(val_iech, iatt_size) : 1.;
        }
      }
    }
    ...
    if (iatt_size >= 0)
    {
      for (Id i = 0; i < dbgrid->getNSample(); i++) tab1[i] = tab2[i];
      st_expand(flag_size, dbgrid, tab1, indg0, indg, tab2);
    }

    /* Transform values into 0 and 1 */

    for (Id i = 0; i < dbgrid->getNSample(); i++)
    {
      if (flag_size)
  ...
  label_suite:
    dbgrid->setColumnByUID(tab2, iatt_edge);
    ...
    VectorDouble& tab)
  {
    ...
    if (db_grid->getNDim() != ndim)
    {
      ...
      messerr(
        "must be equal to the Space Dimension of the coordinate arrays", ndim);
      return 1;
    }
    ...
    bool flag_ball)
  {
    ...
    else
    {
      ...
      {
        if (_expandPointToPoint(db1, db2, iatt1, distType, dmax, tab)) return 1;
      }
    }
   ...
   **
   *****************************************************************************/
  Id CalcMigrate::_migratePointToGrid(
    Db* db_point,
    DbGrid* db_grid,
    Id iatt,
    Id distType,
    ...
    VectorDouble& tab)
  {
    ...
    if (OptDbg::query(EDbg::DB))
      message(
        "Number of nodes directly assigned = %d/%d\n", nb_assign,
        db_grid->getNSample());
   ...
   **
   *****************************************************************************/
  Id CalcMigrate::_expandPointToPointBall(
    Db* db1,
    Db* db2,
    Id iatt,
    Id distType,
    ...
    VectorDouble& tab)
  {
    ...
    for (Id inode = 0; inode < db2->getNSample(); inode++)
    {
      if (!db2->isActive(inode)) continue;
      db2->getCoordinatesInPlace(coor, inode);
      Id iech = ball.queryClosest(coor);
   ...
   **
   *****************************************************************************/
  Id CalcMigrate::_migrateGridToGrid(
    DbGrid* db_gridin,
    DbGrid* db_gridout,
    Id iatt,
    Id distType,
    ...
    VectorDouble& tab)
  {
    ...
    Id ndim_max = MAX(db_gridin->getNDim(), db_gridout->getNDim());
    ...
    for (Id iech = 0; iech < db_gridin->getNSample(); iech++)
    {
      ...
      if (jech < 0) continue;
      double dist_loc =
        distance_inter(db_gridin, db_gridout, iech, jech, dvect);
      if (st_larger_than_dmax(ndim_min, dvect, distType, dmax)) continue;
   ...
   **
   *****************************************************************************/
  Id CalcMigrate::_expandPointToPoint(
    Db* db1,
    Db* db2,
    Id iatt,
    Id distType,
    ...
    VectorDouble& tab)
  {
    ...
    Id ndim_max = MAX(db1->getNDim(), db2->getNDim());
   ...
   **
   *****************************************************************************/
  Id CalcMigrate::_expandGridToGrid(
    DbGrid* db_gridin,
    DbGrid* db_gridout,
    Id iatt,
    Id distType,
    ...
    VectorDouble& tab)
  {
    ...
    for (Id iech = 0; iech < db_gridout->getNSample(); iech++)
    {
   ...
   **
   *****************************************************************************/
  Id CalcMigrate::_interpolateGridToPoint(
    DbGrid* db_grid,
    Db* db_point,
    Id iatt,
    Id distType,
