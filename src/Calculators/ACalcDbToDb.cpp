...
namespace gstlrn
{
    ...
    if (_dbin != nullptr)
    {
      if (nvar > 0)
      {
        if (nvar != _dbin->getNLoc(ELoc::Z))
        {
          ...
          messerr(
            "- Number of variables in 'dbin' = %d", _dbin->getNLoc(ELoc::Z));
          return false;
        }
      }
      else
  ...
  Id ACalcDbToDb::_expandInformation(Id mode, const ELoc& locatorType) const
  {
    ...
    if (!getDbout()->isGrid())
    {
      ...
      messerr(
        "The Input Db does not contain the correct number of External Drifts");
      return 1;
    }
    ...
    if (mode > 0)
    {
      ...
      Id error = migrateByLocator(
        dbgrid, getDbin(), locatorType, 1, VectorDouble(), false, false, false,
        *namconv);
      delete namconv;
      if (error != 0) return 1;
    }
