...
namespace gstlrn
{
   ...
   */
  void
    ACalcDbVarCreator::_storeInVariableList(Id status, const VectorInt& iuids)
  {
    ...
    if (status == 1)
    {
      for (Id i = 0; i < number; i++) _listVariablePermDb.push_back(iuids[i]);
    }
    else
    {
      for (Id i = 0; i < number; i++) _listVariableTempDb.push_back(iuids[i]);
    }
  }
  ...
  {
    if (_db == nullptr) return -1;
    Id iuid = _db->addColumnsByConstant(number,
                                        valinit,
                                        String(),
                                        locatorType,
                                        locatorIndex);
  ...
  void ACalcDbVarCreator::_renameVariable(Id nvar,
                                          ...
                                          Id count)
  {
    _namconv.setNamesAndLocators(_db,
                                 VectorString(),
                                 locatorInType,
                                 nvar,
                                 _db,
  ...
  bool ACalcDbVarCreator::hasDb(bool verbose) const
  {
    if (_db == nullptr)
    {
      if (verbose) messerr("The argument 'db' must be defined");
      return false;
    }
    return true;
  }
