...
namespace gstlrn
{
  ...
  bool CalcSimuPostPropByLayer::_check()
  {
    ...
    {
      messerr(
        "The current version has been coded for a Space Dimension of 'dbout' "
        "(%d) limited to 3",
        ndim_out);
      return false;
    }
    ...
    {
      messerr(
        "The current version has been coded for a non-rotated output grid");
      return false;
    }
    return true;
  }
   ...
   *
   */
  void CalcSimuPostPropByLayer::_transformFunction(
    const VectorDouble& Z_n_k_s,
    VectorDouble& Y_p_k_s) const
  {
    ...
    for (Id ilayer = 0; ilayer <= nlayer; ilayer++) Y_p_k_s[ilayer] /= h_max;
