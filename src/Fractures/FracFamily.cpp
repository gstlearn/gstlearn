...
namespace gstlrn
{
  ...
  String FracFamily::toString(const AStringFormat* /*strfmt*/) const
  {
    ...
    sstr << "Average Fault Orientation       = " << _orient << " (degree)"
         << std::endl;
    sstr << "Tolerance for Orientation       = " << _dorient << " (degree)"
         << std::endl;
  ...
  bool FracFamily::_deserializeAscii(std::istream& is)
  {
    ...
    ret =
      ret && _recordRead<double>(is, "Reference Poisson intensity", _theta0);
    ret = ret
       && _recordRead<double>(
            is, "Power dependency between layer and intensity", _alpha);
    ret = ret
       && _recordRead<double>(
            is, "Ratio of constant vs. shaped intensity", _ratcst);
    ret = ret
       && _recordRead<double>(
            is, "Survival probability (constant term)", _prop1);
    ret = ret
       && _recordRead<double>(
            is, "Survival probability (length dependent term)", _prop2);
    ret = ret
       && _recordRead<double>(
            is, "Survival probability (cumulative length exponent)", _aterm);
    ret = ret
       && _recordRead<double>(
            is, "Survival probability (layer thickness exponent", _bterm);
    ret =
      ret && _recordRead<double>(is, "Fracture repulsion area Range", _range);
    return ret;
  }
  ...
  bool FracFamily::_serializeAscii(std::ostream& os) const
  {
    ...
    ret =
      ret && _recordWrite<double>(os, "Tolerance for orientation", _dorient);
    ret =
      ret && _recordWrite<double>(os, "Reference Poisson intensity", _theta0);
    ret = ret
       && _recordWrite<double>(
            os, "Power dependency between layer and intensity", _alpha);
    ret = ret
       && _recordWrite<double>(
            os, "Ratio of constant vs. shaped intensity", _ratcst);
    ret = ret
       && _recordWrite<double>(
            os, "Survival probability (constant term)", _prop1);
    ret = ret
       && _recordWrite<double>(
            os, "Survival probability (length dependent term)", _prop2);
    ret = ret
       && _recordWrite<double>(
            os, "Survival probability (cumulative length exponent)", _aterm);
    ret = ret
       && _recordWrite<double>(
            os, "Survival probability (layer thickness exponent)", _bterm);
    ret =
      ret && _recordWrite<double>(os, "Fracture repulsion area Range", _range);
    return ret;
  }
