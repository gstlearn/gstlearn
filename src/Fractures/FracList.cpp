...
namespace gstlrn
{
  ...
  String FracList::toString(const AStringFormat* strfmt) const
  {
    ...
    sstr << "Current number of simulated fractures = " << getNFracs()
         << std::endl;

    /* Loop on the fractures */

    for (Id ifrac = 0; ifrac < getNFracs(); ifrac++)
   ...
   **
   *****************************************************************************/
  Id FracList::simulate(
    const FracEnviron& envir,
    bool flag_sim_layer,
    bool flag_sim_fract,
    Id seed,
    bool verbose,
    const VectorDouble& elevations)
  {
    ...
    double y0 = 0.;
    Id ninfos = 1 + envir.getNFamilies() * NPART;
    ...
    _nlayers = static_cast<Id>(thicks.size());
    ...
    for (Id ifault = 0; ifault < envir.getNFaults(); ifault++)
    {
    ...
    if (_verbose) message("Number of main faults        = %d \n", getNFracs());

    if (!flag_sim_fract) return 0;

    /* Loop on the different fault families */

    for (Id ifam = 0; ifam < envir.getNFamilies(); ifam++)
    {
      if (_verbose)
      ...
      for (Id ilayer = 0; ilayer < _nlayers; ilayer++)
      {
        ...
        Id nfracs =
          _simulateFractures(envir, family, ifam, cote, thick, theta2, denstab);
        ...
        _setMemFrac(ilayer, ifam, static_cast<double>(nfracs));
        _setMemTotal(ilayer, ifam, static_cast<double>(getNFracs()));
      }
    }
   ...
   *****************************************************************************/
  VectorDouble
    FracList::_layersManage(const FracEnviron& envir, double* y0) const
  {
    VectorDouble thicks;
    ...
    *y0 = cote_down;
    ...
    if (_verbose)
    {
      Id number = static_cast<Id>(thicks.size());
   ...
   *****************************************************************************/
  VectorDouble
    FracList::_layersRead(const VectorDouble& elevations, double* y0) const
  {
    VectorDouble thicks;

    /* Initializations */

    Id nlayers = static_cast<Id>(elevations.size());
    thicks.resize(nlayers - 1);

    double cote = elevations[0];
    for (Id i = 1; i < nlayers; i++)
    {
    ...
    double* xp)
  {
    ...
    *xp = desc.getXXF(desc.getNPoint() - 1);
    ...
    if (_verbose)
      message(
        "- Adding fracture: (%lf; %lf) to (%lf; %lf)\n", xx, cote,
        desc.getXXF(desc.getNPoint() - 1), desc.getYYF(desc.getNPoint() - 1));
   ...
   **
   *****************************************************************************/
  void FracList::_checkFractureIntersect(double cote, Id ifrac0)
  {
  ...
  bool FracList::_belongToLayer(
    ...
    double* ye) const
  {
    for (Id i = 0; i < desc.getNPoint() - 1; i++)
    {
  ...
  double FracList::_layerIntensity(const FracFamily& family, double thick) const
  {
    ...
    if (_verbose) message("Initial Intensity                 = %lf\n", theta1);
   ...
   **
   *****************************************************************************/
  void FracList::_generateDensity(
    const FracEnviron& envir,
    ...
    VectorDouble& denstab)
  {
    ...
    for (Id idisc = 0; idisc < _ndisc; idisc++) denstab[idisc] = 0.;
    ...
    if (_verbose)
      message(
        "- Cumulated Distribution: Main Fault = %lf\n",
    ...
    VectorDouble& denstab)
  {
    ...
    if (_verbose)
      message(
        "- Cumulated Distribution: Previous families = %lf\n",
        _densityCumulate(denstab));
  }
   ...
   *****************************************************************************/
  double
    FracList::_deriveIntensity(double theta1, double thetap, double propsur)
      const
  {
   ...
   **
   *****************************************************************************/
  double FracList::_extendFractures(
    const FracFamily& family,
    Id ifam,
    double cote,
    double thick,
    VectorDouble& denstab)
  {
    ...
    double propsur = (ntotal > 0)
                     ? static_cast<double>(next) / static_cast<double>(ntotal)
                     : 0.;
    if (_verbose && ntotal > 0)
    {
      message(
        "Survival proportion               = %lf (%d from %d)\n", propsur, next,
        ntotal);
      message(
        "- Cumulated Distribution: After survival = %lf\n",
        _densityCumulate(denstab));
    }
    return (propsur);
  }
   ...
   **
   *****************************************************************************/
  bool FracList::_sameFaultSide(const FracEnviron& envir, Id ifault0, double x0)
  {
    double x1, x2;
    Id ifault;
   ...
   **
   *****************************************************************************/
  double FracList::_densityUpdate(
    const FracFault& fault,
    Id side,
    Id ifam,
    double cote,
    double xx)
  ...
  {
    double total = 0.;
    for (Id idisc = 0; idisc < _ndisc; idisc++) total += denstab[idisc];
    return total;
  }
   ...
   *****************************************************************************/
  void
    FracList::_updateRepulsion(double x0, double range, VectorDouble& denstab)
  {
   ...
   **
   *****************************************************************************/
  bool FracList::_fractureInterrupt(
    const FracFamily& family,
    const FracDesc& desc,
    double thick)
  {
   ...
   **
   *****************************************************************************/
  double FracList::_fractureExtension(
    const FracDesc& desc,
    double cote,
    double dcote)
  {
    ...
    for (Id i = 0; i < desc.getNPoint() - 1; i++)
    {
      ...
      if (
        !FFFF(cote)
        && (desc.getYYF(i) < cote - dcote || desc.getYYF(i + 1) < cote - dcote))
        continue;
   ...
   **
   *******************************************************************F**********/
  Id FracList::_simulateFractures(
    const FracEnviron& envir,
    const FracFamily& family,
    Id ifam,
    double cote,
    ...
    VectorDouble& denstab)
  {
    ...
    Id nfracs = law_poisson(theta * envir.getXextend());

    /* Loop on the fractures to be simulated */

    Id neff = 0;
    for (Id ifrac = 0; ifrac < nfracs; ifrac++)
    {
      ...
      double angle =
        law_uniform(MAX(-90., orient - dorient), MIN(+90., orient + dorient));
   ...
   **
   *****************************************************************************/
  Id FracList::_getDiscretizedRank(double cumdens, const VectorDouble& denstab)
    const
  {
    double local = 0.;
    for (Id idisc = 0; idisc < _ndisc; idisc++)
  ...
  MatrixDense FracList::fractureExport() const
  {
    ...
    for (Id ifrac = 0; ifrac < getNFracs(); ifrac++)
    {
      Id npoint = _descs[ifrac].getNPoint();
      for (Id ip = 0; ip < npoint; ip++)
      {
        segs.setValue(ifrac, 0, static_cast<double>(ifrac) + 1);
        segs.setValue(ifrac, 1, static_cast<double>(ip) + 1);
        segs.setValue(ifrac, 2, static_cast<double>(_descs[ifrac].getFamily()));
        ...
        segs.setValue(ifrac, 6, static_cast<double>(ip == 0));
      }
    }
  ...
  Id FracList::_getEndPointCount() const
  {
    Id number = 0;
    for (Id i = 0; i < getNFracs(); i++) number += _descs[i].getNPoint();
    return (number);
  }

  bool FracList::_isValidDisc(Id idisc) const
  {
   ...
   **
   *****************************************************************************/
  FracList* FracList::fractureImport(
    const VectorDouble& frac_segs,
    const VectorDouble& layinfo,
    Id nfamilies)
  {
    Id nvalf = static_cast<Id>(frac_segs.size());
    if (!isMultiple(nvalf, NBYFRAC))
    {
      messerr(
        "The number of values in 'frac_segs' (%d) should be a multiple of %d",
        nvalf, NBYFRAC);
      return nullptr;
    }
    Id nseg = nvalf / NBYFRAC;
    ...
    if (!layinfo.empty())
    {
      nvall = static_cast<Id>(layinfo.size());
      number = 1 + nfamilies * NPART;
      if (!isMultiple(nvall, number))
      {
        messerr(
          "The number of values in 'layinfo' (%d) should be a multiple of %d",
          nvall, number);
        return nullptr;
      }
      nlayer = nvall / number;
    ...
    for (Id i = 0; i < nseg; i++)
    {
      Id ifam = static_cast<Id>(FRACSEG(i, 2));
    ...
    const NamingConvention& namconv)
  {
    ...
    double delta = dmin / static_cast<double>(ndisc);
    ...
    for (Id ifrac = 0; ifrac < getNFracs(); ifrac++)
    {
      ...
      double perm = 0.;
      if (ifam < static_cast<Id>(permtab.size())) perm = permtab[ifam];
      if (perm <= 0.) continue;
      ...
      if (verbose)
        message(
          "Fracture %d/%d (Number of end-points = %d)\n", ifrac + 1,
          getNFracs(), npoint);

      double permval = permtab[desc.getFamily()];
      for (Id ip = 0; ip < npoint - 1; ip++)
      {
        Id jp = ip + 1;
        _plungeSegment(
          dbgrid, iptr, delta, permval, desc.getXXF(ip), desc.getYYF(ip),
    ...
    double y2)
  {
    ...
    Id number = static_cast<Id>(floor(dist / delta));
    ...
    for (Id i = 0; i <= number; i++)
    {
      ...
      Id iech = dbgrid->coordinateToRank(coor);
   ...
   **
   *****************************************************************************/
  VectorDouble FracList::fractureToWell(
    Id nval,
    ...
    Id* ncol)
  {
    ...
    {
      messerr(
        "The number of values read from 'well' should be a multiple of 2");
      return VectorDouble();
    }
    Id nw_xy = nval / 2;
    if (nw_xy < 2)
    ...
    for (Id iw = 0; iw < nw_xy - 1; iw++)
    {
      ...
      for (Id ifrac = 0; ifrac < getNFracs(); ifrac++)
      {
        ...
        {
          Id jp = ip + 1;
          if (!GH::segmentIntersect(
                x1, y1, x2, y2, desc.getXXF(ip), desc.getYYF(ip),
                desc.getXXF(jp), desc.getYYF(jp), &x, &y))
            continue;
    ...
    (*nint) = static_cast<Id>(wellout.size()) / NBYWOUT;
    (*ncol) = NBYWOUT;
    return wellout;
  }
    ...
    double perm)
  {
    ...
    nloc = static_cast<Id>(wellout.size()) / NBYWOUT;
    wellout.resize(NBYWOUT * (nloc + 1));

    WELLOUT(nloc, 0) = x; /* First coordinate */
    WELLOUT(nloc, 1) = y; /* Second coordinate */
    WELLOUT(nloc, 2) =
      static_cast<double>(ifrac) + 1; /* Rank of the fracture */
    WELLOUT(nloc, 3) =
      static_cast<double>(ip) + 1; /* Rank of segment in frature */
    WELLOUT(nloc, 4) = static_cast<double>(family); /* Rank of family */
   ...
   *****************************************************************************/
  VectorDouble
    FracList::fractureExtractLength(Id ifam, double cote, double dcote)
  {
   ...
   **
   *****************************************************************************/
  VectorDouble FracList::fractureExtractDist(Id ifam, double cote, double dcote)
  {
    ...
    for (Id i = 0; i < getNFracs(); i++)
    {
      ...
      for (Id ip = 0; ip < npoint; ip++)
      {
        ...
        if (!FFFF(cote) && !FFFF(dcote) && ABS(cote - desc.getYYF(ip)) > dcote)
          continue;
   ...
   **
   *****************************************************************************/
  Id FracList::fractureWellToBlock(
    DbGrid* dbgrid,
    Id col_perm,
    Id col_fluid,
    Id flag_fluid,
    ...
    bool verbose)
  {
    ...
    {
      messerr(
        "The number of values read (%d) in well should be a multiple of %d",
        nval, NBYWOUT);
      return 1;
    }
    Id nw_xy = nval / NBYWOUT;
    if (dbgrid->getNDim() != 2)
    {
      messerr("This application is limited to 2-D grid");
      return 1;
    ...
    if (verbose) printMatrix(wellout, NBYWOUT, nw_xy, "Well information");
    ...
    for (Id iw = 0; iw < nw_xy; iw++)
    {
      double xx = WELLOUT(iw, 0);
      double yy = WELLOUT(iw, 1);
      Id ifrac = static_cast<Id>(WELLOUT(iw, 2)) - 1;
      Id ip = static_cast<Id>(WELLOUT(iw, 3)) - 1;
      ...
      if (ifrac >= getNFracs())
      {
        ...
        messerr(
          "- Fracture number (%d) should lie within [1,%d]", ifrac + 1,
          getNFracs());
        continue;
      }
      ...
      if (ip < 0 || ip >= desc.getNPoint())
      {
        messerr("The well information (line %d/%d) is invalid:", iw + 1, nw_xy);
        messerr(
          "- For fracture (%d, Segment number (%d) is not within [1,%d]",
      ...
      for (Id i = 0; i < npoint - 1; i++)
        _plungeSegment(
          dbgrid, iptr_perm, delta, perm2, desc.getXXF(i), desc.getYYF(i),
      ...
      _plungeSegmentGradual(
        dbgrid, iptr_perm, delta, traj, perm1, perm2, range);

      /* Trajectory downwards */

      _trajAdd(traj, xx, yy);
      for (Id i = ip; i < npoint; i++)
        _trajAdd(traj, desc.getXXF(ip), desc.getYYF(ip));
      _plungeSegmentGradual(
        dbgrid, iptr_perm, delta, traj, perm1, perm2, range);
    }
  ...
  {
    Id nloc = static_cast<Id>(traj.size());
   ...
   **
   *****************************************************************************/
  void FracList::_plungeSegmentGradual(
    DbGrid* dbgrid,
    Id iptr,
    double delta,
    VectorDouble& traj,
    ...
    double range)
  {
    ...
    Id ntraj = static_cast<Id>(traj.size());
    double total = 0.;
    for (Id ip = 0; ip < ntraj - 1; ip++)
    {
      double x1 = TRAJ(ip, 0);
      double y1 = TRAJ(ip, 1);
