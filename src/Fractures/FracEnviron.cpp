...
namespace gstlrn
{
  ...
  bool FracEnviron::_deserializeAscii(std::istream& is)
  {
    Id nfamilies = 0;
    Id nfaults = 0;
    bool ret = true;
    ...
    ret =
      ret
      && _recordRead<double>(is, "Dilation along the horizontal axis", _deltax);
    ret = ret
       && _recordRead<double>(is, "Dilation along the vertical axis", _deltay);
    ret =
      ret && _recordRead<double>(is, "Mean of thickness distribution", _mean);
    ret =
      ret && _recordRead<double>(is, "Stdev of thickness distribution", _stdev);
    if (!ret) return ret;

    for (Id ifam = 0; ret && ifam < nfamilies; ifam++)
    {
      FracFamily family;
      ret = ret && family._deserializeAscii(is);
  ...
  bool FracEnviron::_serializeAscii(std::ostream& os) const
  {
    bool ret = true;
    ...
    ret = ret
       && _recordWrite<double>(
            os, "Dilation along the horizontal axis", _deltax);
    ret = ret
       && _recordWrite<double>(os, "Dilation along the vertical axis", _deltay);
    ret =
      ret && _recordWrite<double>(os, "Mean of thickness distribution", _mean);
    ret = ret
       && _recordWrite<double>(os, "Stdev of thickness distribution", _stdev);
    ...
    for (Id ifam = 0; ret && ifam < getNFamilies(); ifam++)
    {
      ...
      ret = ret && family._serializeAscii(os);
    }
    ...
    for (Id ifault = 0; ret && ifault < getNFaults(); ifault++)
    {
      ...
      ret = ret && fault._serializeAscii(os);
    }
    return ret;
  }
...
#ifdef HDF5
  bool FracEnviron::deserializeH5(H5::Group& grp)
  {
    ...
    for (Id ifam = 0; ret && ifam < getNFamilies(); ifam++)
    {
      ...
      FracFamily family;
      ret = ret && family.deserializeH5(*famG);
      if (ret) addFamily(family);
    }
    ...
    for (Id ifault = 0; ret && ifault < getNFaults(); ifault++)
    {
      ...
      FracFault fault;
      ret = ret && fault.deserializeH5(*faultG);
      if (ret) addFault(fault);
    }
  ...
  bool FracEnviron::serializeH5(H5::Group& grp) const
  {
    ...
    for (Id ifam = 0, nfam = getNFamilies(); ret && ifam < nfam; ifam++)
    {
      ...
      ret = ret && family.serializeH5(famG);
    }
    ...
    for (Id ifault = 0, nfault = getNFaults(); ret && ifault < nfault; ifault++)
    {
      ...
      ret = ret && fault.serializeH5(faultG);
    }
