...
namespace gstlrn
{
  ...
  *****************************************************************************/
    ...
    double* vmax_arg) const
  {
    ...
    {
      messerr(
        "Sample %d: Bounds are wrongly ordered: Vmin(%lf) > Vmax(%lf)",
        iech + 1, vmin, vmax);
      return 1;
    }
  ...
  **
  *****************************************************************************/
  void AGibbs::_printInequalities(
    Id iact,
    Id ivar,
    double simval,
    double vmin,
    double vmax) const
  {
    ...
    message(
      "Sample (%3d/%3d) - Variable (%3d/%3d) = %8.4lf in ", iech + 1,
      _db->getNSample(), ivar + 1, nvar, simval);
   ...
   **
   *****************************************************************************/
  void AGibbs::_displayCurrentVector(
    bool flag_init,
    ...
    Id ipgs) const
  {
    ...
    {
      mestitle(
        1, "Gibbs Initial Status (Simu:%d - GS=%d)", isimu + 1, ipgs + 1);
    }
    else
    {
  ...
  VectorInt AGibbs::_calculateSampleRanks() const
  {
    ...
    if (!_db->hasLocVariable(ELoc::SEL)) return VH::sequence(_db->getNSample());
   ...
   * @return
   */
  bool AGibbs::_isConstraintTight(Id icase, Id iact, double* value) const
  {
    auto iech = getSampleRank(iact);
  ...
  void AGibbs::_getBoundsDecay(Id iter, double* vmin, double* vmax) const
  {
    ...
    if (!FFFF(*vmin)) *vmin = THRESH_INF + ((*vmin) - THRESH_INF) * ratio;
    if (!FFFF(*vmax)) *vmax = THRESH_SUP + ((*vmax) - THRESH_SUP) * ratio;
  }
    ...
    bool flagCheck)
  {
    ...
    if (flagCheck) _displayCurrentVector(true, y, isimu0, ipgs0);
  ...
  String AGibbs::toString(const AStringFormat* /*strfmt*/) const
  {
    ...
    if (_flagDecay) sstr << "Decay option is switched ON" << std::endl;
    if (_flagOrder == 1)
      sstr << "Variables are ordered sequentially upwards" << std::endl;
    if (_flagOrder == -1)
      sstr << "Variables are ordered sequentially downwards" << std::endl;
    if (_optionStats == 1)
      sstr << "Statistics on Trajectories are stored for print out"
           << std::endl;
    if (_optionStats == 2)
      sstr << "Statistics on Trajectories are stored in Neutral File"
           << std::endl;
