...
namespace gstlrn
{
  ...
  String ASpace::toStringIdx(const AStringFormat* strfmt, Id ispace) const
  {
    ...
    else
    {
      ...
      {
        sstr << "Space Type      [" << ispace << "] = " << getType().getKey()
             << std::endl;
        sstr << "Space Dimension [" << ispace << "] = " << getNDim()
             << std::endl;
      }
    }
    return sstr.str();
  ...
  {
    return (
      getNComponents() == space->getNComponents()
      && getType() == space->getType() && getNDim() == space->getNDim()
  ...
  VectorDouble
    ASpace::getDistances(const SpacePoint& p1, const SpacePoint& p2) const
  {
    VectorDouble dis;
  ...
  {
    if (vec.size() <= 0 || vec.size() < getOffset() + getNDim()
        || vec.size() != p1.getNDim())
    {
  ...
  double
    ASpace::getDistance(const SpacePoint& p1, const SpacePoint& p2, Id ispace)
      const
  {
    if (p1.getNDim() != p2.getNDim())
    {
  ...
  double ASpace::getFrequentialDistance(
    const SpacePoint& p1,
    const SpacePoint& p2,
    const Tensor& tensor,
    Id ispace) const
  ...
  VectorDouble
    ASpace::getIncrement(const SpacePoint& p1, const SpacePoint& p2, Id ispace)
      const
  {
    if (p1.getNDim() != p2.getNDim())
    /// TODO : test tensor size
  ...
  void ASpace::getIncrementInPlace(
    const SpacePoint& p1,
    const SpacePoint& p2,
    VectorDouble& ptemp,
    Id ispace) const
