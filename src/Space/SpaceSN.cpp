...
namespace gstlrn
{
  SpaceSN::SpaceSN(size_t ndim, double radius)
    ...
    , _radius(radius)
  {
    if (ndim != 2) my_throw("SN is only implemented for ndim=2 (sphere)");
  }
  ...
  double
    SpaceSN::_getDistance(const SpacePoint& p1, const SpacePoint& p2, Id ispace)
      const
  {
    DECLARE_UNUSED(ispace)
    auto offset = static_cast<Id>(getOffset());
    return GH::geodeticAngularDistance(
      p1.getCoord(offset), p1.getCoord(offset + 1), p2.getCoord(offset),
      p2.getCoord(offset + 1), _radius);
  }

  double SpaceSN::_getDistance(
    const SpacePoint& p1,
    const SpacePoint& p2,
    const Tensor& tensor,
    Id ispace) const
  ...
  double SpaceSN::_getFrequentialDistance(
    const SpacePoint& p1,
    const SpacePoint& p2,
    const Tensor& tensor,
    Id ispace) const
  ...
  VectorDouble SpaceSN::_getIncrement(
    const SpacePoint& p1,
    const SpacePoint& p2,
    Id ispace) const
  {
  ...
  void SpaceSN::_getIncrementInPlace(
    const SpacePoint& p1,
    const SpacePoint& p2,
    VectorDouble& ptemp,
    Id ispace) const
