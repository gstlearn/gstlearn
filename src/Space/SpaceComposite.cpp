...
namespace gstlrn
{
  ...
  void SpaceComposite::setOrigin(const VectorDouble& origin)
  {
    ...
    {
      std::cout << "Error: Inconsistent space origin. Origin not changed."
                << std::endl;
      return;
    }
    _origin = origin;
  ...
  std::shared_ptr<SpaceComposite>
    SpaceComposite::create(const std::vector<ASpaceSharedPtr>& vectspace)
  {
  ...
  String
    SpaceComposite::toStringIdx(const AStringFormat* strfmt, Id ispace) const
  {
    ...
    for (Id idx = 0; idx < nc; idx++)
    {
      ...
      if (idx < nc - 1 && strfmt != nullptr && strfmt->getLevel() == 0)
        sstr << " + ";
    }
  ...
  VectorDouble SpaceComposite::getDistances(
    const SpacePoint& p1,
  ...
  {
    std::shared_ptr<ASpace> sp =
      std::shared_ptr<ASpace>(dynamic_cast<ASpace*>(comp->clone()));
    sp->ASpace::setOffset(
      getNDim()); // TODO : I want this to be private and me a friend of ASpace
    _comps.push_back(sp);
    _nDim += sp->getNDim();
  ...
  double SpaceComposite::_getDistance(
    const SpacePoint& p1,
    const SpacePoint& p2,
    Id ispace) const
  {
    if (ispace < 0 || ispace >= static_cast<Id>(getNComponents()))
      return getDistances(p1, p2)
        .norm(); // Return the norm of sub-distances vector
  ...
  /// given tensor
  double SpaceComposite::_getFrequentialDistance(
    const SpacePoint& p1,
    const SpacePoint& p2,
    const Tensor& tensor,
    Id ispace) const
  ...
  VectorDouble SpaceComposite::_getIncrement(
    const SpacePoint& p1,
    const SpacePoint& p2,
    Id ispace) const
  {
  ...
  void SpaceComposite::_getIncrementInPlace(
    const SpacePoint& p1,
    const SpacePoint& p2,
    VectorDouble& ptemp,
    Id ispace) const
