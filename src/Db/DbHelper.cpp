...
namespace gstlrn
{
   ...
   **
   *****************************************************************************/
  static void st_grid_fill_neigh(Id ipos,
                                 Id ndim,
                                 Id radius,
                                 Id* nech_loc,
                                 Id* tabind,
   ...
   **
   *****************************************************************************/
  Id DbHelper::centerPointToGrid(Db* db_point,
                                 DbGrid* db_grid,
                                 double eps_random)
  {
    ...
    {
      messerr("For centering, 'dbin' and 'dbout' should share the same Space "
              "Dimension");
      return 1;
    }
    ...
    for (Id iech2 = 0; iech2 < db2->getNSample(); iech2++)
    {
      ...
      for (Id iech1 = 0; iech1 < db1->getNSample(); iech1++)
      {
        ...
        for (Id idim = 0; idim < db1->getNDim() && !flag_diff; idim++)
        {
          ...
          if (flag_code)
          {
            if (st_code_comparable(db1,
                                   db2,
                                   iech1,
                                   iech2,
                                   opt_code,
                                   static_cast<Id>(tolcode)))
              continue;
          }
                              ...
                              const NamingConvention& namconv)
  {
    ...
    while (skin->remains(verbose))
    {
      ...
      if (st_grid_fill_calculate(ipos,
                                 mode,
                                 nech,
                                 tabind.data(),
                                 tabval.data()))
                              ...
                              const NamingConvention& namconv)
  {
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
    {
      ...
      {
        if (st_grid1D_interpolate_spline(dbgrid,
                                         ivar,
                                         ndef,
                                         X.data(),
                                         Y.data()))
    ...
    namconv.setNamesAndLocators(dbgrid,
                                VectorString(),
                                ELoc::Z,
                                -1,
                                dbgrid,
