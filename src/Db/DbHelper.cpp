...
namespace gstlrn
{
  ...
  class LocalSkin: public ISkinFunctions
  {
    ...
    Id isAlreadyFilled(Id ipos) const override
    {
      ...
      Id value = FFFF(DB_GRID_FILL->getZVariable(ipos, 0)) ? 0 : 1;
      return (value);
    }
    ...
    {
      if (!DB_GRID_FILL->getSelection(ipos)) return (0);
      Id value = FFFF(DB_GRID_FILL->getZVariable(ipos, 0)) ? 1 : 0;
      return (value);
    }
  };
   ...
   **
   *****************************************************************************/
  static void st_grid_fill_neigh(
    Id ipos,
    Id ndim,
    Id radius,
    Id* nech_loc,
  ...
  {
    double value;
    Id ix, iy, iz, jpos, nech, nrx, nry, nrz, nmx, nmy, nmz;
   ...
   **
   *****************************************************************************/
  static Id st_grid_fill_calculate(
    Id ipos,
    Id mode,
    Id nech,
    Id* tabind,
    ...
    switch (mode)
    {
      ...
      case 2:
      {
        ...
        for (Id iech = 0; iech < nech; iech++)
        {
          ...
          for (Id idim = 0; idim < ndim; idim++) f[1 + idim] = coor[idim];
          for (Id j = 0; j < neq; j++)
          {
            b[j] += tabval[iech] * f[j];
            for (Id k = 0; k < neq; k++)
        ...
        DB_GRID_FILL->indicesToCoordinateInPlace(indg, coor);
        for (Id idim = 0; idim < ndim; idim++) f[1 + idim] = coor[idim];
        for (Id j = 0; j < neq; j++) result += sol[j] * f[j];
        break;
      }
   ...
   *****************************************************************************/
  static void
    st_write_active_sample(Db* db, Id iech, Id nvar, Id* iatt, double* tab)
  {
    Id ivar;
   ...
   **
   *****************************************************************************/
  static Id st_read_active_sample(
    Db* db,
    Id flag_zero,
    Id iech,
    Id nvar,
    ...
    double* tab)
  {
    ...
    for (ivar = 0; ivar < nvar; ivar++) tab[ivar] = TEST;
   ...
   **
   *****************************************************************************/
  static void st_grid1D_interpolate_linear(
    Db* dbgrid,
    Id ivar,
    Id ndef,
    const double* X,
   ...
   **
   *****************************************************************************/
  static Id st_grid1D_interpolate_spline(
    Db* dbgrid,
    Id ivar,
    Id ndef,
    const double* X,
    const double* Y)
  {
    ...
    h.resize(nm1);
    for (Id i = 0; i < nm1; i++) h[i] = X[i + 1] - X[i];
    ...
    double eps_random)
  {
    ...
    {
      messerr(
        "For centering, 'dbin' and 'dbout' should share the same Space "
        "Dimension");
      return 1;
    }
  ...
  {
    bool flag_code =
      db1->hasLocVariable(ELoc::C) && db2->hasLocVariable(ELoc::C);
    Id nmerge = 0;
    ...
    for (Id iech2 = 0; iech2 < db2->getNSample(); iech2++)
    {
      ...
      for (Id iech1 = 0; iech1 < db1->getNSample(); iech1++)
      {
        ...
        for (Id idim = 0; idim < db1->getNDim() && !flag_diff; idim++)
        {
          double v1 = db1->getCoordinate(iech1, idim);
          double v2 = db2->getCoordinate(iech2, idim);
          if (flag_code)
          {
            if (st_code_comparable(
                  db1, db2, iech1, iech2, opt_code, static_cast<Id>(tolcode)))
              continue;
          }
   ...
   **
   *****************************************************************************/
  Id DbHelper::normalizeVariables(
    Db* db,
    const char* oper,
    ...
    double stdv)
  {
    ...
    for (Id icol = 0; icol < ncol; icol++) num[icol] = mm[icol] = vv[icol] = 0.;
    ...
    const NamingConvention& namconv)
  {
    ...
    skin = nullptr;
    count =
      static_cast<Id>(pow(2. * radius + 1., static_cast<double>(ndim))) - 1;
    ...
    while (skin->remains(verbose))
    {
      ...
      st_grid_fill_neigh(
        ipos, ndim, radius, &nech, tabind.data(), tabval.data());

      /* Calculate the extrapolated value */

      if (st_grid_fill_calculate(
            ipos, mode, nech, tabind.data(), tabval.data()))
        continue;
    ...
    const NamingConvention& namconv)
  {
    ...
    if (DbHelper::findDuplicates(
          db, db, 1, verbose, opt_code, tolcode, dist, sel))
      return 1;
   ...
   **
   *****************************************************************************/
  Id DbHelper::db_compositional_transform(
    Db* db,
    Id verbose,
    Id mode,
    Id type,
    ...
    Id* numout)
  {
    ...
    switch (type)
    {
      case 0: /* No action */
        ...
        {
          if (!db->isActive(iech)) continue;
          (void)st_read_active_sample(
            db, 1, iech, number, iatt_in, eps, tabin.data());
          sum = 0.;
          for (ivar = 0; ivar < (*numout); ivar++) sum += tabin[ivar];
      ...
      case 1: /* Additive Logratio */
        ...
        else
        {
          ...
          for (iech = 0; iech < nech; iech++)
          {
            if (!db->isActive(iech)) continue;
            if (st_read_active_sample(
                  db, 0, iech, number, iatt_in, eps, tabin.data()))
            {
              sum = 1.;
              ...
              for (ivar = 0; ivar < number; ivar++) tabout[ivar] /= sum;
              tabout[number] = 1 / sum;
            }
            st_write_active_sample(db, iech, *numout, iatt_out, tabout.data());
          }
        ...
        else
        {
          ...
          for (iech = 0; iech < nech; iech++)
          {
                  ...
                  db, 0, iech, number, iatt_in, eps, tabin.data()))
            {
              ...
              for (ivar = 0; ivar < (*numout); ivar++) tabout[ivar] /= sum;
            }
            st_write_active_sample(db, iech, *numout, iatt_out, tabout.data());
          }
        }
    ...
    const NamingConvention& namconv)
  {
    ...
    Id iatt_out = dbgrid->addColumnsByConstant(nvar);
    for (Id ivar = 0; ivar < nvar; ivar++)
    {
      Id iatt_in = dbgrid->getUIDByLocator(ELoc::Z, ivar);
    ...
    for (Id ivar = 0; ivar < nvar; ivar++)
    {
      ...
      {
        if (st_grid1D_interpolate_spline(
              dbgrid, ivar, ndef, X.data(), Y.data()))
          return 1;
      }
    }
