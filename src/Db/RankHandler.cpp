...
namespace gstlrn
{
    ...
    , _workNbgh()
  {
    ...
    if (_useExtD && _db->hasLocator(ELoc::F))
    {
      ...
      for (Id iext = 0; iext < _nExtD; iext++)
   ...
   * @return Id
   */
  Id RankHandler::getCount(Id ivar) const
  {
   ...
   * @return Id
   */
  Id RankHandler::getTotalCount() const
  {
    Id count = 0;
    for (Id ivar = 0; ivar < _nvar; ivar++) count += getCount(ivar);
    return count;
  }
  ...
  Id RankHandler::identifyVariableRank(Id ipos) const
  {
    ...
    for (Id ivar = 0; ivar < _nvar; ivar++)
    {
      ...
      if (ipos < ntotal + size) return ivar; // Found the variable
      ntotal += size;
    }
    return -1; // Not found
  }

  Id RankHandler::identifySampleRank(Id ipos) const
  {
    Id ntotal = 0;
    for (Id ivar = 0; ivar < _nvar; ivar++)
    {
  ...
  void RankHandler::dump(bool flagFull) const
  {
    ...
    message(
      "Use Variance of Measurement Error: %d\n", static_cast<Id>(_useVerr));
    ...
    for (Id ivar = 0, lec = 0; ivar < _nvar; ivar++)
    {
      message("Variable= %d: \n", ivar);
      auto size = getCount(ivar);
      for (Id i = 0; i < size; i++, lec++)
        message(
          "- Sample= %2d : Variable value= %lf\n", _index[ivar][i],
          (*_Zflatten)[lec]);
      message("\n");
    }
  }
