...
namespace gstlrn
{
  ...
  Id Db::resetFromSamples(Id nech,
                          const ELoadBy& order,
   ...
   */
  Id Db::resetFromCSV(const String& filename,
                      bool verbose,
   ...
   */
  bool Db::getLocatorByColIdx(Id icol,
  ...
  bool Db::getLocatorByUID(Id iuid,
                           ELoc* ret_locatorType,
                           Id* ret_locatorIndex) const
  {
  ...
  void Db::setArrayBySample(Id iech, const VectorDouble& vec)
  {
    ...
    if (static_cast<Id>(uids.size()) != static_cast<Id>(vec.size()))
    {
      messerr("Dimension of 'vec'(%d) does not match number of columns(%)",
              static_cast<Id>(vec.size()),
              static_cast<Id>(uids.size()));
      return;
    }
    ...
    Db::getIncrements(const VectorInt& iechs, const VectorInt& jechs) const
  {
    ...
    if (static_cast<Id>(jechs.size()) != number)
    {
      messerr(
        ...
        static_cast<Id>(iechs.size()),
        static_cast<Id>(jechs.size()));
      return tab;
    }
   ...
   */
  void Db::getCoordinatesInPlace(VectorDouble& coor,
                                 Id iech,
  ...
  Id Db::getDistanceVecInPlace(Id iech,
                               Id jech,
                               VectorDouble& dd,
                               const Db* db2) const
  {
  ...
  void Db::setSampleCoordinates(Id iech, const VectorDouble& coor)
  {
    ...
    if (ndim != size)
    {
      messerr("Argument 'coor' (%d) should have dimension ndim (%d)",
              size,
              ndim);
  ...
  {
    return &_array[_getAddress(0,
                               getColIdxByLocator(locatorType, locatorIndex))];
  }
  ...
  {
    auto number = getNLoc(locatorType);
    messerr("Db::_getNextLocator: Selecting next locator for %s: %d (will be "
            "removed soon!)",
            std::string(locatorType.getKey()).c_str(),
            number);
    return number;
  }
   ...
   */
  void Db::setLocatorByUID(Id iuid,
                           const ELoc& locatorType,
                           Id locatorIndex,
                           bool cleanSameLocator)
  {
  ...
  void Db::setLocatorByColIdx(Id icol,
                              const ELoc& locatorType,
                              Id locatorIndex,
                              bool cleanSameLocator)
  {
   ...
   * @param cleanSameLocator When TRUE, clean variables with same locator beforehand
   */
  void Db::setLocatorsByUID(Id number,
                            Id iuid,
                            const ELoc& locatorType,
                            Id locatorIndex,
                            bool cleanSameLocator)
   ...
   */
  Id Db::addColumnsByConstant(Id nadd,
                              double valinit,
                              const String& radix,
                              const ELoc& locatorType,
                              Id locatorIndex,
   ...
   * @return Rank of the first UID
   */
  Id Db::addColumnsRandom(Id nadd,
                          const String& radix,
                          const ELoc& locatorType,
                          Id locatorIndex,
                          Id seed,
   ...
   */
  Id Db::addColumns(const VectorDouble& tab,
                    const String& radix,
                    const ELoc& locatorType,
                    Id locatorIndex,
                    bool useSel,
                    ...
                    Id nvar)
  {
    ...
    {
      messerr("Db::addColumns : Incompatibility between 'tab'(%d) and "
              "'nvar'(%d) * 'nech'(%d)",
              tab.size(),
              nvar,
              nech);
                              ...
                              bool useSel)
  {
    ...
    if (static_cast<Id>(icols.size()) * nech != static_cast<Id>(tabs.size()))
    {
      messerr(
        ...
        static_cast<Id>(icols.size()),
        nech,
        static_cast<Id>(tabs.size()));
      return;
    }
    Id lec = 0;
   ...
   */
  Id Db::addSelection(const VectorDouble& tab,
                      const String& name,
   ...
   * @return
   */
  Id Db::addSelectionByRanks(const VectorInt& ranks,
                             const String& name,
                             const String& combine)
  {
    auto nech = getNSample();
   ...
   */
  Id Db::addSelectionByLimit(const String& testvar,
                             const Limits& limits,
                             const String& name,
                             const String& combine)
  {
   ...
   * @return
   */
  Id Db::addSelectionFromDbByConvexHull(Db* db,
                                        double dilate,
                                        bool verbose,
   ...
   * @return
   */
  Id Db::addSelectionRandom(double prop,
                            Id seed,
                            const String& name,
                            const String& combine)
  {
   ...
   */
  bool haveSameNDim(const Db* db1,
                    const Db* db2,
                    ...
                    Id* ndim)
  {
    *ndim = 0;
    ...
    if (db1 != nullptr)
    {
      ...
      {
        messerr("Db1 (%d) should share common Space Dimension (%d)",
                ndimDb1,
                *ndim);
        return false;
      }
    ...
    if (db2 != nullptr)
    {
      ...
      {
        messerr("Db2 (%d) should share common Space Dimension (%d)",
                ndimDb2,
                *ndim);
        return false;
      }
    ...
    if (model != nullptr)
    {
      ...
      {
        messerr("Model (%d) should share common Space Dimension (%d)",
                ndimModel,
                *ndim);
        return false;
      }
   ...
   * @return true or false
   */
  bool haveCompatibleNVar(const Db* db1,
                          const Db* db2,
                          const ModelGeneric* model,
                          Id* nvar)
  {
  ...
  bool Db::hasLargerDimension(const Db* dbaux) const
  {
    ...
    if (!retOK)
    {
      messerr("The Space Dimension of the Secondary Data base (%d)",
              dbaux->getNDim());
      messerr("should be larger than the Space Dimension of the Current Data "
              "Base (%d)",
              getNDim());
    }
    return retOK;
                             ...
                             bool bySample)
  {
    ...
    {
      messerr("Dimensions of 'icols'(%d), 'iechs'(%d) and 'values'(%d) are "
              "inconsistent",
              static_cast<Id>(icols.size()),
              static_cast<Id>(iechs.size()),
              static_cast<Id>(values.size()));
  ...
  bool Db::isNVarComparedTo(Id nvar, Id compare) const
  {
    if (compare == 0)
    {
      ...
      {
        messerr("This function requires %d variables (locator 'Z'). The 'Db' "
    ...
    else if (compare < 0)
    {
      if (!(getNLoc(ELoc::Z) <= nvar))
      {
        messerr("This function requires nvar <= %d variables (locator 'Z'). "
                "The 'Db' contains %d variables",
                nvar,
                getNLoc(ELoc::Z));
        return false;
      }
  ...
  String Db::_summaryStats(VectorInt cols, Id mode, Id maxNClass) const
  {
    ...
    for (Id jcol = 0; jcol < ncol; jcol++)
    {
      ...
      wgt = getWeights(true);
      StatResults stats = ut_statistics(static_cast<Id>(tab.size()),
                                        tab.data(),
                                        NULL,
                                        wgt.data());
      ...
      else
      {
        ...
        ut_classify(static_cast<Id>(tab.size()),
                    tab.data(),
                    NULL,
                    nclass,
                    vmin,
  ...
  void Db::getValuesByRanksInPlace(VectorDouble* values,
   ...
   */
  Id Db::getListOfSampleIndicesInPlace(Id nvar,
                                       VectorInt& cumul,
                                 ...
                                 bool useExtD) const
  {
    ...
    {
      Id jvar = jvars[ivar];
      getSampleRanksPerVariable(sampleRanks[ivar],
                                nbgh,
                                jvar,
                                useSel,
                                useZ,
   ...
   *
   */
  VectorDouble Db::getColumnsByColIdxInterval(Id icol_beg,
                                              Id icol_end,
                                              bool useSel,
                                              bool flagCompress) const
  {
   ...
   *
   */
  VectorDouble Db::getColumnsByUIDInterval(Id iuid_beg,
                                           Id iuid_end,
                                           bool useSel,
                                           bool flagCompress) const
  {
  ...
  Id Db::setItem(const VectorInt& rows,
                 const VectorString& colnames,
  ...
  Id Db::setItem(const VectorString& colnames,
                 const VectorVectorDouble& values,
                 bool useSel)
  {
  ...
  Id Db::setItem(const ELoc& locatorType,
                 const VectorVectorDouble& values,
                 bool useSel)
  {
    if (!_isValidCountRows(useSel, values[0])) return 1;
  ...
  void Db::_defineDefaultLocators(Id shift, const VectorString& locatorNames)
  {
    ...
    for (Id icol = 0; icol < ncol; icol++)
    {
      if (locatorIdentify(locatorNames[icol],
                          &locatorType,
                          &locatorIndex,
                          &mult)
          == 0)
  ...
  bool Db::deserializeH5(H5::Group& grp)
  {
    ...
    for (size_t i = 0; i < names.size(); ++i)
    {
      ...
      data.read(&_array[_getAddress(0, colIds[i])],
  ...
  *****************************************************************************/
   ...
   **
   *****************************************************************************/
  double Db::getCosineToDirection(Id iech1,
                                  Id iech2,
   ...
   */
  Id Db::resetSamplingDb(const Db* dbin,
                         double proportion,
                         Id number,
                         const VectorString& names,
                         Id seed,
                         ...
                         bool flagAddSampleRank)
  {
    ...
    _defineVariableAndLocators(dbin,
                               namloc,
                               static_cast<Id>(flagAddSampleRank));
   ...
   */
  void Db::_defineVariableAndLocators(const Db* dbin,
                                      const VectorString& names,
                                      Id shift)
  {
    ELoc locatorType;
   ...
   */
  void Db::_loadValues(const Db* db,
                       const VectorString& names,
  ...
  Id Db::resetFromGridRandomized(const DbGrid* dbin,
                                 double randperc,
                                 bool flagAddSampleRank)
  {
    // Creating the vector of selected samples
   ...
   **
   *****************************************************************************/
  Db* Db::createFromDbGrid(Id nech,
                           DbGrid* dbgrid,
                           Id seed,
                           bool flag_exact,
                           bool flag_repulsion,
  ...
  void Db::_combineSelection(VectorDouble& sel, const String& combine) const
  {
    ...
    messerr("('sel' is the current selection and 'oldsel' the already "
            "existing one)");
  ...
  Db* Db::createFromSamples(Id nech,
                            const ELoadBy& order,
                            const VectorDouble& tab,
                            const VectorString& names,
                            const VectorString& locatorNames,
   ...
   */
  Db* Db::createFillRandom(Id ndat,
                           Id ndim,
                           Id nvar,
                           Id nfex,
                           Id ncode,
  ...
  Table Db::printOneSample(Id iech,
                           const VectorString& names,
                                      ...
                                      double eps)
  {
    ...
    for (Id irow = 0; irow < nrows; irow++)
    {
      ...
      Id iuid = addSelectionByVariable(category,
                                       catValue - eps,
                                       catValue + eps,
                                       "catSel");
