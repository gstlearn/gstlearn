...
namespace gstlrn
{
  ...
  {
    auto* dbgrid = new DbGrid;
    if (dbgrid->resetFromPolygon(polygon,
                                 nx,
                                 dx,
                                 flagAddSelection,
                                 flagAddSampleRank))
                                       ...
                                       double eps)
  {
    ...
    for (Id icoor = 0; icoor < ncoor; icoor++)
    {
      ...
      {
        messerr("The grid extension along variable (%d) is not possible",
                icoor + 1);
        messerr(
      ...
      if (nxnew[icoor] < 2)
      {
        messerr("The number of meshes along new direction5%d) should be larger "
                "than 1",
                icoor + 1);
        return gridnew;
      }
      ...
      if (verbose)
        message("Additional coordinate %d: Minimum = %lf - Maximum = %lf - "
                "Nstep = %d\n",
                icoor + 1,
                mini[icoor],
                maxi[icoor],
                               ...
                               bool flagInter)
  {
    ...
    // Migrate all variables (except 'rank'  and coordinates
    (void)dbgrid->migrateAllVariables(dbin,
                                      true,
                                      flagInter,
                                      false,
                                      flagAddSampleRank);
  ...
  void DbGrid::getCoordinatesInPlace(VectorDouble& coor,
                                     Id iech,
  ...
  Id DbGrid::centerCoordinateInPlace(VectorDouble& coor,
                                     bool centered,
                                     bool stopIfOut,
                                     double eps) const
  {
                                                 ...
                                                 double thickmin)
  {
    ...
    if (names.empty())
    {
      messerr("You must designate variable(s) to be copied from input to "
              "output 3D grid");
      return grid3Dout;
    }
    Id ndim = 3;
    ...
    if (topmax <= botmin)
    {
      messerr("The thickness of the target Layer seems too small for a "
              "Squeeze-and-Stretch");
      return grid3Dout;
    }
    ...
    for (Id ix = 0, nx = grid3Dout->getNX(0); ix < nx; ix++)
      for (Id iy = 0, ny = grid3Dout->getNX(1); iy < ny; iy++)
      {
        ...
        for (Id ivar = 0; ivar < nvar; ivar++)
        {
          ...
          VH::squeezeAndStretchInPlaceBackward(vecin,
                                               vecout,
                                               z0out,
                                               dzout,
                                               top,
   ...
   */
  VectorVectorInt
    DbGrid::getLimitsFromVariableExtend(const String& nameTop,
                                        const String& nameBot,
   ...
   */
  Id DbGrid::setSelectionFromVariableExtend(const String& nameTop,
                                            const String& nameBot)
  {
                                        ...
                                        const NamingConvention& namconv)
  {
    ...
    Id err = morpho(EMorpho::DILATION,
                    0.5,
                    1.5,
                    option,
                    dilation,
