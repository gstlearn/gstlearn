...
namespace gstlrn
{
  ...
  String DbGrid::toString(const AStringFormat* strfmt) const
  {
    ...
    sstr << toStrTitle(0, "Data Base Grid Characteristics");
    ...
    bool flagAddCoordinates)
  {
    ...
    Id nech = 1;
    for (Id idim = 0; idim < ndim; idim++) nech *= nx[idim];
    ...
    const VectorDouble& margin)
  {
    ...
    for (Id idim = 0; idim < ndim; idim++)
    {
      ...
      Id nxloc = 10;
      if (ndim == static_cast<Id>(nx.size())) nxloc = nx[idim];
    ...
    bool flagAddSampleRank)
  {
    ...
    // Add a selection (optional)
    if (flagAddSelection)
    {
      db_polygon(this, polygon);
    }
    return 0;
  }

  DbGrid* DbGrid::create(
    const VectorInt& nx,
    const VectorDouble& dx,
    const VectorDouble& x0,
    const VectorDouble& angles,
    ...
    bool flagAddCoordinates)
  {
    ...
    if (dbgrid->reset(
          nx, dx, x0, angles, order, tab, names, locatorNames,
  ...
  DbGrid* DbGrid::createCoveringDb(
    const Db* db,
    const VectorInt& nx,
    const VectorDouble& dx,
    const VectorDouble& x0,
   ...
   */
  DbGrid* DbGrid::createFromPolygon(
    Polygons* polygon,
    const VectorInt& nx,
    const VectorDouble& dx,
    bool flagAddSelection,
  ...
  {
    auto* dbgrid = new DbGrid;
    if (dbgrid->resetFromPolygon(
          polygon, nx, dx, flagAddSelection, flagAddSampleRank))
    {
    ...
    bool flagAddSampleRank)
  {
    ...
    // Create the new grid
    dbgrid = create(
      nx, dx, x0, dbin->getAngles(), ELoadBy::SAMPLE, VectorDouble(),
      VectorString(), VectorString(), flagAddSampleRank);

    // Migrate all variables (except 'rank' and coordinates
    (void)
      dbgrid->migrateAllVariables(dbin, true, true, false, flagAddSampleRank);
   ...
   * @return
   */
  DbGrid* DbGrid::createFromGridExtend(
    const DbGrid& gridIn,
    ...
    double eps)
  {
    ...
    for (Id icoor = 0; icoor < ncoor; icoor++)
    {
      ...
      {
        messerr(
          "The grid extension along variable (%d) is not possible", icoor + 1);
        messerr(
          "The variable has no valid value available or all values are equal");
        return gridnew;
      }
      ...
      {
        messerr(
          "The number of meshes along new direction5%d) should be larger than "
          "1",
          icoor + 1);
        return gridnew;
      }
      ...
      if (verbose)
        message(
          "Additional coordinate %d: Minimum = %lf - Maximum = %lf - Nstep = "
          "%d\n",
   ...
   * @return
   */
  DbGrid* DbGrid::createFromGridShrink(
    const DbGrid& gridIn,
    const VectorInt& deletedRanks)
  {
    ...
    for (Id i = 0; i < static_cast<Id>(deletedRanks.size()); i++)
    {
      ...
      {
        messerr(
          "The dimension to be removed (%d) should lie within [0,%d[", i + 1,
          ndim);
        return gridnew;
      }
    }
    ...
    bool flagInter)
  {
    ...
    // Migrate all variables (except 'rank'  and coordinates
    (void)dbgrid->migrateAllVariables(
      dbin, true, flagInter, false, flagAddSampleRank);
   ...
   * @return
   */
  bool DbGrid::migrateAllVariables(
    Db* dbin,
    bool flag_fill,
    bool flag_inter,
    bool flag_ball,
    bool flagAddSampleRank)
  {
    ...
    if (migrateByAttribute(
          dbin, this, iatts, 2, VectorDouble(), flag_fill, flag_inter,
          flag_ball, NamingConvention(String())))
      return false;
  ...
  void
    DbGrid::getCoordinatesInPlace(VectorDouble& coor, Id iech, bool flag_rotate)
      const
  {
    coor.resize(getNDim());
  ...
  VectorDouble
    DbGrid::getColumnSubGrid(const String& name, Id idim0, Id rank, bool useSel)
  {
    VectorDouble vec;
    if (!isGrid())
    {
  ...
  void DbGrid::getGridPileInPlace(
    Id iuid,
  ...
  void DbGrid::setGridPileInPlace(
    Id iuid,
    const VectorInt& indg,
    Id idim0,
  ...
  void DbGrid::generateCoordinates(const String& radix)
  {
    ...
    {
      messerr(
        "This method is only available in the case of Grid. Nothing done");
      return;
    }
    auto ndim = getNDim();
    VectorDouble coors(ndim);
   ...
   */
  VectorDouble DbGrid::getOneSlice(
    const String& name,
    Id posx,
    Id posy,
    ...
    bool useSel) const
  {
    ...
    if (cornloc.empty()) cornloc.resize(ndim, 0);
   ...
   *
   */
  VectorDouble DbGrid::getOneSliceForCoordinate(
    Id idim,
    Id posx,
    Id posy,
    const VectorInt& corner,
    ...
    bool useSel)
  {
    ...
    {
      messerr(
        "Argument 'idim'(%d) is incompatible with Grid dimension(%d)", idim,
        getNDim());
      return 1;
    }
    ...
    {
      messerr(
        "Argument 'rank'(%d) is incompatible with number of cells(%d)", rank,
        getNX(idim));
      return 1;
    }
  ...
  VectorInt DbGrid::coordinateToIndices(
    const VectorDouble& coor,
    bool centered,
    double eps) const
  {
  ...
  Id DbGrid::coordinateToIndicesInPlace(
    const VectorDouble& coor,
    VectorInt& indices,
    bool centered,
    double eps) const
  ...
  Id DbGrid::centerCoordinateInPlace(
    VectorDouble& coor,
    bool centered,
    bool stopIfOut,
    double eps) const
    ...
    DbGrid::getSlice(const String& name, Id pos, Id indice, bool useSel) const
  {
    ...
    if (pos == 0)
    {
      ...
      if (!checkArg("Error in argument 'indice'", indice, n3))
    ...
    const VectorDouble& tab)
  {
    ...
    DbGrid* db = DbGrid::create(
      nn, dd, xx, angles, order, tab, VectorString(), VectorString(),
      flagAddSampleRank);
   ...
   */
  DbGrid* DbGrid::createFillRandom(
    const VectorInt& nx,
    Id nvar,
    Id nfex,
    Id ncode,
    ...
    VectorDouble& vecout) const
  {
    ...
    for (Id iz = indbot; iz <= indtop; iz++)
    {
      ...
      Id izin =
        static_cast<Id>(static_cast<double>(nzin) * (zz - bot) / (top - bot));
    ...
    bool flagAddCoordinates)
  {
    ...
    // Create the new grid
    gridOut = DbGrid::create(
      NXs, DXs, X0s, angles, ELoadBy::fromKey("SAMPLE"), VectorDouble(),
      VectorString(), VectorString(), 1, flagAddCoordinates);
    ...
    for (Id igout = 0, nout = gridOut->getNSample(); igout < nout; igout++)
    {
      ...
      for (Id idim = 0; idim < ndim; idim++) indg[idim] += limits[idim][0];
   ...
   */
  DbGrid* DbGrid::createSqueezeAndStretchForward(
    const DbGrid* grid3Din,
    const DbGrid* surf2D,
    ...
    double thickmin)
  {
    ...
    {
      messerr(
        "The number of vertical grid meshes 'nzout' must be strictly positive");
      return grid3Dout;
    }
    if (names.empty())
    {
      messerr(
        "You must designate variable(s) to be copied from input to output 3D "
        "grid");
      return grid3Dout;
    }
    Id ndim = 3;
    ...
    {
      messerr(
        "The thickness of the target Layer seems too small for a "
        "Squeeze-and-Stretch");
      return grid3Dout;
    }
    ...
    grid3Dout = create(
      NXs, DXs, X0s, angles, ELoadBy::fromKey("SAMPLE"), VectorDouble(),
      VectorString(), VectorString(), 1, false);
   ...
   *   'top' and 'bot' are defined and correctly ordered (o save time)
   */
  DbGrid* DbGrid::createSqueezeAndStretchBackward(
    const DbGrid* grid3Din,
    const DbGrid* surf2D,
    const String& nameTop,
    const String& nameBot,
    ...
    for (Id ix = 0, nx = grid3Dout->getNX(0); ix < nx; ix++)
      for (Id iy = 0, ny = grid3Dout->getNX(1); iy < ny; iy++)
      {
        ...
        for (Id ivar = 0; ivar < nvar; ivar++)
        {
          ...
          VH::squeezeAndStretchInPlaceBackward(
            vecin, vecout, z0out, dzout, top, bot);
   ...
   */
  VectorVectorInt DbGrid::getLimitsFromVariableExtend(
    const String& nameTop,
    const String& nameBot,
    const VectorInt& dimExclude) const
  {
    ...
    for (Id iexc = 0, nexc = static_cast<Id>(dimExclude.size()); iexc < nexc;
         iexc++)
    {
      vec[iexc][0] = 0;
   ...
   */
  Id DbGrid::setSelectionFromVariableExtend(
    const String& nameTop,
    const String& nameBot)
  {
   ...
   */
  void DbGrid::clean3DFromSurfaces(
    const VectorString& names,
    const DbGrid* surf2D,
    const String& nameTop,
    const String& nameBot,
    bool verbose)
  {
    ...
    {
      messerr(
        "The grid 'surf2D' and the current one should coincide horizontally");
      return;
    }
    if (names.empty())
    {
      ...
      for (Id iy = 0, ny = getNX(1); iy < ny; iy++)
      {
        ...
        if (flagRead)
        {
          for (Id ivar = 0; ivar < nvar; ivar++)
          {
            ...
            // Blank out
            for (Id iz = 0; iz < indzmin; iz++, nmodif3D++) vec[iz] = TEST;
            for (Id iz = indzmax; iz < nz; iz++, nmodif3D++) vec[iz] = TEST;
  ...
  VectorInt DbGrid::locateDataInGrid(
    const Db* data,
   ...
   * @return
   */
  Id DbGrid::addSelectionFromDbByMorpho(
    Db* db,
    Id nmin,
    Id radius,
    Id option,
    ...
    const NamingConvention& namconv)
  {
    ...
    Id err = morpho(
      EMorpho::DILATION, 0.5, 1.5, option, dilation, false, verbose, namconv);
  ...
  void DbGrid::getSampleAsSTInPlace(Id iech, SpaceTarget& P) const
  {
    ...
    // Load the extension
    if (P.checkExtend()) P.setExtend(getBlockExtensions(iech));
  }
   ...
   * random numbers.
   */
  VectorVectorDouble DbGrid::getDiscretizedBlock(
    ...
    Id seed) const
  {
    ...
    for (Id i = 0; i < ntot; i++)
    {
      ...
      {
        double taille =
          (!flagPerCell) ? getDX(idim) : getLocVariable(ELoc::BLEX, iech, idim);
        Id nd = ndiscs[idim];
        nval /= nd;
        Id j = jech / nval;
        jech -= j * nval;
        ...
        if (!flagRandom)
          ...
          discs[i][idim] =
            local + taille * law_uniform(-0.5, 0.5) / static_cast<double>(nd);
      }
    }
    ...
    bool flagAddSampleRank)
  {
    ...
    dbout = DbGrid::create(
      nx, dx, x0, dbin->getAngles(), ELoadBy::COLUMN, VectorDouble(),
      VectorString(), VectorString(), flagAddSampleRank);
  ...
  VectorDouble DbGrid::getDistanceToOrigin(
    const VectorInt& origin,
