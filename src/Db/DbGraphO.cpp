...
namespace gstlrn
{
                                        ...
                                        bool flagAddSampleRank)
  {
    ...
    if (dbgraphO->resetFromSamples(nech,
                                   order,
                                   tab,
                                   NF_arcs,
                                   names,
  ...
  {
    DbGraphO* dbgraphO = new DbGraphO;
    if (dbgraphO->resetFromMatrix(nech,
                                  order,
                                  tab,
                                  MatArcs,
                                  names,
  ...
  bool DbGraphO::isConsistent() const
  {
    ...
    for (Id irow = 0, nrows = _downArcs.getNRows(); irow < nrows; irow++)
      for (Id icol = 0, ncols = _downArcs.getNCols(); icol < ncols; icol++)
      {
        ...
        if (value < 0)
        {
          messerr("The value for Arc(%d; %d) may not be negative (%lf)",
                  irow,
                  icol,
                  value);
          return false;
  ...
  bool DbGraphO::_isValidArcRank(Id iarc) const
  {
    ...
    if (iarc >= narcs)
    {
      messerr("Argument 'iarc' (%d) should be smaller than Number of arcs (%d)",
              iarc,
              narcs);
      return false;
    }
