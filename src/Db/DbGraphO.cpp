...
namespace gstlrn
{
  ...
  DbGraphO* DbGraphO::createFromSamples(
    Id nech,
    const ELoadBy& order,
    ...
    bool flagAddSampleRank)
  {
    ...
    if (dbgraphO->resetFromSamples(
          nech, order, tab, NF_arcs, names, locatorNames, flagAddSampleRank))
    {
  ...
  DbGraphO* DbGraphO::createFromMatrix(
    Id nech,
    const ELoadBy& order,
    const VectorDouble& tab,
  ...
  {
    DbGraphO* dbgraphO = new DbGraphO;
    if (dbgraphO->resetFromMatrix(
          nech, order, tab, MatArcs, names, locatorNames, flagAddSampleRank))
    {
      messerr("Error when creating DbGraphO from Samples");
      delete dbgraphO;
      return nullptr;
  ...
  bool DbGraphO::isConsistent() const
  {
    ...
    {
      messerr(
        "Number of columns of '_connectedArcs' (%d)", _downArcs.getNCols());
      messerr("must not be larger than Sample Number (%d)", nech);
      return false;
    }
    ...
    for (Id irow = 0, nrows = _downArcs.getNRows(); irow < nrows; irow++)
      for (Id icol = 0, ncols = _downArcs.getNCols(); icol < ncols; icol++)
      {
        ...
        {
          messerr(
            "The value for Arc(%d; %d) may not be negative (%lf)", irow, icol,
            value);
          return false;
        }
      }
  ...
  bool DbGraphO::_isValidArcRank(Id iarc) const
  {
    ...
    {
      messerr(
        "Argument 'iarc' (%d) should be smaller than Number of arcs (%d)", iarc,
        narcs);
      return false;
    }
    return true;
  ...
  bool DbGraphO::_isValidNode(Id node) const
  {
    ...
    {
      messerr(
        "Argument 'node' (%d) should be smaller than Number of Samples (%d)",
        node, nodeNumber);
      return false;
    }
    return true;
  ...
  void
    DbGraphO::_updateCumul(Id rank, const VectorDouble& v, VectorDouble& cumul)
  {
    Id nech = static_cast<Id>(v.size());
  ...
  VectorInt DbGraphO::getIndicesNextDown(Id node) const
  {
    ...
    v1[node] = 1.;
    AMatrix::productInPlace(v2, v1, _downArcs);
    return _getRanks(v2);
  }
  ...
  VectorInt DbGraphO::getIndicesNextUp(Id node) const
  {
    ...
    VectorDouble v2(nech, 0.);
    v1[node] = 1.;
    AMatrix::productInPlace(v2, _downArcs, v1);
    return _getRanks(v2);
  }
   ...
   */
  void DbGraphO::_iterateCumul(
    const VectorInt& inds,
    VectorDouble& cumul,
    VectorDouble& v1,
