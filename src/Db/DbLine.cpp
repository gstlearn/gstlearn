...
namespace gstlrn
{
  ...
  bool DbLine::_isLineNumberValid(Id iline) const
  {
    ...
    if (iline >= getNLine())
    {
      messerr("ilin' (%d) should be smaller than Number of Lines (%d)",
              iline,
              getNLine());
      return false;
    }
                                    ...
                                    bool flagAddSampleRank)
  {
    ...
    if (dbline->resetFromSamples(nech,
                                 order,
                                 tab,
                                 lineCounts,
                                 names,
  ...
  {
    DbLine* dbline = new DbLine;
    if (dbline->resetFromSamplesById(nech,
                                     order,
                                     tab,
                                     lineIds,
                                     ranksPerId,
                              ...
                              const VectorInt& ranksPerId)
  {
    ...
    if (static_cast<Id>(ranksPerId.size()) != nech)
    {
      messerr(
        ...
        static_cast<Id>(ranksPerId.size()),
        nech);
      return 1;
    }
                                   ...
                                   Id seed)
  {
    ...
    DbLine* dbline = createFromSamples(nech,
                                       ELoadBy::SAMPLE,
                                       tab,
                                       lineCounts,
                                       names,
  ...
  Id DbLine::getLineSampleRank(Id iline, Id isample) const
  {
    ...
    {
      messerr("Error in Sample number (%d) in line (%d): it must lie within "
              "[0, %d]\n",
              isample,
              iline,
              nsample);
                                         ...
                                         Id byZ)
  {
    ...
    DbLine* dbline = new DbLine;
    if (dbline
          ->resetFromSamples(nech, ELoadBy::SAMPLE, tab, lineCounts, locnames))
      return nullptr;
  ...
  bool DbLine::serializeH5(H5::Group& grp) const
  {
    ...
    for (Id iline = 0, nbline = getNLine(); iline < nbline; iline++)
    {
      ...
      ret = ret
         && SerializeHDF5::writeValue(lineG,
                                      "NSamples",
                                      getNSamplePerLine(iline));
