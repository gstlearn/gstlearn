...
namespace gstlrn
{
  ...
  bool DbLine::_isLineNumberValid(Id iline) const
  {
    ...
    {
      messerr(
        "ilin' (%d) should be smaller than Number of Lines (%d)", iline,
        getNLine());
      return false;
    }
    return true;
  ...
  DbLine* DbLine::createFromSamples(
    Id nech,
    const ELoadBy& order,
    ...
    bool flagAddSampleRank)
  {
    ...
    if (dbline->resetFromSamples(
          nech, order, tab, lineCounts, names, locatorNames, flagAddSampleRank))
    {
  ...
  DbLine* DbLine::createFromSamplesById(
    Id nech,
    const ELoadBy& order,
    const VectorDouble& tab,
  ...
  {
    DbLine* dbline = new DbLine;
    if (dbline->resetFromSamplesById(
          nech, order, tab, lineIds, ranksPerId, names, locatorNames,
          flagAddSampleRank))
    {
  ...
  Id DbLine::_lineLinkage(const VectorInt& lineCounts)
  {
    ...
    {
      messerr(
        "Cumulated number of samples given by 'lineCounts' (%d) should "
  ...
  Id DbLine::_lineLinkageById(
    const VectorInt& linesId,
    const VectorInt& ranksPerId)
  {
    ...
    {
      messerr(
        "Dimension of 'linesId' (%d) should match Number of samples (%d)",
    ...
    {
      messerr(
        "Dimension of 'ranksPerId' (%d) should match Number of samples (%d)",
   ...
   */
  Id DbLine::resetFromSamplesById(
    Id nech,
    const ELoadBy& order,
    const VectorDouble& tab,
    const VectorInt& lineIds,
  ...
  bool DbLine::_serializeAscii(std::ostream& os) const
  {
    ...
    {
      ret = ret
         && _recordWrite<Id>(os, "Number of Samples", getNSamplePerLine(iline));
   ...
   */
  DbLine* DbLine::createFillRandom(
    Id ndim,
    Id nbline,
    Id nperline,
    double deltaX,
    ...
    Id seed)
  {
    ...
    for (Id iline = 0; iline < nbline; iline++)
    {
      ...
      for (Id idim = 0; idim < ndim; idim++)
      {
        coor0[iline][idim] =
          (idim == 0) ? deltaX * iline
                          + deltaX * law_uniform(1. - unifDelta, 1. + unifDelta)
                      : 0.;
      }
    }
    ...
    VectorString locnames =
      generateMultipleNames(String{ELoc::X.getKey()}, ndim, "");
    DbLine* dbline = createFromSamples(
      nech, ELoadBy::SAMPLE, tab, lineCounts, names, locnames);
  ...
  bool DbLine::isConsistent() const
  {
    ...
    if (nech != getNTotal())
    {
      messerr("The number of samples contained in the Db (%d)", getNSample());
      messerr(
        "is not equal to the number of addresses referenced in DbLine (%d)",
        getNTotal());
      return false;
    }
  ...
  Id DbLine::getLineSampleRank(Id iline, Id isample) const
  {
    ...
    {
      messerr(
        "Error in Line number (%d): it must lie within [0, %d]\n", iline,
        getNLine());
      return -1;
    }
    ...
    {
      messerr(
        "Error in Sample number (%d) in line (%d): it must lie within [0, "
        "%d]\n",
  ...
  DbLine* DbLine::createVerticalFromGrid(
    const DbGrid& grid,
    const VectorString& names,
    ...
    Id byZ)
  {
    ...
    for (Id ivar = 0; ivar < nvar; ivar++) locnames.push_back(names[ivar]);

    DbLine* dbline = new DbLine;
    if (dbline->resetFromSamples(
          nech, ELoadBy::SAMPLE, tab, lineCounts, locnames))
      return nullptr;

    return dbline;
  }

  DbLine* DbLine::createMarkersFromGrid(
    const DbGrid& grid,
    const String& name,
    const VectorInt& xranks,
    const VectorInt& yranks,
    ...
    for (Id iline = 0, nbline = getNLine(); iline < nbline; iline++)
    {
      ...
      ret = ret
         && SerializeHDF5::writeValue(
              lineG, "NSamples", getNSamplePerLine(iline));
