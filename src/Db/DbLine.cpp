...
/**
 * @brief Create a DbLine from the following information provided as input arguments
 *
 * @param ndim  Space dimension
 * @param nbline Number of Lines
 * @param nperline Number of samples per line
 * @param deltaZ Average distance between consecutive samples along a line
 ...
 * not defined)
 * @param flagLine when True, add the line number
 * @param flagLayer When True, add the Layer number
 * @param seed Seed used for the random number generator
 * @return DbLine* Pointer to the newly created DbLine structure
 *
 * @remarks The arguments 'coormin' and 'coormax' are not considered in last space dimension
 */
                                 ...
                                 Id seed)
{
  ...
  {
    mini[idim] = (static_cast<Id>(coormin.size()) == ndim) ? coormin[idim] : 0.;
    maxi[idim] = (static_cast<Id>(coormax.size()) == ndim) ? coormax[idim] : 1.;
  }

  // Generate the number of samples per linesamples along the lines
  VectorInt lineCounts = VH::initVInt(nbline, nperline);

  // Generate the coordinates of the first sample per line
  VectorVectorDouble coor0(ndim);
  for (Id idim = 0; idim < ndim; idim++)
  {
    coor0[idim].resize(nbline);
    for (Id iline = 0; iline < nbline; iline++)
      coor0[idim][iline] = law_uniform(mini[idim], maxi[idim]);
  }

  // Loop on the lines
  VectorDouble tab;
  for (Id il = 0; il < nbline; il++)
  {
    // Loop on the number of samples per line
    for (Id ip = 0; ip < nperline; ip++)
    {
      // Assign the ndim-1 first coordinates
      for (Id idim = 0; idim < ndim - 1; idim++)
      {
        double coorH = coor0[idim][il];
        if (randH > 0) coorH += randH * law_uniform(-0.5, 0.5);
        tab.push_back(coorH);
      }

      // Assign the last coordinate
      double coorV = mini[ndim - 1] + ip * deltaZ + deltaZ * randV * law_uniform(-0.5, 0.5);
      tab.push_back(coorV);
  ...
  if (flagLine)
  {
    const String locname(ELoc::C.getKey());
    locnames.push_back(locname);
    names.push_back("Line");
  }
  if (flagLayer)
  {
    const String locname(ELoc::LAYER.getKey());
    locnames.push_back(locname);
    names.push_back("Layer");
  }
  DbLine* dbline = createFromSamples(nbline * nperline, ELoadBy::SAMPLE, tab, lineCounts,
                                     names, locnames);

  // Next line is a fixup (as 'locname' does not function for CODE)
  if (flagLine) dbline->setLocator("Line", ELoc::C);
  return dbline;
}
