...
namespace gstlrn
{
  ...
  DbMeshStandard* DbMeshStandard::create(
    Id ndim,
    Id napexpermesh,
    const VectorDouble& apices,
    const VectorInt& meshes,
    ...
    bool verbose)
  {
    DbMeshStandard* dbmesh = new DbMeshStandard(
      ndim, napexpermesh, apices, meshes, order, tab, names, locatorNames,
      verbose);
    if (dbmesh == nullptr)
    {
  ...
  DbMeshStandard* DbMeshStandard::createFromExternal(
   ...
   */
  DbMeshStandard*
    DbMeshStandard::createFromNF(const String& NFFilename, bool verbose)
  {
    DbMeshStandard* dbmesh = new DbMeshStandard;
  ...
  void DbMeshStandard::getCoordinatesPerMeshInPlace(
    Id imesh,
    Id rank,
  ...
  void
    DbMeshStandard::getApexCoordinatesInPlace(Id i, VectorDouble& coords) const
  {
    for (Id idim = 0; idim < getNDim(); idim++)
  ...
  VectorDouble
    DbMeshStandard::getCoordinatesPerMesh(Id imesh, Id idim, bool flagClose)
      const
  {
    VectorDouble vec;
