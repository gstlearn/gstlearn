...
namespace gstlrn
{
  ...
  void GeometryHelper::_decodeConvRot(
   ...
   *****************************************************************************/
  void
    GeometryHelper::rotationGetSinCos(double angle, double* cosa, double* sina)
  {
    double value;
  ...
  void GeometryHelper::rotationMatrixIdentityInPlace(Id ndim, VectorDouble& rot)
  {
    ...
    for (i = ecr = 0; i < ndim; i++)
      for (j = 0; j < ndim; j++, ecr++) rot[ecr] = (i == j);
  }
   ...
   **
   *****************************************************************************/
  void GeometryHelper::rotation3DMatrixInPlace(
    double alpha,
    double beta,
    double gamma,
    VectorDouble& rot)
   ...
   **
   *****************************************************************************/
  void GeometryHelper::rotationMatrixInPlace(
    Id ndim,
  ...
  void GeometryHelper::rotationMatrixDerivativesInPlace(
    Id ndim,
    const VectorDouble& angles,
    std::vector<MatrixSquare>& dR)
  {
    ...
    {
      dR.resize(3);
      for (Id i = 0; i < 3; i++) dR[i].reset(3, 3);
  ...
  void GeometryHelper::rotation2DMatrixDerivativesInPlace(
    double angle,
    MatrixSquare& dR)
  {
    double ca, sa;
  ...
  void GeometryHelper::rotation3DMatrixDerivativesInPlace(
    const VectorDouble& angles,
    std::vector<MatrixSquare>& dR)
  {
    double ca[3], sa[3];
   ...
   *****************************************************************************/
  VectorDouble
    GeometryHelper::rotationMatrix(Id ndim, const VectorDouble& angles)
  {
    VectorDouble rot;
   ...
   *****************************************************************************/
  void
    GeometryHelper::rotationCopy(Id ndim, const double* rotin, double* rotout)
  {
    for (Id i = 0; i < ndim * ndim; i++) rotout[i] = rotin[i];
  }

  void GeometryHelper::rotationGetAnglesInPlace(
    const VectorDouble& rot,
    VectorDouble& angles)
  {
    ...
    double* angles)
  {
    ...
    for (Id i = 0; i < ndim; i++) angles[i] = 0.;
    ...
    for (Id i = 0; i < nval; i++) angles[i] = ut_rad2deg(angles[i]);
  }
   ...
   **
   *****************************************************************************/
  void GeometryHelper::rotationGetAnglesFromCodirInPlace(
   ...
   * @return
   */
  VectorDouble GeometryHelper::rotationGetAngles(
    const VectorDouble& codir,
    bool flagResize)
  {
    Id ndim = static_cast<Id>(codir.size());
   ...
   * @return VectorDouble
   */
  VectorDouble GeometryHelper::rotationFromIncrements(
    ...
    bool flagDegree)
  {
    ...
    if (ndim == 2)
    {
      ...
      angles.push_back(
        0.); // To make the number of returned values equal to 2 for 2D
    }
    else
    {
      ...
      double pitch =
        atan2(-incr[2], sqrt(incr[0] * incr[0] + incr[1] * incr[1]));
   ...
   **
   *****************************************************************************/
  void GeometryHelper::rotationGetDirection2D(
    const VectorDouble& angles,
    VectorDouble& codir)
  {
    ...
    for (Id i = 0; i < ndim * ndir; i++) codir[i] = 0.;
   ...
   *****************************************************************************/
  bool
    GeometryHelper::rotationIsIdentity(Id ndim, const double* rot, double eps)
  {
    for (Id i = 0; i < ndim; i++)
      for (Id j = 0; j < ndim; j++)
      {
   ...
   **
   *****************************************************************************/
  double GeometryHelper::distancePointToSegment(
    double x0,
    double y0,
    double x1,
    double y1,
   ...
   **
   *****************************************************************************/
  double GeometryHelper::geodeticAngularDistance(
    double long1,
    double lat1,
    double long2,
    double lat2,
   ...
   **
   *****************************************************************************/
  static double st_convertGeodeticAngle(
    double /*sina*/,
    double cosa,
    double sinb,
    double cosb,
   ...
   **
   *****************************************************************************/
  void GeometryHelper::geodeticAngles(
    double long1,
    double lat1,
    double long2,
    double lat2,
   ...
   **
   *****************************************************************************/
  double GeometryHelper::geodeticTrianglePerimeter(
    double long1,
    double lat1,
    double long2,
    double lat2,
    ...
    double lat3)
  {
    ...
    GH::geodeticAngles(
      long1, lat1, long2, lat2, long3, lat3, &a, &b, &c, &ga, &gb, &gc);
    return (a + b + c);
  }
   ...
   **
   *****************************************************************************/
  double GeometryHelper::geodeticTriangleSurface(
    double long1,
    double lat1,
    double long2,
    double lat2,
   ...
   **
   *****************************************************************************/
  bool GeometryHelper::isInSphericalTriangleOptimized(
    const double* coor,
    const double* ptsa,
    const double* ptsb,
    const double* ptsc,
    ...
    double eps)
  {
    ...
    double dab =
      GH::geodeticAngularDistance(ptsa[0], ptsa[1], ptsb[0], ptsb[1]);
    double sinab = sin(dab);
    double cosab = cos(dab);
    double dbc =
      GH::geodeticAngularDistance(ptsb[0], ptsb[1], ptsc[0], ptsc[1]);
    double sinbc = sin(dbc);
    double cosbc = cos(dbc);
    double dac =
      GH::geodeticAngularDistance(ptsa[0], ptsa[1], ptsc[0], ptsc[1]);
    double sinac = sin(dac);
    double cosac = cos(dac);

    double A =
      st_convertGeodeticAngle(sinbc, cosbc, sinac, cosac, sinab, cosab);
    double B =
      st_convertGeodeticAngle(sinac, cosac, sinab, cosab, sinbc, cosbc);
    double C =
      st_convertGeodeticAngle(sinab, cosab, sinbc, cosbc, sinac, cosac);
    double stot = (A + B + C - GV_PI);

    double d0b =
      GH::geodeticAngularDistance(coor[0], coor[1], ptsb[0], ptsb[1]);
    double sin0b = sin(d0b);
    double cos0b = cos(d0b);
    double d0c =
      GH::geodeticAngularDistance(coor[0], coor[1], ptsc[0], ptsc[1]);
    double sin0c = sin(d0c);
    double cos0c = cos(d0c);
    double OA =
      st_convertGeodeticAngle(sinbc, cosbc, sin0c, cos0c, sin0b, cos0b);
    double BA =
      st_convertGeodeticAngle(sin0c, cos0c, sin0b, cos0b, sinbc, cosbc);
    double CA =
      st_convertGeodeticAngle(sin0b, cos0b, sinbc, cosbc, sin0c, cos0c);
    s[0] = (OA + BA + CA - GV_PI);
    total += s[0];
    if (total > stot + eps) return false;

    double d0a =
      GH::geodeticAngularDistance(coor[0], coor[1], ptsa[0], ptsa[1]);
    double sin0a = sin(d0a);
    double cos0a = cos(d0a);
    double AB =
      st_convertGeodeticAngle(sin0c, cos0c, sinac, cosac, sin0a, cos0a);
    double OB =
      st_convertGeodeticAngle(sinac, cosac, sin0a, cos0a, sin0c, cos0c);
    double CB =
      st_convertGeodeticAngle(sin0a, cos0a, sin0c, cos0c, sinac, cosac);
    s[1] = (AB + OB + CB - GV_PI);
    total += s[1];
    if (total > stot + eps) return false;

    double AC =
      st_convertGeodeticAngle(sin0b, cos0b, sin0a, cos0a, sinab, cosab);
    double BC =
      st_convertGeodeticAngle(sin0a, cos0a, sinab, cosab, sin0b, cos0b);
    double OC =
      st_convertGeodeticAngle(sinab, cosab, sin0b, cos0b, sin0a, cos0a);
    s[2] = (AC + BC + OC - GV_PI);
    total += s[2];
    if (ABS(total - stot) > eps) return false;

    for (Id i = 0; i < 3; i++) wgts[i] = s[i] / total;
    return true;
  }
   ...
   **
   *****************************************************************************/
  bool GeometryHelper::segmentIntersect(
    double xd1,
    double yd1,
    double xe1,
    double ye1,
   ...
   **
   *****************************************************************************/
  bool GeometryHelper::isInSphericalTriangle(
    double* coor,
    double surface,
    double* pts1,
    double* pts2,
    ...
    double eps)
  {
    ...
    double total = 0.;
    s[0] = GH::geodeticTriangleSurface(
      coor[0], coor[1], pts2[0], pts2[1], pts3[0], pts3[1]);
    total += s[0];
    if (total > surface + eps) return false;
    s[1] = GH::geodeticTriangleSurface(
      pts1[0], pts1[1], coor[0], coor[1], pts3[0], pts3[1]);
    total += s[1];
    if (total > surface + eps) return false;
    s[2] = GH::geodeticTriangleSurface(
      pts1[0], pts1[1], pts2[0], pts2[1], coor[0], coor[1]);
    total += s[2];
   ...
   **
   *****************************************************************************/
  void GeometryHelper::rotationGetRandomDirection(
    double ct,
    double st,
    const double* a,
    double* codir)
  {
    ...
    double rd = 0.;
    for (Id k = 0; k < 3; k++) rd += codir[k] * a[k];
    for (Id k = 0; k < 3; k++) p[k] = rd * a[k];
    for (Id k = 0; k < 3; k++) b[k] = codir[k] - p[k];

    rd = 0.;
    for (Id k = 0; k < 3; k++) rd += b[k] * b[k];

    rd = sqrt(rd);
    for (Id k = 0; k < 3; k++) b[k] /= rd;

    c[0] = a[1] * b[2] - a[2] * b[1];
    c[1] = a[2] * b[0] - a[0] * b[2];
    c[2] = a[0] * b[1] - a[1] * b[0];

    for (Id k = 0; k < 3; k++) codir[k] = p[k] + rd * (ct * b[k] + st * c[k]);
  }

  VectorVectorDouble GeometryHelper::convert3DToLongLat(
    ...
    double radius_arg)
  {
    ...
    tab.resize(2);
    for (Id idim = 0; idim < 2; idim++) tab[idim].resize(number, TEST);
    ...
    for (Id ip = 0; ip < number; ip++)
    {
      ...
      double yy = y[ip];
      double zz = z[ip];
      if (!FFFF(xx) && !FFFF(yy) && !FFFF(zz))
        convertCart2Sph(xx, yy, zz, &tab[0][ip], &tab[1][ip], radius);
    }
    return tab;
  }

  /**
   * Returns the Vector of Longitude/Latitude corresponding to Sample coordinates in 3-D
   * @param longitude Array of longitude values
   * @param latitude  Array of latitude values
   * @param dilate    Dilation factor
   * @param radius_arg    Radius (if not defined, taken from variety definition)
   * @return
   */
  VectorVectorDouble GeometryHelper::convertLongLatTo3D(
    const VectorDouble& longitude,
    ...
    double radius_arg)
  {
    ...
    // Dimension the returned argument
    tab.resize(3);
    for (Id idim = 0; idim < 3; idim++) tab[idim].resize(number, TEST);
    ...
    for (Id ip = 0; ip < number; ip++)
    {
      ...
      if (!FFFF(lon) && !FFFF(lat))
        convertSph2Cart(
          lon, lat, &tab[0][ip], &tab[1][ip], &tab[2][ip], radius);
    }
    return tab;
  }
   ...
   **
   *****************************************************************************/
  void GeometryHelper::convertCart2Sph(
    double x,
    double y,
    double z,
    double* rlong,
   ...
   **
   *****************************************************************************/
  void GeometryHelper::convertSph2Cart(
    double rlong,
    double rlat,
    double* x,
    double* y,
   ...
   * @param dzoverdy Partial derivative along Y
   */
  MatrixSquare GeometryHelper::gradXYToRotmat(double dzoverdx, double dzoverdy)
  {
    Id ndim = 3;
    VectorDouble axis(ndim, 0.);
    ...
    for (Id idim = 0; idim < ndim; idim++) axis[idim] /= normcross;
   ...
   */
  VectorDouble GeometryHelper::rotationToEuler(
    const MatrixSquare& M,
    const ERotation& convrot,
    double eps)
  {
    ...
    {
      double sy = sqrt(
        M.getValue(i, j) * M.getValue(i, j)
        + M.getValue(i, k) * M.getValue(i, k));
      if (sy > eps)
      {
    ...
    {
      double cy = sqrt(
        M.getValue(i, i) * M.getValue(i, i)
        + M.getValue(j, i) * M.getValue(j, i));
      if (cy > eps)
      {
   ...
   * @remark https://github.com/matthew-brett/transforms3d/blob/master/transforms3d/euler.py
   */
  MatrixSquare GeometryHelper::EulerToRotation(
    const VectorDouble& angles,
    const ERotation& convrot)
  {
    Id firstaxis, parity, repetition, frame;
  ...
  std::vector<MatrixSquare> GeometryHelper::EulerToRotationDerivatives(
    const VectorDouble& angles,
    const ERotation& convrot)
  {
  ...
  void GeometryHelper::EulerToRotationDerivativesInPlace(
    std::vector<MatrixSquare>& dR,
    const VectorDouble& angles,
    const ERotation& convrot)
  {
  ...
  double GeometryHelper::formatAngle(double anglein, double basis)
  {
    ...
    if (angle < 0)
    {
      while (angle < 0.) angle += basis;
    }
    else
    {
      while (angle > basis) angle -= basis;
    }
    return angle;
  }

  VectorDouble
    GeometryHelper::formatAngles(const VectorDouble& anglesin, double basis)
  {
  ...
  VectorDouble GeometryHelper::rayTriangleIntersect(
    ...
    const VectorDouble& v2)
  {
    ...
    VectorDouble v20 = v2 - v0;
    VectorDouble v10 = v1 - v0;
   ...
   */
  VectorVectorDouble GeometryHelper::sphBarCoord(
    const VectorVectorDouble& sphPts,
    ...
    const MatrixInt& meshes)
  {
    ...
    for (Id i = 0; i < 4; i++) res[i].resize(np, 0.);
    ...
    for (Id k = 0; k < np; k++)
    {
      ...
      while (i < nmeshes && notFound)
      {
        ...
        w = rayTriangleIntersect(
          sphPts[k], apices.getRow(iv0), apices.getRow(iv1),
          apices.getRow(iv2));
  ...
  VectorVectorDouble GeometryHelper::getEllipse(
