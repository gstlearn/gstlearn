                                                      ...
                                                      double radius_arg)
{
  ...
  for (Id ip = 0; ip < number; ip++)
  {
    ...
    if (!FFFF(xx) && !FFFF(yy) && !FFFF(zz))
      convertCart2Sph(xx, yy, zz,
                      &tab[0][ip], &tab[1][ip], radius);
  }
  return tab;
}
 ...
 * @return
 */
VectorVectorDouble GeometryHelper::convertLongLatTo3D(const VectorDouble& longitude,
                                                      ...
                                                      double radius_arg)
{
  ...
  for (Id ip = 0; ip < number; ip++)
  {
    ...
    if (!FFFF(lon) && !FFFF(lat))
      convertSph2Cart(lon, lat,
                      &tab[0][ip], &tab[1][ip], &tab[2][ip], radius);
  }
  return tab;
}
...
double util_rotation_gradXYToAngle(double dzoverdx, double dzoverdy)
{
  ...
  double normcross = sqrt(VH::innerProductVD(axis, axis));
  double dot       = VH::innerProductVD(vert, vort);
                                             ...
                                             double eps)
{
  ...
  {
    double sy = sqrt(
      M.getValue(i, j) * M.getValue(i, j) + M.getValue(i, k) * M.getValue(i, k));
    if (sy > eps)
    {
  ...
  {
    double cy = sqrt(
      M.getValue(i, i) * M.getValue(i, i) + M.getValue(j, i) * M.getValue(j, i));
    if (cy > eps)
    {
                                                  ...
                                                  const VectorDouble& v2)
{
  ...
  double det        = VH::innerProductVD(pvec, v10);

  if (det == 0.) return res;

  double invDet = 1. / det;

  double u = -VH::innerProductVD(pvec, v0) * invDet;
  if (u < 0 || u > 1) return res;

  VectorDouble vm0 = v0;
  VH::multiplyConstant(vm0, -1.);
  VectorDouble qvec = VH::crossProduct3D(vm0, v10);
  double v          = VH::innerProductVD(dir, qvec) * invDet;
  if (v < 0 || u + v > 1) return res;

  res[0] = VH::innerProductVD(qvec, v20) * invDet;
  res[1] = u;
  res[2] = v;
