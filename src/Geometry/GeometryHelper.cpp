...
namespace gstlrn
{
  ...
  void GeometryHelper::rotation2DMatrixDerivativesInPlace(double angle,
                                                          MatrixSquare& dR)
  {
    double ca, sa;
   ...
   *****************************************************************************/
  void
    GeometryHelper::rotationGetAnglesFromCodirInPlace(const VectorDouble& codir,
                                                      VectorDouble& angles)
  {
    Id nval;
   ...
   * @return VectorDouble
   */
  VectorDouble GeometryHelper::rotationFromIncrements(const VectorDouble& incr,
                                                      bool flagDegree)
  {
    VectorDouble angles;
                                             ...
                                             double eps)
  {
    ...
    double total = 0.;
    s[0] = GH::geodeticTriangleSurface(coor[0],
                                       coor[1],
                                       pts2[0],
                                       pts2[1],
                                       pts3[0],
    ...
    s[1] = GH::geodeticTriangleSurface(pts1[0],
                                       pts1[1],
                                       coor[0],
                                       coor[1],
                                       pts3[0],
    ...
    total += s[1];
    if (total > surface + eps) return false;
    s[2] = GH::geodeticTriangleSurface(pts1[0],
                                       pts1[1],
                                       pts2[0],
                                       pts2[1],
                                       coor[0],
  ...
  VectorVectorDouble GeometryHelper::convert3DToLongLat(const VectorDouble& x,
   ...
   */
  VectorVectorDouble
    GeometryHelper::convertLongLatTo3D(const VectorDouble& longitude,
                                               ...
                                               double eps)
  {
    ...
    if (repetition)
    {
      double sy = sqrt(M.getValue(i, j) * M.getValue(i, j)
    ...
    {
      double cy = sqrt(M.getValue(i, i) * M.getValue(i, i)
  ...
  std::vector<MatrixSquare>
    GeometryHelper::EulerToRotationDerivatives(const VectorDouble& angles,
                                               const ERotation& convrot)
  {
   ...
   */
  VectorVectorDouble
    GeometryHelper::sphBarCoord(const VectorVectorDouble& sphPts,
                                ...
                                const MatrixInt& meshes)
  {
    ...
    for (Id k = 0; k < np; k++)
    {
      ...
      while (i < nmeshes && notFound)
      {
        ...
        w = rayTriangleIntersect(sphPts[k],
                                 apices.getRow(iv0),
                                 apices.getRow(iv1),
                                 apices.getRow(iv2));
