...
namespace gstlrn
{
    ...
    : _tree()
  {
    ...
    Id n_features;
    auto internal =
      _getInformationFromDb(dbin, dbout, useSel, &n_samples, &n_features);
    ...
    bool useSel)
  {
    ...
    Id n_features;
    auto internal =
      _getInformationFromDb(db, nullptr, useSel, &n_samples, &n_features);
    if (internal.empty()) return;
    ...
    Id rank)
  {
    ...
    return knn.btree_query_inPlace(
      _tree, internal, 1, n_features, n_neighbors, rank, indices, distances);
  }
  ...
  {
    if (empty()) return 1;
    for (Id i = 0, n = _tree.n_samples; i < n; i++) _tree.available[i] = status;
    return 0;
  }
    ...
    bool likelihood)
  {
    ...
    for (Id jech = 0; jech < nech; jech++)
    {
      ...
      mat(irel, 0) = iech;
      ball.setAvailable(
        iech, true); // Provide absolute ranks (even when selection is present)
    ...
    {
      messerr(
        "In function findNN: irel(%d) should be equal to n1+n2(i.e. %d + %d)",
        irel, n1, n2);
      mat.clear();
    }
  ...
  MatrixT<double> Ball::_getInformationFromDb(
    const Db* dbin,
    const Db* dbout,
    bool useSel,
    Id* n_samples,
  ...
  MatrixT<double> Ball::_getInformationFromMesh(
    const AMesh* mesh,
    Id* n_samples,
    Id* n_features)
  {
