...
namespace gstlrn
{
  ...
  Id t_btree::init_node(Id i_node, Id idx_start, Id idx_end)
  {
    ...
    for (Id i = idx_start; i < idx_end; i++)
      radius = fmax(
        radius, dist_func(
                  centroid.data(), this->data.getRow(this->idx_array[i]).data(),
                  n_featuresLocal));
  ...
  Id find_node_split_dim(
    const MatrixT<double>& data,
  ...
  void t_btree::recursive_build(Id i_node, Id idx_start, Id idx_end)
  {
    ...
    else
    {
      ...
      imax = find_node_split_dim(
        this->data, this->idx_array, n_featuresLocal, n_points);
      partition_node_indices(
        this->data, &this->idx_array[idx_start], imax, n_points, n_mid);
  ...
  double t_btree::min_dist(Id i_node, const constvect pt) const
  {
    const auto dist_func = this->default_distance_function == 1
                           ? euclidean_distance
                           : manhattan_distance;
    double dist_pt = dist_func(
      pt.data(), this->node_bounds.getRow(i_node).data(), this->n_features);
    return (fmax(0.0, dist_pt - this->node_data[i_node].radius));
  }

  void t_btree::query_depth_first(
    Id i_node,
    const constvect pt,
    Id i_pt,
    t_nheap& heap,
  ...
  void t_btree::display(Id level) const
  {
    ...
    for (Id i_node = 0; i_node < this->n_nodes; i_node++)
    {
      ...
      message(
        "Node #%3d/%3d - Indices [%5d; %5d[ - Radius = %lf - Centroid = ",
  ...
  double euclidean_distance(const double* x1, const double* x2, Id n_features)
  {
    ...
    // default space dimension matches n_features.
    // Fall back to plain Euclidean when they differ — e.g. a 1-D Db with a 2-D default space —
    ...
    if (space != nullptr && static_cast<Id>(space->getNDim()) == n_features)
    {
      ...
      // Case where Space dimension matches n_features
