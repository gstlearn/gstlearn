...
namespace gstlrn
{
    ...
    Id n_neigh)
  {
    ...
    {
      messerr(
        "'n_neigh' (%d) must be less than or equal to the number of "
        "training points (%d).",
    ...
    VectorDouble& distances)
  {
    ...
    if (res)
    {
      indices = {_heap.indices.getRow(rank).begin(),
                 _heap.indices.getRow(rank).end()};
      distances = {_heap.distances.getRow(rank).begin(),
                   _heap.distances.getRow(rank).end()};
    }
    return 0;
  }
