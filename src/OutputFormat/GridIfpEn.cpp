...
namespace gstlrn
{
  ...
  Id GridIfpEn::writeInFile()
  {
    ...
    _writeLine(
      0, "MATRIX_NAME              # VPCMatrix_test_export", 0, 0., NULL);
    ...
    _writeLine(
      0, "DATA_PROP                # CHANNEL1", 0, 0., "# Facies proportion");
    _writeLine(0, "##########################", 0, 0., NULL);
  ...
  DbGrid* GridIfpEn::readGridFromFile()
  {
    ...
    if (_readLine(0, "##########################", &dumint, &dumrel))
      return dbgrid;
    if (_readLine(0, "FILE_DESCRIPTION         #", &dumint, &dumrel))
      return dbgrid;
    if (_readLine(0, "APPLICATION              #", &dumint, &dumrel))
      return dbgrid;
    if (_readLine(0, "SURVEY_NAME              #", &dumint, &dumrel))
      return dbgrid;
    if (_readLine(0, "MATRIX_NAME              #", &dumint, &dumrel))
      return dbgrid;
    if (_readLine(0, "METHOD                   #", &dumint, &dumrel))
      return dbgrid;
    if (_readLine(2, "FLOAT_NULL_VALUE         #", &dumint, &test))
      return dbgrid;
    if (_readLine(0, "ROW_COLUMN_ORIENTATION   #", &dumint, &dumrel))
      return dbgrid;
    if (_readLine(0, "REPRESENTATION_CODE      #", &dumint, &dumrel))
      return dbgrid;
    if (_readLine(0, "##########################", &dumint, &dumrel))
      return dbgrid;
    if (_readLine(2, "ANGLE                    #", &dumint, angles.data()))
      return dbgrid;
    if (_readLine(1, "ROW_COUNT                #", &nx[1], &dumrel))
      return dbgrid;
    if (_readLine(1, "COLUMN_COUNT             #", nx.data(), &dumrel))
      return dbgrid;
    if (_readLine(2, "ROW_DISTANCE             #", &dumint, &dx[1]))
      return dbgrid;
    if (_readLine(2, "COLUMN_DISTANCE          #", &dumint, dx.data()))
      return dbgrid;
    if (_readLine(1, "LAYER_COUNT              #", &nx[2], &dumrel))
      return dbgrid;
    if (_readLine(2, "X_ORIGIN                 #", &dumint, x0.data()))
      return dbgrid;
    if (_readLine(2, "Y_ORIGIN                 #", &dumint, &x0[1]))
      return dbgrid;
    if (_readLine(1, "FACIES_COUNT             #", &ncol, &dumrel))
      return dbgrid;
    if (_readLine(0, "DATA_PROP                #", &dumint, &dumrel))
      return dbgrid;
    if (_readLine(0, "##########################", &dumint, &dumrel))
      return dbgrid;
    ...
    {
      messerr(
        "Number of decoded values (%d) is not equal to the number of grid "
        "nodes (%d) x number of attributes (%d)",
        lec, nech, ncol);
      return dbgrid;
    }
