...
namespace gstlrn
{
  ...
  void GridBmp::setColors(
    const VectorInt& reds,
  ...
  Id GridBmp::writeInFile()
  {
    ...
    bool flag_color_scale =
      (_ncolor > 0 && !_reds.empty() && !_greens.empty() && !_blues.empty());
    ...
    for (Id iy = 0; iy < ny; iy++)
    {
      ...
      for (Id jmult = 0; jmult < _nmult; jmult++)
      {
        ...
        Id color = 0;
        for (Id i = 0; i < ipad; i++) (void)fwrite(&color, 1, 1, _file);
      }
    }
  ...
  Id GridBmp::_colorRank(Id iech, Id ncolor, double vmin, double vmax)
  {
    ...
    if (ival < 0)
    {
      if (_flag_low) return COLOR_LOWER;
      return (ival);
    }
    ...
    if (ival >= ncolor)
    {
      if (_flag_high) return COLOR_UPPER;
      return (ncolor - 1);
    }
  ...
  */
   ...
   **
   *****************************************************************************/
  void GridBmp::_rgb2num(Id red, Id green, Id blue, Id a, unsigned char* c)
  {
    DECLARE_UNUSED(a);
    double value;
