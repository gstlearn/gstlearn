...
#define SWAP_INT32(x)                                                          \
  ...
  static float st_ibm2ieee(const float ibm)
  {
    ...
    // CONVERT TO FLOAT
    long IntMantissa = (static_cast<long>(src.c[1]) << 16)
                     + (static_cast<long>(src.c[2]) << 8) + src.c[3];

    float Mantissa =
      static_cast<float>(IntMantissa) / static_cast<float>(0x1000000);
    float PosResult = Mantissa
                    * static_cast<float>(
                        pow(16.0, static_cast<double>((src.c[0] & 0x7F) - 64)));

    if (src.c[0] & 0x80) return -PosResult;
    return PosResult;
  }
   ...
   *****************************************************************************/
  static Id
    st_readFileHeader(FILE* file, Id verbOption, Id* NPerTrace, double* delta)
  {
    ...
    VectorDouble& cotes)
  {
    ...
    if (codefmt == 2 || codefmt == 3)
    {
      ...
      if (verbOption >= 2)
        message(
          "Min(%6d) = %11d - Max(%6d) = %11d\n", minsamp, iminvalue, maxsamp,
          imaxvalue);
    }
    ...
    double cost)
  {
    ...
    for (Id i = 0; i < nbrefpt; i++)
    {
      ...
      message("  . Crossline     = %d\n", refpt[i].xline);
      message(
        "  . Trace along X = %12.4lf (Pred. = %12.4lf)\n", refpt[i].xtrace, xn);
      message(
        "  . Trace along Y = %12.4lf (Pred. = %12.4lf)\n", refpt[i].ytrace, yn);
    }
  }
   ...
   **
   *****************************************************************************/
  static void st_grid_from_3refpt(
    RefPt refpt[3],
    RefStats& refstats,
    double dz,
    Grid& def_grid)
  {
    ...
    nz = static_cast<Id>((refstats.zmaxl - refstats.zminl) / dz);
   ...
   **
   *****************************************************************************/
  static void st_grid_from_2refpt(
    RefPt refpt[3],
    RefStats& refstats,
    double dz,
    Grid& def_grid)
   ...
   **
   *****************************************************************************/
  static Id st_complete_squeeze_and_stretch(Id ntab, VectorDouble& tab)
  {
    double v1;
    ...
    *cote_ret = TEST;
    ...
    switch (option)
    {
      ...
      case 1: // Flattening from Top surface
        ...
        iz1 = iz2 = static_cast<Id>((nz - 1) - (cztop - cote) / delta);
        if (ABS(iz1) > nz) return false;
   ...
   */
  static Id
    st_identify_trace_rank(DbGrid* surfaces, double xtrace, double ytrace)
  {
    // Check if the surface file has been defined
    ...
    RefStats& refstats)
  {
    ...
    if (iaux_top >= 0)
    {
      ...
      refstats.auxtop = TEST;
      (void)st_within_layer(
        z0, delta, cztop, czbot, cote, option, nz, &iz1, &iz2,
        &refstats.auxtop);
    }
    ...
    RefStats& refstats)
  {
    ...
    *nbvalues = 0;
    for (Id i = 0; i < nPerTrace; i++) writes[i] = TEST;
    ...
    for (Id i = 0; i < nPerTrace; i++)
    {
      ...
      if (!st_within_layer(
            z0, delta, cztop, czbot, cote, option, nz, &iz1, &iz2, &cote_ret))
        continue;
      ...
      if (!FFFF(dval))
      {
        if (dval < refstats.vming || FFFF(refstats.vming))
          refstats.vming = dval;
        if (dval > refstats.vmaxg || FFFF(refstats.vmaxg))
          refstats.vmaxg = dval;
        if (dval < refstats.vminl || FFFF(refstats.vminl))
          refstats.vminl = dval;
        if (dval > refstats.vmaxl || FFFF(refstats.vmaxl))
          refstats.vmaxl = dval;
      }

      // Writing in temporary array

      for (Id iz = iz1; iz <= iz2; iz++) writes[iz] = dval;
    }
    ...
    RefStats& refstats)
  {
    ...
    if (flag_surf)
      message(
        "(Statistics are calculated taking Surface information into "
        "account)\n");
   ...
   **
   *****************************************************************************/
  static void st_refstats_update(
    Id iline,
    Id xline,
    double xtrace,
    double ytrace,
    RefStats& refstats)
  {
    if (xtrace < refstats.xming || FFFF(refstats.xming))
      refstats.xming = xtrace;
    if (xtrace > refstats.xmaxg || FFFF(refstats.xmaxg))
      refstats.xmaxg = xtrace;
    if (ytrace < refstats.yming || FFFF(refstats.yming))
      refstats.yming = ytrace;
    if (ytrace > refstats.ymaxg || FFFF(refstats.ymaxg))
      refstats.ymaxg = ytrace;
    ...
    Id codefmt)
  {
    ...
    Id nPerTrace, iatt_bot, iatt_top, iaux_top, iaux_bot;
    Id iline, xline, nbvalues;
    RefPt refpt[3];
    RefStats refstats;
    ...
    if (st_surface_identify(
          verbOption, surf2D, bot_name, flag_bot, &iatt_bot, top_name, flag_top,
          &iatt_top, top_aux, &iaux_top, bot_aux, &iaux_bot))
      return segyarg;
    ...
    while (1)
    {
      ...
      st_get_trace_params(
        &traceHead_, &iline, &xline, &delta, &xtrace, &ytrace);

      // Read the Trace contents

      if (st_read_trace(
            file, codefmt, refstats.nbtrace, nPerTrace, delta, verbOption,
            values, cotes))
        continue;
      ...
      if (st_reject_trace(
            iline, xline, iline_min, iline_max, xline_min, xline_max))
        continue;
      ...
      // Locate the trace
      Id rank = st_identify_trace_rank(surf2D, xtrace, ytrace);

      // Compare to the 2-D Surface information
      // If 'surfaces' is not provided, bounds are set to TEST.

      if (st_get_cuts(
            surf2D, rank, iatt_top, iatt_bot, xtrace, ytrace, thickmin, &cztop,
            &czbot))
        continue;
      ...
      if (st_load_trace(
            nPerTrace, nz, option, z0, delta, czbot, cztop, cotes, values,
            writes, &nbvalues, refstats))
        continue;

      // Project the auxiliary surface variables (optional)

      st_auxiliary(
        surf2D, rank, nz, option, z0, delta, czbot, cztop, iaux_bot, iaux_top,
        refstats);
      ...
      for (Id iz = 0; iz < nz; iz++) local[iz] = writes[iz];
      segyarg.tab.push_back(local);
      for (Id iz = 0; iz < nz; iz++) local[iz] = cotes[iz];
    ...
    Id codefmt)
  {
    double xtrace, ytrace;
    Id iline, xline, nbvalues;
    Id nPerTrace, iatt_top = 0, iatt_bot = 0, iaux_top = 0, iaux_bot = 0;
    RefPt refpt[3];
    RefStats refstats;
    Grid def_grid;
    VectorDouble values, cotes, writes;
    ...
    bool flag_bot = flag_surf && (option == -1 || option == -2);
    if (st_surface_identify(
          verbOption, surf2D, name_bot, flag_bot, &iatt_bot, name_top, flag_top,
          &iatt_top, String(), &iaux_top, String(), &iaux_bot))
      return def_grid;
    ...
    if (st_readFileHeader(file, verbOption, &nPerTrace, &delta))
      return def_grid;
    nz = nPerTrace;
    z0 = -delta * nPerTrace;
    if (option == -2 && !isNA(nz_ss)) nz = nz_ss;
    ...
    const NamingConvention& namconv)
  {
    ...
    Id iline, xline, nbvalues, iatt;
    Id rank, iatt_top = 0, iatt_bot = 0, iaux_top = 0, iaux_bot = 0;
    RefPt refpt[3];
    RefStats refstats;
    VectorDouble values, cotes, writes;
