  ...
  Id FileVTK::writeInFile()
  {
    ...
    if (flag_grid)
    {
      ...
      if (dims[0] > 1)
        for (Id i = 0; i < dims[0]; i++)
          xcoor[i] = static_cast<float>(
            _factx * (_dbgrid->getX0(0) + i * _dbgrid->getDX(0)));
      ycoor.resize(dims[1]);
      ycoor[0] = 0.;
      if (dims[1] > 1)
        for (Id i = 0; i < dims[1]; i++)
          ycoor[i] = static_cast<float>(
            _facty * (_dbgrid->getX0(1) + i * _dbgrid->getDX(1)));
      zcoor.resize(dims[2]);
      zcoor[0] = 0.;
      if (dims[2] > 1)
        for (Id i = 0; i < dims[2]; i++)
          zcoor[i] = static_cast<float>(
            _factz * (_dbgrid->getX0(2) + i * _dbgrid->getDX(2)));
    }
    else
    {
    ...
    if (!flag_grid)
    {
      ...
      for (Id iech = 0; iech < nech; iech++)
      {
        ...
        for (Id idim = 0; idim < 3; idim++)
        {
          ...
          points[ecr++] =
            (idim < ndim)
              ? static_cast<float>(fact * _db->getCoordinate(iech, idim))
              : 0.;
        }
      }
    }
    ...
    if (flag_grid)
      write_rectilinear_mesh(
        getFilename().c_str(), _flagBinary, dims, xcoor.data(), ycoor.data(),
        zcoor.data(), ncol, vardim.data(), center.data(), vc.data(), tab);
    else
      write_point_mesh(
        getFilename().c_str(), _flagBinary, nactive, points.data(), ncol,
        vardim.data(), vc.data(), tab);
