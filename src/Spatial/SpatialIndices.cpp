...
namespace gstlrn
{
   ...
   **
   *****************************************************************************/
  bool SpatialIndices::_discardData(
    bool flag_w,
    Id iech,
    const String& name,
    VectorDouble& coor,
    ...
    *value = 1.;
    if (!name.empty())
    {
      *value = _db->getValue(name, iech);
      ...
      {
        messerr(
          "The variable cannot be negative (Sample %d = %lf)", iech + 1,
          *value);
    ...
    *weight = 1.;
    if (flag_w)
    {
      *weight = _db->getWeight(iech);
      ...
      {
        messerr(
          "The weight cannot be negative (Sample %d = %lf)", iech + 1, *weight);
        messerr("Procedure is interrupted");
        return 1;
      }
    }
  ...
  Id SpatialIndices::computeCGI(const String& name)
  {
    ...
    for (Id idim = 0; idim < ndim; idim++) _center[idim] /= _wztot;
    ...
    for (Id iech = 0; iech < nech; iech++)
    {
      if (_discardData(flag_w, iech, name, coor, &value, &weight, &wvalue))
        continue;
      for (Id idim = 0; idim < ndim; idim++) coor[idim] -= _center[idim];
      for (Id idim = 0; idim < ndim; idim++)
      {
        _inertia += wvalue * coor[idim] * coor[idim];
        for (Id jdim = 0; jdim <= idim; jdim++)
          mm.updValue(
            idim, jdim, EOperator::ADD, wvalue * coor[idim] * coor[jdim]);
      }
    }
    ...
    auto eigenvectors = EigenVectors(mm);
    _mvalues = eigenvectors.getEigenValues();
    _mvectors = eigenvectors.getEigenVectors();
  ...
  {
    if (computeCGI(name1) != 0) return TEST;
    VectorDouble center1 = getCenter();
    double inertia1 = getInertia();
    if (computeCGI(name2) != 0) return TEST;
  ...
  MatrixDense SpatialIndices::getMatrixEllipse() const
  {
    ...
    axes.setValue(
      0, 1, _center[1] + sy1 * sqrt(_inertia / (K * (1. + (1. / e2)))));
    axes.setValue(1, 0, 2. * _center[0] - axes.getValue(0, 0));
    axes.setValue(1, 1, 2. * _center[1] - axes.getValue(0, 1));
    axes.setValue(
      2, 0, _center[0] + sx2 * sqrt(_inertia / (K * r * (1. + (1. / e2)))));
    axes.setValue(
      2, 1, _center[1] + sy2 * sqrt(_inertia / (K * r * (1. + e2))));
  ...
  MatrixDense SpatialIndices::getMatrixInertia() const
  {
    ...
    axes.setValue(
      0, 1, _center[1] + sy1 * sqrt((r1 * _inertia) / (1. + (1. / e2))));
    axes.setValue(1, 0, 2. * _center[0] - axes.getValue(0, 0));
    axes.setValue(1, 1, 2. * _center[1] - axes.getValue(0, 1));
    axes.setValue(
      2, 0, _center[0] + sx2 * sqrt(((1. - r1) * _inertia) / (1. + (1. / e2))));
    axes.setValue(
      2, 1, _center[1] + sy2 * sqrt(((1. - r1) * _inertia) / (1. + e2)));
    axes.setValue(3, 0, 2. * _center[0] - axes.getValue(2, 0));
    axes.setValue(3, 1, 2. * _center[1] - axes.getValue(2, 1));
    return axes;
  }
  ...
  String SpatialIndices::toString(const AStringFormat* strfmt) const
  {
    ...
    if (!FFFF(_inertia)) sstr << "Inertia = " << _inertia << std::endl;
    if (!FFFF(_iso)) sstr << "Isotropy = " << _iso << std::endl;
    ...
    Id ndisc)
  {
    ...
    for (Id iech = 0, nech = _db->getNSample(); iech < nech; iech++)
    {
      if (_discardData(flag_w, iech, name, coor, &value, &weight, &wvalue))
        continue;
      number++;
      if (coor[0] < xmin) xmin = coor[0];
      if (coor[0] > xmax) xmax = coor[0];
      if (coor[1] < ymin) ymin = coor[1];
      if (coor[1] > ymax) ymax = coor[1];
    }
    ...
    if (polygon != nullptr) db_polygon(grid, polygon);
  ...
  static void _updateGravityCenter(
  ...
  static SpacePoint _calculateGlobalGravityCenter(
    const VectorDouble& xxs,
    const VectorDouble& yys,
    const VectorDouble& zzs,
    const VectorDouble& wws,
  ...
  std::vector<SpacePoint>
    SpatialIndices::getPatches(const String& name, double Dmin, double Amin)
  {
    std::vector<SpacePoint> centers;
    VectorDouble pa;
    VectorDouble pb;
    ...
    if (nech <= 0) return centers;
    ww.normalizeInPlace(1); // Normalize the weights
    ...
    for (Id jech = 0; jech < nech; jech++)
    {
      ...
      for (Id ic = 0, ncenter = static_cast<Id>(centers.size()); ic < ncenter;
           ic++)
      {
    ...
    double patot;
    double pbtot;
    SpacePoint globalCenter =
      _calculateGlobalGravityCenter(xxs, yys, zzs, wws, &patot, &pbtot);
    pa /= patot;
    pb /= pbtot;
    ...
    message(
      "- Number of patches with abundance > %3.0lf %% = %d\n", Amin, nover);
    message("- Percentage of abundance in these patches = ");
    for (Id ic = 0; ic < ncenter; ic++)
    {
      if (pb[ic] > Amin / 100) message(" %6.3lf", 100. * pb[ic]);
    }
    message("\n");
    message("- Percentage of area in these patches = ");
    for (Id ic = 0; ic < ncenter; ic++)
    {
      if (pb[ic] > Amin / 100) message(" %6.3lf", 100. * pa[ic]);
    }
    message("\n");
