...
namespace gstlrn
{
    ...
    *value = 1.;
    if (!name.empty())
    {
      *value = _db->getValue(name, iech);
      ...
      if ((*value) < 0.)
      {
        messerr("The variable cannot be negative (Sample %d = %lf)",
                iech + 1,
                *value);
    ...
    *weight = 1.;
    if (flag_w)
    {
      *weight = _db->getWeight(iech);
      ...
      if ((*weight) < 0.)
      {
        messerr("The weight cannot be negative (Sample %d = %lf)",
                iech + 1,
                *weight);
        messerr("Procedure is interrupted");
        return 1;
  ...
  MatrixDense SpatialIndices::getMatrixEllipse() const
  {
    ...
    axes.setValue(0,
                  1,
                  _center[1] + sy1 * sqrt(_inertia / (K * (1. + (1. / e2)))));
    axes.setValue(1, 0, 2. * _center[0] - axes.getValue(0, 0));
    axes.setValue(1, 1, 2. * _center[1] - axes.getValue(0, 1));
    axes.setValue(2,
                  0,
                  _center[0]
                    + sx2 * sqrt(_inertia / (K * r * (1. + (1. / e2)))));
    axes.setValue(2,
                  1,
                  _center[1] + sy2 * sqrt(_inertia / (K * r * (1. + e2))));
  ...
  MatrixDense SpatialIndices::getMatrixInertia() const
  {
    ...
    axes.setValue(0,
                  1,
                  _center[1] + sy1 * sqrt((r1 * _inertia) / (1. + (1. / e2))));
    axes.setValue(1, 0, 2. * _center[0] - axes.getValue(0, 0));
    axes.setValue(1, 1, 2. * _center[1] - axes.getValue(0, 1));
    axes.setValue(2,
                  0,
                  _center[0]
                    + sx2 * sqrt(((1. - r1) * _inertia) / (1. + (1. / e2))));
    axes.setValue(2,
                  1,
                  _center[1] + sy2 * sqrt(((1. - r1) * _inertia) / (1. + e2)));
    axes.setValue(3, 0, 2. * _center[0] - axes.getValue(2, 0));
    axes.setValue(3, 1, 2. * _center[1] - axes.getValue(2, 1));
    return axes;
  }
                                           ...
                                           Id ndisc)
  {
    ...
    message("nrow=%d ncol=%d dx=%lf dy=%lf maille=%lf\n",
            ...
            dy,
            maille);
    DbGrid* vmap = db_vmap(grid,
                           0,
                           true,
                           ECalcVario::E_COVARIOGRAM,
                           true,
                           {nlag, nlag},
                           {dx, dy});
