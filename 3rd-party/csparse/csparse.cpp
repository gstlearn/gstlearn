...
/**
 * Print a formatted st_cs_message
 * @param format Output format
 * @param ...    Additional arguments
 */
static void st_cs_message(const char *format, ...)
{
  char str[LONG_SIZE];
  va_list ap;
...
cs* cs_add(const cs *A, const cs *B, double alpha, double beta)
{
  ...
  if (!C || !w || (values && !x))
  {
    st_cs_messerr("Core allocation problem in CSparse Library (%d x %d)", m, n);
...
csn* cs_chol(const cs *A, const css *S)
{
  ...
  if (!L)
  {
    st_cs_messerr("Core allocation problem in CSparse Library (%d x %d)", n, n);
  ...
  for (k = 0; k < n; k++)
  {
    ...
    if (d <= 0)
    {
      st_cs_messerr("Pivot line %d/%d should be positive (d= %lf)", k + 1, n, d);
...
cs* cs_multiply(const cs *A, const cs *B)
{
  ...
  if (!A || !B)
  {
    st_cs_messerr("Both matrices should be defined");
  ...
  for (j = 0; j < n; j++)
  {
    ...
    {
      st_cs_messerr("Out of memory in CSparse Library %d", 2 * (C->nzmax) + m);
...
int cs_print(const cs *A, int brief)
{
  ...
  nzmax = A->nzmax;
  nz = A->nz;
  st_cs_message("CSparse Version %d.%d.%d, %s.  %s\n", CS_VER, CS_SUBVER, CS_SUBSUB,
          CS_DATE, CS_COPYRIGHT);
  if (nz < 0)
  {
    st_cs_message("%d-by-%d, nzmax: %d nnz: %d, 1-norm: %g\n", m, n, nzmax, Ap[n],
            cs_norm(A));
    for (j = 0; j < n; j++)
    {
      st_cs_message("    col %d : locations %d to %d\n", j, Ap[j], Ap[j + 1] - 1);
      for (p = Ap[j]; p < Ap[j + 1]; p++)
      {
        st_cs_message("     %d : %g\n", Ai[p], Ax ? Ax[p] : 1);
        if (brief && p > 20)
  ...
  {
    st_cs_message("triplet: %d-by-%d, nzmax: %d nnz: %d\n", m, n, nzmax, nz);
    for (p = 0; p < nz; p++)
    {
      st_cs_message("     %d %d : %g\n", Ai[p], Ap[p], Ax ? Ax[p] : 1);
      if (brief && p > 20)
...
csn* cs_qr(const cs *A, const css *S)
{
  ...
  if (!R || !V || !Beta)
  {
    st_cs_messerr("Core allocation problem in CSparse Library (%d x %d)", m2, n);
...
cs* cs_transpose(const cs *A, int values)
{
  ...
  if (!C || !w || m <= 0 || n <= 0)
  {
    st_cs_messerr("Problem when transposing a matrix in CSparse Library (%d x %d)", m,
            n);
